...
If reads remain: repoint or delete those consumers if quick; otherwise graveyard now (reversible) and **track the remaining cleanup** in the Open items queue of `../common-docs/operations/db-changeover-board.md` (the tracker that replaced the deleted `docs/db_rebuild/CHANGEOVER_PROGRESS.md`). Do not block the move on a long repoint — but never graveyard a table with live, load-bearing reads you haven't accounted for.
