...
The thunk at `features/agents/redux/execution-system/thunks/launch-agent-execution.thunk.ts` reads `runtime.surfaceName`, fetches the agent's binding layers via `fetchSurfaceBindingLayers` (bindings are `platform.associations` edges read through the `agent.menu_surface` view — written ONLY via `features/surfaces/services/bind-agent-to-surface.service.ts`), merges layers weakest→strongest, applies `value_mappings` via the resolver, and falls back to legacy auto-name-matching for unmapped keys. A documented integration contract or explicit user choice names an outside helper's `named_inputs` or `approved_surface_context`; the latter may be mappings or a task-appropriate snapshot and persists until scope changes. Native subject agents receive only engineered inputs, never an ambient/ad-hoc snapshot. Permanent residents have no automatic inheritance and require their deliberate contract. **Known implementation gap:** the header currently sends full scope and the mapper can deliver unmapped fields, so this documentation does not claim runtime enforcement; verify the effective request and persisted variables. If you skip `surfaceName`, you get the legacy auto-name-match path only — explicit mappings won't apply.
