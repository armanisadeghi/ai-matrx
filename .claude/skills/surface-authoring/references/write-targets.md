...
- **Mirrored registration:** `ui.ui_surface_write_target` is synchronized with the rest of the selected surface mirror. Run the focused sync and matching `--check` from Layer 4 before claiming the target is available to server-side agents. Reference implementation: `content-plan-node.manifest.ts` + `NodePanel.tsx` (10 draft field targets + `save_node`).
