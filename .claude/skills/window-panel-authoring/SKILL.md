...
If this window is an eligible ordinary agent-aware surface, invoke `surface-authoring` and its overlay-surface reference for the manifest, scoped registration receipt, and nested runtime provider. Agent-native windows follow that skill's exclusion boundary.
