...
## 2026-09-16 — the in-app browser pane is shared across sessions

Two cold walks and one fixer saw navigations they did not make (a tab jumping to another lane's rulebook, a `/conduct` page "redirecting" to a plan): the desktop app's browser pane is one pane per machine and several sessions were driving it at once. A wall seen only in that pane is not evidence. Drive trials and walks with a dedicated Playwright context on the session's own preview host, save screenshots from it, and treat the shared pane as read-only when other sessions are active.

## 2026-09-17 — a fix is verified on a brand-new record, with the walker's exact sequence

Walk 3 found the vision-interview Finish broken; the fixer verified its fix on the walk's own old stuck session and reported "live-proven". Walk 4 pressed Finish three times on a fresh session and nothing happened. A fixer verifies on a brand-new record, following the walker's entry point and click sequence exactly, and writes that sequence into the Fixed column; a claim verified on the reporting record alone is not evidence.

## 2026-09-18 — a stale shared preview is a silent failure class; keep it current by refreshing the checkout, never the shared tree

The matrx-frontend `:3001` preview had been serving `.matrx/topical-map-walk/checkout` for ~3h47m (package.json `0.4.2042`, branch `codex/topical-map-walk`) while `origin/main` had moved to `0.4.2050` — every walker/verifier hitting `:3001` in that window was certifying code nobody could see in `origin/main`, and nothing on screen said so (`/api/version` returns `{"deploymentId":null,"commit":null}` locally; it does not name the checkout or its age). The recipe any agent can repeat, never touching the shared working tree and never `git stash`/`git add -A`:
1. **Identify the current owner honestly.** `lsof -i :3001 -sTCP:LISTEN` for the pid, then `lsof -a -p <pid> -d cwd` for the checkout it is actually serving from (not the label `preview:status` prints — that names whoever started it, which drifts). Compare that checkout's `package.json` version and `git log -1` against `git log origin/main -1` in the main checkout.
2. **Confirm it is safe to swap.** Tail the shared dev log (`pnpm preview:status` prints the path, under `${TMPDIR}/matrx-frontend-preview-<uid>/shared-next-dev.log`) and watch the line count for ~60s of real quiet — a mid-run walker keeps writing `GET`/`POST` lines. Only swap once it goes quiet.
3. **Build the fresh checkout as a worktree, never in place.** From the main checkout: `git fetch origin main && git worktree add --detach .matrx/<name>/checkout origin/main` — this leaves the shared tree and every other lane's dirty files untouched. `.env.local` is gitignored and identical across sessions; copy it from any other live checkout rather than reconstructing it (its absence makes every SSR page 500 with `MissingSupabaseIdentityError`, not a helpful "preview is stale" message).
4. **Install only if the lockfile changed** (`md5`/`diff pnpm-lock.yaml` against a known-good checkout) — the install-gate blocks `pnpm install` only while *that checkout's own* preview lease is live, so installing in a brand-new, not-yet-started checkout is always safe and touches nothing another lane depends on.
5. **Stop the stale preview from its owning checkout** (`pnpm preview:stop`, which preserves `.next-preview` cache), then **start the fresh one from the new checkout** with `MATRX_PREVIEW_SESSION=<name> pnpm preview:start` — never `preview_start`/raw `pnpm dev`, which create untracked server trees the harness cannot manage.
6. **Verify against reality, not the label.** `git log -1` in the checkout plus `package.json` version prove what SHOULD be served; then actually curl it — `/`, `pnpm dev-login /<route>` followed by `/api/whoami` (confirms `admin@admin.com`, not just a 200), and the routes in question — because a version string and a working app are two different claims.

The API-target-Production toggle is a per-browser Redux-persisted default (`lib/redux/slices/apiConfigSlice.ts`, `activeServer: "production"`), not an env var or launch flag — nothing to set at preview-start time as long as no prior session flipped it to a non-default value in that same cookie jar.

## 2026-09-18 — Ad hoc SQL on the shared database starves the whole platform

An agent's diagnostic query (`insert into post_ids select … count(*), md5(string_agg(id::text,',')) from files.files`) ran under the `postgres` role, which has no statement ceiling, held an exclusive lock on `files.files` for 130 s and starved the shared pool platform-wide (rulebook creation 500s, messaging error storm) — the 2026-09-15 lock-storm class again. Rule: any ad hoc SQL an agent runs through the Supabase MCP or a script must begin with `SET LOCAL statement_timeout = '5s'` (and `lock_timeout = '2s'`) inside a transaction, must read only (`select`) unless it is a rehearsed migration, and must never create scratch tables in a production schema. Heavy reads use a worktree server with a ≤3-connection pool, never the pooler's `postgres` role. The permanent fix landed 2026-09-19 (board row Z9, aidream migration `0919_postgres_role_statement_and_lock_ceilings.sql`): the `postgres` role now carries `statement_timeout = '30s'` and `lock_timeout = '5s'`, and every legitimately long job raises its own with `SET LOCAL` inside its transaction. The rule above still stands and is now also a tool — `python db/adhoc_sql.py "select …"` in aidream is read-only and bounded at 5s/2s by default. Two things measured while fixing it, both easy to get wrong: the role was never actually unbounded (it inherited the cluster's 2min — the missing half was `lock_timeout = 0`, an unbounded WAIT), and `ALTER FUNCTION f SET statement_timeout` does NOT give a long function headroom, because the timer is armed when the caller's statement starts.

## 2026-09-18 — "admin-owned" means `created_by`, never the organization

A lane attached 11 sample test cases to an agent because its organization matched the test org; the agent belonged to Arman's personal account. Organization is tenancy; ownership is `created_by`/owner. Before writing anything onto an existing record in a test, read its owner column and refuse unless it is `admin@admin.com`'s user id — or create a fresh disposable record instead. Cleanup: delete only the rows you created (filter on your own provenance marker), verify the count returns to the prior value, and confirm the record's `updated_at` never moved.

## 2026-09-19 — a hard-linked shim breaks silently when its copy lands at a different nesting depth

Starting the shared preview from a fresh worktree at `.matrx/acquisition-frontier/checkout` crashed with `Error: Cannot find module '/Users/…/matrx-frontend/Users/…/matrx-frontend/node_modules/next/dist/bin/next'` — a doubled absolute path, seen twice by an independent verifier. Root cause: `matrx-frontend/scripts/agent-dev-server.sh` launched Next.js by exec'ing `node_modules/.bin/next`, a pnpm-generated POSIX shim whose body finds its real target by counting a FIXED number of `../` hops from its own directory up to what it assumes is the filesystem root, then re-descending through the target's absolute path baked in at generation time (leading slash stripped). `ensure_worktree_node_modules` seeds a worktree's `node_modules` with a same-device hard-link copy (`cp -al`) of the primary checkout's tree — same bytes, same baked-in hop count — but the worktree lived five directories deeper than the primary checkout, so the hop count stopped five directories short of the real root and the shim silently doubled the primary checkout's own path onto itself instead of erroring. This is not specific to one hop count: ANY shim (or any script whose correctness depends on its own path depth) that is hard-linked or otherwise byte-copied to a directory nested at a different depth than where it was generated resolves to the wrong target, silently — no exit code, no message, just a crash three commands later that names a path nobody wrote. The fix (`resolve_next_bin()` in `scripts/agent-dev-server.sh`) stopped trusting the shim's arithmetic entirely and pointed straight at `node_modules/next/dist/bin/next`, a real Node script reachable through the `node_modules/next` symlink into the pnpm store — a symlink resolves correctly regardless of nesting depth, because it carries no baked-in hop count. Guard: `pnpm test:worktree-next-bin-resolution`. Compounding failure that made this dangerous rather than merely broken: while the shared preview was crash-looping from the stale worktree, port 3001 was still being served by the PRIMARY checkout, which was dirty (48 modified files) and behind `origin/main` — every walker hitting `:3001` in that window was certifying code nobody could see, the same silent-failure class as the 2026-09-18 stale-shared-preview entry above, but from the opposite direction: there the shared tree was stale relative to `origin/main`; here the worktree meant to replace it couldn't even start, so nobody noticed the primary was still the one actually answering requests.

## 2026-09-19 — a documented "admin@admin.com user id" was never checked against a live login; it was Arman's personal account

A brief to set `admin@admin.com`'s default organization (root-caused in `common-docs/projects/acquisition-frontier/screen-reverify-2026-09-19/prod/README-prod.md`) named the account by `user_id=4cf62e4e-2679-484f-b652-034e697418df`. Minting a real dev-login session for that exact id returned Supabase auth data for `arman@armansadeghi.com` — full name "Arman Sadeghi", personal org named "Arman's Org", Google/GitHub providers — Arman's own personal account, not the disposable test account. The real `admin@admin.com` is `user_id=87a6e699-3622-4869-8843-d0867456c0dd` (confirmed from a cached magic-link session file, `email` field literally `admin@admin.com`), and that account's `default_organization_id` was already **valid** (`7cd12da2-2213-4378-8fba-a9e2dc4ea657`, "Castellano & Reyes, LLP") — the missing-default-org root cause the doc described does not exist for the real account. Nobody had wrongly written anything yet; the mix-up was caught before any preference write, and no write was made to either account under this finding. The rule: never trust a documented "user X's id is `<uuid>`" claim, however confidently a prior root-cause doc states it — mint a real session (dev-login, cached magic-link file, or `/auth/organizations` read) for that exact id and read back its own email/org names before writing anything to it. An id is not an identity until a live credential says so, and `admin@admin.com` is verified by its email, never by an id carried forward from an earlier session's notes. `README-prod.md` and the row's owner should re-run the original defect against the correct id before assuming it still exists.

## 2026-09-19 — a typed refusal with its own remedy sentence, flattened by 20 separate catch-alls

`cloud_browser` navigate/list_profiles calls all returned a bare, undiagnosed `internal_error` for
any account whose MCP connection named no organization — fixed same-day (commit `f68db152f4`) by
mapping the boundary's `MissingMcpOrganization` (raised in `context_edge.py`, always carrying a
full, safe, platform-authored remedy sentence) to a public `organization_required` error inside
`browser_tools.py`'s own exception handler. Hours later the `errors` tool (`error_surface_tools.py`)
hit the exact same boundary for the exact same reason and returned its own bare
`{"type":"execution","message":"Error-surface action failed. Retry or inspect server status."}` —
the fix had been applied to one tool, not the class. A census of the other ~19 MCP tool modules in
`aidream/api/mcp/agent_service/` found the SAME defect in two different shapes: half had a generic
`except Exception:` that discarded the remedy exactly like `errors` did (`system_error_tools.py` x3,
`work_loop_tools.py`, `workflow_tools.py`, `conversation_tools.py`); the other half
(`claude_asset_tools.py`, `coding_session_tools.py`, `debug_trace_tools.py`, `google_tools.py`,
`provision_tools.py`, `question_desk_tools.py`, `schedule_claim_tools.py`, `storage_source_tools.py`,
and three functions in `tools.py`) had **no exception handling at all** around the boundary, so
FastMCP's own `Tool.run` wrapped it as `ToolError(f"Error executing tool {name}: {e}")` — the real
sentence leaked, but as an unstructured string invisible to any client parsing
`structuredContent.error.type`, and indistinguishable in shape from every other uncaught crash.
Fixed at ONE seam instead of a 20th copy: `context_edge.organization_required_envelope(exc)` returns
the shared `{"type": "organization_required", "message": str(exc)}` pair (safe because
`MissingMcpOrganization`'s message is always platform-authored, never caller-supplied free text),
and every tool's exception handling — adding one where none existed — now routes through it before
falling through to a generic catch-all. Two tools (`server_status`, which only reads platform-wide
worker liveness) were found to need no organization AT ALL and were re-pointed at
`mcp_organization_free_context` — for those the failure mode is now structurally impossible, not
merely handled. The rule: **when a typed refusal at a shared boundary gets a per-call-site fix,
that fix is not "done" until every other call site is censused** — a fix scoped to the symptom that
happened to page someone first (the busiest tool) leaves the same live outage in every quieter one,
discoverable only by a customer hitting them one at a time. A guard belongs at the boundary too: a
parametrized test over every registered tool that resolves an organization must assert an org-less
caller gets the named refusal, never a flattened generic error and never an uncaught exception.

## 2026-09-19 — two context systems that only a host joins: an ambient read is a dependency on a call nobody can see

Every upload to `files.matrxserver.com` returned `400 … this write carries no organization` while the browser sent a correct, membership-VERIFIED `X-Organization-Id`. `AuthMiddleware` pins the verified organization on the **matrx-connect** `AppContext`; the file write funnel read it from the **matrx-utils** ambient context; the two are joined only when a host calls `matrx_utils.configure_context`. `aidream` calls it; the matrx-files **standalone** app — the service the browser actually uploads to — never did, so the ambient read answered `None` for every request that service had ever served. The failure had also been invisible for as long as the service existed: until the refusal landed (2026-09-17), the same blind read fell through to "the owner's personal workspace", so a header the caller did send was silently discarded and the file filed in the wrong tenant. The rule: **a request's scope (organization, tenant, actor) is resolved ONCE at the API edge, from the admitted request, and threaded explicitly to whoever needs it.** A deep site that reaches for an ambient context — or for a key in a `metadata` dict some upstream layer was supposed to fill — has a hidden dependency on a call in a host it cannot see, and it fails per-host, at request time, with a message about the header the caller *did* send. Two cheap correctness moves fall out of it: a **boot guard** (a process with no scope seam at all refuses to start rather than letting the first person to drag a file discover it), and *not* re-verifying the admitted value as if it were a fresh claim — it was already verified at admission, so threading it must cost nothing extra. Diagnostic note: the 400-not-403 was the whole tell. A denied, malformed or unverifiable organization is answered by the auth boundary *before* route code runs; a refusal from inside the write path means the value was admitted and then lost between two layers, which is a seam, never a permission.
