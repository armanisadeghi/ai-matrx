...
description: Wire an existing UI surface into the agent context system end-to-end — declare all generic + custom Surface Values, emit them at runtime with no unnecessary refetches, mount the canonical v3 context menu (`EditableContextMenu` / `NonEditableContextMenu`) on BOTH editable and presentational regions, and replace every plain `<textarea>` / `<input>` with the auto-wired `ProTextarea` / `ProInput`. Use whenever the task is "wire surface X", "add the context menu to X", "make X's inputs Pro", "roll out surface agents to X", or finishing a surface that already has a manifest/initial values. Builds on `surface-authoring` (the manifest/DB contract) — read that first. NOT for creating a brand-new surface from nothing (do `surface-authoring` first), nor for the context-menu component internals (that's `features/context-menu-v3/FEATURE.md`).
---
...
> **Prereq:** read `.claude/skills/surface-authoring/SKILL.md` (the manifest + `ui_surface_value` + naming contract). This skill is the consumer/runtime layer on top of it. Menu mount mechanics: the `context-menu-v3` skill.
>
...
> - Live wiring example: `app/(dev)/demos/context-menu/canonical` (every panel is a real v3 wiring)

---

## Two hard rules (this is the whole point — read before anything else)

**1. ONE canonical menu everywhere — never a bespoke fork.** Every surface mounts the SAME v3 menu: `EditableContextMenu` / `NonEditableContextMenu` from `features/context-menu-v3/`. Do NOT build, keep, or wire a surface-specific menu component or a parallel data hook. `/notes` WAS the cautionary tale: its live editor once rendered a bespoke `NoteContextMenuContent` fed by its own Supabase fetch + cache slice, so its core items behaved DIFFERENTLY from chat's canonical menu; it was migrated 2026-06-23 and the bespoke chain deleted. (The interim v2 `UnifiedAgentContextMenu` was itself deleted 2026-07-19 — every consumer now renders v3.) Migrating a surface = ripping out its bespoke menu and mounting the canonical one. Two menus that "should be the same" but aren't is the #1 defect this rollout exists to kill.

**2. The menu is the surface's command center — pass it EVERYTHING.** Every action the surface can perform (save, export, share, move, delete, rename, duplicate, toggle-complete, version-history, …) AND every piece of live state must reach the menu. Wire actions via `extraSections` bound to the surface's REAL handlers (NEVER toast stubs — a stub that fakes "Delete" is worse than nothing), and state via `getApplicationScope` / `contextData`. If a surface has a method, the menu gets it. The goal: a user right-clicks anywhere on a surface and can do anything that surface supports, plus run any agent — from one menu that looks and behaves identically everywhere.
...
3. **`buildApplicationScopeFromMenuContext`** (`features/context-menu-v3/utils/build-application-scope.ts`) already guarantees `selection` / `text_before` / `text_after` / `content` / `context` from the live DOM. Always build the menu's scope through it. The menu additionally self-resolves `content` from the wrapped DOM when a surface supplies nothing (the no-fake-menu floor) — that floor is a fallback, not your emit path.
...
- **`<XXX>_CONTEXT_MENU_PROPS`** — `{ sourceFeature, surfaceName: "<client>/<local>" }` (+ optional `placementMode`). `surfaceName` MUST equal `ui_surface.name`. (`enabledPlacements` and `isEditable` are dead — editability is the wrapper choice, placements are `placementMode`.)
- **`create<Xxx>ExtraSections(handlers)`** — NOT optional when the surface has actions (rule 2). Every surface action (Save, Export, Share, Move, Delete, Rename, version-history, …) as `ContextMenuExtraSection[]` (type from `@/features/context-menu-v3/types`), each `onSelect` bound to the surface's REAL handler passed in by the host — never a toast stub. The host (the component that owns the handlers + state) builds the sections with its live callbacks and passes them to the menu via `extraSections`. See `features/notes/agent-context/notesEditorExtraSections`.
...
### 4. Mount the menu on EVERY region — two wrappers

**Import the wrappers statically** — they are v3's lightweight shell; the heavy layer (`MenuContent`) lazy-loads behind the shell's own `next/dynamic({ ssr: false })` boundary on first open (eslint-enforced: `contextMenuV3StaticImportBan`). Never add a per-consumer `dynamic()` around the wrapper — that re-adds a loading fallback and a layout-collapse null state for zero benefit. (History: the v2 menu had no internal split, so a static import of it once ballooned the prod build 15 → 24 min; v3's T0/T1 split is the permanent fix.)
...
- **Editable region** (textarea/input/editor): wrap in `<EditableContextMenu {...PROPS} getTextarea getApplicationScope contextData onTextReplace onTextInsertBefore onTextInsertAfter extraSections>` and put a **`ProTextarea` / `ProInput`** inside.
- **Presentational region** (rendered markdown, a transcript, a result the user reads): wrap in `<NonEditableContextMenu {...PROPS} getApplicationScope contextData>` — right-click offers agent actions on the displayed text; no text-replace callbacks (read-only). This is the "assist the user with information presented to them" half — do not skip it.

Also pass **`contentSource`** (rich-document source — unlocks Copy-as / Export / Download as Markdown / Convert) and **`entity`** (`{type,id,title,resourceType?,isOwner?}` — unlocks Attach To + Share) whenever the surface has a real entity. See the `context-menu-v3` skill.
...
- `<input type=text>` → `<ProInput>` (`@/components/official/ProInput`). **`ProInput` does NOT accept `surfaceName` / `getApplicationScope`** (those are ProTextarea-only) — a single-line input gains agent access only from wrapping it (or its region) in the context menu. Passing them to `ProInput` is a type error.
- Schema-bound fields (Entity/Settings): build a thin wrapper that renders Pro — never re-implement voice/copy/submit.
...
- Bound agents + the unified menu load on menu OPEN only (the menu/Pro components already do this, with dedup + inflight guards) — don't prefetch per keystroke.
...
- [ ] Menu is the **canonical** v3 wrapper (footer shows `v3.n · V<n>`), NOT a bespoke fork. Mounted on BOTH editable and presentational regions; `surfaceName` correct.
- [ ] Wrappers imported statically (no per-consumer `dynamic()`); no static import of `MenuContent`/`MobileMenuContent`.
- [ ] EVERY surface action is in `extraSections` wired to a REAL handler (no toast stubs); all live state reaches the menu; `contentSource`/`entity` passed where the surface has a real entity.
- [ ] Open the menu and watch the dev console — v3 SCREAMS on an inert menu or a dropped declared value; a scream is a wiring defect, fix it.
