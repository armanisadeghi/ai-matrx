...
The one-pass recipe (backfill → capture → repoint the legacy version RPCs → graveyard), its gotchas, and the two live CRACKS in the canonical version system. **Entity carries a per-entity `*_versions` table (or the Prime Directive versioning question above was answered "retire it") → read [retire-versions-table.md](retire-versions-table.md) before Step 5.**
