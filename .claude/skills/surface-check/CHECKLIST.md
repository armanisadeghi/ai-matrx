...
- MUST (**THE DISCLOSURE LAW — invoke `agent-disclosure`**): every fixed agent this surface runs as a job for the user is NAMED in two places — an inline `<PageAgents agents={[{mandateKey, does}]} surfaceName>` chip where the agent acts, and a manifest `agentRole` carrying `mandateKey` (never `defaultAgentId`, never a raw UUID). `pnpm check:agent-disclosure` must not name this surface's files. Agent-AUTHORING surfaces disclose nothing because the agent is the subject; universal agent hosts such as Chat disclose no available-agent roster because the user may choose any agent and none is a bound surface worker.
