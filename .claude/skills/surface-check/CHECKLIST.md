...
## S5 · Agent context contracts — `agent-disclosure` + `surface-authoring` + `features/agents/components/chat/FEATURE.md` §self-referential loop

- MUST (**THE DISCLOSURE LAW — invoke `agent-disclosure`**): register every fixed AI job this surface ALREADY runs in the existing top Agents menu through a manifest `agentRole` carrying `mandateKey` or UI-free `useDeclaredSurfaceMandates` when live state selects the job. **Certification must never add agent chips, badges, cards, rows, labels, rosters, callouts, copy, icons, or sections to visible page content.** A disclosure-only page-content change is an automatic FAIL. Never invent an AI integration to make S5 pass. `pnpm check:agent-disclosure` must not name this surface's files.
- MUST: an outside helper may bind to any surface, including Chat and Agent Builder. Its data delivery follows an explicit context contract: record agent/job, surface, purpose, mode (`none`, `named_inputs`, or `approved_surface_context`), allowed names or approved snapshot, approval provenance, and evidence. A documented integration contract or explicit user choice persists until scope changes; approved surface context may be mappings or a task-appropriate page snapshot. Binding availability and the data handoff are separate decisions. This is required evidence, not a claim that a unified tracker exists.
- MUST: when an approved outside helper's purpose is agent definition work, it
  may edit all user-editable definition fields, enter runtime variables and user
  input, and run the test. Enumerate the actual installed targets and evidence;
  do not represent unavailable controls or unbuilt runtime enforcement as live.
- MUST: the native subject agent receives only its engineered inputs and never ambient/ad-hoc surface values. Every permanent resident helper, including a window panel, receives no automatic inheritance; its deliberate contract may use named inputs or approved surface context when it works on page data. Test native isolation and each resident's chosen contract.
- MUST: a mandate is opened IN PLACE. Any `<Link>` from this surface to `/mandates` or `/administration/mandates` is replaced by `useOpenMandateWindow()`; the routes stay for browsing all 365.
- MUST: inventory every button, menu item, assist, automatic action, panel, and launch path that starts a named agent or Mandate. Classify each as outside helper, native subject agent, or permanent resident; recurring fixed jobs appear in the header roster only when they already exist.
- MUST: inspect the newest 5–6 runs for each outside helper. When the page already holds repeated discovery facts, declare and emit granular Surface Values, add matching optional agent variables, and map only the approved names. Do not hide structured context in `user_input`; do not add one junk-drawer context variable when stable fields deserve names.
- MUST: mappings are exact and current: every destination is a declared agent variable or context-policy name; every `surface_value` target exists in this surface's resolved manifest and is emitted on the route/tab where the action runs. Bind through the UI/service (`upsertAgentSurfaceBinding` / `assoc_add`), never a hand-written edge insert. Run `pnpm check:surface-impact <surface>` before and after changing value vocabulary.
- MUST: embedded child components do not steal the host surface identity. Open every tab/mode containing its own `SurfaceRuntimeProvider`; the global header must still name the product surface whose agents the user expects, unless the child is deliberately a first-class nested surface. Host-mode components publish only the child state named in the helper's approved contract.
- Verify: open "Agents for this page" on the exact action-bearing tab and confirm each existing fixed job appears once. Run an outside helper with deliberately non-matching mapped names or an approved snapshot, then run the native subject agent and permanent residents to prove native isolation and the residents' deliberate contracts. **Known gap:** header full-scope delivery and unmapped-field mapper delivery are not this policy's enforcement; this documentation requires evidence and does not claim that a runtime guard exists.

- MUST: launch paths that bypass `launchAgentExecution` (own context assembly, e.g. War Room) are inspected by hand: the agent must never receive its own transcript/definition/run evidence as ad-hoc surface context.
...
- Evidence: launch inventory; header roster for existing fixed jobs; one record per invocation with agent/job, surface, purpose, mode, allowed names, approval provenance, and durable evidence; outside-helper mapping proof; native and resident isolation proof.
...
- 🚨 MUST — **THE DENSITY LAW** (Arman, 2026-08-25): a menu item is a short verb phrase and carries **NO `description`/subtext**. The ONE exception is a `disabled` item, whose `description` says why it is off and where it works. "If macOS would not put it in a menu, neither do we." Any other description needs Arman's explicit approval, requested AFTER the menu ships bare (~95% are refused). Machine-checked: `pnpm check:context-menu --population=density`.
- 🚨 MUST — **CHECK THE SHARED-SECTION REGISTRY BEFORE WRITING `extraSections`**: [`features/context-menu-v3/SECTIONS.md`](../../../features/context-menu-v3/SECTIONS.md). If the right-clicked identity (keyword, class, level, page, CRM row, …) already has a registered builder, USE it and GROW it with the actions this surface makes obvious — every prior consumer gains them. If the identity appears on 2+ surfaces with no builder, extract and register one. Only a genuinely page-local identity gets inline items. An action that cannot work here stays VISIBLE but `disabled` with the reason naming where it does work (`unavailableHere`), never absent — the menu is identical everywhere; only availability changes.
- Check: `pnpm check:context-menu` — the surface must not appear in the `tables`/`editables`/`windows` populations, and if it mounts a menu it must not be graded a SHELL.
...
- MUST: the menu's last entry shows THIS surface's label. Expose the appropriate location / Surface Context / Agents / Bind rows only where the existing product and approved contract make them applicable; do not add rows as disclosure decoration. A wrong or "This page" label means S1 is wrong.
...
- MUST (**FIXED-ASSISTANT CLEARANCE**): every fixed assistant or bottom control identifies the element that actually scrolls the terminal meaningful content. Reserve clearance on that owner, not an outer full-height route wrapper. If editor, preview, split panes, or tabs own independent scrolling, every active terminal owner clears the fixed control and removes its clearance on unmount/mode change. A second scroll region, clipped ending, or outer padding that shrinks an `h-full` child is a fail.
- MUST: live geometry proves the final meaningful element can be scrolled completely above the fixed control at desktop, intermediate, and narrow widths. Exercise every content mode and wait for transitions/keyboard effects; a clean route load or the presence of a padding class is not proof. Mobile behavior must be explicit and reachable rather than silently suppressing the assistant.
