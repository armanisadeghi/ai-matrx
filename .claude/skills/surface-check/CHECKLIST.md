...
- Verify: for a fixed surface job, open "Agents for this page" on the exact action-bearing tab and confirm it appears once. For a universal host, confirm the available-agent roster is absent. Trigger one safe representative fixed-job run when one exists and compare it with the baseline 5–6: already-mapped facts cause zero rediscovery-only calls. Record baseline to after counts and any remaining calls required for writes or fresh identifiers.
