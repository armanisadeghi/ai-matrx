...
- MUST (**THE DISCLOSURE LAW — invoke `agent-disclosure`**): every agent this surface runs for the user is NAMED in two places — an inline `<PageAgents agents={[{mandateKey, does}]} surfaceName>` chip where the agent acts, and a manifest `agentRole` carrying `mandateKey` (never `defaultAgentId`, never a raw UUID). `pnpm check:agent-disclosure` must not name this surface's files. The INVERSE holds on an agent-AUTHORING surface (builder, mandate console, agent settings): it discloses nothing — the agent there is the subject, not the worker.
