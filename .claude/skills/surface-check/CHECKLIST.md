...
## S5 · Agent parity & agent-purpose boundary — `agent-disclosure` + `surface-authoring` + `features/agents/components/chat/FEATURE.md` §self-referential loop

- MUST (**THE DISCLOSURE LAW — invoke `agent-disclosure`**): register every fixed AI job this ordinary product surface ALREADY runs in the existing top Agents menu through a manifest `agentRole` carrying `mandateKey` or UI-free `useDeclaredSurfaceMandates` when live state selects the job. **Certification must never add agent chips, badges, cards, rows, labels, rosters, callouts, copy, icons, or sections to visible page content.** A disclosure-only page-content change is an automatic FAIL. Never invent an AI integration to make S5 pass. `pnpm check:agent-disclosure` must not name this surface's files.
- MUST (**AGENT-NATIVE EXCLUSION**): Chat, Agents Hub, Agent Apps, Agent Build/Builder, Agent Run/Runner/history, Agent Battle/comparison, mandate authoring/settings, and agent/widget test harnesses have no surface roles, defaults, bindings, bound roster, or Bind control. Their agent UI is the product itself, never disclosure. Rolling/live-UI fleets skip them; a direct check records this branch as `na` and does not author surface bindings.
...
- MUST: on an eligible ordinary product surface, inventory every button, menu item, assist, or automatic action that launches a named agent or Mandate. Each recurring fixed surface job appears in the header roster through a manifest `agentRole` and/or a canonical surface binding. Agent-native hosts are excluded wholesale; no embedded fixed job makes them eligible.
- MUST (eligible ordinary surfaces only): inspect the newest 5–6 runs for each recurring surface agent. Count read-only discovery calls and group them by repeated tool plus returned fact. If the page already holds those facts, declare and emit granular Surface Values, add matching optional agent variables, and map them on the canonical binding. Do not hide structured context in `user_input`; do not add one junk-drawer context variable when stable fields deserve names.
- MUST (eligible ordinary surfaces only): binding mappings are exact and current: every destination is a declared agent variable or context-policy name; every `surface_value` target exists in this surface's resolved manifest and is emitted on the route/tab where the action runs. Bind through the UI/service (`upsertAgentSurfaceBinding` / `assoc_add`), never a hand-written edge insert. Run `pnpm check:surface-impact <surface>` before and after changing value vocabulary.
- MUST (eligible ordinary surfaces only): embedded child components do not steal the host surface identity. Open every tab/mode containing its own `SurfaceRuntimeProvider`; the global header must still name the product surface whose agents the user expects, unless the child is deliberately a first-class nested surface. Host-mode components must publish the child state their bound agent needs.
- Verify: on an eligible ordinary surface, open "Agents for this page" on the exact action-bearing tab and confirm its fixed job appears once, then compare one safe run with the 5–6-run baseline. On an agent-native surface, verify only explicit `surfaceName: null`, zero roles/bindings/Bind/disclosure additions, and no subject agent in page disclosure.
...
- MUST: the `SurfaceRuntimeProvider` may stay mounted on such pages for neutral context-menu/runtime identity, but it must expose no surface roles, bindings, bound roster, or Bind control. Every primary agent conversation opts out.
...
- Evidence: eligible ordinary surface — launch inventory, header roster, mappings, and 5–6-run baseline; agent-native — opt-out line(s) `file:line` plus zero roles/bindings/Bind/disclosure additions.
...
- MUST: the menu's last entry shows THIS surface's label. Eligible ordinary surfaces expose the appropriate location / Surface Context / Agents / Bind rows; agent-native surfaces must omit Agents and Bind. A wrong or "This page" label means S1 is wrong.
