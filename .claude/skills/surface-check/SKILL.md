...
- *"When a surface passes, then we know it absolutely meets all of these things."* Every section gets a verdict; "didn't look" is a fail.
- *"These are agents who are gonna be empowered to also fix and improve and enhance."* You fix. A finding without a fix is only acceptable when the fix is an ARMAN-class decision (product semantics) — then you log a chip / review row and keep going.
- *"If they come back to me with questions, that just kills the whole thing."* The checklist's DECIDE rules exist so you never ask. Decide by the rule, log the decision, move on.
- *"Have a way to know which ones haven't been checked in a long time or have never been checked."* Every run ends by writing the ledger row. An unlogged check did not happen.
