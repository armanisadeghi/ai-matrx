...
**Direction doctrine: little points to big — content/child = SOURCE, container = TARGET** (a task points to its project). `container_side='target'` is the norm; `'source'` = deliberately-inverted stored edge, documented in `notes`. The registry is the single truth of direction: `trg_associations_auto_orient` (BEFORE INSERT) **loudly flips** any write whose reverse pair is the registered one (`RAISE WARNING` + `metadata.__auto_oriented`), and `admin_relationship_rules().reverse_edge_count` flags wrong-way data in the UI. Never write an edge big→little "because the caller has the container handy" — flip the args (FE precedent: `REVERSED_ASSIGNMENT_TYPES` in `features/war-room/service/associations.ts`, where a project assigned to a room is stored `war_room→project`). Two-way pairs exist only when BOTH directions are registered active, by design.
