...
| Apply DDL | **Arman's preference (2026-09-18): directly through the Supabase MCP** — the database is the source of truth and type generation pulls from it. A migration FILE is fine only if you OWN it end to end (write, apply, regenerate, confirm nothing broke); then its ONE apply path is **`pnpm db:apply migrations/<file>.sql`** (whole file, one transaction, ledgers what executed). Never mix the two (a file's SQL hand-run through the MCP plus a hand-written ledger row). Never psql, never the app (no DDL path), never a command for Arman to run. Autocommit files (`CONCURRENTLY`, `VACUUM`, `ALTER TYPE … ADD VALUE`) → aidream `python db/apply_migrations.py --source matrx-frontend` |
...
### `platform.association_types` + `platform.reachability` — the containment/sharing cascade (live 2026-07-06)
...
`iam.has_access(token, id, required permission_level='viewer')` is the resolver (SECURITY DEFINER). Order it checks: owner (`created_by`) → org-admin read oversight (viewer only) → public visibility (viewer) → explicit `public.has_permission` grant → direct membership → **inherited containment via `platform.reachability`** (holding a container conveys `LEAST(level, conveys_max)` on contents — §3) → org context for `visibility >= internal` (`has_org_access`) → FK `containment` cascade for `visibility >= internal`. `iam.has_org_access(org)` = membership in `iam.organization_member`. Public/internal visibility on a container does **not** cascade to contents — deliberate.
...
7. **A migration file is not applied** — a `.sql` in `migrations/` changes nothing until `pnpm db:apply migrations/<file>.sql` has run it AND you have verified the object live. (The applier records the ledger row; you never do.) "Wrote the file" ≠ done.
8. **Database-tool behavior (learned the hard way)** — the Supabase MCP is for reads, verification **and direct database changes** (Arman's preference); a migration FILE goes through `pnpm db:apply`. `execute_sql` returns **only the last statement's result set**, and a multi-statement batch is **atomic** — one syntax error rolls the whole batch back (so a trailing typo'd SELECT silently undoes anything before it). Put the verifying SELECT last and on its own; don't bundle statements behind a SELECT that might error.
...
10. **`ADD COLUMN IF NOT EXISTS` silently no-ops on a NONCONFORMING existing column** — a pre-existing nullable `metadata` stays nullable; the FAIL persists. After every column add, verify the SHAPE (`is_nullable`, default), not just presence; fix with backfill → `SET DEFAULT` → `SET NOT NULL`.
11. **`iam.canonical_certify` reads the cached `audit.*` snapshot** — after fixing a broken dependent function, it still shows FAIL until `select audit.refresh();`. Re-check the function directly with `plpgsql_check_function(...)`, refresh, then trust the gate.
...
- **`platform.deprecate_relation(p_schema, p_name, p_new_ref, p_reason)`** — the **tripwire** for a table that must physically stay. Renames the table aside to `<name>__deprecated` (zero data loss, reversible), then replaces the old name with a view + INSTEAD-OF triggers that **RAISE on any read or write** (`platform.dead_relation_read()` / `dead_relation_write()`). The read tripwire is an uncorrelated VOLATILE qual (`… WHERE platform.dead_relation_read(old,new)`) — a one-time filter that fires on ANY query, even `count(*)` on an empty table (verified). The error names the new location. Reverse: `drop view; alter table <name>__deprecated rename to <name>; delete from platform.deprecated_relations`. Use ONLY when you can't make the old name vanish — the default (move / `SET SCHEMA`) is louder and cleaner.
  - 🚨 **Both tripwire functions are installed ON DEMAND, so `dead_relation_write()` being attached to ZERO tables is its CORRECT resting state — never "an orphan trigger to clean up".** Every one of the 195 ledgered retirements so far took the default move path, so no `<name>__deprecated` view exists to carry it. It therefore sits permanently in `audit.broken_functions` at `severity='unchecked'` ("trigger fn not attached; not checkable"), and that row is a deliberate keep asserted for by name in `migrations/retire_orphan_updated_at_trigger_helpers.sql`. Retiring it would silently disarm `platform.deprecate_relation` for the next table that needs it. (`dead_relation_read()` is a plain function, so it never shows up in a trigger scan at all.)
