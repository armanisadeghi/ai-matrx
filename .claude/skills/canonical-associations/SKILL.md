...
3. **Tokens come from the installed `@ai-matrx/associations` vocabulary** (`EntityTypeToken`, `ENTITY_TYPE_METADATA`), generated inside the package from `platform.entity_types` — after the DB registers the token, regenerate + patch-release the package and `pnpm up @ai-matrx/associations` here (`pnpm check:entity-types` shows the drift) so the token exists before you reference it in the overlay. There is no local copy.
