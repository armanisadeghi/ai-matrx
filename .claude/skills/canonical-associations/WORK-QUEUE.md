...
| 1 | ✅ Stage 1–2 done (3 gated) | `agent.agent_surface` | agent_id (uuid) ↔ surface + `value_mappings`, scope cols | **MIGRATED (2026-07-03).** The product decision was made: surfaces are now first-class uuid entities (`ui.ui_surface.id` + registered `surface` token), so the agent→surface binding is a canonical edge (`source=agent`→`target=surface`). Stage 1 (surface UUID + `surface` entity + count-verified backfill) + Stage 2 (menu_surface view + create_shortcut RPC repoint) applied + verified live; FE service rewritten onto `associationsService`. Stage 3 (graveyard `agent_surface`) GATED behind soak + repoint of the admin-stats readers. Tier/scope → edge `role`+`organization_id`; `value_mappings` → edge `metadata`. See `features/surfaces/FEATURE.md` + `docs/handoffs/AGENT_SURFACE_ASSOCIATIONS_STATUS.md`. **Surfaces are the container for future source types** (tool↔surface, roles/prefs — "then take it from there").|
