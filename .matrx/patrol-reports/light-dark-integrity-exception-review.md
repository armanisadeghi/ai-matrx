...
- **Pending proposals:** 51 files / 106 raw-token lines
- **Reviewable now:** 50
...
- 30 lines were classified compliant because they have a real explicit theme
  branch, are overridden by a consumer, are commented/non-rendered, or have no
  current consumer. They are not allowlisted and will be scanned again if code
  changes make them live.
- 3 lines were confirmed defects and repaired in two files:
  `CandidateProfileView.tsx` (2 skeleton lines) and `RoomHeader.tsx` (1
  mobile sheet row). Certification is recorded in the main patrol report.
