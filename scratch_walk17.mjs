...
(async () => {
  ...
  await urlInput.fill('https://www.bbc.com/news');
  ...
  await page.waitForTimeout(800);
  await page.fill('input[placeholder*="Search scopes" i]', 'AI Matrx');
  await page.waitForTimeout(1000);
  await page.click('button:has-text("AI Matrx"), [role="button"]:has-text("AI Matrx")').catch(async()=>{
    const opts = await page.$$('text=AI Matrx');
    for (const o of opts) { if (await o.isVisible()) { await o.click(); break; } }
  });
  await page.waitForTimeout(500);
  await shot(page, '07c-project-and-scope-samorg.png');
  ...
  await saveBtn.click();
  await page.waitForTimeout(5000);
  await shot(page, '07d-after-save-final.png');
  const dialogGone = !(await page.$('text=Captured, not saved'));
  log('7-save-bbc', dialogGone ? 'PASS' : 'FAIL', dialogGone ? 'saved successfully' : 'still shows captured-not-saved state / error');
  fs.writeFileSync(path.join(OUT, 'results-7b.json'), JSON.stringify(results, null, 2));
  await context.close();
})();
