...
(async () => {
  ...
  await shot(page, '07a-news-save-panel.png');
  ...
  await page.waitForTimeout(800);
  // Try searching for a scope name
  await page.fill('input[placeholder*="Search scopes" i]', 'AI Matrx');
  await page.waitForTimeout(1000);
  await shot(page, '07b-scope-search-aimatrx.png');
  fs.writeFileSync(path.join(OUT, 'results-7.json'), JSON.stringify(results, null, 2));
  await context.close();
})();
