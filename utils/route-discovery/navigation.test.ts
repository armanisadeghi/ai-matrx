...
const concrete = [
  "mandates",
  "mandates/new",
  "mandates/research_client.output_slides",
];
...
describe("filesystem routes used as navigation", () => {
  ...
  it("excludes templates from every generated module/search/group menu", () => {
    ...
    expect(
      toModulePages(routes, "/administration").map((page) => page.path),
    ).toEqual(concrete);
    expect(
      buildRouteSearchRows(routes, "/administration").map((row) => row.route),
    ).toEqual(concrete);
    expect(Object.values(groupRoutes(routes)).flat().sort()).toEqual(
      [...concrete].sort(),
    );
  });
});
