...
(async () => {
  ...
  // locate the Save button precisely at bottom near Cancel
  const saveBtn = await page.$('button:has-text("Save"):near(button:has-text("Cancel"))');
  console.log('saveBtn found', !!saveBtn);
  if (saveBtn) {
    await saveBtn.scrollIntoViewIfNeeded();
    await shot(page, '06f-before-final-save.png');
    await saveBtn.click();
  }
  await page.waitForTimeout(5000);
  await shot(page, '06g-after-final-save.png');
  const dialogGone = !(await page.$('text=Save Example Domain'));
  log('6-save-final', dialogGone ? 'PASS' : 'FAIL', dialogGone ? 'dialog closed after Save click' : 'dialog still open after Save click');
  fs.writeFileSync(path.join(OUT, 'results-6d.json'), JSON.stringify(results, null, 2));
  await context.close();
})();
