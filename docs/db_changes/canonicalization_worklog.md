...
- `audit.table_impact(schema,table)` — **PREFLIGHT**: every fn touching the table · `dependency` (precise|text-qualified) · `currently_broken` (keys on `severity='real'`) · exact `referenced_columns[]`. Run before any rename/drop to get the blast radius. The `signature` join key is `(oid::regprocedure)::text` on BOTH sides — never record it any other way, and never compute it while `search_path` is pointed elsewhere.
- `audit.function_runtime_probe` — registered read-only probes actually EXECUTED by `audit.refresh()`. The only thing that can prove a runtime failure (a privilege error inside dynamic SQL is invisible to any static checker). Failures land as `level='runtime_error'`, `severity='real'`.
- `audit.m2m_candidates` · `audit.unregistered_candidates` · `audit.stale_registry` · `audit.refresh_log` (per-severity counts **derived from** `broken_functions` by `audit.refresh_log_recount()` after every phase — `broken_fn` = distinct `real` signatures; it cannot drift from the table).
