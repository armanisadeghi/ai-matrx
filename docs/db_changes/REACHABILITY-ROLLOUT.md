...
**Database:** `automation-matrx` (Supabase project `brsgrqvjdzwihsvnfqkf`)
...
11. **The cache is now guarded, not assumed** (added 2026-08-21; drift audit 2026-08-15 finding 8, risks 1 and 2 — `migrations/reachability_standing_guards.sql`). Two standing guards, run by `pnpm check:reachability-guards` and wired into `run-release-gates.sh`:
    ...
    - **Scheduled since 2026-08-21** (Arman approved it by name and interval: "Daily + self-heal"). pg_cron job `reachability-drift-selfheal`, `10 9 * * *` UTC, runs `platform.heal_reachability_drift()`; `public.admin_heal_reachability_drift()` is the super-admin on-demand door. Every run since has reported `succeeded` and drift has been 0 every time, so no incident has ever been filed.
    - **The heal is loud, ordered, and freezable** (2026-09-15, DD-259, `migrations/dd259_reachability_selfheal_loud_and_pausable.sql`). On non-zero drift the healer files `ops.system_error` kind `reachability_drift_detected` — which aidream's `_patrol_priority()` ranks `urgent`, so it surfaces in the triage queue — **before** it repairs anything, carrying the disagreeing-row count, the breakdown by disagreement kind, a 25-row evidence sample, and the containers touched. The rebuild then runs inside a subtransaction: if it throws, the incident row stands and records the failure instead of being rolled back with it (that ordering was the DD-259 defect). The repair — never the detection or the filing — is frozen by the knob `platform.reachability.selfheal_pause` (default false, system-scoped, no organization override), for a cutover that is deliberately rewriting containment; a skipped repair files a louder row saying access is knowingly still wrong and how to clear the freeze.
