...
| 15 | DB canon satellite | aidream `db/canonical_db.md` (named "canonical") | demote to pointer at common-docs/systems/platform/db-rules |
...
- **Pointer-path standardization** (added on takeover review): stubs and CLAUDE.md lines hardcoded three spellings of the common-docs path — the `Users`-rooted form, the `Volumes`-rooted form, and the repo-name (`matrx-common-docs`) form. All resolve on this machine via symlinks, but cloud/sandbox sessions get silent dead pointers. RESOLVED: canonical form is `/Users/armanisadeghi/code/common-docs/<systems|projects|policies|meta|skills>/...`; the guards' pointer-path lint enforces it.
...
- [x] Wave 3 EXECUTED (2026-07-26): all 12 pre-approved winners across both repos — tool system (17 losers archived, injection-precedence + naming rules merged into tool_system_rules.md), request system, agent system (OAuth wiring merged into FEATURE.md), scope/context both halves (18 docs archived; agent-context FEATURE kept live), CMS authority demotions, chat standards (rules merged into chat FEATURE.md, root file archived), versioning (VERSIONING_RULES stubbed; unique rules merged into common-docs), cloud files (15 archived), scraper (5 archived), workflow (specs moved to matrx-graph/docs, hub README now a router), package doctrine (audit archived, BOUNDARIES stubbed; MIGRATION_PLAN kept as live tracker with demoted authority), db canonical_db stubbed. Skipped for Arman: docs/knowledge/scope-* trio, PACKAGE_DOCTRINE.md, WEB_SCHEMA pair (live, not stale — see doc-campaign-remaining-work.md).
- [x] Wave 4 EXECUTED (2026-07-26): Next.js/SSR 5→3 (ssr-zero-layout-shift survivor + 2 deliberate companions), surface-authoring⇐surface-registration, overlay-system⇐message-actions, ui-dense⇐data-dense-panels, ui-sharp⇐modern-web-design-expert, notes-actions boundary, task-hygiene dangling ref removed, ctx_scope_assignments groomed, ai-matrx .cursor retired (11 deleted, 5 kept with DEPRECATED headers naming migration targets), aidream's 10 .cursor skills MOVED into .claude/skills (all unique; fixes CLAUDE.md's dead matrx-agents pointer), finalize-and-ship gained the archive-on-complete rule.
- [x] Wave 5 EXECUTED (2026-07-26): docs guards live in both repos (confident-title vs allowlist, root-.md ban, pointer-path lint) — ai-matrx `pnpm check:docs-guards` + advisory release gate; aidream `scripts/check_docs_guards.py` in release.sh. Both regenerators guarded with loud exit-1 banners. Current advisory backlog: ~108 ai-matrx + ~83 aidream violations (see doc-campaign-remaining-work.md).
- [x] Prevention automation: `docs-hygiene` skill (common-docs/skills, symlinked to ~/.claude/skills) + `docs-hygiene-weekly` scheduled task (Mondays 07:00).
- [x] Campaign CLOSED 2026-07-26. Remaining items: [`doc-campaign-remaining-work.md`](doc-campaign-remaining-work.md).
