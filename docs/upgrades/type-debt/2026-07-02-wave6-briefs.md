...
**Status:** DECIDED: delete BookmarkViewer.tsx's local `PathBookmark` interface, import the canonical one from json-path-navigation-util.ts. Trust importBookmarks's return type directly instead of assigning into the structurally-similar-but-distinct local type.
