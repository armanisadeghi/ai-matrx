...
**conflict:** Two independent PathBookmark type declarations for the same runtime object; the shared one's index-segment value is numeric while the consumer's local copy assumes value:string. Kept both compiling by loosening the shared segments field to `{type:string; value:string}[]` and giving getValueByBookmark a minimal `{path:string}` parameter type instead of the full PathBookmark.
**decisionNeeded:** Should BookmarkViewer.tsx's local PathBookmark be deleted and replaced with an import of the shared PathBookmark from json-path-navigation-util.ts (now exported)? That would also let importBookmarks's return type be trusted directly instead of assigned into a structurally-similar-but-distinct local type.
**Status:** DECIDED: delete BookmarkViewer.tsx's local `PathBookmark` interface, import the shared one from json-path-navigation-util.ts. Trust importBookmarks's return type directly instead of assigning into the structurally-similar-but-distinct local type.
...
**Status:** DECIDED: add a real runtime shape check to importBookmarks (each entry has string path/name, array segments), drop entries that fail, and surface a `toast.error`/count in BookmarkViewer.tsx (handleImport) instead of silently rendering broken cards.
...
**Status:** DECIDED: delete `components/ui/samples/accordion.tsx` — zero importers repo-wide, and its rotate animation is permanently dead regardless (Radix never surfaces `data-state` as a consumable prop the way this component assumes).
...
**Status:** DECIDED: delete `componentCompatibility`, `getRelevantComponentProps`, and `getVisiblePropsForComponentType` from `field-constants.tsx` — zero importers repo-wide, dead applet-era scaffolding (part of the same broker/applet system flagged for rip-out in the fleet-briefs doc's Brief 5/22 campaign).
...
**Status:** DECIDED: delete `config/ui/density.tsx` entirely — confirmed zero importers repo-wide.
