...
|---|---|---|
| `'use cache'` + `cacheTag`/`cacheLife` | Explicit, opt-in caching at file/component/function level; compiler auto-generates cache keys. ([docs](https://nextjs.org/docs/app/api-reference/directives/use-cache)) | **Not adopted — NOT available here** (corrected 2026-09-14): `cacheComponents` is off, so the directive is a build error (`CLAUDE.md` § Core invariants). Live opt-in caching is `unstable_cache` with `tags`/`revalidate` over cookie-free reads, `updateTag` from Server Actions, and route-level `export const revalidate` (exemplar `features/education/publishing/queries.ts` + `actions.ts`). |
...
| Implicit/legacy caching assumptions | App Router is dynamic-by-default in 16; caching is opt-in (`'use cache'` only exists under `cacheComponents`, which is off here). | Audit places assuming auto-caching; make caching explicit + tagged with `unstable_cache` (see the `'use cache'` row above). |
