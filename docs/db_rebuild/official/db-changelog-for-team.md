...
## 5. `cx_conversation` — visibility now enforced (the half-migrated root, finished)
`visibility` existed but wasn't enforced. Now it is, via canonical `has_access` RLS.
- **Reads:** owner (their own) + org **admin** oversight + public/link via `visibility` + explicit grants + project collaborators. **Behavior change (applying the conversation paradigm):** regular org members can no longer read other members' conversations just by being in the org — only owners, org admins, project collaborators, or explicitly granted users. Public sharing is now `visibility='public'`, not `is_public`.
- **Writes:** owner via `created_by` (backfilled from `user_id`). `_stamp_actor` sets `created_by` on insert, so existing inserts still pass.
- **App actions:** write `visibility` (not `is_public`) and rely on `created_by` (not `user_id`). These two legacy columns are **kept for now** and will be dropped once you've cut over — tell me when. (`cx_message` public-read still reads the parent's `is_public`; it'll move to `visibility` when messages are canonicalized.)
