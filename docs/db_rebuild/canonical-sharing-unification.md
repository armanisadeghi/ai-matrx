...
- ~~**`is_public` → `visibility`**~~ **DONE 2026-06-26** (`sharing_make_public_drives_visibility.sql`, verified live): `make_resource_public` sets `visibility='public'` (+ mirrors `is_public` for transition); `make_resource_private` sets `visibility` back to `entity_types.default_visibility`; "publishable" now means *has visibility OR is_public column* (canonical-only tables were previously un-publishable). Proven: make public → anon reads the note via `pub_read`; make private → anon loses it. **Follow-ups:** FE `useSharingStatus`/`getResourceVisibility` should read `visibility` (works today via the `is_public` mirror); registry `owner_column` `user_id`→`created_by`; drop `is_public` (gated) once readers are on `visibility`.
