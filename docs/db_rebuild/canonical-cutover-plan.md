...
- **DB function readers repointed (correctness, not just drop-prep):** `auth_is_project_member`/`auth_is_project_admin`/`auth_is_project_owner`, `check_resource_access`, `get_project_members_with_users`, `agx_get_user_shortcuts` now read `iam.memberships` — `migrations/repoint_project_member_functions_to_iam.sql` (applied + ledgered + impersonation-verified). These are access-control helpers; reading stale `ctx_project_members` was an active bug (invited members unrecognized).
- **Part 6 (decommission)** — NOT started; destructive, gated. **Remaining `ctx_project_members` readers (the last 3, deferred as a self-consistent pair+trigger):** the creator trigger `ctx_projects_add_creator_membership` (still writes `ctx_project_members`, keeping it populated with owners) + the two context builders `get_user_full_context` / `get_user_hierarchy` (still read it). They're coupled (repoint the trigger and these miss new personal projects) and `get_user_full_context` is entangled with the active personal-org-sentinel refactor + the `ctx_scope_assignments` track — repoint all three together in the context-track wave BEFORE dropping `ctx_project_members`. Then drop `_mirror_assoc` trigger + the legacy-table own policies, then the 7 tables. Also droppable: `get_entity_scopes`/`set_entity_scopes`/`list_entities_by_scopes`.
