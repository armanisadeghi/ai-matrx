...
**Phase 2 — SET SCHEMA move ✅ DONE (applied + verified live, ledgered):** `migrations/communication_move_phase2.sql`. `communication` exposed to PostgREST; all 13 tables moved (counts unchanged pre/post — emails 10, dm_conversations 7, dm_messages 183, participants 13, sms_* all matched; old names error = clean cut; policies/triggers/realtime publication followed). Registry `schema_name` updated. Repointed 5 functions (`get_ssr_shell_data`, `get_ssr_agent_shell_data`, `get_user_dashboard_metrics`, `sms_handle_opt_out_keywords`, `sms_update_conversation_on_message`) — also fixed pre-existing `public.user_preferences`→`users.user_preferences` drift in the two SSR fns. `--schema communication` added to `pnpm db-types`. **FE:** 100 `.from()` callsites → `.schema('communication').from()` + 7 realtime `postgres_changes` schema filters, across ~20 files (single-line per the dead-relations guard); `scripts/dead-relations.json` + `platform.deprecated_relations` registered (guard clean for communication). **aidream:** already pre-aligned — `db/models/communication.py` carries the canonical columns + `_db_schema='communication'`, models load clean; no changes needed. (`dm_default_org`/`dm_participant_sync_grant` touch only stable `public` tables — no repoint.)
...
- **2026-06-28** — **pdf/redaction cluster (4 tables) moved `public`→`pdf` schema.** `pdf_consolidation_log`, `pdf_redaction_audits`, `pdf_redaction_key_escrow`, `redaction_mapping` — all **empty (0 rows)** at move time, moved atomically via `ALTER TABLE … SET SCHEMA pdf`. RLS policies (8), the `_110_stamp_actor` trigger, indexes, and cross-schema outbound FKs (`files.files`/`files.pages`/`auth.users`/`iam.organizations`) all followed automatically. **No inbound FKs, zero views, zero DB functions/triggers** reference these (grep + live `pg_get_functiondef` sweep both empty). 1 `entity_types` row repointed (`pdf_redaction_audits` → `schema_name='pdf'`). 4 entries added to `platform.deprecated_relations` + `scripts/dead-relations.json`. **FE never queries these directly via PostgREST** (Python owns all redaction write paths) so no `.from()` repoints — only `schemaName:'pdf'` added to the `pdf_redaction_audits`+`redaction_mapping` registry-mirror entries; `pdf` added to `db-types --schema`. Migration ledgered (`move_pdf_redaction_cluster_to_pdf_schema.sql`). FE committed+pushed (`ad320c173`). **aidream:** the live service-role audit write (`pdf_processing.py` raw `insert into public.pdf_redaction_audits` → `pdf.`), `matrx_orm.yaml` (`pdf` in `additional_schemas` + a `pdf` generate block, host-owned managers like docproc), and the matrx-utils redact package (`SCHEMA.sql`+docstrings) repointed + committed+pushed (`589a38041`). The full ORM model/manager regen (`db/models/pdf.py`, `db/managers/pdf/` with explicit `_db_schema='pdf'`; removal of the now-stale `public.py` model stubs + 4 old flat managers) is **generated in the working tree but intentionally left uncommitted** to ride the in-flight `public`-managers flat→subdir regen (29 hand files still import flat public managers → the subdir tree doesn't boot yet; committing it standalone would break aidream's boot). **⚠️ PENDING (non-blocking):** (1) expose `pdf` in Supabase dashboard for symmetry — not required now (FE never PostgREST-queries these); (2) the aidream broad regen commit drops the public stubs + completes the redaction-mapping ORM path (the compliance-audit raw-SQL path already works).
- **2026-06-28** — **docproc cluster (7 tables) moved `public`→`docproc` schema.** `processed_documents`, `processed_document_pages`, `derive_runs`, `page_extraction_jobs`, `page_extraction_runs`, `page_extraction_page_runs`, `page_extraction_results` — all moved atomically via `ALTER TABLE … SET SCHEMA docproc`. Policies, triggers, indexes, inbound/outbound FKs all followed automatically. 10 functions repointed (hardcoded `public.*` refs): `_extracted_documents_view_*` (×3), `agx_usage_history_counts`, `can_curate_library_document`, `can_read_processed_document`, `fork_processed_document`, `page_extraction_clear_job_results`, `page_extraction_runs_rollup`, `pdf_resolve_file_page_link`. `pdf_unified_pages` view recreated pointing at `docproc`. 3 `entity_types` rows updated (`schema_name`→`'docproc'`). 7 entries added to `platform.deprecated_relations` + `scripts/dead-relations.json`. FE: 14 files repointed across `features/page-extraction/`, `features/rag/`, `features/pdf/`, `features/notes/`, `features/kg-suggestions/`, `features/files/`, `features/pdf-extractor/` — all now `.schema("docproc").from(...)`. Zero stale `public.*` references remain in FE (grep-verified). `docproc` added to `db-types --schema`. Migration ledgered as `move_docproc_tables_to_docproc_schema.sql`. **⚠️ PENDING:** (1) expose `docproc` in Supabase dashboard (Settings → API → Exposed schemas) — FE 404s until done; (2) `pnpm db-types` after exposure; (3) aidream ORM update (`db/matrx_orm.yaml` + `python db/generate.py`) + repoint all Python managers/routers; (4) commit + push both repos.
- **2026-06-28** — **`agent_run` + `agent_run_stage` → `chat` schema (canonicalize + move).** `agent_run` was already base-retrofitted + registered (entity) with `_version_capture`/`stamp_run_org`/`emit_run_lifecycle` triggers; finished it: backfilled `created_by` from `user_id` (13 rows, triggers disabled to avoid version churn), added `visibility`, replaced the single legacy `user_id`-based policy with `iam.apply_rls(...,'entity')`. `agent_run_stage` registered as a **component** of `agent_run` (via `run_id`) + `apply_rls component`. Then **moved both `public`→`chat`** (`chat` already PostgREST-exposed → single pass); registry `schema_name`→`chat`; **no DB functions referenced them** (only the platform triggers, which follow the table); inbound `pc_studio_run_assets` FKs follow. FE: 5 `.from()` calls → `.schema('chat')` in 2 podcast-studio files. aidream models regenerated (agent_run → `chat`, picks up the chat `Cx` model prefix → `CxAgentRun`); matrx-ai `arm_managers` registry refs updated. **Verified live:** both `verify_canonical_ok`, owner-impersonation (51 runs/535 stages visible, others excluded), PostgREST `chat.agent_run`=200, `public.agent_run`=404 (clean cut), counts preserved (59/639). 2 migrations ledgered; dead-relations + `platform.deprecated_relations` (2 entries).
- **2026-06-28** — **Research cluster fully canonicalized (13 `rs_*` relations)** via `db-canonicalize-table`. **`rs_topic`** → entity (token `research_topic`, registered + sharing registry + `apply_rls entity`; added `visibility`+`deleted_at`; 13 existing topics set `visibility='internal'` to preserve project-collaborator access; **zero FAIL / zero WARN**). **`rs_template`** → entity (token `research_template`; 5 system templates set `visibility='public'`; **zero WARN**). **8 content tables** (`rs_keyword`/`rs_source`/`rs_tag`/`rs_synthesis`/`rs_document`/`rs_content`/`rs_analysis`/`rs_media`) → **components** of `research_topic` (single-level composition via `topic_id` — every child carries it; `created_by` is null on all of them because they're service-role/backend writes, which the `component` variant is built for). **2 junctions** (`rs_source_tag`→`research_source`, `rs_keyword_source`→`research_keyword`) → components. `rs_source_keywords` is a VIEW (untouched). Replaced the non-canonical project-cascade RLS everywhere; dropped the legacy `set_updated_at` double-fire trigger on all 10 trigger-bearing tables (kept `_stamp_actor`/`_touch_row`, `rs_keyword` position trigger, `rs_topic._mirror_proj`). **All 12 tables zero FAIL + zero WARN.** Owner-impersonation verified: a real topic owner reads all their topics/sources/content/analysis/media/junctions/templates, with the one other-org topic correctly excluded — no data hidden. `is_versioned=false` on all (no `_history` attached — high-volume; deferred, not silently added). 4 migration files written + ledgered (`research_canon_01..04`). **Schema MOVE → `research` DONE** (after the user exposed `research` to PostgREST): `research_canon_05` moved 12 tables + the `rs_source_keywords` view `public`→`research`; registry `schema_name` updated; **8** functions repointed (4 hardcoded `public.rs_*` + 4 with BARE refs the first pass missed — `get_topic_overview`, `get_user_hierarchy`, `agx_usage_history_counts`, `container_resource_counts`). FE: 55 `.from()` + 3 `Database["public"]…["rs_*"]` type refs → `.schema('research')`/`["research"]` across 4 files; `research` added to `db-types --schema`; types regenerated; dead-relations.json + `platform.deprecated_relations` (13 entries). aidream was already modeled on `research` (`_db_schema='research'`); only `matrx-rag/sources.py` raw SQL needed repointing. **Verified live:** PostgREST `research.rs_topic`=200, `public.rs_topic`=404 (clean cut), counts preserved (13/2474/3204/3023), `get_topic_overview` correct, FE tsc research-clean. 5 migrations ledgered. **99 tables in entity_types.**
