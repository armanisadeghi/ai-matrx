...
**Status (2026-09-14, honest):** Steps 1–5 live since 2026-08-25. **Step 6 in progress:** the value resolver, every database placement/worth writer, the aidream Topic Assigner and site valuer, the breakdown filter/sort, and the client doors are on the canonical model, live and ledgered (6a–6e below), with the equivalence, tenancy and legacy-reference guards committed. **Not done:** the screens (keyword workbench Offering column, Offerings screen, Search Console cells, run console, content-plan taggers, intake, packs admin, receipts) still call the legacy `gsc_topic_*` / `gsc_set_keyword_topic` RPCs, which now also write the canonical rows (TRANSITION); step 7 is enforced in the canonical writer but not yet for legacy callers; step 8 (retire) has not started.
