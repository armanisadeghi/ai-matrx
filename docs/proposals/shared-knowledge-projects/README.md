...
| Provenance RPC (NEW) | **One signature, defined here only** — briefs must link, not restate:<br>`public.library_grant_provenance(p_store uuid) → setof {audience text, industry_id uuid, industry_name text, industry_slug text, organization_id uuid}` and the batch form `public.library_grant_provenance_batch(p_stores uuid[]) → setof {store_id uuid, audience text, industry_id uuid, industry_name text, industry_slug text, organization_id uuid}`. Returns ONLY grants reaching `auth.uid()` — never the full grant list. The batch form exists because the catalog renders N stores and must not do N round-trips. SECURITY DEFINER, `authenticated`, anon-revoked. **P3 owns and publishes both day 1; P2 consumes them.** **LIVE 2026-07-23** (`migrations/library_provenance_and_catalog_entitlement.sql`, applied + ledgered + typed; probes: entitled reader gets the industry row, control gets zero) | **P3** |
| Catalog shape (SETTLED 2026-07-23) | `rag.fn_list_library_catalog(p_organization_id default null)` now returns per-row caller entitlement: `…, subscribed boolean, entitled_via text ('organization'\|'industry'\|'global'\|null), entitled_industry_name text, entitled_industry_slug text`. Still lists every `discoverable AND is_active` store — the **chip** is what differs per caller. Audience logic mirrors `user_can_read_data_store_via_grant` (no third reader). Same migration as above | **P3** |
| Grant-list gate (Decision 2, SETTLED 2026-07-23; **admin tier revised same day**) | Who may list a store's grants: **ANY Matrx admin OR store owner (`created_by`).** Decision 2 first landed as super-admin+owner; Arman then directed "system admins and super admins are no different for this — stop making everything super-admin gated," so the whole shared-knowledge issuance + admin-read surface is now **any-admin** (`public.is_admin()` / `ctx.is_admin`), not super-admin. Applies identically to `rag.fn_list_data_store_grants` and the aidream HTTP endpoint. Live: `migrations/library_issuance_any_admin_gate.sql` + aidream `179af9d6a` | P3 (RPC) + P2 (HTTP) |
| Issuance gate (SETTLED 2026-07-23) | **Every shared-knowledge issuance RPC is any-admin**, via the single choke point `public._library_assert_admin` (was `_library_assert_super_admin`): `industry_upsert`, `industry_curator_grant/_revoke`, `rag.library_grant_publish/_revoke`. Actor-safety shape unchanged (`COALESCE(auth.uid(), p_actor)`, anon revoked). NOTE: `industry_curator_grant` is a write-escalation path (curator ⇒ library-doc write) now open to any admin tier — audit-logged | frozen |
