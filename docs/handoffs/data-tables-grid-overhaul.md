...
7. ~~**Copy → Text writes the literal word `null` for empty cells**~~ FIXED in `@ai-matrx/kit` 0.16.1 (2026-09-21, `content-transfer` `textValue`: null → ""; guard test proven failing-then-passing; tag `npm/kit/v0.16.1` pushed). Adoption here = the next `pnpm update @ai-matrx/kit` (this repo depends on `latest`). Original note: in the TSV (seen live 2026-09-15 on the test table's empty row). Not a formula defect; belongs to `@ai-matrx/design-system/content-transfer`'s text format (THE SAME-SESSION LAW: fix in the package).
