...
0d. **Drag-to-reorder headers and the summary bar** shipped 2026-09-21 and live-verified the same day (FEATURE.md change log). **Column widths on touch devices.** The drag handle is desktop-only; the Layout menu could offer per-column width presets (S/M/L) for phones.
0e. ~~Live verification of the layout + time work~~ DONE 2026-09-21 (two independent passes; row `98caf7ab…` is `ready_for_human`).
