...
- The problem underneath all of it: "users do not use context policies a lot, and if context policies are used properly it allows the agent to instantly have visibility for the things it needs to see, and it can also help remove things we don't want the agent to see… expose and utilize all of those advanced techniques, which users are currently not using because it's complicated even for me."
...
- The agent's DB prompt carries the **context-slot doctrine**: context policies are mapped aggressively to rich values (visibility), variables conservatively; `typical_char_count` guards context bloat; `unmapped` is used deliberately to keep noise away from the agent.
...
- Producer A: detect surfaces the user runs agents on with empty/never-supplied context policies or heavily `prompt_user`-mapped bindings → assist chip "Let AI remap this agent to the page" → opens the Phase-1 tab pre-run.
...
- Mandate seeding: `surfaces_client.binding_mapper` was mirror-seeded in the DB matching the declaration; the next aidream boot re-syncs code-owned columns (expected no-op). The seed agent is `builtin` but MCP-created — if the system-agent sweep flags ownership, promote via `agx_duplicate_agent(p_as_system=>true)` and rebind.

## Resources

- Quick-bind chain: `features/surfaces/FEATURE.md` Change Log 2026-08-16/17 entries.
- Mandate system SoR: `common-docs/systems/mandates/STATE.md`; variable binding SoR: `common-docs/systems/agents/agent-variable-binding/FEATURE.md`.
