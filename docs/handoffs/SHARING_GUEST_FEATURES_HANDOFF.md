...
1. **Guest run (public capability).** Backend is DONE, frontend read surface is NOT.
   - **Build `get_agent_public(p_agent_id uuid)`** — a SECURITY DEFINER RPC returning ONLY `id, name, description, variable_definitions, context_slots, agent_type, category, tags`, `GRANT EXECUTE TO anon, authenticated`. This is the missing piece: verified that anon can read NO agent today — `agent.definition` RLS `pub_read` requires `visibility='public'`, and agents are CHECK-capped non-public, so a guest can't render the variable form. (The `agent.card` view exists but omits `context_slots` and has no captured anon GRANT — build the RPC instead of hardening the view.) The secret-leaking `agx_get_execution_full` is INVOKER, so it returns 0 rows for anon — the guest run must route through `baseSettings:{}` (`create-instance.thunk.ts:456-464`) and never call it.
   - **Wire a guest run surface**: `/s/[token]` is NOT the run path (running needs no token). The run form belongs on a public agent page (extend the `/p/chat/a/[id]` shell or a new `/p/a/[id]` "run this agent" route) that reads `get_agent_public`, renders the variable form, and streams via `X-Fingerprint-ID` + `baseSettings:{}`.
   - **aidream guardrails (backend run already works — it's under-guarded, not blocked).** Verified: `POST /ai/agents/{id}` runs ANY agent by id for a fingerprint guest with **zero ownership check** (`agent_run.py:162` loads by id over service-role, no visibility gate) and secrets stay server-side (good). Missing: **(a) per-guest run quota** — none exists for agent runs (only file/sandbox); add a count against `guest_execution_log` before `prepare_agent_run`, return 429. **(b) tool-injection lock** — the request body's `tools`/`tools_replace` (`models.py:44-49`) let a caller add/replace an agent's tools; ignore them for `auth_type=='fingerprint'`. See §Decisions — "run any agent by id" is the stated model but has cost/abuse implications worth a conscious yes.
...
1. **"Run any agent by id" — confirm the blast radius.** Situation: the backend already runs ANY agent by id for a logged-out guest with no ownership check (verified in aidream) — so today, anyone who learns an agent's UUID can run someone else's private/in-development agent, and each run spends tokens. This matches your stated model ("the non-secret parts of all agents are public… you can use them"), but it means private agents are runnable-if-id-known and there's currently no per-guest quota. Decide: (a) keep fully-open + add a guest quota (recommended); (b) narrow guest-run to agents explicitly marked runnable/published, keeping private drafts un-runnable. This decides whether §1 adds only a quota or also a "runnable" visibility gate.
