...
> - **`agx_get_execution_full` is now `SECURITY DEFINER`** (not INVOKER as stated in §Resources) gated `iam.has_access(...,'viewer') OR (agent_type='builtin' AND is_active)` — and **GRANTED TO `anon`**, so any anonymous visitor can read builtin agents' `settings/model_id/tools/custom_tools`. Correct under the access-ladder law only if builtin agents are Public-level. Browser still calls it from `launch-agent-execution.thunk.ts:409-431`.
...
> - **Registry:** 44 of 295 registered types are `is_link_shareable=true` (live 2026-09-26); the law says every record can be shared by link, so the rest are short of it.
...
- **Access and sharing follow the law: `common-docs/policies/access-ladder.md`.** An **Agent** is Organization-level — every member of the owning organization can open it; a **marketplace agent** is Public — anyone can open it, signed in or not. Any agent can be shared by its owner by public link or directly with a person, and a share opens the whole agent (so the recipient can see and fork it).
- **Running follows opening:** a guest may run an agent they can open — a Public agent or one shared with them. Today `POST /ai/agents/{id}` runs any agent by id for a fingerprint guest; that is wider than the law and is a gap to close.
...
- **Verify UI already exists per-app**: `features/agent-apps/components/AgentAppAdminActions.tsx` (Verify + Feature toggles, status setter) inside `app/(admin)/administration/agents/agent-apps/edit/[id]/page.tsx`. Write path `lib/services/agent-apps-admin-service.ts#updateAgentAppAdmin` (`:318-342`) is a **direct RLS-gated `.update()` on `app.definition`** — no admin RPC. What's missing is a **review queue** (list of published-but-unverified apps), not the per-app toggle.
...
1. **Guest run of an agent the guest can open (Public, or shared with them).** Read surface + backend DONE; only the FE run route remains.
...
2. **Fork-on-share.** The fork PRIMITIVE already exists — `agx_duplicate_agent(p_agent_id, p_as_system)` (`migrations/agx_duplicate_agent_preserve_system_type.sql`): SECURITY DEFINER, copies the full config, gated `check_resource_access('agx_agent', id, 'viewer') OR is_public`. A viewer+ share-holder can already fork. Remaining:
   - **The `OR is_public` clause** lets any signed-in user duplicate a Public agent in full. That matches the law — a Public record is open to anyone, so it can be copied — and it is how forking a platform template into your own organization works. Leave it.
...
3. **Agent-app SEO — published + verified** (decision made). Infra partly exists — EXTEND, don't build.
   ...
   - **Review queue** (Arman's condition): the per-app Verify toggle + write path + filtered fetch all exist (`AgentAppAdminActions`, `updateAgentAppAdmin`, `fetchAgentAppsAdmin` `is_verified/status` filters) — so this is a filtered list page + inline verify under `app/(admin)/administration/agents/agent-apps/`, not new infra. Optional public `/discover` index for internal linking.
