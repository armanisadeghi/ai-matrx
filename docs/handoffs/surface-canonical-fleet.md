...
## Remaining work — the board drives everything (live 2026-08-09: **57 verified / 55 partial / 25 stub / 24 unregistered-active**; 133 manifests, 3,237 declared values — always read fresh counts from the board)

1. **Admin fleet: partial → verified.** `cx-dashboard` / `applications` / `agent-apps` are DONE (verified, all emitters wired + DB synced 2026-08-09). `agent-review` needs ONLY a live browser pass (load `/administration/users/agent-review`, open the Agents chrome, confirm the scope) — its emitter is wired and code-audited; the 2026-08-09 remote session couldn't reach production (network policy blocked aimatrx.com), so it stays partial. The stub group (`server-logs`, `sandbox`, `users`, `email`, `feedback`, `bundles`, `mcp-servers`, `lookups`, `official-components`, `scheduling`) needs emitters from scratch — `server-logs` is the hairiest (15 useState hooks in a 1,487-line component).
...
5. **Stub → verified (25 remaining).** DONE with live nested emitters (now partial, each needing only a browser pass): image-viewer, gallery, share, voice-pad (2026-08-09). Remaining manifest-only overlay windows: image-uploader, feedback, canvas-viewer, transcript-studio *(preserve its hand-coded 3-pipeline vocabulary)*, observational-memory, agent-gate, agent-run-history, agent-settings, smart-code-editor; the toy manifests (messages, lists, canvas, ai-results, agent-advanced-editor, mermaid-editor); and the admin stubs (item 1). Each needs a live audit + nested emitter. Pattern to copy: `ImageViewerWindow.tsx` / `ShareModalWindow.tsx` (provider inside the WindowPanel, scope from live state at Run time).
6. **Matrx-vs-matrix live binding test** (surface-authoring Layer 6) — still not run post-overhaul on any surface. Requires production browser access (bind a non-matching-name test agent, launch from the surface, check `cx_conversation.variables` + the Matrx spelling); the 2026-08-09 remote session was network-blocked from aimatrx.com, so this must run from a local/dev machine.
...
- Admin fleet wave 2 (2026-08-09): cx-dashboard (6 tab emitters), applications (5 tab emitters), agent-apps (5 section emitters) all VERIFIED + DB-synced (`migrations/surface_sync_admin_fleet_verified_20260809.sql`); agent-review emitter code-audited (browser pass pending).
- Overlay emitter wave 1 (2026-08-09): image-viewer, gallery, share, voice-pad wired with nested providers (stub → partial); gallery's dead footer view-mode buttons fixed (duplicate state collapsed into `GalleryFloatingWorkspace`).
