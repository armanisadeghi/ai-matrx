...
3. **Remaining unregistered real user pages**: `/rag/repositories` (code repos — distinct from data-stores, do NOT merge), `/rag/flow`, `/rag/visualization`, `/legal/ca-wc`, `/artifacts`, `/reports`, `/surfaces` (the hub itself), `/education`'s ~85 uncovered routes (next best: quizzes+practice-tests as one assessment surface, flashcards `[setId]/study`, fastfire, planner+progress), `/images` ~13 remaining static explainers/stubs after the 2026-08-09 studio-family wave (avatar, presets, library, all-files, branded, from-base64, upload, profile-photo, public-search, tools, manager, studio-library, studio-light — most load nothing; register only if a real tool ships), `/podcast/[slug]` + `/podcast/studio/show/[showId]`. Plus the non-web rows: `chrome-extension/{assistant,pilot}`, `matrx-local/desktop`, `matrx-default/*`, `matrx-public/*` (5), `matrx-user/{quick-data,workflow}` — decide register-or-retire per row.
