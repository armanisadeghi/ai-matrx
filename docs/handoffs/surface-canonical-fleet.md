...
## Remaining work — the board drives everything (live 2026-08-12 post-batch-3: **70 verified / 87 partial / 5 stub / 7 unregistered-active**; 162 manifests, 4,098 declared values, 371 write targets — always read fresh counts from the board)

1. **Admin fleet: partial → verified — the remaining gap is BROWSER PASSES, not emitters.** Every wave-1 admin surface now has a live emitter (verified in code 2026-08-12: server-logs, sandbox, email, feedback, scheduling ×7 tabs, official-components ×2 routes, plus the wave-2 verified group). Real named exceptions: `feedback` needs a UI refactor — `FeedbackDetailDialog` is a Radix modal whose `pointer-events: none` blocks the floating agent chat, so its authored fields are unreachable by agents; `server-logs` caps `raw_logs`/`visible_log_lines` by design. Next concrete step: browser-verify each partial on a dev server and promote.
...
4. **Register later**: `matrx-user/workflow` — DB-rendering-contract surface for aidream Workflow Studio emit components (`features/workflow-emit/surface.ts`); register when the FE consumer wires `DbEmitRenderer`, do NOT retire. `/p/e/*` public-resource viewer — a different surface from `matrx-public/p`; currently mis-attributed to it by the `/p` prefix, give it its own entry ABOVE `/p` when it gets a manifest. **`/p` shell-kind sweep**: `matrx-public/p` wiring covers only the `CustomComponentRenderer` path — the built-in shells (`features/agent-apps/components/shells/*`, `AgentAppFullyCustomShell.tsx`) likely launch without `runtime.surfaceName` too; audit and wire each shell kind.
