...
2. **Bespoke-menu consolidation campaign** — files menus DONE (2026-07-21); next up: ItemMenu's right-click mode (~35 consumers), notes-legacy shell, rich-document's own menu, code trees, coordinate menus, markdown-block menus. Inventory + status in `FEATURE.md` → "Consolidation backlog". Pattern for coexistence with kebab menus: `ItemRow.disableContextMenu` + v3 wrap (notes sidebar did this); pattern for row menus: keep component exports/props, swap internals to v3 `extraSections` (files menus are the exemplar).
