...
2. **Bespoke-menu consolidation campaign** — files menus + ItemContextMenu DONE (2026-07-22). DONE 2026-07-22: **rich-document cursor menu** (callsite swap + ContextMenuMount/variants ContextMenu deleted; verify-pass restored non-copy registry actions as extras + suppressed floating icon). Latent follow-ups CLOSED 2026-07-22 (richDocCtxExtras prop + raw instanceKey hash). Previously landed prerequisite: v3 now has `suppressed` (yield-to-native while streaming, children never unmount). The swap happens at the CALLSITE, `features/rich-document/RichDocument.tsx` ~line 280: replace the `ContextMenuMount` wrap with `NonEditableContextMenu` passing `suppressed={isStreamActive}`, `contentSource={source}` (v3 then renders the SAME registry actions itself — do NOT also convert the full action list or every Copy-as/Export item doubles), `contextData={{content: <doc content>}}`, and extraSections built ONLY from the custom `cmOptions.extra`/`actionsProp.extra` actions; needs a `rich-document` (or reuse existing) SourceFeature literal. Then DELETE runtime/ContextMenuMount.tsx + variants/ContextMenu.tsx (single consumer). CAMPAIGN COMPLETE 2026-07-22 — every bespoke right-click menu is on v3 (files, ItemContextMenu, rich-document, code trees, user-lists, transcript/task/profile blocks, json-explorer, processor-extractor). Only exceptions, both deliberate: notes-legacy shell menus (die with legacy NotesLayout — do not migrate) and PDF region plumbing polish (owned by the PDF focused session). Inventory + status in `FEATURE.md` → "Consolidation backlog". Row-menu pattern: keep exports/props, swap internals to v3 extraSections (files + ItemContextMenu are exemplars).
...
4. **Planned demo pages** — `surface-mappings` DONE 2026-07-22 (live, browser-verified; note: definition_surface is gone — bindings are association edges via agent.menu_surface, all tiers merge; the page shows the layer stack + per-key winning tier). `launch-inspector` DONE 2026-07-22 (live, browser-verified; also fixed launchShortcut dropping onConversationCreated). ALL self-serviceable loop work is now drained — every remaining item below is Arman-gated (review verdicts on 11 queue rows, inline-edit prod retest, PDF focused session, streaming-edit-visualization design decision).
