...
4. **Brand-scope the remaining org-wide mounts**: `content/plan` (PlanSitesList), `planning/initiatives`, `email`, and `pr`/`pr/outreach` still mount org-wide components with NOTE comments. Websites, competitors, monitoring, and the press room gained brand scoping 2026-08-30 (`BrandScoped*` wrappers, `listSites(brandId)`); the superseded `MonitoringFrontDoor` file awaits deletion by its owning session.
