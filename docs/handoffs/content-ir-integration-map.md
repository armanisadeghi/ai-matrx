...
- **Persistence:** `content_ir.kind_instance` *(ratified: paraphrase — no verbatim source located)* — `iam.verify_canonical` 0 FAIL / 0 WARN; pinned versions; derived-on-write verdicts; fresh-version pinning at save; title derivation (explicit → `metadata.title_key` → shared list, mirrored both repos); reserved-slug guards both repos.

<!-- CONVERGE: Kinds hold no data — "A kind is a shape for agent output. Persisted data lands in standard or custom tables through bindings. The kind-instance store is retired." (declared 2026-09-10, AI Matrx Data Doctrine R6; §7 lists content_ir.kind_instance → Deprecated). Observed here: this map presents kind_instance as the completed persistence design for the shapes user loop. Live 2026-09-10: 130 rows, newest write 2026-08-30, five server tools plus this browser write path. Reconcile when you next change this for another reason. Do NOT escalate. Register: /projects/data-doctrine-adoption/REGISTER.md#dd-027 -->
...
1. **Deploy aidream → drift → flips (owner-gated start, then mechanical).** After Arman runs `bash scripts/release.sh`: verify prod (creator agent live, tool stamping, envelope producer), let traffic soak, read `workflow_io` drift logs (`degraded_reason=` tokens + `output kind … DEGRADED`) — the only family with real gates today; `tool_io`/`action_io`/`agent_io`/`render` flags had zero call sites and were deleted rather than converted — fix screamers, then flip the `workflow_io` entry in the code-constant `matrx_graph.kinds.KIND_ENFORCEMENT` dict via a code push, then remaining families as logs come clean, then delete the hand-coded detector literals (gated on the flip). **Not an env var** — Arman ruled 2026-08-25 that enforcement must never be one; `MATRX_KINDS_ENFORCE_*` was deleted (see `common-docs/systems/content-ir-system/FEATURE.md` § Ratified integration decisions). Post-soak: drop `content_ir._backup_*` + the two `matrx_orm.yaml` exclude lines.
