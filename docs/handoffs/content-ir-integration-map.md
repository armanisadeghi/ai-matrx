...
3. **Lane C — education renders kind-aware.** Swap education study/quiz content rendering from `BasicMarkdownContent` to `EnhancedChatMarkdown` so the study agents' existing `flashcard_set`/`diagram_spec`/etc. payloads render as components. `features/education/**` only.
