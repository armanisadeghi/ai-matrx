...
The module was **structurally repaired and then given its full intended shape**. The repair is shipped and verified in production; the shape is declared, and the pillars are filling in one by one (ranks, AI Visibility, Competitor Autopsy, Initiatives, and Client Reports so far — 7 of 16 reserved routes left).
...
- **Reserved routes: 7 of 16 remain** *(corrected 2026-08-19 — the count had been stale at 11 on five lines)*. Shipped at their permanent URLs: `/marketing/ranks`, AI Visibility, Competitor Autopsy, Initiatives, Client Reports, **`/marketing/local`** (real `LocalListingsWorkspace`), and the three front doors **`/marketing/email`**, **`/marketing/outreach`**, **`/marketing/monitoring`** (shipped 2026-08-19). Remaining 7 stubs: `ads`, `analytics`, `audience`, `automations`, `calendar`, `content-studio`, `social`. The coming-soon registry also holds 11 `marketing.tools.*` analyzer promises + `marketing.generate-video`.
