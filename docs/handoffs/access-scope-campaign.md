...
5. **Open briefs** (`common-docs/systems/platform/access/DECISION_BRIEFS.md`): Brief 1 (Module Settings as the one org-admin knob + the agent/skill `moduleKey` collision), Brief 3 (shared-with-me + mine-first list contract; `is_discoverable` unused by list surfaces — G14), Brief 4 (`/p/e` allowlist → registry flag).
