...
5. **LiveCaptureButton migration** (`features/education/notes/LiveCaptureButton.tsx`) off its private `useChunkedRecordAndTranscribe` instance onto `useGlobalRecording()` — running as a spawned background task; if that session died, its brief is in the eslint-disable comment above the import in that file. Remove the disable when done.
