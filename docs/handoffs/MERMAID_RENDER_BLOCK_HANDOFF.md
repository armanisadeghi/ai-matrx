...
| Adapters (round-trip + ops) | 40 unit tests — `serialize(parse(s)) ≡ s` and op correctness for all 9 editable types (flowchart/mindmap/sequence/pie/timeline/journey/quadrant/state/ER). |
...
2. **Structural editing covers 9 of ~20 diagram types** (flowchart/mindmap/sequence/pie/timeline/journey/quadrant/state/ER — *2026-06-14*). The rest are render + code + AI only. By design (one backbone), each new type is an additive adapter. Note: **visual** (tap-to-edit on the SVG) stays flowchart-only; the other eight use **outline + code** structural editing.
