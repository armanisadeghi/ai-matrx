...
- **Cross-subdomain auth:** `utils/supabase/authCookie.ts` — cookie `sb-matrx-auth`, `Domain=.aimatrx.com` on apex hosts only. Every frontend Supabase client uses its options: `utils/supabase/client.ts`, `server.ts`, `middleware.ts`, and `debugClient.ts`.
