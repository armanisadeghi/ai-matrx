...
4. **Soft-delete vs upsert-arbiter rule (server side).** The scraper's plain unique arbiters (`page(site_id,url_hash)`, `sitemap(site_id,url)`, `page_sitemap`, `discovered_item` dedup, `gsc_page_stat(page_id,date)`) also match soft-deleted rows: **every server upsert's ON CONFLICT DO UPDATE must also set `deleted_at = null`** when the meaning is "exists again" — today a user-deleted sitemap that re-syncs silently updates an invisible row forever. FE side fixed (`createManualPage` restore-on-23505). Same rule owed to `analysis_item(org,key)`, `provider(org,key)`, `site_item_config(site,item)` when they get delete UIs.
...
7. **Crawl robustness batch — FE half DONE 2026-08-08, server half in flight** (board bullets have per-item status): FE shipped the one `crawl_defaults` round-trip (`features/marketing/crawler/crawl-defaults.ts`, full contract + regex validation + full settings editor), active-crawl-aware `/crawls` (live duration, Cancel on rows/detail, Start→"Open live crawl"), cancel-to-terminal before deleting an active session or its site, and the `urls_classified`/`page_captured` event-vocabulary fix. Server batch (one-active-full-crawl guard, `CrawlStartRequest` regex 422, tldextract registrable domain, transient-failure page-state fix, deleted_at-reviving upserts) is being implemented in aidream `packages/matrx-scraper` — verify + deploy before closing. Still open after that: discovery inbox 500-cap/bulk actions · brand-asset file-backed rendering + single-primary · brand/site soft-delete leaves live `property(kind='website')` rows.
