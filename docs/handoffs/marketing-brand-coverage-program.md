...
7. **Crawl robustness batch (parking-lot class, board bullets have detail):** one-active-crawl-per-site guard + timestamp-monotonic current pointers · running crawl unmanageable after leaving launch screen (no reattach/cancel) · deleteSite/deleteCrawlSession can hide active work without canceling it · invalid include/exclude regexes accepted then silently skipped · transient fetch failures marking pages `missing` · lossy `crawl_defaults` round-trip · `_registrable_domain()` naive two-label logic (use tldextract) · discovery inbox 500-cap with no bulk actions · brand-asset file-backed rendering + single-primary enforcement · brand/site soft-delete leaves live `property(kind='website')` rows.
