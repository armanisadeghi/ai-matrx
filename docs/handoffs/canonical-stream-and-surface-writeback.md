...
**Findings for the owner (not fixed here).** (i) The global sweep's skip is
invisible in the UI — the one thing WP5 added to the sweep is the one thing an
operator cannot see. (ii) There is no scoped sweep, so the recency skip cannot
be exercised live without a platform-wide deletion; a `surfaceNames` filter on
`applyManifestSync` would make this verifiable and would also give admins a
lever matching the dialog's own "READ THIS LIST BEFORE ACTING" warning.
(iii) The sync toast omits URL-pattern changes.
