...
1. **Raw-table sweep — 5 files left, all `(transitional)`** (was 92; swept 2026-08-09). The treatment is now a primitive: **`components/official/mobile-table/mobileTable.ts`** — `MOBILE_TABLE_FROZEN` is ONE class on the `<table>` and does the whole job; granular constants for tables that freeze a non-first column. Read its header before touching any table: it encodes the three CSS traps AND the two design rules the sweep learned the hard way — **the frozen cell paints its own opaque background** (never `bg-inherit` + an opaque row: variant tints out-specify that and bleed, plain tints lose and silently kill the selected-row affordance), and **freeze the IDENTITY column, not column 1** when column 1 is an index/rank/kind.
