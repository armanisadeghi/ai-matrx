...
> **Objective gate:** `iam.verify_canonical_ok('<schema>','<table>','<token>'[,'<variant>'])` → `true` (target: zero WARN).
...
- `table_ref IS NULL` = the table is gone (renamed pre-trigger or dropped): the defined **needs-reconcile/retire** signal, not an anomaly — query for it.
...
**`organization_id` is set, never hand-NULLed.** Child entities inherit the parent's org via `platform.inherit_org_from_parent()` (BEFORE-INSERT trigger; `TG_ARGV` = parent schema/table/fk col) — if unresolved it falls through to the `NOT NULL` constraint, which fires loudly, never a silent NULL. Root entities take org from active context; un-scoped/legacy rows fall back to the creator's personal org (`iam.organizations.is_personal`), never the system org (genuine global content is the deliberate exception — §6e).
...
### 6e. System/global scope (orthogonal to visibility)
- **Global content** = owned by a `global_readable` system org (`iam.system_orgs`); `has_access` then grants every authenticated user read at `visibility ≥ internal`. There is **no per-row `is_system` access flag** — `is_system` / `builtin` is descriptive only.
- Otherwise **org-scoped**: `organization_id` + `iam.has_org_access`.
...
- **Gate:** `iam.verify_canonical_ok('<schema>','<table>','<token>'[,'<variant>'])` → `true`, zero WARN. The optional 4th `variant` (`entity`/`component`/`ledger`, same as `apply_rls`) makes the check RLS-aware; pass the table's variant. `iam.verify_canonical` shares the signature.
