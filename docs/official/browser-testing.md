...
- 🚨 **Never run `pnpm install` while the preview is running.** It kills the server for everyone, and `.pnpmfile.cjs` now refuses it — `install`, `add`, `remove`, `update`, even `--ignore-scripts`; the root `preinstall` script it replaced was useless because pnpm runs that AFTER linking (proof and measurements in `scripts/agent-harness/install-gate.cjs`). Two installs also cannot run at once here: the second waits, then refuses naming the first's pid. To refresh the lockfile while the preview runs, `pnpm install --lockfile-only` is always allowed; `MATRX_ALLOW_INSTALL_WITH_PREVIEW=1` overrides, loudly. And if the server does die with node_modules changed underneath it, it now says so in one line in its log, its lease file and `pnpm preview:status` instead of leaving you a `RangeError` stack. Measured 2026-09-12: an install relinked `@ai-matrx/design-system` for a few seconds; 1,268 files import it (the root layout imports its CSS), so Turbopack reported a module-not-found for every one of them with full import traces — 3,565,305 trace lines, ~460 MB in ONE compile. Serialising that to the browser overflows V8's string limit, the dev server dies with `RangeError: Invalid string length` as an uncaughtException, and every signed-in session goes with it. Coordinate with the owner, then `pnpm preview:stop` → `pnpm install` → `pnpm preview:start`. The dev log is also capped now (`MATRX_PREVIEW_MAX_LOG_GB`, default 2 GB) because that storm wrote 1.46 GB in minutes; a rotation announces itself in the log.
...
1. **`node_modules` in a worktree is a symlink to the primary checkout, and Turbopack refuses to serve it** ("Symlink `[project]/node_modules` is invalid" — it refuses anything outside the project root). `pnpm preview:start` now detects this itself and replaces the symlink with a same-device hard-link copy (`cp -al`, seconds, no extra disk) before starting Next — it announces what it found and what it did; you do not need to do this by hand.
