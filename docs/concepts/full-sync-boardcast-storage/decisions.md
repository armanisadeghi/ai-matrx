...
- **Phase 10** — RSC `'use cache'` policy for public content. Audit every server fetch; classify as public-cacheable or user-specific-via-sync-engine. *(2026-09-14: `'use cache'` is unavailable here — see the D6 note; public-cacheable means `unstable_cache` + tags.)*
