...
- **Missing a route: 12** ⚠️ `tool_bundle_listing` plus the 11-kind `fs_*` cluster (`file_read_result`, `file_write_result`, `directory_listing`/`directory_entry`, `file_search_results`/`file_search_match`, `file_edit_result`, `file_patch_result`/`file_edit_applied`/`file_edit_failure`, `directory_create_result`) — all added 2026-08-23 by the aidream tools family sweep. Every OTHER active non-contract-artifact kind resolves a registered `(kind,'web','output')` component. **This ledger is no longer complete, and it will keep growing:** the tools sweep mints result kinds for 279 tools, and each one that lands here needs a route. Treat new rows from `aidream/docs/workflow/KIND_TOOL_LEDGER.md` as the expected inflow, not as regressions.
...
| `directory_create_result` | Directory Create Result | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_mkdir. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `directory_entry` | Directory Entry | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: nested in `directory_listing`. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `directory_listing` | Directory Listing | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_list. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_edit_applied` | Applied File Edit | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: nested in `file_patch_result`. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_edit_failure` | Failed File Edit | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: nested in `file_patch_result`. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_edit_result` | File Edit Result | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_edit. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_patch_result` | File Patch Result | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_patch. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_read_result` | File Read Result | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_read. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_search_match` | File Search Match | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: nested in `file_search_results`. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_search_results` | File Search Results | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_search. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `file_write_result` | File Write Result | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 2, the `fs_*` family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`). Producer: fs_write. Registered and resolving live, verified end-to-end through a real tool dispatch; **activation BLOCKED** on a `(kind,'web','output')` row. `generic_structured` is very likely the right explicit basic route. |
| `tool_bundle_listing` | Tool Bundle Listing | placeholder | 1 | — | **claimed** | tools-reconcile-01 | NEW 2026-08-23 from the aidream TOOLS FAMILY SWEEP (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-01`) — the first tool-result kind. Registered and resolving live, but **activation is BLOCKED**: no active `(kind,'web','output')` row, so it reaches the reader through the platform floor. Shape is five keys (`bundle`, `tools_loaded[]`, `count`, `skipped_unresolved[]`, `server_slug`); `generic_structured` is very likely the right explicit basic route — it needs registering, not designing. Produced by all 43 `bundle:list_*` tools. The registry re-versioned, so `pnpm shape:types --all-generated` is owed. |
