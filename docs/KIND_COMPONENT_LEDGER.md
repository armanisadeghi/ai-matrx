...
| `directory_create_result` | Directory Create Result | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `directory_entry` | Directory Entry | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `directory_listing` | Directory Listing | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
...
| `field_problem` | Field Problem | distilled | 1 | `generic_structured` | **done (route)** | kinds-B3 | The nested child of `node_error` — one bad field, sectioned not flattened (§4.4). explicit basic route LIVE ([`content_ir_node_error_output_routes.sql`](../migrations/content_ir_node_error_output_routes.sql), 2026-08-23) and the kind is ACTIVE (dual gate satisfied: canonical example + registered route). Renders by recursion through the registry from its parent; a bespoke row-level view rides the same upgrade as `node_error`. |
| `file_edit_applied` | Applied File Edit | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_edit_failure` | Failed File Edit | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_edit_result` | File Edit Result | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_patch_result` | File Patch Result | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_read_result` | File Read Result | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_search_match` | File Search Match | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_search_results` | File Search Results | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `file_write_result` | File Write Result | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
...
| `markdown` | Markdown | distilled | 1 | `markdown_stream` | **done** | army-fe-wd2 | THE STREAMING MARKDOWN RENDERER, registered. `(markdown,'web','output') → markdown_stream` LIVE ([`content_ir_markdown_kind_route.sql`](../migrations/content_ir_markdown_kind_route.sql)); `role='input'` D1 floor row registered too (family `primitive`, not a machine contract). Verified in the browser on `/shapes/markdown/test` (2026-08-20). Kind was `is_active=false` at registration (correct then — activation is a separate governed act); ACTIVATED 2026-08-21 on Arman's instruction (dual gate satisfied, see the row's `activation_note`). |
| `node_error` | Node Error | distilled | 1 | `generic_structured` | **done (route)** | kinds-B3 | ERRORS ARE KINDS (KINDS_EVERYWHERE_PLAN §10d-B3). explicit basic route LIVE ([`content_ir_node_error_output_routes.sql`](../migrations/content_ir_node_error_output_routes.sql), 2026-08-23) and the kind is ACTIVE (dual gate satisfied: canonical example + registered route). **UPGRADE WARRANTED — this is the one row where the basic route is explicitly not the finish line:** B3's whole point is that a node failure renders through a real component instead of a raw dump. A failed node now ships a `node_outcome` wrapper with `status='failed'` and this nested under `error`, so the runtime-wrapper views are its natural home. Nests `field_problem`. Search done before registering: no web/output row and no compiled definition renders an engine failure; the closest, `run_failure`, is not registered as a kind. |
...
| `string_list` | String List | distilled | 4 | `generic_structured` | **done** | copy-D → text-reseed 2026-08-21 | explicit basic route; live + tested. OBJECT since the 2026-08-21 re-seed (v4 `{"items": [str]}` — the scalar array was undeclarable, audit break #4); route unchanged, now takes the block path (`kind-primitive-routes.test.tsx`). |
| `table_rows` | Table Rows | — | 1 | `generic_structured` | **done** | copy-D | explicit basic route; live + tested |
| `text` | Text | distilled | 4 | `generic_structured` | **done** | copy-D → text-reseed 2026-08-21 | explicit basic route; live + tested. OBJECT since the 2026-08-21 re-seed (v4 `{"text": str}`, mirroring `markdown`); route unchanged, now takes the block path (`kind-primitive-routes.test.tsx`). Declared by `text.join`/`data.stringify_json` specs. |
| `tool_bundle_listing` | Tool Bundle Listing | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | explicit basic route LIVE (`migrations/content_ir_tool_result_kind_routes.sql`) and **ACTIVATED** — `content_ir.set_kind_activation` returned `would_activate:true` with empty reasons for all 12 (they failed the RENDER leg only). Searched for reuse first: `features/files/components/core/FileTree` and `features/code/views/explorer` render cloud-file rows keyed by `file_id`, not a tool's `{path, entries[]}` payload — rejected as a new integration wearing a reuse costume. Maturity untouched (rule 9); a bespoke view is the distillation pass and is an UPGRADE of `component_key` on this row. Proof: `features/content-ir/__tests__/tool-result-kind-routes.test.tsx` (27 assertions over the LIVE canonical examples — before/after `by:'generic'` -> `by:'db'`, plus real render). |
| `tool_call_record` | Tool Call Record | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: nested in `tool_trace_call_detail` (`debug_traces_by_call`). Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_call_detail` | Tool Call Forensics | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: `debug_traces_by_call`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_event` | Tool Trace Event | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: nested in `tool_trace_event_page` / `tool_trace_call_detail`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_event_page` | Tool Trace Events | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: `debug_traces_recent`, `_failures_since`, `_by_conv`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_file` | Tool Trace File | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: nested in `tool_trace_file_listing`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_file_listing` | Tool Trace File Listing | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: `debug_traces_list_files`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_file_window` | Tool Trace File Window | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: `debug_traces_get_file`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_incident` | Tool Trace Incident | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: nested in `tool_trace_incident_list`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_incident_filter` | Tool Trace Incident Filter | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: nested in `tool_trace_incident_list`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_incident_list` | Tool Trace Incidents | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: `get_open_trace_incidents`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
| `tool_trace_incident_report` | Tool Trace Incident Report | placeholder | 1 | `generic_structured` | **done** | tools-reconcile-01 | NEW 2026-08-23 — aidream TOOLS FAMILY SWEEP batch 3, the trace-debugging family (`aidream/docs/workflow/KIND_TOOL_LEDGER.md`, agent `claude-tools-02`). Producer: `report_trace_incident`. Explicit basic route LIVE (`migrations/content_ir_tool_trace_kind_routes.sql`) and **ACTIVATED** in the same pass. Reuse searched: nothing here renders `cx_tool_trace` rows as a kind — the admin trace surfaces read the REST routes and draw their own tables, so they are not kind components. A bespoke timeline view for `tool_trace_event_page` is a good future UPGRADE of `component_key` on this row, not a second registration. Maturity untouched (rule 9). Proof: `features/content-ir/__tests__/tool-trace-kind-routes.test.tsx`. |
