...
const windowPanelsImportRestriction = {
  patterns: [
    ...
    {
      group: ["@/features/files/services", "@/features/files/services/*"],
      message:
  ...
  rules: {
    ...
    "no-file-url-refetch": {
      meta: {
        ...
        docs: {
          description:
            "Disallow resolving an owned file ID to a URL and then fetching that URL for bytes. Ask fileHandler for blob/array_buffer directly.",
        },
        schema: [],
        messages: {
          banned:
            'Do not resolve a file_id to html_src/fetchable_url and then fetch it. The ID is the identity and fileHandler owns authorization: request {kind:"blob"} or {kind:"array_buffer"} directly. Public CDN URLs are selected inside the canonical media primitives, never by this caller.',
        },
      },
      create(context) {
        const resolvedFileUrlNames = new Set();
        const unwrap = (node) => {
          let current = node;
          while (
            current &&
        ...
        const isFileUrlResolution = (node) => {
          ...
          return target.properties.some((property) => {
            ...
            return (
              key === "kind" &&
              (value === "html_src" || value === "fetchable_url")
            );
          });
        };
        const isTrackedFetchArgument = (node) => {
          const value = unwrap(node);
          return (
            isFileUrlResolution(value) ||
            (value?.type === "Identifier" &&
              resolvedFileUrlNames.has(value.name))
          );
        };
        return {
          VariableDeclarator(node) {
            if (
              ...
              isFileUrlResolution(node.init)
            ) {
              resolvedFileUrlNames.add(node.id.name);
            }
          },
          AssignmentExpression(node) {
            if (
              node.left?.type === "Identifier" &&
              isFileUrlResolution(node.right)
            ) {
              resolvedFileUrlNames.add(node.left.name);
            }
          },
          CallExpression(node) {
            if (
              ...
              node.arguments?.[0] &&
              isTrackedFetchArgument(node.arguments[0])
            ) {
    ...
    "no-bespoke-stream-renderer": {
      ...
      create(context) {
        ...
        const BANNED_NAMES = new Set(["useLiveJsonRegion", "openParseSession"]);
      ...
      create(context) {
        ...
        return {
          CallExpression(node) {
            ...
            if (!arg || arg.type !== "Literal" || arg.value !== "definition") {
              return;
            }
    ...
    "no-bare-id-text": {
      ...
      create(context) {
        ...
        const rootName = (expr) => {
          ...
          return cur && cur.type === "Identifier" && cur !== expr
            ? cur.name
            : null;
        };
        ...
        const terminalName = (expr) => {
          ...
          if (expr.type === "ChainExpression")
            return terminalName(expr.expression);
          return null;
        };
...
export default [
  ...
  {
    // This file defines the restricted selector and diagnostic strings, so
    // linting the configuration against its own literal bans is a false
    // positive. Runtime source files remain covered by the global rule.
