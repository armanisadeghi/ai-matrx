...
1. The nonce handshake — no credential in any URL. Run `pnpm dev-login /<route>` in this
   checkout and open the URL it prints; it lands you signed in on the route you name and
   the nonce dies on use. The URL is on YOUR session's hostname (`<label>.localhost:3001`),
   never `localhost` — cookies are per host, not per port, so every agent that opens
   `localhost` shares one session and evicts the others. The nonce file is per hostname for
   the same reason. `?token=` was REMOVED and 401s — a durable token in a URL leaks into
   history, logs and transcripts.
