...
| Boundaries | `ErrorBoundaryView.tsx` (all route `error.tsx` delegate here), `app/global-error.tsx` | Explicit chunk-load failure → calm “couldn’t finish loading” prompt with Refresh button. Non-chunk errors → normal error UI + Error Inspector capture. |
