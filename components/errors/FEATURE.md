...
- **Guard** — `__tests__/error-renders-carry-alchemy.test.ts` over `__tests__/error-display-census.ts`: the JSX is parsed and each ELEMENT judged. An error display is any `role="alert"` box (any colour), red-styled text rendering an error value or a failure sentence, a failure sentence in any colour, or an amber / `role="status"` notice rendering an error value. It must carry the menu inside its own box (a direct sibling `ErrorAlchemyMenu` counts for a heading row; an unrelated carrier elsewhere never does). Self-tests replay each verify probe. The baseline is empty and stays empty. Fix a new one with `<ErrorNotice error={…}>` or `<ErrorAlchemyMenu error={…} />` inside the box.
- **Payload facts the sentence drops** — the menu attaches the structured errors the Error Inspector captured on the same route in the last two minutes (`matchCapturedErrors`, newest first). A capture is pinned as the cause (`error.code/status/relation`, `captured_errors`) ONLY when its relation is one of the box's declared `calls` (`ErrorNotice`/`ErrorAlchemyMenu` prop) — never because the sentence contains a table's name; the rest are listed as `recent_unmatched_errors` with a note. Same-origin `/api/*` failures are captured too (`lib/diagnostics/captureAppApiFetch.ts`, source `app-api-http`, orange). A menu with no `input` reads the box it sits in (`errorRootFor`), block by block (`readRenderedError`: paragraphs become sentences, a short first block is the title).
- **Baseline ceiling** — `MAX_BASELINE_TOTAL` (0) in the census test, plus a comparison against the baseline committed at HEAD: adding a baseline entry fails. Raising the ceiling is a reviewed edit to the test, never a JSON change.
- **Declared calls are retained** — for each declared call, the latest failure of that call on the route is offered however old, however many requests failed after it; other failures are capped at five recent.
- **Orphan guard** — a DOM-read menu outside any error display and any error branch renders when nothing failed; the census test fails on one.
- **Route crashes** — `app/(core)/error.tsx` sends every (core) segment without its own boundary to `ErrorBoundaryView` (shell kept, crash captured, menu); `app/global-error.tsx` carries the menu too.

## Change Log

- 2026-09-26 — RC-B12 round 5: an error handed by prop to a component that does not draw the menu (`<EmptyState description={error}/>`, `<EmptyCatalogue message={`…${error}`}/>`), an error constant (`ORGANIZATION_UNAVAILABLE_TITLE`), and "page crashed" / "server refused" are displays; scale-0, `display:none` and `{false && …}` menus are absent. Components whose own render draws the menu carry every error handed to them — resolved across imports and re-exports (`__tests__/error-census-carriers.ts`). OrganizationRequiredNotice's could-not-check state (73 pages) and SkillConfigPicker render through ErrorNotice; 13 failure components draw the menu themselves; generic notices take a `menu` node or the menu at the call site. Mobile /tasks and /podcast no longer turn a failed read into an empty screen. `humanizeBackendError` cuts only at section headers (`Query:`), never the word "query".
- 2026-09-26 — RC-B12 round 4: the menu's DOM read never takes an ancestor that is not itself an error box (role=alert, a declared ErrorNotice/ErrorBox root, or a red/destructive box) — a short strip copies short, and a short line climbs at most three levels, only into an error card (the /crm, /crm/chasebox and /organizations leaks). Copied sentences never double a full stop ("again.."), a real ellipsis stays. One error box carries one menu (findDoubleMenus guard; 24 doubles removed). Census adds pink/fuchsia, "Oops"/"didn't work", a destructive Badge rendering the error value (status chips are labels, not errors), zero-size and hidden-parent menus as not carrying, and a menu beside a role=alert box as not carrying it. Scheduling tasks and /education/creator no longer swallow a failed read; a re-scoped Supabase client (`.schema(a).schema(b)`) is captured.
- 2026-09-26 — RC-B12 round 3: detector adds "Template Error / Failed to compile / Permission denied / timed out", red "Error: {…}", reason/detail names, orange error text, any error value in an error branch, and hover-less opacity-0 menus as hidden; one box spans plain wrappers in a destructive card; id/digest lines are references. Baseline ceiling. Declared calls retained per call. Profile panels declare their /api calls.

- 2026-09-26 — RC-B12 round 2: detector counts message-like values, error-branch renders, inline red, and "Unable to / Error loading / Save failed"; conditional classes never make a box; orphan-menu guard; pin only declared `calls`; block-aware text; /api failures captured.

- 2026-09-26 — RC-B12 verify reopen: the census is semantic and per element (1141 uncarried displays in 879 files → 0), the payload borrows the captured network error, `(core)/error.tsx` added, named screens (chat directives, StaleDataNotice, AccessDenied, /data tables, profile and four private ErrorPanels) moved onto the primitive.
