...
- **Payload facts the sentence drops** — the menu attaches the structured errors the Error Inspector captured on the same route in the last two minutes (`matchCapturedErrors`: relation named in the sentence first, then same words, then recency). One certain match fills `error.code/status/relation`; several equally likely ones are listed under `captured_errors`, never pinned. A menu with no `input` reads the box it sits in (`errorRootFor`), or the small card a short title heads.
