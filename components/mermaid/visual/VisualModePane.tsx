...
export function VisualModePane({
  source,
  options,
  doc,
  selection,
  ...
  const select = (sel: EditorSelection | null) =>
  ...
  const handleSvgMounted = (svg: SVGSVGElement | null) => {
    ...
    applySelection(
      svg,
      selection?.kind === "edge"
        ? edgePairKey(flowDoc, selection.id)
        : (selection?.id ?? null),
    );
    ...
    return (
      <div className="flex h-full flex-col">
        ...
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          Tap-to-edit is available for flowcharts. Use Outline or Code mode to
          edit this diagram.
        </p>
      </div>
    );
  ...
  const selectedNode =
    selection?.kind === "node"
      ? flowDoc.nodes.find((n) => n.id === selection.id)
      : null;
  const selectedEdge =
    selection?.kind === "edge"
      ? flowDoc.edges.find((e) => e.id === selection.id)
      : null;
    ...
    popoverAt && (selectedNode || selectedEdge) ? (
      selectedNode ? (
        <NodeActions
          ...
          onRename={(label) =>
            apply({ type: "renameNode", id: selectedNode.id, label })
          }
          onAddConnected={() => {
            apply({
              type: "addNode",
          ...
          onShape={(shape) =>
            apply({ type: "setNodeShape", id: selectedNode.id, shape })
          }
          onPalette={(key) =>
            apply({
              type: "setNodePalette",
        ...
        <EdgeActions
          ...
          onLabel={(label) =>
            apply({ type: "setEdgeLabel", id: selectedEdge.id, label })
          }
          onReverse={() => apply({ type: "reverseEdge", id: selectedEdge.id })}
          onStyle={(style) =>
            apply({ type: "setEdgeStyle", id: selectedEdge.id, style })
          }
          onDelete={() => {
  ...
  return (
    <div ref={containerRef} className="relative flex h-full flex-col">
      ...
      {!interactive && (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          Visual editing is unavailable for this render — use Outline or Code
          mode.
        </p>
      )}
      ...
      <SimpleTooltip text="Add step">
        <button
          ...
          className="absolute bottom-4 left-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-md transition-transform hover:scale-105"
        >
        ...
        {Object.entries(FLOW_PALETTE).map(([key, palette]) => (
          <SimpleTooltip key={key} text={palette.label}>
            <button
              ...
              onClick={() =>
                props.onPalette(props.paletteKey === key ? null : key)
              }
              className={cn(
                "h-6 w-6 rounded-full border-2",
                props.paletteKey === key
                  ? "border-foreground"
                  : "border-transparent",
              )}
