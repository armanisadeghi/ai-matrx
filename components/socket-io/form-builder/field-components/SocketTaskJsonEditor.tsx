...
import {
  formatLabel,
  formatPlaceholder,
} from "@/components/socket/utils/label-util";
...
const SocketTaskJsonEditor: React.FC<SocketTaskJsonEditorProps> = ({
  ...
  const reduxValue = useAppSelector((state) =>
    selectFieldValue(taskId, fullPath)(state),
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      (reduxValue === undefined || reduxValue === null)
    ) {
  ...
  const Icon = (props: any) => (
    <DynamicIcon
      name={fieldDefinition.ICON_NAME}
      fallbackIcon="File"
      {...props}
    />
  );
  const placeholder = showPlaceholder
    ? fieldDefinition.DESCRIPTION || formatPlaceholder(fieldName)
    : "";
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <textarea
        ...
        spellCheck="false"
        {...Object.fromEntries(
          Object.entries(finalProps).filter(([key]) => key !== "className"),
        )}
      />
      {notice && <span className="text-yellow-600 text-sm">{notice}</span>}
      {jsonError && (
        <span className="text-red-500 text-sm">
          Invalid JSON format - click Auto-fix to attempt repair
        </span>
      )}
      {warnings.length > 0 && (
