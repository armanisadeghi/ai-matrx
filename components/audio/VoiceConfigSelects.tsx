...
const BASE_EMOTIONS: BaseEmotion[] = [
  "anger",
  "positivity",
  "surprise",
  "sadness",
...
export function VoiceConfigSelects({
  ...
  const [selectedEncoding, setSelectedEncoding] =
    useState<AudioEncoding>(DEFAULT_ENCODING);
  ...
  const parseEmotion = (
    emotion: Emotion,
  ): { base: BaseEmotion; intensity: string } => {
  ...
  return (
    <div className="space-y-6 w-full">
      {/* canonical-model-picker-exempt: Cartesia wire-model enum, not an ai.model_definition id */}
              ...
              {isEmotionSelected(baseEmotion) && (
                <Select
                  ...
                  onValueChange={(intensity) =>
                    handleIntensityChange(baseEmotion, intensity)
                  }
                  disabled={isPlaying}
                >
                  ...
                  <SelectContent>
                    {Object.entries(INTENSITY_DISPLAY_NAMES).map(
                      ([value, name]) => (
                        <SelectItem
                          key={`${baseEmotion}-${value}`}
                          value={value}
                        >
                          Intensity: {name}
                        </SelectItem>
