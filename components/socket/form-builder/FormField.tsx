...
const FormField: React.FC<FormFieldProps> = ({
  ...
  const getComponentType = (): FieldType => {
    if (field.COMPONENT) {
      ...
      if (VALID_COMPONENTS.includes(componentLower)) {
        ...
        console.warn(
          `Component type "${field.COMPONENT}" is not supported. Falling back to default.`,
        );
      }
    }
    ...
    if (
      ...
      (field.DATA_TYPE === null && typeof field.DEFAULT === "boolean")
    ) {
      return "switch";
    }
    ...
    return (
      <div className="w-full">
        <div className="grid grid-cols-12 gap-2 mb-2">
          <div className="col-span-1 text-slate-700 dark:text-slate-300 font-medium">
            {formatLabel(fieldKey)}
          </div>
          <div className="col-span-11">
            <div className="border-l border-slate-200 dark:border-slate-700 pl-4">
              {value.map((item: any, index: number) => (
                <div key={`${fullPath}[${index}]`} className="relative">
                  {Object.entries(field.REFERENCE).map(
                    ([nestedKey, nestedField]) => (
                      <FormField
                        ...
                        value={
                          item?.[nestedKey] ??
                          (nestedField as SchemaField).DEFAULT
                        }
                        errors={errors}
                        notices={notices}
                        formData={formData}
              ...
              {Object.entries(field.REFERENCE).map(
                ([nestedKey, nestedField]) => (
                  <FormField
                    ...
                    path={fullPath}
                    value={
                      value?.[nestedKey] ?? (nestedField as SchemaField).DEFAULT
                    }
                    errors={errors}
                    notices={notices}
                    formData={formData}
  ...
  const Icon = (props: any) => (
    <DynamicIcon name={iconName} fallbackIcon="File" {...props} />
  );
  const placeholder = field.DESCRIPTION || formatPlaceholder(fieldKey);
            ...
            <Slider
              ...
              {...Object.fromEntries(
                Object.entries(componentProps).filter(
                  ([key]) =>
                    !["className", "min", "max", "step", "range"].includes(key),
                ),
              )}
            />
          </div>
      ...
      case "select":
        return (
          <div className="flex flex-col gap-2">
            ...
            {notice && (
              <span className="text-yellow-600 text-sm">{notice}</span>
            )}
          </div>
        );
      case "radiogroup":
      ...
      case "jsoneditor":
        ...
        return (
          <div className="flex flex-col gap-2">
            ...
            {jsonError && (
              <span className="text-red-500 text-sm">Invalid JSON format</span>
            )}
          </div>
        );
      case "textarea":
