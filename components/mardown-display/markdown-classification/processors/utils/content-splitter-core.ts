 ...
 *
 * THIS module is the environment-neutral core (RC-B2b, 2026-09-24): it never
 * imports the kind/component registries — those arrive as SplitterEnvelopes.
 * Client code imports content-splitter-v2.ts (hooks bound); the server
 * rich-content level calls splitContentIntoBlocksWith(src, NO_SPLITTER_ENVELOPES).
 */
...
// ============================================================================
// ENVELOPE HOOKS — the ONLY coupling to the kind registries
// ============================================================================

/**
 * The three kind-envelope hooks the splitter calls. They attach the canonical
 * content-IR envelope to a COMPLETED region's `metadata` — they never change a
 * block's type, boundaries or content. The client entry
 * (content-splitter-v2.ts) binds the registry-backed hooks; the server
 * rich-content level passes NO_SPLITTER_ENVELOPES, so this module's graph
 * stays free of the kind/component registries (which create React contexts
 * at module scope and cannot load in a React Server Component).
 */
export interface SplitterEnvelopes {
  withIrEnvelope(
    source: string,
    metadata: Record<string, unknown> | undefined,
  ): Record<string, unknown> | undefined;
  fenceRegion(language: string, regionText: string): CanonicalBlockIR | null;
  xmlRegion(tag: string, regionText: string): CanonicalBlockIR | null;
}

/** No envelopes: identical blocks, no kind metadata (server rendering). */
export const NO_SPLITTER_ENVELOPES: SplitterEnvelopes = {
  withIrEnvelope: (_source, metadata) => metadata,
  fenceRegion: () => null,
  xmlRegion: () => null,
};

/**
 * SplitterBlock — the output type of splitContentIntoBlocksWith / splitContentIntoBlocksV2.
 *
 ...
 */
export function recoverEmbeddedKindJsonBlocksWith(
  input: SplitterBlock[],
    ...
    for (const piece of pieces) {
      if (piece.type === "kind") {
        ...
        recovered.push({
          ...
          content: piece.content,
          language: "json",
          metadata: envelopes.withIrEnvelope(piece.content, undefined),
        });
        continue;
      }
...
export const splitContentIntoBlocksWith = (
  mdContent: string,
  envelopes: SplitterEnvelopes,
): SplitterBlock[] => {
          ...
          blocks.push({
            ...
            metadata:
              jsonType === "matrx"
                ...
                : envelopes.withIrEnvelope(extraction.content, streamingState.metadata),
          });
        } else {
          ...
          if (jsonType) {
            ...
            blocks.push({
              ...
              metadata:
                jsonType === "matrx"
                  ? streamingState.metadata
                  : envelopes.withIrEnvelope(jsonContent, streamingState.metadata),
            });
          } else {
            blocks.push({
              type: "code",
              content: jsonContent,
              language: "json",
              metadata: envelopes.withIrEnvelope(jsonContent, undefined),
            });
          }
          i = j;
          continue;
  ...
  return recoverEmbeddedKindJsonBlocksWith(blocks, envelopes);
};
