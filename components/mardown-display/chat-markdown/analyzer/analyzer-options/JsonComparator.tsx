...
export default function JsonComparator() {
  ...
  const handleCompare = () => {
    if (!leftParsed || !rightParsed) {
      setComparison({
        identical: false,
        error: "Both sides must contain valid JSON",
      });
      return;
    }
  ...
  const renderDifference = (diff, index) => {
    ...
    return (
      <div
        key={index}
        className="border-l-4 border-red-500 pl-3 py-2 mb-2 bg-red-50 dark:bg-red-950/20"
      >
        ...
        {diff.rightValue !== undefined && (
          <div className="text-sm">
            Right: {JSON.stringify(diff.rightValue)}
          </div>
        )}
