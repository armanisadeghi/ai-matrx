...
- 2026-08-30 — `SortableList` row controls take ZERO layout space: the
  ...
  three always-visible 28px icon buttons that burned ~90px of every row's
  right edge. Coarse-pointer devices (no hover) keep a compact always-visible
  inline cluster with full-size tap targets. Every DB-authored component
  using the kit inherits this with no code change.
