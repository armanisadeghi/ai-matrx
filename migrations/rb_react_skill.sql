...
INSERT INTO skill.definition (
  ...
  is_system, is_public, is_active, visibility,
  organization_id, user_id, project_id, task_id, sort_order
)
SELECT
  'react-component',
...
WHERE NOT EXISTS (
  ...
  WHERE skill_id = 'react-component' AND user_id IS NULL
);

-- Refresh the body/metadata on re-apply (global row: user/project/task NULL).
UPDATE skill.definition SET
  label = 'Live React Component',
  description = 'How and when to emit ```react (```jsx / ```tsx) blocks that compile and render as a live, interactive React component: the allowlisted imports/hooks/UI available, the no-props/default-export requirement, syntax rules that prevent compile failures, sizing guidance, and editing etiquette.',
  skill_type = 'render_block'::skl_skill_type,
  body = $BODY$# Live React Component
...
$BODY$,
  ...
  visibility = 'public',
  updated_at = now()
WHERE skill_id = 'react-component'
  AND user_id IS NULL
