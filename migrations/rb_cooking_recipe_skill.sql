...
$BODY$,
    ...
    true, true, true, 'public', '39c38960-d30c-4840-b0c1-c9960de95582'
WHERE NOT EXISTS (
    SELECT 1 FROM skill.definition
    WHERE skill_id = 'cooking-recipe' AND user_id IS NULL
);

-- Refresh body/metadata on re-apply (composite unique, so no ON CONFLICT).
UPDATE skill.definition SET
    label            = 'Cooking Recipe',
    description      = 'Emit an interactive, structured cooking recipe (ingredients checklist, step-by-step instructions, serving scaler, timers) as a ```cooking_recipe code fence.',
