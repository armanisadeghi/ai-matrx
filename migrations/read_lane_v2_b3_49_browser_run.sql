...
-- read_lane_v2_b3_49_browser_run — enroll browser.run in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'browser' and table_name = 'run' and is_active on conflict do nothing;
