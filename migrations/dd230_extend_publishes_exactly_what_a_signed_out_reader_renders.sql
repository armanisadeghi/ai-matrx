-- dd230_extend_publishes_exactly_what_a_signed_out_reader_renders
...
--   extend.wbx_capture → 4 column(s). the Chrome extension, which holds the publishable key and no account until its user signs in. Measured on production: select=id,url,captured_at,title with no JWT, real browser, no referer.
--
-- ═══ CLOSED — no signed-out reader exists, so the bound is the EMPTY list ═════════════════════
--   extend.wbx_demo — two 400s and one 401 in 24 h — no successful anon read.
--   extend.wbx_guidance — zero anon requests in 24 h.
--   extend.wbx_highlight — zero anon requests in 24 h.
--   extend.wbx_pattern — twenty-five anon requests in 24 h, every one a 401 already. The door was shut in practice; this closes it in the catalog.
--   extend.wbx_screenshot — zero anon requests in 24 h.
--   extend.wbx_seo_audit — zero anon requests in 24 h.
...
begin
  for r in
    select * from (values
      ('extend.wbx_capture', array['id', 'url', 'captured_at', 'title'])
    ) as t(rel, keep)
  loop
