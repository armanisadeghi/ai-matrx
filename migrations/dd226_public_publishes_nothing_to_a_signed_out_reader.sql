-- dd226_public_publishes_nothing_to_a_signed_out_reader
...
--   public.current_user_is_admin — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 2)); the grant itself is the finding.
--   public.pdf_unified_pages — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 21)); the grant itself is the finding.
--   public.v_context_item_suggestions — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 18)); the grant itself is the finding.
--   public.v_kg_alerts — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 17)); the grant itself is the finding.
--   public.v_kg_sweep_effectiveness — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 17)); the grant itself is the finding.
--   public.v_kg_value_matches — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 19)); the grant itself is the finding.
--   public.v_ner_canonicalizer_shadow — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 20)); the grant itself is the finding.
--   public.v_scope_suggestion_stats — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 3)); the grant itself is the finding.
--   public.v_scope_suggestions — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 31)); the grant itself is the finding.
--   public.v_scope_suggestions_new — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 17)); the grant itself is the finding.
--
...
begin
  for r in
    select unnest(array[
      ...
      'public.pdf_unified_pages',
      'public.v_context_item_suggestions',
      'public.v_kg_alerts',
      'public.v_kg_sweep_effectiveness',
      'public.v_kg_value_matches',
      'public.v_ner_canonicalizer_shadow',
      'public.v_scope_suggestion_stats',
      'public.v_scope_suggestions',
      'public.v_scope_suggestions_new'
    ]) as rel
  loop
