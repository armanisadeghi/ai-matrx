-- iam_personal_row_wall_dd165e2_batch3_part2 — REGENERATION BATCH 3 OF 4, PART 2 OF 3 (DD-165).
--
...
declare
  ...
  v_tokens text[] := array['approach','category','comment','custom_entity_definition','custom_field_definition','custom_field_target','domain_classification','flexible_data','guided_checklist_run','platform_outcome_event','platform_outsider_consumer','purpose','route_manifest_entry','rulebook','pc_article'];
begin
  ...
  loop
    ...
    exception when others then
      if sqlstate in ('P0001','42703','42883')
        ...
        raise notice 'dd165e2: % (%.%) keeps its bespoke policy — %',
          r.token, r.schema_name, r.table_name, sqlerrm;
      else
        raise exception 'dd165e2: % (%.%) failed with %: %. That is not a structural refusal, so '
  ...
  raise notice 'dd165e2: batch 3 part 2/3 — regenerated %, structurally refused %', v_ok, v_refused;
  if v_ok + v_refused < 15 then
    raise exception 'dd165e2: only % of 15 tokens were reached — a silent drop is exactly the '
