...
INSERT INTO skill.definition (
    ...
    disable_auto_invocation, platform_targets, version, category_id,
...
WHERE NOT EXISTS (
    ...
    WHERE skill_id = 'html-webpage' AND user_id IS NULL
);

-- Refresh the body/metadata of the global row on re-apply (idempotent update).
UPDATE skill.definition SET
    label = 'HTML Webpage',
    description = 'How and when to emit a ```html render block: a self-contained webpage deliverable that materializes into a persistent, sandboxed live preview. Covers the completeness rule that prevents fragments from rendering, sandbox/self-containment constraints, sizing, and editing etiquette.',
    skill_type = 'render_block'::public.skl_skill_type,
    body = $BODY$# HTML Webpage
