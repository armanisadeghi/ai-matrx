-- DD-192 — `coalesce(p_user_id, auth.uid())` is not a default, it is an identity swap.
--
-- Found by `pnpm check:door-rows` over the whole declared signed-in door
-- population (890 doors), calling each door as two real signed-in identities
-- with arguments naming a THIRD identity's rows.
--
-- THE CLASS. A `SECURITY DEFINER` function granted to `authenticated` that
-- starts `v_uid := coalesce(p_user_id, auth.uid())` and never checks the
-- argument has replaced the caller with whoever the argument names. The census
-- over every client-callable definer found FOUR functions doing it; ONE
-- (`platform.lifecycle_user_notice`) already carried the check. These are the
-- other three.
--
--   public.get_user_nav_tree(p_user_id)     — every organization the target
--       belongs to, their role in each, and every project inside.
--   public.get_user_full_context(p_user_id) — the same, plus the target's scope
--       types, scopes and context items.
--   public.hr_access_explain(p_user, ...)   — why another person can or cannot
--       reach a resource.
--
-- Proven live, in rolled-back transactions, as test@test.com (a plain member of
-- two organizations) and as a signed-in user who is a member of none, both
-- naming admin@admin.com: before, the nav tree came back with organizations
-- `304cd2ed-…` and `fcddc623-…`, which neither caller can SELECT under RLS.
--
-- The guard is not a new one. It is the wording `public.get_user_session_data`
-- and the rest of the `p_user_id` family already use, kept word for word, plus
-- the platform-admin arm those surfaces rely on.
...
AS $function$
begin
  -- 🚨 DD-192: same class. `coalesce(p_user, auth.uid())` let any signed-in
  -- caller ask why SOMEBODY ELSE can or cannot reach a resource — an access
  -- explanation is a description of another person's standing.
  if not (auth.role() = 'service_role'
          or coalesce(p_user, auth.uid()) = ( SELECT auth.uid())
          or public.is_platform_admin()) then
    raise exception 'access denied: caller is not the target user' using errcode = '42501';
  end if;
  return hr.access_explain(coalesce(p_user, auth.uid()), p_token, p_id);
end;
$function$
;
