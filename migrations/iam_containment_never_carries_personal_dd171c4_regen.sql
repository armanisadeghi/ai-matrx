-- iam_containment_never_carries_personal_dd171c4_regen — THE REGENERATION, PART 4 OF 4 (DD-171).
--
...
begin
  ...
  loop
    ...
    exception when others then
      ...
      if sqlstate in ('P0001','42703','42883')
        ...
        raise notice 'dd171c4: % (%.%) keeps its bespoke policy — %',
          r.token, r.schema_name, r.table_name, sqlerrm;
      else
        raise exception 'dd171c4: % (%.%) failed with %: %. That is not a structural refusal, so this '
  ...
  if v_seen = 0 then
    raise notice 'dd171c4: nothing was RED — every containment arm is already walled';
  else
    raise notice 'dd171c4: % token(s) were RED; regenerated %, structurally refused %',
