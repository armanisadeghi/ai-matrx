...
-- read_lane_v2_b1_02_runtime_global_execution — enroll runtime.global_execution in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b1' from platform.entity_types where schema_name = 'runtime' and table_name = 'global_execution' and is_active on conflict do nothing;
