-- dd186_anon_columns_admin — THE SIGNED-OUT COLUMN SURFACE OF THE `admin` SCHEMA
...
-- admin.admin_markdown_samples — revoked: created_by
revoke select on admin.admin_markdown_samples from anon;
grant select (id, name, description, content, detected_blocks, created_at, updated_at) on admin.admin_markdown_samples to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['admin.admin_markdown_samples']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (admin): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
