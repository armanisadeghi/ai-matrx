-- crm_10 — THE ONE SEND AUTHORITY learns the two facts callers were checking
-- for themselves (WP1 round 2, D13 + the DNC fold, 2026-08-15).
--
-- 1. THE ONE BLOCKLIST (IC-11, D13). `crm.blocklist_entry` is subject-scoped —
--    a domain, an address, or a party — written by WP2 and already enforced at
--    prospecting ingestion. This closes the send-side half: a blocked subject
--    is refused AT THE AUTHORITY, in both lanes, with the reason and its fix.
--    An org row is the customer's own list; a row under the system org
--    (39c38960-d30c-4840-b0c1-c9960de95582, the documented platform-global
--    tier) blocks everyone and outranks the org row. A row with a past
--    `expires_at` is a lapsed snooze, not a block — same rule as the Python
--    reader (aidream/services/crm/blocklist.py), which stays the ingestion-path
--    twin of this check.
--
-- 2. `crm.party.do_not_contact`. Until now the authority ignored it, and the
--    integration map carried a live-schema warning telling every caller to
--    check the flag themselves — WP1's runner and WP3's candidate promotion
--    both did. Closing the hole at the authority beats every caller
--    remembering; those caller-side guards are removed in the same change.
--
-- No new check takes an override, and the blocklist read here must stay
-- match-identical to the Python normalizer: lowercase, bare host, no trailing
-- dot, no `www.` (migration 0362's CHECKs guarantee stored subjects are
-- already in that form, so only the PROBE value needs normalizing here).
--
-- Everything else in the function body is byte-identical to crm_07 (verified
-- against the live definition via pg_get_functiondef before this replace).
...
declare
  ...
  v_accepted boolean;
  v_domain text;
  ble crm.blocklist_entry;
  v_ble_platform boolean;
  v_dnc_name text;
  v_dnc_reason text;
begin
  select * into cm
  ...
  -- 1b. THE ONE BLOCKLIST (crm.blocklist_entry, IC-11 / D13). Subject-scoped:
  -- the address itself, the domain after @, or any party this address belongs
  -- to. Both lanes. A platform-tier row outranks the org row so the message
  -- names the scope the customer cannot change.
  if cm.channel = 'email' then
    v_domain := rtrim(split_part(lower(cm.value_key), '@', 2), '.');
  ...
  select b.* into ble
    from crm.blocklist_entry b
   where b.deleted_at is null
     and (b.expires_at is null or b.expires_at > now())
     and b.organization_id in
       (cm.organization_id, '39c38960-d30c-4840-b0c1-c9960de95582'::uuid)
     and (
       (cm.channel = 'email' and b.subject_kind = 'email'
          and b.subject_value = lower(cm.value_key))
       or (b.subject_kind = 'domain' and v_domain is not null
          and b.subject_value = v_domain)
       or (b.subject_kind = 'party' and b.subject_value in (
             select lower(pcp.party_id::text)
               from crm.party_contact_point pcp
              where pcp.medium_id = cm.id and pcp.deleted_at is null))
     )
   order by (b.organization_id = '39c38960-d30c-4840-b0c1-c9960de95582'::uuid) desc
   limit 1;
  if ble.id is not null then
    v_ble_platform :=
      ble.organization_id = '39c38960-d30c-4840-b0c1-c9960de95582'::uuid;
    blocks := blocks || jsonb_build_object(
      'code', 'blocklisted',
      'message', ble.subject_value
        || case when v_ble_platform
             then ' is blocked across the whole platform'
             else ' is on your blocklist' end
        || coalesce(' — ' || nullif(btrim(ble.reason), ''), '') || '.',
      'fix', case when v_ble_platform
        then 'Nothing — subjects blocked at the platform level cannot be contacted.'
        else 'Remove it from your blocklist if this was a mistake.' end,
      'detail', jsonb_build_object(
        'subject_kind', ble.subject_kind,
        'subject_value', ble.subject_value,
        'scope', case when v_ble_platform then 'platform' else 'organization' end,
        'source', ble.source,
        'entry_id', ble.id));
  end if;

  -- 1c. party.do_not_contact. The person-level flag, folded into the authority
  -- so no caller needs its own guard (previously WP1's runner and WP3's
  -- candidate promotion each checked it; the integration map carried a warning
  -- for everyone else).
  select p.display_name, p.do_not_contact_reason
    into v_dnc_name, v_dnc_reason
    from crm.party_contact_point pcp
    join crm.party p
      on p.id = pcp.party_id and p.deleted_at is null
   where pcp.medium_id = cm.id
     and pcp.deleted_at is null
     and p.do_not_contact
   limit 1;
  if v_dnc_name is not null then
    blocks := blocks || jsonb_build_object(
      'code', 'do_not_contact',
      'message', v_dnc_name || ' asked not to be contacted'
        || coalesce(' — ' || nullif(btrim(v_dnc_reason), ''), '') || '.',
      'fix', 'Clear ''do not contact'' on the CRM record if it was set by mistake.');
  end if;
...
comment on function crm.check_send_eligibility(uuid, uuid, uuid) is
  'THE ONE SEND AUTHORITY. Every send path asks this and refuses on a block. MX/disposable verification, purchased-list detection, RFC 8058 DKIM coverage, consent, jurisdiction, suppression, the subject-scoped blocklist (crm.blocklist_entry), party.do_not_contact, sender readiness, postal address, and AUP are floor checks with a fix on every block. No caller-side blocklist or DNC check is needed.';
