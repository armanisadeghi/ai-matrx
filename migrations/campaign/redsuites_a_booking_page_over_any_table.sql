...
-- based-on: custom.booking_declare(uuid, uuid, text, jsonb, jsonb, jsonb, integer, uuid, uuid, uuid, text, uuid) fd5057fe0c1cd556d596879a6a4c2e1372738c2039ba5119aa45892ef00e0599
--
-- RED-SUITES — A BOOKING PAGE CAN BE PUT OVER A TABLE AGAIN.
--
-- `custom.booking_declare` fills in the three columns a booking page needs — `slot`, `status`
-- and `booked_with` — when the Table does not already have them, which is the whole point of
-- being able to put a booking page over "a Table that was made for something else". Each of
-- the three then appended its key to the local `text[]` of the Table's column names as
-- `v_keys := v_keys || 'slot';`.
--
-- With an untyped literal on the right, Postgres resolves `anyarray || anyarray` rather than
-- `anyarray || anyelement` and casts `'slot'` to `text[]`:
--     ERROR:  malformed array literal: "slot"
--     CONTEXT: PL/pgSQL function custom.booking_declare(uuid,uuid,text,…) line 46 at assignment
-- So the door raised a Postgres internals error, at the FIRST of the three, for every Table
-- that did not already carry all three columns. VERIFIER-8 Part C recorded it on `booking_green`
-- and `booking_red` together — both twins dying on one line is the tell that the defect is the
-- door's and not the suites'.
--
-- Three casts. Nothing else in the body moves.
