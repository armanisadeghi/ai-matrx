...
-- based-on: custom.capture_submit(uuid, uuid, text, jsonb, jsonb, text, timestamp with time zone, jsonb) b168305e86c3214ad916eadadfcf825a0a2d2f69b4f784217252d29252321921
--
-- LANE CAPTURE — THE SECOND DEFECT THE SUITE FOUND, and it would have cost a real capture.
--
-- `custom.capture_submit` wrote every file-answered field as a LIST of File record ids,
-- whatever the Field said. A Field that holds one value — `photo` on a bin — refuses a list
-- (*photo holds one value, and it was given a list*, FLD-2), and it refuses it at
-- `custom.record_write`, which is the LAST step: after the walk to the bin, after the
-- photograph, after the upload. The crew member would have watched a finished queue item
-- fail with a sentence about lists.
--
-- The door now reads the Field's own `multi` and writes what that Field holds — one id, or a
-- list of them — and refuses a second file for a single-valued field UP FRONT, by name,
-- while the phone can still do something about it.
--
