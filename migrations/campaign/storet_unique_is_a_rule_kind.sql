-- chair-step: it replaces `custom._field_shape_guard`, a BEFORE trigger on custom.record, to
--   add ONE word to its closed set of rule kinds — `unique`. The body does reach
--   `custom/system_enabled`: its first statement is `perform custom.assert_store_door(...)`,
--   which resolves that knob. The static check is a LITERAL string search over the stripped
--   SQL and cannot follow a call, so it sees no knob (the identical situation
--   `storerel_the_delete_rules_land.sql` is a chair step for). Writing a second, redundant knob
--   read into a trigger that already asks the one door predicate would be a mention rather than
--   a switch. Otherwise additive in effect: it carries its `-- based-on:` line, nothing is
--   dropped, nothing is granted or revoked, no row of any feature is deleted or rewritten, and
--   the only behaviour change is that a rule kind which used to be REFUSED is now accepted —
--   carried out by custom._unique_rule_holds, which the file beside this one creates.
-- based-on: custom._field_shape_guard() 8232769a705903723d0f46726edc2454184381d8d8729a477a53237731fbda44
--
-- STORE-T / B1 — `unique` IS A RULE KIND THIS STORE CAN EXECUTE.
--
-- Measured 2026-09-20 from a signed-in seat: asking for a column to be unique was refused at
-- declaration time — "the field Code carries a rule of kind unique, which this validator cannot
-- execute" — so there was no uniqueness anywhere to test and two sessions writing the same
-- value both won. Uniqueness is not a judgement a shape guard can make: it sees ONE row, and
-- uniqueness is a statement about the others. So the word is accepted here and carried out by
-- `custom._unique_rule_holds` (storet_a_choice_is_a_word_and_unique_means_unique.sql), which is
-- a trigger and can take the transaction-scoped lock that makes it true under concurrency.
