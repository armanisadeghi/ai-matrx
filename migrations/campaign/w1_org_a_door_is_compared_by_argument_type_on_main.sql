-- chair-step: this REPLACES a live body, platform._provision_shape_settled(), whose door_orphaned arm compares two RENDERED signature strings and so reports a LIVE function as dropped whenever a door row names a type outside pg_catalog — an enum, a domain, a composite. The rehearsed file is headed  only because the night it was written was branch-only; the owner's 2026-09-18 ruling replaced that, and it says in its own words that these exact bytes are the production repair. These ARE those bytes. A person reads the whole body.
