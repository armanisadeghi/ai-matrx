...
-- based-on: custom.field_update(uuid, uuid, jsonb) 26bcd98dfb53fda0ca253baf6341d626bd35a24ae84a615d5a7b3107748dfeca
--
-- RED-SUITES-2 — THE COLUMN DOOR APPLIES THE TWO KEYS IT WAS STILL THROWING AWAY.
--
-- MEASURED on the MAIN database on 2026-09-21, from the seat `authenticated`, through
-- `scripts/campaign-tests/w1_rel_c12.sql` REL-2 / T7 once that suite was moved INTO the seat:
--
--   custom.field_update(org, f_owner, '{"on_target_delete":"restrict"}')  ->  returns the id
--   platform.relation_declaration(org, f_owner) ->> 'on_delete'            ->  "set_null"
--
-- The door answered "saved" and the column had not moved. `on_target_delete` and
-- `relation_target` are both declared on the PUBLISHED contract — `FieldPatch` in
-- `@ai-matrx/records` `src/field.ts` — and both were honoured only by this door's BEHAVIOUR
-- arm, which runs only when the same patch also carries `type` / `plain` / `parity_type`. A
-- person changing only "what happens when the thing this points at is deleted" sends none of
-- those, so the SETTINGS arm ran, and the settings arm's key list never carried either one.
--
-- IN THE PRODUCT: the column editor's delete-rule control and its change-the-table control
-- both save successfully and change nothing. The second is worse than the first — a caller who
-- believed the column had been re-pointed would have every edge still naming the old table.
--
-- THE CLASS, AND ITS CENSUS: this is the fifth time this one door has been found accepting a
-- documented key and discarding it, and the previous four are written into the body itself —
-- `promoted` / `unique` (SEAT-SUITES, 2026-09-19), `multi` (FIX-7B-FIELD, 2026-09-20),
-- `source` / `review_interval_days` (ENRICH, 2026-09-20), `compute_on` (TAILS-2, 2026-09-21).
-- Measuring `FieldPatch` against the settings arm's key list, THESE TWO ARE THE LAST OF THEM:
-- after this file every key the published contract declares is either applied or refused by
-- name. A guard for the class belongs with the field-door lane and is named in this lane's
-- PROGRESS doc rather than taken here.
--
-- WHAT THIS FILE DOES, and it is additive in every arm — a patch that does not carry these two
-- keys takes exactly the path it took before:
--   1. `on_target_delete` is APPLIED. It is refused by name on a column that points at nothing,
--      and refused by name for a word that is not one of the three things that can happen.
--   2. `relation_target` is APPLIED, and the old edges are WITHDRAWN in the same operation —
--      the rule the behaviour arm above already states in full. It asks
--      `custom.assert_may_know_table` about the new target first, because "may I point at it"
--      is "may I see it" (RELATION-DECLARE, 2026-09-20); without that this arm would be a way
--      to reach a table the caller may not see by patching instead of declaring.
--
-- ITS INVERSE: `migrations/inverse/redsuites2_the_column_door_applies_the_delete_rule_down.sql`,
-- which is this same body with the two arms removed.
--
-- THE SUITE: `scripts/campaign-tests/w1_rel_c12.sql` REL-2 / T7 asks both from the seat.
