-- additive: yes
-- based-on: custom.where_id_opens(uuid) da1c895b1840261f369a8982373b0a1b4c7c9bb028ab50c2508268b6d8896754
-- based-on: custom.table_home(uuid, uuid) 070f1f5e3fde581cc08e615fe7bdb6866b6b2b303a1314f4915cd7ceb413bb3f
-- based-on: custom.table_move(uuid, uuid, integer) 83e9e40f85f7cdeef3a318da45072bd2195c35578bf33d70f80699436d6dde79
-- based-on: custom.doc_render_read(uuid, uuid) 421e34cbb064f74bfaf1812ccf5acf97f70c18092294c5e971b22ab4310fc198
...
--
-- chair-step: it REPLACES five live function bodies with identical signatures —
--   `custom.where_id_opens`, `custom.table_home`, `custom.table_move`, `custom.doc_render_read`
--   and `custom.resolve_context` — so each asks the store's decision in its OWN body instead of
--   only through a helper it calls. Nothing is dropped, renamed, granted or revoked; no row of
--   anybody's data is touched. For every caller the doors admitted before, the answer is the same
--   (each added question is one the door's own helper already asked, with the same arguments, and
--   whose yes is memoised for the transaction). The inverse is
--   `migrations/inverse/suitehealth2_every_door_decides_in_its_own_body_down.sql`.
--
-- LANE SUITE-HEALTH-2 — `pnpm check:store-doors-decide` named these doors on production:
--   census 1  custom.doc_render_read       (decided with assert_client_may_open, which census 1
--                                           does not count; the wall is now named in the body)
--   census 2  custom.table_move            (wrote records; the wall was only inside where_id_opens)
--   census 5  where_id_opens, table_home, table_move, resolve_context
--                                          (reached the ladder only through _where_id_may_open,
--                                           where_id_opens or _where_ids_open_with)
--   census 6  custom.table_move            (wrote into the destination without asking its switch;
--                                           _table_move_plan already refuses a closed destination,
--                                           so assert_store_door there refuses nobody it admitted)
--
-- NOT IN THIS FILE, on purpose (reported to the chair with two readings each):
--   custom.context_resolve(p_organization_id, p_bindings, p_scope_slug) — its register row says
--     it "decides nothing itself" and ignores the organization argument by design;
--   custom.where_tables_live(uuid[]) — its register row says any signed-in client may ask about
--     any id and it reveals only the store word;
--   custom._older_table_copy_refusal(uuid) — its register row says it names the table only for a
--     member (iam.has_org_access), which no ladder rung reproduces exactly.
--   Adding a rung to any of those three changes an answer the register promises.
...
begin
  ...
  -- THE DECISION, IN THIS BODY (lane SUITE-HEALTH-2): the ladder is asked here too, with the
  -- very arguments custom.where_id_opens just asked it (the Table's own organization, the Table,
  -- viewer), so this door never rests on the resolver alone. It cannot refuse anyone
  -- where_id_opens admitted — the wall's yes is memoised for this transaction and the ladder's
  -- question is identical — so an outsider a Table was SHARED with still reads where it lives
  -- (SHARE-GATE-OFF's walk: "Lives in: Organization unknown" was a screen that lied).
  perform custom.assert_client_may_open((v_opens ->> 'organization_id')::uuid,
                                        (v_opens ->> 'resolved_id')::uuid, 'custom.table_home',
...
begin
  ...
  -- THE DECISION, IN THIS BODY (lane SUITE-HEALTH-2): the organization wall of the Table's own
  -- organization, asked here and not only inside custom.where_id_opens. The row half — who may
  -- move it (its maker, or an owner or admin) — is custom._table_move_plan's may_move below.
  perform custom.assert_client_may_reach(v_from, 'custom.table_move');
  ...
  -- THE SWITCH, IN THIS BODY (lane SUITE-HEALTH-2): the rows are written into the destination, so
  -- the destination's store is asked in the store's own words. A destination the plan passed as
  -- ok already has its store open, so this refuses nobody the plan admitted.
  perform custom.assert_store_door(p_to_organization_id, 'custom.table_move');
