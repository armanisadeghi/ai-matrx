-- additive: yes
-- based-on: custom.where_id_opens(uuid) 80119904e51475ceddbbbcd5ea947d741e24ad398f175f40925262b29c30a507
--
-- chair-step: it REPLACES one live function body with an identical signature,
--   `custom.where_id_opens(uuid)`, changing only the door name its two in-body ladder questions
--   pass (`platform.resolve_id` -> `custom.where_id_opens`). Nothing is dropped, granted or
--   revoked; no row is touched. Every answer is unchanged — the name only appears in a refusal,
--   and this door catches the refusal and answers null. The inverse is
--   `migrations/inverse/suitehealth2_where_id_opens_decides_in_its_own_name_down.sql`.
--
-- LANE SUITE-HEALTH-2 follow-up to `suitehealth2_every_door_decides_in_its_own_body.sql`: moving
-- the decision out of `custom._where_id_may_open` into this body carried that helper's door name
-- with it, and census 17 of `pnpm check:store-doors-decide` (a refusal names the door the person
-- called) named it on production. A door decides under its own name.
...
begin
  ...
  begin
    perform custom.assert_client_may_reach(v_org, 'custom.where_id_opens');
    perform custom.assert_client_may_open(v_org, v_wall, 'custom.where_id_opens',
