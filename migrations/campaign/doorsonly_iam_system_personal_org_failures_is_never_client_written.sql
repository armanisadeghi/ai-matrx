...
--
-- iam.system_personal_org_failures -- IDENTITY SEMANTICS.
-- The record of personal-organization provisioning failures, written by the provisioning path server-side. Zero client mentions.
--
-- WHY IT MATTERS: A forgeable record of what the identity system failed to do.
--
...
--
-- THE DOOR IS UNAFFECTED. `iam.system_personal_org_failures` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_iam_system_personal_org_failures_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "system_personal_org_failures_client_insert_refused" on iam.system_personal_org_failures
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "system_personal_org_failures_client_update_refused" on iam.system_personal_org_failures
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "system_personal_org_failures_client_delete_refused" on iam.system_personal_org_failures
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "system_personal_org_failures_client_insert_refused" on iam.system_personal_org_failures is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A forgeable record of what the identity system failed to do. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "system_personal_org_failures_client_update_refused" on iam.system_personal_org_failures is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A forgeable record of what the identity system failed to do. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "system_personal_org_failures_client_delete_refused" on iam.system_personal_org_failures is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A forgeable record of what the identity system failed to do. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
