-- chair-step: it REPLACES the body of `custom.record_write_graph`, the door lane STORE-TXN
--   landed earlier today, and a replacement of a live body is outside the additive allow-list
--   by design. Nothing is dropped, no grant moves, no row of anybody's data is touched, and
--   the door's signature, its `platform.client_callable_door` row and its EXECUTE grant are
--   all unchanged. The `-- based-on:` line below pins the exact body this was written against.
--
-- STORE-TXN — THE GRAPH'S CHILDREN ARE THE PARENT'S OWN DECLARED RELATION.
--
-- WHAT WAS WRONG, MEASURED ON THE DEV CLONE 2026-09-22. The first cut linked each child to
-- its parent with `public.assoc_add` carrying the parent's field in `role`. The store refused
-- it, by name, on the very first child:
--
--     custom.record_write_graph refused the WHOLE graph on the edge of child 1 of 3 to its
--     parent: a relation on a record has to say which field it came from, and
--     "related_keywords" does not
--
-- That refusal is `custom._store_relation_edge_names_its_field` (REL-10 / T7) and it is
-- RIGHT: an association out of a record that carries a role and no `relation_field_id` is an
-- edge nothing can read the rules of — not what happens when a target is archived, not how
-- many targets the field allows, not which Tables it may point at — so the delete rules, the
-- cardinality and the organization wall all silently do nothing. The door was reaching for
-- the wrong writer, and the guard caught it before a single graph shipped.
--
-- WHAT IT IS NOW. The children of one field are linked in ONE `platform.relation_set` call —
-- the store's own relation writer, which resolves the declared field itself, asks viewer on
-- every target and editor on the parent, and writes each child's index into `position` so a
-- parent with two array fields of the same child shape keeps its slots apart (DD-178).
-- `public.assoc_add` stays for the edges that point OUT of the store, which is what the
-- provenance edge from a `chat.message` is. A child that names no field is now refused BEFORE
-- anything is written, with the field to declare named in the hint: a line that cannot say
-- which promise it fills is not one of the lines the parent promised.
--
-- The answer gains `relations` beside `edges`, so a caller can tell the two writers apart.

-- based-on: custom.record_write_graph(uuid, uuid, jsonb, jsonb, jsonb) 67f80fea6406164dab1213338caa392c884b3803268561135e690cdc81877b32
