...
--
-- based-on: custom._entity_custom_fields_guard() 7ed4147c4d0eb0cb1561110efac331cec3787b1c6a70efa02decc9e804ea095a
--
-- ENTITY-FIELDS 1b — A ROW WITH NO CUSTOM FIELDS HAS NONE.
--
-- `entityfields_the_guard_can_do_its_job.sql` landed on the main database at 03:48 UTC and
-- was measured from the seat one minute later. It had traded one outage for a smaller one:
--
--     insert into crm.party (organization_id, party_kind, display_name) values (...)
--       -> 22023  The custom fields of a party are a set of named values, and this write
--                 gives them as null.
--
-- `crm.party.custom_fields` is NULLABLE with no default (W1-STORE added it that way, before
-- REC-40's shape was settled), so `to_jsonb(new) -> 'custom_fields'` is JSON `null` on every
-- INSERT that does not mention the column, and `coalesce` cannot see it because JSON null is
-- not SQL NULL. Absent, SQL NULL and JSON null are now all read as the empty set, which is
-- what a row with no custom fields actually has. A value of the wrong SHAPE - a string, a
-- number, an array - is still refused by name, which is what that refusal was always for.
--
-- The lesson, recorded because it is the class: this guard is the FIRST code in the campaign
-- that runs on the write path of live business tables, so every arm of it has to be true for
-- a row nobody has ever customised. That is now what the green suite asserts first.
--
-- INVERSE: migrations/inverse/entityfields_the_guard_can_do_its_job_down.sql restores the
-- pre-campaign body and is the inverse of this file too - there is one live body, not two.
