...
--
-- platform.feature_knob -- KNOB SEMANTICS.
-- Every ceiling, quota and behavioural default on the platform. Its one client write path is the SECURITY DEFINER door `platform.feature_knob_set`; the five client call sites (`features/admin/limits/service.ts` x2, `features/hr/settings/service.ts`, `features/marketing/seo/topical-map/knobs.ts`, `lib/knobs/featureKnobs.ts`) are ALL `.select(...)` -- read, every one, checked by reading the file rather than counting `.from()`.
--
-- WHY IT MATTERS: A client that writes this table rewrites its own limits, and everybody else's in the same organization.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.feature_knob` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_feature_knob_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "feature_knob_client_insert_refused" on platform.feature_knob
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "feature_knob_client_update_refused" on platform.feature_knob
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "feature_knob_client_delete_refused" on platform.feature_knob
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "feature_knob_client_insert_refused" on platform.feature_knob is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). KNOB semantics: A client that writes this table rewrites its own limits, and everybody else''s in the same organization. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "feature_knob_client_update_refused" on platform.feature_knob is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). KNOB semantics: A client that writes this table rewrites its own limits, and everybody else''s in the same organization. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "feature_knob_client_delete_refused" on platform.feature_knob is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). KNOB semantics: A client that writes this table rewrites its own limits, and everybody else''s in the same organization. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
