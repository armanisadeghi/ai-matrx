...
--
-- iam.system_orgs -- IDENTITY SEMANTICS.
-- The named system organizations the platform resolves by key (`lib/organizations/systemOrg.ts`, a single `.select("organization_id")`).
--
-- WHY IT MATTERS: A client that writes this table repoints a platform-level key at an organization of its choosing.
--
...
--
-- THE DOOR IS UNAFFECTED. `iam.system_orgs` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_iam_system_orgs_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "system_orgs_client_insert_refused" on iam.system_orgs
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "system_orgs_client_update_refused" on iam.system_orgs
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "system_orgs_client_delete_refused" on iam.system_orgs
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "system_orgs_client_insert_refused" on iam.system_orgs is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A client that writes this table repoints a platform-level key at an organization of its choosing. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "system_orgs_client_update_refused" on iam.system_orgs is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A client that writes this table repoints a platform-level key at an organization of its choosing. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "system_orgs_client_delete_refused" on iam.system_orgs is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A client that writes this table repoints a platform-level key at an organization of its choosing. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
