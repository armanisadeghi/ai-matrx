...
-- removing the door.
--
-- platform.custom_field_target -- SCHEMA DEFINITION SEMANTICS.
-- Which entities a custom field applies to. Read by `features/hr/settings/service.ts`; no writer anywhere.
--
-- WHY IT MATTERS: A member who writes a field target attaches a field to somebody else's entity.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.custom_field_target` is owned by `postgres` and does NOT carry
