-- lock: custom
-- lane: ARGS-RULED-2
-- based-on: custom.table_archive(uuid, uuid, integer, boolean) 3bbea00f44d83b1070f31db676fb09dbad5b497daa88cc4c6137cd656aeb772a
--
-- ARGS-RULED-2 — `custom.table_archive` SAYS WHAT IT DID TO AN EMPTY TABLE, AND ITS PREVIEW
-- CHANGES NOTHING.
--
-- MEASURED 2026-09-23 on the MAIN database from test@test.com's seat, on a Table with no records:
--   · `table_archive(org, table, 50, true)` archived the Table record itself and answered
--     "<name> is already archived. Nothing was changed." — the opposite of what had happened;
--   · worse, `p_chunk = 0` — documented as "tell me, change nothing", and exactly what
--     @ai-matrx/records-ui's TableSettings calls as its PREVIEW when a person clicks Archive,
--     BEFORE they confirm — archived an empty Table too, because the Table-goes-last step never
--     looked at the chunk.
--
-- THE FIX: the Table record is archived only by a call that was asked to change something
-- (p_chunk > 0), and two sentences are added that say what happened — "would archive the table
-- itself. Nothing has been changed yet." for the preview, "the table itself is now archived" for
-- the call that did it. "already archived" is now said only when it was already archived before
-- the call. Nothing else in the body moves.
--
-- Seat suite: scripts/campaign-tests/argsruled2_table_archive_green.sql; red twin
-- scripts/campaign-tests/argsruled2_table_archive_red.sql. Inverse:
-- migrations/inverse/argsruled2_an_empty_table_is_told_what_happened_to_it_down.sql.
