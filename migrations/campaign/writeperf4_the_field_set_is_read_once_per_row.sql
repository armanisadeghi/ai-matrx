-- additive: yes
--
-- chair-step: it adds ONE function (`custom.applicable_fields_json`) and its declared client-door
--   row, rewrites `custom.applicable_fields` as a wrapper over it, and changes EIGHT call sites in
--   six trigger/helper bodies — nothing else in those bodies moves. Nothing is dropped, nothing is
--   revoked, no row of anybody's data is touched. The inverse is
--   `migrations/inverse/writeperf4_the_field_set_is_read_once_per_row_down.sql`.
--
-- WRITE-PERF-4 WAVE 2 — THE FIELD SET IS READ ONCE PER ROW, NOT MATERIALISED FOUR TIMES.
--
-- KEPT BY RULING, NOT BY MEETING THE GATE: wave 1 was retained by the campaign chair's ruling of
-- 2026-09-22 after it delivered 333–540 ms against a 600 ms gate whose model priced an item wave 1
-- was not permitted to touch. THIS FILE IS THAT ITEM.
--
-- WHERE THE 290 US ACTUALLY WENT. Wave 1 got a hit on `custom.applicable_fields` down to its own
-- GUC and still measured 290 us a call on the campaign's 7-field fixture. Taken apart on the main
-- database, 1,000 calls, argument varying so nothing constant-folds:
--
--   platform.memo_k_get + ::jsonb + jsonb_populate_recordset ........  37.8 us
--   the three door asserts (all memo hits) .........................  41.8 us
--   `custom.applicable_fields` end to end .......................... 290.0 us
--                                                                    ───────
--   unaccounted, and it is the machinery, not the work ............. 210.4 us
--
-- That 210 us is what a plpgsql `RETURNS SETOF custom.record` function costs to RETURN: every row
-- is built as a composite of a twenty-column rowtype whose `data` column is the document itself,
-- and the whole result is materialised into a tuplestore before the caller sees a row. The MEMO
-- was never the problem after wave 1 — the RETURN TYPE was.
--
-- THE FIX. `custom.applicable_fields_json` is the same function with the same three doors in the
-- same order, the same predicate character for character, the same memo key, returning the jsonb
-- the memo already holds. `custom.applicable_fields` becomes a wrapper over it, so there is still
-- exactly ONE read and ONE memo and the doors are still asserted exactly once, and its twenty-five
-- other callers are untouched. The eight call sites ON THE WRITE PATH substitute
--
--     custom.applicable_fields(A, B, C)
--       ->  jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(A, B, C))
--
-- which yields THE SAME ROWS, THE SAME COLUMNS, IN THE SAME ORDER — both sides materialise the
-- same jsonb array through the same call — at ~55 us instead of ~290.
--
-- THE DIFF IS THE PROOF. These six bodies were not rewritten; the substitution above was applied
-- to their live `pg_get_functiondef` output by `sub.py`, and the complete unified diff of all six
-- is EIGHT hunks, every one of them the call site and nothing else:
--
--   --- custom__dated_values_guard (before)
--   +++ custom__dated_values_guard (after)
--   @@ -60,3 +60,3 @@
--          select f.data into v_field
--   -        from custom.applicable_fields(new.organization_id, new.table_id, v_rtype) f
--   +        from jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(new.organization_id, new.table_id, v_rtype)) f
--           where f.data ->> 'key' = v_key
--   
--   --- custom__derived_fields (before)
--   +++ custom__derived_fields (after)
--   @@ -34,3 +34,3 @@
--    
--   -  for f in select * from custom.applicable_fields(new.organization_id, new.table_id, v_rtype) loop
--   +  for f in select * from jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(new.organization_id, new.table_id, v_rtype)) loop
--        if custom.parity_type(f.data) in ('lookup', 'rollup', 'formula')
--   
--   --- custom__record_field_validation (before)
--   +++ custom__record_field_validation (after)
--   @@ -52,3 +52,3 @@
...
--   +      from jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(new.organization_id, new.table_id,
--   +                                    old.data ->> v_type_field)) f
--         where not exists (select 1
--   -                         from custom.applicable_fields(new.organization_id, new.table_id, v_rtype) a
--   +                         from jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(new.organization_id, new.table_id, v_rtype)) a
--                            where a.id = f.id);
--   
--   --- custom__value_envelope (before)
--   +++ custom__value_envelope (after)
--   @@ -100,3 +100,3 @@
...
--        loop
--   
--   --- custom_computed_provenance (before)
--   +++ custom_computed_provenance (after)
--   @@ -59,3 +59,3 @@
--                 now()
--   -        from custom.applicable_fields(p_organization_id, v_rec.table_id, v_type) f
--   +        from jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(p_organization_id, v_rec.table_id, v_type)) f
--           where custom.parity_type(f.data) in ('lookup', 'rollup', 'formula')
--   
--   --- custom_derived_values_of (before)
--   +++ custom_derived_values_of (after)
--   @@ -39,3 +39,3 @@
...
--   +  for f in select * from jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(v_rec.organization_id, v_rec.table_id, v_rtype)) loop
--        if custom.parity_type(f.data) in ('lookup', 'rollup', 'formula')
--   
--   --- custom_io_changed_field_ids (before)
--   +++ custom_io_changed_field_ids (after)
--   @@ -17,3 +17,3 @@
--          from unnest(custom.io_changed_keys(p_old, p_new)) k
--   -      join custom.applicable_fields(p_organization_id, p_table_id, null) f
--   +      join jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(p_organization_id, p_table_id, null)) f
--            on (f.data ->> 'key') = k
--   
--   --- custom_io_record_changed_stmt_insert (before)
--   +++ custom_io_record_changed_stmt_insert (after)
--   @@ -34,3 +34,3 @@
--                                   from unnest(c.keys) k
--   -                               join custom.applicable_fields(c.organization_id, c.table_id, null) f
--   +                               join jsonb_populate_recordset(null::custom.record, custom.applicable_fields_json(c.organization_id, c.table_id, null)) f
--                                     on (f.data ->> 'key') = k), '[]'::jsonb) end,
--
-- DELIBERATELY NOT DONE: `custom._record_field_validation`'s UPDATE-only retype path still asks
-- the current field set a second time (`not exists (select 1 from … a where a.id = f.id)`) when it
-- already holds it in `v_fields`. Collapsing that would be a real body change rather than a call
-- site, and this file's whole warrant is that the diff is mechanical. It is named here so the next
-- lane does not have to find it.
--
-- NOT ONE REFUSAL MOVES. No guard's logic, message, SQLSTATE, detail or hint is touched;
-- `scripts/campaign-tests/writeperf4_refusals_are_identical.sql` hashes the twelve guards' whole
-- refusal set before and after inside one transaction and requires the two hashes to be equal.
--
-- based-on: custom.applicable_fields(uuid, uuid, text) c3a6e6abddd3e6f9786f6900d5ef0877b036727c405d2ed48a2e9576293de95a
-- based-on: custom._dated_values_guard() d254d34e74e1e9db53365d0504f4f55f639cd78f994e0b3999de383ab4f161fe
...
-- based-on: custom.computed_provenance(uuid, uuid) 5b65c323d2fb3dda80447d5b2a1c75997f569295117bd50db3944a3672d3a846
-- based-on: custom.derived_values_of(custom.record) 6f692f50e9dcfda06b63eeff0a918dc7cb2a1f42ba2dd0b62f8c01b64942240a
-- based-on: custom.io_changed_field_ids(uuid, uuid, jsonb, jsonb) dc1b74b1ae8a74248f51224ae1d7951675001166b5262aad391c7ef66d77ed8c
-- based-on: custom.io_record_changed_stmt_insert() 0f0134ec1bfef8a7e7108ddc1c75c5557c60af696a9668c9ad8d0d38051d71db

create or replace function custom.applicable_fields_json(p_organization_id uuid, p_table_id uuid, p_record_type text DEFAULT NULL::text)
returns jsonb
language plpgsql
stable
security definer
...
values
  ('custom', 'applicable_fields_json', 'p_organization_id uuid, p_table_id uuid, p_record_type text',
   'WRITE-PERF-4',
   'WRITE-PERF-4 wave 2. The same door as custom.applicable_fields — the same three asserts, in the same order, before the first read — answering the same question in the shape the store''s own guards need it: the Field declarations as jsonb, rather than as a materialised setof custom.record. custom.applicable_fields is now a wrapper over this, so there is one body, one memo and one decision. Membership is decided before the first read, so a foreign organization id and an invented one answer identically. It returns Field declarations, never a record''s values.',
   true, false)
on conflict do nothing;

grant execute on function custom.applicable_fields_json(uuid, uuid, text) to postgres, authenticated;
...
as $function$
begin
  -- ONE BODY, ONE MEMO, ONE DECISION. Everything this function used to be now lives in
  -- `custom.applicable_fields_json` — including the three door asserts, which run there, first,
  -- exactly as they ran here. It stays SECURITY DEFINER: it is a declared client door and
  -- dropping that would change WHO its twenty-five callers are, which is not what this file is
  -- for. It hands its caller the composite rows built from the jsonb the door behind it
  -- returned — the same rows, the same columns, in the same order.
  return query select * from jsonb_populate_recordset(null::custom.record,
                              custom.applicable_fields_json(p_organization_id, p_table_id, p_record_type));
end $function$;
