...
--
-- iam.industries -- IDENTITY SEMANTICS.
-- The shared industry registry. The whole feature is ALREADY doored: `features/industries/service.ts` writes through `industry_upsert`, `industry_set_active`, `industry_assign_org`, `industry_unassign_org`, `industry_curator_grant` and `industry_curator_revoke`, and its only `.from("industries")` is a `.select("*")`.
--
-- WHY IT MATTERS: A member who writes the shared registry changes a vocabulary every organization on the platform sees.
--
...
--
-- THE DOOR IS UNAFFECTED. `iam.industries` is owned by `postgres` and does NOT carry
