-- chair-step: this REPLACES the bodies of 39 live functions in schemas platform, iam, hr, esign, files, rag, mandate, provider, content_ir, web, crm, scheduler, workbench and custom, changing ONE kind of statement and nothing else: every `raise exception … using errcode = 'P0002'` (39 of them) becomes `perform platform.refuse_not_found(<the same sentence>, <the same hint>, <the same detail>)`, and 3 functions that catch a caught error's sentence read it through platform.refusal_message. Called directly (the server, every suite, every other function) the refusal is byte-for-byte what it was: SQLSTATE P0002, same message, hint and detail. Called through PostgREST it answers HTTP 404 with error code P0002 instead of HTTP 500. No table, column, policy, trigger or grant is touched; the only rows written are 13 platform.client_callable_door declarations for SECURITY DEFINER bodies that had none (provision_shape_guard refuses a replaced definer without one), each declared as the non-client function it already is. Needs errorshonest_s1_one_way_to_say_not_found.sql first. Inverse: migrations/inverse/errorshonest_s5_every_other_schema_says_not_found_down.sql restores every body verbatim.
-- lane: ERRORS-HONEST
-- based-on: content_ir.edit_kind_instance_value(uuid, text, jsonb) 38e723f77e3079dfff5a8e1da9b434b8e48553c63e0a919bb765a1b69f7a815c
-- based-on: crm.issue_unsubscribe_token(uuid, uuid, uuid) f38e0439257e5e7f8d185f335c3003e40574c545617ad039e393405c17c83dfe
...
-- based-on: esign._ctx_internal(uuid, inet, text) e1c946c268e97d05149a73ad1ad3b8598fec995952f73e3108fb2a6bcefa0fde
-- based-on: esign._event(uuid, text, text, uuid, uuid, uuid, uuid, text, text, inet, text, text, jsonb, text) fd696a4272918c25bcd1427f324505bdfd66bd16ab7bbf6fcc08ca8b34839889
-- based-on: esign._notify(uuid, text, uuid, uuid, text, uuid, text, text, text, jsonb, text) 719ccd78f0002140cb20469a29cedbca885b4d41961da78278d2b6d949b2b997
-- based-on: esign.generate_certificate(uuid) 439a8437676f7d1b30a2705af5f0600debd859d2c2a803443a6a838998d62819
-- based-on: files.file_path_matches_parent(uuid) 0947779a89a96d5f08a20237e8499f24520f9dcb2cac922ebc2587642a5b4117
-- based-on: files.webhook_rotate_secret(uuid) d3a96cd206b6c04777ba86cd09d8f412da31c202c78a75064e0a615050101ee1
-- based-on: hr._door_verdict(uuid, text, uuid, boolean) 1ce7f54e405ce3211f860c88c4b2006173093a42b5a6f7c60b043c98d98439cf
-- based-on: hr.export_finish(uuid, uuid, jsonb, uuid, text, text, text, uuid[], jsonb) 4dbc9e606debfcfcb1ea637d011bb2d63be61a97a1fbf82cd0fe821c09cffc36
-- based-on: hr.export_transition(uuid, uuid, text, text, timestamp with time zone, text) 7b34fafb88c0fcb53047f702aba049dedf24a8186c6e5511ac46448d73e9a2ae
-- based-on: hr.reveal_ssn(uuid, text, text) 1279d90787db8491fe631204d67f5ca4f7d9d94cd68d9b8583fd6ae6f790621f
-- based-on: hr.transfer_restricted_note(uuid, uuid) cc13aef161847e9a412210383e8cc0903fc428d7e5af9ebba841fa5c3e8e5514
-- based-on: hr.wf_request(text, text, uuid, uuid, jsonb, uuid, boolean, text) fa5d7e9d4d99ed51ee3d7275c8fd15091f9e4825c9b0ccc8e36fec73b3d68f5f
-- based-on: hr.wf_resolve_approvers(uuid, uuid[]) 114b9861139b38a0555a89fb31f4af2d58df53527174be934e26b0e1e7935df7
-- based-on: iam._managed_invitation(uuid) 84459f2715bdb649a8ab26f3e1cdcec95930013ec50a07d2306cba660a0c56e3
-- based-on: iam.emergency_door_approve(uuid, text) 0205435f7c3fff6f0bdce1610ed3fe794874bc98a5e879c5ee6c3b6c0d7e1f02
-- based-on: iam.emergency_door_deny(uuid, text) c80b3aede1dd4b013078ea75762a47d8598a9391605bf0aa6058b5266cfccbc7
-- based-on: iam.emergency_door_open(text, uuid, text, text) 170657b442cbde1ec810ba6c4ea73862a68c17dadd2b34636b9bdccc43dc7a01
-- based-on: iam.organization_archive(uuid, text, text) a59a27c1e9e3320da6308f8dfc4645c23d7a63c8dc1bff9cc54c54bfbbe288f5
-- based-on: iam.organization_restore(uuid, text) 64ad269109fdc331ecbec520af8d8c704b57037b8179c605883a893aacec226f
-- based-on: mandate.duplicate_mandate(uuid, boolean, uuid) d539f6bd63e044d601febc36e95084e3e0093b117dca5969bb32182a22a90627
-- based-on: mandate.submit_scan_report(jsonb) 5175a542fdf6c40c4c382bdeb35ad7d236879ffc2860d0fa6f8f6c3a0df6ba56
-- based-on: platform.admin_db_cron_job_update(bigint, text, boolean, uuid) c31a30ae5bba23216fe9dcd352994452a39bdc3f07aed628e7662b282777201e
-- based-on: platform.admin_db_cron_job_update(bigint, text, boolean) d94bb56068bd075fb1db97456b229d66bc184754d1cece6dd36570787bfab40d
-- based-on: platform.knob_unarchive(text, text) b93ae9be24da83d0c6b15f032e5b62267662aabf59ec00a176ff45d394347195
-- based-on: platform.lifecycle_file_custody_finalize_delete(uuid, uuid) 941bff0643be77607f3c39be4bc7c881f7c47bfebdefcfd8c5f20ef3f29da138
-- based-on: platform.lifecycle_meet_custody_stage(uuid, uuid, uuid, uuid, timestamp with time zone, timestamp with time zone) 3ef29a54e5c4936b8c92122cb54bb0185aa64219d47a5993b2cf9a55f80e4b69
-- based-on: platform.provision_grant_open(uuid, text, text) 042fda772d6fd303829336419bab5208f29e616ebfd74de630ee14c758dcde81
-- based-on: platform.resolve_change_handling(text, uuid) c77e6969015aecd9d422918605871c69b4660b96982d0b4906dbec92d6c02897
-- based-on: platform.upsert_unit_purpose(text, uuid, text, text, text, jsonb, jsonb, jsonb, integer) 843f757b0e612cd3415fb57ed0fc9f7ad2ca00510bec6847f7ee501203d2d164
-- based-on: provider.attach_credential(uuid, uuid, text) bf9fdc7be459133df140e1a2eb9005eab52362472bd1c8acf247d5b907f31dc0
-- based-on: provider.set_account_status(uuid, text, text, timestamp with time zone, uuid) 352875c73395d1ceb563700741eceac81e8937554af91fcef3ccb1fc73d84b6a
-- based-on: rag.fn_get_library_full_page(uuid, integer) a8d28d7c3abe44c78d9de9a1a3561fac7cd9c81901459f9a21cdb151e9878c6e
-- based-on: rag.fn_list_library_chunks(uuid, integer, integer, boolean, boolean, integer) fff30a9f7af1892b599847f9280606e9db3e1d78ee8d26b048654852744ab432
-- based-on: scheduler.sch_run_claim(uuid, text, uuid, text, integer) 340b865c892399f736c27738e7fde448975388efb9fe991c8b7461e8593b1a52
-- based-on: web.set_site_offering_availability(uuid, uuid, uuid[], boolean, text) e37d9b0fdb1e6392ee2f1df47cd47520d58395efcbf913e79761329138f663a1
-- based-on: workbench.note_folder_get_or_create(uuid, text) fbd093248df27366ed6bb2003ffc4decd2b7ab8106eac14042a53856399799f9
--
...
--   content_ir.edit_kind_instance_value(uuid, text, jsonb): 1 not-found raise
--   crm.issue_unsubscribe_token(uuid, uuid, uuid): 1 not-found raise
--   custom.record_write_graph(uuid, uuid, jsonb, jsonb, jsonb): the whole-graph refusal re-raises a caught not-found the one way a not-found is raised (it re-raised `errcode = v_state`, which for PostgREST's shape would be an unparseable fault)
--   esign._ctx_internal(uuid, inet, text) [not client-granted; reached through a door]: 1 not-found raise
--   esign._event(uuid, text, text, uuid, uuid, uuid, uuid, text, text, inet, text, text, jsonb, text) [not client-granted; reached through a door]: 1 not-found raise
--   esign._notify(uuid, text, uuid, uuid, text, uuid, text, text, text, jsonb, text) [not client-granted; reached through a door]: 1 not-found raise
--   esign.generate_certificate(uuid) [not client-granted; reached through a door]: 1 not-found raise
--   files.file_path_matches_parent(uuid): 1 not-found raise
--   files.webhook_rotate_secret(uuid): 1 not-found raise
--   hr._door_verdict(uuid, text, uuid, boolean) [not client-granted; reached through a door]: 1 not-found raise
--   hr.export_finish(uuid, uuid, jsonb, uuid, text, text, text, uuid[], jsonb) [not client-granted; reached through a door]: 1 not-found raise
--   hr.export_transition(uuid, uuid, text, text, timestamp with time zone, text) [not client-granted; reached through a door]: 1 not-found raise
--   hr.reveal_ssn(uuid, text, text): 1 not-found raise
--   hr.transfer_restricted_note(uuid, uuid) [not client-granted; reached through a door]: 1 not-found raise
--   hr.wf_request(text, text, uuid, uuid, jsonb, uuid, boolean, text) [not client-granted; reached through a door]: the caught error's sentence (sqlerrm, 2 places) is read through platform.refusal_message, so a not-found caught inside a PostgREST request carries its sentence, never PostgREST's JSON error body
--   hr.wf_resolve_approvers(uuid, uuid[]) [not client-granted; reached through a door]: the caught error's sentence (sqlerrm, 1 place) is read through platform.refusal_message, so a not-found caught inside a PostgREST request carries its sentence, never PostgREST's JSON error body
--   iam._managed_invitation(uuid) [not client-granted; reached through a door]: 2 not-found raises
--   iam.emergency_door_approve(uuid, text): 1 not-found raise
--   iam.emergency_door_deny(uuid, text): 1 not-found raise
--   iam.emergency_door_open(text, uuid, text, text): 1 not-found raise
--   iam.organization_archive(uuid, text, text): 1 not-found raise
--   iam.organization_restore(uuid, text): 1 not-found raise
--   mandate.duplicate_mandate(uuid, boolean, uuid): 1 not-found raise
--   mandate.submit_scan_report(jsonb) [not client-granted; reached through a door]: 3 not-found raises
--   platform.admin_db_cron_job_update(bigint, text, boolean, uuid) [not client-granted; reached through a door]: 1 not-found raise
--   platform.admin_db_cron_job_update(bigint, text, boolean) [not client-granted; reached through a door]: 1 not-found raise
--   platform.knob_unarchive(text, text): 1 not-found raise
--   platform.lifecycle_file_custody_finalize_delete(uuid, uuid) [not client-granted; reached through a door]: 1 not-found raise
--   platform.lifecycle_meet_custody_stage(uuid, uuid, uuid, uuid, timestamp with time zone, timestamp with time zone) [not client-granted; reached through a door]: 1 not-found raise
--   platform.provision_grant_open(uuid, text, text): 1 not-found raise
--   platform.resolve_change_handling(text, uuid): 1 not-found raise
--   platform.upsert_unit_purpose(text, uuid, text, text, text, jsonb, jsonb, jsonb, integer): 1 not-found raise
--   provider.attach_credential(uuid, uuid, text) [not client-granted; reached through a door]: 1 not-found raise
--   provider.set_account_status(uuid, text, text, timestamp with time zone, uuid) [not client-granted; reached through a door]: 1 not-found raise
--   rag.fn_get_library_full_page(uuid, integer): 1 not-found raise
--   rag.fn_list_library_chunks(uuid, integer, integer, boolean, boolean, integer): 1 not-found raise
--   scheduler.sch_run_claim(uuid, text, uuid, text, integer): 1 not-found raise
--   web.set_site_offering_availability(uuid, uuid, uuid[], boolean, text): 1 not-found raise
--   workbench.note_folder_get_or_create(uuid, text): 1 not-found raise
...
BEGIN
  ...
  IF NOT FOUND THEN
    perform platform.refuse_not_found('That record does not exist, or it has been deleted.');
  END IF;
...
begin
  ...
  if v_org is null then
    perform platform.refuse_not_found(format('unsubscribe token: contact medium %s not found', p_contact_medium_id));
  end if;
...
begin
  ...
  if not found then
    perform platform.refuse_not_found(format('esign: signer %s does not exist', p_signer_id));
  end if;
...
begin
  ...
  if v_org is null then
    ...
    perform platform.refuse_not_found(format('esign._event: envelope %s does not exist', p_envelope_id));
  end if;
...
begin
  ...
  if not found then
    perform platform.refuse_not_found(format('esign.generate_certificate: envelope %s does not exist', p_envelope_id));
  end if;
...
begin
  ...
  if v_owner is null then
    perform platform.refuse_not_found('That webhook no longer exists.');
  end if;
...
begin
  ...
  if v_org is null then
    perform platform.refuse_not_found(format('hr audited door: no %s row with id %s', p_token, p_id));
  end if;
...
begin
  ...
  if v_org is null then
    perform platform.refuse_not_found(format('hr.reveal_ssn: no hr.employee row with id %s', p_employee_id));
  end if;
...
begin
  ...
  if v_note.id is null then
    perform platform.refuse_not_found(format('hr.transfer_restricted_note: note %s not found', p_id));
  end if;
    ...
    exception when others then
      return jsonb_build_object('granted', false, 'reason', 'definition_invalid',
        'detail', format('approval_subject_unmapped: hr.can_approve cannot resolve a subject for %s (%s)',
                         v_tbl, platform.refusal_message(sqlstate, sqlerrm)),
        ...
        foreach c in array v_cands loop
          ...
          if v_action is not null then
            ...
            exception when others then
              ...
              return jsonb_build_object('granted', false, 'reason', 'definition_invalid',
                'detail', format('approval_subject_unmapped: hr.can_approve cannot resolve a subject for %s (%s)',
                                 v_target_tbl, platform.refusal_message(sqlstate, sqlerrm)));
            end;
            if not v_ok then
...
begin
  ...
  select * into q
   ...
   for update;
  if not found then
    perform platform.refuse_not_found(format('emergency_door_deny: no request %s', p_request_id));
  end if;
  if q.status <> 'pending' then
    return jsonb_build_object('granted', false, 'reason', 'not_pending',
      'message', format('This request was already %s.', q.status));
...
BEGIN
  ...
  IF NOT FOUND THEN
    perform platform.refuse_not_found('No live mandate answers to that id, so there is nothing to copy.', 'Open the mandate from the list and try again — it may have been deleted.');
  END IF;
...
BEGIN
  ...
  IF v_org IS NULL THEN
    perform platform.refuse_not_found('SYSTEM_ORG_MISSING: iam.system_orgs has no key=''system'' row; every row here is homed in the System Organization and NULL is never a scope.');
  END IF;
  ...
  IF v_phase = 'references' THEN
    ...
    IF v_scan_id IS NULL THEN
      perform platform.refuse_not_found(format('CHUNK_WITHOUT_OPEN: no open scan for %s / %s at %s (%s, scanner %s).',
                         v_repo, v_pkg_path, v_revision, v_kind, v_scanner), 'Send the open phase for this package first.', 'A references batch names the scan it belongs to. Writing one under a scan that was never opened would produce references nothing can finalize, count or retire.');
    END IF;
  ...
  IF v_phase IN ('full','finalize') THEN
    FOR v_pkg IN SELECT jsonb_array_elements(coalesce(p_report->'packages', '[]'::jsonb)) LOOP
      ...
      IF v_scan_id IS NULL THEN
        perform platform.refuse_not_found(format('FINALIZE_WITHOUT_OPEN: no scan for %s / %s at %s (%s, scanner %s).',
                           v_repo, v_pkg_path, v_revision, v_kind, v_scanner), 'Send the open phase (and the reference batches) before finalizing.');
      END IF;
...
begin
  ...
  if not found then
    perform platform.refuse_not_found(format('no pg_cron job with jobid %s', p_jobid));
  end if;
...
begin
  ...
  if v_row.feature is null then
    perform platform.refuse_not_found(format('platform.knob_unarchive: %s.%s is not a registered knob', p_feature, p_key));
  end if;
...
BEGIN
  ...
  IF NOT FOUND THEN
    perform platform.refuse_not_found('file custody finalization lost its locked file row');
  END IF;
  RETURN true;
END;
$function$;
...
BEGIN
  ...
  IF NOT FOUND OR v_file.deleted_at IS NOT NULL THEN
    perform platform.refuse_not_found('file custody file unavailable');
  END IF;
...
begin
  ...
  if not exists (select 1 from iam.organizations o where o.id = p_organization_id) then
    perform platform.refuse_not_found('provision_grant_open: no such organization.');
  end if;
...
begin
  ...
  if not found then
    ...
    perform platform.refuse_not_found(format('[change-policy] unknown change_type_key "%s" — not in platform.change_type_default. Register it in CHANGE_TYPE_CATALOGUE (matrx-frontend features/change-policy/catalogue.ts) and apply the generated seed. Refusing to guess a handling mode.', p_change_type_key));
  end if;
...
BEGIN
  ...
  IF v_org IS NULL THEN perform platform.refuse_not_found('provider account not found'); END IF;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM docproc.processed_documents WHERE id = p_id) THEN
    perform platform.refuse_not_found(format('document %s is not available to this account — it may not exist, or your access may not reach it', p_id));
  END IF;
...
begin
  ...
  if v_id is null then
    perform platform.refuse_not_found('notes_folder_not_visible: the folder exists but this caller cannot read it.');
  end if;
  return v_id;
end
$function$;

-- ── 13 SECURITY DEFINER bodies above had no platform.client_callable_door row. provision_shape_guard refuses to
...
select 'esign', '_ctx_internal', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'internal: neither a client role nor service_role holds EXECUTE; it is reached only from inside public.esign_sign, public.esign_sign_adopt_signature, public.esign_sign_consent, public.esign_sign_decline, public.esign_sign_download, public.esign_sign_load and 1 more, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
  ...
  from pg_proc p
 where p.oid = 'esign._ctx_internal(uuid, inet, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'esign' and x.function_name = '_ctx_internal'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'esign', '_event', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'internal: neither a client role nor service_role holds EXECUTE; it is reached only from inside esign._act_adopt, esign._act_consent, esign._act_decline, esign._act_delegate, esign._act_download, esign._act_load and 13 more, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'esign._event(uuid, text, text, uuid, uuid, uuid, uuid, text, text, inet, text, text, jsonb, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'esign' and x.function_name = '_event'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'esign', '_notify', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'internal: neither a client role nor service_role holds EXECUTE; it is reached only from inside esign._act_decline, esign._maybe_complete, esign._notify_actionable, esign._notify_actionable_capped, public.esign_expire_sweep, public.esign_resend_signer and 1 more, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'esign._notify(uuid, text, uuid, uuid, text, uuid, text, text, text, jsonb, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'esign' and x.function_name = '_notify'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'esign', 'generate_certificate', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside esign._maybe_complete, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'esign.generate_certificate(uuid)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'esign' and x.function_name = 'generate_certificate'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'hr', '_door_verdict', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside hr._door_get, hr._door_list, hr._wf_may_see_change, public.hr_break_glass, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'hr._door_verdict(uuid, text, uuid, boolean)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'hr' and x.function_name = '_door_verdict'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'hr', 'export_finish', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       ...
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'hr.export_finish(uuid, uuid, jsonb, uuid, text, text, text, uuid[], jsonb)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'hr' and x.function_name = 'export_finish'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'hr', 'export_transition', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'hr.export_transition(uuid, uuid, text, text, timestamp with time zone, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'hr' and x.function_name = 'export_transition'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'hr', 'transfer_restricted_note', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'hr.transfer_restricted_note(uuid, uuid)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'hr' and x.function_name = 'transfer_restricted_note'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'iam', '_managed_invitation', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside public.inv_get_managed, public.inv_resend, public.inv_revoke, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'iam._managed_invitation(uuid)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'iam' and x.function_name = '_managed_invitation'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'platform', 'admin_db_cron_job_update', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'owner_only: no role but its owner holds EXECUTE, so only an operator connected as the owner can run it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'platform.admin_db_cron_job_update(bigint, text, boolean, uuid)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'platform' and x.function_name = 'admin_db_cron_job_update'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));
...
select 'platform', 'admin_db_cron_job_update', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       ...
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'platform.admin_db_cron_job_update(bigint, text, boolean)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'platform' and x.function_name = 'admin_db_cron_job_update'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));
...
insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'platform', 'lifecycle_file_custody_finalize_delete', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'platform.lifecycle_file_custody_finalize_delete(uuid, uuid)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'platform' and x.function_name = 'lifecycle_file_custody_finalize_delete'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'platform', 'lifecycle_meet_custody_stage', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s5_every_other_schema_says_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'platform.lifecycle_meet_custody_stage(uuid, uuid, uuid, uuid, timestamp with time zone, timestamp with time zone)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'platform' and x.function_name = 'lifecycle_meet_custody_stage'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));
