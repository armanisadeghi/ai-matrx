...
--
-- communication.meet_meetings.host_user_id genuinely names a person other than the writer, so pinning it to
...
-- Inverse: migrations/inverse/secsweep2_communication_meet_meetings_host_user_id.inverse.sql
...
create policy "meet_meetings_host_user_id_is_addressable_insert" on communication.meet_meetings
  as restrictive for insert to authenticated, anon
  with check (host_user_id = (select auth.uid()) or iam.may_address_user_in_org(host_user_id, organization_id) or (select public.is_platform_admin()));

create policy "meet_meetings_host_user_id_is_addressable_update" on communication.meet_meetings
