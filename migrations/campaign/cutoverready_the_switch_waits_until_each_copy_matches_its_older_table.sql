-- chair-step: lane CUTOVER-READINESS (chair brief 2026-09-25, from the /data-v2 readiness audit blockers 5 and 7: "the readiness check does not compare colours, strictness, formats or shares between the older table and the copy … the press is refused, with the differences named, when any of those differ"). ADDS platform.cutover_copy_differences(uuid) and REPLACES the body of platform._cutover_seam_readiness(text,uuid) with four more checks on the older_tables switch. No row of any table is written. No lock beyond the two function definitions.
-- based-on: platform._cutover_seam_readiness(text, uuid) c471eb2c66b8dc1d4f385b4c95d1f01fe1ee6aef0a3cc2f08462be2de5d10b7b
-- lane: CUTOVER-READINESS
-- INVERSE: migrations/inverse/cutoverready_the_switch_waits_until_each_copy_matches_its_older_table_down.sql
--
-- THE RULE BEFORE THIS FILE: the Data tables switch was ready when every older table had a copy
-- with every row present and current. It never looked at what a person SEES and what a writer
-- MEETS: the audit found an organization reporting ready over a copy missing three colour rules
-- and a choice the owner added after the copy (measured on production 2026-09-26 05:3xZ).
--
-- THE RULE AFTER IT: four more checks, each met only when nothing differs, each naming up to five
-- differences in plain words (the press already refuses when any check is unmet and says which):
--   · colours_match — every colour the older SCREEN paints (rows, columns, cells, colour-by, colour
--     rules; the older grid paints only the store's seven words, so a stored "purple" it never
--     showed is not compared) is the colour the copy paints, and the copy paints nothing else;
--   · checks_match — the older tables' writes of the last 30 days (what each writer changed, as it
--     was sent) and every row edited after its copy (whole) go through the copy's own judge
--     (custom.validate_values), after the mover's conversions; a value the copy would refuse is
--     named with its table (156 of the 158 live older tables are permissive: they enforced nothing);
--   · formats_match — each older column's format is what its copy enforces or draws;
--   · shares_match — each older share is the same person or organization at the same level on the
--     copy (a record grant, or an outside invitation), a public older share is named (a public link
--     does not carry), and nobody holds a copy who does not hold its older table.
-- The copy is read AS THE SWITCH LEAVES IT: where a person's test note is open on the Table or a
-- Field (COPY-WRITABLE), its kept image is compared, because the re-sync puts that back first.
-- Colours, checks and formats live in the Table and Field documents, so the re-sync already puts a
-- tested one back; the mover's rerun keeps those kept images equal to the older table (aidream
-- matrx-records, CUTOVER-READINESS). Shares are not test edits and are compared as they stand.

-- ── 1. WHAT THE SWITCH WOULD SHOW DIFFERENTLY: one door, four questions ───────────────────────
-- Each copied table of an organization (older table live, same-id copy live, not an app-kept option
-- list) is compared with its older table on the four things the rows check never looked at. The
-- copy is read as the switch will leave it: where a person's test note is open on the Table or a
-- Field, the kept image (what the re-sync puts back) is compared, not the test edit.
...
comment on function platform.cutover_copy_differences(uuid) is
  'CUTOVER-READINESS: what the Data tables switch would show differently from the older tables, per organization — colours the older screen paints vs the copy (A), each column''s format (B), shares (C), and the older table''s recent writes the copy''s checks would refuse (D, at most 400 rows judged, newest first). The copy is read as the switch leaves it (a person''s test note: its kept image). Read by platform._cutover_seam_readiness; no client role calls it.';
revoke all on function platform.cutover_copy_differences(uuid) from public, anon, authenticated;
grant execute on function platform.cutover_copy_differences(uuid) to service_role;

-- ── 2. THE READINESS JUDGEMENT, WITH THE FOUR CHECKS ─────────────────────────────────────────────
