...
-- additive: yes
--   It REPLACES one body, `custom.list_door_disagreements` (census 13), with a `-- based-on:`
--   line, widening WHO it asks about. Nothing is dropped, revoked or narrowed.
-- guard: custom/external_principal_enabled
-- based-on: custom.list_door_disagreements(text, uuid, integer, boolean) ddc07f06481e680c81e6e2f679accb75815024392e185b63e4438088589190b2
--
-- PORTAL — CENSUS 13 NOW COVERS A PORTAL PRINCIPAL, AND IT DID NOT.
--
-- Census 13 is LEAK-T10's guard: every list-shaped door must answer the same as
-- `custom.read_record`, per (person, record), in every organization the store is open in. It
-- enumerated people from `iam.memberships` — and a portal principal is deliberately NOT a
-- member. That is the whole of VIS-31. So the ONE kind of person whose entire access comes
-- from a grant plus a carried edge was the one kind of person the guard never asked about.
--
-- MEASURED, 2026-09-20, in this lane's red twin: with `custom.carrying_edges_in` taught the
-- portal's arm and `custom.carrying_edges_of` NOT taught it, the list doors handed an
-- outsider ten jobs that `custom.read_record` then refused, one by one — exactly the
-- disagreement census 13 exists to name — and census 13 answered ZERO. It answers that
-- disagreement now, which is what makes the red twin red.
--
-- THE FOUR-ARGUMENT BODY IS THE ONE THAT DOES THE WORK; the three-argument overload beside
-- it forwards to this one and is untouched, so both callers get the wider census.
