...
-- removing the door.
--
-- platform.custom_field_definition -- SCHEMA DEFINITION SEMANTICS.
-- What a custom field IS. `features/hr/settings/service.ts` reads it; no file that names it contains a write verb.
--
-- WHY IT MATTERS: A member who writes a field definition directly bypasses every validation the field doors apply, for every organization that reads the catalogue.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.custom_field_definition` is owned by `postgres` and does NOT carry
