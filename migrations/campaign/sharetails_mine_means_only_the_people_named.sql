-- chair-step: lane SHARE-TAILS. "Only people I share it with" means only the owner and the people named. REPLACES custom.share_lane_set (the lane and custom.record.visibility now move in one transaction: mine -> personal, organization/world -> internal), iam.member_lane_confers (membership stops at a Table that is below internal, so a "mine" Table's rows are not reached through the member lane), custom.reaches_directly (a personal record is carried by no container, matching the access kernel's own rule), and custom.share_access (the "organization default" row is said only when membership really reaches the thing). Then repairs, in production, every record on the explicit mine lane whose visibility was still internal. Same signatures.
-- based-on: custom.share_lane_set(uuid, uuid, text, permission_level) 0fa1a4fe437827e78b1f813d3cb0a71dc69a6c76c84ba57015964d1f33ca0899
...
-- based-on: custom.share_access(uuid, uuid) d98a5106e46d3996bf544c6bbb36acec2fb4d7f16b7c22a622160c59cb8e8795
...
-- lane: SHARE-TAILS
-- INVERSE: migrations/inverse/sharetails_mine_means_only_the_people_named_down.sql
...
begin
  ...
  if v_table is not null and v_table is distinct from custom.table_kernel_id()
...
-- ═══ THE PRODUCTION REPAIR, IN THIS SAME FILE: every record whose owner CHOSE "mine" (an explicit
-- iam.content_lane row on the mine lane, with no active organization-lane row of its own
-- organization) but whose visibility still says `internal`. Those are the owner's intent, written
-- by this door before it moved the visibility. A record with NO lane row never had a choice made
-- about it and is not touched here. Every repaired id and its visibility before are kept so the
-- inverse can put them back exactly.
create table if not exists custom._share_tails_mine_repair (
  record_id         uuid primary key,
  organization_id   uuid not null,
  is_table          boolean not null,
  visibility_before platform.visibility not null,
  repaired_at       timestamptz not null default now()
);
revoke all on custom._share_tails_mine_repair from anon, authenticated;
comment on table custom._share_tails_mine_repair is
  'SHARE-TAILS 2026-09-25: records on the explicit "mine" lane whose visibility was still internal, repaired to personal. Read by the inverse.';

select set_config('app.actor_system', 'sharetails_mine_repair', true);

with chosen as (
  select r.id, r.organization_id, coalesce(r.table_id = custom.table_kernel_id(), false) is_table, r.visibility
    from custom.record r
    join iam.content_lane c on c.resource_type = 'record' and c.resource_id = r.id and c.lane = 'mine'
   where r.visibility >= 'internal'::platform.visibility
     and r.visibility <> 'public'::platform.visibility
     and not exists (select 1 from iam.permissions p
                      where p.resource_type = 'record' and p.resource_id = r.id
                        and p.granted_to_organization_id = r.organization_id
                        and p.status = 'active')
), kept as (
  insert into custom._share_tails_mine_repair (record_id, organization_id, is_table, visibility_before)
  select id, organization_id, is_table, visibility from chosen
  on conflict (record_id) do nothing
  returning record_id
)
update custom.record r
   set visibility = 'personal'::platform.visibility
  from kept k
 where r.id = k.record_id;

do $census$
declare v_n int; v_t int; v_left int;
begin
  select count(*), count(*) filter (where is_table) into v_n, v_t from custom._share_tails_mine_repair;
  select count(*) into v_left
   ...
   where r.visibility = 'internal'::platform.visibility
     and not exists (select 1 from iam.permissions p
                      where p.resource_type = 'record' and p.resource_id = r.id
                        and p.granted_to_organization_id = r.organization_id and p.status = 'active');
  if v_left <> 0 then
    raise exception 'SHARE-TAILS census: % record(s) on the explicit mine lane are still internal.', v_left;
  end if;
  raise notice 'SHARE-TAILS repair: % record(s) on the explicit mine lane moved to personal (% of them Tables).', v_n, v_t;
end $census$;
