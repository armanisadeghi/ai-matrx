...
--
-- platform.guided_checklist_run -- DISPOSABLE PER-PERSON PROGRESS, AND THE HELPER TRAP.
--
...
--
-- The ONE application writer is `lib/guided-setup/service.ts`, and it reaches the
-- table through `function table() { return createClient().schema("platform").from(...) }`
-- declared thirty lines above the `.insert(` -- the exact shape DOORS-ONLY-2 recorded as the
-- reason this table was held back from its batch 2. A second writer is the live RLS probe
-- `scripts/access-matrix/check-guided-checklist-run.ts`, which mints a real user JWT and writes
-- over PostgREST on purpose; it is moved to the door in the same commit, because a probe that
-- tests the old path would go red for the right reason and read as a regression.
--
-- THE DOOR THEY NOW USE: `public.checklist_run_start` and `public.checklist_run_save`,
...
-- Inverse: migrations/inverse/doorsonly3_platform_guided_checklist_run_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "guided_checklist_run_client_insert_refused" on platform.guided_checklist_run
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "guided_checklist_run_client_update_refused" on platform.guided_checklist_run
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "guided_checklist_run_client_delete_refused" on platform.guided_checklist_run
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "guided_checklist_run_client_insert_refused" on platform.guided_checklist_run is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). A guided checklist is disposable progress, but it is progress SOMEBODY ELSE can see in a shared organization, and the base-table INSERT let a caller name any created_by the WITH CHECK admitted. The door stamps created_by from auth.uid() and does the read-then-insert inside the database, so two tabs can no longer create two runs for one checklist. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
comment on policy "guided_checklist_run_client_update_refused" on platform.guided_checklist_run is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). A guided checklist is disposable progress, but it is progress SOMEBODY ELSE can see in a shared organization, and the base-table INSERT let a caller name any created_by the WITH CHECK admitted. The door stamps created_by from auth.uid() and does the read-then-insert inside the database, so two tabs can no longer create two runs for one checklist. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
comment on policy "guided_checklist_run_client_delete_refused" on platform.guided_checklist_run is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). A guided checklist is disposable progress, but it is progress SOMEBODY ELSE can see in a shared organization, and the base-table INSERT let a caller name any created_by the WITH CHECK admitted. The door stamps created_by from auth.uid() and does the read-then-insert inside the database, so two tabs can no longer create two runs for one checklist. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
