...
--
-- hr.eeo_response.actor_user_id records WHO DID IT. The actor of an action is, by definition, the caller,
...
-- Inverse: migrations/inverse/secsweep2_hr_eeo_response_actor_user_id.inverse.sql
-- Guard: `pnpm check:unpinned-security-columns` -- both baseline rows for this pair leave.

set local lock_timeout = '2s';

create policy "eeo_response_actor_user_id_is_the_caller_insert" on hr.eeo_response
  as restrictive for insert to authenticated, anon
  with check (actor_user_id is null or actor_user_id = (select auth.uid()) or (select public.is_platform_admin()));

create policy "eeo_response_actor_user_id_is_the_caller_update" on hr.eeo_response
