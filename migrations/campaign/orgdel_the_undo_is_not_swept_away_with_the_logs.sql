...
--   minutes ago and nothing outside this lane has called yet. Nothing is dropped or renamed and
--   the signature is identical. The inverse is
--   migrations/inverse/orgdel_the_undo_is_not_swept_away_with_the_logs_down.sql.
-- additive: yes
-- guard: custom/system_enabled
-- based-on: custom.organization_clear(uuid, text, boolean) 7d81c36fff50ebf090e55e80a808ca4cb1866b93464105ab3f0bb7d5a8ac8aa1
--
-- ORG-DELETE — THE UNDO IS NOT SWEPT AWAY WITH THE WORK LOGS.
--
-- FOUND BY `scripts/campaign-tests/orgdel_green.sql` ON ITS FIRST RUN. The destroying arm
-- cleared every table in `custom` and `history` that holds an organization's rows except
-- `custom.record` — and `history.migration_log` was in that sweep. So the door retired the
-- organization's Tables, told the person "you can still put this back until 20 October", and
-- then deleted the entries that were the only way to put anything back. A promise made and
-- destroyed in the same call.
--
-- The migration log is the undo, so it goes LAST and only when `custom.record` holds nothing
-- of this organization at all — which is the moment there is nothing left to put back and the
-- organization is about to be deleted anyway.
--
-- WHAT MAKES IT FAIL (rule 3): put the log back in the work-log sweep, which is what
-- migrations/inverse/orgdel_the_undo_is_not_swept_away_with_the_logs_down.sql does — and
-- `scripts/campaign-tests/orgdel_green.sql` PART 3 goes red on the undo.
