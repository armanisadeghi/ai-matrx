...
-- additive: yes
--   It REPLACES two bodies SC-1' made (sc1p_a_table_says_where_it_lives_and_its_owner_can_move_it.sql),
--   each declared below with the body it was written against:
--     · custom._table_move_plan(uuid, uuid, uuid)   what a move carries and what refuses it
--     · custom.table_move(uuid, uuid, integer)       the door that moves it
--   No table, column, trigger, policy or grant is touched; nothing is dropped or revoked; nothing
--   is written by this file. custom.table_home is unchanged (it reads the plan).
--   The inverse is `migrations/inverse/sc1t_a_table_moves_with_the_tables_inside_it_down.sql`,
--   which restores both bodies byte for byte.
--   Runs AFTER the six sc1p_* files.
-- guard: custom/system_enabled
-- lock: custom
-- lane: SC-1-TAILS
-- based-on: custom._table_move_plan(uuid,uuid,uuid) b310c7a25fe23c924d74d027d07959e7288d7e076450d4c69399c63fbbcd69f4
-- based-on: custom.table_move(uuid,uuid,integer) 8d70e52b537c113d33dae81d93e0a3b9ed49811bcc6d686235f56005591a59f2
--
-- LANE SC-1-TAILS (the tails of SC-1', SCOPES-CONTEXT-TRANSITION.md; the owner, 2026-09-23:
-- "see what org this data is in … and set the org for something").
--
-- THE USE CASE. Cascade Electronics Recovery's Tacoma Yard keeps "Scale tickets". Each ticket
-- that fails inspection gets its own "Hand-sort log" table INSIDE that ticket's row (a Table
-- whose home is the row), and one of those logs keeps its own "Sort bins" table inside one of
-- ITS rows. Portland is taking over weighing. Before this file, moving Scale tickets was refused
-- with "Hand-sort log lives inside one of its rows … take it out first" — the owner would have
-- had to un-nest every log by hand. Now the move carries every table inside its rows, and every
-- table inside those, with their columns, rows, choice lists and history; the confirm says how
-- many ride along; and Tacoma's and Portland's live readers (the grid, matrx-local's sync, every
-- realtime subscriber) hear of it at once through the store's own outbox.
--
-- WHAT CHANGED, AND WHY EACH PART IS THE DOCTRINE'S:
--   1. CONTAINED TABLES MOVE WITH THEIR PARENT (REC-9 / REC-10: a record whose row holds
--      something is its container, and what is contained follows it). The carry set is now a
--      closure: the Table, its own choice lists, every Table whose home is one of the carried
--      Tables or one of their rows (live or in the trash), and the same again, to the
--      containment ceiling (REC-N-4: 16). Records of a Table that STAYS but live inside a moving
--      row are still refused — a Table's rows never split across two organizations.
--   2. THE MOVE RE-KEYS IN CONTAINMENT ORDER. custom._containment_guard and the wall judge each
--      row as it lands, so a row moves only once what it points at — its container, its Table,
--      the Table its Field belongs to — has already moved. One loop, one statement per wave.
--   3. A LINK BETWEEN ROWS MAY CROSS THE WALL WHERE THE WALL IS OPEN (REC-29 / VIS-34, and
--      RELATION-TARGETS: a relation across organizations holds its value). A relation VALUE
--      between a moving row and a row that stays is kept when the Table it starts from allows
--      links to other organizations AND both organizations have turned links on; the edge stays
--      filed under the organization of the row it starts at (REL-12). When the wall is shut the
--      move is refused with the sentence that says which of the three to turn on.
--   4. A COLUMN'S OWN TARGET NEVER CROSSES. custom.organization_references marks a Field's
--      `relation_target` and `options_table_id` NOT openable ("the table a field points at never
--      crosses an organization"), so a column linking to a Table that stays is still refused —
--      measured on the clone with the wall fully open: 23503 "the table this relation field
--      points at belongs to a different organization". Whether linked Tables should move
--      together, or the wall should open for a column's target, is the chair's ruling
--      (PROGRESS-SC-1-TAILS.md), not this file's.
--   5. A MOVE IS A CHANGE EVENT. A row that changes organization is, underneath, a DELETE from
--      one partition and an INSERT into another, and the store's capture triggers pair old and
--      new by (organization, id) — so until now a move emitted nothing and a sync client learnt
--      of it on its next full read. The door now writes, for every live row it moved, one
--      `deleted` event in the organization it left and one `created` event in the one it joined
--      (custom.io_outbox, event_key records.changed — the same row every writer produces), so
--      pg_notify('records_changed') and the realtime broadcast fire in the same transaction.
--      The actor says `declared: table_move` with both organizations.
--
-- LOCKS. create or replace function only — no table lock. Not window-class.
...
comment on function custom._table_move_plan(uuid, uuid, uuid) is
  'SC-1'' + SC-1-TAILS. The ONE place that decides what moving a Table to another organization '
  'carries (the Table, every Table inside it or its rows to the containment ceiling, their own '
  'choice lists and what is built only on them) and what refuses it, for both custom.table_home and '
  'custom.table_move. A relation value between a moving row and a row that stays holds across the '
  'wall where REC-29 / VIS-34 open it; a column''s own target never crosses. Internal: no client '
  'grant; it trusts its caller to have decided that the person may open the Table.';

create or replace function custom.table_move(p_table_id uuid, p_to_organization_id uuid,
...
comment on function custom.table_move(uuid, uuid, integer) is
  'SC-1'' + SC-1-TAILS. Moves a Table — with every Table inside it or its rows, their Fields, '
  'Records, own choice lists, rules, dashboards, templates, forms, comments, links, saved views, '
  'associations and history — into another organization the caller belongs to, in containment '
  'order, and writes the change events (deleted where it was, created where it is) on '
  'custom.io_outbox; or refuses with ONE sentence (custom._table_move_plan decides both). Only the '
