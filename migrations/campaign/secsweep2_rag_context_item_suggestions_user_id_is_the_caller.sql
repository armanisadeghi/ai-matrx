...
--
-- rag.context_item_suggestions.user_id is the person the row is about, and the app writes this row AS that
...
-- Inverse: migrations/inverse/secsweep2_rag_context_item_suggestions_user_id_is_the_caller.inverse.sql
-- Guard: `pnpm check:unpinned-security-columns` -- both baseline rows for this pair leave.

set local lock_timeout = '2s';

create policy "context_item_suggestions_user_id_is_the_caller_insert" on rag.context_item_suggestions
  as restrictive for insert to authenticated, anon
  with check (user_id = (select auth.uid()) or (select public.is_platform_admin()));

create policy "context_item_suggestions_user_id_is_the_caller_update" on rag.context_item_suggestions
  as restrictive for update to authenticated, anon
  with check (user_id = (select auth.uid()) or (select public.is_platform_admin()));

comment on policy "context_item_suggestions_user_id_is_the_caller_insert" on rag.context_item_suggestions is
  'SECURITY-SWEEP-2 2026-09-21: rag.context_item_suggestions.user_id names the CALLER, so it is pinned to auth.uid(). The generated std_insert/std_update policies pin created_by and organization_id and say nothing about this column, so a signed-in member could POST a row naming somebody else. RESTRICTIVE, so it ANDs with every permissive policy present or future; bespoke on purpose, so iam.apply_rls preserves it (DD-147). Evidence the column names the caller: every live row has user_id = created_by. NULL is allowed where the column is nullable -- NULL names nobody and cannot impersonate. Reads, deletes and service_role are untouched.';
comment on policy "context_item_suggestions_user_id_is_the_caller_update" on rag.context_item_suggestions is
  'SECURITY-SWEEP-2 2026-09-21: rag.context_item_suggestions.user_id names the CALLER, so it is pinned to auth.uid(). The generated std_insert/std_update policies pin created_by and organization_id and say nothing about this column, so a signed-in member could POST a row naming somebody else. RESTRICTIVE, so it ANDs with every permissive policy present or future; bespoke on purpose, so iam.apply_rls preserves it (DD-147). Evidence the column names the caller: every live row has user_id = created_by. NULL is allowed where the column is nullable -- NULL names nobody and cannot impersonate. Reads, deletes and service_role are untouched.';
