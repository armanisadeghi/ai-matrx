-- chair-step: this builds the external tier by CALLING platform.provision(spec) twice — a spec-driven builder, the shape §6b.2's additive allow-list refuses by name because it cannot read what the statement will execute — and it revokes the new private schema custom_external from every client role; both are correct refusals and the sanctioned route for both is an attended step | LAND 2026-09-18: this is the rehearsed file with TWO mechanical changes and no others — (1) every `arg_checks.null_rule` is written as the OBJECT migration 0796's validator demands instead of an English sentence, and every uuid argument that named no `entity` now says `"entity": null` with an `entity_reason`, because the main database enforces 0796 and the rehearsal copy did not; (2) each `platform.provision(spec)` call is guarded by `to_regclass` so the file is idempotent and can rehearse on the copy, where these relations already stand. Every English sentence is kept verbatim in its `check`. The corrected specs were run through `platform.provision_validate(<spec>)` on the main database, read-only and rolled back, before this file was applied.
--
...
do $land_external_source$
begin
  -- IDEMPOTENT BY `to_regclass`, and not for convenience: the rehearsal copy already carries
  -- this relation, provisioned there under the older validator, and `platform.provision`
  -- refuses a token whose relation still stands with a different declaration — so the copy
  -- would refuse this file and the main database would never see it.
  if to_regclass('custom.external_source') is null then
    perform platform.provision($provision_source$
{
  "schema": "custom",
  ...
  "functions": [
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the caller's own organization, asserted by the caller before it reaches this door: the row is written with exactly this organization_id, which is the leading column of every RLS policy on custom.external_source and of the unique index that makes a source unique per tenant. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_tier": {
        ...
        "p_link_template": {
          ...
          "null_rule": {
            "means": "the caller supplies each stub's whole link"
          }
        }
      },
    ...
    },
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the caller's own organization: the UPDATE is keyed (organization_id, id), so a source belonging to another tenant is not found and the call raises 0 rows rather than flipping somebody else's opt-in. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_source_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the external source whose opt-in is moving; it must exist within p_organization_id or the call is refused with 02000 (no_data) naming both ids. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_enabled": {
...
$provision_source$::jsonb, 'runner');
    ...
    raise notice 'custom.external_source already exists here; platform.provision(spec) was not called.';
  end if;
end
$land_external_source$;

-- ── REC-N-10 / REL-N-1 / HIS-N-3 / DOOR-N-6: the stubs, THROUGH THE DOOR ──────
do $land_external_link$
begin
  -- IDEMPOTENT BY `to_regclass`, and not for convenience: the rehearsal copy already carries
  -- this relation, provisioned there under the older validator, and `platform.provision`
  -- refuses a token whose relation still stands with a different declaration — so the copy
  -- would refuse this file and the main database would never see it.
  if to_regclass('custom.external_link') is null then
    perform platform.provision($provision_link$
{
  "schema": "custom",
  "table": "external_link",
  "token": "external_link",
  ...
  "functions": [
    {
      ...
      "arg_checks": {
        "p_entity_token": {
          ...
          "null_rule": {
            "means": "the constructor still builds the reference; the CHECK on custom.external_link.target_ref refuses a reference whose entity key is missing, and a json null is a present key"
          }
        },
        "p_row_id": {
          "check": "the target row's uuid in our own database. It is the only arm where a uuid is the key at all.",
          "null_rule": {
            "means": "the constructor still builds the reference; the column CHECK is what refuses a malformed one at write time"
          },
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the target row's uuid in our own database. It is the only arm where a uuid is the key at all. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        }
      },
    ...
    },
    {
      ...
      "arg_checks": {
        "p_connection_token": {
          ...
          "null_rule": {
            "means": "the constructor still builds the reference; the column CHECK refuses a reference missing this key at write time"
          }
        },
        "p_external_table": {
          ...
          "null_rule": {
            "means": "the constructor still builds the reference; the column CHECK refuses it at write time"
          }
        },
        "p_external_key": {
    ...
    },
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the caller's own organization, asserted by the caller before it reaches this door: the stub Record and the link row are both written with exactly this organization_id, which is custom.record's partition key and the leading column of every policy on both tables. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_source_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the registered external source, which must exist inside p_organization_id; it is read for the tier, the connection token and the link template, and a source from another tenant is simply not found (02000). It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_table_id": {
          ...
          "null_rule": {
            "means": "a stub that belongs to no custom table"
          },
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the custom Table this stub belongs to; it is passed straight through to custom.record_write, stored, and never used to decide access. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_external_key": {
        ...
        "p_link_url": {
          ...
          "null_rule": {
            "default": "the source's link_template, and then to null"
          }
        },
        "p_cached_title": {
          ...
          "null_rule": {
            "means": "the remote reported no title; fetched_at still moves, because the fetch happened"
          }
        }
      },
    ...
    },
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the caller's own organization: the stub and its source are both looked up keyed by it, so a record in another tenant is not found (02000) rather than written through. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_record_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the stub Record whose outside row is being written. It must carry a link row in this organization; a native Record with no link is refused with 22023, because writing through a row that is not a stub means the caller has the wrong id. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_patch": {
          ...
          "null_rule": {
            "means": "null is legal and is refused exactly as a non-null patch is; the refusal does not depend on the payload"
          }
        }
      },
    ...
    },
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the caller's own organization; the link row is looked up keyed by it and the event is stamped with it, so an event can never be written against another tenant's link. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_link_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the link the event is about. It must exist inside p_organization_id (02000 otherwise); everything written into row_data is read from THIS row, which is ours. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_operation": {
    ...
    },
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the caller's own organization; every stub consulted is read keyed by it, so no row of another tenant's link table can be joined into the answer. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_source_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the registered source whose relation is being read. Its external_table is resolved ONLY inside custom_external, with format(%I) quoting, and a source whose relation is absent there is refused with 0A000 naming DOOR-N-6 rather than reading anything else. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        }
      },
...
$provision_link$::jsonb, 'runner');
    ...
    raise notice 'custom.external_link already exists here; platform.provision(spec) was not called.';
  end if;
end
$land_external_link$;
