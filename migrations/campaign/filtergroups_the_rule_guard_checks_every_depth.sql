-- chair-step: it replaces the BODY of the trigger function custom._rule_shape_guard() (the
--   BEFORE trigger that judges every Rule record written into custom.record) so that its three
--   jsonpath walks — field ids by REC-17, node names by REC-15, sibling_count's fields — read EVERY
--   depth (`$.**`) instead of stopping at twelve JSON steps (`$.**{0 to 12}`, which is six nested
--   ALL / ANY groups). Every check is the same check; it only reaches further. No trigger is
--   created, dropped, enabled or disabled, so no table lock is taken beyond ACCESS SHARE. It is a
--   chair step, not a guarded replacement, because the body reads its switch through
--   custom.assert_store_door rather than naming custom/system_enabled itself, and a trigger-returning
--   function is not exempt from guard-unread. The inverse is
--   `migrations/inverse/filtergroups_the_rule_guard_checks_every_depth_down.sql`.
-- lane: S2-PRIME
-- lock: custom
-- based-on: custom._rule_shape_guard() 0ed917e6c4ea9690a99561ed6ebaa85b2519f4c232127679ec9a52bb389ac798
--
-- LANE S2-PRIME FILTER-GROUPS. A view's filter nests as deep as a person builds it (the condition
-- builder now draws groups inside groups), and custom.rule_filter_node_sql compiles any depth. The
-- guard that refuses a Rule pointing at a field that is not one of its table's fields, or asking a
-- node nobody implements, has to reach as far: measured on the dev clone, a Rule whose bad field id
-- sat seven groups down was STORED (filtergroups_red.sql, R4).
...
begin
  ...
  -- The switch never removes a check: everything below runs exactly as before. (The switch
  -- is custom/system_enabled, which custom.assert_store_door resolves.)
  ...
  -- S2-PRIME FILTER-GROUPS: EVERY DEPTH. These walks read `$.**{0 to 12}` — twelve JSON steps,
  -- which is SIX nested ALL / ANY groups — so a field id or a node seven groups down was stored
  -- without ever being checked. A view's filter nests as deep as a person builds it, and so does
  -- this check now.
  ...
  for v_leaf in
    select jsonb_path_query(d -> 'expr',
             '$.** ? (exists(@.field) || exists(@.parent_field) || exists(@.field_name) || exists(@.field_key) || exists(@.field_label))')
  loop
  ...
  for v_leaf in
    select jsonb_path_query(d -> 'expr', '$.** ? (exists(@.op))')
  loop
  ...
  for v_leaf in select jsonb_path_query(d -> 'expr', '$.** ? (@.op == "sibling_count")')
  loop
