...
--
-- 🚨 WHY THIS FILE IS NOT `w1_prov_the_provisioner_honours_declared_exposure.sql`, WHICH THE
...
begin
  ...
  if v_exposed is not null and not v_exposed then
    ...
    raise notice
      'apply_table_grants: %.% — schema % is declared CLOSED to client roles in platform.schema_client_exposure (%). NO grant was issued to PUBLIC, anon, authenticated or service_role and every standing one was revoked; the % variant''s grants were NOT applied. To open the schema: update platform.schema_client_exposure set client_exposed = true, reason = ''<why>'', declared_by = ''<who>'' where schema_name = %L; then re-run the provisioner.',
      p_schema, p_table, p_schema, v_closed_reason, p_variant, p_schema;
    return;
  end if;
