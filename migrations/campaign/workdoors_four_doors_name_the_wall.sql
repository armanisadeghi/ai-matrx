...
-- based-on: custom.work_approval_request(uuid, uuid, jsonb, text, uuid, text, uuid) 0af2edce8fbb04c332aa5ff301f594798705c8d077b6a29e6f3c137c81187cf5
-- based-on: custom.work_instantiation_shape(uuid, uuid) bf5d8924d43eaa76362965975c4a032633543e7460b16b8cd4e32ad94fbdcc35
-- based-on: custom.work_record_states(uuid, uuid) ecfa921c7fba06c2ccd9b784f3df1601b807fd6ba9be2caa643a907ccabc4d44
-- based-on: custom.work_template_shape(uuid, uuid) c75ba123b5deda0f85d88edc1c7108de39138f73032973fd0299e7d0812f7ed4
--
-- WORK-DOORS — FOUR MORE DOORS NAME THE WALL THEMSELVES (census 1).
--
-- `pnpm check:store-doors-decide` census 1 asks a body whether it DECIDES THE CALLER, and it
-- asks by reading the body for one of six names. `custom.assert_client_may_open` is not one of
-- them — it is the ROW question, and it happens to ask the wall on its own first line. So four
-- doors that were in fact perfectly walled read to the census as walls-by-implication.
--
-- The census is right to read it that way and this file agrees with it rather than widening
-- it: a door states its own wall, by name, as its first question — which is exactly what
-- `custom.my_level` has always done (`assert_client_may_reach` then `assert_client_may_open`),
-- and what makes the refusal say "you are not a member of that organization" instead of "you
-- do not have access to this record" to somebody who is in the wrong organization entirely.
-- Nothing about who may do what changes; four bodies gain one line each.
--
-- THE INVERSE: `migrations/inverse/workdoors_four_doors_name_the_wall_down.sql`.
...
AS $function$
begin
  perform custom.assert_client_may_reach(p_organization_id, 'custom.work_instantiation_shape');
...
AS $function$
begin
  perform custom.assert_client_may_reach(p_organization_id, 'custom.work_template_shape');
