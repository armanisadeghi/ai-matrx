...
--
-- iam.org_industries -- IDENTITY SEMANTICS.
-- Which industries an organization claims. Doors: `industry_assign_org` / `industry_unassign_org`; both client call sites are `.select(...)`.
--
-- WHY IT MATTERS: A client write here reassigns an organization's identity in the shared registry.
--
...
--
-- THE DOOR IS UNAFFECTED. `iam.org_industries` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_iam_org_industries_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "org_industries_client_insert_refused" on iam.org_industries
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "org_industries_client_update_refused" on iam.org_industries
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "org_industries_client_delete_refused" on iam.org_industries
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "org_industries_client_insert_refused" on iam.org_industries is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A client write here reassigns an organization''s identity in the shared registry. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "org_industries_client_update_refused" on iam.org_industries is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A client write here reassigns an organization''s identity in the shared registry. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "org_industries_client_delete_refused" on iam.org_industries is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A client write here reassigns an organization''s identity in the shared registry. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
