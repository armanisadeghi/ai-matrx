-- LANE S5-PRIME-2, FOLLOW-UP — AN ARCHIVED ORGANIZATION IS NEVER REMINDED.
--
-- THE DEFECT (found on production before the reminder tick first ran, 2026-09-25 02:13 UTC): a
-- rolled-back dry run of custom.inbox_remind_tick() on production would have sent 97 reminders, 54
-- of them about asks left in 14 ARCHIVED organizations (suite and walk organizations nobody works
-- in any more). custom.inbox_counts already skips an archived organization; the tick did not, so
-- the badge and the reminders disagreed about the same person. The job was paused
-- (cron.alter_job active=false) within a minute, before its first run, and this file fixes the
-- tick and turns the job back on.
--
-- WHAT IT CHANGES: custom.inbox_remind_tick() only (one join on iam.organizations, archived_at is
-- null), and re-activates pg_cron job custom-inbox-remind-tick wherever pg_net exists (production;
-- the quarantined clone keeps it inactive).
--
-- INVERSE: migrations/inverse/uichamp_s5b2_an_archived_organization_is_never_reminded_down.sql
--
-- based-on: custom.inbox_remind_tick() 01262e654d73ffffb3ed154a8dc171ad7246a36aa03ae952f6b46a9aba338b5e
-- lane: S5-PRIME-2
...
select cron.alter_job(j.jobid, active := exists (select 1 from pg_extension where extname = 'pg_net'))
  from cron.job j
