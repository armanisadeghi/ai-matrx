...
--   The inverse is migrations/inverse/portalbind_a_withdrawn_link_says_so_down.sql.
--
-- PORTAL-BIND — A WITHDRAWN LINK SAID "ALREADY OPEN TO YOU".
--
-- Caught by clause 8 of scripts/campaign-tests/portalbind_green.sql before it shipped, on the
-- real bytes: the office revoked the customer, `custom.read_records` correctly refused her in
-- the same statement — and the peek her link lands on read `accepted` and said *"This link has
-- been used — your portal is already open to you."* It is not. That is the exact shape this
-- campaign calls a screen lying: the read path already answered "no" and the screen beside it
-- said "yes".
--
-- The cause is an ORDER, and the order is the class. `iam.invitations.status = 'accepted'` is
-- the HISTORY of how she was asked; it does not become false when access is taken away, and
-- `custom.portal_revoke` leaves it standing on purpose (soft-delete-everything: an accepted
-- invitation is a record of something that happened). Whether she can get in is a question
-- about the PRINCIPAL. The peek asked the invitation first, so a revoke after an accept was
-- invisible to it. The principal is now asked first.
--
-- based-on: public.portal_share_peek(text) c90287f2605906ef42ea8a0539b1772684a6afb97bae786baf5be3d19ab26b7f
