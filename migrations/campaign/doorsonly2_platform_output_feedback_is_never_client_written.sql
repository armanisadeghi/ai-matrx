...
-- removing the door.
--
-- platform.output_feedback -- FEEDBACK SEMANTICS.
-- Feedback on an output. `lib/output-feedback/service.ts` and the Masterwork signature reader name it; neither contains a write verb.
--
-- WHY IT MATTERS: Feedback a client can write directly is feedback a client can write in somebody else's name.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.output_feedback` is owned by `postgres` and does NOT carry
