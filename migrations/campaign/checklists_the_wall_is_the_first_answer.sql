...
-- based-on: custom.checklist_declare(uuid, jsonb, uuid) 4860d94ccb8daac92db7c2c7019462590c2af29d525a3695bacd74806e27280f
-- based-on: custom.checklist_start(uuid, uuid, uuid, jsonb, timestamp with time zone) 22bba11a70d394262c79318c2282385cef7df4c179c779f1aa5fa09d1471feb9
-- based-on: custom.checklist_step_complete(uuid, uuid, jsonb) f612358df7e0ccbaa83b42a2aa511751d637b1d7f10e65a75ce656d432cec289
--
-- CHECKLISTS — the seventh thing the green suite caught, and it is about what a person is TOLD.
--
-- All three writing doors asked `custom.assert_store_door` before the organization wall, which
-- is the convention the sibling work doors follow. The effect, measured at 15:26 UTC: somebody
-- who typed an organization id they have nothing to do with was answered *"The custom data
-- store is switched off, so custom.checklist_start is not taking writes from
-- \"authenticated\"."* That is a true sentence about somebody else's organization, said to
-- somebody who is not in it — and it is not the thing that is actually wrong.
--
-- `custom.assert_client_may_change` already carries this rule in its own body: *"ONE order,
-- always: the organization wall first, then the row. A door that asked the row first would
-- answer 'you may not touch this record' to somebody who should have been told they are in the
-- wrong organization entirely."* The switch is the same kind of second question. So the order
-- on these three is the wall, then the switch, then the row.
--
-- NOTHING IS OPENED. The OFF switch still closes every one of these doors for every member of
-- that organization, in the same call, with the same sentence; the only person whose answer
-- changes is somebody who was never getting through either question.
--
-- The inverse is `migrations/inverse/checklists_the_wall_is_the_first_answer_down.sql`.
...
begin
  -- THE WALL FIRST, THEN THE SWITCH. Somebody in another organization is told THAT, and is
  -- not told anything at all about whether that organization's store is switched on.
...
AS $function$
begin
  -- THE WALL, BY NAME, FIRST — before the row AND before the switch. Somebody in another
  -- organization is told THAT: not that they cannot open a checklist they were never allowed
  -- to know exists, and not that a store they have nothing to do with is switched off.
