...
-- removing the door.
--
-- platform.outsider_consumer -- IDENTITY SEMANTICS.
-- Who outside the platform may consume from it.
--
-- WHY IT MATTERS: A member who writes this table admits an outsider.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("outsider_consumer")` search AND a
-- bare-name search of `"outsider_consumer"` / 'outsider_consumer' / `outsider_consumer` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.outsider_consumer` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_outsider_consumer_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "outsider_consumer_client_insert_refused" on platform.outsider_consumer
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "outsider_consumer_client_update_refused" on platform.outsider_consumer
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "outsider_consumer_client_delete_refused" on platform.outsider_consumer
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "outsider_consumer_client_insert_refused" on platform.outsider_consumer is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A member who writes this table admits an outsider. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "outsider_consumer_client_update_refused" on platform.outsider_consumer is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A member who writes this table admits an outsider. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "outsider_consumer_client_delete_refused" on platform.outsider_consumer is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). IDENTITY semantics: A member who writes this table admits an outsider. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
