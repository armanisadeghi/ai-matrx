...
-- additive: yes
--   It ADDS `custom.scope_table_provision` and its `platform.client_callable_door` row. Nothing
...
create function custom.scope_table_provision(p_organization_id uuid, p_home_id uuid,
                                             p_item_id uuid, p_scope_id uuid)
returns uuid
language plpgsql
security definer
...
values ('custom', 'scope_table_provision', 'p_organization_id uuid, p_home_id uuid, p_item_id uuid, p_scope_id uuid',
        ...
        jsonb_build_object('version', 1, 'declared_by', 'gridprim_a_scope_provisions_its_table_in_the_store.sql',
          ...
          'arguments', jsonb_build_object(
            ...
            'p_home_id', jsonb_build_object('type', 'uuid', 'position', 2, 'entity', 'custom_record',
              ...
              'verified', '2026-09-23 lane GRID-PRIMITIVES — written with this body'),
            'p_scope_id', jsonb_build_object('type', 'uuid', 'position', 4, 'entity', 'context_scope',
              ...
              'foreign', jsonb_build_object('sqlstate', '42501', 'same_as_invented', true),
              'verified', '2026-09-23 lane GRID-PRIMITIVES — written with this body'),
            'p_item_id', jsonb_build_object('type', 'uuid', 'position', 3, 'entity', 'context_item',
...
begin
  ...
  if v_moved then
    select h.id into v_home from custom.record h
     where h.organization_id = v_org and h.table_id = custom.organization_kernel_id() and h.deleted_at is null
  ...
  if tg_table_name='scopes' then
    ...
    loop
      if v_moved then perform custom.scope_table_provision(v_org, v_home, r.id, new.id);
    ...
    if new.is_active and new.deleted_at is null and new.reference_source->>'container_type'='dataset_template' then
      ...
      loop
        if v_moved then perform custom.scope_table_provision(v_org, v_home, new.id, r.id);
