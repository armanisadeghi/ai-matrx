...
--
-- Two defects the FIELD-ADD lane left behind, and both of them are a class.
--
-- ONE. `custom._field_shape_guard`'s empty-key refusal stopped in mid-air:
--
--     a field needs a key made of lower-case letters, digits and underscores,
--     and this one says
--
-- because the sentence ends in `coalesce(v_key, 'nothing')` and the key it was
-- refusing was the EMPTY STRING, which coalesce happily keeps. Twenty-four
-- refusals across twelve of the store's guards were written the same way, so
-- every one of them says nothing at all about the one value a person is most
-- likely to send: blank. The class is closed by a single helper every one of
-- them now calls - `custom.said(value, when_there_is_none)` - which treats
-- blank and missing as the same thing, and by a census,
-- `custom.guard_refusals_are_complete()`, which lists any guard that could
-- stop mid-air again. It returns rows today and none after this file.
--
-- TWO. `custom._field_type_parity_guard` refused a soft delete. Its sibling
-- `custom._field_shape_guard` carries the rule DOOR-FIX and TABLE-DELETE
-- built - a retirement is not a change of shape, because the document is
-- byte-for-byte what it was - and the parity guard did not, so it re-ran its
-- whole FLD-11 check on a row that was only leaving. Retiring a lookup in the
-- same statement as the relation it reads through was therefore refused with
--
--     the field Lead name reads through a relation called crew_lead,
--     and this table has no field called crew_lead
--
-- which is true, and is caused by the very delete being judged. Eleven guards
-- on `custom.record` judged a retirement that way. They now all ask the same
-- question, `custom.is_a_retirement(...)`, and the two that had written the
-- rule out by hand ask it through that one predicate instead.
--
-- AND THE DOOR. `custom.field_retire` refused to remove a relation while a
-- lookup or a rollup read through it, telling a person to go and find it. It
-- now retires the dependent fields WITH the relation, in the same operation -
-- the shared delete rule, the same one `custom.delete_rule` applies when a
-- Table takes its fields with it - and says which ones went. It still refuses,
-- by name, when the set would take the title field or the table's last field.
--
-- ADDITIVE: two new helper functions, one census function, fourteen guard
-- bodies replaced in place, one door body replaced in place. Nothing is
-- dropped, no signature changes, no trigger is re-pointed.
--
-- THE INVERSE: migrations/inverse/fieldguards_a_refusal_is_a_whole_sentence_down.sql
--
-- THE BODIES THIS FILE WAS WRITTEN AGAINST (DD-220: a replace declares its base):
-- based-on: custom._containment_guard() 3bf209c1f5f1fc260f79b58065206ffd9b62686b8df437bf26681ca432dd29c7
-- based-on: custom._dated_values_guard() 441ff35e6404c5d4ade92cb4def9a91719a1246d2138153ba9a72b1d44e30530
-- based-on: custom._field_shape_guard() 9bf09aedf257e9a073e78c378578d3739388a1b7ab59cc9c43c2e9b5886a3889
-- based-on: custom._field_type_converts_values() 2c0fa095574b2b70d3bf966ebbf9f47a6239c0c1aa5dbbaeb00890cba66cc8d2
...
-- based-on: custom._merge_field_temporal_guard() 0889d6f462e02295be30a8aada4f332987a87889c7a033b3306e810f447bdd98
-- based-on: custom._promoted_field_cap_guard() ba27b379d80783c20c4cac613d51a65f16c71f22045bf8dda0a94bb0e820e7fd
-- based-on: custom._record_field_validation() 3b33cbc26297b261f19a6bd3a914a6714c79ac09c5f49ae893c3940b1d12b2ab
...
-- based-on: custom._store_relation_edge_names_its_field() 6dbfdd94fb04bdc23edd1f2b924c64eab16859b5c8dc49f36deabcedc4b1a63b
-- based-on: custom._table_shape_guard() 44493144d20640d82cf6eb693a21ba6e646db8d4a069bc77d7d4ea3a4191e041
-- based-on: custom._work_shape_guard() 72b86289a0db4875c538b3b17f915c0e1bfa5e3e4776f865a6d6ca8f313ce20e
-- based-on: custom.field_retire(uuid, uuid) fa792f7c7eb05e0534701125d00b57e20c34e60c497f74bed658ba0c2ce72c9d

set lock_timeout = '5s';
set statement_timeout = '600s';

-- ── THE WORD FOR A VALUE THAT IS NOT THERE. Missing and blank are the same thing to a
--    person reading a refusal, so they are the same thing here. btrim means a key of
--    three spaces is also "nothing" rather than three spaces of silence.
create or replace function custom.said(p_value text, p_when_there_is_none text)
returns text
language sql
immutable
set search_path to 'pg_catalog'
as $$
  select coalesce(nullif(btrim(p_value), ''), p_when_there_is_none);
$$;

comment on function custom.said(text, text) is
  'The value a guard is about to name in a refusal, or the words for "there was none". A guard that writes coalesce(x, ''nothing'') by hand says nothing at all when x is the empty string - which is exactly the write a person is most likely to make. custom.guard_refusals_are_complete() lists any guard that still does.';

-- ── THE ONE QUESTION EVERY SHAPE GUARD ASKS ABOUT A SOFT DELETE. Taking a row OUT of the
--    store is the one write a shape check has nothing to say about: the document, the
--    table, the organization and the data class are all byte-for-byte what they were, and
--    the only change is `deleted_at` going from nothing to a time. A restore (a time going
--    back to nothing) is NOT a retirement and is judged in full, as is any write that
--    changes the document. The columns are passed one by one rather than as a row, because
--    custom.record is partitioned and a partition's row type is a different type.
create or replace function custom.is_a_retirement(
  p_old_deleted timestamptz, p_new_deleted timestamptz,
  p_old_data jsonb,          p_new_data jsonb,
  p_old_table uuid,          p_new_table uuid,
  p_old_org uuid,            p_new_org uuid,
  p_old_class text,          p_new_class text)
returns boolean
language sql
immutable
set search_path to 'pg_catalog'
as $$
  select p_old_deleted is null
     and p_new_deleted is not null
     and p_old_data    is not distinct from p_new_data
     and p_old_table   is not distinct from p_new_table
     and p_old_org     is not distinct from p_new_org
     and p_old_class   is not distinct from p_new_class;
$$;

comment on function custom.is_a_retirement(timestamptz,timestamptz,jsonb,jsonb,uuid,uuid,uuid,uuid,text,text) is
  'A RETIREMENT IS NOT A CHANGE OF SHAPE. True when the only difference between the old row and the new one is that deleted_at went from nothing to a time. Every shape guard on custom.record asks this before it judges anything, which is what lets a dependent field retire in the same operation as the relation it reads through - the shared delete rule DOOR-FIX and TABLE-DELETE built.';

-- ── THE CENSUS. A refusal that names a value has to name it even when the value is blank.
--    This finds the shape that cannot: a two-argument coalesce whose first argument is a
--    plain read (no nullif, no custom.said) and whose second is the words for "none".
create or replace function custom.guard_refusals_are_complete()
returns table (guard text, said text)
language sql
stable
set search_path to 'pg_catalog'
as $$
  select n.nspname || '.' || p.proname, m[1]
    ...
    join pg_namespace n on n.oid = p.pronamespace,
         lateral regexp_matches(p.prosrc, 'coalesce\([^(),]*,\s*''[^'']+''\)', 'g') m
   where n.nspname = 'custom'
     and p.prorettype = 'trigger'::regtype
   order by 1, 2;
$$;

comment on function custom.guard_refusals_are_complete() is
  'Every place in the store''s guards where a refusal could stop in mid-air: coalesce(x, ''nothing'') says nothing at all when x is the empty string. Zero rows is the law. The remedy is custom.said(x, ''nothing''), which treats blank and missing alike.';
...
begin
  ...
  if coalesce(d ->> 'source', '') <> all (c_sources) then
    raise exception '% says its value comes from %, and the list of places a value may come from is closed',
                    v_name, custom.said(d ->> 'source', 'nothing')
      using errcode = '23514',
  ...
  if coalesce(d ->> 'semantic_type', '') <> all (c_semantic) then
    raise exception '% says it resolves into %, and it resolves into a value, a reference, a resolver, something computed, or a collection',
                    v_name, custom.said(d ->> 'semantic_type', 'nothing')
  ...
  if d ? 'override_policy'
    ...
    raise exception '% says a person may change it by %, and that is not one of the five ways',
                    v_name, custom.said(d ->> 'override_policy', 'nothing')
      using errcode = '23514',
...
begin
  ...
  if coalesce(v_mode, '') not in ('live', 'as_of', 'snapshot') then
    raise exception '% reads its value %, and a merge field reads it live, as of a moment, or as a snapshot.', v_name, custom.said(v_mode, 'nobody said how')
    ...
    loop
      ...
      if jsonb_typeof(v_leaf -> 'parent_field') <> 'string'
        ...
        raise exception 'the rule % points at the parent''s field with % instead of with its id',
                        v_name, custom.said(v_leaf ->> 'parent_field', 'something that is not there')
          using errcode = '23514',
...
begin
  ...
  if v_type is null or v_type not in ('entity', 'detail') then
    raise exception 'a table is an entity or a detail, and this one says %', custom.said(v_type, 'nothing')
  ...
  if coalesce(d ->> 'weight', '') not in ('heavy', 'light') then
    raise exception 'a table is heavy or light, and this one says %', custom.said(d ->> 'weight', 'nothing')
...
-- ── THE DOOR. A dependent field retires WITH the relation it reads through, in the same
--    operation - the shared delete rule, the same one custom.delete_rule applies when a
--    Table takes its fields, its views and its rules with it. What is still refused is the
--    same thing REC-2 and REC-1 always refused, now said with every name in it: a set that
--    would take what every record of this table is called, or the last field it has.
...
comment on function custom.field_retire(uuid, uuid) is
  'Removes a field from a table somebody made, and takes with it - in the same operation - every field of that table that works out its answer through it, transitively. Refuses by name, with every field in the set, when the set would take the title field or the table''s last field.';
