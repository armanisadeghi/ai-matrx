-- chair-step: this REPLACES four live function bodies, each with one added condition and nothing else (lane SC-4, P4's reader half). The three generic "everything attached to this" doors — public.assoc_for_entity, public.assoc_for_sources, public.assoc_list — leave out an edge whose role is `context_tag` on the side that lists an entity's OUTGOING edges, and custom.resolve_context reads an entity's tags under the store's own token `record` beside `scope` and `custom_record`. It is unconditional on purpose: a copied tag exists only while the copy follows, and an old list must never show one whatever a knob says, so no knob holds it off (the additive route's guard rule cannot apply). Signatures, grants, comments and every other line are unchanged; no row is written. Inverse: migrations/inverse/sc4_old_attached_lists_leave_out_the_tag_copies_down.sql.
-- lane: SC-4 (PICKER-AND-TAGS: the tag copies)
-- window-class: function bodies only (CREATE OR REPLACE FUNCTION); applied directly per the
--   owner's ruling of 2026-09-24 ~17:30 PT, under lock_timeout.
-- based-on: public.assoc_for_entity(text, uuid) b1e607d6d85c9f9ebf49d8be4f264690152c4e42872ca2986d66f44607d58979
-- based-on: public.assoc_for_sources(text, uuid[], text) 9b691c319805272127197643cd07064a7b92440441c179fee7cde80024491b5b
-- based-on: public.assoc_list(text, uuid, text, text) 776e5f3c50812e233354fbcd1186b91687f2d135a65919aa53dc948ef6047a23
-- based-on: custom.resolve_context(text, uuid, uuid[], uuid[]) 4d01d926de2fb7ff0dfb508f6c098395df16af9a3fe5a358e95571600d7b9362
--
-- LANE SC-4 PICKER-AND-TAGS — P4's reader half, decided by the census in PROGRESS-SC-4.md.
--
-- THE USE CASE. Priya opens a contract PDF's share popover (@ai-matrx/media, "attached to") or a
-- note's attachments: both list the item's outgoing edges of EVERY token through
-- public.assoc_for_entity. The PDF is tagged to the Harborline Dispatch scope, so from tonight it
-- also carries its copy, `file -> record` (role context_tag, sc4_every_context_tag_has_one_copy_in_the_store.sql).
-- Without this file the popover would list Harborline Dispatch twice — once as the scope and once
-- as an unnamed record. The census found every old tag reader keyed on `target_type = 'scope'`
-- (untouched by a `record` edge) and exactly three doors that list outgoing edges of any token;
-- those three leave the copy out. The INCOMING side is left as it is: an edge listed from the
-- Record's end is the new system's own "tagged to this" list.
--
-- THE AGENT'S SIDE. custom.resolve_context (SC-3') already read an entity's tags by target id
-- under `scope` or `custom_record`; the copy's token is `record`, so it reads that too. It groups
-- by target id, so an entity tagged under two tokens is one candidate — never a scope delivered twice.
...
AS $function$
  ...
  select a.id, 'outgoing'::text, a.target_type, a.target_id, a.role, a.label, a.position, a.metadata, a.organization_id, a.created_at
   ...
   where anchor_ok.ok                                                -- DD-205: may I read the anchor I named?
     ...
     and a.role is distinct from 'context_tag'                       -- SC-4: a copied tag is not an attachment
...
AS $function$
  ...
  select a.id, a.source_id, a.target_type, a.target_id, a.role, a.label, a.position, a.metadata, a.organization_id, a.created_at
   ...
   where a.source_type = p_source_type
     ...
     -- SC-4: a copied context tag is not an attachment; it shows only to a caller asking for Records.
     and (a.role is distinct from 'context_tag' or p_target_type = 'record')
     and iam.org_readable(a.organization_id, a.target_type)     -- the EDGE's own tenancy, unchanged
     and iam.assoc_side_readable(a.target_type, a.target_id)    -- DD-195: AND the row it reveals
  order by 7 nulls last, 10;
$function$;
