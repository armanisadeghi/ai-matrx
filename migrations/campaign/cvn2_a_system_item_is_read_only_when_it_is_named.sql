-- chair-step: public.resolve_full_context and custom.resolve_context each gain a last argument, p_system_item_refs text[] DEFAULT NULL (a signature change, so each is dropped and recreated with its grants and comment), and read a System context item ONLY when that list names it — NULL or empty reads none. Adds context.named_system_context_items(text[]) (the one gate, index lookups only), re-points context.deliverable_system_context_items() at it, adds the platform knob context/system_item_defaults (default current_date, current_datetime, current_timezone) and the System item current_timezone, and says in the two date items' descriptions that they are rendered in the person's timezone. WHAT CHANGES FOR A CALLER: a caller that does not pass the list gets no System item (the server passes it; the admin preview passes the knob). No table, column, policy or row other than the knob and the one System item is touched.
-- lane: CONTEXT-VALUES-NAMED-2
-- lock: platform
-- based-on: public.resolve_full_context(uuid, text, uuid, uuid[]) eec33db0cf17be604ccabdd9698390534253c6b367a895a9939d3e81c5c1474e
-- based-on: custom.resolve_context(text, uuid, uuid[], uuid[]) 42b120440dccfc71d1f3b4235590ab2ca16e1341ca6c06244613c293a1d3d9de
-- based-on: context.deliverable_system_context_items() b2eda89928701b3e6473b6f8883b344aae09ea9638b4ac6aae17a9970c87a864
--
-- Inverse: migrations/inverse/cvn2_a_system_item_is_read_only_when_it_is_named_down.sql.
--
-- THE USE CASE. Arman, 2026-09-25: "It must be named in context values or variables or it should not
-- be fed … these will grow to thousands … some of them will also trigger api calls or other
-- fetching." Lane 1 (aidream 7a62e5009f) stopped an unnamed System item reaching an agent, but both
-- resolvers still READ every deliverable System row on every turn and the server threw the unnamed
-- ones away. The chair ruled (c): the naming reaches the SQL itself, so nothing is read for an item
-- nobody named. The server hands both resolvers the same list (SystemContextNames.refs()): an
-- agent's bound System items plus the platform's default list, which is now this knob instead of a
-- constant (ruling a), and which carries the per-person timezone item (ruling b).

set local lock_timeout = '30s';
set local statement_timeout = '60s';

-- ── 1. THE ONE GATE, NAMED ─────────────────────────────────────────────────────────────────────
-- Only the rows the list names are fetched, by the primary key (an id) or the unique key index (a
-- key). Sequential scans are switched off inside it so an 8-row table is not read whole either: the
-- suite proves an unnamed row is never fetched (pg_stat_xact_user_tables: seq_scan 0, fetched =
-- named). The deliverability rule is unchanged: active, not deleted, public, no more sensitive than
-- internal, owned by the Matrx System tenant.
create or replace function context.named_system_context_items(p_refs text[])
...
as $function$
    -- One index lookup per ref: by the unique key index when the ref is a key, by the primary key
    -- when it is an id. The deliverability rule is applied to the rows found, never used to find
    -- them (an organization-index scan would fetch every System row to filter it).
    select distinct on (sci.id) sci.*
      from (select distinct u.ref from pg_catalog.unnest(coalesce(p_refs, '{}'::text[])) u(ref)) r
      cross join lateral (
            select s.* from context.system_context_item s where s.key = r.ref
            union all
            select s.* from context.system_context_item s
             where s.id = case when r.ref ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
                               then r.ref::uuid end
               and s.key is distinct from r.ref
      ) sci
...
comment on function context.named_system_context_items(text[]) is
  'THE ONE GATE for the System context lane (lane CONTEXT-VALUES-NAMED-2). Returns the deliverable System items the list names — each ref an id or a key — and nothing else; NULL or empty returns none without touching the table. Deliverable: active, not deleted, public, no more sensitive than internal, owned by the Matrx System tenant. Index lookups only.';

revoke all on function context.named_system_context_items(text[]) from public, anon, authenticated, service_role;
grant execute on function context.named_system_context_items(text[]) to service_role;

-- Every deliverable item, through the same gate (one rule, never two copies of it).
create or replace function context.deliverable_system_context_items()
 ...
 set search_path to ''
as $function$
    -- Every deliverable System item: the named gate asked for every key. The resolvers no longer
    -- call this — they ask context.named_system_context_items for the items a turn names.
    select d.* from context.named_system_context_items(
        array(select sci.key from context.system_context_item sci)) d
$function$;

-- ── 2. THE DEFAULT LIST IS A KNOB (ruling a) ───────────────────────────────────────────────────
...
values
  ('context', 'system_item_defaults',
   '["current_date", "current_datetime", "current_timezone"]'::jsonb,
   '["current_date", "current_datetime", "current_timezone"]'::jsonb,
   'json',
   'System items every agent receives',
   'Every agent receives these without naming them. A list of System context item keys. Any other '
   'System item reaches an agent only when the agent names it (a variable or context slot bound to '
   'it) or someone picks it in the context inspector; nothing else is read for it.',
   'agent',
   'Chair ruling, lane CONTEXT-VALUES-NAMED-2, 2026-09-25: exactly today''s date and the current date-time, plus the person''s timezone.',
...
-- ── 3. THE PERSON'S TIMEZONE IS A SYSTEM ITEM (ruling b) ──────────────────────────────────────
-- Computed by the server per person from the one timezone ladder
-- (communication.person_notification_window: what the person declared, what their browser reported
-- through /api/person/timezone, their profile, their work location, their organization's default),
-- UTC when nothing answers.
insert into context.system_context_item
  (key, display_name, description, item_class, value_type, value, feed_type, feed_config,
   sensitivity, is_active, sort_order)
values
  ('current_timezone', 'Your Timezone',
   'The timezone of the person running the agent (IANA name, e.g. America/Los_Angeles), from what '
   'they declared or their browser reported; UTC when nothing is known. Always fresh.',
   'ambient', 'string', '"(computed at runtime)"'::jsonb, 'computed',
   '{"kind": "reserved", "reserved_key": "current_timezone"}'::jsonb,
   'internal', true, 0)
on conflict (key) do update
   set deleted_at = null, is_active = true, updated_at = now(),
       version = context.system_context_item.version + 1
 where context.system_context_item.deleted_at is not null;

update context.system_context_item
   set description = 'Today''s date where the person running the agent is (their timezone, ISO-8601). Always fresh.',
       updated_at = now(), version = version + 1
 where key = 'current_date'
   and description = 'Today''s date (UTC, ISO-8601). Always fresh.';

update context.system_context_item
   set description = 'The current date and time in the timezone of the person running the agent, with its UTC offset (ISO-8601).',
       updated_at = now(), version = version + 1
 where key = 'current_datetime'
   and description = 'The current date and time (UTC, ISO-8601).';

-- ── 4. BOTH RESOLVERS TAKE THE NAMES (ruling c) ───────────────────────────────────────────────
drop function public.resolve_full_context(uuid, text, uuid, uuid[]);
...
update platform.client_callable_door d
   set identity_args = pg_catalog.pg_get_function_identity_arguments('public.resolve_full_context(uuid, text, uuid, uuid[], text[])'::regprocedure),
       identity_argtypes = platform.door_argtypes((select p.proargtypes from pg_catalog.pg_proc p where p.oid = 'public.resolve_full_context(uuid, text, uuid, uuid[], text[])'::regprocedure)),
       reason = d.reason || ' p_system_item_refs (lane CONTEXT-VALUES-NAMED-2) names the System items this turn may read — ids or keys; NULL or empty reads none; a ref naming nothing deliverable is simply absent. It is read only through context.named_system_context_items.',
       declared_by = 'cvn2_a_system_item_is_read_only_when_it_is_named.sql'
 where d.schema_name = 'public' and d.function_name = 'resolve_full_context'
   and d.identity_args = 'p_user_id uuid, p_entity_type text, p_entity_id uuid, p_scope_ids uuid[]';
revoke all on function public.resolve_full_context(uuid, text, uuid, uuid[], text[]) from public, anon, authenticated, service_role;
grant execute on function public.resolve_full_context(uuid, text, uuid, uuid[], text[]) to service_role;

drop function custom.resolve_context(text, uuid, uuid[], uuid[]);
...
-- A DOOR FOLLOWS ITS FUNCTION: the door row moves to the new signature in this same transaction.
update platform.client_callable_door d
   set identity_args = pg_catalog.pg_get_function_identity_arguments('custom.resolve_context(text, uuid, uuid[], uuid[], text[])'::regprocedure),
       identity_argtypes = platform.door_argtypes((select p.proargtypes from pg_catalog.pg_proc p where p.oid = 'custom.resolve_context(text, uuid, uuid[], uuid[], text[])'::regprocedure)),
       reason = d.reason || ' p_system_item_refs (lane CONTEXT-VALUES-NAMED-2) names the System items this turn may read — ids or keys; NULL or empty reads none; a ref naming nothing deliverable is simply absent. System items are platform-wide and read only through context.named_system_context_items.',
       declared_by = 'cvn2_a_system_item_is_read_only_when_it_is_named.sql'
 where d.schema_name = 'custom' and d.function_name = 'resolve_context'
   and d.identity_args = 'p_entity_type text, p_entity_id uuid, p_record_ids uuid[], p_table_ids uuid[]';
revoke all on function custom.resolve_context(text, uuid, uuid[], uuid[], text[]) from anon, service_role;
grant execute on function custom.resolve_context(text, uuid, uuid[], uuid[], text[]) to authenticated;

comment on function custom.resolve_context(text, uuid, uuid[], uuid[], text[]) is
  'P3 (b), lane SC-3''. The record store''s twin of public.resolve_full_context, in the same shape {scope_labels, variables, sources, cell_values, context, resolved_at} plus checks and withheld. ONE RULE CHANGED ON PURPOSE: every contributing record — the selection, the entity''s tags, the project''s tags — is checked for the person through custom.where_id_opens (members, direct and outside grants, and a scope membership through custom.scope_member_reaches), and read under its own organization. What is refused is named in `checks`; a Field the person may not see is named in `withheld`. A relation Field is delivered on demand (DYN-17). Writes nothing. A System item is read only when p_system_item_refs names it (lane CONTEXT-VALUES-NAMED-2).';
