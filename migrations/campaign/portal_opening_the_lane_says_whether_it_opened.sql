...
-- additive: yes
--   It REPLACES one body, `custom.portal_declare`, with a `-- based-on:` line. Nothing else.
-- guard: custom/external_principal_enabled
--
-- PORTAL — DEFECT FOUND BY RUNNING IT, 2026-09-20. NOTHING FAILS SILENTLY.
--
-- `custom.portal_declare` opened the organization's external lane with
-- `perform platform.knob_override_set(...)` and threw the answer away. That door does not
-- RAISE: it returns `{"ok": false, "reason": …}` — `not_authenticated` when there is no
-- signed-in person, `forbidden` when the caller is not an owner or admin of the
-- organization, `wrong_door` when the key belongs to another writer. So a portal could be
-- declared, answer with its id, and leave the lane SHUT, and the first thing anybody would
-- see is `custom.share_grant` refusing the invitation several minutes later with a sentence
-- about a knob nobody knew had not been written. Measured exactly that way by this lane's
-- first fixture run.
--
-- TWO THINGS CHANGE, AND ONLY IN THIS BODY:
--
--   1. THE ANSWER IS READ. A refusal is re-raised with the door's own reason and detail, so
--      the portal is not created at all rather than created shut.
--   2. THE SERVER LANE CAN OPEN IT. `platform.knob_override_set` reads `auth.uid()` and
--      refuses `not_authenticated`, which is right for a browser and wrong for the store's
--      own lane — an agent turn, a migration, the portal route holding the service key — all
--      of which have already been judged by `custom.assert_client_may_change` at the `admin`
--      rung of the client Table, a HIGHER bar than the knob door's "owner or admin of the
--      organization". So when there is no principal AND the caller is the role that owns the
--      store, it writes through `platform._knob_override_write`, which is the same writer the
--      door itself ends in — the same row, the same audit, the same history — and never a
--      second way for a browser.
-- based-on: custom.portal_declare(uuid, text, uuid, jsonb, uuid, text, text) 8fdfb354b3989c53fe3a9125493bd140e1d639f5d798f85a059d45b8b038c646
