...
--
-- platform.assist_producer_policy_history -- AUDIT SEMANTICS.
-- The history of changes to assist producer policy. Zero client mentions; the live table is written by the policy table's own trigger.
--
-- WHY IT MATTERS: A history a client can write is a history that proves nothing.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.assist_producer_policy_history` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_assist_producer_policy_history_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "assist_producer_policy_history_client_insert_refused" on platform.assist_producer_policy_history
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "assist_producer_policy_history_client_update_refused" on platform.assist_producer_policy_history
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "assist_producer_policy_history_client_delete_refused" on platform.assist_producer_policy_history
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "assist_producer_policy_history_client_insert_refused" on platform.assist_producer_policy_history is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: A history a client can write is a history that proves nothing. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "assist_producer_policy_history_client_update_refused" on platform.assist_producer_policy_history is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: A history a client can write is a history that proves nothing. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "assist_producer_policy_history_client_delete_refused" on platform.assist_producer_policy_history is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: A history a client can write is a history that proves nothing. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
