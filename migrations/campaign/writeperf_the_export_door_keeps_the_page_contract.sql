-- additive: yes
-- based-on: custom.io_export(uuid, uuid, text[], integer, text) 93e7b3f101f1365c3663d697d64f6a8f4a404202547a55b71a0fb69535ddce87
--
-- chair-step: it REPLACES one live body, `custom.io_export`, to put back the page contract that
--   `writeperf_a_page_says_what_it_did.sql` gave it at 19:39:54Z and that lane EXPORTFIX's
--   `exportfix_the_export_is_the_read_doors_answer.sql` overwrote at 19:47:56Z — an ordinary
--   collision on a shared database, not anybody's mistake. Nothing is created, dropped, granted
--   or revoked and no data is touched. The `-- based-on:` hash is EXPORTFIX's new body, which is
--   kept whole; only the clamp expression moves and the `page` key is added to the answer.
--   The inverse is `migrations/inverse/writeperf_the_export_door_keeps_the_page_contract_down.sql`.
--
-- WRITE-PERF — THE EXPORT DOOR KEEPS THE PAGE CONTRACT.
--
-- `custom.silent_page_doors()` went from 0 to 1 the moment EXPORTFIX landed, naming
-- `custom.io_export` and the expression it had brought back:
--
--     greatest(1, least(coalesce(p_limit, 10000), 100000))
--
-- That is the census doing exactly the job it was built for: the class is closed by a guard
-- that names a regression, not by everybody remembering. The door now asks
-- `custom.page_size(..., custom.export_ceiling(org))` like every other paged door — an export
-- over the ceiling is REFUSED by name instead of quietly becoming a smaller export — and its
-- answer carries `page` with `requested`, `returned`, `ceiling` and `next`.
