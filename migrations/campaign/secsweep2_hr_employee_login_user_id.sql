...
create policy "employee_login_user_id_is_addressable_insert" on hr.employee
  ...
  with check (login_user_id is null or login_user_id = (select auth.uid()) or iam.has_org_access_for(login_user_id, organization_id) or (select public.is_platform_admin()));
