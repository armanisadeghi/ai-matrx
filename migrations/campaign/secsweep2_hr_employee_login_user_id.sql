...
--
-- hr.employee.login_user_id genuinely names a person other than the writer, so pinning it to
...
-- Inverse: migrations/inverse/secsweep2_hr_employee_login_user_id.inverse.sql
...
create policy "employee_login_user_id_is_addressable_insert" on hr.employee
  as restrictive for insert to authenticated, anon
  with check (login_user_id is null or login_user_id = (select auth.uid()) or iam.may_address_user_in_org(login_user_id, organization_id) or (select public.is_platform_admin()));

create policy "employee_login_user_id_is_addressable_update" on hr.employee
