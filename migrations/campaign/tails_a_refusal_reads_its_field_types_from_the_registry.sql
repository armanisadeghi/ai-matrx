-- additive: yes
--
-- chair-step: it REPLACES the live bodies of `custom._field_document_for` and
--   `custom._field_type_parity_guard`, and adds ONE new function,
--   `custom._parity_types_sentence()`. Nothing is dropped, nothing is revoked, no enum
--   value is added, no row is touched. The only behavioural change is that three refusal
--   HINTS now read their count and their list out of `custom.parity_field_types()`
--   instead of spelling them out.
--
-- TAILS — A REFUSAL THAT LISTS THE FIELD TYPES READS THEM FROM THE REGISTRY.
--
-- THE DEFECT, and it is a CLASS. When lane W1-FIELD-TYPES shipped the parity floor the
-- refusals said "the thirteen are select, … datetime". When lane LIMITS-FIX added
-- `checkbox` the registry grew to fourteen and the three sentences were edited BY HAND to
-- match — which is the same mistake with a fresh coat: the next parity type will make all
-- three lie again, and a person reading "there is no kind of column called checkbox … the
-- thirteen are …" is being told, by the store itself, that a type the store ships does not
-- exist. Refusal text that enumerates a registry must be GENERATED from that registry.
--
-- THE FIX. `custom._parity_types_sentence()` builds the sentence — the count as an English
-- word and the names in the registry's own order, joined with the last one after "and" —
-- from `custom.parity_field_types()`, which is itself the one body the whole store reads
-- its parity floor from (the `@ai-matrx/records` package regenerates from it too). The
-- three hints call it. Add a fifteenth parity type and every sentence says fifteen with no
-- edit anywhere; there is no longer a place for the number to go stale.
--
-- Two stale COMMENTS inside `_field_document_for` that still said "thirteen" are rewritten
-- to name no number at all, for the same reason.
--
-- based-on: custom._field_document_for(uuid, uuid, jsonb) 57a3c43456fb6bc481e00b2367f3148dda664e74e4233cc646f9380bfb716a6b
-- based-on: custom._field_type_parity_guard() 3f7d0d264e3d557fda0bd782a3e913c0d561ec27ff3325b5a239fa24b3175981

-- ── THE SENTENCE, BUILT FROM THE REGISTRY ──────────────────────────────────────────────
-- Internal (leading underscore): it is called only from bodies that already run as the
-- store's owner, it takes no organization id and decides no caller, so it is not a door.
create or replace function custom._parity_types_sentence()
returns text
language plpgsql
stable
...
as $sentence$
declare
  -- English up to twenty; past that the numeral, because a refusal that says "twenty-three"
  -- wrongly is worse than one that says "23".
  ...
  v_total integer;
begin
  select array_agg(t.parity_type) into v_names from custom.parity_field_types() t;
  v_total := coalesce(array_length(v_names, 1), 0);
  if v_total = 0 then
    -- Unreachable while the registry has rows, and it says so rather than returning ''.
    return 'this store lists no field types at all, which is itself the defect';
  end if;
  if v_total = 1 then
    return format('the only field type is %s', v_names[1]);
  end if;
  return format('the %s are %s and %s',
