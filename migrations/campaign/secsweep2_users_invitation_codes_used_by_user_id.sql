...
--
-- users.invitation_codes.used_by_user_id records WHO DID IT. The actor of an action is, by definition, the caller,
...
-- Inverse: migrations/inverse/secsweep2_users_invitation_codes_used_by_user_id.inverse.sql
-- Guard: `pnpm check:unpinned-security-columns` -- both baseline rows for this pair leave.

set local lock_timeout = '2s';

create policy "invitation_codes_used_by_user_id_is_the_caller_insert" on users.invitation_codes
  as restrictive for insert to authenticated, anon
  with check (used_by_user_id is null or used_by_user_id = (select auth.uid()) or (select public.is_platform_admin()));

create policy "invitation_codes_used_by_user_id_is_the_caller_update" on users.invitation_codes
