...
-- transaction.
--
-- THE MACHINERY TABLES, BATCH 1 OF 4 — 8 tables.
--
...
--
-- Inverse: migrations/inverse/doorsonly5_machinery_batch_01_the_for_select_twin.inverse.sql
...
-- ── iam.access_requests ──
create policy "platform_admin_select" on iam."access_requests"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."access_requests" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.access_requests` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."access_requests";

-- ── iam.industry_curators ──
create policy "platform_admin_select" on iam."industry_curators"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."industry_curators" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.industry_curators` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."industry_curators";

-- ── iam.invitations ──
create policy "platform_admin_select" on iam."invitations"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."invitations" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.invitations` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."invitations";

-- ── iam.membership_grant ──
create policy "platform_admin_select" on iam."membership_grant"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."membership_grant" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.membership_grant` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."membership_grant";

-- ── iam.memberships ──
create policy "platform_admin_select" on iam."memberships"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."memberships" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.memberships` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."memberships";

-- ── iam.organizations ──
create policy "platform_admin_select" on iam."organizations"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."organizations" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.organizations` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."organizations";

-- ── iam.permissions ──
create policy "platform_admin_select" on iam."permissions"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."permissions" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.permissions` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."permissions";

-- ── iam.system_personal_org_failures ──
create policy "platform_admin_select" on iam."system_personal_org_failures"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on iam."system_personal_org_failures" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `iam.system_personal_org_failures` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on iam."system_personal_org_failures";
