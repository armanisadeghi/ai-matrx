...
-- removing the door.
--
-- platform.assist_producer_policy -- POLICY SEMANTICS.
-- Which producer may raise which assist. Its history table was already closed by DOORS-ONLY, and `features/admin/assist-control/service.ts` only reads it.
--
-- WHY IT MATTERS: A member who writes an assist policy decides what the platform is allowed to say to everybody else.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.assist_producer_policy` is owned by `postgres` and does NOT carry
