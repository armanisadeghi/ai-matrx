...
-- additive: yes
--   It REPLACES one function, custom.resolve_context(text, uuid, uuid[], uuid[]), with its existing
--   signature, security (SECURITY DEFINER, STABLE), search_path and grants, changing ONE block: the
--   per-record value-version triple is read from the record the door has already opened for the
--   person instead of by calling custom.record_values_versioned once per record. No table, column,
--   trigger, policy or grant is touched; no row is written. The answer is byte-identical (proven on
--   70 live scope records: the old and the new triple agree on every one).
-- guard: custom/system_enabled
-- lane: INSPECTOR-TAILS
-- lock: custom
-- based-on: custom.resolve_context(text, uuid, uuid[], uuid[]) 9b95715e6cff2dc965ddd963193c02349097b34ab3914b8517b8b978d46d1597
--
-- WHY. The context inspector's "record store" side took ~1,000 ms for a scope type of 17 scopes
-- against 52 ms on the current system (VERIFIER-22). Measured inside the database as admin@admin.com:
-- custom.resolve_context for those 17 scopes = 852-1064 ms; for one = 68 ms. Per record it called
-- custom.read_record (~22 ms, almost all custom.read_mask) AND custom.record_values_versioned
-- (~23 ms), which re-runs the organization wall, the ladder and the same read_mask a second time.
-- The second is gone. Inverse: migrations/inverse/inspectortails_the_context_door_reads_the_triple_from_the_record_it_opened_down.sql restores the body verbatim.
