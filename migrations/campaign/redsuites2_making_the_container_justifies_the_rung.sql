...
-- based-on: iam.member_level_justified(uuid, uuid, uuid) ea669bf9c0adaba709e78785de49acafd62f1a9e1ac0105fe28943401460b61f
--
-- RED-SUITES-2 — MAKING THE CONTAINER JUSTIFIES THE RUNG IT CARRIES.
--
-- `iam.member_level_overreach()` is the platform's own detector for "a member reaches more
-- than her grants and her organization's knob justify", and `laddercap_green` 4b is the guard
-- over it. MEASURED on the MAIN database on 2026-09-21, from that guard:
--
--   4b FAILED — 1 member(s) reach more than their grants and their organization's knob
--   justify: Ironclad Mobile Mechanic/test@test.com 4
--
-- NOBODY OVER-REACHED ANYTHING. Those four records sit in a Table whose row carries
-- `created_by` = that same person. `custom.effective_level` — the real ladder — carries a
-- container at its EFFECTIVE level, and creating a row is the top of that ladder, so she
-- answers `admin` on the records inside the Table she made. `iam.member_level_justified`
-- computed the container's rung with `iam.access_arms_from_sources`, which knows about
-- grants, shares and the organization's knob and knows NOTHING about who made the row, so it
-- answered `editor`. The two disagreed, and the detector called the difference an overreach.
--
-- The rule was already written in this very function, one line from the top:
--     if v_created = p_user_id then return iam.top_content_level(); end if;
-- It was applied to the RECORD and never to anything ABOVE it. This file applies it to the
-- container too, in the ancestor loop, and nowhere else.
--
-- A DETECTOR THAT CRIES WOLF IS WORSE THAN NO DETECTOR — a member who really did reach past
-- her grants in that organization would have been one more row in the noise.
--
-- THIS GRANTS NOBODY ANYTHING. `iam.member_level_justified` decides no access: it is the
-- second opinion the census compares `custom.effective_level` against. Raising what it
-- considers justified can only ever REMOVE a false finding; it cannot open a door.
--
-- ITS INVERSE: `migrations/inverse/redsuites2_making_the_container_justifies_the_rung_down.sql`.
