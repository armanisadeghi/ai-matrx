...
-- removing the door.
--
-- platform.org_change_policy -- CONFIG SEMANTICS.
-- An organization's change policy. `features/change-policy/service.ts` reads it and writes nothing.
--
-- WHY IT MATTERS: Change policy decides what an organization is allowed to change and how; it is a settings decision, and settings decisions go through the ladder.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.org_change_policy` is owned by `postgres` and does NOT carry
