-- chair-step: this replaces the bodies of four live client doors, so the judge cannot read what the statements will do from its allow-list
-- based-on: custom.table_share_outside(uuid, uuid) 58206199cdd9f5d94fa3bbcb44c49cf40ebb4f83f688bd02a05cfa8ddc138042
-- based-on: custom.table_share_outside_invite(uuid, uuid, text, permission_level) 175b16bb4c3f5cf3ebd13c70025d7a2ec169c709acb3829b4023f5097f0fcbe6
-- based-on: custom.table_share_outside_accept(text) e8d776061695692a8ffe5189bfbf26e6cf19d1e58a3ba0e9e854ea50f7492970
-- based-on: custom.table_share_outside_for_me() b1aa79a4d330c0f5fde0d85255d6c7f7826f55c022f67dfb85fcf7f9f2db37fa
--
-- SHARE-OUT item 1 — WHAT THE OUTSIDE LANE SAYS, SAID IN ENGLISH.
--
-- TWO THINGS THE SEAT SUITE CAUGHT THE FIRST TIME IT RAN, both about the sentences and
-- neither about the decision. Nothing here changes who may do what: the ladder calls, the
-- knob reads, the level cap and every identity check are byte-identical.
--
-- 1. A REAL BUG. `raise exception` uses `%`, not `%s`. One placeholder was written `%s`,
--    so the refusal printed the organization's name with a stray letter glued to it:
--      "Sharing with people outside Rincon Plumbing Cos is turned off here"
--    A screen never lies, and it does not stammer either.
--
-- 2. THE MACHINERY SHOWING THROUGH. `iam.level_label('table','viewer')` is "Table viewer"
--    — the right label for a badge and the wrong words inside a sentence, which read
--    "test@test.com is invited to Jobs at table viewer." The label stays exactly where it
--    belongs, as its own `level_label` field for the UI to draw; the sentences now say
--    "as a viewer", which is what a person says out loud.
--
-- THE INVERSE: `migrations/inverse/shareout_outside_words_down.sql`.
