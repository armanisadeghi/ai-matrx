...
-- based-on: custom.visible_set(uuid, uuid, uuid, permission_level) 7176b59df0d1acbfde3f5da6acb3296113b37f2f88306b5d891d472e481f2341
--
-- SHARED-ONLY — THE TABLE THAT CARRIES IS THIS ORGANIZATION'S OWN ROW, NOT A KERNEL ONE.
--
-- Found by `custom.shared_only_disagreements()` — this lane's own census — on the main
-- database within a minute of the whole-Table shortcut landing, and fixed here:
--
--     organization          pairs   ladder   read door   RLS mirror
--     admin's Workspace       186     f          t            f
--     admin's Workspace        12     f          t          (null)
--     SHARED-ONLY Throwaway     1     f          t            f
--
-- WHY. `custom.visible_set` asked "does the caller reach the Table itself" about whatever id
-- it was handed, and a record's `table_id` is not always this organization's own Table: the
-- kernel rows `Table` (…0001), `Field` (…0002) and the home-record kernel (…0004) live in the
-- SYSTEM organization, which is `global_readable`, so `iam.has_access_for` answers YES about
-- them to every signed-in person. A page of Fields, or of home records, therefore came back
-- whole to a plain member of an organization that had said `shared_only`.
--
-- Arm 3 of `custom.carrying_edges_of` already had the guard — it joins the container Table
-- back to `custom.record` ON THE SAME ORGANISATION, which is why the per-row ladder answered
-- `f` throughout and why the census had two sides to compare. This puts the same condition on
-- the set-based side, so the two say one thing again.
