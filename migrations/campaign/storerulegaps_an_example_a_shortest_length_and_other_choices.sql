-- chair-step: it REPLACES four live bodies in schema `custom`, two of them trigger functions on
--   custom.record: `custom._field_shape_guard` judges three new words on a column (a pattern's
--   `example`, a length rule's `min`, a choice list's `config.allow_other`); `custom.validate_values`
--   refuses a value shorter than `min` and, when the pattern carries an example, says "Enter it
--   like …"; `custom._resolve_choice_words` ADDS an off-list word to a list that allows other
--   values; `custom.field_update` stores `allow_other`. Both trigger bodies already read the
--   store's switch the only way this store reads it (`custom.assert_store_door` /
--   `custom.store_is_open`), so a literal knob read beside them would exist only to satisfy the
--   runner's grep (the ruling in choiceval_two_doors_on_the_one_ladder.sql). Every column declared
--   before this file carries none of the three words and behaves byte for byte as before.
--   Nothing is added, dropped or revoked; no table, trigger, policy, grant or row is touched.
--   The inverse is `migrations/inverse/storerulegaps_an_example_a_shortest_length_and_other_choices_down.sql`.
-- lock: custom
-- based-on: custom._field_shape_guard() 56c35ec5adf94831719d4c87ab52abc48bc408ce02e9170486954a5fe1dbe825
-- based-on: custom.validate_values(uuid, custom.record[], jsonb, text) 2e583703cc20fe866a48f0a79956035cdf97ebc465fdc27e413fae31132672c9
-- based-on: custom._resolve_choice_words() fa8b8790461ad733a193bd1dfb6cf79cd340e0ec554c39f2f0f5b82c9390311f
-- based-on: custom.field_update(uuid, uuid, jsonb) caeca3bfcbd01f176f9d41e4353599e305b08fda58c87f52d8606097c185ac7f
--
-- LANE STORE-RULE-GAPS (2026-09-23) — THREE COLUMN SETTINGS THE OLDER GRID HELD AND THE RECORD
-- STORE HAD NOWHERE TO PUT (OLD-TABLES-4's attribute carry, "no home in the store"):
--   (1) a pattern's EXAMPLE (the older grid's `patternHint`):
--       {"kind":"pattern","value":"^\\(?[0-9]{3}\\)?[ -]?[0-9]{3}-[0-9]{4}$","example":"949-555-0142"}
--       A refusal then says "Phone is not written the way this field expects." with the remedy
--       "Enter it like 949-555-0142." — and carries {"example": …} in its detail so
--       @ai-matrx/records-ui's one refusal builder reads the example as data.
--   (2) a choice list's OTHER VALUES ALLOWED (the older grid's `allowOther`):
--       config.allow_other = true. A word that is none of the choices is added to the column's
--       own choices Table as typed, by the write itself, and the cell holds its key. Absent = off.
--       It is the COLUMN's setting, never a global switch; the mover turns it on exactly for the
--       columns that carried `allowOther`.
--   (3) a length rule's SHORTEST (the older grid's `minLength`):
--       {"kind":"length","min":3,"value":40}. `value` keeps meaning the longest; `min` may stand
--       alone. A blank is `required`'s question and is not refused for being short.
--
-- LOCKS. create or replace function only (ACCESS SHARE). Not window-class.
...
begin
  ...
  for e in select key as k, value as v from jsonb_each(v_map) loop
    ...
    for v_one in select x from jsonb_array_elements(v_items) x loop
      ...
      -- ── STORE-RULE-GAPS (2), 2026-09-23: A COLUMN THAT TAKES OTHER VALUES ADDS THEM. ──────
      -- The older grid's `allowOther`. When the column says so (`config.allow_other`, the
      -- column's own setting, off unless declared), a word that is none of its choices is
      -- ADDED to the column's list of choices exactly as it was typed, and the cell holds the
      -- new choice's key like any other. It is added through the same row every option is
      -- (a record of the column's own choices Table, key derived by this trigger, placed
      -- last), so history, the change feed and every reader see an ordinary new choice. Only
      -- a person who may add records to that list may add one this way; everybody else is
      -- refused by the list's own door. A RETIRED choice's name is not "other": it names a
      -- choice, and picking it anew is still refused below.
      if v_hit is null then
        ...
        v_opts := nullif(v_field ->> 'options_table_id', '')::uuid;
        if coalesce(v_allow, false) and v_opts is not null then
          perform custom.assert_client_may_change(new.organization_id, v_opts, 'custom.record_write',
                                                  'editor'::public.permission_level, 'table');
          insert into custom.record (organization_id, table_id, data, metadata)
          values (new.organization_id, v_opts, jsonb_build_object('title', v_word),
                  jsonb_build_object('option_position',
                    (select coalesce(max((o.metadata ->> 'option_position')::integer), 0) + 1
                       from custom.record o
                      where o.organization_id = new.organization_id
                        and o.table_id = v_opts)))
          returning id into v_added;
          select nullif(o.metadata ->> 'option_key', '') into v_hit
            from custom.record o
           where o.organization_id = new.organization_id
             and o.id = v_added;
          -- The list as it now stands, so a second new word in the same cell sees the first.
          v_field := coalesce(custom.choice_field_map(new.organization_id, new.table_id) -> v_key, v_field);
          raise notice 'custom: "%" was not one of the choices for %, so it was added to them.', v_word, v_label;
        end if;
      end if;
