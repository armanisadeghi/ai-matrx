...
-- based-on: custom._containment_guard() 466a2dca2ad17debefc27a2a08d2fe11e6f08727c557722eea6ad70a252528bb
-- based-on: custom._derived_fields() f01e1c65a65c57bc7aeee90ce5f3cf702b5c7b903bf61aba495e78640a8e0d90
-- based-on: custom._field_definition_write() 0bfb042c50829617ef9a57b6563d05efb70242e5a3d3f25fcb9bef2e5373f1ee
-- based-on: custom._field_shape_guard() b7dbd2053d8b2fc82c870019ea9540e79cddc15f0e7b511c0200de61da76472f
-- based-on: custom._field_type_parity_guard() 2f79fa782aa7ab7bbf42920c3b812c6e6557c94c4759dae2a3cc8fce3ba552b4
-- based-on: custom._merge_field_shape_guard() 7445e97b1a70db4acdd5ede27dd1288d9430311baeb8a017b511126937b36d19
-- based-on: custom._record_field_validation() 1518b2f22d95c9a9a6a0ad99f0fff669f6f2bd2ee8701aba5b8aef600f2defd0
-- based-on: custom._record_rule_uses() cd8f5651e1580897a13141e3cdba7ab77fe06e907b0525d2d8cfc3f40900635d
-- based-on: custom._rule_definition_write() 61039cefa8953a471b56dda0b0597753faa06605393a55e10b3e5d7cad799a70
-- based-on: custom._rule_shape_guard() 58470fac1e48b80a1a28cbd0f34ab2a99f242ee233c884e8646e743e9981d228
-- based-on: custom._rule_topology_guard() 06d90ae2e3f41e18e53528eb573a318780f6eadeb82c91ba9dd19e8d44940b8d
-- based-on: custom._table_shape_guard() e217d373894d9128ff77525ec60b3f06ba123afde4a83e93e86162d8b2647b94
--
-- W1-V1-FIXES, FINDING 1 CONTINUED — EVERY `RETURNS trigger` IN `custom` READS THE ONE
-- GUARD, AND NOT ONE OF THEM CARRIES A COPY OF IT.
--
-- `w1_v1_fixes_one_door_predicate.sql` built the predicate and put it in every SECURITY
-- DEFINER write door. This file finishes the class at the other end: the triggers.
--
-- WHAT WAS TRUE BEFORE THIS FILE, CENSUSED ON THE BRANCH 2026-09-17
-- ------------------------------------------------------------------
-- Fourteen functions in schema `custom` return `trigger`. Eleven fire on `custom.record`
-- (and its sixteen hash partitions), two are INSTEAD OF triggers on the projections
-- `custom.field` and `custom.rule`, and one — `custom._entity_custom_fields_guard` — fires
-- on `crm.party` and is NOT this store's door at all: it is guarded by its own knob,
-- `custom/entity_custom_fields_guard`, whose OFF meaning is "answer exactly as before",
-- and putting the store door on it would refuse live `crm.party` writes. It is named here
-- so the next author does not "finish the sweep" and take a live table down.
--
-- Of the thirteen that ARE this store's, THREE carried the door and each carried its OWN
-- COPY of the same eleven lines — `_record_field_validation`, `_derived_fields`,
-- `_field_type_parity_guard` — and TEN had no door at all. Every copy asked
-- `pg_has_role(current_user, …)`, so every copy was wrong in the same way, which is what
-- makes this a class and not three bugs.
--
-- WHAT THIS FILE DOES
-- -------------------
--   · STRIPS all three copy-pasted blocks (and the now-unused `v_owner` declaration each
--     one needed). After this file, `custom.store_is_open` has exactly ONE caller in a
--     trigger path: `custom.assert_store_door`.
--   · Puts ONE line — `perform custom.assert_store_door(…)` — at the top of all twelve
--     bodies below. Nothing else in any body moves; every line is the live definition the
--     `-- based-on:` hashes name.
--   · The two INSTEAD OF bodies fire on DELETE as well, where `new` is unassigned and
--     reading it would RAISE rather than refuse, so those two branch on `tg_op`.
--
-- THE SWITCH STILL NEVER REMOVES A CHECK — every body runs in full once the door has let
-- the caller through, so this is inert for the store's owner and a named refusal, with the
-- remedy, for everybody else.
--
-- THE INVERSE: `migrations/inverse/w1_v1_fixes_every_trigger_reads_its_guard_down.sql`.
...
comment on trigger custom_record_field_validation on custom.record is
  'The first of thirteen triggers in schema custom that all read ONE door: while custom/system_enabled resolves false, custom.assert_store_door refuses every writer that is not a member of custom.record''s owner, naming the caller''s real identity (custom.caller_role(), never current_user) and the door it was refused at. No trigger in this schema carries its own copy of that decision.';
