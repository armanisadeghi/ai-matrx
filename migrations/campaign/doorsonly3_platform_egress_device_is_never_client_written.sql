...
--
-- platform.egress_device -- A RESIDENTIAL EGRESS DEVICE HOLDS A TOKEN, AND A TABLE GRANT CANNOT SAY WHICH COLUMNS.
--
...
--
-- The ONE client writer is `features/residential-egress/service.ts`, two updates, and
-- between them they write exactly two columns: `enabled` and `display_name`. Everything else --
-- `token_hash`, `token_prefix`, `token_rotated_at`, `connected`, `connection_id`, `gateway_host`,
-- `gateway_port`, the counters -- is written by aidream's Python lane over its own Postgres
-- credentials, which no `authenticated` policy touches.
--
-- THE DOOR THEY NOW USE: `public.egress_device_set`,
...
-- Inverse: migrations/inverse/doorsonly3_platform_egress_device_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "egress_device_client_insert_refused" on platform.egress_device
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "egress_device_client_update_refused" on platform.egress_device
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "egress_device_client_delete_refused" on platform.egress_device
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "egress_device_client_insert_refused" on platform.egress_device is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). THE COLUMN SET IS THE WHOLE POINT. A table UPDATE grant is all-or-nothing over the row: the owner`s std_update policy let a browser write token_hash, token_prefix, connection_id, gateway_host and gateway_port on a device it owns -- the pairing secret and the relay address of a machine in somebody`s house. The door takes `enabled` and `display_name` and nothing else, so those columns are unreachable from a client by construction rather than by a column grant nobody maintains. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
comment on policy "egress_device_client_update_refused" on platform.egress_device is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). THE COLUMN SET IS THE WHOLE POINT. A table UPDATE grant is all-or-nothing over the row: the owner`s std_update policy let a browser write token_hash, token_prefix, connection_id, gateway_host and gateway_port on a device it owns -- the pairing secret and the relay address of a machine in somebody`s house. The door takes `enabled` and `display_name` and nothing else, so those columns are unreachable from a client by construction rather than by a column grant nobody maintains. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
comment on policy "egress_device_client_delete_refused" on platform.egress_device is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). THE COLUMN SET IS THE WHOLE POINT. A table UPDATE grant is all-or-nothing over the row: the owner`s std_update policy let a browser write token_hash, token_prefix, connection_id, gateway_host and gateway_port on a device it owns -- the pairing secret and the relay address of a machine in somebody`s house. The door takes `enabled` and `display_name` and nothing else, so those columns are unreachable from a client by construction rather than by a column grant nobody maintains. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
