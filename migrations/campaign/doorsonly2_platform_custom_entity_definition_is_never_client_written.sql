...
-- removing the door.
--
-- platform.custom_entity_definition -- SCHEMA DEFINITION SEMANTICS.
-- What a custom entity IS -- its identity in the catalogue, which the whole custom-data stack reads.
--
-- WHY IT MATTERS: A member who writes an entity definition directly bypasses every validation the definition doors apply, and a malformed definition breaks the catalogue for the organizations that read it.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("custom_entity_definition")` search AND a
-- bare-name search of `"custom_entity_definition"` / 'custom_entity_definition' / `custom_entity_definition` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.custom_entity_definition` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_custom_entity_definition_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "custom_entity_definition_client_insert_refused" on platform.custom_entity_definition
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "custom_entity_definition_client_update_refused" on platform.custom_entity_definition
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "custom_entity_definition_client_delete_refused" on platform.custom_entity_definition
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "custom_entity_definition_client_insert_refused" on platform.custom_entity_definition is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). SCHEMA DEFINITION semantics: A member who writes an entity definition directly bypasses every validation the definition doors apply, and a malformed definition breaks the catalogue for the organizations that read it. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "custom_entity_definition_client_update_refused" on platform.custom_entity_definition is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). SCHEMA DEFINITION semantics: A member who writes an entity definition directly bypasses every validation the definition doors apply, and a malformed definition breaks the catalogue for the organizations that read it. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "custom_entity_definition_client_delete_refused" on platform.custom_entity_definition is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). SCHEMA DEFINITION semantics: A member who writes an entity definition directly bypasses every validation the definition doors apply, and a malformed definition breaks the catalogue for the organizations that read it. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
