...
--
-- platform.activity_log -- AUDIT SEMANTICS.
-- The platform activity record, 252,284 rows. Its one write path is the SECURITY DEFINER door `platform.log_activity`; the two client call sites (`lib/communications/voice/storage-canary-readiness.ts`, `provider-configuration-readiness.ts`) are both `.select(...)` through `createAdminClient()`, which is `service_role` and untouched by this policy.
--
-- WHY IT MATTERS: An audit anybody may write is not an audit: a client could record that somebody else did a thing, or that a thing that happened did not.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.activity_log` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_activity_log_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "activity_log_client_insert_refused" on platform.activity_log
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "activity_log_client_update_refused" on platform.activity_log
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "activity_log_client_delete_refused" on platform.activity_log
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "activity_log_client_insert_refused" on platform.activity_log is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: An audit anybody may write is not an audit: a client could record that somebody else did a thing, or that a thing that happened did not. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "activity_log_client_update_refused" on platform.activity_log is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: An audit anybody may write is not an audit: a client could record that somebody else did a thing, or that a thing that happened did not. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "activity_log_client_delete_refused" on platform.activity_log is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: An audit anybody may write is not an audit: a client could record that somebody else did a thing, or that a thing that happened did not. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
