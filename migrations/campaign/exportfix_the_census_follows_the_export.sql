...
-- based-on: custom.list_door_disagreements(text, uuid, integer, boolean) 26db5b6e4c2155e55b26dd5b1e8a570db64bc5a2a2a3980f201a591f10fe07cc
--
-- EXPORT-FIX — CENSUS 13 FOLLOWS THE EXPORT ONE HOP FURTHER OUT.
--
-- The census's fast path substitutes `custom.visible_set` for each list-shaped door, and it
-- checks in the catalogue that the door is still BUILT on that set before it believes the
-- substitution. `custom.io_export` no longer names the set: since the door was rewritten to
-- take the read door's own answer, it calls `custom.read_records` and projects the columns of
-- the documents that door hands back. The check therefore said, correctly for what it knew:
--
--   unmeasured: … custom.io_export, which is where custom.io_export builds its predicate,
--   no longer reaches it.
--
-- WHAT IS TRUE is that the export's rows ARE `custom.read_records`' rows — the same door this
-- census measures `custom.read_record` against, one row above in the same list, whose own row
-- still demands the direct reach. So the accepted route becomes a PER-DOOR pattern and
-- `custom.io_export`'s admits `custom.read_records`. Nothing is waived: if `read_records` ever
-- stops reaching the set, its own row goes red in the same run.
--
-- It is held per door and not widened for everybody because `pg_get_functiondef` prints a
-- function's own name, so allowing `read_records` everywhere would let `custom.read_records`
-- satisfy the check by existing.
--
-- ITS INVERSE is migrations/inverse/exportfix_the_census_follows_the_export_down.sql.
