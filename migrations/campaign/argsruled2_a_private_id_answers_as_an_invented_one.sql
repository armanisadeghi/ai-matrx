-- lock: platform
-- lane: ARGS-RULED-2
-- based-on: public.fork_shared_conversation(uuid, uuid, text) 2d3789cef1b9dce79cb7cff3ffe6d8778e91959fbc0646277f23c6290fcc4514
-- based-on: public.fork_shared_flashcard_set(uuid, uuid, text) a5e22fbe69bf0d634a89ed69600ca60cc02ac1c54354dcdb28feefce931ebce0
-- based-on: public.fork_shared_quiz(uuid, uuid, text) 8f9bdd125c1298787de0a5bdea8a24d0512e6f9aed9c011230cf0a9a17e78990
--
-- ARGS-RULED-2 — THE THREE `fork_shared_*` DOORS ANSWER ONE WAY FOR ANYTHING YOU MAY NOT COPY.
--
-- MEASURED 2026-09-22 on the MAIN database from test@test.com's `authenticated` seat, copying
-- into an organization she belongs to: an invented id answered "Conversation not found" /
-- "Flashcard set not found" / "Quiz not found", and a REAL conversation, set or quiz of another
-- organization that was not shared with her answered "This conversation is not shared" / "This
-- set is not shared" / "This quiz is not shared". Two sentences, so one call per guessed uuid
-- told a signed-in account which ids are real — another organization's private conversation
-- included. (DEFINER-7 found it, 2026-09-22, and left it to be ruled.)
--
-- THE FIX makes the two cases the SAME BRANCH OF CODE, not two branches that happen to agree:
-- each body declares ONE constant refusal, `v_unavailable`, and both the not-found line and the
-- not-shared line return it. One sentence, one `code` ('not_available'), no id echoed. The
-- entitled path (a shared set, a share link, a viewer grant) and every earlier refusal — not
-- signed in, no organization named, not a member of that organization — are byte-for-byte
-- unchanged. No product code keys on the two retired sentences (grepped, 2026-09-23).
--
-- Seat suite: scripts/campaign-tests/argsruled2_fork_green.sql; red twin
-- scripts/campaign-tests/argsruled2_fork_red.sql. Inverse:
-- migrations/inverse/argsruled2_a_private_id_answers_as_an_invented_one_down.sql.
