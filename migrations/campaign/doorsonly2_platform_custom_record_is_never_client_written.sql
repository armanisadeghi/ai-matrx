...
-- removing the door.
--
-- platform.custom_record -- RECORD STORE SEMANTICS.
-- The older custom-record table. Its only mention outside generated catalogues is a registration line in `lib/knobs/unifiedDataCampaign.register.ts`; the live record store the campaign built is `custom.record`, which has its own write door.
--
-- WHY IT MATTERS: A record store reachable directly over REST is the whole thing the store's door design exists to prevent.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.custom_record` is owned by `postgres` and does NOT carry
