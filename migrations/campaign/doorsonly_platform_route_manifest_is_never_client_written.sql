-- lane: DOORS-ONLY
-- `platform` AND `iam` ARE NOT CLIENT-WRITABLE THROUGH PostgREST. (Chair ruling; VERIFIER-8
-- HIGH-3, 2026-09-21; the half of SECURITY-SWEEP's ruling 1 nobody had executed.)
--
-- Every write to these two schemas goes through a SECURITY DEFINER door that decides through
-- the one ladder. Reads stay exactly as they are, under RLS. The reason is not that any one
-- policy is wrong today: it is that a base table reachable directly over REST is safe only
-- while EVERY policy on it is complete, forever, including the ones `iam.apply_rls` will
-- regenerate tomorrow -- and CRITICAL-1 (`iam.api_keys.service_user_id`) is the proof that
-- "every policy is complete" is not a property this database has. A safe path beside an
-- unsafe one is no fix; closing a class means removing the door.
--
-- platform.route_manifest -- ROUTE SEMANTICS.
-- The route manifest the notification spine reads BEFORE it puts a deep link in an email, an in-app notice or a text (`aidream/services/notifications/link_honesty.py`). Its one writer is the SECURITY DEFINER door `platform.sync_route_manifest(p_app, p_routes, p_source_sha)`, called by `pnpm route-manifest:sync` from the deploy that changes the routes, with the ADMIN (service_role) client.
--
-- WHY IT MATTERS: PROVEN OVER THE WIRE, 2026-09-21: signed in as `test@test.com` (asserted zero `admin.admins` rows first), `POST /rest/v1/route_manifest` with `Content-Profile: platform` returned **201 Created** with `created_by` set to that plain member, and `DELETE` of the same row returned 200. A non-admin could tell the notification spine that any route is live, or delete the row that says a real one is.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a bare-name grep of the table name
-- across matrx-frontend (app, features, lib, components, hooks, utils), matrx-extend/src and
-- matrx-local/src, excluding generated entity-type catalogues, and then READING every hit rather
-- than counting `.from()`. `.from()` counts are a signal, never a proof.
--
-- THE DOOR IS UNAFFECTED. `platform.route_manifest` is owned by `postgres` and does NOT carry
-- FORCE ROW LEVEL SECURITY (read from pg_class, not assumed), and every writer is a
-- SECURITY DEFINER function owned by `postgres`. The owner is not subject to these policies,
-- so no door, trigger, event trigger or server path changes behaviour. `service_role` is
-- untouched. SELECT is untouched -- this constrains INSERT, UPDATE and DELETE only.
--
-- RESTRICTIVE, so it ANDs with every permissive policy present or future: no regeneration of
-- `std_insert` / `std_update` / `std_delete`, and no future hand-written permissive policy,
-- can re-open the write. The names are bespoke on purpose, so they are NOT in
-- `iam.generated_policy_names()` and `iam.apply_rls` preserves them across every regeneration
-- (DD-147).
--
-- ADDITIVE. Nothing is dropped, renamed or revoked. The dead GRANT that remains is the
-- lane's next step and is a chair step, because `platform` and `iam` are REVOKE-protected
-- schemas (scripts/lib/migration-target.ts REVOKE_PROTECTED_SCHEMAS).
-- Inverse: migrations/inverse/doorsonly_platform_route_manifest_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "route_manifest_client_insert_refused" on platform.route_manifest
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "route_manifest_client_update_refused" on platform.route_manifest
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "route_manifest_client_delete_refused" on platform.route_manifest
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "route_manifest_client_insert_refused" on platform.route_manifest is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). ROUTE semantics: PROVEN OVER THE WIRE, 2026-09-21: signed in as `test@test.com` (asserted zero `admin.admins` rows first), `POST /rest/v1/route_manifest` with `Content-Profile: platform` returned **201 Created** with `created_by` set to that plain member, and `DELETE` of the same row returned 200. A non-admin could tell the notification spine that any route is live, or delete the row that says a real one is. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "route_manifest_client_update_refused" on platform.route_manifest is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). ROUTE semantics: PROVEN OVER THE WIRE, 2026-09-21: signed in as `test@test.com` (asserted zero `admin.admins` rows first), `POST /rest/v1/route_manifest` with `Content-Profile: platform` returned **201 Created** with `created_by` set to that plain member, and `DELETE` of the same row returned 200. A non-admin could tell the notification spine that any route is live, or delete the row that says a real one is. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "route_manifest_client_delete_refused" on platform.route_manifest is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). ROUTE semantics: PROVEN OVER THE WIRE, 2026-09-21: signed in as `test@test.com` (asserted zero `admin.admins` rows first), `POST /rest/v1/route_manifest` with `Content-Profile: platform` returned **201 Created** with `created_by` set to that plain member, and `DELETE` of the same row returned 200. A non-admin could tell the notification spine that any route is live, or delete the row that says a real one is. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
