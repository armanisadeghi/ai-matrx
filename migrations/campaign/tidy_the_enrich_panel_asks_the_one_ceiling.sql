...
--
-- based-on: custom.enrich_due(uuid, uuid, integer, boolean) 8e874a62b859c2a92724fa713f0fe0fb877b446a710b926650d4b23886156fef
--
-- TIDY 2b — THE THIRD COPY, FOUND BY THE GUARD ON ITS FIRST REAL RUN.
--
-- CONTEXT-PERF named two implementations of the freshness ceiling. `pnpm
-- check:one-freshness-ceiling`, run against the live database the moment it existed,
-- named a third nobody had: `custom.enrich_due` — the door behind "which rows need this
-- column filled in again" — derived the age, compared it to `review_interval_days` and
-- wrote its own sentence, in two places that could disagree with each other as well as
-- with the other two copies. That is the guard earning its keep on day one, and it is why
-- a census beats a hand list.
--
-- WHAT CHANGES. Both places now ask `custom.freshness_verdict`: the `reason` column takes
-- its `stale_note`, and the WHERE clause that decides whether a row IS work takes its
-- `freshness`. They were separately derived before, so a row could be listed as due with a
-- sentence saying it was fresh; now one answer does both.
--
-- THE SENTENCE A PERSON READS CHANGES, and that is the point. "past its freshness date —
-- written 120 days ago and looked at again every 90 days" becomes "last written 120 day(s)
-- ago, past the 90-day freshness this field declares — it is delivered anyway and marked
-- stale rather than dropped". One rule, one sentence, everywhere it is said.
--
-- Everything else in the body is the live definition byte for byte (the `-- based-on:`
-- line above hashes what was replaced).
--
-- INVERSE: migrations/inverse/tidy_the_enrich_panel_asks_the_one_ceiling_down.sql
