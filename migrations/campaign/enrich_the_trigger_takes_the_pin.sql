-- chair-step: this REPLACES ONE live trigger body, `custom._value_envelope()`, under the
-- `-- based-on:` hash below, and does nothing else. No table, column, index, policy, grant
-- or row is touched. Its inverse, which puts the old body back byte for byte, is
-- `migrations/inverse/enrich_the_trigger_takes_the_pin_down.sql`.
-- lane: ENRICH (PRODUCTS row 10, "Fill in each company's industry and headcount, and keep it fresh")
-- based-on: custom._value_envelope() ff72af8fcc4fb53aa080f337515a8cdb338b03edd3f590f1aa43552ee98b567f
--
-- WHY IT CARRIES NO `-- guard:` LINE, AND WHY THAT IS THE HONEST ANSWER.
--
-- `guardUnreadBy` refuses a guarded file that replaces a `returns trigger` body in schema
-- `custom` unless the FILE TEXT names the knob. This body's very first statement is
--
--     perform custom.assert_store_door(new.organization_id, 'custom.record');
--
-- which IS the read of `custom/system_enabled` — the check cannot see it through a helper,
-- and it is right not to try. The two ways to satisfy the text search would each be worse
-- than stating the judgement plainly: duplicating `platform.knob_resolve` into a body that
-- already asks the one door would give this trigger a second, drifting copy of the switch,
-- and putting the knob's name in a comment or a hint string would be decoration dressed as
-- a mechanism. So this file is production-only and deny-list judged, and says so here —
-- the same route and the same reasoning as
-- `apprvfix_the_queue_holds_a_record_write_too.sql` (lane APPROVAL-FIX, 2026-09-19).
--
-- THE SWITCH STILL CLOSES IT. The replaced body's first line is untouched, so while
-- `custom/system_enabled` resolves false for an organization this trigger refuses every
-- write into that organization's store exactly as it does today.
--
-- WHAT CHANGES, in three lines: the loop that already walks this Table's Fields also notes
-- which of them a model owns (`source = 'agent'`); `custom.pin_agent_cells` then pins a cell
-- a PERSON typed into one of those columns; and `custom.carry_unchanged_value_stamps` puts
-- back the moment and the author of every value whose version did not move, so
-- "when was THIS value written" stops answering "when was this RECORD written". Both
-- functions are created by `enrich_a_persons_edit_holds_the_cell.sql`, which runs first.
-- The full argument for both, with the measurements, is in that file's header.
