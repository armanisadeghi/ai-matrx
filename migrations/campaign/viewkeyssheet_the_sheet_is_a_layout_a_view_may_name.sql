...
-- additive: yes
--   It REPLACES two bodies S1-PRIME created, and nothing else: `custom.view_keys()` (the registry)
--   and `custom._view_key_value(...)` (the one-key judge), both declared below with the bodies they
--   were written against (uichamp_s1_a_view_keeps_every_setting_it_is_given.sql, byte for byte).
--   So this file applies only AFTER S1-PRIME's; the based-on gate refuses it anywhere else. Every
--   S1-PRIME rule is kept word for word; the one change is that the `kind` shape — the view's
--   `layout` — admits "sheet" beside grid, kanban, calendar and gallery. No table, trigger, policy
--   or grant is touched (CREATE OR REPLACE keeps every EXECUTE); no stored view is rewritten.
--   The inverse is `migrations/inverse/viewkeyssheet_the_sheet_is_a_layout_a_view_may_name_down.sql`.
-- guard: custom/system_enabled
-- lock: custom
-- lane: VIEW-KEYS-SHEET
-- based-on: custom.view_keys() 8eb5e8dd065c5b714f7f4e7b839e2c362e66d50bfd8186b4a2110b0be2e26870
-- based-on: custom._view_key_value(uuid, uuid, text, text, jsonb, jsonb) 0f591c52b11f446f37574988a454a821cab9a56dbaf0f7a9551115c5d974008e
--
-- LANE VIEW-KEYS-SHEET. "THIS TABLE OPENS AS THE SHEET" IS ONE THING: `layout: "sheet"` ON ITS
-- DEFAULT VIEW, A LAYOUT THE VIEW-KEYS REGISTRY DECLARES.
--
...
-- it should in the default view."
--
-- THE CONFLICT THIS FILE ENDS. Two words said "Sheet": DATA-V2-FACE wrote `layout: "sheet"` on
-- Rincon Service Calls' default view, and DATA-V2-FACE-2's mover and repair said it by the mover's
-- mark (`moved_from.kind = "default_view"`, no layout) because S1-PRIME's guard refused "sheet" as a
-- layout. records-ui `designatedLayoutOf` read both. Once S1-PRIME landed, the designation script
-- (datav2face_designate.mjs) would have been refused, and the Sheet — a layout like grid or kanban,
-- which S1-PRIME's own registry already lists as a reader of 25 keys — would not have been a legal
-- value of `layout` at all. Now:
--   · `layout` is one of grid · kanban · calendar · gallery · sheet (the four records-ui draws, and
--     the Sheet the host draws). Any other word is refused by name, as before.
--   · the mover (aidream matrx_records/movers/attributes.py) and the repair
--     (viewkeyssheet_a_copied_tables_default_view_names_the_sheet.sql) write `layout: "sheet"`;
--     `moved_from` stays as provenance only (where the view came from), never as the designation.
--   · the layout buttons still never write "sheet" (records-ui VIEW_LAYOUTS is the four), so a
--     click can never take the layout chooser away; a designation stays a deliberate write.
--
...
-- ── 1. THE REGISTRY: `layout` names the Sheet among its values ─────────────────────────────────
...
-- ── 2. ONE KEY, BY ITS SHAPE: the kind admits the Sheet ─────────────────────────────────────────
create or replace function custom._view_key_value(p_organization_id uuid, p_table_id uuid, p_path text, p_shape text,
