-- chair-step: public.resolve_full_context and custom.resolve_context (both five-argument, same signatures, so every grant and door row stays) read a NULL p_system_item_refs as the platform's default list (knob context/system_item_defaults) instead of none; an explicit empty array still reads none. Adds context.system_item_refs_or_defaults(text[]). WHAT CHANGES FOR A CALLER: a caller that does not pass the list — the server released before lane CONTEXT-VALUES-NAMED-2, which calls with four arguments — gets today's date, the date-time and the timezone again instead of no System item; a caller naming a list, or naming none with '{}', is unchanged.
...
-- based-on: custom.resolve_context(text, uuid, uuid[], uuid[], text[]) 89e381e89719691138040a1a4bc561da743726bc3892893fc58a244bf941d1a0
--
-- Inverse: migrations/inverse/cvn3_an_old_caller_gets_the_default_system_items_down.sql.
--
-- THE USE CASE. cvn2 made both resolvers read only the System items a turn names, with NULL
-- reading none. The server already on production (released before the lane's engine) calls the old
-- four-argument shape, so for over two hours every agent turn carried no date at all. The chair's
-- ruling: NULL means "the caller did not know about the list" → the knob's default list; an explicit
-- empty array means "none". The new server always sends an explicit array.

set local lock_timeout = '30s';
set local statement_timeout = '60s';

create or replace function context.system_item_refs_or_defaults(p_refs text[])
 returns text[]
 language sql
 stable
 set search_path to ''
as $function$
    -- NULL (a caller that does not pass the list) is the platform's default list, the knob
    -- context/system_item_defaults; any array, including an empty one, is the caller's own naming.
    select case
             when p_refs is not null then p_refs
             else coalesce(
               (select array(select pg_catalog.jsonb_array_elements_text(k.value))
                  from platform.feature_knob k
                 where k.feature = 'context' and k.key = 'system_item_defaults'
                   and k.archived_at is null
                   and pg_catalog.jsonb_typeof(k.value) = 'array'),
               '{}'::text[])
           end
$function$;

comment on function context.system_item_refs_or_defaults(text[]) is
  'Lane CONTEXT-VALUES-NAMED-2 (chair ruling): the System item refs a resolver reads. NULL = the platform default list (knob context/system_item_defaults); an array (even empty) = exactly that naming.';

revoke all on function context.system_item_refs_or_defaults(text[]) from public, anon, authenticated;
grant execute on function context.system_item_refs_or_defaults(text[]) to service_role;
