-- chair-step: this REPLACES three live bodies — platform.provision, iam.apply_table_grants and iam._apply_rls_unchecked — so that the provisioner stops handing client-role grants to a schema declared closed. It is a SECURITY REPAIR (rule 4's first exception), which is exactly why it must NOT read `custom/system_enabled`: a repair held OFF by the campaign's own switch is not a repair. The additive allow-list refuses it for that reason, by name, with `guard-unread`, and that refusal is correct — so the change travels the loud route instead, and a person reads all three bodies before they run.
...
--
-- 🚨 THIS FILE IS THE THIRD REHEARSAL OF ONE CHANGE, AND THE TWO BEFORE IT ARE LEDGER ROWS OF
-- WORK THAT RAN AND WAS REVERSED BY ITS OWN INVERSE (§4.13 working, twice). Said plainly rather
-- than tidied away, because the ledger names them and the next reader will find them:
--   `w1_prov_the_provisioner_honours_declared_exposure.sql` (18:01:19Z, reversed 18:06:44Z)
--     carried the registry and its two functions but NOT the three replaced bodies: the editing
--     pass that moved the seed row into its own file sliced the file at that marker and took the
--     function definitions with it, and the apply still reported OK because everything that
--     remained was valid SQL. The tell was the GREEN probe, which measures the CATALOGUE rather
...
--     the remedy it printed read `where schema_name = customL`, a statement nobody can paste.
-- A new filename each time because the runner refuses a file already ledgered under a different
-- SHA-256, which is right: `--accept-drift` would have re-pointed the row without executing
-- anything, and that is the one move that makes the ledger lie.
--
-- OLD NOTE, KEPT FOR THE LEDGER TRAIL — why this file is not `w1_prov_the_provisioner_honours_declared_exposure.sql`, WHICH THE
-- BRANCH LEDGER NAMES. Those bytes RAN at 18:01:19Z and were REVERSED by their own inverse at
-- 18:06:44Z (§4.13 working). They carried the registry and its two functions but NOT the three
-- replaced bodies: the editing pass that moved the seed row into its own file sliced the file at
-- that marker and took the function definitions with it, and the apply still reported OK because
-- everything that remained was valid SQL. The tell was the GREEN probe, which measured the
-- catalogue rather than trusting the apply: schema `custom` re-opened on the very next provision
-- exactly as it had in RED. A new filename because the runner refuses a file already ledgered
-- with a different SHA-256, which is right — `--accept-drift` would have re-pointed that row
-- without executing anything, and that is the one move that makes the ledger lie.
--
-- 🚨 THE `-- based-on:` HASH FOR `platform.provision` IS THE **BRANCH'S**, NOT PRODUCTION'S, AND
-- THAT IS AN ORDERING FACT SOMEBODY HAS TO ACT ON. Measured 2026-09-17: the branch's body is
-- sha256 21d50d62…, production's is ec762f6a… — `W1-PROV`'s `w1_prov_provisioner_speaks_partitions.sql`
-- widened the provisioner to speak hash partitions on the BRANCH and its production half is still
-- an attended step. This file is written against the branch body, so at `--target production` the
-- runner will REFUSE it by name until that partition file has landed there. That refusal is the
-- DD-220 guard doing precisely its job and must not be routed around: apply `W1-PROV`'s file
-- first, then re-read the live body and regenerate this line before this one goes anywhere near
-- production.
--
...
--
-- WHAT LANDS ON PRODUCTION FROM THIS FILE: three new objects and three replaced function bodies,
-- and no registry row at all. Until `w1_prov_closed_declares_custom_closed.sql` seeds one, the
-- registry is EMPTY and `platform.schema_is_client_exposed` answers `true` for every schema and the three replaced bodies take their existing branch for
