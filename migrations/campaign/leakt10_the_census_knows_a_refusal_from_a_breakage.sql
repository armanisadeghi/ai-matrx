...
-- based-on: custom.list_door_disagreements(text, uuid, integer) 09d0510b011cf9e3e6fa205e79030e4eca515f1456076ee300d95be07c4b164a
--
-- LEAK-T10 — CENSUS 13 KNOWS A REFUSAL FROM A BREAKAGE, AND CALLS ONE DOOR CORRECTLY.
--
-- Its first run on the main database named 147 rows. TWELVE of them said four list doors were
-- leaking, and they were not: `custom.read_record` was raising 22023 — "this rule points at a
-- field with title instead of with its id" — on those Tables, and the census read every
-- exception as "she may not see this row". A door that DIES is not a door that says no.
-- It now treats ONLY `insufficient_privilege` as the answer "no", and reports anything else as
-- `unmeasured: custom.read_record raised <sqlstate>`, which is a real finding of its own: one
-- malformed Rule row makes a whole Table unreadable through every read door for everybody.
--
-- The other 129 were the census calling `custom.query_by_coordinates` with `{}` where it takes a
-- JSON ARRAY of coordinates. `[]` is the right empty.
--
-- A census that guesses is worse than no census, and a census whose own broken call reads as a
-- product defect trains the next person to ignore it.
