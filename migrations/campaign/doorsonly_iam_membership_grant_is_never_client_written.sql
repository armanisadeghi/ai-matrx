...
--
-- iam.membership_grant -- GRANT SEMANTICS.
-- The grant half of organization membership. Zero client mentions in any repository; every writer is a SECURITY DEFINER door owned by `postgres` (`public.admin_manage_organization_membership`, the `iam.*` grant family).
--
-- WHY IT MATTERS: This is the table that decides what a member of an organization may do. A client INSERT here is a self-service promotion.
--
...
--
-- THE DOOR IS UNAFFECTED. `iam.membership_grant` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_iam_membership_grant_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "membership_grant_client_insert_refused" on iam.membership_grant
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "membership_grant_client_update_refused" on iam.membership_grant
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "membership_grant_client_delete_refused" on iam.membership_grant
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "membership_grant_client_insert_refused" on iam.membership_grant is
  'DOORS-ONLY 2026-09-21: iam and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). GRANT semantics: This is the table that decides what a member of an organization may do. A client INSERT here is a self-service promotion. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "membership_grant_client_update_refused" on iam.membership_grant is
  'DOORS-ONLY 2026-09-21: iam and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). GRANT semantics: This is the table that decides what a member of an organization may do. A client INSERT here is a self-service promotion. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "membership_grant_client_delete_refused" on iam.membership_grant is
  'DOORS-ONLY 2026-09-21: iam and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). GRANT semantics: This is the table that decides what a member of an organization may do. A client INSERT here is a self-service promotion. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
