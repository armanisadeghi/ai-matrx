...
--
-- GUARD-PERF — `strict` IS A PLPGSQL WORD, SO THE CENSUS COULD NOT READ ITS OWN COLUMN.
--
-- guardperf_a_census_that_dies_on_the_clock_is_not_a_verdict.sql selected the organization's
-- privacy setting as `... as strict` and then read `v_org.strict`. plpgsql owns that word
-- (`select ... into strict`), so the field was never resolvable and every call raised
--     record "v_org" has no field "strict"
-- — which is a census that cannot run at all, caught on its first live call. The column is
-- now `shared_only_org`. Nothing else in the body changes.
--
-- THE INVERSE is migrations/inverse/guardperf_a_word_plpgsql_owns_is_not_a_column_name_down.sql.
--
-- based-on: custom.list_door_disagreements(text, uuid, integer, boolean) a0e0ab260625a2bcbf657f4fef9b8e18e5458c69d1e0cddc50a69eda45353ae0
