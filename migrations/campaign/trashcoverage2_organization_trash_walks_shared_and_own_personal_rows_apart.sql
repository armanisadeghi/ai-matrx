-- chair-step: lane TRASH-COVERAGE-2. Organization Trash reads a table that carries visibility as two bounded index walks — the organization's shared archived rows, and the caller's own personal ones — instead of one walk that reads and discards every member's personal row. Same rows, same order.
--
-- Found by pnpm check:trash-answers-fast on production (org_trash_list file for AI Matrx 100-890 ms and
-- the merged organization page 308-412 ms, ceiling 300): AI Matrx holds 75,540 archived personal files
-- and 3 shared ones. The shared half rides files_trash_org_shared_deleted_idx
-- (trashcoverage2_organization_trash_reads_shared_rows_through_their_own_index.sql); the own-personal
-- half rides files_trash_owner_deleted_idx (TRASH-2). The predicate is the old one split in two:
-- "shared, or personal and mine" = "shared" UNION ALL "personal and mine" (disjoint).
--
-- INVERSE: migrations/inverse/trashcoverage2_organization_trash_walks_shared_and_own_personal_rows_apart_down.sql
