...
--
-- interview.decision_interview.respondent_user_id genuinely names a person other than the writer, so pinning it to
...
-- Inverse: migrations/inverse/secsweep2_interview_decision_interview_respondent_user_id.inverse.sql
...
create policy "decision_interview_respondent_user_id_is_addressable_insert" on interview.decision_interview
  as restrictive for insert to authenticated, anon
  with check (respondent_user_id = (select auth.uid()) or iam.may_address_user_in_org(respondent_user_id, organization_id) or (select public.is_platform_admin()));

create policy "decision_interview_respondent_user_id_is_addressable_update" on interview.decision_interview
