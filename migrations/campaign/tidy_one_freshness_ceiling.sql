...
--
-- based-on: custom.context_resolve(uuid, jsonb, text) 43f15a2005b5b843208ff72424ba158c7c243ad7a21bb46cd38244aa19214a80
--
-- TIDY 2 — THE FRESHNESS CEILING IS COMPUTED ONCE.
--
-- CONTEXT-PERF's closing note: *"The freshness ceiling is computed twice — once by the door
-- (returned as `freshness`/`stale_note`) and once by the resolver from the same `written_at`.
-- They read the same input under the same rule, so they cannot disagree today, but one of
-- them should eventually be the only one."*
--
-- They could not disagree only because two people had written the same eighteen lines
-- twice and nothing checked. `custom.freshness_verdict(written_at, ceiling_seconds)` is now
-- the one implementation. `custom.context_resolve` calls it; everything else in the file is
-- the live body byte for byte (the `-- based-on:` line above hashes what was replaced), and
-- the three local variables the deleted block owned — `v_age`, `v_ceiling`, `v_written` —
-- are gone with it. The merge resolver in `matrx-records` deletes its Python copy in the
-- same landing and reads this door's answer.
--
-- THE SENTENCE IS THE PRODUCT. A stale value is DELIVERED, never dropped: dropping a real
-- value because nobody re-verified it is worse than saying how old it is. An unreadable or
-- missing write time is NAMED, never treated as fresh — "we could not tell how old it is"
-- and "it is current" are different answers, and the panel shows the difference.
--
-- INVERSE: migrations/inverse/tidy_one_freshness_ceiling_down.sql
...
as $function$
  select case
    when p_ceiling_seconds is null or p_ceiling_seconds <= 0 then
      jsonb_build_object('freshness', 'live', 'stale_note', null)
    when p_written_at is null then
      jsonb_build_object('freshness', 'live', 'stale_note',
        'this value carries no write time, so whether it is inside its freshness ceiling is unknown')
    when extract(epoch from (now() - p_written_at)) > p_ceiling_seconds then
      jsonb_build_object('freshness', 'stale', 'stale_note',
        format('last written %s day(s) ago, past the %s-day freshness this field declares — it is delivered anyway and marked stale rather than dropped',
               floor(extract(epoch from (now() - p_written_at)) / 86400)::int,
               greatest(floor(p_ceiling_seconds / 86400)::int, 1)))
    else
      jsonb_build_object('freshness', 'live', 'stale_note', null)
  end;
$function$;
...
values
  ('custom', 'freshness_verdict', 'p_written_at timestamp with time zone, p_ceiling_seconds numeric',
   array['timestamptz'::regtype, 'numeric'::regtype]::oid[],
   'Takes no entity id at all: two scalars in, a verdict out. It reads no table and can therefore leak nothing, whoever calls it.',
   'tidy_one_freshness_ceiling.sql',
   'server_only: called inside custom.context_resolve, which is the door a client reaches. A client that wants a value''s freshness gets it in that door''s answer, so this one needs no grant of its own.',
   false, false)
on conflict do nothing;

comment on function custom.freshness_verdict(timestamptz, numeric) is
  'THE ONE freshness ceiling. Given when a value was written and the ceiling its own '
  'declaration promises (review_interval_days x 86400), answers {freshness, stale_note}: '
  'live, or stale WITH its age in a sentence. A stale value is delivered anyway; a missing '
  'or unreadable write time is named rather than passed off as current. A second '
  'implementation of this rule anywhere fails pnpm check:one-freshness-ceiling.';
