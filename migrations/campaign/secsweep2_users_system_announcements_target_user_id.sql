...
create policy "system_announcements_target_user_id_is_addressable_insert" on users.system_announcements
  ...
  with check (target_user_id is null or target_user_id = (select auth.uid()) or iam.has_org_access_for(target_user_id, organization_id) or (select public.is_platform_admin()));
