...
--
-- users.system_announcements.target_user_id genuinely names a person other than the writer, so pinning it to
...
-- Inverse: migrations/inverse/secsweep2_users_system_announcements_target_user_id.inverse.sql
...
create policy "system_announcements_target_user_id_is_addressable_insert" on users.system_announcements
  as restrictive for insert to authenticated, anon
  with check (target_user_id is null or target_user_id = (select auth.uid()) or iam.may_address_user_in_org(target_user_id, organization_id) or (select public.is_platform_admin()));

create policy "system_announcements_target_user_id_is_addressable_update" on users.system_announcements
