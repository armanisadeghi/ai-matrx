-- chair-step: a STORE FIX, the second half of storetails3_what_one_archive_takes_one_restore_brings_back.sql.
--   REPLACES one body, declared below with the body it was written against:
--   `history.migration_undo`. No table, column, trigger, policy or grant is added; no row of
--   anybody's data is rewritten.
--   Inverse: `migrations/inverse/storetails3_an_undo_counts_what_came_back_with_it_down.sql`.
--   Applied directly (owner, 2026-09-24 ~17:30 PT: "all db stuff applied directly"), proven on the
--   dev clone first (tabledelete_green 1b and doorfix_green 1c RED over the new restore with this
--   body's old count, GREEN with this one; up, inverse, up).
-- lock: custom
-- lane: STORE-TAILS-3
-- based-on: history.migration_undo(uuid, uuid) efb161e91c23e6db45e48c7e5c8739266a7909dc47261f1f395251c3ef521a2f
--
-- WHY. Undoing a logged delete (`history.migration_undo`, the "undo" of custom.migrate_delete)
-- restores the root through custom.record_restore and then walks the delete's `also` list,
-- counting each row it has to bring back itself as `also_restored`. Since the restore brings back
-- the whole unit its archive took, those rows are already back when the walk reaches them, and the
-- answer said "0 came back with it" about a delete that took three records and an undo that
-- returned all three. The count is now what it claims: the rows of `also` that were archived
-- before the undo and are here after it. What the undo DOES is unchanged.
