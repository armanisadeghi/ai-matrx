...
--
-- platform.ddl_guard_log -- AUDIT SEMANTICS.
-- The record of what the database's own DDL guards refused and why, 3,605 rows, written by event triggers running as the table owner. Zero client mentions.
--
-- WHY IT MATTERS: A client that writes the guard log can forge the evidence of what the schema guards did.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.ddl_guard_log` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_ddl_guard_log_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "ddl_guard_log_client_insert_refused" on platform.ddl_guard_log
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "ddl_guard_log_client_update_refused" on platform.ddl_guard_log
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "ddl_guard_log_client_delete_refused" on platform.ddl_guard_log
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "ddl_guard_log_client_insert_refused" on platform.ddl_guard_log is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: A client that writes the guard log can forge the evidence of what the schema guards did. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "ddl_guard_log_client_update_refused" on platform.ddl_guard_log is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: A client that writes the guard log can forge the evidence of what the schema guards did. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "ddl_guard_log_client_delete_refused" on platform.ddl_guard_log is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: A client that writes the guard log can forge the evidence of what the schema guards did. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
