-- chair-step: lane SHARE-LANE-2 (round 2). The personal-Table question the access kernel's organization-role arms ask plans ONCE per session and prunes to one partition, and the kernel fingerprint the provisioner checks is re-recorded. REPLACES custom.row_sits_in_a_personal_table(text, uuid) (LANGUAGE sql, non-inlinable, so it re-planned on every call and custom.ladder_replanners() named it — writeperf3_green 10, check:store-doors-decide) with custom.row_sits_in_a_personal_table(text, uuid, uuid) in plpgsql, handed the organization the kernel already holds, so both lookups are custom.record's primary key (organization_id, id) in one hash partition instead of an id-only probe of all sixteen. REPLACES iam.has_access_for_base to call it (the only change: the extra argument). Re-records iam.entity_read_kernel_expected() — sharelane2_an_owner_does_not_open_a_personal_table.sql changed a fingerprinted body by ruling and re-recorded nothing, so platform.provision refused every spec with preflight.read_kernel from 20:38Z until this file (D249's class). The literal is the fingerprint these exact bodies produce, measured on the clone in a rolled-back transaction, and the file refuses itself if the live fingerprint after the replace is anything else. Same answers: sharelane2_green, sharetails_green, sharelane_green, sharepeople_green, share_green on the clone. No data write.
-- based-on: iam.has_access_for_base(uuid, text, uuid, permission_level, boolean, text[]) a484caa5d4d202a8a32d4a025e94574e820828330d1b93624f2a4b63eecdb9f3
-- based-on: custom.row_sits_in_a_personal_table(text, uuid) aabac5ded6184ad56132cb071f2b5cdbfa8525696db6e1810728b57ca57080d0
-- lane: SHARE-LANE-2
-- INVERSE: migrations/inverse/sharelane2_the_personal_table_question_plans_once_down.sql
set local lock_timeout = '30s';

CREATE OR REPLACE FUNCTION custom.row_sits_in_a_personal_table(p_type text, p_organization_id uuid, p_id uuid)
 RETURNS boolean
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
-- A Table row itself answers false: its own visibility is its answer.
-- plpgsql so it plans once per session (LADDER-PERF; custom.ladder_replanners() is the census), and
-- keyed by the organization so both reads are custom.record's primary key in one hash partition.
...
declare
  v_table uuid;
begin
  if p_type is distinct from 'record' or p_organization_id is null or p_id is null then
    return false;
  end if;
  select r.table_id into v_table
    from custom.record r
   where r.organization_id = p_organization_id and r.id = p_id;
  if v_table is null or v_table = custom.table_kernel_id() then
    return false;
  end if;
  return exists (select 1 from custom.record t
                  where t.organization_id = p_organization_id and t.id = v_table
                    and t.visibility < 'internal'::platform.visibility);
end;
$function$;
revoke all on function custom.row_sits_in_a_personal_table(text, uuid, uuid) from public, anon, authenticated;
...
drop function if exists custom.row_sits_in_a_personal_table(text, uuid);

-- THE RE-RECORD (D249). The bodies above changed by ruling; this is the fingerprint they produce.
...
begin
  if iam.entity_read_kernel_fingerprint() is distinct from iam.entity_read_kernel_expected() then
    raise exception 'sharelane2: the live access-kernel fingerprint is % but this file records %; another lane moved a fingerprinted body — re-derive the file.',
