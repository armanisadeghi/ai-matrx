...
-- based-on: custom.rule_eval(uuid, jsonb, jsonb, jsonb) 4450d53d5614a1bcd38d2ad112c4cd99c0fffa8512c24667a1f23239f7f392d3
--
-- ══════════════════════════════════════════════════════════════════════════════════════
-- THE CENSUS OF THE NULL-INSTEAD-OF-TRUE TEST, AND THE ONE SITE IT LEFT OPEN.
-- lane STAGE-RULES-2 · 2026-09-21
--
-- STAGE-RULES-2 found `jsonb_typeof(x) <> 'object'` accepting a MISSING key in two doors
-- and named the wider sweep rather than shipping it blind. This is the sweep.
--
-- WHAT WAS MEASURED. Every `jsonb_typeof(…) <>` / `!=` comparison in every function body in
-- schema `custom`, read out of `pg_proc.prosrc` — 89 lines across 40 functions — and then
-- narrowed to the ones that can actually hurt: the test is inside a branch that RAISES, and
-- its argument is a `->` lookup that can be ABSENT. Fourteen lines survived that narrowing.
-- Thirteen of them are already safe, and it is worth saying HOW, because the shape alone
-- does not tell you:
--
--   · guarded by `?` on the same line or the line above — the key is known to exist:
--     `_field_shape_guard` (compute_on), `_merge_field_temporal_guard` (at),
--     `_rule_shape_guard` (use_types, and its per-use lists keyed by `jsonb_object_keys`
--     of that same object), `enrich_normalize` (review_interval_days),
--     `pin_agent_cells` (pinned), `rule_eval` (field, parent_field);
--   · guarded by the jsonpath that selected the leaf — `_rule_topology_guard` only ever
--     sees leaves where `exists(@.parent_field)`, and the two-steps-up leaves are refused
--     by name four lines earlier;
--   · already coalesced — `_rule_shape_guard` and `rule_eval` on `same`/`differs`,
--     `custom.pipeline_declare` and `custom.pipeline_gate_preview` (this lane, an hour ago).
--
-- Every other site in the schema is a DEFAULT rather than a refusal (`if not an object then
-- use {} / return the input unchanged / skip this row`), where NULL falling through to the
-- else arm is the intended answer and changing it would change behaviour for no reason.
--
-- THE ONE THAT WAS REAL: `custom.rule_eval`'s `previous` node, which lane STAGE-RULES added.
--
--   select custom.rule_eval(<org>, '{"op":"previous"}', '{}', '{}');   -->  null
--   select custom.rule_eval(<org>, '{"op":"previous","field":"not-an-id"}', …);
--       -->  ERROR: this rule asks what a field used to say and points at it with
--            not-an-id instead of with its id
--
-- A `previous` node with a BAD field is refused; a `previous` node with NO field at all was
-- not. It fell through the type test (NULL), through the uuid test (NULL), and returned
-- `p_context -> 'previous_values' -> NULL`, which is NULL. `custom.rule_truth(NULL)` is
-- NULL — not false — so a stage gate written with a bare `previous` node PASSED EVERY CARD:
-- it stopped nobody and it said nothing. That is the same failure as the half-written gate
-- this lane closed two files ago, one layer down, and in the node the gates are built on.
--
-- The refusal's own sentence — "points at it with nothing" — is the proof that the absent
-- case was meant to reach it. Nothing else in this file changes.
-- ══════════════════════════════════════════════════════════════════════════════════════
