...
--
-- iam.memberships -- MEMBERSHIP SEMANTICS.
-- Who belongs to which organization, 775 rows -- the table every access decision on the platform ultimately reads. Its client write door is `public.admin_manage_organization_membership`; the two client call sites (`app/api/stripe/class-checkout/route.ts`, `lib/organizations/organizationRequiredServerError.ts`) are both `.select(...)`.
--
-- WHY IT MATTERS: A client INSERT here is joining an organization you were never invited to. VERIFIER-8 named this table in HIGH-3 by name.
--
...
--
-- THE DOOR IS UNAFFECTED. `iam.memberships` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_iam_memberships_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "memberships_client_insert_refused" on iam.memberships
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "memberships_client_update_refused" on iam.memberships
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "memberships_client_delete_refused" on iam.memberships
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "memberships_client_insert_refused" on iam.memberships is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). MEMBERSHIP semantics: A client INSERT here is joining an organization you were never invited to. VERIFIER-8 named this table in HIGH-3 by name. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "memberships_client_update_refused" on iam.memberships is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). MEMBERSHIP semantics: A client INSERT here is joining an organization you were never invited to. VERIFIER-8 named this table in HIGH-3 by name. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "memberships_client_delete_refused" on iam.memberships is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). MEMBERSHIP semantics: A client INSERT here is joining an organization you were never invited to. VERIFIER-8 named this table in HIGH-3 by name. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local or the @ai-matrx packages, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
