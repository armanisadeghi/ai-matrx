...
--
-- communication.sms_notifications.user_id is the person the row is about, and the app writes this row AS that
...
-- Inverse: migrations/inverse/secsweep2_communication_sms_notifications_user_id_is_the_caller.inverse.sql
-- Guard: `pnpm check:unpinned-security-columns` -- both baseline rows for this pair leave.

set local lock_timeout = '2s';

create policy "sms_notifications_user_id_is_the_caller_insert" on communication.sms_notifications
  as restrictive for insert to authenticated, anon
  with check (user_id is null or user_id = (select auth.uid()) or (select public.is_platform_admin()));

create policy "sms_notifications_user_id_is_the_caller_update" on communication.sms_notifications
  as restrictive for update to authenticated, anon
  with check (user_id is null or user_id = (select auth.uid()) or (select public.is_platform_admin()));

comment on policy "sms_notifications_user_id_is_the_caller_insert" on communication.sms_notifications is
  'SECURITY-SWEEP-2 2026-09-21: communication.sms_notifications.user_id names the CALLER, so it is pinned to auth.uid(). The generated std_insert/std_update policies pin created_by and organization_id and say nothing about this column, so a signed-in member could POST a row naming somebody else. RESTRICTIVE, so it ANDs with every permissive policy present or future; bespoke on purpose, so iam.apply_rls preserves it (DD-147). Evidence the column names the caller: every live row has user_id = created_by. NULL is allowed where the column is nullable -- NULL names nobody and cannot impersonate. Reads, deletes and service_role are untouched.';
comment on policy "sms_notifications_user_id_is_the_caller_update" on communication.sms_notifications is
  'SECURITY-SWEEP-2 2026-09-21: communication.sms_notifications.user_id names the CALLER, so it is pinned to auth.uid(). The generated std_insert/std_update policies pin created_by and organization_id and say nothing about this column, so a signed-in member could POST a row naming somebody else. RESTRICTIVE, so it ANDs with every permissive policy present or future; bespoke on purpose, so iam.apply_rls preserves it (DD-147). Evidence the column names the caller: every live row has user_id = created_by. NULL is allowed where the column is nullable -- NULL names nobody and cannot impersonate. Reads, deletes and service_role are untouched.';
