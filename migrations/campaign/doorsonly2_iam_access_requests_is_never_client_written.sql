...
-- removing the door.
--
-- iam.access_requests -- REQUEST SEMANTICS.
-- Somebody asking for access they do not have. The ONE writer in the codebase --
-- `app/api/cms/access-context/route.ts` -- writes through `createAdminClient()`, which is
-- `service_role` and is untouched by an `authenticated` policy. That was read from the file's
-- own `const admin = createAdminClient()` line, not inferred from the `.insert(` call, and an
-- INDEPENDENT census run by an agent that never saw this lane's file list reached the same
-- verdict and said so in the same words.
--
-- WHY IT MATTERS: A member who writes an access request writes, in somebody else's name, the row an
-- administrator then approves. The whole point of a request table is that the requester cannot
-- grant it -- and a forgeable request is a request an approver cannot trust.
--
...
-- door built and their callers moved first. This table's only write goes through service_role.
--
-- THE DOOR IS UNAFFECTED. `iam.access_requests` is owned by `postgres` and does NOT carry
