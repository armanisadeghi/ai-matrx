...
-- based-on: custom.organization_clear(uuid, text, boolean) 0650e94c561c8816fe50c3c4505f067d7f4335a99a147296ff58e049c0cac0b9
--
-- ORG-DELETE — THE CLEAR ORDERS ITS OWN CENSUS.
--
-- `custom.organization_clear` asks the catalog which tables reach `iam.organizations` and
-- clears `custom.record` and `history.migration_log` LAST, because the other tables point at
-- rows in them. That ORDER BY was written over a SELECT DISTINCT without the sort expression
-- in the select list, which Postgres refuses at run time (0A000, "for SELECT DISTINCT, ORDER
-- BY expressions must appear in select list") — so the destroying arm died on its own loop the
-- first time anybody ran it, found by `scripts/campaign-tests/orgdel_green.sql` PART 2b.
--
-- Same body, same decisions, same doors: the census is moved into a subquery so the ordering
-- column can be selected and then dropped. Nothing else changes.
--
-- WHAT MAKES IT FAIL (rule 3): put the ORDER BY back on the DISTINCT, which is what
-- migrations/inverse/orgdel_the_clear_orders_its_own_census_down.sql does — and
-- `scripts/campaign-tests/orgdel_green.sql` PART 2b goes red on 0A000.
