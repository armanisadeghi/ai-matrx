...
--
-- based-on: custom._field_type_converts_values() ddcb1beb629c4db8fd7cb5d924f3a5840d346914250ba79718f98cb2b151a9db
--
-- DOOR-FIX 3b — THE ALTERNATES CONVERT TOO.
--
-- VAL-3: an alternate is a candidate for the SAME field, and custom.validate_value_envelope
-- judges it by the SAME behaviour. So converting a record's value while its merged-in
-- alternate stayed the old shape produced a document that could not be written — the very
-- write doing the converting failed with "Phone takes a number, and it was given a string",
-- which is the T12 symptom again one level down. Measured on the main database, 13:43Z.
--
-- Every alternate is converted with the same one conversion table; one that does not convert
-- is kept in `_retired` as what it was, with the rank it held and where it came from.
--
-- INVERSE: migrations/inverse/doorfix_the_alternates_convert_too_down.sql
