-- IMPORT-2 — every key of a field patch is either STORED or REFUSED BY NAME.
--
-- THE DEFECT (FIX-10B's own concern (2)): `custom.field_update` accepted a patch carrying
-- `expr` — a formula — reported success and stored nothing. A silent no-op.
--
-- THE CLASS, NOT THE INSTANCE. `expr` was the fifth key of this shape to be found by hand:
-- `plain`/`type` (T12, closed by STORE-T), `promoted`/`unique` (SEAT-SUITES, 2026-09-19),
-- `multi` (FIX-7B-FIELD, 2026-09-20), `source`/`review_interval_days` (ENRICH, 2026-09-20),
-- `compute_on` (TAILS-2, 2026-09-21). Each was found the same way — somebody noticed a control
-- that ticked and changed nothing — and each was closed for itself. The door had no list of
-- what it accepts, so nothing could tell a key it stores from a key it drops.
--
-- The CENSUS of the door against what actually calls it, 2026-09-22:
--   · read by the body, in every arm: label, required, dated, sort, sensitivity,
--     context_policy, unit, rules, options, display, multi, promoted, unique, depends_on,
--     compute_on, relation_target, relation_max, on_target_delete, source, source_config,
--     review_interval_days, parity_type, plain, type, key
--   · read ONLY inside the behaviour arm (so dropped without a type word beside them, which is
--     the defect): expr, applies_to_types, options_table_id
--   · sent by the Edit-field panel (`records-ui` FieldEditor): label, required, dated,
--     sensitivity, context_policy, rules, multi, unit, options, display — all stored.
--   · sent by `systemTable.ts` when it levels a standard table: type, multi — both stored.
--   · sent by the Grid when a person adds pasted choices: options — stored.
--   · the SIBLING DOOR `custom.entity_field_update` has had exactly this guard since FLD-12
--     ("A field has no setting called %"), so this is the one of the pair that was missing it.
--
-- WHAT THIS FILE DOES
--   1. THE CLOSED LIST. Every key of the patch is checked against what this door stores,
--      before anything is read or written, and an unknown key is refused BY NAME with the
--      list — the same sentence and the same SQLSTATE `custom.entity_field_update` uses.
--   2. THE THREE ORPHANS ARE STORED. `expr` changes the formula of a column that is already
--      worked out (and is refused by name on a column that is not); `applies_to_types` and
--      `options_table_id` are written in the settings arm as they always were in the other.
--      A formula that names a column which does not exist is still refused by FIX-10B-F5's
--      guard, which judges `config.expr` as the Field document lands.
--
-- Nothing else moves. Every existing arm is the bytes that were already there.
--
-- based-on: custom.field_update(uuid, uuid, jsonb) 5d317e5dcee7ad98b11a78f0db3d65a0faf8a158a40c493cee4dc863e59bc9bf
