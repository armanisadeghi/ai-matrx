...
-- removing the door.
--
-- platform.org_module_config -- CONFIG SEMANTICS.
-- Which modules an organization has switched on. A behavioural choice, and therefore a knob -- knobs are written through the knob doors.
--
-- WHY IT MATTERS: Module configuration decides what a whole organization can see and do; it is a settings decision, and settings decisions go through the ladder, not through a REST write.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("org_module_config")` search AND a
-- bare-name search of `"org_module_config"` / 'org_module_config' / `org_module_config` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.org_module_config` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_org_module_config_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "org_module_config_client_insert_refused" on platform.org_module_config
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "org_module_config_client_update_refused" on platform.org_module_config
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "org_module_config_client_delete_refused" on platform.org_module_config
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "org_module_config_client_insert_refused" on platform.org_module_config is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). CONFIG semantics: Module configuration decides what a whole organization can see and do; it is a settings decision, and settings decisions go through the ladder, not through a REST write. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "org_module_config_client_update_refused" on platform.org_module_config is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). CONFIG semantics: Module configuration decides what a whole organization can see and do; it is a settings decision, and settings decisions go through the ladder, not through a REST write. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "org_module_config_client_delete_refused" on platform.org_module_config is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). CONFIG semantics: Module configuration decides what a whole organization can see and do; it is a settings decision, and settings decisions go through the ladder, not through a REST write. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
