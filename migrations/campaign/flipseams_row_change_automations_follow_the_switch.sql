-- chair-step: lane FLIP-SEAMS (follow-up D8 from CUTOVER-CENSUS) — the Data tables switch carries "when a row changes, run an agent" automations with it. platform._cutover_seam_apply re-keys every event automation on an archived older table to its copy (record:<table>, row.* -> record.*, row.deleted -> record.archived), keeping its config before on the press and on the automation; Switch back puts each back exactly. platform._cutover_seam_readiness holds the switch back, named, while an automation watches "any older table" (no table to follow) or an active outbound webhook listens for older row events (nothing to listen to after the switch).
-- based-on: platform._cutover_seam_apply(text, uuid, text, uuid, uuid) d98e357df5c4378c0c99f9234c748a44aab18056763106affccce4fc303f5300
-- based-on: platform._cutover_seam_readiness(text, uuid) b167d2762687533ae1a4dd409425435e2ecc3f1e1340f096eea055e4d6a1fcac
-- lane: FLIP-SEAMS
-- INVERSE: migrations/inverse/flipseams_row_change_automations_follow_the_switch_down.sql
