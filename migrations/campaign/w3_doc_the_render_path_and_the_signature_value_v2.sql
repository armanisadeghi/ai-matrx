...
--
-- THIS FILE SUPERSEDES `w3_doc_the_render_path_and_the_signature_value.sql`. Byte-identical
-- except for one arm of `custom.doc_render_body`, fixed after this lane's own acceptance test
-- (`w3_doc_c43.sql` part H) reached it for the first time and found a real class defect:
-- `doc_sign` writes the signer's name onto the record's OWN signature Field (VAL-10 requires
-- the signature to be a Value on the record), and the original `doc_render_body` merged that
-- Field's live value into the body like any other token. Because the body is what gets
-- HASHED and SEALED, a document holding its own signature token seals a version that cannot
-- exist yet at render time and is immediately falsified the instant it IS signed — every
-- signed document read as `broken` one statement after `doc_sign` returned, before a human
-- ever touched the record again. FIXED: a Field whose format is `signature` renders BLANK in
-- the merge, exactly as DocuSign does not bake the signature image into the same content
-- stream its certificate hashes — the signature is carried separately, as the Value and the
-- seal, and the document body a seal binds to never depends on it. Superseding filename for
-- the same reason as `w3_doc_the_template_is_a_record_v2.sql`: the live body is this lane's
-- own and a replacement would need a `-- based-on:` this branch cannot generate against
-- production, where the function has never existed.
--
