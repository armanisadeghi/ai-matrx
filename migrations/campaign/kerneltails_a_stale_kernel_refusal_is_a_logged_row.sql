-- chair-step: lane KERNEL-TAILS (tail 1, the guard). A provisioner refusal for a STALE ACCESS-KERNEL FINGERPRINT is a logged row, never a silent refusal. platform.provision and platform.provision_batch raised 'PREFLIGHT REFUSED' on preflight.read_kernel, and a raise rolls back the caller's whole transaction, so the refusal left nothing behind: SHARE-LANE-2's first file (20:38Z) and rca2b (21:51Z) each refused every spec on production (26 and 57 minutes) and nobody saw it until a lane ran check:store-doors-decide. Now, when the preflight's findings include preflight.read_kernel, the builder writes ONE ops.system_error row per refused spec (kind provisioner_fingerprint_stale, error_type preflight.read_kernel, source_app database, source_feature provisioning) naming the moved kernel body or bodies and the remedy, raises a WARNING with the same words, and RETURNS {ok:false, refused:true, rule_id, kind, message, hint, findings, system_error_id} instead of raising — nothing else has been written at that point (the run claim is an advisory lock). Every other preflight finding still raises exactly as before. To NAME the body, the member md5s the fingerprint covers are recorded beside it: iam.entity_read_kernel_members_expected() (recorded now, the same sixteen names iam.entity_read_kernel_fingerprint() hashes, 20 overloads, carrying the fingerprint they belong to) against iam.entity_read_kernel_members_live(). REPLACES platform.provision and platform.provision_batch (the only change in each: the eleven-line branch before the raise). aidream's lane-B service turns the returned refusal into ProvisionRefused (same commit series). Proof: scripts/campaign-tests/kerneltails_stale_kernel_green.sql RED before this file (0 rows, the call raised), GREEN after (one row per refused spec, nothing provisioned). No data write.
-- based-on: platform.provision(jsonb, text, uuid, text) d01c450cc02c0843cd5684818ba51f5757f3ba93157b829b27bcf1f47695a60c
-- based-on: platform.provision_batch(jsonb, text, uuid, text) 66fed2a6e8a45e379ed0772933bcf1002a6ec824acbab951e53defb0fbc56423
-- lane: KERNEL-TAILS
-- INVERSE: migrations/inverse/kerneltails_a_stale_kernel_refusal_is_a_logged_row_down.sql
set local lock_timeout = '30s';

-- THE MEMBERS THE FINGERPRINT COVERS, LIVE. The same sixteen (schema, name) pairs
-- iam.entity_read_kernel_fingerprint() hashes (and aidream scripts/check_db_guards.py lists).
-- search_path public, pg_catalog: the member KEYS are pg_get_function_identity_arguments text,
-- which qualifies a type only when it is not on the path — the same spelling aidream's snapshot uses.
...
-- THE MEMBERS AS RECORDED, with the fingerprint they belong to. A lane that re-records
-- iam.entity_read_kernel_expected() replaces this in the SAME file with
-- jsonb_build_object('fingerprint', <new>, 'members', iam.entity_read_kernel_members_live()).
CREATE OR REPLACE FUNCTION iam.entity_read_kernel_members_expected()
 RETURNS jsonb
 LANGUAGE sql
 IMMUTABLE
AS $function$
  SELECT '{"fingerprint": "181368ebc712dc15dc8c07e1b8f1974e", "members": {"files.crawl_site_conveys(p_user_id uuid, p_file_id uuid)": "5fadac4e0d1ad31e788cdb446422d8fc", "files.has_access_for(p_user_id uuid, p_file_id uuid, p_required permission_level)": "d324b5143d4172b0a6b8b8188930ff7b", "files.is_crawl_artifact(p_file_id uuid)": "7eb586213cedff72ee4abb4dd60a0433", "iam.accessible_entity_ids(p_type text, p_required permission_level, p_depth integer)": "9fe155aa00093efd6fc9c89ab94b8479", "iam.accessible_entity_ids(p_type text, p_required permission_level, p_depth integer, p_include_public boolean)": "db927981b7f146579baead6f11e2a22c", "iam.has_access_for(p_user_id uuid, p_type text, p_id uuid, p_required permission_level)": "c7e2eec401c991f06be4bf28453548e5", "iam.has_access_for_base(p_user_id uuid, p_type text, p_id uuid, p_required permission_level)": "e37fdacb359b9a528d7aef6b2bfb5270", "iam.has_access_for_base(p_user_id uuid, p_type text, p_id uuid, p_required permission_level, p_include_public boolean)": "e6b147f6962003e0dc2c8b826ef4ee06", "iam.has_access_for_base(p_user_id uuid, p_type text, p_id uuid, p_required permission_level, p_include_public boolean, p_path text[])": "6ac2a3205822b7d4eaae176315ab4454", "iam.has_org_access_for(p_user_id uuid, p_org uuid)": "ca4adbe46e1778b5c2ef8bad18ee0d7a", "platform.entity_row_access_attrs(p_schema text, p_table text, p_id uuid, OUT o_vis platform.visibility, OUT o_owner uuid, OUT o_org uuid, OUT o_found boolean)": "4cab0999cad6cee27fa4c6804d2f0955", "public._edu_can_read_via_assignment(p_type text, p_id uuid)": "d97bbb3323238c5b8afb88e3e6337434", "public._edu_can_read_via_assignment(p_user_id uuid, p_type text, p_id uuid)": "a0d7ac13ea23ec81b8eb15bbb87e3cbb", "public.has_permission_for(p_user_id uuid, p_resource_type text, p_resource_id uuid, p_required_permission permission_level)": "9dc1eecf01de31f4db0b0e07b1665a2b", "public.is_org_admin_for(p_user_id uuid, p_org_id uuid)": "d130a594ece79535bc39f8da5abadc57", "public.is_pack_curator(p_user uuid, p_pack_id uuid)": "5e6f2b3c9c4f0f9011655974ef1532b7", "public.is_rulebook_curator(p_user uuid, p_rulebook_id uuid)": "b781c4c0210974d680f53a603cb723aa", "public.library_is_open(p_entity_type text, p_entity_id uuid)": "36c934bb956df459e334c15085aacd30", "public.user_can_read_data_store_via_grant(p_user uuid, p_store uuid)": "63b3fd7f798351c9c8e7517fcfedc3fc", "public.user_can_read_via_library_grant(p_user uuid, p_type text, p_id uuid)": "a49b44fa2f0de5d3aecace9d950f49e4"}}'::jsonb
$function$;

CREATE OR REPLACE FUNCTION platform._provisioner_refuses_a_stale_kernel(
  p_spec jsonb, p_pre jsonb, p_applied_via text, p_org_id uuid, p_lane text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
revoke all on function iam.entity_read_kernel_members_live() from public, anon, authenticated;
revoke all on function iam.entity_read_kernel_members_expected() from public, anon, authenticated;
revoke all on function platform._provisioner_refuses_a_stale_kernel(jsonb, jsonb, text, uuid, text) from public, anon, authenticated;
...
do $g$
begin
  if (iam.entity_read_kernel_members_expected()->'members') is distinct from iam.entity_read_kernel_members_live() then
    raise exception 'kerneltails: the recorded kernel member snapshot does not match the live bodies — re-derive the file.';
  end if;
  if (iam.entity_read_kernel_members_expected()->>'fingerprint') is distinct from iam.entity_read_kernel_expected()
     or iam.entity_read_kernel_fingerprint() is distinct from iam.entity_read_kernel_expected() then
    raise exception 'kerneltails: the snapshot, the recorded fingerprint and the live fingerprint disagree (% / % / %).',
      iam.entity_read_kernel_members_expected()->>'fingerprint', iam.entity_read_kernel_expected(), iam.entity_read_kernel_fingerprint();
  end if;
end $g$;
