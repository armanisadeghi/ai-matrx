-- chair-step: the production judge refuses the rehearsed `w1_val_the_value_envelope.sql` with `guard-unread` — it replaces custom._record_field_validation(), a TRIGGER function bound to the live custom.record, and never names custom/system_enabled. The refusal is right, and the campaign already knew it: `w1_val_validation_reads_its_switch.sql` is the follow-up that makes exactly that trigger read the switch. This file is the TWO OF THEM JOINED, so the trigger function is never once installed without its switch, plus `drop ... if exists` for the campaign's own objects so one set of bytes runs on the copy and on the main database. The drops are why this is a chair step: the allow-list refuses every DROP by name, and correctly, because a blacklist cannot tell a campaign recreating its own function in one transaction from a live path being taken away. A person reads the whole file.
--
-- LAND — THE VALUE ENVELOPE AND ITS SWITCH, AS ONE FILE.
--
-- The rehearsed bytes of w1_val_the_value_envelope.sql (copy, 2026-09-17 17:30 UTC) followed
-- by those of w1_val_validation_reads_its_switch.sql (18:50 UTC), in that order. Nothing is
-- added but the drops and the one trigger re-creation, and no SQL semantics are changed.

set lock_timeout = '5s';
set statement_timeout = '600s';
-- The drops above mean a function may be created before one it calls: the copy had them all
-- standing, the main database is mid-build, and one order cannot suit both. Body checking is
-- off for THIS transaction only; every object is created before the transaction commits.
set check_function_bodies = off;

-- ── the drops that make these bytes re-runnable on BOTH databases ────────────
-- Every object below is this campaign's own, created in schema `custom` — declared CLOSED
-- to PUBLIC, anon, authenticated and service_role — within the last hour. Dropping and
-- recreating them inside ONE transaction is what lets the SAME bytes run on the rehearsal
-- copy (where they already stand, with bodies later files moved) and on the main database
-- (where most do not yet exist), so the two end identical. It is also why every `create or
-- replace` below is written `create`: after the drop there is nothing to replace, and a
-- `create or replace` of a live body would need a `-- based-on:` hash, which cannot be one
-- value for two databases that are deliberately at different points of this build.
...
drop function if exists custom.value_envelope_refusal;
drop function if exists custom.value_envelope_ok;
drop function if exists custom.source_pointer;
drop function if exists custom.source_next_pointer;
drop function if exists custom.intern_provenance;
drop function if exists custom.stamp_value_envelopes;
drop function if exists custom.value_versions;
drop function if exists custom.actor_word;
drop function if exists custom._value_envelope;
drop function if exists custom.record_values_versioned;
drop function if exists custom.value_read;
drop function if exists custom.validate_value_envelope;
drop function if exists custom._record_field_validation;
drop function if exists custom.record_values;
drop function if exists custom.store_is_open;
...
create function custom._record_field_validation()
 returns trigger
 language plpgsql
...
-- The follow-up replaces the body the section above installed. After the drop there is
-- nothing to replace, which is what keeps this file free of a `-- based-on:` hash that
-- could only ever be right for one of the two databases.
drop function if exists custom._record_field_validation;
...
-- ── the validation trigger, put back on custom.record ────────────────────────
-- Dropped above only so `custom._record_field_validation()` could be dropped with it; this
-- is `pg_get_triggerdef` of the live trigger, read from the main database before the drop.
CREATE TRIGGER custom_record_field_validation BEFORE INSERT OR UPDATE ON custom.record FOR EACH ROW EXECUTE FUNCTION custom._record_field_validation();
