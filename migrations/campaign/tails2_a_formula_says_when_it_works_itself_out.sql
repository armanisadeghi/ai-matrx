...
-- based-on: custom.field_update(uuid, uuid, jsonb) 95823882c8401a329099a881ed1800ba183cb0d4077ae66b97fec011aae5e10f
--
-- ══════════════════════════════════════════════════════════════════════════════════════
-- A FORMULA SAYS WHEN IT WORKS ITSELF OUT, AND THE DOOR STOPS THROWING THAT AWAY.
-- lane TAILS-2 · 2026-09-21
--
-- Lane SHARE-OUT left this behind, named rather than silently dropped: *"`custom.field_update`
-- cannot say WHEN a field works itself out. Its spec builder uses a fixed key list that does
-- not carry `compute_on`, so a person declaring a write-stamped formula through that door
-- silently gets a read-time one."*
--
-- MEASURED on the main database before this file, from the seat `authenticated`: declare a
-- formula column, call `custom.field_update(org, field, '{"compute_on":"write"}')`, read the
-- Field back — `"compute_on": "read"`. The door returned the field id and reported success.
--
-- THE ANSWER IS TO APPLY IT. Both halves of `compute_on` are already real and already work:
-- `custom._derived_fields` stamps a `write` formula into `_derived` on every save, and
-- `custom.derived_values_of` works a `read` one out on every read. Nothing had to be built
-- for the word to mean something — the door simply never carried it. So it carries it now,
-- in BOTH arms: the behaviour arm's spec (a retype that also says compute_on) and a settings
-- arm of its own (compute_on alone).
--
-- THE TWO CASES THAT CANNOT BE APPLIED ARE REFUSED BY NAME, WITH THE WAY TO CHANGE THEM,
-- because a refusal a person can act on is worth more than a setting quietly forced:
--   · a column that is not worked out by the store has no moment to be worked out at — the
--     refusal says to make it a worked-out column first, through this same door;
--   · a ROLL-UP adds up OTHER records, so a number stamped when this record was last saved
--     would be wrong the moment one of them changed (FLD-11). `custom._field_document_for`
--     has always forced it to `read`; forcing it silently is the same defect wearing the
--     fix's clothes, so the door now says so.
--
-- AND THE ANSWERS ALREADY OUT THERE MOVE WITH THE SETTING. `_derived` is written by the save
-- path and by nothing else. A column switched to `write` would therefore hold no stamped
-- answer on ANY existing record until each one happened to be saved again — empty on every
-- old row, full on every new one, with nothing on the screen saying why — and a column
-- switched back to `read` would leave a stale stamp that `custom.computed_provenance` keeps
-- reporting as a fact. Both are closed here, through the ordinary write path, so every guard
-- and every history row sees it exactly as it sees a save. The door says how many moved.
-- ══════════════════════════════════════════════════════════════════════════════════════
