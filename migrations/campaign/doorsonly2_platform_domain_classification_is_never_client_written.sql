...
-- removing the door.
--
-- platform.domain_classification -- VOCABULARY SEMANTICS.
-- The shared domain-classification vocabulary.
--
-- WHY IT MATTERS: A member who writes a shared vocabulary changes words every organization on the platform sees.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("domain_classification")` search AND a
-- bare-name search of `"domain_classification"` / 'domain_classification' / `domain_classification` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.domain_classification` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_domain_classification_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "domain_classification_client_insert_refused" on platform.domain_classification
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "domain_classification_client_update_refused" on platform.domain_classification
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "domain_classification_client_delete_refused" on platform.domain_classification
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "domain_classification_client_insert_refused" on platform.domain_classification is
...
comment on policy "domain_classification_client_update_refused" on platform.domain_classification is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). VOCABULARY semantics: A member who writes a shared vocabulary changes words every organization on the platform sees. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "domain_classification_client_delete_refused" on platform.domain_classification is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). VOCABULARY semantics: A member who writes a shared vocabulary changes words every organization on the platform sees. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
