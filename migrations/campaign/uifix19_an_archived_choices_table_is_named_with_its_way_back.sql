-- additive: yes
--
-- chair-step: it REPLACES one function body, `custom._field_shape_guard()` (the before-row
--   trigger that judges a Field record's shape). Only one refusal branch changes: when a list
--   field's choices table is missing, the guard now looks whether that table is this
--   organization's and ARCHIVED, and if so names it and says to bring it back first. Nothing
--   that was accepted is refused and nothing that was refused is accepted; only the words of one
--   refusal change. No table, trigger, grant or policy is created, dropped or altered, and no
--   row is written. The inverse is
--   `migrations/inverse/uifix19_an_archived_choices_table_is_named_with_its_way_back_down.sql`.
-- lock: custom
-- lane: UI-FIX-19
--
-- based-on: custom._field_shape_guard() 688b4c2bedbcf80b4475dcdced6750eefac931b0f639c2467a7faf035602df1c
--
-- UI-FIX-19 — A REFUSED RESTORE NAMES ITS FIX (VERIFIER-19 #4).
--
-- MEASURED on production, 2026-09-24: "Bring it back" on Rooms (admin's Workspace) refused with
-- `This could not be brought back as it was archived: "Status" is refused on its own (the list
-- field Status points at something that is not a table of this organization), so nothing was
-- brought back.` Rooms and its choice list "Status choices" were archived as two separate
-- archives. STORE-TAILS-3's `custom.record_restore` brings back everything ONE archive took, so
-- Rooms' restore brings back its Status field while Status choices is still archived, and this
-- guard, which asks only for a LIVE table, said the choices table is "not a table of this
-- organization". That is false and names no remedy. The class: a guard that reads only live rows
-- treats "archived" as "does not exist". The refusal now tells the two apart.
