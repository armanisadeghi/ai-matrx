-- chair-step: lane TRASH-COVERAGE-2. Trash's "(in <parent>)" title is computed after the page is cut, so it costs one parent lookup per listed row instead of one per candidate row: org_trash_list for the file kind in AI Matrx measured 890.7 ms (ceiling 300) after the first TRASH-COVERAGE-2 file put the lookup inside the sort.
--
-- Found by pnpm check:trash-answers-fast on production after
-- migrations/campaign/trashcoverage2_every_archivable_thing_a_person_sees_is_in_trash.sql (2 of 271
-- measurements over: org_trash_list file 890.7 ms, the merged organization page 412.0 ms). Same rows,
-- same titles, same order; only where the suffix is computed moves (an outer select over the limited page).
--
-- INVERSE: migrations/inverse/trashcoverage2_the_in_parent_title_is_computed_after_the_page_is_cut_down.sql
