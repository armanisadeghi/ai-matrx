-- additive: yes
--
-- chair-step: it REPLACES three existing HR notification producers —
--   `hr._wf_notify`, `hr._l1_notify_consent_requested` and `hr._punch_notify_edited` — with their
--   live bodies verbatim except for the ONE line in each that builds the deep link, and CREATES
--   `hr.link_names_its_employer`, and REPLACES `hr.hr_links_without_employer` so the repo's own
--   census recognises that one builder, and DECLARES the three producers' access doors in
--   platform.client_callable_door (rows in our own registry, nothing granted or revoked). Nothing is dropped, nothing is revoked, no table is altered. The
--   one data statement is an idempotent UPDATE that adds a missing `org=` to an `/hr` notice link
--   and can only ever run on rows that are already wrong. The inverse is
--   `migrations/inverse/links2_an_hr_link_names_its_employer_down.sql`.
--
-- based-on: hr.hr_links_without_employer() f7eac8b3b806c173b1ab4a49114bd13ea4fa8706af7fde7da14cc0d3d2702549
-- based-on: hr._wf_notify(uuid, uuid, text, text, uuid, uuid, jsonb) 8ed7f03f855eced28c375d04592d4c1f114d4f6e5accffb3f515eaee17923900
-- based-on: hr._l1_notify_consent_requested(uuid) 03688336ca2af8489010ef95c9df6ed19e8ef8acc071e453103705ad4d96f0e8
-- based-on: hr._punch_notify_edited(uuid, uuid, uuid, uuid, text, uuid, jsonb) 7a43249d7c45d420f464f827fa23b780b3bd72ce63609fd596ea625a4d4efe0f
--
-- LINKS-2 — AN HR NOTICE LINK NAMES ITS EMPLOYER THROUGH THE ONE RULE, NOT BY HAND.
--
-- TAILS-4 put the rule where every notice arrives: `platform.link_carries_its_organization`,
-- called by a BEFORE trigger on `communication.notification`. It then left three pre-`?org=`
-- HR files behind, "stamped by the trigger at write time … but the templates themselves still
-- say nothing about an employer" — `migrations/hr_c4_47_…sql:123`, `hr_l3_03a_…sql:87` and
-- `hr_l1_54_…sql:92` (a bare `/hr/me`).
--
-- WHAT THE CENSUS ACTUALLY FOUND, ON THIS DATABASE, 2026-09-21
-- ------------------------------------------------------------
-- `hr.hr_links_without_employer()` — the repo's own live census — returns ZERO rows, and
-- `communication.notification` holds 928 `/hr` links of which 0 are employer-free, as do all
-- 161 `/hr` `deep_link_template` rows in `communication.notification_event_type`. The three
-- functions those files created were re-written by later lanes and now name the employer. So
-- the employer-free defect is closed live and there is nothing to backfill.
--
-- WHAT IS NOT CLOSED, AND IS THE REAL CLASS: all three live producers HAND-BUILD `?org=`.
-- 36 live sites do (the full census is in this lane's report). A hand-built `?org=` is a second
-- implementation of a rule that already exists — it cannot honour the off switch, it cannot
-- learn a new organization-free route, and it is exactly how 26 SQL sites came to write 535
-- employer-free links before 2026-08-28. This file converts the three the trigger's own lane
-- named. The other 33 are error-payload `door` hints and inbox RPC rows, a different surface,
-- and they are reported rather than swept in.
--
-- WHY A THIN HR WRAPPER RATHER THAN CALLING THE PLATFORM RULE DIRECTLY
-- --------------------------------------------------------------------
-- 🚨 `platform.link_carries_its_organization` reads the knob
-- `communication/notice_link_names_its_organization` on every call and returns the link
-- UNCHANGED when it is off. That is right for a general notice link. It is NOT right for `/hr`:
-- HR is strictly single-employer, SPEC-UI-IA §1 resolves the active employer from `?org=`
-- FIRST, and `services/notifications/service.py::_render_deep_link` DROPS an employer-free
-- `/hr` link outright. Turning a general presentation knob off must not silently take HR's
-- links away. So `hr.link_names_its_employer` delegates to the one rule and then holds `/hr`'s
-- own floor — announcing itself with a `warning` when it has to, never silently.
--
-- There is still exactly ONE place that decides what stamping means. The floor is one
-- function, three callers, and it is the only place in HR that may write `org=` into a link.

set local statement_timeout = '600s';
set local lock_timeout = '5s';

-- ═══ 1. THE /hr FLOOR, ON TOP OF THE ONE RULE ════════════════════════════════
create or replace function hr.link_names_its_employer(p_link text, p_organization_id uuid)
returns text
language plpgsql
stable
set search_path to 'pg_catalog', 'platform', 'hr'
as $fn$
declare
  v_link     text;
  v_fragment text := '';
  v_i        integer;
begin
  -- THE ONE RULE FIRST, ALWAYS. Route list, already-named check, absolute-URL check,
  -- null handling and the off switch all live there and are not repeated here.
  v_link := platform.link_carries_its_organization(p_link, p_organization_id);

  if p_organization_id is null or v_link is null or btrim(v_link) = '' then
    return v_link;
  end if;

  -- Only an in-app `/hr` path has this floor. `/onboard`, `/apply`, `/sign` and `/records`
  -- are the outsider token routes and are deliberately employer-free; an absolute URL
  ...
  if v_link ~ '[?&]org=' then
    return v_link;   -- the rule did its job, or the caller already named one
  end if;

  -- IT GOT HERE, SO THE RULE DECLINED. The only ways that happens for an `/hr` path are the
  -- knob being off or `/hr` being added to the organization-free list. Either is a decision
  -- about presentation; neither is a decision that HR may open the wrong employer's data.
  raise warning
    'hr.link_names_its_employer: platform.link_carries_its_organization returned the HR link % '
    'with no employer (the knob communication/notice_link_names_its_organization is off, or /hr '
    'reached platform.organization_free_link_prefixes). HR is strictly single-employer and an '
    'employer-free /hr link opens whichever employer the picker holds, so the employer was named '
    'anyway. Turn the knob back on, or take /hr off that list.', v_link;

  v_i := position('#' in v_link);
  if v_i > 0 then
    v_fragment := substr(v_link, v_i);
    v_link := left(v_link, v_i - 1);
  end if;

  return v_link
       || case when position('?' in v_link) > 0 then '&' else '?' end
       || 'org=' || p_organization_id::text
       || v_fragment;
end;
$fn$;

comment on function hr.link_names_its_employer(text, uuid) is
  'THE only place in HR that may put an employer into a link. Delegates to platform.link_carries_its_organization — the ONE rule — and then holds /hr''s own floor: HR is strictly single-employer, SPEC-UI-IA §1 reads ?org= first and the notification spine drops an employer-free /hr link, so a general presentation knob may not take HR''s employer away. When it has to name one the rule declined, it says so with a warning. (LINKS-2.)';

revoke all on function hr.link_names_its_employer(text, uuid) from public;

-- ═══ 2. THE THREE PRODUCERS TAILS-4 NAMED ════════════════════════════════════
-- Live bodies verbatim, one line changed in each. `-- based-on:` above is what proves that.
  ...
  v_ins integer := 0;   -- hr_l3_117: ROW_COUNT of the insert just made
   v_link text; v_payload jsonb;
    ...
    begin
      ...
      -- LINKS-2: §2.1's object route, handed to THE ONE builder. The employer is no longer
      -- concatenated here: hr.link_names_its_employer calls platform.link_carries_its_organization
      -- and holds /hr's own floor, so this line can never drift from the rule again.
      v_link := hr.link_names_its_employer('/hr/tasks/' || p_instance::text || coalesce('?step=' || p_step::text, ''), inst.organization_id);
...
begin
  ...
  loop
    ...
    values (v_org, 'hr.people.verification_consent_requested', v_uid, 'user', v_ch,
            ...
            v_addr, v_status, v_errcode, v_errmsg,
            -- LINKS-2: THE ONE builder, never a hand-built ?org=.
            'hr_verification_letter_request', p_request_id, hr.link_names_its_employer('/hr/me', v_org),
...
begin
  ...
  -- LINKS-2: THE ONE builder, never a hand-built ?org=.
  v_link := hr.link_names_its_employer('/hr/me/timesheet?punch=' || coalesce(p_replacement_punch_id, p_voided_punch_id)::text, p_organization_id);
...
-- ═══ 2c. THE DOORS THE THREE PRODUCERS OWE ═══════════════════════════════════
-- 🚨 These three are SECURITY DEFINER and were never declared, so replacing any of them trips
-- `provision_shape_guard` ("reached COMMIT with no access decision declared"). That debt is
-- real and it is paid here rather than worked around: none of them is a client door — live
-- `proacl` is `{postgres}` / `{postgres, service_role}` with no anon or authenticated grant,
-- and none of them checks a caller, because each is called from inside an HR write that has
-- already authorised the actor. Declaring them `server_only` is what is TRUE, and it is what
-- keeps the guard able to refuse the next undeclared definer.
insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, reason, declared_by,
   non_client_lane, signed_in_callers, anonymous_callers)
values
  ('hr', '_wf_notify',
   'p_instance uuid, p_step uuid, p_event_key text, p_notice_kind text, p_user uuid, p_employment uuid, p_extra jsonb',
   array['uuid','uuid','text','text','uuid','uuid','jsonb']::regtype[]::oid[],
   'No argument is checked against the caller, because there is no caller to check: every entity id is read from a row the CALLING workflow step already authorised. p_instance is looked up in hr.workflow_instance and the organization comes from THAT row, never from an argument; p_step, p_user and p_employment are written onto the notice as given. NULL p_user returns 0 and a p_instance with no instance row returns 0 — neither raises and neither writes.',
   'links2_an_hr_link_names_its_employer.sql',
   'server_only: hr._wf_engine and the HR workflow steps call it inside the transaction that already decided the step, so the authorisation happened before this function was reached and a browser has nothing it could safely pass.',
   false, false),
  ('hr', '_l1_notify_consent_requested', 'p_request_id uuid',
   array['uuid']::regtype[]::oid[],
   'p_request_id is checked against hr.verification_letter_request (deleted_at is null); the organization, the employment and the recipient are all read from THAT row, so a caller cannot aim the notice at anybody. A request id that does not resolve returns 0, and an employment with no linked login returns 0 — no row, no raise.',
   'links2_an_hr_link_names_its_employer.sql',
   'server_only: public.hr_verification_request_create calls it after its own write gate has authorised the raise, so this function is the tail of a decision rather than a decision of its own.',
   false, false),
  ('hr', '_punch_notify_edited',
   'p_organization_id uuid, p_employment_id uuid, p_voided_punch_id uuid, p_replacement_punch_id uuid, p_reason text, p_actor_user uuid, p_change jsonb',
   array['uuid','uuid','uuid','uuid','text','uuid','jsonb']::regtype[]::oid[],
   'p_employment_id is resolved through hr.employment to the employee''s login, and that login — never an argument — is the recipient. p_organization_id is written onto the notice as given and is the organization the calling punch-edit door already proved it was acting in. An employment with no linked login returns an envelope saying it reached nobody; a NULL p_reason becomes the truthful default sentence. Nothing here raises.',
   'links2_an_hr_link_names_its_employer.sql',
   'server_only: the punch correct and punch void doors call it after they have written the punch, so the actor was authorised by the door and a client would only be able to forge who was told what.',
   false, false)
on conflict (schema_name, function_name, identity_argtypes) do nothing;

-- ═══ 2b. THE REPO'S OWN CENSUS LEARNS THE ONE BUILDER ════════════════════════
-- Live body verbatim, one WHERE clause added. Without it the guard goes red on the only
-- correct way to build an HR link — and a guard that is red when the code is right is a guard
...
AS $function$
  ...
  select schema_name, function_name, line_no, btrim(line)
   ...
   -- 🚨 LINKS-2: A LINE THAT HANDS ITS PATH TO THE ONE BUILDER IS COMPLIANT BY CONSTRUCTION.
   -- `hr.link_names_its_employer` delegates to platform.link_carries_its_organization and then
   -- holds /hr's floor, so its argument CANNOT reach a reader without an employer — while the
   -- literal it is handed, read on its own, looks exactly like the banned shape. Without this
   -- clause the guard goes red on the only correct way to build an HR link, which is how a
   -- guard gets switched off. It stays narrow: the builder call must be on the SAME line as the
   -- literal, so nothing is exempted by proximity, and a hand-built `/hr` path anywhere else in
   -- the same function is still named.
   where line !~ 'org=' and line !~ 'hr-url-exempt:' and line !~ 'link_names_its_employer'
   order by schema_name, function_name, line_no;
$function$;

-- ═══ 3. THE LIVE ROWS ════════════════════════════════════════════════════════
-- Idempotent and self-limiting: it can only touch a notice whose `/hr` link has no employer,
-- and there are none today (928 `/hr` links, 0 employer-free — measured before this file ran).
-- It is here anyway because "the defect is closed in effect" is a measurement, not a mechanism,
-- and a file that only fixes the code leaves a future row with no owner. `payload.deep_link` is
-- included because the BEFORE trigger only ever touched the COLUMN, and two of these three
-- producers write the link into both.
do $rows$
declare
  v_try   integer := 0;
  v_col   integer := 0;
  ...
  loop
    ...
    begin
      ...
      update communication.notification n
         set deep_link = hr.link_names_its_employer(n.deep_link, n.organization_id)
      ...
      update communication.notification n
         set payload = n.payload
                    || jsonb_build_object('deep_link',
                         hr.link_names_its_employer(n.payload ->> 'deep_link', n.organization_id))
       where n.organization_id is not null
         and n.payload ->> 'deep_link' ~ '^/hr(/|\?|#|$)'
         and (n.payload ->> 'deep_link') !~ '[?&]org=';
      get diagnostics v_pay = row_count;

      exit;
    exception when lock_not_available then
      if v_try >= 5 then raise; end if;
      raise notice 'links2: lock_timeout on communication.notification, retry % of 5', v_try;
      perform pg_sleep(2 * v_try);
    end;
  end loop;

  raise notice 'links2: employer named on % notice deep_link column rows and % payload copies',
               v_col, v_pay;
end
$rows$;

-- ═══ 4. THE FILE PROVES ITSELF ═══════════════════════════════════════════════
do $assert$
declare v_bad text; v_n integer;
begin
  -- The three producers now route through the one builder, and none of them concatenates org=.
  ...
  if v_bad is not null then
    raise exception 'links2: these producers do not call the one builder: %', v_bad;
  end if;
  ...
  select count(*) into v_n
   ...
   where n.nspname = 'hr'
     and p.proname in ('_wf_notify', '_l1_notify_consent_requested', '_punch_notify_edited')
     and l.t like '%' || chr(39) || '/hr%'
     and l.t ~ 'org=';
  if v_n <> 0 then
    raise exception 'links2: % hand-built ?org= sites remain in the three producers', v_n;
  end if;

  -- The builder itself, on the shapes the three producers hand it.
  if hr.link_names_its_employer('/hr/tasks/abc?step=def', '39c38960-d30c-4840-b0c1-c9960de95582')
     !~ '[?&]org=39c38960-d30c-4840-b0c1-c9960de95582$' then
    raise exception 'links2: the builder did not name the employer on an /hr/tasks link';
  end if;
  if hr.link_names_its_employer('/hr/me', '39c38960-d30c-4840-b0c1-c9960de95582')
     <> '/hr/me?org=39c38960-d30c-4840-b0c1-c9960de95582' then
    raise exception 'links2: the builder did not name the employer on a bare /hr/me';
  end if;
  -- THE CONTROL, the other way: a declared organization-free route is still left alone.
  if hr.link_names_its_employer('/notifications', '39c38960-d30c-4840-b0c1-c9960de95582')
     <> '/notifications' then
    raise exception 'links2: the builder stamped a declared organization-free route';
  end if;
  -- And a link that already names one is never given a second.
  if hr.link_names_its_employer('/hr/me?org=11111111-1111-1111-1111-111111111111',
                                '39c38960-d30c-4840-b0c1-c9960de95582')
     <> '/hr/me?org=11111111-1111-1111-1111-111111111111' then
    raise exception 'links2: the builder overwrote an employer the caller already named';
  end if;

  -- And the live census the repo already keeps stays at zero.
  select count(*) into v_n from hr.hr_links_without_employer();
  if v_n <> 0 then
    raise exception 'links2: hr.hr_links_without_employer() returned % rows', v_n;
  end if;

  raise notice 'links2: the three producers build through hr.link_names_its_employer; census 0';
end
$assert$;
