...
-- removing the door.
--
-- platform.lifecycle_entity_plan -- LIFECYCLE SEMANTICS.
-- The per-entity archive/restore plan the lifecycle engine executes.
--
-- WHY IT MATTERS: A member who edits a lifecycle plan changes what happens to other organizations' rows when they are archived.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("lifecycle_entity_plan")` search AND a
-- bare-name search of `"lifecycle_entity_plan"` / 'lifecycle_entity_plan' / `lifecycle_entity_plan` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.lifecycle_entity_plan` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_lifecycle_entity_plan_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "lifecycle_entity_plan_client_insert_refused" on platform.lifecycle_entity_plan
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "lifecycle_entity_plan_client_update_refused" on platform.lifecycle_entity_plan
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "lifecycle_entity_plan_client_delete_refused" on platform.lifecycle_entity_plan
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "lifecycle_entity_plan_client_insert_refused" on platform.lifecycle_entity_plan is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). LIFECYCLE semantics: A member who edits a lifecycle plan changes what happens to other organizations'' rows when they are archived. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "lifecycle_entity_plan_client_update_refused" on platform.lifecycle_entity_plan is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). LIFECYCLE semantics: A member who edits a lifecycle plan changes what happens to other organizations'' rows when they are archived. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "lifecycle_entity_plan_client_delete_refused" on platform.lifecycle_entity_plan is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). LIFECYCLE semantics: A member who edits a lifecycle plan changes what happens to other organizations'' rows when they are archived. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
