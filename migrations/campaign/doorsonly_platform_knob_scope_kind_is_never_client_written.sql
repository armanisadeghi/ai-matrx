...
--
-- platform.knob_scope_kind -- KNOB SEMANTICS.
-- The registry of the scope rungs configuration can be set at (org, sub-org, user). Zero mentions in any client repository outside generated entity-type files. The `platform.knob_*` door family is what the app calls.
--
-- WHY IT MATTERS: A member who writes this registry changes which rung every knob in the platform resolves at.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.knob_scope_kind` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_knob_scope_kind_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "knob_scope_kind_client_insert_refused" on platform.knob_scope_kind
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "knob_scope_kind_client_update_refused" on platform.knob_scope_kind
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "knob_scope_kind_client_delete_refused" on platform.knob_scope_kind
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "knob_scope_kind_client_insert_refused" on platform.knob_scope_kind is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). KNOB semantics: A member who writes this registry changes which rung every knob in the platform resolves at. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "knob_scope_kind_client_update_refused" on platform.knob_scope_kind is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). KNOB semantics: A member who writes this registry changes which rung every knob in the platform resolves at. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "knob_scope_kind_client_delete_refused" on platform.knob_scope_kind is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). KNOB semantics: A member who writes this registry changes which rung every knob in the platform resolves at. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
