...
create policy invitations_client_insert_refused on iam.invitations
  as restrictive for insert to authenticated, anon with check (false);
create policy invitations_client_update_refused on iam.invitations
  as restrictive for update to authenticated, anon using (false) with check (false);
create policy invitations_client_delete_refused on iam.invitations
  as restrictive for delete to authenticated, anon using (false);

comment on policy invitations_client_insert_refused on iam.invitations is
  'SECURITY-SWEEP 2026-09-21, chair ruling 1. The invitation token is what a stranger presents to join an organization. features/organizations/FEATURE.md already declared this table to have no direct client grant and the DATABASE disagreed with it (insert/update/delete were all true for authenticated). Writes go through the inv_* doors. RESTRICTIVE and bespoke on purpose, so iam.apply_rls preserves it.';
comment on policy invitations_client_update_refused on iam.invitations is
  'SECURITY-SWEEP 2026-09-21: an invitation is accepted, resent or revoked through the inv_* doors, never edited in place by a client.';
comment on policy invitations_client_delete_refused on iam.invitations is
  'SECURITY-SWEEP 2026-09-21: an invitation is revoked, never deleted by a client.';
