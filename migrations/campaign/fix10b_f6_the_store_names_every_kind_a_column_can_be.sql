-- FIX-10B-F6 — the store publishes EVERY kind a column can be, and `signature` is one of them.
--
-- WHAT A PERSON GETS: "Signature" in the add-a-column panel, so the sentence under a rendered
-- document — "To ask somebody to sign one of these, Crews needs a signature column" — names a
-- precondition they can actually meet, and the e-sign half of Documents becomes reachable.
--
-- THE CLASS, NOT THE INSTANCE (VERIFIER-10 F6, admin seat, Rincon Plumbing's Crews table).
-- The store had ONE published list of what a column can be — `custom.parity_field_types()`,
-- the fourteen-row Airtable/Notion parity FLOOR. Every kind that is not on that floor lived as
-- an inline literal inside `custom._field_document_for`: plain text, long text, number, a
-- relation a person aims themselves, and a signature. Nothing enumerated them, so every client
-- retyped its own list from reading the door's source, and a kind the store took but no list
-- named was invisible to every screen. `signature` is the one that cost: `custom.doc_sign` is
-- granted to `authenticated`, accepts exactly one field shape, and no screen could declare it.
--
-- THE FIX: `custom.field_kinds()` — the ONE registry, the parity floor plus the five kinds
-- that are not on it, each saying what it is made of. `@ai-matrx/records` generates its mirror
-- from it (`pnpm records:generate`), `@ai-matrx/records-ui` explains every row of that mirror
-- and its suite fails when one has no explanation, and this door answers to every word in it.
--
-- WHAT IT REPLACES: `custom._field_document_for`, body-for-body as it stands on this database
-- (see `-- based-on:`), with the `signature` word resolved beside the other kind words, the
-- plain-text arm reading it, and the "there is no kind of column called X" hint naming every
-- kind rather than only the parity floor. Nothing is removed: a caller that sends
-- `format: signature` with plain text is written exactly as before.
--
-- INVERSE: migrations/inverse/fix10b_f6_the_store_names_every_kind_a_column_can_be_down.sql
-- target: branch,production
-- additive: yes
-- guard: custom/system_enabled
-- based-on: custom._field_document_for(uuid, uuid, jsonb) f85e468fe4e0ffeb56a9ef44394c9054a417572bf06d5021ed246e45f196c5d0

-- ── THE ONE REGISTRY ────────────────────────────────────────────────────────────────────
-- Every kind a column can be DECLARED as, in one place, so no client has to read this door's
-- source to find out. The parity floor keeps its own function and its own meaning — it is the
...
declare
  ...
  v_total integer;
begin
  select array_agg(k.kind order by k.kind) into v_names from custom.field_kinds() k;
  v_total := coalesce(array_length(v_names, 1), 0);
  if v_total = 0 then
    return 'this store lists no kinds of column at all, which is itself the defect';
  end if;
  if v_total = 1 then
    return format('the only kind of column is %s', v_names[1]);
  end if;
  return format('the %s kinds of column are %s and %s',
                coalesce(c_words[v_total], v_total::text),
                array_to_string(v_names[1:v_total - 1], ', '),
                v_names[v_total]);
end;
$function$;
