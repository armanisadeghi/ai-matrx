...
-- removing the door.
--
-- platform._bak_assoc_type_file_processed_document_20260812 -- DEAD BACKUP SEMANTICS.
-- A dated backup of the association-type table, left behind by the 2026-08-12 association work. Nothing reads it and nothing writes it.
--
-- WHY IT MATTERS: The next lane should graveyard it rather than keep it doored -- a backup nobody reads is not a table, it is a liability.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("_bak_assoc_type_file_processed_document_20260812")` search AND a
-- bare-name search of `"_bak_assoc_type_file_processed_document_20260812"` / '_bak_assoc_type_file_processed_document_20260812' / `_bak_assoc_type_file_processed_document_20260812` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform._bak_assoc_type_file_processed_document_20260812` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform__bak_assoc_type_file_processed_document_20260812_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "_bak_assoc_type_file_processed_document_20260812_client_insert_refused" on platform._bak_assoc_type_file_processed_document_20260812
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "_bak_assoc_type_file_processed_document_20260812_client_update_refused" on platform._bak_assoc_type_file_processed_document_20260812
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "_bak_assoc_type_file_processed_document_20260812_client_delete_refused" on platform._bak_assoc_type_file_processed_document_20260812
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "_bak_assoc_type_file_processed_document_20260812_client_insert_refused" on platform._bak_assoc_type_file_processed_document_20260812 is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). DEAD BACKUP semantics: The next lane should graveyard it rather than keep it doored -- a backup nobody reads is not a table, it is a liability. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "_bak_assoc_type_file_processed_document_20260812_client_update_refused" on platform._bak_assoc_type_file_processed_document_20260812 is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). DEAD BACKUP semantics: The next lane should graveyard it rather than keep it doored -- a backup nobody reads is not a table, it is a liability. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "_bak_assoc_type_file_processed_document_20260812_client_delete_refused" on platform._bak_assoc_type_file_processed_document_20260812 is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). DEAD BACKUP semantics: The next lane should graveyard it rather than keep it doored -- a backup nobody reads is not a table, it is a liability. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
