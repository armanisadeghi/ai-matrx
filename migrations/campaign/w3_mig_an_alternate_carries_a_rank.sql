...
-- based-on: custom.migrate_merge(uuid,uuid,uuid,text) 5383bb3a6c11332419e98752c14666556f7a51c886480e2572dc431b7bd069c3
--
-- W3-MIG, part three — THE MERGE COULD NOT WRITE ITS OWN ALTERNATE.
--
-- MEASURED (branch, 2026-09-18, scripts/campaign-tests/w3_mig_c18.sql PART 5):
--   ERROR: phone: every alternate carries a whole rank of 1 or more — the order its source
--          is trusted in.
--
-- W1-VAL's envelope law (VAL-3 / VAL-4) says an alternate holds exactly `value`, `src` and
-- `rank`, that the rank is a whole number of 1 or more, and that two alternates never claim
-- one rank — because a rank is the ORDER an organization trusts its sources in, and a list
-- without one is a pile. `custom.migrate_merge` wrote `value` and `src` and no rank, so T5's
-- central sentence — "the two phone numbers are alternates inside the Person's single
-- document, each with its source" — could not execute at all. The verb raised on the first
-- disagreeing value it met.
--
-- THE FIX. The rank is the count of alternates already on that value plus one: the winner's
-- own value is the trusted one, the first record merged in ranks behind it, the second behind
-- that. It is the arrival order, which is the only thing a merge actually knows, and it is
-- never a score or a confidence invented for the occasion (VAL-4's own words).
--
-- AND THE SOURCE IS WRITTEN AS THE SOURCE, NOT AS A POINTER. `custom.intern_provenance` turns
-- an inline `{"kind":"record","id":"…"}` into `s1` and files the description in `_sources`
-- (measured on the branch). A caller that wrote `s1` itself would be choosing a pointer the
-- store interns, which VAL-1 refuses by name.
--
-- BASED ON, VERIFIED: the body below is the one this file replaces, with only the alternate's
...
comment on function custom.migrate_merge(uuid, uuid, uuid, text) is
  'T5 / REC-21: two records become one. Values the winner lacks are taken; values that disagree become ALTERNATES inside the winner''s one document, each with a rank (arrival order — VAL-4 forbids an invented score) and each carrying the record it came from as a source the store interns. The losing id resolves to the winner forever, and the whole of the loser is stored as the inverse so undo restores both records and both ids.';
