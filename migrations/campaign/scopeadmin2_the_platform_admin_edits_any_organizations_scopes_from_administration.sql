-- chair-step: lane SCOPE-ADMIN-2. The platform-admin scope console (/administration/scopes-context/organizations/<id>) opens ANY organization's scope types and scopes — the admin is usually not a member. Reads already pass (every context table carries the platform_admin arm, which is live only on an admin-lane request). The six structural write doors refused a non-member admin (iam.has_org_access / an owner-or-admin membership), so the console listed rows it could not edit. Each door gains ONE arm, `public.is_platform_admin()`, which is true only for a platform admin on a request carrying the admin-lane header (platform.admin_lane_open()) — on every user page it is false, so a platform admin there is an ordinary member exactly as before. Nothing else in any body changes.
-- based-on: public.create_scope_type(uuid, text, text, uuid, text, text, smallint, smallint, text[], text, text) 5ea9cf24a34d4ca0a25a93c8d9568ea534635f1454ff0d99c228557d0b77ad07
...
-- based-on: public.create_scope(uuid, uuid, text, uuid, text, jsonb, text, smallint) 75adbddc92f707ac9b6d6d7dcb3e52f8ac20c15fe5dc411fa5bc28b29d9b0b15
-- based-on: public.update_scope(uuid, text, text, jsonb, text, smallint) a29171539b5c68787f331b241e48e63d5336b525f55cd450b8390493dff88d02
-- based-on: public.delete_scope(uuid) ace08a603e80ceff7c2e9251759abe4fa6aed14a4f9b9271d36ba6800887a2c7
--
-- THE ADMIN LANE (Arman, 2026-09-25): "Admin privileges cannot ever extend beyond the admin sections
-- of the system." The arm added here is the lane's own predicate, so it widens nothing outside
-- /administration/**. Champion: Google Workspace / Microsoft 365 admin consoles — an administrator
-- edits another tenant's directory objects from the admin console, never from the user's own app.
--
-- INVERSE: migrations/inverse/scopeadmin2_the_platform_admin_edits_any_organizations_scopes_from_administration_down.sql
-- lane: SCOPE-ADMIN-2
