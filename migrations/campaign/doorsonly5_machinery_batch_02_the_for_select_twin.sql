...
-- transaction.
--
-- THE MACHINERY TABLES, BATCH 2 OF 4 — 8 tables.
--
...
--
-- Inverse: migrations/inverse/doorsonly5_machinery_batch_02_the_for_select_twin.inverse.sql
-- Guard: `pnpm check:doors-only-schemas`, re-run live after THIS batch.

set local lock_timeout = '2s';


-- ── platform._base_entity ──
create policy "platform_admin_select" on platform."_base_entity"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."_base_entity" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform._base_entity` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."_base_entity";

-- ── platform.activity_log ──
create policy "platform_admin_select" on platform."activity_log"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."activity_log" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.activity_log` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."activity_log";

-- ── platform.assist_producer_policy_history ──
create policy "platform_admin_select" on platform."assist_producer_policy_history"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."assist_producer_policy_history" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.assist_producer_policy_history` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."assist_producer_policy_history";

-- ── platform.association_types ──
create policy "platform_admin_select" on platform."association_types"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."association_types" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.association_types` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."association_types";

-- ── platform.associations ──
create policy "platform_admin_select" on platform."associations"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."associations" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.associations` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."associations";

-- ── platform.ddl_guard_log ──
create policy "platform_admin_select" on platform."ddl_guard_log"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."ddl_guard_log" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.ddl_guard_log` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."ddl_guard_log";

-- ── platform.deprecated_relations ──
create policy "platform_admin_select" on platform."deprecated_relations"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."deprecated_relations" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.deprecated_relations` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."deprecated_relations";

-- ── platform.edge_payload_kind ──
create policy "platform_admin_select" on platform."edge_payload_kind"
  as permissive for select to authenticated
  using ((select public.is_platform_admin()));
comment on policy "platform_admin_select" on platform."edge_payload_kind" is
  'DOORS-ONLY-5 2026-09-22: the FOR SELECT twin of the platform_admin_all this file dropped. `platform.edge_payload_kind` is access MACHINERY -- iam.apply_rls refuses a machinery token by name, so nothing regenerates its policies and this hand-written pair is durable, which is the opposite of every generated table in this campaign. The predicate is byte-identical to the USING half of the policy it replaces, so platform-staff READS do not move; the write half is not re-emitted, because a FOR ALL permissive policy naming authenticated is the client write SURFACE the ruling closed. authenticated holds SELECT and no write privilege on this table, so nothing anybody can do today changes.';
drop policy "platform_admin_all" on platform."edge_payload_kind";
