-- target: branch
-- based-on: platform._provision_shape_settled() e54e90e014353b5076bfb03e6f9d09103a7eece5b0c34240e146742166c066b2
--
-- W1-V1-FIXES, FOUND OUTSIDE BRIEF AND FIXED BECAUSE IT BLOCKS EVERY LANE'S INVERSE
-- (rule 20: different class, blocks my exit -> fix it and name it prominently).
--
-- WHAT IS BROKEN ON THE BRANCH, MEASURED 2026-09-17
-- -------------------------------------------------
-- `platform._door_follows_its_function()` is an EVENT TRIGGER on `sql_drop`. When any
-- transaction drops a function in a schema that holds `platform.client_callable_door`
-- rows, it writes a `provision_shape_debt` row of kind `door_orphaned`. On the BRANCH the
-- CHECK constraint on that table allows only three kinds, so the insert raises
--
--     SQLSTATE 23514  new row for relation "provision_shape_debt"
--                     violates check constraint "provision_shape_debt_kind_check"
--
-- and the WHOLE transaction dies. Reproduced by this lane at 20:35Z: the inverse of
-- `w1_v1_fixes_one_door_predicate.sql` (three `drop function` statements in schema
-- `custom`, which holds eleven door rows) failed with exactly that, so rule 27's
-- "the inverse was RUN on the branch" was unreachable for this lane and for every later
-- lane whose inverse drops a function in `custom`, `platform` or `iam`.
--
-- IT IS NOT A DESIGN QUESTION - IT IS BRANCH DRIFT (rule 36)
-- ----------------------------------------------------------
-- PRODUCTION IS ALREADY CORRECT, read SELECT-only inside `begin transaction read only`
-- on 2026-09-17 (system_identifier 7642734024280108049):
--
--   · `provision_shape_debt_kind_check` on production reads
--     CHECK (kind = ANY (ARRAY['definer_no_door','fk_without_index','nullable_tenant_fk','door_orphaned']))
--     - the branch's is the same list WITHOUT `door_orphaned`.
--   · `platform._provision_shape_settled()` on production carries the whole
--     `elsif new.kind = 'door_orphaned'` arm (the guard that actually decides the debt at
--     COMMIT); the branch's body has no such arm, so even with the constraint widened the
--     debt would be recorded and never judged.
--   · `platform._door_follows_its_function()` is BYTE-IDENTICAL on both and is not touched.
--
-- So this file is a levelling file in rule 36's exact sense: `-- target: branch`,
-- catalog-derived, the body taken VERBATIM from production's own
-- `pg_get_functiondef`, applied through the runner. It changes production nothing,
-- because production already holds it. `pnpm check:branch-schema-drift` exits 0 today and
-- did not see this: it compares objects production HAS and the branch LACKS, and here
-- both objects exist on both sides - it is the CHECK CONSTRAINT list and the FUNCTION BODY
-- that differ. That blind spot is reported to the chair rather than fixed here.
--
-- NO INVERSE FILE. Its inverse is to re-narrow a CHECK constraint and to put back a body
-- that takes the whole transaction down - restoring a defect is not a down-migration. Rule
-- 27's clause for this file is that it is branch-only levelling toward production, it
...
--    place, so this is drop-and-add - which is why the file is branch-only (rule 9 keeps
--    a DROP off production in every lane, and production needs nothing).
alter table platform.provision_shape_debt
  drop constraint if exists provision_shape_debt_kind_check;
alter table platform.provision_shape_debt
  add constraint provision_shape_debt_kind_check
  check (kind = any (array['definer_no_door'::text, 'fk_without_index'::text,
                           'nullable_tenant_fk'::text, 'door_orphaned'::text]));

-- 2. THE SETTLE TRIGGER, VERBATIM FROM PRODUCTION'S CATALOGUE.
