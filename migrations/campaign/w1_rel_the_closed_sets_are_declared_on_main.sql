-- chair-step: the runner's additive allow-list names eight registry tables and
-- `platform.association_types` / `platform.edge_payload_kind` are not among them, so W1-REL's
-- own file carries `-- target: branch` and is refused at production for these two INSERTs
-- alone. Widening the judge is not a lane's call (W1-REL says so in the body below, and that
-- remains the real fix); the owner's 2026-09-18 ruling makes an additive row in one of our own
-- registry tables something that never stops, so the same bytes reach the main database here as
-- an attended step. Nothing is dropped, replaced or renamed: two `on conflict do nothing`
-- INSERTs, and with `custom/associations_guard` false nothing reads them yet.
--
-- The bytes below are `w1_rel_the_closed_sets_are_declared.sql` unchanged, header apart.
--
