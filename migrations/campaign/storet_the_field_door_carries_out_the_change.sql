...
-- based-on: custom._field_document_for(uuid, uuid, jsonb) 731696175967ec59ff7a887953e761532a4afd5a9f97f26bd79bdd6bd1440e10
-- based-on: custom.field_update(uuid, uuid, jsonb) c479faf4c3013a4a220f29274227e81e29ce072b25cff7b222a942298af834a1
--
-- STORE-T / T12 + T7 — THE FIELD DOOR CARRIES OUT THE CHANGE IT WAS ASKED FOR.
--
-- T12, MEASURED FROM A SIGNED-IN SEAT on 2026-09-20. A text column holding "abc" on one record
-- and "12" on another. `custom.field_update(org, field, {"plain":"number"})` returned the field
-- id — ok — and did nothing. `{"type":"range"}` returned ok and did nothing. Afterwards the
-- column still behaves as text, "abc" is still "abc", "12" is still the string "12", and
-- History has no row and no reason. The sixth pass's words: "The door said yes and nothing
-- happened."
--
-- WHY THE LANE THAT BUILT T12 SAW IT WORK. The conversion itself was built and IS correct:
-- `custom._field_type_converts_values` converts what converts, keeps what does not in
-- `_retired` with a sentence naming the field and the value, and writes the
-- `history.migration_log` row. It is a trigger on `custom.record`, and DOOR-FIX's suite fired
-- it by calling `custom.record_update` on the FIELD ROW with the store's own internal keys
-- (`{"type":"range","config":{"kind":"number"}}`) — as the superuser. The door a person and an
-- agent actually reach is `custom.field_update`, and `custom.field_update` patches nine keys,
-- none of which is the behaviour: it REFUSES `parity_type` outright ("Changing what X holds is
-- a conversion, not a setting") and it reads neither `plain` nor `type` at all, so it returned
-- success for a change it had not made. The engine was built; the door to it was not.
--
-- SO THE DOOR IS THE CONVERSION'S DOOR NOW. `custom.field_update` given `parity_type`, `plain`
-- or `type` rebuilds the field's document through `custom._field_document_for` — the same
-- function `custom.field_declare` uses, so a column changed is shaped exactly like a column
-- created — and writes it. The trigger then does what it always did. One path, one trace, and
-- the answer a caller gets says what happened.
--
-- AND TWO THINGS THE DECLARING DOOR THREW AWAY, both of them halves of T7 and of the sixth
-- pass's first "other thing we found":
--   · `depends_on` was hard-coded to the empty list, so no client-made formula ever had
--     dependencies and REC-18's "this field is used by …" could never fire for one.
--   · `on_target_delete` was hard-coded to `set_null` for the two relation types, so no client
--     could declare the `restrict` T7 asks for, and deleting a pointed-at record was always
--     accepted.
--   · `type` was read by nobody: `{"type":"number"}` gave text and `{"type":"banana"}` was
--     accepted as text. It is now read as what it means, and a word that means nothing is
...
-- ── THE DOOR THAT CHANGES A COLUMN ──────────────────────────────────────────────────────
