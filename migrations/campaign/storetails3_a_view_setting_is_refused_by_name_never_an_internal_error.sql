-- chair-step: a STORE FIX (STORE-TAILS-3, the chair's follow-up of 2026-09-24; S5-PRIME-2 found
--   it). REPLACES one body, declared below with the body it was written against:
--   `custom._view_key_value` (the one guard every saved-view setting passes). No table, column,
--   trigger, policy or grant is added; no row of anybody's data is rewritten.
--   Inverse: `migrations/inverse/storetails3_a_view_setting_is_refused_by_name_never_an_internal_error_down.sql`.
--   Applied directly (owner, 2026-09-24 ~17:30 PT), proven on the dev clone first (RED: `filters`
--   answered XX000; GREEN: judged, and every shape answers a mapped refusal; up, inverse, up).
-- lock: custom
-- lane: STORE-TAILS-3
...
--
-- WHY. The view-key registry (`custom.view_keys()`) names `filters` with shape `filter` and three
-- store-written settings with shape `server`; the guard had no arm for either, and its last line
-- raised `XX000` — Postgres's "internal error" — "The view key … has shape …, which the guard does
-- not know." A door never answers a caller's input with an internal error: the records package
-- cannot map it (its live census found it), and a person is told nothing they can act on. Now
-- `filters` is judged exactly as custom.view_declare judges it, a `server` setting is refused by
-- name, and any shape the guard does not know is a named 22023 refusal of the request.
