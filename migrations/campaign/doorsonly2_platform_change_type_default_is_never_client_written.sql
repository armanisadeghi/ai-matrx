...
-- removing the door.
--
-- platform.change_type_default -- POLICY SEMANTICS.
-- The default change policy per change type. Its one reader, `features/change-policy/generate-seed.ts`, is a `--check` that reads with SUPABASE_SECRET_KEY -- service_role, which this closure does not touch.
--
-- WHY IT MATTERS: A member who writes the change-type defaults changes how every organization's changes are governed.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `platform.change_type_default` is owned by `postgres` and does NOT carry
