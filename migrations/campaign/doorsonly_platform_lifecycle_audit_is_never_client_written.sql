...
--
-- platform.lifecycle_audit -- AUDIT SEMANTICS.
-- The data-lifecycle audit: what was archived, purged or retained and on whose policy. Written only by the `platform.lifecycle_*` server functions, every one SECURITY DEFINER and owned by `postgres`. Zero client mentions.
--
-- WHY IT MATTERS: The evidence that a retention obligation was met is exactly the thing that must not be client-writable.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.lifecycle_audit` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_lifecycle_audit_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "lifecycle_audit_client_insert_refused" on platform.lifecycle_audit
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "lifecycle_audit_client_update_refused" on platform.lifecycle_audit
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "lifecycle_audit_client_delete_refused" on platform.lifecycle_audit
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "lifecycle_audit_client_insert_refused" on platform.lifecycle_audit is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: The evidence that a retention obligation was met is exactly the thing that must not be client-writable. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "lifecycle_audit_client_update_refused" on platform.lifecycle_audit is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: The evidence that a retention obligation was met is exactly the thing that must not be client-writable. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "lifecycle_audit_client_delete_refused" on platform.lifecycle_audit is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: The evidence that a retention obligation was met is exactly the thing that must not be client-writable. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
