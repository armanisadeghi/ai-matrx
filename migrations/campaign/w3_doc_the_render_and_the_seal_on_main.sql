...
begin
  ...
  if to_regclass('custom.doc_render') is null then
    perform platform.provision($provision_render$
{
  "schema": "custom",
  "table": "doc_render",
  "token": "doc_render",
  ...
  "functions": [
    {
      ...
      "arg_checks": {
        ...
        "p_template_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: a doc_template Record of that organization, already resolved by custom.doc_render_document through the store's own read. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_record_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: a Record of that organization, already resolved by the merge. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_table_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the Table that record belongs to, which is the Table whose Fields the template's tokens name. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_template_version": {
...
$provision_render$);
    ...
    raise notice 'custom.doc_render already exists here; platform.provision(spec) was not called.';
  end if;
end
$land_doc_render$;

-- ── THE SEAL ───────────────────────────────────────────────────────────────────
do $land_doc_signature$
begin
  -- IDEMPOTENT BY `to_regclass`, and not for convenience: the rehearsal copy already carries
  -- this relation, provisioned there under the older validator, and `platform.provision`
  -- refuses a token whose relation still stands with a different declaration — so the copy
  -- would refuse this file and the main database would never see it.
  if to_regclass('custom.doc_signature') is null then
    perform platform.provision($provision_signature$
{
  "schema": "custom",
  "table": "doc_signature",
  "token": "doc_signature",
  ...
  "functions": [
    {
      ...
      "arg_checks": {
        "p_organization_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the tenant; leading column of the row, of the unique index and of the prior-signature lookup. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_render_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: one row of custom.doc_render in that organization; unique here, so a document version is sealed once. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_record_id": {
          ...
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the record the document was rendered from. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_field_key": {
        ...
        "p_signer_user_id": {
          ...
          "null_rule": {
            "means": "an outside signer with no account of ours"
          },
          "entity": null,
          "entity_reason": "This door makes NO access decision with this uuid: the account that signed, when there was one. It lives in schema `custom`, which is declared CLOSED to PUBLIC, anon, authenticated and service_role in platform.schema_client_exposure, is absent from pgrst.db_schemas, and every campaign knob resolves false, so `client_access` is server_only and the only caller is a server lane that has already decided access. Declaring an entity token here would declare a check the body does not perform, which is the false declaration lessons ledger 28 exists to stop; when switch-checklist step 3 opens the schema the door is re-declared with the token and the level it then really enforces."
        },
        "p_document_hash": {
        ...
        "p_document_version": {
          ...
          "null_rule": {
            "means": "the column is NOT NULL and refuses it"
          }
        }
      },
...
$provision_signature$);
    ...
    raise notice 'custom.doc_signature already exists here; platform.provision(spec) was not called.';
  end if;
end
$land_doc_signature$;
