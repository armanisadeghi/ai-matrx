-- chair-step: lane LISTS-AFTER-SWITCH. The Data tables press archives an organization's live older pick lists with their tables (OLDER-DOORS-AFTER-SWITCH) and refuses the whole switch, rolled back, when a list's Table-of-choices copy or any of its live choices is not in the store — but the readiness card never looked at lists, so it said "Ready", offered no Copy again, and the press then failed with "the pick list … has not arrived in the new system yet" (walked on Harbor Dental Group 2026-09-26 16:18 UTC after a list was made between the copy and the press). REPLACES platform._cutover_seam_readiness: one more check, "Every pick list is copied into the new system", with copy_again_clears so the settings card offers Copy again when it is unmet, and the same count the press enforces.
...
-- INVERSE: migrations/inverse/listsafter_b_the_switch_says_when_a_pick_list_is_not_copied_down.sql
