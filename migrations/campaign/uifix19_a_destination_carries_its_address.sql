-- additive: yes
--
-- chair-step: it REPLACES one function body, `custom._table_move_plan(uuid, uuid, uuid)` (the
--   plan `custom.table_home` and `custom.table_move` read). Each destination organization in the
--   answer gains one key, `slug` (the organization's web address). Nothing is refused or accepted
--   differently; no table, trigger, grant or policy changes; no row is written. The inverse is
--   `migrations/inverse/uifix19_a_destination_carries_its_address_down.sql`.
-- lock: custom
-- lane: UI-FIX-19
--
-- based-on: custom._table_move_plan(uuid, uuid, uuid) d39e038ba19d0ca2d80d0faf8e5223189f63bb5bb4586732dea44213541ada41
--
-- UI-FIX-19 — THE MOVE LIST TELLS TWO SAME-NAMED ORGANIZATIONS APART (VERIFIER-19 #9).
--
-- admin@admin.com owns organizations that share a name ("Ironclad Mobile Mechanic",
-- "Birchwood Avenue Renovation"). The move dialog's destinations carried only id, name and role,
-- so a screen had nothing to draw that told two of them apart. Each destination now carries its
-- slug, which the dialog draws beside a name another destination also carries — the way GitHub
-- shows owner/name.
