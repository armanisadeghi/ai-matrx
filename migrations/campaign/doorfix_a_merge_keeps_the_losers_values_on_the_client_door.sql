...
--
-- based-on: custom.migrate_merge(uuid, uuid, uuid, text) c8d090bec1acacb0ccfb09c9a3268c385da7291ce7680264a14a1bd0d082fc28
--
-- DOOR-FIX 2b — THE SAME FIX, RE-LANDED ON THE CLIENT-CALLABLE MERGE.
--
-- WHAT HAPPENED. `doorfix_a_merge_keeps_the_losers_values.sql` landed at 13:38:46Z. Two
-- minutes later lane REACH replaced `custom.migrate_merge` from a body it had read BEFORE
-- that, turning it into a SECURITY DEFINER client door with the access prologue — and taking
-- the value fix back out with it. That is the DD-220 class from the other side: both files
-- declared what they were based on, and the second one was based on a body that had already
-- moved. Measured straight afterwards: `values_taken: 1` and the winner's document unchanged,
-- exactly the defect again.
--
-- THIS FILE IS BOTH: REACH's four prologue lines and SECURITY DEFINER, and DOOR-FIX 2's value
-- handling, read-back and relation retargeting. Neither is a copy of the other's rule — the
-- access question is asked once, at the top, by the one ladder; the value question is asked
-- once, below it.
--
-- WHY IT IS ITS OWN FILE rather than an edit of the first: the first one RAN, and it is in the
-- ledger. A file that is re-written after it applied is a file nobody can reason about.
--
-- INVERSE: migrations/inverse/doorfix_a_merge_keeps_the_losers_values_on_the_client_door_down.sql
