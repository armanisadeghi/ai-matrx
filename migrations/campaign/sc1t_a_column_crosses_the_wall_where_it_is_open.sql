...
-- additive: yes
--   It REPLACES four bodies, each declared below with the body it was written against:
--     · custom.organization_references(text, uuid, jsonb)  a relation Field's target becomes an
--                                                          OPENABLE site, like a relation's `to`
--     · custom.assert_organization_wall(text, uuid, jsonb) the opening for a Field's target is read
--                                                          off the Field's own Table
--     · custom._table_move_plan(uuid, uuid, uuid)          a column crossing the wall is judged by
--                                                          that wall, not refused outright
--     · custom.table_move(uuid, uuid, integer)             a Field moves after the Tables it points at
--   No table, column, trigger, policy or grant is touched; nothing is dropped, revoked or written.
--   Runs AFTER sc1t_a_table_moves_with_the_tables_inside_it.sql.
--   The inverse is `migrations/inverse/sc1t_a_column_crosses_the_wall_where_it_is_open_down.sql`.
-- guard: custom/system_enabled
-- lock: custom
-- lane: SC-1-TAILS
-- based-on: custom.organization_references(text,uuid,jsonb) da5f28690ec0b8de20cc0320a273728bcb3fea2159832771e63847999e939eea
-- based-on: custom.assert_organization_wall(text,uuid,jsonb) b6873afa8061be3c991e4f5bf08ce400164d93a82d5975cb5548d6bdbb42575e
...
--
-- THE CHAIR'S RULING (2026-09-24): "A moved table whose own column points at a table that stays
-- behind is the same case as a row link that crosses: when the wall is open (source table allows
-- it, both organizations have cross-organization links on) the relation holds its value across
-- organizations; when shut, the move is refused by name. The 23503 … is the field-level wall
-- check treating the relation target's organization as a hard constraint instead of consulting
-- the same wall the row links consult; that is a store defect, not a rule."
--
-- THE DEFECT, MEASURED ON THE DEV CLONE. With the Table allowing links to other organizations
-- and both organizations' custom/cross_organization_links on, moving a Field whose
-- relation_target stays behind was refused 23503 "the table this relation field points at belongs
-- to a different organization": custom.organization_references marked `relation_target` NOT
-- openable, so custom.assert_organization_wall never asked the wall. A relation record's `to`
-- and an association edge (platform.enforce_relation_edge) already asked it; the Field did not.
--
-- THE FIX IS ONE WALL. `relation_target` is openable; assert_organization_wall reads the opening
-- off the Field's own Table (the Table the relation starts at) and asks the same
-- custom.cross_organization_links_open. A Field DECLARED pointing into another organization is
-- therefore judged exactly as its values (custom.relation_value_target_ok) and edges are.
-- The move plan counts such columns (`carries.columns_across`), refuses by name when the source
-- Table does not allow links, and asks both organizations per destination.
--
-- USE CASE. Cascade Electronics Recovery moves "Scale tickets" from Tacoma to Portland; its
-- Hauler column points at Tacoma's "Haulers", which stays (the hauler contracts stay with the
-- yard). With the wall open the column keeps pointing at Tacoma's Haulers and every ticket's
-- hauler still reads; shut, the move says which of the three switches is off.
--
...
begin
  ...
  return query
    ...
    -- SC-1-TAILS (chair ruling 2026-09-24): a relation Field's declared target is the SAME
    -- opening as a relation record's `to` — REC-29's "unless the Table allows it" — judged by the
    -- one wall below, never a harder constraint of its own. Its source Table is the Field's own.
    select 'data.relation_target', 'the table this relation field points at',
           nullif(p_row -> 'data' ->> 'relation_target', '')::uuid, true
     where p_kind = 'custom.record'

    union all
    select 'data.' || e.k, e.w, nullif(p_row -> 'data' ->> e.k, '')::uuid, false
      from (values ('entity_definition_id', 'the table this field belongs to'),
...
begin
  for r in select * from custom.organization_references(p_kind, p_organization_id, p_row)
    ...
    v_opened := false;
    if r.openable and r.site = 'data.relation_target' then
      -- SC-1-TAILS: a Field's target — the Table the relation STARTS at is the Field's own Table.
      select coalesce((t.data ->> 'cross_organization_relations')::boolean, false)
        into v_opened
        from custom.record t
       where t.organization_id = p_organization_id
         and t.id = nullif(p_row -> 'data' ->> 'entity_definition_id', '')::uuid
       limit 1;
    elsif r.openable then
      select coalesce((t.data ->> 'cross_organization_relations')::boolean, false)
        into v_opened
