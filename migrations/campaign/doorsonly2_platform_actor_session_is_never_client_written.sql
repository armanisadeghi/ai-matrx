...
-- removing the door.
--
-- platform.actor_session -- AUDIT SEMANTICS.
-- Who was acting, from where, and when. It is written by the server as a record of what happened.
--
-- WHY IT MATTERS: An audit row a client can write is an audit row a client can forge -- the same shape SECURITY-SWEEP-2 closed across fifty columns.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("actor_session")` search AND a
-- bare-name search of `"actor_session"` / 'actor_session' / `actor_session` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.actor_session` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_actor_session_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "actor_session_client_insert_refused" on platform.actor_session
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "actor_session_client_update_refused" on platform.actor_session
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "actor_session_client_delete_refused" on platform.actor_session
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "actor_session_client_insert_refused" on platform.actor_session is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: An audit row a client can write is an audit row a client can forge -- the same shape SECURITY-SWEEP-2 closed across fifty columns. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "actor_session_client_update_refused" on platform.actor_session is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: An audit row a client can write is an audit row a client can forge -- the same shape SECURITY-SWEEP-2 closed across fifty columns. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "actor_session_client_delete_refused" on platform.actor_session is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). AUDIT semantics: An audit row a client can write is an audit row a client can forge -- the same shape SECURITY-SWEEP-2 closed across fifty columns. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
