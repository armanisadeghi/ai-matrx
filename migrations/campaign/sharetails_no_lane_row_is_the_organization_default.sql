-- chair-step: lane SHARE-TAILS (chair ruling 2026-09-25). A missing lane row is read everywhere as the organization default, never as "mine". REPLACES iam.lane_of (no row -> organization, unless the record itself is personal; the organization lane's mine+availability pair -> organization), public.store_door_lane (reads through iam.lane_of and says `organization_default` whenever membership really reaches the thing) and custom.share_access (asks for the world rows by name). No data is written. Same signatures.
-- based-on: iam.lane_of(text, uuid) 865e474d82c77312a240a84df139240278820c0ebba595d50b0d1e7506998187
-- based-on: public.store_door_lane(text, uuid) 8eda0d03c4bdc5031e4859cd1cddc25a4406633f99b47d5ff75bbdc7a2525cd8
-- based-on: custom.share_access(uuid, uuid) 0f120bd12ad6eb9a0bc99dccd11ea2d2934289e173016b221295249380810590
-- lane: SHARE-TAILS
-- INVERSE: migrations/inverse/sharetails_no_lane_row_is_the_organization_default_down.sql
...
CREATE OR REPLACE FUNCTION iam.lane_of(p_resource_type text, p_resource_id uuid)
 ...
 SET search_path TO ''
AS $function$
  -- SHARE-TAILS (chair ruling 2026-09-25): A MISSING LANE ROW IS THE ORGANIZATION DEFAULT, NEVER
  -- "MINE". Nobody chose "Only people I share it with" for a thing that has no lane row, and every
  -- member reaches it through the organization's member default, so reading it as "mine" was the
  -- ambiguity that made the dialog and the access kernel disagree. Three answers:
  --   · no row, and the record itself is below `internal` (its owner marked it personal) -> mine
  --   · no row otherwise                                                              -> organization
  --   · a "mine" row whose own organization holds an active availability row (the organization
  --     lane is written as mine + that row by custom.share_lane_set)                  -> organization
  --   · any other row                                                                 -> its lane
  select case
           when c.resource_id is null then
             case when p_resource_type = 'record'
                       and exists (select 1 from custom.record r
                                    where r.id = p_resource_id
                                      and r.visibility < 'internal'::platform.visibility)
                  then 'mine' else 'organization' end
           when c.lane = 'mine'
                and exists (select 1 from iam.permissions p
                             where p.resource_type = c.resource_type and p.resource_id = c.resource_id
                               and p.granted_to_organization_id = c.organization_id
                               and p.status = 'active' and p.granted_via = 'availability')
             then 'organization'
           else c.lane
         end
    from (select 1) as one
    left join iam.content_lane c
      on c.resource_type = p_resource_type and c.resource_id = p_resource_id;
$function$;
...
begin
  ...
  -- SHARE-TAILS: iam.lane_of now answers "organization" for a thing with no lane row, so the
  -- world rows are asked for by name rather than as "anything but mine".
  if v_lane = 'world' then
    return query
