-- additive: yes
--   It REPLACES three functions of schema `custom` that `workdoors_one_approval_queue.sql`
--   created and `apprvfix_the_queue_holds_a_record_write_too.sql` last touched, and ADDS one
--   new function, `custom.work_approval_kinds()`. Every replacement keeps the existing
--   signature, security, search_path and every existing branch; the first two lines of each
--   body — the OFF switch and the wall — are byte-identical to what is live. What changes is
--   that the one queue accepts THREE more kinds of pending change (`record_delete`,
--   `record_restore`, `table_add`) where it accepted three, and applies them through the
--   store's own doors. No table, column, policy or grant is touched; no row is deleted or
--   rewritten; the three existing kinds behave exactly as before.
--   The inverse is `migrations/inverse/apprvtail_the_queue_holds_a_delete_and_a_new_table_down.sql`.
-- NO `-- target:` AND NO `-- guard:` HEADER, ON PURPOSE — for the reason
--   `apprvfix_the_queue_holds_a_record_write_too.sql` states in its own header: this file adds
--   no served surface, it replaces bodies whose FIRST line is already
--   `custom.assert_store_door(…)`, which IS the OFF switch, and it leaves that line untouched.
-- based-on: custom.work_approval_request(uuid, uuid, jsonb, text, uuid, text, uuid) 16b21a0c3879f78538b32f11c819a0e763cdc3901f636be40152b4e18d60c9e9
-- based-on: custom.work_approval_decide(uuid, uuid, boolean, text) 93cd84a2adaaa9b3ced3bbc8ce3cfa0a6c3ae1129d0f80eee2d66159a6891695
-- based-on: custom._workdoors_approval_guard() 5eda3ee1ec54e0f09ceed35ad075a29a93cd52076fb5a098670fc6cdf170b0c2
--
-- WHY THIS EXISTS — TWO HOLES IN ONE DECISION.
--
-- 1. AN AGENT COULD DELETE WITHOUT ASKING ANYBODY. Under the default `ask`, writing a record
--    into somebody's existing table waited for a person (APPROVAL-FIX) and DELETING one did
--    not: `custom.record_delete` was called straight from the tool. A person who reads "ask
--    before an agent changes my tables" does not mean "except when it removes them". The
--    decision is the SAME single SQL decision every other change asks —
--    `custom.agent_change_approval` — so the agent's own table of this conversation still goes
--    ahead and a table that existed before the conversation waits. Restoring is the same verb
--    from the other side and is decided the same way.
--
-- 2. AN `always_ask` ORGANIZATION COULD NOT QUEUE A NEW TABLE. The queue files against an
--    existing subject, and a table that does not exist has none — so `table_propose` under
--    `always_ask` answered with a sentence and no button, which is a dead end wearing a
--    politeness. A new table is not nothing, though: it is made IN a Home, and that Home is a
--    record this organization already has. So the wait hangs on the organization and the HOME,
--    its approvers are resolved on the one ladder (admins on that Home, then the
--    organization's owners and admins — `custom.work_approval_approvers`, unchanged), and
--    approving runs `custom.table_declare` and then `custom.field_declare` per field, which is
--    exactly what the direct path runs. Same doors, same order, same bytes.
--
-- THE LIST OF KINDS IS SAID ONCE, in `custom.work_approval_kinds()`, and the row guard, the
-- filing door and the deciding door all read it. Three copies of one list is how a kind gets
-- accepted by the guard and then refused by the door that would apply it.
...
comment on function custom.work_approval_kinds() is
  'The kinds of change the one approval queue can hold. Read by custom._workdoors_approval_guard, custom.work_approval_request and custom.work_approval_decide so a kind cannot be accepted by one and refused by another.';
...
-- ─────────────────────────────────────────────────────────── the filing door
create or replace function custom.work_approval_request(
  p_organization_id uuid,
  p_subject_id uuid,
  p_change jsonb,
  p_note text default null::text,
  p_approver_id uuid default null::uuid,
  p_origin text default 'person'::text,
...
-- ──────────────────────────────────────────────────────── the deciding door
create or replace function custom.work_approval_decide(
  p_organization_id uuid,
  p_approval_id uuid,
  p_approve boolean,
  p_note text default null::text)
