...
--
-- iam.organization_preferences -- A PREFERENCE TABLE WITH A LIVE SPEND METER IN IT.
--
...
--
-- ONE client writer file, `features/organizations/hooks/useOrgAutoRagPreference.ts`, four
-- upserts, one flag each. Four more writers are aidream's auto-ingest budget code
-- (`services/auto_ingest/budget.py`, `cost_recording.py`) over the ORM's own Postgres
-- connection, which no `authenticated` policy touches. The generic registry writer cannot
-- reach this table at all -- its `title_column` is null, and `requireTitleColumn` refuses
-- both create and rename for it.
--
-- THE DOOR THEY NOW USE: `public.org_preferences_set`,
...
-- Inverse: migrations/inverse/doorsonly3_iam_organization_preferences_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "organization_preferences_client_insert_refused" on iam.organization_preferences
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "organization_preferences_client_update_refused" on iam.organization_preferences
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "organization_preferences_client_delete_refused" on iam.organization_preferences
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "organization_preferences_client_insert_refused" on iam.organization_preferences is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). Three of this table''s columns are a LIVE SPEND LEDGER -- daily_auto_rag_cost_used_usd, daily_auto_rag_window_start and the budget they are compared against -- written by aidream''s auto-ingest code and read to decide whether an organization may spend more today. The base-table UPDATE grant let an organization admin zero their own consumed cost and roll their own window from a browser, which is the entire budget. The door writes the four preference flags and the budget CEILING, and cannot reach the meter. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit -- including the GENERIC registry-driven writer in @ai-matrx/associations, which this campaign had silently broken on every table it closed and which now goes through public.entity_row_create / entity_row_rename. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
comment on policy "organization_preferences_client_update_refused" on iam.organization_preferences is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). Three of this table''s columns are a LIVE SPEND LEDGER -- daily_auto_rag_cost_used_usd, daily_auto_rag_window_start and the budget they are compared against -- written by aidream''s auto-ingest code and read to decide whether an organization may spend more today. The base-table UPDATE grant let an organization admin zero their own consumed cost and roll their own window from a browser, which is the entire budget. The door writes the four preference flags and the budget CEILING, and cannot reach the meter. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit -- including the GENERIC registry-driven writer in @ai-matrx/associations, which this campaign had silently broken on every table it closed and which now goes through public.entity_row_create / entity_row_rename. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
comment on policy "organization_preferences_client_delete_refused" on iam.organization_preferences is 'DOORS-ONLY-3 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). Three of this table''s columns are a LIVE SPEND LEDGER -- daily_auto_rag_cost_used_usd, daily_auto_rag_window_start and the budget they are compared against -- written by aidream''s auto-ingest code and read to decide whether an organization may spend more today. The base-table UPDATE grant let an organization admin zero their own consumed cost and roll their own window from a browser, which is the entire budget. The door writes the four preference flags and the budget CEILING, and cannot reach the meter. Census: every file naming this table was listed and then checked for ANY write verb anywhere in it, not only beside the .from(, and every writer was moved to the door in the same commit -- including the GENERIC registry-driven writer in @ai-matrx/associations, which this campaign had silently broken on every table it closed and which now goes through public.entity_row_create / entity_row_rename. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
