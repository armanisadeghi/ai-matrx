...
--
-- REGENERATION BATCH 6 OF 7 -- 8 table(s) in `platform`/`iam` through the canonical route.
--
...
-- 8 to a transaction instead of all fifty-two in one -- a short lock per table rather than one
...
--
-- Inverse: migrations/inverse/doorsonly4_regenerate_batch_06.inverse.sql
set lock_timeout = '2s';
set statement_timeout = '600s';

select iam.apply_rls('platform', 'outsider_consumer', 'platform_outsider_consumer', 'system');
select iam.apply_rls('platform', 'purpose', 'purpose', 'entity');
select iam.apply_rls('platform', 'retention_policy', 'retention_policy', 'system');
select iam.apply_rls('platform', 'route_manifest', 'route_manifest_entry', 'system');
...
select iam.apply_rls('platform', 'share_links', 'platform_share_link', 'entity');
select iam.apply_rls('platform', 'shareable_resource_registry', 'shareable_resource_registry', 'system');
