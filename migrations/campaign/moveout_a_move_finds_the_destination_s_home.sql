...
-- additive: yes
--   It REPLACES one body, declared below with the body it was written against:
--     · custom.table_move(uuid, uuid, integer)   the door that moves a table
--   No table, column, trigger, policy or grant is touched; nothing is dropped or revoked; nothing
--   is written by this file. The inverse is
--   `migrations/inverse/moveout_a_move_finds_the_destination_s_home_down.sql`, which restores the
--   body byte for byte.
-- guard: custom/system_enabled
-- lock: custom
-- lane: MOVE-AND-OUTSIDER
-- based-on: custom.table_move(uuid, uuid, integer) e7e6ba70790acd0160a86b70f0713c314d6f666061f37c462e92d717ffd3e32c
--
-- LANE MOVE-AND-OUTSIDER (VERIFIER-19 finding 1): EVERY MOVE FROM THE CHIP WAS REFUSED.
--
-- WHAT A PERSON MET. On production every custom.table_move from the chip's dialog was refused
-- 23514 "a table has to live somewhere - give it a home" (3 of 3: Rooms to Birchwood Avenue
-- Renovation, Rooms to Elm Street Workshop, Tool checkout log to Ashford Labs).
--
-- ROOT CAUSE. The door re-homed the moved Table only when it sat directly in the source
-- organization's own record AND the destination had one; in every other case it took `parent_id`
-- away (`x.data - 'parent_id'`), and custom._table_shape_guard refuses a Table with no home. But
-- the app declares every table inside its OWN Home record ("<name> Home", a Person-kernel record:
-- records declareTable / records-ui systemTable), never in the organization's record, so the
-- "else" arm was the arm every real table took. And G11 made an organization's record optional
-- at provision, so a destination may have none at all.
--
-- THE FIX. The moved Table is always put in the destination organization's own record (the one
-- custom.organization_home_id names), unless its container moves with it. A destination the store
-- never gave a record gets one here, named after the organization, inside the same transaction —
-- a person is never refused for a fact the store owns. `carried.home` says which record it went
-- into and whether it was made. The table's old "<name> Home" record stays in the organization
-- it was in (never deleted), holding nothing.
--
-- PROOF. scripts/campaign-tests/moveout_a_move_finds_the_destination_s_home_red_green.sql
-- (Mesa Verde Landscape Design's Studio moves "Irrigation zones" to its Install Crew and back):
-- RED before this file with the production sentence; GREEN H1–H4 after.
