...
-- based-on: custom.pipeline_declare(uuid, uuid, jsonb) 1cd457d667c436e7278ee16043b6d086a0417a3775358c55c659dcf58652e5bd
-- based-on: custom.pipeline_gate_preview(uuid, uuid, text, jsonb) 2f92a367910069e12e6e7230f157b5c24f28f8448fd75fed92e276a2d730a9d9
--
-- ══════════════════════════════════════════════════════════════════════════════════════
-- A HALF-WRITTEN RULE TURNED THE WORKING ONE OFF, AND SAID IT HAD SUCCEEDED.
-- lane STAGE-RULES-2 · 2026-09-21
--
-- Found by this lane's own green suite, then reproduced on the live board. One call:
--
--   custom.pipeline_declare(org, quotes,
--     '{"stage_field":{"key":"quote_stage"},"gates":{"Approved":[{"message":"half written"}]}}')
--
-- It ANSWERED SUCCESS. It reported `"said": ["Approved: half written"]`. And what it had
-- actually done was overwrite the Birchwood Avenue renovation's only house rule — "Nothing
-- over $5,000 moves to Approved without a second quote from a different contractor" — with
-- a Rule whose entire expression was `{"op":"or","args":null}`. A rule with no expression
-- refuses nobody. The board looked exactly the same and enforced nothing.
--
-- THE CAUSE, AND WHY IT IS A CLASS AND NOT A TYPO. Both doors tested the shape of the
-- gate's demand like this:
--
--   if jsonb_typeof(v_one -> 'demands') <> 'object' then raise ...
--
-- `jsonb_typeof(NULL)` is NULL. `NULL <> 'object'` is NULL. NULL is not true, so the
-- `raise` never fired — for the one input it exists to catch, the key being ABSENT. A gate
-- carrying `"demands": 42` or `"demands": null` was refused correctly; a gate carrying no
-- `demands` at all was waved through. The check was written against the wrong half of the
-- problem, and every reader of it (including two code reviews) read `<>` as "is not".
--
-- BOTH SITES ARE FIXED HERE, in one file, because they are one defect written twice:
-- `custom.pipeline_declare`, where it destroyed a live rule, and
-- `custom.pipeline_gate_preview`, where it answered a confident "0 cards would be refused"
-- about a rule that has no condition — and "this stops nobody" is exactly the sentence
-- that gets a rule saved.
--
-- WHAT ELSE THIS FILE DOES NOT DO. It does not try to find other `jsonb_typeof(x) <> 'y'`
-- comparisons across the store; that sweep is a census worth running and is named in this
-- lane's progress document rather than smuggled into a fix.
-- ══════════════════════════════════════════════════════════════════════════════════════
  ...
  foreach v_stage in array v_stages loop
    ...
    if jsonb_typeof(v_gates) = 'array' and jsonb_array_length(v_gates) > 0 then
      ...
      for v_one in select t from jsonb_array_elements(v_gates) t loop
        v_gate_n := v_gate_n + 1;
        -- coalesce, AND IT IS THE WHOLE FIX. `jsonb_typeof(NULL)` is NULL, and
        -- `NULL <> 'object'` is NULL, which is not true — so a gate with no `demands` KEY
        -- AT ALL walked straight past this check. What it then compiled to was
        -- `{"op":"or","args":null}`: a rule with no expression, which refuses nobody. A
        -- half-written gate did not fail; it SILENTLY REPLACED the working one beside it
        -- and turned the board's only rule off. Measured on the Birchwood Avenue
        -- renovation on 2026-09-21 — one call left the homeowner's $5,000 rule inert, with
        -- a success answer and her own sentence replaced by the word "half written".
        if coalesce(jsonb_typeof(v_one -> 'demands'), 'absent') <> 'object' then
...
begin
  ...
  -- The same NULL trap as `custom.pipeline_declare`'s, closed the same way and in the same
  -- file: `jsonb_typeof(NULL) <> 'object'` is NULL, not true, so a gate with no `demands`
  -- key reached the compiler and the preview answered a confident zero about a rule that
  -- has no condition. Zero beside a half-written rule reads as "this stops nobody", which
  -- is the sentence that gets it saved.
  if coalesce(jsonb_typeof(p_gate -> 'demands'), 'absent') <> 'object' then
