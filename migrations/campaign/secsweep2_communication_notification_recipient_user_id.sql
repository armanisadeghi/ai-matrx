...
create policy "notification_recipient_user_id_is_addressable_insert" on communication.notification
  ...
  with check (recipient_user_id is null or recipient_user_id = (select auth.uid()) or iam.may_address_user_in_org(recipient_user_id, organization_id) or (select public.is_platform_admin()));
