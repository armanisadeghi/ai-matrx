...
-- removing the door.
--
-- iam.permissions -- GRANT SEMANTICS.
-- Who was granted what. Every client mention is a `.select(...)`, and `features/files/filesDb.ts` says so in its own header comment: list, grant and revoke go through `iamDb(supabase).rpc('fn_list_resource_permissions', ...)`, admin-gated inside the RPC by `iam.has_access` -- "never a plain `.from('permissions')` select".
--
-- WHY IT MATTERS: THIS IS THE GRANT TABLE. A member who can write it grants themselves anything. It is the same shape as CRITICAL-1 (`iam.api_keys.service_user_id`), and the doors for it already exist.
--
...
-- names it.
--
-- THE DOOR IS UNAFFECTED. `iam.permissions` is owned by `postgres` and does NOT carry
