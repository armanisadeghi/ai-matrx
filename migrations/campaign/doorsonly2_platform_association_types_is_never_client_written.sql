...
-- removing the door.
--
-- platform.association_types -- VOCABULARY SEMANTICS.
-- The registry of which entity may be associated with which, and in which direction -- read by `trg_associations_enforce_known` and `_auto_orient` on every association write.
--
-- WHY IT MATTERS: A member who adds an association type teaches the association engine a new legal shape for the whole platform; a member who deletes one makes existing associations unenforceable.
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("association_types")` search AND a
-- bare-name search of `"association_types"` / 'association_types' / `association_types` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.association_types` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_association_types_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "association_types_client_insert_refused" on platform.association_types
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "association_types_client_update_refused" on platform.association_types
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "association_types_client_delete_refused" on platform.association_types
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "association_types_client_insert_refused" on platform.association_types is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). VOCABULARY semantics: A member who adds an association type teaches the association engine a new legal shape for the whole platform; a member who deletes one makes existing associations unenforceable. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "association_types_client_update_refused" on platform.association_types is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). VOCABULARY semantics: A member who adds an association type teaches the association engine a new legal shape for the whole platform; a member who deletes one makes existing associations unenforceable. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "association_types_client_delete_refused" on platform.association_types is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). VOCABULARY semantics: A member who adds an association type teaches the association engine a new legal shape for the whole platform; a member who deletes one makes existing associations unenforceable. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
