...
--
-- platform.entity_grants -- GRANT SEMANTICS.
-- Per-entity access grants. Zero client mentions; written by the `iam` grant doors.
--
-- WHY IT MATTERS: Same shape as `iam.membership_grant`: a client write is a self-service grant.
--
...
--
-- THE DOOR IS UNAFFECTED. `platform.entity_grants` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly_platform_entity_grants_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "entity_grants_client_insert_refused" on platform.entity_grants
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "entity_grants_client_update_refused" on platform.entity_grants
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "entity_grants_client_delete_refused" on platform.entity_grants
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "entity_grants_client_insert_refused" on platform.entity_grants is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). GRANT semantics: Same shape as `iam.membership_grant`: a client write is a self-service grant. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "entity_grants_client_update_refused" on platform.entity_grants is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). GRANT semantics: Same shape as `iam.membership_grant`: a client write is a self-service grant. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';

comment on policy "entity_grants_client_delete_refused" on platform.entity_grants is
  'DOORS-ONLY 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). GRANT semantics: Same shape as `iam.membership_grant`: a client write is a self-service grant. Census: zero client writers in matrx-frontend, matrx-extend or matrx-local, every hit read rather than counted. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched.';
