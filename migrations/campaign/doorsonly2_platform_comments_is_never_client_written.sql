...
-- removing the door.
--
-- platform.comments -- CONTENT SEMANTICS.
-- Comment threads. The whole feature is already doored: `public.cmt_add`, `cmt_edit`, `cmt_delete` and `cmt_list` are declared in `platform.client_callable_door`, and the CRM panel reads through `cmt_list`.
--
...
--
-- THE CENSUS, done the way SECURITY-SWEEP said to do it -- a `.from("comments")` search AND a
-- bare-name search of `"comments"` / 'comments' / `comments` across matrx-frontend
...
--
-- THE DOOR IS UNAFFECTED. `platform.comments` is owned by `postgres` and does NOT carry
...
-- Inverse: migrations/inverse/doorsonly2_platform_comments_is_never_client_written.inverse.sql
-- Guard: `pnpm check:doors-only-schemas` -- these three triples move from OPEN to RESIDUAL.

set local lock_timeout = '2s';

create policy "comments_client_insert_refused" on platform.comments
  as restrictive for insert to authenticated, anon
  with check (false);

create policy "comments_client_update_refused" on platform.comments
  as restrictive for update to authenticated, anon
  using (false) with check (false);

create policy "comments_client_delete_refused" on platform.comments
  as restrictive for delete to authenticated, anon
  using (false);

comment on policy "comments_client_insert_refused" on platform.comments is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). CONTENT semantics: A base-table grant beside a complete set of doors is a second path no code walks -- exactly the `iam.api_keys` shape that became CRITICAL-1. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "comments_client_update_refused" on platform.comments is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). CONTENT semantics: A base-table grant beside a complete set of doors is a second path no code walks -- exactly the `iam.api_keys` shape that became CRITICAL-1. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';

comment on policy "comments_client_delete_refused" on platform.comments is
  'DOORS-ONLY-2 2026-09-21: platform and iam are not client-writable through PostgREST -- every write goes through a SECURITY DEFINER door that decides through the one ladder; reads stay under RLS (VERIFIER-8 HIGH-3). CONTENT semantics: A base-table grant beside a complete set of doors is a second path no code walks -- exactly the `iam.api_keys` shape that became CRITICAL-1. Census: zero client writers in matrx-frontend, matrx-extend, matrx-local, the @ai-matrx packages or the aidream dashboard -- every hit read rather than counted, and every one was a generated catalogue, a read-only registry entry or a doc. RESTRICTIVE so it ANDs with every permissive policy present or future, and bespoke so iam.apply_rls preserves it (DD-147). The table is owned by postgres with no FORCE RLS, so every SECURITY DEFINER door is unaffected; service_role and SELECT are untouched and platform_admin_all is kept.';
