-- chair-step: this REPLACES the bodies of 15 live functions in schema communication, changing ONE kind of statement and nothing else: every `raise exception … using errcode = 'P0002'` (20 of them) becomes `perform platform.refuse_not_found(<the same sentence>, <the same hint>, <the same detail>)`. Called directly (the server, every suite, every other function) the refusal is byte-for-byte what it was: SQLSTATE P0002, same message, hint and detail. Called through PostgREST it answers HTTP 404 with error code P0002 instead of HTTP 500. No table, column, policy, trigger or grant is touched; the only rows written are 7 platform.client_callable_door declarations for SECURITY DEFINER bodies that had none (provision_shape_guard refuses a replaced definer without one), each declared as the non-client function it already is. Needs errorshonest_s1_one_way_to_say_not_found.sql first. Inverse: migrations/inverse/errorshonest_s4_the_communication_doors_say_not_found_down.sql restores every body verbatim.
-- lane: ERRORS-HONEST
-- based-on: communication.claim_voice_call_consent_event(text, text, text, text, text, text, text, timestamp with time zone, text, text, timestamp with time zone, text) a2b028738700e5f684e83a10d0c6901389eb9613764203cd197340863022c5ef
-- based-on: communication.claim_voice_call_lifecycle_event(text, text, text, text, integer, text, timestamp with time zone) 0fcd94bac56a02afb25d464566c4927a456e836fcdf797cf0108aa2c6159b56d
-- based-on: communication.claim_voice_playback_activity(uuid, uuid, text, uuid, text, text, boolean, jsonb) 581acc9167f66df3019ea4bde7f037e18ac9cc7b5b62384b6e1cded58b410ad2
-- based-on: communication.claim_voice_recording_lifecycle_event(text, text, text, text, text, text, timestamp with time zone, integer, smallint, text, text, text) 57bc165d8d0e27f18020763eb5c94b18819d09aef650dab180025e97bb9e4a76
-- based-on: communication.configure_my_sms_task_notifications(boolean, text) 1e1ce1e89a02465a5221a3d39c13b120a9a9224dd29f996571436a67a16a57c7
-- based-on: communication.enqueue_task_sms_reminder_for_user(uuid, uuid, text, text) 0decdcce99e64f178aa410859b0ab0de7354fe4912e68c9668b5532fde26cd82
-- based-on: communication.enroll_verified_phone_for_assistant(uuid, uuid, text, timestamp with time zone, text) 461fbc454a7f497539dc6a7791ced360a0f13e4fc50e0ad81f25eef1eee4a8da
-- based-on: communication.fail_voice_recording_custody_work(bigint, uuid, text, text, boolean, integer, text, uuid, boolean, integer) 953e26a4952ef98dcee486b6fb43c2b9b9f97a2a8fc1f9dfa9cb82aa43cebf76
-- based-on: communication.finalize_voice_recording_file(text, text, text, text, text, uuid) e14864786ffe30ba8ad655cd452b2dd581c8fc7293f6cd7bbbde7c5262c961e0
-- based-on: communication.issue_voice_agent_session_reference_unfenced(text, uuid, timestamp with time zone, uuid, uuid, bigint, uuid, text, uuid, uuid, text, jsonb, text) 3310ec0a864ebb06ee2bec7548a7fe29adf7b4bfbd3cfa7163ffbd0655f1ab40
-- based-on: communication.meet_meeting_by_slug(text) 14d19af1a09a8feb668777e9a8dc8c7a2ad8c33ed58e0b9892c9edd837d0ffba
-- based-on: communication.meet_record_consent(uuid, text, timestamp with time zone) 9195f60df99b4d155ba048265ddcbe0b4c5bdbe341b30630134feb31d7c6d521
-- based-on: communication.meet_settle_call_invite(uuid, text, text) 9fe4bb54e247c2d004f3d010be7cd2c94103527c04de915e2d3eca9e9aab4a76
-- based-on: communication.resolve_voice_owner_call_context(text, uuid, text, text, text, text) a588fa41d75b91fc700ebc8b85318dc8a9abc8cabb22d1114de79ef92a214cda
-- based-on: communication.set_my_sms_assistant_enabled(text, boolean) f1eabe2f95607a41e828506a21644e6fdf85075d22ffc015faa4f64e1c0403ed
--
...
--   communication.claim_voice_call_consent_event(text, text, text, text, text, text, text, timestamp with time zone, text, text, timestamp with time zone, text) [not client-granted; reached through a door]: 1 not-found raise
--   communication.claim_voice_call_lifecycle_event(text, text, text, text, integer, text, timestamp with time zone) [not client-granted; reached through a door]: 1 not-found raise
--   communication.claim_voice_playback_activity(uuid, uuid, text, uuid, text, text, boolean, jsonb) [not client-granted; reached through a door]: 3 not-found raises
--   communication.claim_voice_recording_lifecycle_event(text, text, text, text, text, text, timestamp with time zone, integer, smallint, text, text, text) [not client-granted; reached through a door]: 1 not-found raise
--   communication.configure_my_sms_task_notifications(boolean, text): 1 not-found raise
--   communication.enqueue_task_sms_reminder_for_user(uuid, uuid, text, text) [not client-granted; reached through a door]: 1 not-found raise
--   communication.enroll_verified_phone_for_assistant(uuid, uuid, text, timestamp with time zone, text) [not client-granted; reached through a door]: 1 not-found raise
--   communication.fail_voice_recording_custody_work(bigint, uuid, text, text, boolean, integer, text, uuid, boolean, integer) [not client-granted; reached through a door]: 1 not-found raise
--   communication.finalize_voice_recording_file(text, text, text, text, text, uuid) [not client-granted; reached through a door]: 1 not-found raise
--   communication.issue_voice_agent_session_reference_unfenced(text, uuid, timestamp with time zone, uuid, uuid, bigint, uuid, text, uuid, uuid, text, jsonb, text) [not client-granted; reached through a door]: 4 not-found raises
--   communication.meet_meeting_by_slug(text): 1 not-found raise
--   communication.meet_record_consent(uuid, text, timestamp with time zone): 1 not-found raise
--   communication.meet_settle_call_invite(uuid, text, text): 1 not-found raise
--   communication.resolve_voice_owner_call_context(text, uuid, text, text, text, text) [not client-granted; reached through a door]: 1 not-found raise
--   communication.set_my_sms_assistant_enabled(text, boolean): 1 not-found raise
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('No canonical Voice interaction matches the exact call and program');
  end if;
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('No canonical voice interaction matches the exact provider account and call');
  end if;
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('No exact canonical Voice interaction matched playback evidence');
  end if;
  ...
  if v_consumed_count <> 1 then
    perform platform.refuse_not_found('Voice playback session consumption is missing or ambiguous');
  end if;
  ...
  if v_consumed.metadata ->> 'provider_session_id' is distinct from p_provider_session_id
    ...
    perform platform.refuse_not_found('Voice playback session identity is not exact');
  end if;
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('Verify a mobile number before enabling task reminders');
  end if;
...
begin
  ...
  for share;
  if not found then
    perform platform.refuse_not_found('Task was not found');
  end if;
...
begin
  ...
  if v_party_id is null then
    perform platform.refuse_not_found('enroll_verified_phone_for_assistant: no CRM party could be established');
  end if;
  select * into v_point
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('Completed voice recording lifecycle source does not exist');
  end if;
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('No exact completed voice recording interaction is available for file binding');
  end if;
...
begin
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('No exact canonical Voice interaction matched');
  end if;
  ...
  if not found then
    perform platform.refuse_not_found('No exact durable Voice consent evidence matched');
  end if;
  ...
  for update;
  if not found then
    perform platform.refuse_not_found('No exact persisted Voice chat conversation matched');
  end if;
  ...
  if not exists (
    ...
    perform platform.refuse_not_found('Voice Mandate snapshot is not exactly runnable');
  end if;
...
begin
  ...
  if not found then
    perform platform.refuse_not_found('meet_settle_call_invite: no such invite');
  end if;
...
begin
  ...
  if v_candidate_count = 0 then
    perform platform.refuse_not_found('No exact pre-existing enrolled CRM caller context matched');
  end if;
...
begin
  ...
  if updated_count = 0 then
    perform platform.refuse_not_found('SMS preferences and program must already be explicitly bound');
...
-- ── 7 SECURITY DEFINER bodies above had no platform.client_callable_door row. provision_shape_guard refuses to
...
select 'communication', 'claim_voice_call_consent_event', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside communication.voice_call_consent_persistence_readiness, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s4_the_communication_doors_say_not_found.sql (lane ERRORS-HONEST)',
  ...
  from pg_proc p
 where p.oid = 'communication.claim_voice_call_consent_event(text, text, text, text, text, text, text, timestamp with time zone, text, text, timestamp with time zone, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'claim_voice_call_consent_event'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'communication', 'claim_voice_call_lifecycle_event', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside communication.voice_recording_persistence_readiness, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s4_the_communication_doors_say_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'communication.claim_voice_call_lifecycle_event(text, text, text, text, integer, text, timestamp with time zone)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'claim_voice_call_lifecycle_event'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'communication', 'claim_voice_playback_activity', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       ...
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'communication.claim_voice_playback_activity(uuid, uuid, text, uuid, text, text, boolean, jsonb)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'claim_voice_playback_activity'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'communication', 'claim_voice_recording_lifecycle_event', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside communication.voice_recording_persistence_readiness, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s4_the_communication_doors_say_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'communication.claim_voice_recording_lifecycle_event(text, text, text, text, text, text, timestamp with time zone, integer, smallint, text, text, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'claim_voice_recording_lifecycle_event'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'communication', 'fail_voice_recording_custody_work', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s4_the_communication_doors_say_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'communication.fail_voice_recording_custody_work(bigint, uuid, text, text, boolean, integer, text, uuid, boolean, integer)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'fail_voice_recording_custody_work'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'communication', 'finalize_voice_recording_file', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside communication.voice_recording_persistence_readiness, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s4_the_communication_doors_say_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'communication.finalize_voice_recording_file(text, text, text, text, text, uuid)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'finalize_voice_recording_file'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));

insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, signed_in_callers, anonymous_callers, non_client_lane, declared_by, reason)
select 'communication', 'issue_voice_agent_session_reference_unfenced', iam.door_identity_args(p.oid), platform.door_argtypes(p.proargtypes), false, false,
       'server_only: service_role alone holds EXECUTE; anon and authenticated hold none, and it is also called from inside communication.issue_voice_agent_session_reference, which decide access before they call it. Declared by lane ERRORS-HONEST (2026-09-24) when its not-found raise moved to platform.refuse_not_found; its grants are unchanged.',
       'migrations/campaign/errorshonest_s4_the_communication_doors_say_not_found.sql (lane ERRORS-HONEST)',
       'Declared, not changed, by lane ERRORS-HONEST: provision_shape_guard requires every SECURITY DEFINER body that is replaced to say in data who may call it. This body changed only in how it raises a not-found (platform.refuse_not_found). It is not a client door, and no client role holds EXECUTE on it.'
  from pg_proc p
 where p.oid = 'communication.issue_voice_agent_session_reference_unfenced(text, uuid, timestamp with time zone, uuid, uuid, bigint, uuid, text, uuid, uuid, text, jsonb, text)'::regprocedure
   and not exists (select 1 from platform.client_callable_door x
                    where x.schema_name = 'communication' and x.function_name = 'issue_voice_agent_session_reference_unfenced'
                      and x.identity_argtypes = platform.door_argtypes(p.proargtypes));
