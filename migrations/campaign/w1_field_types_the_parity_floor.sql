...
as $fn_parity_type$
  ...
  select case
    ...
    when p_field_data ->> 'type' = 'formula'     then 'formula'
...
begin
  ...
  if v_derived = 'formula' and nullif(d -> 'config' ->> 'expr', '') is null
     and jsonb_typeof(d -> 'config' -> 'expr') is distinct from 'object' then
