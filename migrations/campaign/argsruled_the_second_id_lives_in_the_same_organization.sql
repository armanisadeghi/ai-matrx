-- lane: ARGS-RULED
--
-- chair-step: it REPLACES the live bodies of eight client doors, so the judge cannot read from an
--   allow-list what the statements will do. Nothing is dropped, nothing is revoked, no row of
--   anybody's data is touched: eight existing bodies gain one access decision each. The inverse is
--   migrations/inverse/argsruled_the_second_id_lives_in_the_same_organization_down.sql.
--
-- based-on: hr.attendance_exception_resolve(uuid, text, text, uuid) 67a91a984dc496a977040349427c0cbae631f5822b6df1b0e282eccebab97967
-- based-on: hr.kiosk_pairing_code_create(uuid, text, uuid, uuid) 184bb9c3ad9ab535ab13a59f61f6775ff5bedfffcfb72a06884c4b7a129bf961
-- based-on: hr.leave_enroll(uuid, uuid[], date, text) a23f1605823c353f5ae840d5f11d20504fa44ce3f44754a5f32fcba9358b5352
-- based-on: provider.attach_credential(uuid, uuid, text) 695c8157c0b8686447a613acbaf3adb795fb74a7ed56d5fb74c97e7f8320a236
-- based-on: provider.set_account_status(uuid, text, text, timestamp with time zone, uuid) a2c73b4388f2649dd34df858cb4c67d5592c887495de0c1657fde9c1c433ab32
-- based-on: public.hr_authority_delegation_request(uuid, uuid, date, date, text) 056eee6d16891a7ff829f1d6fbf7fa091ff32d464e7c260be81c15369a183161
-- based-on: public.hr_incident_assign(uuid, uuid, text) 24735d07587730bbbf06a6a45f4edce714bed418b90895b577ed42e414f76d24
-- based-on: public.hr_role_assign(uuid, text, text, uuid, uuid[], date, date, text) 4e6465754c05f89a1399de026b835e3b75ac7cb451623c792b0b8b283b03e94a
--
-- ARGS-RULED — THE SECOND ID LIVES IN THE SAME ORGANIZATION AS THE FIRST.
--
-- ONE CLASS, EIGHT MEMBERS, FOUND BY READING THE EIGHTEEN DOORS OF THE 236 THAT REACH NO LADDER
-- PRIMITIVE AT ALL. Every one of these doors checks its FIRST id properly — it resolves a row,
-- takes that row's `organization_id`, and gates the caller against it. Every one then writes a
-- SECOND id into that row without ever comparing it to the organization the first id established.
-- The per-function guards cannot see this: the body demonstrably decides, and it decides
-- correctly, about the other argument.
--
--   hr.attendance_exception_resolve(p_premium_earning_code_id)   any earning_code on the database
--   hr.kiosk_pairing_code_create(p_location_id)                  a kiosk pointed at another org's site
--   hr.leave_enroll(p_employment_ids)                            another org's people, enrolled here
--   provider.attach_credential(p_credential_item_id)             ANY credential item, bound to an account
--   provider.set_account_status(p_duplicate_of_id)               a duplicate across the wall
--   public.hr_authority_delegation_request(p_delegate_…)         an approval authority handed across
--   public.hr_incident_assign(p_employment_id)                   a case assigned across
--   public.hr_role_assign(p_scope_id, p_scope_employment_ids)    a role SCOPED across
--
-- FOUR OF THEM WERE ALREADY DECLARED. `p_premium_earning_code_id`, `p_location_id` and
-- `p_employment_ids` have carried `{"unchecked": true}` in platform.client_callable_door since
-- 2026-09-17 — three of the five such declarations on the whole platform. That declaration is only
-- legitimate when the argument CANNOT confer access, and each of these three reaches a table
-- predicate and lands in a row. They are checks now, and the declarations go with them.
--
-- ONE OF THE EIGHT WAS NOT EXPLOITABLE, AND THIS FILE SAYS SO RATHER THAN CLAIMING EIGHT.
-- `provider.attach_credential` reaches its INSERT and is refused there by a TRIGGER —
-- `provider._credential_link_guard`: "must be an active organization-owned Vault item in the same
-- organization". Measured, not assumed: scripts/campaign-tests/argsruled_second_id_red_green.sql
-- puts the old body back inside a rolled-back transaction and watches the table stop it. The door
-- still decided nothing about that id, and a refusal from a trigger cannot say what to do, so the
-- decision moves to the door and the trigger stays the backstop it always was.
--
-- THE OTHER SEVEN HAVE NO BACKSTOP. hr.kiosk_device, hr.leave_enrollment, hr.approval_delegation,
-- hr.incident, hr.role_assignment and provider.account carry ONLY plain foreign keys on the second
-- id — existence, never organization — and not one CHECK constraint comparing the two. The same
-- suite asserts that emptiness, because it is the claim these seven fixes rest on.
--
-- `provider.attach_credential`'s rule is worth writing out because it is not the obvious one.
-- 370 of the 406 rows of users.credential_items carry no organization at all: they are PERSONAL.
-- So the door's rule is not "this organization's" but "this organization's, OR the caller's own",
-- which is the only rule that closes the door and still lets a person attach their own credential.
--
-- WHAT THIS DOES NOT DO. `p_scope_kind` values `direct_reports`, `position_subtree` and
-- `employment_set` carry no id of their own (they are resolved relative to the employment or from
-- p_scope_employment_ids, which IS checked here), so the scope CASE lets them through unchanged.
...
begin
  ...
  if v_wants then
    ...
    if p_premium_earning_code_id is not null then
      -- ARGS-RULED (2026-09-21). THE CODE MUST BE THIS ORGANIZATION'S, OR THE PLATFORM'S.
      -- This id was DECLARED `{"unchecked": true}` on 2026-09-17 and it reached a table
      -- predicate with no organization in it: any earning_code row on the database could be
      -- named, and the mismatch refusal below then read that foreign row's `code`, `is_active`
      -- and `is_statutory_premium` back to the caller. The same two organizations the automatic
      -- resolver a few lines down already allows, and no third.
      select * into v_code from hr.earning_code
       where id = p_premium_earning_code_id and deleted_at is null
         and organization_id in (v_ae.organization_id,
...
begin
  ...
  -- ARGS-RULED (2026-09-21). THE LOCATION MUST BE THIS ORGANIZATION'S. `p_location_id` was
  -- DECLARED `{"unchecked": true}` and written straight onto hr.kiosk_device.location_id beside
  -- `p_organization_id`, which the gate above DOES check — so a kiosk could be registered in one
  -- organization pointing at another organization's work location, and a kiosk's location is what
  -- its punches are checked against and what cross-location flagging compares to.
  if p_location_id is not null
    ...
    return hr._punch_refusal('hr_kiosk_location_not_in_this_organization',
      'That work location belongs to a different organization, so no kiosk was registered.',
      jsonb_build_object('location_id', p_location_id, 'organization_id', p_organization_id));
  end if;
...
begin
  ...
  foreach v_emp in array coalesce(p_employment_ids, '{}'::uuid[]) loop
    ...
    -- ARGS-RULED (2026-09-21). THE PERSON MUST BE EMPLOYED BY THE POLICY'S ORGANIZATION.
    -- `p_employment_ids` was DECLARED `{"unchecked": true}`, and the only gate above is
    -- `hr._leave_admin_rung(pol.organization_id)` — about the CALLER and the POLICY, never about
    -- the people. An HR admin of one organization could therefore enrol another organization's
    -- employments into their policy, writing hr.leave_enrollment rows stamped with THIS
    -- organization's id over somebody else's people.
    if not exists (select 1 from hr.employment em
                    where em.id = v_emp and em.organization_id = pol.organization_id
                      and em.deleted_at is null) then
      v_skipped := v_skipped || jsonb_build_array(jsonb_build_object(
        'employment_id', v_emp, 'reason','not_in_this_organization',
        'detail','That person is not employed by the organization this policy belongs to, so they were not enrolled.'));
      continue;
    end if;
...
begin
  ...
  -- ARGS-RULED (2026-09-21). THE INVESTIGATOR IS EMPLOYED BY THE ORGANIZATION THE CASE BELONGS
  -- TO. `v_org` comes off the incident and gates the caller; `p_employment_id` was only checked
  -- for EXCLUSION, never for organization, so a case could be assigned to somebody employed
  -- somewhere else entirely.
  if not exists (select 1 from hr.employment em
                  where em.id = p_employment_id and em.organization_id = v_org
                    and em.deleted_at is null) then
    return jsonb_build_object('ok', false, 'reason', 'assignee_not_in_this_organization',
      'detail', 'A case is investigated by somebody employed by the organization it belongs to.');
  end if;
...
-- ── THE DECLARATIONS DD-223 ASKS FOR ────────────────────────────────────────────────────────
-- `provision_shape_guard` refuses a replaced SECURITY DEFINER body at COMMIT unless somebody has
-- said IN DATA who may call it. Three of the eight are INNER functions with no client grant at
-- all — `authenticated` and `anon` both answer false, and the client reaches them only through
-- the `public.hr_*` wrappers, which carry their own door rows. That is a `non_client_lane`
-- declaration, and it is the honest one: they are not doors, they are what doors call.
insert into platform.client_callable_door
  (schema_name, function_name, identity_args, identity_argtypes, reason, declared_by,
   non_client_lane, signed_in_callers, anonymous_callers)
values
  ('hr', 'attendance_exception_resolve',
   'p_exception_id uuid, p_resolution_state text, p_note text, p_premium_earning_code_id uuid',
   array['uuid'::regtype, 'text'::regtype, 'text'::regtype, 'uuid'::regtype]::oid[],
   'p_exception_id: resolved from hr.attendance_exception, and hr.capability(time.read) is asked over that exception''s employment as of its work date. p_premium_earning_code_id: ARGS-RULED 2026-09-21 - scoped to the exception''s own organization or the platform system organization, the same two the automatic resolver allows, and then required to be the expected statutory premium code. NULL for either is refused by name (hr_arguments_incomplete / the automatic resolver takes over).',
   'argsruled_the_second_id_lives_in_the_same_organization.sql',
   'server_only: no client role holds EXECUTE on it (authenticated and anon both answer false). The client reaches this through public.hr_attendance_exception_resolve, which is the declared door and carries its own argument rules.',
   false, false),
  ('hr', 'kiosk_pairing_code_create',
   'p_organization_id uuid, p_device_name text, p_location_id uuid, p_device_id uuid',
   array['uuid'::regtype, 'text'::regtype, 'uuid'::regtype, 'uuid'::regtype]::oid[],
   'p_organization_id: hr._kiosk_admin_gate(p_organization_id) on the first line. p_location_id: ARGS-RULED 2026-09-21 - must be a live hr.location of that same organization, because a kiosk''s location is what its punches are checked against. p_device_id: scoped to the organization in the UPDATE''s own WHERE and answers hr_kiosk_device_not_found otherwise. NULL device and NULL location together are refused by name.',
   'argsruled_the_second_id_lives_in_the_same_organization.sql',
   'server_only: no client role holds EXECUTE on it. The client reaches this through public.hr_kiosk_pairing_code_create, which is the declared door.',
   false, false),
  ('hr', 'leave_enroll',
   'p_leave_policy_id uuid, p_employment_ids uuid[], p_effective_from date, p_override_reason text',
   array['uuid'::regtype, 'uuid[]'::regtype, 'date'::regtype, 'text'::regtype]::oid[],
   'p_leave_policy_id: resolved by hr._leave_policy_at and the caller must hold hr_admin or hr_owner in that policy''s organization. p_employment_ids: ARGS-RULED 2026-09-21 - every employment must be live and in the policy''s organization, and one that is not is named and skipped rather than enrolled. NULL policy answers not_found; a NULL array is the empty array and enrolls nobody.',
   'argsruled_the_second_id_lives_in_the_same_organization.sql',
   'server_only: no client role holds EXECUTE on it. The client reaches this through public.hr_leave_enroll, which is the declared door.',
   false, false)
