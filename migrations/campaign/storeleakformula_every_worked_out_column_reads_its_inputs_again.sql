...
--   would be its depends_on list, is left as it is and named; a LIVE one that must be RAISED is never
--   skipped — its refusal fails the file; an ARCHIVED one whose table is archived cannot be written
--   and cannot be read, and is raised by the trigger on the write that brings it back — named), and
--   each write leaves one `history.migration_log` row (verb
...
begin
  for r in select * from storeleakformula_repair order by organization_id, id loop
    ...
    exception when check_violation or invalid_parameter_value or raise_exception then
      ...
      -- An ARCHIVED column of an ARCHIVED table cannot be written at all (the shape guard asks for
      -- a live table), and that is also why it cannot leak: nothing reads a retired column, and the
      -- only way back to live is a write of this row with its table live again — which passes
      -- custom._field_reads_what_it_reads and is raised then. Named, never silently skipped.
      if v_raise and not r.archived then
      ...
      update storeleakformula_repair
         set outcome = 'left: ' || case when v_raise then 'archived; raised by the trigger when it comes back: ' else '' end || v_msg
       where organization_id = r.organization_id and id = r.id;
...
begin
  ...
  raise notice 'STORE-LEAK-FORMULA repair: % columns re-derived (% raised in sensitivity), % older definitions left as they are and named above',
    ...
    (select count(*) from storeleakformula_repair r
      where r.outcome not like 'left:%' and r.floor is not null
        and custom.sensitivity_rank(r.sensitivity_before) < custom.sensitivity_rank(r.floor)),
