...
-- based-on: platform.enforce_relation_edge() 9f5fac7fab8188cd9068c116592efac9c6a3854ce7010f7f84351ac760168cb5
-- based-on: custom.field_update(uuid, uuid, jsonb) 7744ff399ac3351659a6916f672f077ae4ad61e0832259f154aa3d6c9ccd5558
-- based-on: custom.field_retire(uuid, uuid) 70ccc58b96daa799fb151d73c33a579a99e057562ca91ff80aae4f02155d65fa
--
-- LANE RELATION-DECLARE, 2026-09-20 — UNMAKING A RELATION WAS ITSELF REFUSED BY THE RELATION
-- CONTRACT. Found by this lane's own green suite the first time it ran, at PART 5:
--
--   ERROR: the field Crew behaves as text, so it declares no relation
--   CONTEXT: platform.relation_declaration → platform.enforce_relation_edge
--            → custom.relation_edges_withdraw → custom.field_update
--
-- Two halves of one mistake, and they are the same mistake TABLE-DELETE recorded for records
-- ("a retirement is not a change of shape"), now for edges:
--
--   1. `platform.enforce_relation_edge` re-read the DECLARATION of every association row it
--      saw, INCLUDING a row being withdrawn. A withdrawal states no new fact — it takes one
--      back — so there is nothing there to check against a declaration, and demanding one
--      meant the last act of a relation's life could only succeed while the relation still
--      existed. GATE ZERO: a row that is not live is not enforced. An UNDELETE still is,
--      because that row becomes live and does state a fact again.
--
--   2. `custom.field_update` and `custom.field_retire` withdrew the links AFTER changing the
--      column, so even with gate zero they would have been asking about a field that had
--      already stopped being a relation. The links go FIRST, while the column still says what
--      they mean. That order is the honest one in its own right: you unmake the links, then
--      you change the column.
--
-- WHY IT IS ADDITIVE: three CREATE OR REPLACEs of bodies this file names by hash. No table,
-- column, policy, grant, row or signature changes. Nothing that was enforced before is
-- un-enforced: gate zero only skips rows that are already not live, which no reader counts.
--
-- ITS INVERSE: migrations/inverse/reldecl_unmaking_a_relation_is_not_a_claim_down.sql
...
comment on function platform.enforce_relation_edge() is
  'Held by the knob custom/system_enabled, resolved for the organization through platform.relations_are_on. A row that is not live is not enforced: a withdrawal states no fact to check.';
