...
-- additive: yes
--   It REPLACES four read-door bodies and changes ONE thing in each: every ORDER BY that
--   ordered a Table's records by `created_at desc` (or, for the archive, `deleted_at desc`)
--   now ends in `, r.id`. Nothing else in any body moves — the ladder, the masking, the
--   page ceiling, the filter and the answers are byte-for-byte what they were. No table,
--   column, trigger, policy, grant or row is touched. The inverse is
--   `migrations/inverse/gridport_a_page_of_records_is_always_the_same_page_down.sql`.
-- guard: custom/system_enabled
-- lock: custom
-- lane: GRID-PORT
-- based-on: custom.read_records_matching(uuid, uuid, jsonb, boolean, integer, integer) a92b93b3a4f9d5db3b82f36492aef1fe3c9092abe59dcc7b01fe72596679498d
-- based-on: custom.read_records(uuid, uuid, boolean, integer, integer) f926694f2d8ccb3647101f05612e9669d18026f413e45e88fdebf58581367d4c
-- based-on: custom.read_records_by_ids(uuid, uuid, uuid[], boolean) 730fce3850ccfd5f5bf480231c2487ba45961549064492727e32902957fcca53
-- based-on: custom.read_records_archived(uuid, uuid, text, boolean, integer, integer) 0c08c11f3d343669f5b7f1baae3c88519090596572fae8d00b70504cd3e0e878
--
-- LANE GRID-PORT (GRID-REBUILD.md) — finding F1.
--
-- A PAGE OF RECORDS WAS NOT ALWAYS THE SAME PAGE. The store's read doors order a Table's
-- records by `created_at desc` and nothing after it, which is a PARTIAL order: two records
-- created in the same statement tie, and Postgres may hand tied rows back in any order —
-- differently from one call to the next. The older store's page door ends every order with
-- `, id`, under the comment "the unique tiebreaker that makes this a TOTAL order. Do not
-- remove it." The store lost that sentence, and it matters twice:
--
--   1. PAGING SKIPS AND REPEATS. `limit/offset` over a partial order can put one record on
--      page 1 AND page 2 and another on neither. Every table OLD-TABLES-4 moved landed its
--      records in ONE transaction, so every moved record of a table shares one `created_at`
--      — the whole table is one tie. Grid Parity Fixture (120 rows) fits one 200-row page
--      today; the first moved table past a page would read wrong without anything raising.
--   2. THE SAME TABLE OPENS IN A DIFFERENT ORDER. The /data grid over the record store
--      (lane GRID-PORT) opened Grid Parity Fixture on Job 065; the older grid opened it on
--      Job 119, because the older door broke the tie by id.
--
-- `, r.id` makes every one of these orders total. For a moved table (one `created_at`) the
-- order becomes id order — exactly the older door's tie-break, so a moved table now opens
-- the way it did before the move whenever its rows were created together there too.
--
-- LOCKS. Four `create or replace function` statements: ACCESS SHARE on catalogue relations,
-- nothing on `custom.record`. Not window-class.
