...
--
-- based-on: custom.promote_field(uuid, uuid, uuid) a2de4b63b9ea08b2fd4acf9736a052fdd393d1b57cf8c1dac8b1781f96d7e0cd
-- based-on: custom.promoted_index_ddl(uuid, uuid) 0340a367275547c0df6964859bee26d2124f86c5983275ff0dd1ed9e52e34c5b
-- based-on: custom._promoted_field_cap_guard() 9add20c7b41963063dbab3b6632f33234ec9bb3cbcc27deff2826ed08a8339e5
-- based-on: custom.work_slots_declare(uuid, text, text, uuid) 98eb738af389cecb3d544b4121923fb23a5585ac86a77ac888ae4032c4e40a95
--
-- DOOR-FIX 4 — B1. PROMOTING A FIELD FOLLOWS THE ORGANIZATION'S SYSTEM SWITCH.
--
-- B1 (uniqueness under concurrency) could not be set up at all, because it needs a unique
-- index on a promoted Field and `custom.promote_field` refused: `custom/field_index_guard`
-- resolves false and there is no override for it anywhere on the main database. Meanwhile
-- `custom/system_enabled` — the switch that actually says whether an organization is on the
-- record store — carries organization overrides. A per-object knob with no route to being
-- turned on is not a switch; it is an outage with a name, and it was holding back a rule the
-- store is supposed to enforce.
--
-- The four places that read `custom/field_index_guard` now read the organization's own system
-- switch through `custom.store_is_open`, which is the same read every other door in this
-- schema already makes and which treats a switch it cannot READ as closed. Nothing is relaxed:
-- an organization whose store is off still cannot promote, and the cap on promoted fields per
-- table is refused whether the switch is on or off. What changes is that an organization whose
-- store is ON can now promote a field, which is what B1 needs and what a person would expect.
--
-- `custom/field_index_guard` itself is left in `platform.feature_knob` and relabelled, because
-- a knob row that still claims to govern promotion while nothing reads it would be a lie on
-- the settings screen.
--
-- INVERSE: migrations/inverse/doorfix_promoting_a_field_follows_the_store_switch_down.sql
...
-- The retired knob row is relabelled in
-- migrations/campaign/doorfix_the_retired_promotion_knob_says_so.sql, which carries no
-- `-- target:` header because an UPDATE of a registry row is not one of the enumerated
-- additive shapes and does not pretend to be.
