-- dd186_anon_columns_pdf — THE SIGNED-OUT COLUMN SURFACE OF THE `pdf` SCHEMA
...
-- pdf.pdf_redaction_audits — revoked: organization_id, created_by, updated_by, metadata, version
revoke select on pdf.pdf_redaction_audits from anon;
grant select (id, parent_file_id, file_id, reason, redaction_kind, redaction_params, tier_used, status, bytes_removed_estimate, regions_count, created_at, updated_at, deleted_at, visibility) on pdf.pdf_redaction_audits to anon;

-- pdf.pdf_redaction_key_escrow — revoked: owner_id, organization_id
revoke select on pdf.pdf_redaction_key_escrow from anon;
grant select (id, session_id, file_id, wrapped_key, wrap_alg, created_at, revoked_at) on pdf.pdf_redaction_key_escrow to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['pdf.pdf_redaction_audits','pdf.pdf_redaction_key_escrow']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (pdf): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
