-- dd226_transcripts_publishes_nothing_to_a_signed_out_reader
...
--   transcripts.studio_cleaned_segments — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 12)); the grant itself is the finding.
--   transcripts.studio_concept_items — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 11)); the grant itself is the finding.
--   transcripts.studio_module_segments — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 10)); the grant itself is the finding.
--   transcripts.studio_raw_segments — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 10)); the grant itself is the finding.
--   transcripts.studio_session_settings — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 15)); the grant itself is the finding.
--   transcripts.studio_sessions — Latent (the grant is open; no row is on the public side of the gate today). RED, one row flipped to visibility='public' inside a rolled-back transaction and read AS ANON: {"id":"360299c2-e2ce-4dd2-a292-6590190d4c81","project_id":null,"transcript_id":null,"title":"Recording","status":"idle","module_id":"tasks","started_at":"2026-07-23T18:11:49.971Z","ended_at":null,"total_duration_ms":0,"audio_storage_path":null,"created_at":"20…
--   transcripts.transcripts — Latent (the grant is open; no row is on the public side of the gate today). RED, one row flipped to visibility='public' inside a rolled-back transaction and read AS ANON: {"id":"9b7acc6c-0e67-4a25-aff4-8a6f77dca657","title":"Expert Voices Research 11th And 12th Pillars","description":"","segments":[{"id":"1784870081439","text":"I think the concept you have here is absolutely perfect. I do definitely want to want the 11th and 12…
--
...
begin
  for r in
    select unnest(array[
      'transcripts.studio_cleaned_segments',
      'transcripts.studio_concept_items',
      'transcripts.studio_module_segments',
      'transcripts.studio_raw_segments',
      'transcripts.studio_session_settings',
      'transcripts.studio_sessions',
