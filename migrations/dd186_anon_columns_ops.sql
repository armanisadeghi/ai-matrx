-- dd186_anon_columns_ops — THE SIGNED-OUT COLUMN SURFACE OF THE `ops` SCHEMA
...
-- ops.app_log — revoked: user_id, metadata, created_by, updated_by
revoke select on ops.app_log from anon;
grant select (id, ts, level, level_no, logger_name, feature, route, message, exc_type, traceback, request_id, conversation_id, process_pid, host_role, classified, created_at, stage) on ops.app_log to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['ops.app_log']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (ops): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
