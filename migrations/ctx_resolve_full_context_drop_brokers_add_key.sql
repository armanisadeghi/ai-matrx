-- ctx_resolve_full_context_drop_brokers_add_key.sql
--
-- Two changes to resolve_full_context, in one redefinition:
--  1) REMOVE the dead broker loop (ctx_context_variables — verified 0 rows, 0 active). The
--     broker system is retired; everything unifies on Context Items.
--  2) ADD `key` to each cell_values entry. The Python ambient layer overrides computed
--     System items (current_date, etc.) by KEY, and cell_values is keyed by UUID — so it
--     needs the item key to match a registered ambient provider.
--
-- Nothing else changes: scope cells + system cells still populate `variables` (keyed by key)
-- and `cell_values` (keyed by context_item_id UUID). Idempotent.
