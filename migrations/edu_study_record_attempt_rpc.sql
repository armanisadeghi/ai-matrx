-- study_record_attempt: the canonical, mode-agnostic write point for studying ANY item.
-- Atomically (a) appends an immutable study_attempt ledger row and (b) upserts the learner's
-- item_mastery (per created_by,item_type,item_id) with a sound box→interval scheduler. Returns
-- the attempt id + the fresh mastery row, so the client never re-reads stale state (the old
-- stale-read bug is structurally impossible). FSRS (lib/srs/fsrs.ts) can supersede the scheduler
-- without changing this contract. SECURITY DEFINER (writes two tables as the acting user).
...
create or replace function public.study_record_attempt(
  ...
  p_result text default null,                 -- 'correct' | 'partial' | 'incorrect'
  p_score jsonb default null,
  p_score_value numeric default null,         -- normalized 0..1
  p_response_kind text default null,          -- 'spoken'|'written'|'typed'|'handwritten'|'selected'
