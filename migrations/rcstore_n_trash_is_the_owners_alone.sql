...
--
-- ONE rule: platform.trash_hides(token, deleted_at, owner, user), for tokens that declare it in
-- platform.trash_is_owner_only (today: document). It is asked in the three places every content
-- door reads through:
--   iam.has_access_for_base   the kernel — DEFINER doors (univer_save, version_publish/restore,
--                             realtime_topic_admits, comments and associations on the document)
--   iam.accessible_entity_ids its set form — the read policies of content.document_version and
--                             content.univer_payload (version_list / version_get / version_diff)
--   iam.entity_read_expr      the generated std_select of content.document (document_get,
--                             find_document, local_sync_changes, search_documents, annotation_create)
...
as $$
  ...
  select p_token = any (array['document'])
$$;
comment on function platform.trash_is_owner_only(text) is
  'RC-A1: the tokens whose trashed rows are read by their owner only (and the platform admin lane). Asked by platform.trash_hides.';

create function platform.trash_hides(p_token text, p_deleted_at timestamptz, p_owner uuid, p_user uuid)
returns boolean
language sql
immutable
parallel safe
set search_path = pg_catalog
as $$
  select platform.trash_is_owner_only(p_token) and p_deleted_at is not null and p_owner is distinct from p_user
$$;
comment on function platform.trash_hides(text, timestamptz, uuid, uuid) is
  'RC-A1: THE trash rule — true when a row of a declared token is in its owner''s trash and the person asking is not its owner. Asked by the access kernel, its set form and the generated read policy.';
grant execute on function platform.trash_is_owner_only(text) to anon, authenticated, service_role;
grant execute on function platform.trash_hides(text, timestamptz, uuid, uuid) to anon, authenticated, service_role;
...
-- The kernel changed, so its proved fingerprint moves with it in the same transaction: the bump is
-- what keeps iam.entity_read_expr emitting the BOUNDED (indexable) read lane platform-wide. The
