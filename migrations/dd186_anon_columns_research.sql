-- dd186_anon_columns_research — THE SIGNED-OUT COLUMN SURFACE OF THE `research` SCHEMA
...
-- research.rs_template — revoked: is_system, created_by, metadata, organization_id, updated_by, version
revoke select on research.rs_template from anon;
grant select (id, name, description, keyword_templates, default_tags, default_search_params, agent_config, autonomy_level, created_at, updated_at, visibility, deleted_at, intent_key) on research.rs_template to anon;

-- research.rs_topic — revoked: metadata, created_by, organization_id, updated_by, version
revoke select on research.rs_topic from anon;
grant select (id, name, autonomy_level, default_search_provider, default_search_params, good_scrape_threshold, scrapes_per_keyword, status, template_id, agent_config, created_at, updated_at, description, max_keywords, analyses_per_keyword, max_keyword_syntheses, max_topic_syntheses, max_documents, max_tag_consolidations, max_auto_tag_calls, tone_profile, outputs, tag_suggestions, visibility, deleted_at, videos_per_keyword, intent_key, intent_brief, refresh_interval_hours, next_refresh_at, refresh_claim_token, refresh_claim_expires_at, last_refresh_at, last_refresh_outcome, last_refresh_error, last_refresh_trigger, consecutive_refresh_failures) on research.rs_topic to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['research.rs_template','research.rs_topic']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (research): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
