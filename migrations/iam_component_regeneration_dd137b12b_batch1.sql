-- iam_component_regeneration_dd137b12b_batch1 — REGENERATION BATCH 1 OF 4 (DD-137b, fix round 1).
--
...
begin
  ...
  loop
    ...
    exception when others then
      ...
      if sqlstate in ('P0001','42703','42883')
        ...
        raise notice 'dd137b12b: % (%.%) keeps its bespoke policy — %',
          r.token, r.schema_name, r.table_name, sqlerrm;
      else
        raise exception 'dd137b12b: % (%.%) failed with %: %. That is not a structural refusal, '
  ...
  raise notice 'dd137b12b: batch 1/4 — regenerated %, structurally refused %', v_ok, v_refused;
  if v_ok + v_refused < 80 then
    raise exception 'dd137b12b: only % of 80 tokens were reached — a silent drop is exactly '
