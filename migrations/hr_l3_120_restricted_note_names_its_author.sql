-- hr_l3_120 — A RESTRICTED NOTE NAMES ITS AUTHOR.
--
...
begin
  ...
  -- hr_l3_120: the row's AUTHOR, on its own key. A note or a piece of guidance names the person
...
insert into hr.function_contract
  (schema_name, function_name, home_migration, must_contain, must_not_contain, reason,
   must_be_definer)
select 'hr', '_project_row', 'hr_l3_120',
       array['subject_employment_id','author_employment_id','_subject_display_name'],
       array['directory_opt_out','_punch_capability','p_token ='],
       'hr_l3_120: this is the ONE projection every audited HR door returns through. It resolves '
    || 'a person''s name for BOTH shapes a table can carry — subject (hr_l3_41) and author '
    || '(hr_l3_120) — and it resolves neither itself: both go through hr._subject_display_name '
...
begin
  ...
  if v_txt not like '%author_employment_id%' then
    raise exception 'hr_l3_120: hr._project_row has no author branch';
  end if;
  if v_txt like '%directory_opt_out%' or v_txt like '%_punch_capability%' then
    raise exception 'hr_l3_120: hr._project_row re-implements the suppression rule';
  end if;
  ...
  if v_note.id is null then
    raise notice 'hr_l3_120: no live hr.restricted_note row to project — payload proof skipped';
  else
    v_row := hr._project_row('hr_restricted_note','hr','restricted_note', v_note.id);
    if not (v_row ? 'author_name') then
      raise exception 'hr_l3_120: the projected restricted_note carries no author_name key';
    end if;
    if v_row ->> 'author_name' = v_note.author_employment_id::text then
      raise exception 'hr_l3_120: author_name is the uuid — a name is a name or it is absent';
    end if;
    ...
    if v_login is not null then
      ...
      if v_name is null then
        raise exception 'hr_l3_120: the author cannot resolve their own name through the display door';
      end if;
    end if;
  end if;
  ...
  if v_inc_id is null then
    raise notice 'hr_l3_120: no live hr.incident with a subject — subject-branch proof skipped';
  else
    v_row := hr._project_row('hr_incident', v_schema, v_table, v_inc_id);
    if not (v_row ? 'subject_name') then
      raise exception 'hr_l3_120: hr._project_row stopped emitting subject_name';
    end if;
  end if;
  ...
  if not exists (select 1 from hr.function_contract
    ...
    raise exception 'hr_l3_120: no active hr.function_contract row for hr._project_row';
  end if;
  select count(*) into v_n from hr.function_contracts_broken();
  if v_n > 0 then
    raise exception 'hr_l3_120: hr.function_contracts_broken() returns % row(s)', v_n;
  end if;
  ...
  if has_function_privilege('authenticated','hr._project_row(text,text,text,uuid)','execute')
    ...
    raise exception 'hr_l3_120: hr._project_row is executable by a client role';
  end if;

  raise notice 'hr_l3_120: author branch live, name resolves through the one display door, subject branch intact.';
end
$proof$;
