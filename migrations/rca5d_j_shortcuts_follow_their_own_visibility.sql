...
--
-- THE DEFECT (latent on production 2026-09-26: 0 personal organization shortcuts yet): agx_get_shortcuts_initial
-- listed every shortcut of every organization the caller belongs to; agx_get_user_shortcuts listed every shortcut
-- of an organization (or project) the caller administers; agx_get_shortcuts_for_context listed every shortcut
-- attached to a project/task the caller may open. A PERSONAL shortcut of another member rode along — its label,
-- description, prompts, variables and agent id — on organization membership (tenancy, never permission).
--
-- THE FIX: each loader asks the kernel's set form for agent_shortcut ONCE (iam.accessible_entity_ids, ~15 ms) and
-- the membership / administration / containment arms admit only shortcuts in it. The caller's own shortcuts, the
-- system shortcuts and direct grants are unchanged. The mandate-backed twins (_m) read mandate definitions and are
-- not touched here.
-- Inverse: migrations/inverse/rca5d_j_shortcuts_follow_their_own_visibility_down.sql.
-- based-on: public.agx_get_shortcuts_initial() e51738e34b6b5a64bacdc80201aadff316b614aa6cac6dd2cfa9b8395a3e1829
-- based-on: public.agx_get_shortcuts_for_context(uuid, uuid) f6614d4d2a6b9b1bb0d37e5bc41b20af57ab6d773fad4a7a3fbd5495ee5bf3e6
-- based-on: public.agx_get_user_shortcuts() e59cfbb342d7921f3b22227c2b7fe862a14430422b12d23d7f073c2560815350
...
AS $function$
DECLARE
  -- rca5d_j: the shortcuts the caller may open, asked of the kernel once.
  v_sc_ids uuid[] := iam.accessible_entity_ids('agent_shortcut', 'viewer'::public.permission_level);
BEGIN
  RETURN QUERY
  ...
  WHERE s.is_active = true
    ...
    AND (
      ...
      OR (s.organization_id IN (SELECT mo.organization_id FROM my_orgs mo) AND s.id = ANY(v_sc_ids))  -- rca5d_j
    )
...
BEGIN
  ...
  WHERE s.is_active = true
    AND (
      ...
      (p_project_id IS NOT NULL AND sp.target_id = p_project_id AND iam.assoc_side_readable('project', sp.target_id) AND s.id = ANY(v_sc_ids))
      OR (p_task_id IS NOT NULL AND st.target_id = p_task_id AND iam.assoc_side_readable('task', st.target_id) AND s.id = ANY(v_sc_ids))
      OR EXISTS (
...
begin
  ...
  where s.created_by = v_uid
     or (s.organization_id in (select om.organization_id from iam.organization_member om
        where om.user_id = v_uid and om.role in ('owner','admin')) and s.id = any(v_sc_ids))  -- rca5d_j
     or (sp.target_id in (select m.container_id from iam.memberships m
        where m.container_type='project' and m.user_id = v_uid and m.deleted_at is null and m.role in ('owner','admin')) and s.id = any(v_sc_ids))  -- rca5d_j
