...
-- read_lane_v2_b3_57_platform_assists — enroll platform.assists in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'platform' and table_name = 'assists' and is_active on conflict do nothing;
