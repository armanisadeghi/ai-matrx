-- dd186_anon_columns_public — THE SIGNED-OUT COLUMN SURFACE OF THE `public` SCHEMA
...
-- public.app_config — revoked: updated_by, organization_id, metadata, created_by, version
revoke select on public.app_config from anon;
grant select (app, schema_version, min_supported_app_version, config, updated_at, id, created_at, visibility) on public.app_config to anon;

-- public.catalog_entries — revoked: (none; bounded so a new column is closed by default)
revoke select on public.catalog_entries from anon;
grant select (id, app, kind, key, schema_version, payload, artifact_url, artifact_sha256, artifact_size_bytes, min_app_version, is_active, sort_order, notes, updated_at) on public.catalog_entries to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['public.app_config','public.catalog_entries']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (public): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
