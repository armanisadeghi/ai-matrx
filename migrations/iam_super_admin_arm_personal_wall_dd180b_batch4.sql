-- iam_super_admin_arm_personal_wall_dd180b_batch4 — DD-180 REGENERATION BATCH 4 OF 7 (24 tokens).
--
...
declare
  ...
  v_tokens text[] := array[
    'hr_provider_binding','hr_record_class','hr_retention_rule','hr_workflow_definition',
    'hr_workflow_flow_type','iam_api_key','invitation_code','library_doc','mandate',
    'mandate_treatment','marketing_initiative','meet_meeting','message_template','note','note_folder',
    'notification_event_override','notification_event_type','ops_proof_check','ops_proof_scenario',
...
begin
  ...
  loop
    ...
    if not (v_pre -> r.token ? 'super_admin_system_org_arm_walled') then
      raise exception 'dd180b4: % does not FAIL super_admin_system_org_arm_walled before this file runs. It is not in the RED set this batch was written against; re-census.', r.token;
    end if;
  end loop;

  -- ═══════ 2. THE BEFORE, pinned to this transaction's instant ═════════════════════════════════
  v_before := iam.access_delta_snapshot('DD-180 b4 BEFORE', v_principals, v_tokens, 400000,
    'DD-180 batch 4: tokens carrying the UNWALLED system-org super-admin read arm, before regeneration', v_as);
  ...
  v_after := iam.access_delta_snapshot('DD-180 b4 AFTER', v_principals, v_tokens, 400000,
    'DD-180 batch 4 confirmation, pinned to the same instant as the baseline', v_as);
  ...
  loop
    if r.verdict = 'WIDER' then
      if not ((r.token || '|' || r.principal_label) = any (v_approved)) then
        ...
        raise notice 'dd180b4: WIDER (approved by name) % for % : % -> %', r.token, r.principal_label, r.count_before, r.count_after;
      end if;
      ...
      v_narrower := array_append(v_narrower,
        format('%s for %s (%s -> %s)', r.token, r.principal_label, r.count_before, r.count_after));
      raise notice 'dd180b4: NARROWED % for % : % -> %', r.token, r.principal_label, r.count_before, r.count_after;
    end if;
  end loop;
  if cardinality(v_hard) > 0 then
    raise exception 'dd180b4: % read(s) this gate could not prove either way: %', cardinality(v_hard), array_to_string(v_hard, ' ; ');
  end if;
  if cardinality(v_unapproved) > 0 then
    raise exception 'dd180b4: % door(s) opened that nobody approved by name: %. Over-opening and over-tightening are the same class of defect and neither ships on a count.',
  ...
  if cardinality(v_stillred) > 0 then
    raise exception 'dd180b4: % token(s) still FAIL super_admin_system_org_arm_walled after regeneration: %. The generator did not wall the arm this batch exists to wall.',
      cardinality(v_stillred), array_to_string(v_stillred, ', ');
  end if;
  if cardinality(v_newfail) > 0 then
    raise exception 'dd180b4: % verify_canonical FAIL(s) that were NOT there before this file ran: %. A regeneration that introduces a finding does not stand.',
      cardinality(v_newfail), array_to_string(v_newfail, ' ; ');
  end if;
  raise notice 'dd180b4: 0 unapproved widenings, % narrowing(s), 0 new verify_canonical FAILs, % token(s) FAIL -> PASS on super_admin_system_org_arm_walled',
