...
-- read_lane_v2_b3_63_transcripts_studio_sessions — enroll transcripts.studio_sessions in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'transcripts' and table_name = 'studio_sessions' and is_active on conflict do nothing;
