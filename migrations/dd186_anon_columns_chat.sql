-- dd186_anon_columns_chat — THE SIGNED-OUT COLUMN SURFACE OF THE `chat` SCHEMA
...
-- These 4 relations are anon-readable ON PURPOSE: each carries a permissive SELECT policy
...
-- chat.agent_memory — revoked: metadata, organization_id, created_by, updated_by, version
revoke select on chat.agent_memory from anon;
grant select (id, memory_type, scope, scope_id, key, content, importance, access_count, last_accessed_at, expires_at, created_at, updated_at, deleted_at, visibility) on chat.agent_memory to anon;

-- chat.agent_run — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on chat.agent_run from anon;
grant select (id, kind, status, input_fingerprint, request, result, error, total_cost, created_at, updated_at, episode_id, last_heartbeat_at, deleted_at, visibility) on chat.agent_run to anon;

-- chat.conversation — revoked: metadata, organization_id, created_by, updated_by, version
revoke select on chat.conversation from anon;
grant select (id, title, system_instruction, config, status, message_count, forked_from_id, forked_at_position, created_at, updated_at, deleted_at, last_model_id, parent_conversation_id, variables, overrides, description, keywords, task_id, source_app, source_feature, is_ephemeral, initial_agent_id, initial_agent_version_id, is_favorite, cache_state, last_context_breakdown, sandbox_instance_id, last_request_status, last_request_id, app_instance_id, exclude_from_kg, conversation_type, visibility, origin_class) on chat.conversation to anon;

-- chat.user_request — revoked: metadata, organization_id, created_by, updated_by, version
revoke select on chat.user_request from anon;
grant select (id, total_input_tokens, total_output_tokens, total_cached_tokens, total_tokens, total_cost, total_duration_ms, api_duration_ms, tool_duration_ms, iterations, total_tool_calls, status, finish_reason, error, created_at, completed_at, deleted_at, source_app, source_feature, agent_id, agent_version_id, last_activity_at, updated_at, origin_class, origin_witness, visibility) on chat.user_request to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['chat.agent_memory','chat.agent_run','chat.conversation','chat.user_request']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (chat): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
