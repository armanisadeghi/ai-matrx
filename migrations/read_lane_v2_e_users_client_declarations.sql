...
--   * users.credential_attachments and users.integration_connections: no client and no lane-admin hard
--     delete — archive, never delete (Arman's standing law; chair 2026-09-26). The declaration issues no
--     DELETE grant, so neither a client policy nor the generated admin lane (platform_admin_all) can
--     hard-delete; apply_table_grants asserts it. Admin archive (an UPDATE of deleted_at) is untouched.
...
begin
  ...
  update platform.entity_types
   ...
   where token = 'integration_connection' and schema_name = 'users' and table_name = 'integration_connections'
     and is_active and not client_deletes_refused;
  get diagnostics n = row_count;
  if n <> 1 then raise exception 'read_lane_v2_e: integration_connection deletes refused: touched % rows, expected 1', n; end if;
end $$;
