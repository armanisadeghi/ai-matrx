-- dd186_anon_columns_workspace — THE SIGNED-OUT COLUMN SURFACE OF THE `workspace` SCHEMA
...
-- workspace.projects — revoked: created_by, organization_id, metadata, version, updated_by
revoke select on workspace.projects from anon;
grant select (id, name, description, created_at, updated_at, slug, settings, status, priority, start_date, target_date, deleted_at, visibility) on workspace.projects to anon;

-- workspace.tasks — revoked: organization_id, created_by, version, metadata, updated_by
revoke select on workspace.tasks from anon;
grant select (id, title, description, project_id, status, due_date, created_at, updated_at, parent_task_id, priority, assignee_id, settings, visibility, deleted_at, completed_at, origin, source_type, source_id, source_url, source_label, dedupe_key, start_date, due_time, timezone, recurrence_rule, reminders) on workspace.tasks to anon;

-- workspace.threads — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on workspace.threads from anon;
grant select (id, active_tab, position, title, created_at, updated_at, anchor_type, anchor_id, visibility, deleted_at) on workspace.threads to anon;

-- workspace.war_rooms — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on workspace.war_rooms from anon;
grant select (id, title, description, icon, color, active_thread_id, last_opened_at, created_at, updated_at, anchor_type, anchor_id, visibility, deleted_at) on workspace.war_rooms to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['workspace.projects','workspace.tasks','workspace.threads','workspace.war_rooms']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (workspace): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
