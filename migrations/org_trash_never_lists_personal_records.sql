-- An organization's Trash never lists a member's PERSONAL records (their private highlights and notes).
-- Found by verify RC-B11 round 3: private annotation documents (visibility personal, filed in the
-- member's organization) appeared by title — the quoted passage — in the organization Trash that the
-- organization's admins read. _trash_kind_rows' organization mode had no visibility rule. Personal Trash
-- (the owner's own) is unchanged.
