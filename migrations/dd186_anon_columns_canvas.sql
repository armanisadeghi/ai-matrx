-- dd186_anon_columns_canvas — THE SIGNED-OUT COLUMN SURFACE OF THE `canvas` SCHEMA
...
-- canvas.canvas_items — revoked: user_id, organization_id, version, created_by, updated_by, metadata
revoke select on canvas.canvas_items from anon;
grant select (id, type, content, title, description, is_favorited, is_archived, tags, session_id, source_message_id, task_id, is_public, created_at, updated_at, last_accessed_at, content_hash, project_id, conversation_id, artifact_index, parent_canvas_id, source_type, external_system, external_id, deleted_at, visibility, source_system, source_id) on canvas.canvas_items to anon;

-- canvas.shared_canvas_items — revoked: created_by, organization_id, updated_by, version, metadata
revoke select on canvas.shared_canvas_items from anon;
grant select (id, title, description, canvas_type, canvas_data, thumbnail_url, creator_username, creator_display_name, original_id, forked_from, version_number, fork_count, view_count, like_count, share_count, comment_count, play_count, completion_rate, has_scoring, high_score, high_score_user, average_score, total_attempts, visibility, allow_remixes, require_attribution, featured, tags, categories, created_at, updated_at, published_at, last_played_at, trending_score, search_vector, deleted_at) on canvas.shared_canvas_items to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['canvas.canvas_items','canvas.shared_canvas_items']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (canvas): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
