...
-- read_lane_v2_b3_56_platform_assist_producer_policy — enroll platform.assist_producer_policy in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'platform' and table_name = 'assist_producer_policy' and is_active on conflict do nothing;
