-- dd186_anon_columns_extend — THE SIGNED-OUT COLUMN SURFACE OF THE `extend` SCHEMA
...
-- extend.wbx_capture — revoked: metadata, organization_id, created_by, updated_by, version
revoke select on extend.wbx_capture from anon;
grant select (id, url, captured_at, title, description, lang, soup, markdown, ld_json, media_count, pattern_id, created_at, updated_at, deleted_at, visibility) on extend.wbx_capture to anon;

-- extend.wbx_demo — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on extend.wbx_demo from anon;
grant select (id, name, description, start_url, step_count, parameter_names, body, is_deleted, created_at, updated_at, deleted_at, visibility) on extend.wbx_demo to anon;

-- extend.wbx_guidance — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on extend.wbx_guidance from anon;
grant select (id, domain, kind, caption, origin_url, data, created_at, updated_at, is_deleted, deleted_at, visibility) on extend.wbx_guidance to anon;

-- extend.wbx_highlight — revoked: metadata, organization_id, created_by, updated_by, version
revoke select on extend.wbx_highlight from anon;
grant select (id, conversation_id, mode, url, domain, page_title, color, text, anchor, is_deleted, created_at, updated_at, deleted_at, visibility) on extend.wbx_highlight to anon;

-- extend.wbx_pattern — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on extend.wbx_pattern from anon;
grant select (id, name, domain, route_pattern, list_root_selector, fields, last_used_at, created_at, kind, config, target_user_table_id, last_run_at, last_status, last_run_count, updated_at, deleted_at, visibility) on extend.wbx_pattern to anon;

-- extend.wbx_recipe — revoked: (none; bounded so a new column is closed by default)
revoke select on extend.wbx_recipe from anon;
grant select (id, label, description, hosts, routes, kind, config, yields_rows, is_active, last_verified_at, created_at, updated_at) on extend.wbx_recipe to anon;

-- extend.wbx_screenshot — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on extend.wbx_screenshot from anon;
grant select (id, page_url_canonical, page_url_full, page_title, file_id, file_url, width, height, mime_type, byte_length, source, captured_at, created_at, updated_at, deleted_at, visibility) on extend.wbx_screenshot to anon;

-- extend.wbx_seo_audit — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on extend.wbx_seo_audit from anon;
grant select (id, url, audited_at, signals, recommendations, flesch_reading_ease, word_count, notes, created_at, updated_at, deleted_at, visibility) on extend.wbx_seo_audit to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['extend.wbx_capture','extend.wbx_demo','extend.wbx_guidance','extend.wbx_highlight','extend.wbx_pattern','extend.wbx_recipe','extend.wbx_screenshot','extend.wbx_seo_audit']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (extend): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
