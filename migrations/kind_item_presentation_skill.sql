...
-- Keep body/label/etc. fresh on re-apply (idempotent update).
UPDATE skill.definition AS d SET
    label            = 'Item Card (__kind)',
    description      = 'How and when to emit item_presentation as a content-ir __kind block: the flat { "__kind": "item_presentation", "type", "id", ... } shape, the type enum, click-to-open enrichment, and editing etiquette.',
    icon_name        = 'MousePointerClick',
    platform_targets = '["web"]'::jsonb,
    category_id      = '49c845cb-9314-485c-88ed-a7ace4f286ca'::uuid,
    is_system        = true,
    is_public        = true,
    is_active        = true,
    visibility       = 'public'::platform.visibility,
    body             = $BODY$# Item Card — content-ir `__kind` block (`item_presentation`)
...
```
$BODY$
WHERE d.skill_id = 'item-presentation-kind'
