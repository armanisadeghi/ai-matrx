...
--
-- 2. seo.gsc_perf_class_movers gained `p_filters jsonb` (today: `{"levels":
--    ["platinum", ...]}`) and a `value_band` output column in this change, so
--    the Traffic quality view can decompose by value LEVEL beside traffic
--    CLASS and filter the movers to one level — the "site flat while Platinum
--    fell" headline. Levels resolve through seo.keyword_value_map, the ONE
--    resolver; a band is never re-derived.
--
--    That function body has since MOVED to `seo_class_map_scope_fix.sql`,
--    which keeps every line of the level work and adds the scoped
--    `gsc_keyword_class_map` call the `page` dimension needs to stay inside
--    the 8 s statement timeout. It is the current definition; re-running the
--    body from here would silently undo that fix, so it does not live here
--    any more. ONE file owns a function at a time.
--
