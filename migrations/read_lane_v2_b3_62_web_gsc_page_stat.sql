...
-- read_lane_v2_b3_62_web_gsc_page_stat — enroll web.gsc_page_stat in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'web' and table_name = 'gsc_page_stat' and is_active on conflict do nothing;
