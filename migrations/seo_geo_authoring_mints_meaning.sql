...
-- Identical body to C1's seo.fn_evaluate_matchers, minus the editor assert, so
-- triggers and backfills can run the ONE engine rather than a copy of it.
CREATE OR REPLACE FUNCTION seo.fn_evaluate_matchers_internal(p_site_id uuid, p_keyword_ids uuid[] DEFAULT NULL)
RETURNS jsonb
...
BEGIN
  ...
  IF v_val IS NULL THEN
    ...
    IF v_existing_slug_owner IS NOT NULL AND v_existing_slug_owner <> p_area_id::text THEN
