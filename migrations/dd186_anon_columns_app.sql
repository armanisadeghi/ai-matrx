-- dd186_anon_columns_app — THE SIGNED-OUT COLUMN SURFACE OF THE `app` SCHEMA
...
-- These 1 relations are anon-readable ON PURPOSE: each carries a permissive SELECT policy
...
-- app.definition — revoked: organization_id, version, metadata, created_by, updated_by
revoke select on app.definition from anon;
grant select (id, project_id, task_id, agent_id, agent_version_id, use_latest, slug, name, tagline, description, category, tags, component_code, component_language, allowed_imports, variable_schema, layout_config, styling_config, app_kind, shared_context_policies, preview_image_url, favicon_url, status, is_featured, is_verified, rate_limit_per_ip, rate_limit_window_hours, rate_limit_authenticated, pinned_version, total_executions, total_tokens_used, total_cost, unique_users_count, success_rate, avg_execution_time_ms, last_execution_at, created_at, updated_at, published_at, search_tsv, shell_kind, shell_config, slot_overrides, slot_code, visibility, deleted_at, mandate_id) on app.definition to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['app.definition']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (app): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
