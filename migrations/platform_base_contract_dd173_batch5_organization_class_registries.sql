-- platform_base_contract_dd173_batch5_organization_class_registries — DD-173 BATCH 5 (6 tokens).
--
-- THE SIX: the assist producer policy, the change-type defaults the workflow approval ladder reads
-- (42 rows), the knob scope kinds the settings resolver keys on, the SEO AI capability list, the
-- growth stage reference kinds and the platform taxonomy nodes. All `rls_variant = system`,
-- `data_class = organization`, registered by DD-159 batch 1, refused by `iam.apply_rls` ever since.
--
-- 🚨 VISIBILITY IS 'internal', NOT 'public'. None of the six is readable signed-out today, and the
--    §6e system-org arm in the generated `std_select` admits `visibility >= 'internal'` on a
--    global-readable organization — so 'internal' reproduces exactly today's reach (every
--    signed-in reader sees them, anonymous sees none) while 'public' would have published six
--    internal registries to the open web. Batch 4 records the measurement that taught this: a
--    flat 'public' on the announcement banner moved anonymous 0 -> 1 and the gate refused it.
--
-- 🚨 `platform.retention_policy` WAS THE SEVENTH AND IS HELD BACK ON A CHECK CONSTRAINT THAT
--    ENCODES THE BANNED PATTERN. Its `retention_policy_scope_addressing` constraint REQUIRES
--    `organization_id IS NULL` for a `scope = 'global'` row, so setting the system org on those
--    rows raises 23514. That is the seo NULL-lane shape db-rules §2 closed by ruling ("NULL is not
--    a scope... this closes the seo NULL-lane question, the answer is NO") living on inside a
--    CHECK. Paying it means giving the global rows the system org AND rewriting the constraint in
--    one migration with its own proof, which is a different change from this one. Written up in
--    the B-65 report.
--
...
declare
  ...
  v_tokens text[] := array[
    'assist_producer_policy','change_type_default','knob_scope_kind','seo_ai_capability',
    'stage_ref_kind','taxonomy_node'];
  v_plan constant text[][] := array[
    array['assist_producer_policy','platform','assist_producer_policy', $x$'internal'$x$],
    array['change_type_default','platform','change_type_default',       $x$'internal'$x$],
    array['knob_scope_kind','platform','knob_scope_kind',               $x$'internal'$x$],
    array['seo_ai_capability','seo','ai_capability',                    $x$'internal'$x$],
    array['stage_ref_kind','growth','stage_ref_kind',                   $x$'internal'$x$],
    array['taxonomy_node','platform','taxonomy_node',                   $x$'internal'$x$]];
  v_bespoke constant text[] := array[
    'assist_producer_policy|assist_producer_policy_read,platform_admin_delete_only,platform_admin_insert_only,platform_admin_update_only',
    'change_type_default|ctd_read,ctd_service,platform_admin_delete_only,platform_admin_insert_only,platform_admin_update_only',
    'knob_scope_kind|knob_scope_kind_read',
    'seo_ai_capability|ai_capability_read',
    'stage_ref_kind|all_read,platform_admin_delete_only,platform_admin_insert_only,platform_admin_update_only',
    'taxonomy_node|platform_admin_delete_only,platform_admin_insert_only,platform_admin_update_only,taxonomy_node_read'];
...
begin
  ...
  v_before := iam.access_delta_snapshot('DD-173 b5 BEFORE', v_principals, v_tokens, 400000,
    'DD-173 batch 5: the ten public reference catalogs, before the base retrofit and generation', v_as);
  raise notice 'dd173b5: baseline % over % tokens x % principals, pinned at %',
  ...
  foreach f in array v_bespoke loop
    ...
    if v_extra <> '{}' then
      raise exception
        'dd173b5: %.% carries hand-written policy/policies % that this file has never seen. Nothing was dropped. A policy nobody enumerated is not superseded on a guess — re-census and re-write this file.',
  ...
  loop
    if r.verdict = 'WIDER' then
      if not ((r.token || '|' || r.principal_label) = any (v_approved)) then
        ...
        raise notice 'dd173b5: WIDER (approved by name) % for % : % -> %', r.token, r.principal_label, r.count_before, r.count_after;
      end if;
      ...
      v_narrower := array_append(v_narrower,
        format('%s for %s (%s -> %s)', r.token, r.principal_label, r.count_before, r.count_after));
      raise notice 'dd173b5: NARROWED % for % : % -> %', r.token, r.principal_label, r.count_before, r.count_after;
    end if;
  end loop;
  if cardinality(v_unapproven_hard) > 0 then
    raise exception 'dd173b5: % read(s) this gate could not prove either way: %',
      cardinality(v_unapproven_hard), array_to_string(v_unapproven_hard, ' ; ');
  end if;
  if cardinality(v_unapproved) > 0 then
    raise exception
      'dd173b5: % door(s) opened that nobody approved by name: %. Over-opening and over-tightening are the same class of defect and neither ships on a count.',
      cardinality(v_unapproved), array_to_string(v_unapproved, ' ; ');
  end if;
  raise notice 'dd173b5: 0 unapproved widenings, % narrowing(s), % pair(s) whole-table-identical (id-hash UNPROVEN by construction, resolved), across % tokens x % principals',
  ...
  if cardinality(v_base_fail) > 0 then
    raise exception 'dd173b5: % base-contract FAIL(s) remain after the retrofit: %. This file exists to close exactly these.',
      cardinality(v_base_fail), array_to_string(v_base_fail, ' ; ');
  end if;
  if cardinality(v_policy_fail) > 0 then
    raise exception 'dd173b5: % POLICY-family FAIL(s) after generation: %. The generator produced a policy set its own certification refuses; this round does not stand.',
      cardinality(v_policy_fail), array_to_string(v_policy_fail, ' ; ');
  end if;
  raise notice 'dd173b5: iam.verify_canonical — 0 base-contract FAIL and 0 POLICY-family FAIL across all 6 tokens';
