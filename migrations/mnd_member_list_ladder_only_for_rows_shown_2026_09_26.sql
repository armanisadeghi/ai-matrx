-- based-on: public.mnd_member_list(text, text, text, uuid, uuid, text, jsonb, text, text, integer, integer) f6fe8e83bfdf5a77839f23281b6d240100ff6f19c4cb660f5ad88c81b9b3f83d
-- based-on: mandate._member_list_rows(text, uuid, uuid, text, text[]) d960aaf67932b537a45d79b0abe8c82a0de665dff78872a1b4fe4d4bd278e5ac
-- based-on: mandate.missing_output_keys(text[], jsonb) 73e155459e05371c489ecf569f1a217cf0bf38c1b3057ec0b2383eaecc30ae92
-- based-on: mandate.output_schema_keys(jsonb) 8cbb20f5a0acef49a9c1c7bc3957ef7a0c5ca73b5bbde07f5577a21f1d386505
--
-- mnd_member_list_ladder_only_for_rows_shown_2026_09_26.sql
--
-- THE MEMBER MANDATE LIST RUNS THE LADDER ONLY FOR THE ROWS IT SHOWS.
--
-- WHY (2026-09-26, /mandates/list-preview as test@test.com): every tab count read 0 and the
-- list sometimes failed with 57014 (statement timeout, 8 s for `authenticated`). Measured on
-- live data as that person (RLS real): each of the page's three calls (page, counts, facets)
-- took 2.0-3.1 s alone, 3.3-8.2 s when the browser fired them together, and the 8 s ones were
-- cancelled. Root cause, by profile: every call built the FULL row — `mandate._rungs` (per-row
-- access check, per-holder reachability for every member of the home organization, the output
-- contract) for the WHOLE corpus (~720 mandates) — to paint 50 rows and five numbers. On top,
-- the scope was decided per row through `mandate._member_scope_ok` /
-- `mandate._member_shared_org_ids` (~2,000 calls a page, each re-running the RLS of
-- iam.permissions and mandate.binding), and facets cross-joined every column with every row.
--
-- NOW:
--   * `mandate._member_list_rows(..., p_light)` — a light row (definition columns only, no
--     ladder, no agent/workflow reads) for callers that read none of the ladder columns.
--   * `mandate._member_list_seat` — the light rows plus the five scope verdicts, decided ONCE
--     per call with one read of permissions and bindings (the same rules, line for line).
--   * `public.mnd_member_list` — counts come from the seat alone; a page that neither searches
--     nor filters or sorts on a ladder column is chosen from the seat and the ladder runs for
--     its 50 rows; facets and the other pages run the ladder for the SCOPED rows only; facets
--     walk each row's keys once (the admin list's 2026-09-26 fix).
--   * `mandate.missing_output_keys` never parses a schema for a mandate that requires nothing;
--     `mandate.output_schema_keys` is the same steps in PL/pgSQL (~4x faster per call; proven
--     identical on all 7,541 live agent and version schemas).
--   * `mandate._member_scope_ok` / `mandate._member_shared_org_ids` lose their only caller;
--     they are left in place (dropping them is a non-additive chair step, not this file's).
--
-- SAME ANSWERS: the md5 of every answer was identical before/after, in one rolled-back
-- transaction on production as test@test.com, for 20 cases (five scopes, sorts, search, light
-- and ladder filters, org narrowing, active-org resolution, and the organization level).
-- Measured there (serial, warm): counts 2.9 s -> 0.12 s; page (system) 2.1 s -> 0.6-0.9 s;
-- page (mine) 2.5 s -> 0.3 s; facets (system) 2.2 s -> 1.0 s; search page 2.1 s -> 1.5 s.
--
-- Functions only: no table is touched, no lock beyond the catalogue rows replaced.

CREATE OR REPLACE FUNCTION mandate.output_schema_keys(p_output_schema jsonb)
 RETURNS text[]
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'public'
AS $function$
-- A line-for-line mirror of aidream `_schema_keys`: the same `schema` unwrap,
-- the same array-root -> items step, the same union of `required` and
-- `properties`. Change one, change all three in the same breath — a verdict
-- that disagrees with the server is worse than no verdict, because it is
-- believed.
-- 2026-09-26: the same steps in PL/pgSQL (the SQL body re-entered its CTE executor on
-- every call: ~0.27 ms each, ~4x this). Proven identical on every live agent and
-- version schema (7,541 inputs, 0 differences) before it replaced the SQL body.
DECLARE
  s jsonb;
  v text[];
BEGIN
  IF p_output_schema IS NULL OR jsonb_typeof(p_output_schema) <> 'object' THEN
    RETURN ARRAY[]::text[];
  END IF;
  s := CASE WHEN jsonb_typeof(p_output_schema -> 'schema') = 'object'
            THEN p_output_schema -> 'schema' ELSE p_output_schema END;
  IF s ->> 'type' = 'array' AND jsonb_typeof(s -> 'items') = 'object' THEN
    s := s -> 'items';
  END IF;
  SELECT array_agg(DISTINCT k) INTO v
  FROM (SELECT jsonb_array_elements_text(CASE WHEN jsonb_typeof(s -> 'required') = 'array'
                                              THEN s -> 'required' ELSE '[]'::jsonb END) AS k
        UNION
        SELECT jsonb_object_keys(CASE WHEN jsonb_typeof(s -> 'properties') = 'object'
                                      THEN s -> 'properties' ELSE '{}'::jsonb END)) q;
  RETURN coalesce(v, ARRAY[]::text[]);
END
$function$;
CREATE OR REPLACE FUNCTION mandate.missing_output_keys(p_required_output_keys text[], p_output_schema jsonb)
 RETURNS text[]
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO 'public'
AS $function$
  -- Which required keys the holder does NOT declare. Empty array = passing, and
  -- a mandate that requires nothing is always passing.
  -- 2026-09-26: a mandate that requires nothing never parses the schema (CASE is
  -- lazy) — every list row paid ~0.25 ms of `output_schema_keys` for nothing.
  SELECT CASE
    WHEN coalesce(cardinality(p_required_output_keys), 0) = 0 THEN ARRAY[]::text[]
    ELSE coalesce(
      (SELECT array_agg(r ORDER BY r)
       FROM unnest(p_required_output_keys) r
       WHERE NOT (r = ANY (mandate.output_schema_keys(p_output_schema)))),
      ARRAY[]::text[])
  END;
$function$;
...
END;
$function$;
GRANT EXECUTE ON FUNCTION mandate._member_list_rows(text, uuid, uuid, text, text[], boolean)
  TO authenticated, service_role;
-- The five-argument row builder keeps its signature (the Intelligence page's key pushdown
-- and every older caller) and always builds the full row.
CREATE OR REPLACE FUNCTION mandate._member_list_rows(p_q text, p_res_user uuid, p_res_org uuid, p_level text, p_keys text[])
 ...
 SET search_path TO 'public'
AS $function$
  SELECT * FROM mandate._member_list_rows(p_q, p_res_user, p_res_org, p_level, p_keys, false);
$function$;
-- The member list's SEAT: every row this seat looks at, from the definition alone (no
-- ladder), with the five scope verdicts decided ONCE for the whole set. Replaces the
-- per-row mandate._member_scope_ok / mandate._member_shared_org_ids calls — same rules,
-- line for line, asked with one read of iam.permissions and mandate.binding instead of
-- one per row per scope (each re-running both tables' RLS).
CREATE OR REPLACE FUNCTION mandate._member_list_seat(p_res_user uuid, p_res_org uuid, p_level text,
                                                     p_keys text[], p_org_id uuid, p_uid uuid,
                                                     p_my_orgs uuid[])
 RETURNS TABLE(id uuid, mandate_key text, name text, created_by uuid, organization_id uuid,
               is_system boolean, is_personal_home boolean, visibility text, vals jsonb, sortv jsonb,
               shared_org_ids uuid[], in_mine boolean, in_shared boolean, in_orgs boolean,
               in_public boolean)
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
GRANT EXECUTE ON FUNCTION mandate._member_list_seat(uuid, uuid, text, text[], uuid, uuid, uuid[])
  TO authenticated, service_role;
...
BEGIN
  ...
  IF v_mode = 'counts' THEN
    ...
    SELECT jsonb_build_object(
      ...
      'shared', CASE WHEN v_level = 'person' THEN (SELECT count(*) FROM narrowed n WHERE n.in_shared) ELSE 0 END,
