-- Keep linked-agent sync aligned with the complete portable definition.
--
-- The comparison UI exposed a long-standing contract split:
--   - duplicate carried matrx_actions + ui_gates, but sync left them behind;
--   - rag_awareness_mode synced in the DB but was absent from the FE model.
--
-- Signature and authorization behavior stay unchanged. This function remains
-- the single direction-agnostic sync path for a linked user/system pair.
