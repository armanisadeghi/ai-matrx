-- iam_personal_row_wall_dd165e3_batch3_part3 — REGENERATION BATCH 3 OF 4, PART 3 OF 3 (DD-165).
--
...
declare
  ...
  v_tokens text[] := array['pc_episode','pc_show','pc_studio_run','library_doc','research_context_bundle','research_template','research_topic','youtube_search','youtube_video','global_origin','global_request','runtime_operation_stream','runtime_operation_stream_batch','sch_task','seo_collection_run'];
begin
  ...
  loop
    ...
    exception when others then
      if sqlstate in ('P0001','42703','42883')
        ...
        raise notice 'dd165e3: % (%.%) keeps its bespoke policy — %',
          r.token, r.schema_name, r.table_name, sqlerrm;
      else
        raise exception 'dd165e3: % (%.%) failed with %: %. That is not a structural refusal, so '
  ...
  raise notice 'dd165e3: batch 3 part 3/3 — regenerated %, structurally refused %', v_ok, v_refused;
  if v_ok + v_refused < 15 then
    raise exception 'dd165e3: only % of 15 tokens were reached — a silent drop is exactly the '
