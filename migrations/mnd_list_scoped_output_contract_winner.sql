-- mnd_list_scoped — THE WINNER STOPS NAMING A RUNG THE RUN DOOR REFUSES.
--
-- Campaign: one-resolution, FIX-R7 part B (with aidream `0599`, which taught
-- `mandate._rungs` to judge the OUTPUT half of the contract and added the
-- 22nd column `dropped_code`).
--
-- THE DEFECT, live before this: `health` said `output contract unmet` (FIX-R5)
-- while `resolved_layer` said `system` and `resolved_agent_name` named the very
-- holder `resolve_mandate` refuses. Two live rows, both system-homed:
-- `research_client.output_slides` (requires title + slides; holder *Research →
-- Slides Generator* declares no output_schema at all) and
-- `research_client.output_seo`.
--
-- WHAT CHANGES:
--   * the winner skips ANY rung the ladder dropped for `output_contract_unmet`,
--     the SYSTEM floor included — so those rows name no rung and no agent;
--   * `resolved_layer` is NULL in exactly that state instead of coalescing to
--     'system' (the client renders it as "No rung decides");
--   * `health` reads the floor's verdict BEFORE `holder missing`, which would
--     otherwise blame the absence the skip just created;
--   * the OUTPUT verdict is no longer computed here at all — it comes from
--     `mandate._rungs`, which judges the PINNED VERSION's schema when a rung
--     pins one (this door judged the agent's schema even for a pinned rung, a
--     latent disagreement with `_agent_ref_contract_problems` in Python).
--     `mandate.missing_output_keys` is still called for the winner's own row so
--     a winning BINDING rung's shortfall keeps its badge.
--
-- Reachability is untouched: the floor is still never dropped for a holder one
-- member cannot open (FIX-R1's law, 79 live rows).
--
-- Plain CREATE OR REPLACE — signature and the 25-column return shape are
-- unchanged, so no client type regeneration. Rows changed: 0.
...
BEGIN
  ...
  winner AS (
    ...
    WHERE r.chose_holder AND r.is_enabled AND r.holder_type = 'agent'
      ...
      -- FIX-R1c: a BINDING rung the ladder has already dropped cannot be named
      -- as the resolution. FIX-R7: neither can ANY rung — the system floor
      -- included — whose holder fails the OUTPUT half of the contract, because
      -- `resolve_mandate` refuses that holder for everyone. Reachability still
      -- never drops the floor (79 live system rungs depend on that, and no
      -- answer changes); `dropped_code` is what tells the two apart, so this
      -- never matches on a sentence.
      AND (r.dropped_reason IS NULL
           OR (r.binding_id IS NULL
               AND r.dropped_code IS DISTINCT FROM 'output_contract_unmet'))
    ORDER BY r.mandate_id, r.rung_order DESC
  ),
  -- The FLOOR itself failing the output contract. Its own CTE because it is the
  -- one state in which this list names NO rung at all, and the row must say so
  -- rather than fall through to 'holder missing' (which would blame the absence
  -- the skip created).
  sys_unmet AS (
    SELECT DISTINCT r.mandate_id AS u_id
    FROM rungs r
  ...
  resolved AS (
    SELECT
      ...
      -- 🚨 NO WINNER, NO LAYER. Before FIX-R7 this coalesced to 'system' even
      -- when the system rung was the thing that could not run, so the list
      -- named a rung the run door refuses. NULL is the honest answer and the
      -- screen renders it as "No rung decides" (features/mandates/browse/types.ts).
      CASE WHEN w.w_id IS NULL AND su.u_id IS NOT NULL THEN NULL
           ELSE coalesce(w.w_rung, 'system') END AS r_layer,
      (su.u_id IS NOT NULL)             AS r_sys_unmet,
    ...
    LEFT JOIN sys_unmet su ON su.u_id = m.id
  ...
  shaped AS (
    SELECT
      ...
      CASE WHEN NOT e.r_enabled THEN 'disabled'
           ...
           -- FIX-R7: the SYSTEM rung's holder cannot produce the required
           -- output keys, so no rung decides. This must be read BEFORE
           -- `holder missing` — there is no agent on this row precisely
           -- because the ladder dropped the one that was there, and calling
           -- that "holder missing" would name a defect the job does not have.
           WHEN e.r_sys_unmet THEN 'output contract unmet'
