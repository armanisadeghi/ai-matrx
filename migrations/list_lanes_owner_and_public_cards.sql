-- based-on: public.agx_list_scoped(text, uuid, text, boolean, text, text, boolean, text, jsonb, integer, integer) 4ecbeb4692d3fca4b49ac184a23f4bc157f53ef010748eaee2c972fc52522832
-- based-on: public.wfx_list_scoped(text, uuid, text, boolean, text, text, boolean, text, jsonb, integer, integer) 290791042f98a006d90a42745ebee646e8b63d98ec67405fd17041e05b706aa6
-- based-on: public.cvx_list_scoped(text, uuid, text, boolean, text, text, boolean, text, jsonb, integer, integer) 69bf5a15cafa071327af3c60b402fd036edcfcf494f56d37627c779f7df5defd
-- based-on: public.ivw_list_scoped(text, uuid, text, text, text, jsonb, integer, integer) 8b1a36d22d89b68fb1527eaf0d9dc7e45d07a1847cd37a762d889e4a97f2c1fa
-- based-on: public.trx_list_scoped(text, uuid, text, boolean, text, text, jsonb, integer, integer) bf32315a3d8a91b8b194b7f654191f87497c8d0c06425f92d47009a677dad32b
-- based-on: public.seo_rank_target_list_scoped(text, uuid, text, text, text, jsonb, integer, integer) def38253f9eda50ba31060e230fa90dbf8234e75555c39170b9dd4be01c6cb20
-- based-on: public.shx_list_scoped(text, uuid, text, boolean, text, text, jsonb, integer, integer) 23ff1d99fa21942ceb1dd8bcbce30c729c8ea8058ac9c8266a4194f6a8ab7dd5
-- based-on: public.get_share_capabilities(text) 4516595b936e5f86ada2860ba6c7026272bb8a943d05a606082ff9e82cd1a4f2
-- list_lanes_owner_and_public_cards — three platform-wide list-lane defects (2026-09-26,
-- mandate-sharing verification; common-docs/systems/intelligence/mandates/STATE.md § Visibility and sharing).
--
-- 1. THE MY ORGS LANE SAID "YOU DON'T OWN THIS" ABOUT YOUR OWN ROWS. Seven *_list_scoped functions
--    returned `is_owner = false, access_level = 'org'` for EVERY orgs-lane row, the viewer's own
--    included, so Share / Rename / Archive / Delete were disabled with a false sentence. The orgs
--    arm now answers the comparison, exactly as edu_library_scope_rows (DD-137c) always has:
--    agx (agents), wfx (workflows), cvx (conversations), ivw (interviews), trx (transcripts),
--    seo_rank_target (rank targets) and shx (shapes — the seventh, found by census).
--    Guard: scripts/check-list-lane-owner.ts (pnpm check:list-lane-owner).
-- 2. THE AGENTS PUBLIC LANE WAS ALWAYS EMPTY — see § 2 below.
-- 3. The share-capabilities read names the thing's organization column, so the Share dialog can
--    tell a thing homed in its owner's personal workspace for EVERY kind.
--
-- No data changes. Function bodies only (plus two new read-only definers and their door rows);
-- CREATE OR REPLACE takes no table lock.

set local lock_timeout = '2s';

-- ── 1 + 2. THE LIST FUNCTIONS ───────────────────────────────────────────────────────────────────
-- ── 2. THE PUBLIC LANE READS PUBLIC CARDS ──────────────────────────────────────────────────────
-- An agent's or workflow's BODY can never be public (agent_definition_body_not_public_chk,
-- workflow_definition_body_not_public_chk); publishing (public.make_resource_public) writes
-- `card_visibility`, and the card door (public.get_agent_public) reads `card_visibility = 'public'`.
-- That is the single source of truth for "this is public". agx_list_scoped read `visibility =
-- 'public'` — a state the CHECK forbids — so its Public lane was always empty. And both list
-- functions are SECURITY INVOKER, so even the right predicate returns nothing for a stranger's
-- card: RLS hides the body. These definers return exactly the card projection (the fields the
-- list shows; prompts, messages, settings, tools and node bodies are NULL) of every live public
-- card, so the Public lane shows what is actually public and nothing more.
create or replace function agent.public_card_rows()
returns setof agent.definition
language sql
stable
security definer
set search_path to 'pg_catalog', 'agent', 'public'
as $function$
  select jsonb_populate_record(null::agent.definition, jsonb_build_object(
    'id', d.id, 'agent_type', d.agent_type, 'name', d.name, 'description', d.description,
...
values
  ('agent', 'public_card_rows', '', '{}'::oid[], 'list_lanes_owner_and_public_cards.sql',
   'SIGNED-IN door, no arguments. Called by public.agx_list_scoped (SECURITY INVOKER) for the Public lane. Returns the card projection of every live agent whose card_visibility is public: the fields public.get_agent_public already serves to anyone, plus owner and organization ids; prompts, messages, settings and tools are NULL.',
   true, false),
  ('workflow', 'public_card_rows', '', '{}'::oid[], 'list_lanes_owner_and_public_cards.sql',
   'SIGNED-IN door, no arguments. Called by public.wfx_list_scoped (SECURITY INVOKER) for the Public lane. Returns the card projection of every live workflow whose card_visibility is public; node bodies are replaced by empty objects (the step count only), edges and settings are NULL.',
   true, false);

grant execute on function agent.public_card_rows() to authenticated;
grant execute on function workflow.public_card_rows() to authenticated;
...
-- ── 3. SHARE CAPABILITIES NAME THE HOME COLUMN ──────────────────────────────────────────────────
...
begin
  ...
  -- only organization is its owner's personal workspace — for every kind, not only mandates.
  select c.column_name
  into v_org_column
  from information_schema.columns as c
  where c.table_schema = v_r.schema_name
