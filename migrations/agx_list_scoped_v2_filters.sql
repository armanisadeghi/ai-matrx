-- agx_list_scoped v2 — full server-side filtering, facets, favorites-first.
--
-- Supersedes the v1 signature in agx_list_scoped.sql (kept as the historical
-- record of the scoped reader's introduction; THIS file is the live shape).
-- The v1 overloads are dropped rather than left to accumulate.
--
-- What v2 adds, and why each one had to be server-side:
--   * p_categories / p_tags (OR-sets; '__none__' = uncategorized / untagged)
--     and a tri-state p_favorites. A client-side filter over a paged list only
--     ever filters the page — a lie at any scale that matters.
--   * p_favorites_first (default TRUE) — favorites pin above EVERY sort. What
--     you starred is what you reach for; burying it under 400 rows sorted A–Z
--     is how a favorites feature stops being used.
--   * agx_list_facets — the filter panel's options WITH counts. One live
--     account has 34 categories and 773 distinct tags; deriving "which
--     categories exist" from loaded rows means loading every row, which is the
--     exact pattern this reader replaced.
--   * Default p_limit drops 50 → 25. The first page must be cheap.

DROP FUNCTION IF EXISTS public.agx_list_scope_counts(text, boolean, boolean, text, text);
DROP FUNCTION IF EXISTS public.agx_list_scoped(text, uuid, text, boolean, text, text, boolean, text, text, integer, integer);
...
DECLARE
  ...
  v_fav text := lower(coalesce(p_favorites, 'all'));
  -- '__none__' is the sentinel for "has no category" / "has no tags".
  v_cats text[] := nullif(p_categories, ARRAY[]::text[]);
  v_tags text[] := nullif(p_tags, ARRAY[]::text[]);
BEGIN
  ...
  IF v_sort NOT IN ('updated','created','name','category') THEN v_sort := 'updated'; END IF;
  ...
  filtered AS (
    ...
    WHERE s.agent_type='user' AND s.deleted_at IS NULL
      AND (CASE v_fav
             WHEN 'only'    THEN s.is_favorite IS TRUE
             WHEN 'exclude' THEN s.is_favorite IS NOT TRUE
             ELSE true END)
      AND (CASE lower(coalesce(p_archived,'active'))
             WHEN 'archived' THEN s.is_archived IS TRUE
             WHEN 'all' THEN true
             ELSE s.is_archived IS NOT TRUE END)
      AND (v_cats IS NULL
           OR s.category = ANY(v_cats)
           OR ('__none__' = ANY(v_cats) AND (s.category IS NULL OR s.category = '')))
      AND (v_tags IS NULL
           OR (coalesce(s.tags, ARRAY[]::text[]) && v_tags)
           OR ('__none__' = ANY(v_tags) AND coalesce(array_length(s.tags,1),0) = 0))
      AND (v_search IS NULL
        OR s.name ILIKE '%'||v_search||'%'
        OR s.description ILIKE '%'||v_search||'%'
        OR s.category ILIKE '%'||v_search||'%'
        OR EXISTS (SELECT 1 FROM unnest(coalesce(s.tags, ARRAY[]::text[])) t
  ...
  SELECT c.id, c.agent_type, c.name, c.description, c.model_id, c.category,
    ...
    c.visibility::text, c.user_id, c.organization_id, o.name, c.project_id, c.task_id,
  ...
  ORDER BY
    ...
    -- pages (that bug already cost this table 59 of 365 agents once).
    c.id
  LIMIT greatest(coalesce(p_limit,25),1) OFFSET greatest(coalesce(p_offset,0),0);
END;
$function$;

REVOKE ALL ON FUNCTION public.agx_list_scoped(text,uuid,text,boolean,text,text,boolean,text,text,text[],text[],integer,integer) FROM public;
GRANT EXECUTE ON FUNCTION public.agx_list_scoped(text,uuid,text,boolean,text,text,boolean,text,text,text[],text[],integer,integer) TO authenticated;
...
BEGIN
  FOREACH v_scope IN ARRAY ARRAY['mine','orgs','shared','public'] LOOP
    ...
    FROM public.agx_list_scoped(v_scope, NULL, p_search, p_deep, 'updated', 'desc',
      true, p_favorites, p_archived, p_categories, p_tags, 1, 0) r;
  END LOOP;
  ...
  LEFT JOIN LATERAL public.agx_list_scoped('orgs', o.id, p_search, p_deep, 'updated','desc',
    true, p_favorites, p_archived, p_categories, p_tags, 1, 0) r ON true
  WHERE o.is_personal IS NOT TRUE
  GROUP BY o.id;
END;
$function$;

REVOKE ALL ON FUNCTION public.agx_list_scope_counts(text,boolean,text,text,text[],text[]) FROM public;
GRANT EXECUTE ON FUNCTION public.agx_list_scope_counts(text,boolean,text,text,text[],text[]) TO authenticated;
...
BEGIN
  RETURN QUERY
  WITH base AS (
    SELECT r.category, r.tags, r.is_favorite, r.is_archived
    FROM public.agx_list_scoped(p_scope, p_org_id, p_search, p_deep, 'updated','desc',
      false, 'all', p_archived, NULL, NULL, 1000000, 0) r
  )
  SELECT 'category'::text,
         COALESCE(NULLIF(b.category, ''), '__none__'),
         count(*)
  ...
  GROUP BY t.tag
  UNION ALL
  SELECT 'favorite'::text, 'only', count(*) FILTER (WHERE b.is_favorite)
  FROM base b
  UNION ALL
  SELECT 'archived'::text, 'archived', count(*) FILTER (WHERE b.is_archived)
  FROM base b;
END;
$function$;
