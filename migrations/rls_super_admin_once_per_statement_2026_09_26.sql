-- chair-step: replaces the RLS generator body (its string literals hold the DROP/REVOKE/CREATE POLICY text it emits) and rewrites the four restricted read-path tables policies in place with the super-admin call wrapped; answers unchanged, proven in a rolled-back production transaction.
...
--
-- rls_super_admin_once_per_statement_2026_09_26.sql
--
-- THE RLS TEMPLATE ASKS "IS THIS A SUPER ADMIN?" ONCE PER STATEMENT, NOT ONCE PER ROW.
--
-- WHY (2026-09-26, found by the mandate.reference lane): `iam._apply_rls_unchecked` emitted every
-- super-admin arm as a bare `public.is_super_admin()`. PostgreSQL wraps a bare call in an InitPlan
-- only when it is written as a scalar subquery, so each arm ran once per row the policy judged —
-- measured live: std_select on the `restricted` tables (mandate.scan 85k rows, ai.offering,
-- ai.endpoint, ai.api, mandate.reference) and every std_insert WITH CHECK (~200 tables) carried it.
-- The other two per-row identity calls the template emits were already wrapped (`( SELECT
-- auth.uid() AS uid)`, `( SELECT is_platform_admin() AS is_platform_admin)`); a census of every
-- live policy found no bare auth.uid() / is_platform_admin() left.
--
-- NOW: the template emits `(select public.is_super_admin())` in all four places; the verifier
-- (iam.verify_canonical `personal_row_wall`, `super_admin_system_org_arm_walled`) and the frontend
-- guard (scripts/check-row-visibility.ts) accept the walled arm in both spellings. The four
-- read-path tables have their policies rewritten in place (ALTER POLICY, a short ACCESS EXCLUSIVE each). mandate.reference is left to the lane redesigning it; the INSERT-only
-- tables pick the new form up the next time they are provisioned (their arm runs once per row
-- INSERTED, not per row read).
--
-- SAME ANSWERS: is_super_admin() takes no row argument and is STABLE, so hoisting it changes the
-- number of calls, never the answer. Proven on production in one rolled-back transaction: all 32
-- policies of the four tables identical before/after once the bare call is read as the wrapped one
-- (12 bare calls before, 0 after); reads of all four tables as test@test.com and admin@admin.com
-- return the same rows (md5); iam.verify_canonical reports no FAIL on any of them. Measured there:
-- a full read of mandate.scan 1,554 ms -> 119 ms (test@test.com), 1,525 -> 118 ms (admin@admin.com).
...
-- The four read-path tables: each policy that carries a bare call is rewritten IN PLACE with the
-- call wrapped and every other byte unchanged. Not iam.apply_rls: regenerating these tables would
-- also drop their `pub_read` policy (their class no longer declares the anonymous lane), which is a
-- separate access decision this file does not make.
DO $wrap$
DECLARE
  r record;
  v_q text; v_c text; n int := 0;
BEGIN
  FOR r IN
    SELECT n.nspname AS s, c.relname AS t, p.polname AS pol,
           pg_get_expr(p.polqual, p.polrelid) AS q, pg_get_expr(p.polwithcheck, p.polrelid) AS c
      FROM pg_policy p JOIN pg_class c ON c.oid = p.polrelid JOIN pg_namespace n ON n.oid = c.relnamespace
     WHERE (n.nspname, c.relname) IN (('mandate','scan'), ('ai','offering'), ('ai','endpoint'), ('ai','api'))
  LOOP
    v_q := replace(replace(r.q, '( SELECT is_super_admin() AS is_super_admin)', '#SA#'), 'is_super_admin()', '(SELECT public.is_super_admin())');
    v_q := replace(v_q, '#SA#', '( SELECT is_super_admin() AS is_super_admin)');
    v_c := replace(replace(r.c, '( SELECT is_super_admin() AS is_super_admin)', '#SA#'), 'is_super_admin()', '(SELECT public.is_super_admin())');
    v_c := replace(v_c, '#SA#', '( SELECT is_super_admin() AS is_super_admin)');
    IF v_q IS DISTINCT FROM r.q THEN
      EXECUTE format('ALTER POLICY %I ON %I.%I USING (%s)', r.pol, r.s, r.t, v_q); n := n + 1;
    END IF;
    IF v_c IS DISTINCT FROM r.c THEN
      EXECUTE format('ALTER POLICY %I ON %I.%I WITH CHECK (%s)', r.pol, r.s, r.t, v_c); n := n + 1;
    END IF;
  END LOOP;
  RAISE NOTICE 'super-admin arm wrapped in % policy expressions', n;
END
$wrap$;
