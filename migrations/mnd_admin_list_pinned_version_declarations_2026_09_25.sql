-- mnd_admin_list — a PINNED Holder version answers with its own declarations.
--
-- based-on: mandate._admin_list_rows(jsonb, text) e51a0717a30f96cd82a3369a59981f75c4ade2e72c501c9db442dcdef336559a
-- based-on: public.mnd_admin_list(text, text, uuid, text, jsonb, text, text, integer, integer, jsonb) 100f4a5e1ec07118b5ae4e8a3e80de6f4efb5a1118149f5b6061074d8216c3ee
--
-- Review 2026-09-25: shortcut.agent_generator pins version 1 of "Agent
-- Structure Builder" (one variable). The list and its peek read the LIVE agent
-- (two variables — v16 added a context-bound one), so the peek said 2 inputs
-- while the Definition tab (the server's input surface, which resolves the pin)
-- said 1. Same class for the output shape the list judges.
--
-- 1. mandate._admin_list_rows: h_declared and h_output_schema come from the
--    pinned version when there is one, else the live agent.
-- 2. public.mnd_admin_list: the page's versions carry the version's
--    variable_definitions / context_policies / output_schema, so the browser
--    builds the peek from the same declarations
--    (features/mandates/admin/mandate-health.ts buildRow).
-- Bodies are the LIVE definitions (pg_get_functiondef) with only those edits.
