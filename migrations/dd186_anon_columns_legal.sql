-- dd186_anon_columns_legal — THE SIGNED-OUT COLUMN SURFACE OF THE `legal` SCHEMA
...
-- legal.wc_claim — revoked: metadata, user_id, organization_id, created_by, updated_by, version
revoke select on legal.wc_claim from anon;
grant select (id, created_at, applicant_name, person_id, date_of_birth, date_of_injury, age_at_doi, occupational_code, weekly_earnings, gender, case_number, evaluator_name, comments, project_id, is_public, tags, updated_at, p_s_date, job_offer_date, large_employer, deleted_at, visibility) on legal.wc_claim to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['legal.wc_claim']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (legal): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
