-- chair-step: grants only, nothing narrowed — SELECT to service_role on the relations the admin door
--   cannot read, ALTER DEFAULT PRIVILEGES … GRANT SELECT ON TABLES TO service_role, and an event trigger
--   ON REVOKE that re-grants the door's SELECT. The deny-list matches the REVOKE tag and the default-privilege
--   GRANT; no client role (anon/authenticated/PUBLIC) gains anything. Law:
--   common-docs/policies/our-own-admin-database-access.md item 3 (relations_closed_to_the_admin_door must be 0).
--
-- admin_access_door_closes_nothing
--
