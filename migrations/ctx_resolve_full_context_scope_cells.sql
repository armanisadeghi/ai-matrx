-- ctx_resolve_full_context_scope_cells.sql
--
-- Plug scope CELLS into agent context delivery + accept explicit scopes.
--
-- Before this migration, resolve_full_context (the single RPC behind
-- aidream's build_agent_context) resolved variable values exclusively from
-- ctx_context_variables (brokers). The scope cells users author in the scope
-- grid (ctx_context_item_values — "the 50 datapoints per Client") were never
-- read on any invocation path, so the core scope promise was unwired.
--
-- This migration:
--   1. Adds `p_scope_ids uuid[] DEFAULT NULL` — explicit active scopes (the
--      user's global scope selections) passed by the caller. They are
--      membership-validated against p_user_id and unioned with the entity's
--      ctx_scope_assignments tags (deduped).
--   2. Reads every CURRENT cell (ctx_context_item_values.is_current) of every
--      ACTIVE item (ctx_context_items.is_active, fetch_hint != 'never') for
--      all contributing scopes and merges them into the `variables` output
--      with the same {value, type, inject_as, source, description} shape the
--      brokers use. inject_as = 'direct' so the deployed renderer
--      (matrx_ai.context_engine.AgentContext.build_system_prompt_block)
--      delivers them with ZERO backend code change.
--   3. Cells are merged BEFORE the broker loop, so explicit broker variables
--      still win on key collisions (brokers are deliberate injections).
--   4. The signature change requires dropping the old 3-arg function first
--      (otherwise the 3-arg call becomes ambiguous). PostgREST callers using
--      3 named args resolve to this function via the default.
--
-- Sensitivity gating of cells (context_sensitivity) is deliberately NOT done
-- here — that lands with the security overhaul. fetch_hint='never' is the
-- only exclusion.


DROP FUNCTION IF EXISTS public.resolve_full_context(uuid, text, uuid);

CREATE OR REPLACE FUNCTION public.resolve_full_context(
    p_user_id uuid,
    p_entity_type text,
    p_entity_id uuid,
...
BEGIN
    ...
    -- ── 2) Entity-tagged scopes (unchanged) ────────────────────────────
    ...
    -- Project-tag fallback (unchanged)
    ...
    -- ── 3) NEW: explicit active scopes (global selections) ─────────────
    -- Membership-guarded: only scopes in orgs p_user_id belongs to count.
    -- Deduped against the entity tags so a scope contributes once.
    ...
    -- ── 4) Scope labels (unchanged, null-safe) ─────────────────────────
    ...
    -- ── 5) NEW: scope CELLS → variables ────────────────────────────────
    -- Current cell per (item, scope) for every contributing scope. Runs
    -- BEFORE the broker loop so brokers override cells on key collisions.
    -- Within cells, later scope-type sort_order wins on a duplicate key.
    IF v_entity_scopes IS NOT NULL THEN
        FOR rec IN (
            SELECT ci.key,
                   ci.description,
                   ci.value_type::text AS value_type,
    ...
    -- ── 6) Broker variables (unchanged; overrides cells on collision) ──
    FOR rec IN (
