-- dd226_docproc_publishes_nothing_to_a_signed_out_reader
...
--   docproc.page_extraction_results — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 9)); the grant itself is the finding.
--   docproc.page_extraction_runs — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 15)); the grant itself is the finding.
--
...
begin
  for r in
    select unnest(array[
      'docproc.page_extraction_results',
      'docproc.page_extraction_runs'
    ]) as rel
  loop
