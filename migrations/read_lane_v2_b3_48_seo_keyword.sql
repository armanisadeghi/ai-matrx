...
-- read_lane_v2_b3_48_seo_keyword — enroll seo.keyword in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'seo' and table_name = 'keyword' and is_active on conflict do nothing;
