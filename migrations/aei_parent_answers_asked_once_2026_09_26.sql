...
--
-- aei_parent_answers_asked_once_2026_09_26.sql
--
-- THE SET FORM OF THE ACCESS KERNEL ASKS EACH PARENT QUESTION ONCE PER CALL.
--
-- WHY (2026-09-26, perf lane after the mandate member list): working out which agents a person
-- can see — `iam.accessible_entity_ids('agent', 'viewer', 0, true)`, the candidate set every read
-- of agent.definition / agent.definition_version runs through its std_select — cost ~180-225 ms
-- per read as test@test.com. Profiled live (a pg_temp copy timing every nested call): the agent's
-- own work was ~8 ms; the rest was the parent cascade agent -> task -> project -> organization,
-- which asks every parent twice (with and without public rows) and re-asks the grandparent both
-- ways inside the first, so the without-public answers were recomputed at every level: `project`
-- 3 times (~50 ms each), `organization` 4 times (~15 ms each). Every token with a composition /
...
-- Inverse: migrations/inverse/aei_parent_answers_asked_once_2026_09_26_down.sql (the live body).
...
declare
  ...
  rec record;
  -- 🚨 AEI-MEMO (2026-09-26) — ONE ANSWER PER PARENT QUESTION PER CALL. The parent cascade
  -- below asks each parent twice when it widens to public (once with, once without public
  -- rows), and the with-public call asks the grandparent both ways again, so the without-public
  -- answers were recomputed at every level: `agent` (-> task -> project -> organization) asked
  -- `project` 3 times and `organization` 4 times, ~180 ms of the ~225 ms one read of
  -- agent.definition spends here as test@test.com. A frame opened by the depth-0 call memoizes
  -- every nested (person, type, level, depth, include_public) answer for the rest of THIS call and
  -- is closed (restored) before it returns. Every answer is computed exactly as before, under the
  -- same snapshot (this function is STABLE, so every nested call reads the snapshot of the
  -- statement that called it) — the memo only stops the same question being asked twice.
  -- Only a depth-0 call opens a frame and reads nothing it did not write inside it, so no caller
  -- outside this cascade ever reads a memoized answer (every policy and door calls depth 0).
  v_memo_prev text;
  v_memo_key text;
  v_memo jsonb;
begin
...
BEGIN
  ...
  IF cardinality(v_added) <> 0 OR cardinality(v_removed) <> 0
    ...
    RAISE EXCEPTION 'aei-memo: refusing to re-record unrelated kernel drift (added %, removed %, changed %)',
...
RETURNS jsonb LANGUAGE sql IMMUTABLE AS $f$ SELECT %L::jsonb $f$$ddl$,
  ...
  IF iam.entity_read_kernel_fingerprint() IS DISTINCT FROM iam.entity_read_kernel_expected()
    ...
    RAISE EXCEPTION 'aei-memo: kernel fingerprint re-record did not match the live kernel';
  END IF;
END
$rerecord$;
