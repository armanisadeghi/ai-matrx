-- Explicit matching semantics for the keyword classification primary search.
-- "contains" preserves the familiar substring search; "whole_words" requires
-- every entered token to exist as a complete keyword token before pagination.
...
CREATE OR REPLACE FUNCTION seo.gsc_keyword_class_review(
  ...
  p_search_mode text DEFAULT 'contains'
)
RETURNS TABLE (
  keyword_id uuid,
  query text,
