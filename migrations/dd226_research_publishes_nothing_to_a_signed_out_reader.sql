-- dd226_research_publishes_nothing_to_a_signed_out_reader
...
--   research.rs_source_keywords — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 20)); the grant itself is the finding.
--   research.rs_template — LIVE TO THE INTERNET RIGHT NOW — 5 row(s) readable by `anon` today. RED, one row flipped to visibility='public' inside a rolled-back transaction and read AS ANON: {"id":"006a687f-3cb2-43e8-a932-bf4f3815b136","name":"Scientific Research","description":"Research on scientific topics including latest findings, clinical trials, and academic papers.","keyword_templates":["${name}","${name} latest research","${name} clinical …
--   research.rs_topic — Latent (the grant is open; no row is on the public side of the gate today). RED, one row flipped to visibility='public' inside a rolled-back transaction and read AS ANON: {"id":"32a6aee9-9217-4e90-8dd9-2bf5c57d4a9f","name":"2026 Medspa Trends: Innovations and Clinical Aesthetic Breakthroughs","autonomy_level":"auto","default_search_provider":"brave","default_search_params":{},"good_scrape_threshold":1000,"scrapes_per_keyword":1…
--
