-- dd186_anon_columns_scheduler — THE SIGNED-OUT COLUMN SURFACE OF THE `scheduler` SCHEMA
...
-- scheduler.sch_task — revoked: user_id, organization_id, created_by, updated_by, version, metadata
revoke select on scheduler.sch_task from anon;
grant select (id, kind, title, description, queue, surfaces, enabled, expires_at, tags, next_due_at, last_run_at, created_at, updated_at, deleted_at, visibility, taxonomy_node_id) on scheduler.sch_task to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['scheduler.sch_task']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (scheduler): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
