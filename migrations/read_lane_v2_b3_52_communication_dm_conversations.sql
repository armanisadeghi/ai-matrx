...
-- read_lane_v2_b3_52_communication_dm_conversations — enroll communication.dm_conversations in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'communication' and table_name = 'dm_conversations' and is_active on conflict do nothing;
