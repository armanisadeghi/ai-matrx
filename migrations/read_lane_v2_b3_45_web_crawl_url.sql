...
-- read_lane_v2_b3_45_web_crawl_url — enroll web.crawl_url in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'web' and table_name = 'crawl_url' and is_active on conflict do nothing;
