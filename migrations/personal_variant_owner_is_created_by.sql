-- based-on: iam._apply_rls_unchecked(text, text, text, text) a025c95267755e79088342b7f87cdb6e9dae377abc0c78229fb522a9724b65cb
-- based-on: iam.verify_canonical(text, text, text, text) 928a958c8a916eac46c3b0b0f53ae26b0ae5cde3418d013bbaa631289f3879d0
-- chair-step: replaces two whole function bodies (the RLS generator and the grader); the DROP/REVOKE/CREATE POLICY ... true the judge sees are DDL string literals inside those bodies, not statements — no table, policy or grant is changed by this file. Rehearsed in a rolled-back transaction 2026-09-25 (lane B-TOOL).
-- Personal variant: the owner is created_by, in the generator AND the grader (lane B-TOOL pilot).
--
-- user_id was retired in favour of created_by as the owner column (Arman, 2026-09-23). The kernel
-- (platform.entity_row_access_attrs) and the set lane (iam.accessible_entity_ids) read the owner from
-- created_by/owner_id, but iam._apply_rls_unchecked keyed every `personal` policy on user_id and
-- refused a table without one, and iam.verify_canonical FAILED a personal table without user_id.
-- So platform.create_entity_table(p_variant => 'personal') could not build a personal table, and
-- the live personal tables' RLS lane disagreed with iam.has_access on every row.
--
-- After this file:
--   * the generator keys std_select/insert/update/delete on created_by; refuses a table without
--     created_by (42703) and a table whose lingering user_id DISAGREES with created_by (22023);
--   * the grader requires the actor pair on personal (created_by is the owner), reports user_id as
--     the legacy WARN like every other variant, SKIPs visibility (WARNs a stray one), and demands
--     created_by = auth.uid() in std_select.
-- Existing personal tables are untouched until regenerated; each is brought to created_by through
-- the Entities system (tool.mcp_user_conn first — see tool_schema_certify_phase1.sql).
-- Guard: aidream tests/test_personal_variant_owner_is_created_by.py (failed before, passes after).
