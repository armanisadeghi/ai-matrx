-- dd226_podcast_publishes_nothing_to_a_signed_out_reader
...
--   podcast.pc_shows / pc_episodes — /podcast/[slug]/feed.xml and chapters.json are podcast feeds, fetched by podcast clients with no account.
--   podcast.pc_articles — /podcast/[slug]/blog renders public articles to a signed-out reader.
--
-- ═══ RED, MEASURED AGAINST THE LIVE DATABASE, EVERY PROBE ROLLED BACK ═════════════════════════
--   podcast.pc_race — LIVE TO THE INTERNET RIGHT NOW — 4 row(s) readable by `anon` today. RED, one row flipped to visibility='public' inside a rolled-back transaction and read AS ANON: {"id":"fbdaf34d-9e63-4b48-920d-7ebd54a43ac5","topic":"The latest developments in artificial intelligence and machine learning, focused specifically on how far AGENT HARNESSES and GRAPH ENGINEERING have come: what changed in agent harness design, workflow/graph…
--   podcast.pc_studio_runs — Latent (the grant is open; no row is on the public side of the gate today). RED, one row flipped to visibility='public' inside a rolled-back transaction and read AS ANON: {"id":"9b556beb-503b-41da-bf94-351ff93f909a","status":"failed","input_data_type":"topic","podcast_type":"educational","request":{"format":"educational","language":"en-US","host_count":2,"input_data":"Next.js Version 16 best practices","max_images":0,"max_video…
--
