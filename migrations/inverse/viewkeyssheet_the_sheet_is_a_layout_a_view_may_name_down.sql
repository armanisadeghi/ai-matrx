...
-- additive: yes
--   The inverse of viewkeyssheet_the_sheet_is_a_layout_a_view_may_name.sql: S1-PRIME's two bodies
--   back, byte for byte (custom.view_keys() and custom._view_key_value from
--   uichamp_s1_a_view_keeps_every_setting_it_is_given.sql), so `layout` is again one of grid,
--   kanban, calendar or gallery. CREATE OR REPLACE only; no grant, no stored view touched.
--   A view already stored with layout "sheet" keeps it (a stored value is not re-judged unless sent).
