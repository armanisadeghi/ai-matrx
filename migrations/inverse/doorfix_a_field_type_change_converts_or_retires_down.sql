-- chair-step: DOOR-FIX 3's inverse — removes the field-type conversion trigger, its function, the conversion table and the behaviour signature, and puts custom.migrate_retype back. Values already converted or already in _retired stay: an inverse that rewrote them would be a data change dressed as a rollback.
