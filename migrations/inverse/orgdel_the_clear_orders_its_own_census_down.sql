-- INVERSE of migrations/campaign/orgdel_the_clear_orders_its_own_census.sql (lane ORG-DELETE).
-- It puts the ORDER BY back on the SELECT DISTINCT, which is the state the green suite found:
-- the destroying arm of custom.organization_clear dies on 0A000 the moment it is called.
