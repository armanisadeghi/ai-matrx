...
--   migrations/campaign/storetails2_a_joined_column_prints_the_words_a_person_reads.sql. It
--   puts back the exact `custom.rule_eval` body that was live before that file (sha256
--   a3a8971b1a6152b8902a29750c74de0d72e97ce82c35146db07fbf7f174049a7), so a joined column goes
--   back to printing the option key and ignoring `separator`. `custom.field_words` is LEFT
--   STANDING and said so out loud: it is new, it is granted to nobody, dropping it would break
--   any later reader, and a function nothing calls costs nothing.
--
-- WHAT IT PROVES (law 3): with this applied, scripts/campaign-tests/storetails2_green.sql goes
-- RED on its first clause — a joined column reads `GL-2034mow_edge` again.
