-- chair-step: rule-27 rehearsal inverse of rca2b_the_access_check_asks_a_comments_record.sql — restores the kernel, set-form, cmt_add and generator bodies it patched and regenerates platform.comments. It RE-OPENS the comment leak (verify-RC-A2 F1-F4); never run it on production except as an explicit emergency rollback of a broken kernel.
...
-- exactly once. Then the table is regenerated through the generator, which re-emits the old
-- std_select (no soft-delete conjunct).
...
begin
  ...
  loop
    ...
    if v_n <> 1 then
      raise exception 'rca2b inverse %: anchor occurs % time(s) in %, expected exactly 1 — nothing was changed', r.ord, v_n, r.fn;
    end if;
