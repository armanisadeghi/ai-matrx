...
-- migrations/campaign/secsweep_two_more_credential_columns_lose_their_client_door.sql
--
-- Removes the two declared exclusions and re-derives the grants, putting each table back on
-- a table-level grant to `authenticated`. It re-opens two credential columns to every signed-in
