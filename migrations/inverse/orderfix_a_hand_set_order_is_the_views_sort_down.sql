...
-- additive: yes
--   The inverse of orderfix_a_hand_set_order_is_the_views_sort.sql: puts back the two bodies it
--   replaced, byte for byte (VIEW-KEYS-SHEET's S1 view_declare, G13's view_record_order_set), and
--   their comments. The one-row repair has its own inverse
--   (orderfix_a_hand_ordered_view_opens_as_the_sheet_down.sql).
