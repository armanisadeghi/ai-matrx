...
--
-- ORDER, AND THE TWO BODIES THIS FILE ALSO RE-STATES (amended 2026-09-23 for the ground-standing
-- check, clause a). custom._rule_shape_guard and custom._field_type_parity_guard — attached to
-- custom.record — call custom.rule_node_kinds(), and G6
-- (gridprim_a_rule_speaks_the_formula_language.sql) makes that function and custom.rule_eval reach
-- custom.formula_node_kinds / custom.formula_eval, which this file drops. So before anything is
-- dropped, this file writes both back as production held them BEFORE G6 (the `-- based-on:` lines
-- above are those pre-G6 bodies): with G6 not applied they are re-stated unchanged; with G6 applied
-- the runner refuses this file by name until G6's inverse
