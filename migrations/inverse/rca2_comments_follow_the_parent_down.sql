...
-- entity set byte-for-byte as the old one did), then every function body is restored from the
-- pg_get_functiondef taken on the main database immediately before the up was written.
-- based-on: iam._apply_rls_unchecked(text, text, text, text) d13e060db138e04f070095686df5a0f4fa357af61612d965f573cd7811146a8a
-- based-on: iam.verify_canonical(text, text, text, text) 368dcc7f6ef59e2eb6c3b2e7fd6ffdd0f3d59e0b67c6371119394d5bbf11a738
-- based-on: public.cmt_add(text, uuid, text, uuid, uuid) 7b571f58b5a00502c0388fe85d0e07b3bb429250eb6ec377319f78c3b896e9b2
...
update platform.entity_types set rls_variant = 'entity', type = 'entity' where token = 'comment';
select iam.apply_rls('platform', 'comments', 'comment', 'entity');

delete from platform.client_callable_door where schema_name = 'public' and function_name = 'cmt_resolve';
drop function if exists public.cmt_resolve(uuid, boolean);

update platform.client_callable_door d
   set argument_rules = jsonb_set(
         jsonb_set(d.argument_rules, '{arguments,p_entity_id,check}', to_jsonb('p_entity_id -> iam.has_org_access(...)'::text)),
         '{arguments,p_entity_id,verified}', to_jsonb('static reading 2026-09-17'::text))
...
alter table platform.comments drop constraint if exists comments_anchor_is_a_kind;
