...
--
-- THE INVERSE of migrations/campaign/leakt10_a_home_of_a_table_is_not_the_whole_table.sql: it
-- puts back the two bodies exactly as they stood on the main database on 2026-09-20, in which
--
--   * `custom.visibility_ancestors` computed its terminal-Table flag only in the SEED, so the
--     walk still climbed from a Home's own Table into that Table's Homes one level out, and
--   * `custom.visible_set`'s whole-Table shortcut asked `custom.reaches_directly` about the
--     Table, which climbs from the Table into the Table's HOMES — so a person shared ONE Home
--     of a multi-Home Table was handed every record of that Table in every other Home by
--     `custom.read_records`, `custom.query_visible_ids`, `custom.query_across_homes` and
--     `custom.io_export`, while `custom.read_record` refused her the same row.
--
-- Run it and census 13 of `pnpm check:store-doors-decide` goes red again, and
-- `scripts/campaign-tests/leakt10_green.sql` CLAUSE 1 with it.
