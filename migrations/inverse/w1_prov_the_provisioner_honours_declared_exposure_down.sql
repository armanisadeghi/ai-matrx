...
-- BASED-ON-LINES-GO-HERE
--
-- THE INVERSE of `migrations/campaign/w1_prov_the_provisioner_honours_declared_exposure.sql`.
--
-- It restores the three function bodies to the LIVE definitions this change was based on —
-- the same three hashes named above, byte for byte, taken from pg_get_functiondef on the
-- rehearsal branch 2026-09-17 — and drops the three new objects. After it runs, the
-- provisioner is exactly what it was before the fix, which is the state the RED probe
-- measured. `-- target: branch`: it DROPs, and rule 9 forbids a DROP on production.
--
...
-- They were therefore generated by `pnpm db:based-on` AFTER the up-migration landed on the
