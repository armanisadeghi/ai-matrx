-- INVERSE of migrations/campaign/seat_a_column_can_point_at_your_own_table.sql
-- Restores custom._field_document_for exactly as it stood before SEAT-SUITES, i.e. a person
-- again cannot make a column that points at another of their own tables, and
-- review_interval_days and default are again dropped on the floor.
  ...
  if v_alias <> '' and v_parity is null and nullif(p_spec ->> 'plain', '') is null then
    if exists (select 1 from custom.parity_field_types() t where t.parity_type = v_alias) then
      ...
      raise exception 'A column that points at other records is a Person column or a File column, and "%" does not say which.', v_alias
        using errcode = '23514',
              hint = 'FLD-11: say member for a person or attachment for a file. Both are relations; they differ in what they point at.';
    else
