-- INVERSE of migrations/campaign/flipseams_a_switch_goes_back_without_losing_what_was_written.sql (lane FLIP-SEAMS): both doors back to production's bodies before it, byte for byte, and the reverse-readiness helper removed.
-- chair-step: restores platform.cutover_seams and platform.cutover_seam_press as the first FLIP-SEAMS file left them; drops platform._cutover_seam_reverse_readiness.
...
drop function if exists platform._cutover_seam_reverse_readiness(text, uuid);
