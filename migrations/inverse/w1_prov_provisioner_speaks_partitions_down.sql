-- chair-step: this restores the PREVIOUS live bodies of platform.provision and platform.provision_validate — the one door every new table passes through — and removes the two spec keys and the eight refusal messages that go with them; a body replacement is never additive and never unattended
--
-- THE INVERSE of `migrations/campaign/w1_prov_provisioner_speaks_partitions.sql` (§4.13,
-- rule 27). Every body below is the one that file's own `-- based-on:` line declares it was
-- written against, read from production's `pg_get_functiondef` on 2026-09-17:
--
--   platform.provision_base_columns()                  116f53dc16d0d6dc…
--   iam.entity_read_kernel_fingerprint()               815aa1624b0b158e…
--   platform.provision_validate(jsonb, text, uuid)     8e81b9f03b4b8f84…
--   platform.provision(jsonb, text, uuid, text)        411b3f4fef5de4c9…
--
-- 🚨 WHAT RUNNING THIS COSTS, SAID PLAINLY. Restoring the old fingerprint re-breaks
-- `platform.provision_preflight()` on the rehearsal branch — the OID-ordering defect is
-- exactly what the up-migration repaired — so no provisioning run can be rehearsed until
-- the up is re-applied. And any table provisioned WITH a partition or a single write door
-- stays exactly as it was built: this file changes the door, never the tables that came
-- through it. `platform.stamped_write_table` rows are left alone on purpose, because
-- deleting one would make the next `iam.apply_rls` regeneration re-grant
-- `insert, update, delete` to `authenticated` on a table whose only write path is a door.
--
-- HEADER-LESS ON PURPOSE: a body replacement with no guard to read is refused by the
-- allow-list in every lane; the one route is a header-less chair step, rehearsed on the
-- branch with `--target branch`.

--
-- THE BODIES THIS INVERSE WAS WRITTEN AGAINST (rule 6) — i.e. the ones the UP-migration
-- installs. Measured on the rehearsal branch after that file applied, 2026-09-17; the same
-- bytes produce the same hashes on production once the chair step has run there.
...
-- refused by the shape check rather than reaching a validator that no longer reads them.
update platform.provision_schema
   set schema = (schema #- '{properties,partition}') #- '{properties,write_door}',
       updated_at = now()
 where name = 'full';

delete from platform.provision_rule_message
 where rule_id in ('partition.strategy.unsupported','partition.key.missing',
                   'partition.key.not_a_candidate','partition.count.out_of_range',
                   'partition.unique_index.missing_key','write_door.unknown',
                   'write_door.no_door_declared','write_door.proof_failed');
