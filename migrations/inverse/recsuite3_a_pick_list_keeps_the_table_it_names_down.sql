...
-- additive: yes
--   The inverse of migrations/campaign/recsuite3_a_pick_list_keeps_the_table_it_names.sql.
--   It REPLACES `custom._field_document_for` with the body that file was written against,
--   byte for byte (measured on the clone and the branch 2026-09-22). Nothing else.
...
--
-- THE INVERSE puts back the pick-list arm that overwrites `config.options_table_id` with the
-- spec's top-level key — the defect the forward file closes. Run it only to undo that file.
