-- chair-step: THE INVERSE of migrations/campaign/versionhistoryfix_c_the_hr_sweep_stops_re_saving_what_did_not_change.sql.
--   It puts back, verbatim, the production bodies of `hr.wf_tick()`, `hr.wf_escalate(uuid,text)` and
--   `public.wsp_upsert_system_task(...)` as they stood on 2026-09-23, which puts the loop back: a
--   refused escalation is retried every sweep, with a new failure row and notification each time,
--   and unchanged step definitions and approval tasks are re-saved. It exists for rule 27.
-- lock: platform
-- lane: VERSION-HISTORY-FIX
-- based-on: hr.wf_tick() 46457f50d17a78422511955b57a738aecec24c62ead73a7a67b40325b5f91779
-- based-on: hr.wf_escalate(uuid, text) c125ef54d768bd4fb4cb32da9b5a4ed8f70460884a9924290c86f5907e7af07c
-- based-on: public.wsp_upsert_system_task(text, text, text, text, text, text, text, text, date, text, uuid, uuid, uuid, jsonb) 032c7bad3c7eb5e84d2ed3c3893b5d64d59955329bb2ccd078332021a2b300b0
