...
--
-- THE INVERSE of migrations/campaign/exportfix_the_export_pages_the_read_door.sql: it puts
-- `custom.io_export` back asking `custom.read_records` for the EXPORT's page size in one call —
-- up to 100,000 rows from a door that serves 1,000 — so the export raised PAGE-1 on its own
-- default of 10,000 and `select custom.io_export(org, table)` did not work at all.
