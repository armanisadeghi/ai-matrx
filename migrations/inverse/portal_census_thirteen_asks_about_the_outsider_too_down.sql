-- chair-step: this puts census 13 (`custom.list_door_disagreements`) back to the body it held before lane PORTAL, which enumerated people from `iam.memberships` alone. After it the guard is blind to portal principals again — the one kind of person whose entire access is a grant plus a carried edge — and a list door that disagrees with `custom.read_record` about an outsider goes unreported. It measures; it decides nothing, so nothing a person can do changes.
