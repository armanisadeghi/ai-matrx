-- chair-step: it DROPS custom.table_carries_its_rows and its door row, and puts back the three
-- bodies that called it, so the per-row door and the set-based door hold two copies of one
-- question again.
--
-- THE INVERSE of migrations/campaign/leakt10_one_function_answers_whether_a_table_carries.sql.
-- Run it and census 4 of `pnpm check:store-doors-decide` names custom.visible_set again for
-- reaching a rung of the ladder directly.
...
DELETE FROM platform.client_callable_door
 WHERE schema_name = 'custom' AND function_name = 'table_carries_its_rows';
DROP FUNCTION IF EXISTS custom.table_carries_its_rows(uuid, uuid, permission_level);
