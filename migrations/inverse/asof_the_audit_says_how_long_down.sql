-- STORE-ASOF (3b of 4) — THE INVERSE. `custom.visibility_as_of` goes back to the
-- seven-column VIS-2/GUARD-SWITCH body with no interval. Verbatim the body that stood on
-- the main database before this lane.
--
-- chair-step: putting the seven-column custom.visibility_as_of back needs the same drop the forward file needed, for the same reason.
...
CREATE FUNCTION custom.visibility_as_of(p_organization_id uuid, p_record_id uuid, p_at timestamp with time zone)
