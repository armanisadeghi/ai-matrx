-- INVERSE of migrations/campaign/reldecl_a_column_that_stops_pointing_takes_its_edges.sql
-- Restores the four bodies exactly as they stood before lane RELATION-DECLARE, so a relation
-- column may again point at a uuid that is no Table at all, a caller may again point at a
-- Table they were never shown, and retyping or retiring a relation column again leaves its
-- links live in platform.associations — which is the 23514 that took down the reverse side of
-- every record of the table it pointed at.
...
delete from platform.client_callable_door
 where schema_name = 'custom' and function_name = 'relation_edges_withdraw';

drop function if exists custom.relation_edges_withdraw(uuid, uuid[], text);
