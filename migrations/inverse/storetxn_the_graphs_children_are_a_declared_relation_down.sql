...
--   `migrations/campaign/storetxn_the_graphs_children_are_a_declared_relation.sql`, for rule 27
--   (up -> inverse -> up) ON THE CLONE. It REPLACES the body of `custom.record_write_graph`,
--   which is outside the additive allow-list; it is never run on the main database.
--
-- It puts back the door's FIRST body, byte for byte from
-- `migrations/campaign/storetxn_a_record_graph_is_one_transaction.sql` — the one that linked a
-- child with `public.assoc_add` and was refused by `custom._store_relation_edge_names_its_field`.
-- A `drop function` would NOT be the inverse: the forward file is a REPLACE and declares the
-- body it is based on, so leg 3 of rule 27 has to find that exact body live.
--
