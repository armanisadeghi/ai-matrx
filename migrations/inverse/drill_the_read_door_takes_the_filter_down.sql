-- chair-step: THE INVERSE of drill_the_read_door_takes_the_filter.sql and drill_the_switch_that_opens_the_filtered_door.sql. It is non-additive by construction — it DROPs the two functions that file created (custom.read_records_matching, custom.record_filter_sql), REVOKEs the client grant that file's companion issued, DELETEs the platform.client_callable_door row that file inserted, and restores custom.agg_sql to the exact body it had before, with the filter loop back inside it so nothing is left calling a function that no longer exists. Every object it touches was created or last written by that same pair of files; it removes nothing that existed before them and moves no data. It exists so rule 27 can be run: up, down, up.
-- lane: DRILL
...
-- DRILL — THE INVERSE. Puts the ground back exactly as it stood before
-- drill_the_read_door_takes_the_filter.sql and drill_the_switch_that_opens_the_filtered_door.sql:
-- the filtered door and the extracted evaluator are dropped, its door row and its client
-- grant go with it, and custom.agg_sql is restored to the body it had — with the filter loop
-- back INSIDE it, so nothing is left calling a function that no longer exists.

revoke all on function custom.read_records_matching(uuid, uuid, jsonb, boolean, integer, integer) from authenticated;

delete from platform.client_callable_door
 where schema_name = 'custom' and function_name = 'read_records_matching';
...
drop function if exists custom.read_records_matching(uuid, uuid, jsonb, boolean, integer, integer);
drop function if exists custom.record_filter_sql(jsonb);
