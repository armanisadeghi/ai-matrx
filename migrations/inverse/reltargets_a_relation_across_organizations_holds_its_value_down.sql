...
--   `migrations/campaign/reltargets_a_relation_across_organizations_holds_its_value.sql`.
--   It puts `custom.validate_values` back to the body it had before (the relation arm that
--   demands a live record of THIS organization in the field's `relation_target` table), then
--   drops `custom.relation_value_target_ok`, which only that body calls. No row of anybody's
--   data is touched; a cross-organization or `several`/`any` relation value written while the
--   up stood stays in its document and is refused on that record's next save, which is what
...
--
-- ground-standing-ok: c — `custom.relation_value_target_ok` is this lane's own, created by the
--   up, and after the body below is restored nothing on this database calls it.
...
drop function if exists custom.relation_value_target_ok(uuid, uuid, jsonb, uuid);
