-- chair-step: lane TRASH-COVERAGE-2 inverse. Puts back the body of public._trash_kind_rows that read Organization Trash as one filtered walk (captured live 2026-09-26). Nothing archived or restored is touched.
-- INVERSE of migrations/campaign/trashcoverage2_organization_trash_walks_shared_and_own_personal_rows_apart.sql
-- lane: TRASH-COVERAGE-2
-- ground-standing-ok: b — the body this puts back belongs to trashcoverage2_the_in_parent_title_is_computed_after_the_page_is_cut.sql and calls platform.archived_parent_of, created by trashcoverage2_every_archivable_thing_a_person_sees_is_in_trash.sql. Run this inverse FIRST; the earlier files' inverses run after it, never before.
