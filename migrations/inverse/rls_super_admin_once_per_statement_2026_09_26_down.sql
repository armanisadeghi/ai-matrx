-- chair-step: restores iam._apply_rls_unchecked and iam.verify_canonical to the bodies live before rls_super_admin_once_per_statement_2026_09_26.sql and unwraps the super-admin call in the four tables' policies — only if that change must be undone.
-- based-on: iam._apply_rls_unchecked(text, text, text, text) 44b0c2d63d18e92ca62f1c808d476b9427eb214ea4d9d90e4e8168e6e5665c45
-- based-on: iam.verify_canonical(text, text, text, text) f67fec7e42468598bf7a825a44051b7df8ba992342682b057aab85c511216267
-- Inverse of migrations/rls_super_admin_once_per_statement_2026_09_26.sql. No data was written.
  ...
  FOR r IN
    SELECT n.nspname AS s, c.relname AS t, p.polname AS pol,
     ...
     WHERE (n.nspname, c.relname) IN (('mandate','scan'), ('ai','offering'), ('ai','endpoint'), ('ai','api'))
       AND p.polname IN ('std_select', 'std_insert', 'std_update', 'std_delete')
  LOOP
    v_q := replace(r.q, '( SELECT is_super_admin() AS is_super_admin)', 'is_super_admin()');
    v_c := replace(r.c, '( SELECT is_super_admin() AS is_super_admin)', 'is_super_admin()');
    IF v_q IS DISTINCT FROM r.q THEN EXECUTE format('ALTER POLICY %I ON %I.%I USING (%s)', r.pol, r.s, r.t, v_q); END IF;
    IF v_c IS DISTINCT FROM r.c THEN EXECUTE format('ALTER POLICY %I ON %I.%I WITH CHECK (%s)', r.pol, r.s, r.t, v_c); END IF;
  END LOOP;
END
$unwrap$;
