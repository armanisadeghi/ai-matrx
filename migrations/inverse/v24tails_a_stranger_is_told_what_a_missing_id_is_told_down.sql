-- chair-step: inverse of lane V24-TAILS's campaign file: restores public.access_denied_context byte-for-byte, drops public.access_request_blind (and its door row), retires the platform.access.request_received notice kind (soft). Runs only to undo the ruling.
-- based-on: public.access_denied_context(text, uuid) ad5cd571be125957542c898a234787e6a1c264dd1ca307417148ebfb9030ee30
-- lane: V24-TAILS — inverse of campaign/v24tails_a_stranger_is_told_what_a_missing_id_is_told.sql:
-- puts access_denied_context back to the body it replaced, drops the blind ask, retires the notice kind.
...
DELETE FROM platform.client_callable_door WHERE schema_name = 'public' AND function_name = 'access_request_blind';
DROP FUNCTION IF EXISTS public.access_request_blind(text, uuid, text, text);
UPDATE communication.notification_event_type SET deleted_at = now()
 WHERE event_key = 'platform.access.request_received' AND deleted_at IS NULL;
