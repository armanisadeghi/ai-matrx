-- chair-step: restores the member mandate list's full-ladder bodies (mnd_member_list, the
-- plpgsql five-argument row builder, output_schema_keys / missing_output_keys) and drops the
-- light row builder and the seat that mnd_member_list_ladder_only_for_rows_shown_2026_09_26
-- added — only if that change must be undone.
-- Inverse of migrations/mnd_member_list_ladder_only_for_rows_shown_2026_09_26.sql. Bodies are
-- the live definitions captured 2026-09-26 immediately before the up applied.
...
AS $function$
  ...
  SELECT coalesce(
    (SELECT array_agg(r ORDER BY r)
     FROM unnest(coalesce(p_required_output_keys, ARRAY[]::text[])) r
...
AS $function$
  ...
  WITH raw AS (
    SELECT CASE
             WHEN p_output_schema IS NULL OR jsonb_typeof(p_output_schema) <> 'object'
               THEN NULL::jsonb
             WHEN jsonb_typeof(p_output_schema -> 'schema') = 'object'
               THEN p_output_schema -> 'schema'
             ELSE p_output_schema
           END AS s
  ),
  unwrapped AS (
    SELECT CASE
             WHEN s IS NULL THEN NULL::jsonb
             WHEN s ->> 'type' = 'array' AND jsonb_typeof(s -> 'items') = 'object'
               THEN s -> 'items'
             ELSE s
           END AS s
    FROM raw
  ...
  SELECT coalesce(
    (SELECT array_agg(DISTINCT k)
     FROM unwrapped u,
     LATERAL (
       SELECT jsonb_array_elements_text(u.s -> 'required') AS k
       WHERE jsonb_typeof(u.s -> 'required') = 'array'
       UNION
       SELECT jsonb_object_keys(u.s -> 'properties') AS k
       WHERE jsonb_typeof(u.s -> 'properties') = 'object'
     ) keys),
    ARRAY[]::text[]);
$function$;
