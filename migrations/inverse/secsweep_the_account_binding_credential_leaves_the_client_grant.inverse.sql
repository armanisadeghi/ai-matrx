...
-- migrations/campaign/secsweep_the_account_binding_credential_leaves_the_client_grant.sql
--
-- Removes the declared exclusion and re-derives the grants, putting each table back on
-- a table-level grant to `authenticated`. It re-opens the account binding credential reference to every signed-in
