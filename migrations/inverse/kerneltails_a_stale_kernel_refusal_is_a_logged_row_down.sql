-- INVERSE of migrations/campaign/kerneltails_a_stale_kernel_refusal_is_a_logged_row.sql (lane KERNEL-TAILS).
-- Restores platform.provision and platform.provision_batch byte-for-byte (the preflight raise only)
-- and drops the three helpers. ops.system_error rows already written stay (they are history).
...
drop function if exists platform._provisioner_refuses_a_stale_kernel(jsonb, jsonb, text, uuid, text);
drop function if exists iam.entity_read_kernel_members_expected();
drop function if exists iam.entity_read_kernel_members_live();
