...
-- chair-step: the inverse of oneview_every_view_bar_view_moves_into_the_one_store.sql. It brings
--   every `records_ui_view` Table back — the Table, its Fields and the view records the move
--   archived, found by the one instant the move wrote (`moved_from.archived_at`), so a view that
--   was archived BEFORE the move stays archived — and WITHDRAWS the copies the move made in
--   `platform.saved_view` (soft: `deleted_at` set and `moved_from.withdrawn` true; nothing is
--   deleted; re-applying the move brings each copy back as it was, edits included). A mover
--   view's hidden columns go back to `definition.hidden_fields` by Field id.
...
begin
  ...
  raise notice 'S0 ONE-SAVED-VIEW inverse: % record(s) of records_ui_view tables brought back; % copied view(s) withdrawn; % hidden-column list(s) put back by id.',
    v_restored, v_withdrawn, v_hidden;
end
$back$;
