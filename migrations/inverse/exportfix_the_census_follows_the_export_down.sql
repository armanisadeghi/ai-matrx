...
--
-- THE INVERSE of migrations/campaign/exportfix_the_census_follows_the_export.sql: it puts back
-- the census as it stood on 2026-09-20, in which every list-shaped door had to name
-- `custom.visible_set`, `custom.visible_predicate_sql` or `custom.query_visible_ids` in its own
-- body — so `custom.io_export`, which builds no predicate of its own and takes the read door's
-- answer instead, was reported `unmeasured` on every run.
