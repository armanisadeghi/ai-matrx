-- chair-step: inverse of admin_seat_platform_scopes_2026_09_26.sql — puts the admin mandate list back on Mine / Org / System over the viewer's own orgs and removes the admin platform scopes from the agents list. Run only to back the admin-seat change out.
-- inverse of admin_seat_platform_scopes_2026_09_26.sql
-- WHAT IT DOES NOT UNDO: nothing is lost — no data was written. Each body below is the live body
-- with the 2026-09-26 admin-seat edits reversed text-for-text (the same fragments the forward
-- patches replaced), then the two owner helpers are dropped once nothing calls them.
-- Frontend coupling: the admin list sends p_scope 'users'/'all', which the restored
-- mnd_admin_list refuses — revert features/mandates/admin-list/service.ts with it.
...
DROP FUNCTION IF EXISTS mandate._admin_owner_label(uuid, boolean);
DROP FUNCTION IF EXISTS mandate._admin_owner_level(uuid, boolean);
