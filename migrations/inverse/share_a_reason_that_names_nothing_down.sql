-- chair-step: the inverse of share_a_reason_that_names_nothing.sql. It puts custom.share_access back to the body share_the_store_has_a_share_door.sql wrote (sha256 c0b54fc6…, the up-file's own based-on record), whose containment reason asks platform.entity_title and so names nothing for a custom record. It replaces that body in place and never drops it, so its grants and its door row stand. It leaves custom.share_subject_name, its door row and every row of data alone: share_subject_name was born in the up-file, has no earlier body anywhere in history (its based-on hash is the up-file's own output), and tails2_a_name_belongs_where_a_name_belongs.sql adopted it, so dropping it would break a later lane. Rule 27 only. Rebased onto these bytes by lane LEDGER-REBASE (chair ruling 2026-09-23) after the bytes that ran on 2026-09-19 were found to exist nowhere.
