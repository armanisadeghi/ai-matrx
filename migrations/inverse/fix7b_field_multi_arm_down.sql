-- INVERSE of migrations/campaign/fix7b_field_multi_arm.sql — FIX-7B-FIELD.
--
-- Restores `custom.field_update` to the body it had on the main database on 2026-09-20,
-- before the `multi` arm was added to its settings branch: the definition whose hash is
-- `6830cdc5970e16dff7631d9b49b7d1f574a0bdf7ad7cd7bc704040f1d709237e`, dumped with
-- `pg_get_functiondef` and pasted here unchanged.
--
-- APPLYING THIS PUTS THE DEFECT BACK, deliberately and knowingly: a patch carrying `multi`
-- alone is answered with the field id and changes nothing. `scripts/campaign-tests/fix7b_field_red.sql`
-- runs these exact bytes inside a rolled-back transaction to prove the green suite is
