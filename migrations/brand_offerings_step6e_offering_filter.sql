-- Brand-offerings cutover, step 6e: filter, sort and placement-source reads
-- over the canonical offering model.
--
-- Plan of record: docs/db_rebuild/proposals/brand-offerings-cutover.md.
--
-- 1. seo.gsc_offering_keyword_set(site, offering) - the keywords THIS site has
--    placed on an offering or anything beneath it in the brand's catalog. The
--    canonical twin of seo.gsc_topic_keyword_set, which had no site at all.
-- 2. seo.gsc_perf_breakdown / seo.gsc_breakdown_keyword_ids accept
--    filters.offering (an offering id or "none") beside the legacy
--    filters.topic key the deployed screens still send, and gsc_perf_breakdown
--    sorts by 'offering'. The placement-source filter (proposed / AI / human)
--    now reads this site's canonical placement instead of every tenant's topic
--    rows.
-- 3. seo.gsc_confirm_keyword_topic (TRANSITION) also marks the site's
--    canonical placement confirmed, so the proposed filter and the approval
--    queue agree while the queue still calls the legacy confirm.
--
-- Both breakdown bodies are the live bodies with these edits applied, generated
-- from pg_get_functiondef at authoring time; the based-on lines refuse the apply
-- if either moved since.
--
-- based-on: seo.gsc_perf_breakdown(uuid, text, date, date, date, date, jsonb, text, text, text, integer, integer) 9fa7df1188af81412197570938c015de806c219e7340bce08471cb310aef43c0
-- based-on: seo.gsc_breakdown_keyword_ids(uuid, date, date, jsonb, text, integer) 0e3158a3361c5c35f33f40a5bd1fa7fe9e260ce176b6b15c1801aa76381a4bdb
-- based-on: seo.gsc_confirm_keyword_topic(uuid, uuid[]) e14dec2c5d2621559844d4a9e66bcdb7b981dc12c041baa0b94f8517e4a195ec

CREATE OR REPLACE FUNCTION seo.gsc_offering_keyword_set(p_site_id uuid, p_offering_id uuid)
...
AS $function$
  WITH RECURSIVE sub AS (
    SELECT bo.id, 0 AS depth FROM web.brand_offering bo
    WHERE bo.id = p_offering_id AND bo.deleted_at IS NULL
    UNION ALL
    SELECT bo.id, s.depth + 1
    FROM sub s JOIN web.brand_offering bo ON bo.parent_id = s.id AND bo.deleted_at IS NULL
    WHERE s.depth < 32
  )
  SELECT DISTINCT sko.keyword_id
  FROM seo.site_keyword_offering sko
  JOIN sub ON sub.id = sko.brand_offering_id
  WHERE sko.site_id = p_site_id AND sko.is_primary AND sko.deleted_at IS NULL;
$function$;

REVOKE ALL ON FUNCTION seo.gsc_offering_keyword_set(uuid, uuid) FROM PUBLIC, anon, authenticated;
COMMENT ON FUNCTION seo.gsc_offering_keyword_set(uuid, uuid) IS
  'Keywords this site has placed on an offering or anything beneath it in the brand catalog. Internal to the site-guarded breakdown reads; not client-callable.';
...
DECLARE
  ...
  -- TRANSITION (brand-offerings cutover): `topic` is the legacy key the
  -- deployed screens still send (a seo.topic id); `offering` is the canonical
  -- key (a brand offering id, resolved for THIS site only). The topic branches
  -- are deleted when the legacy key has no caller.
  f_tp text := NULLIF(btrim(p_filters->>'topic'), '');
  f_tp_id uuid;
  f_of text := NULLIF(btrim(p_filters->>'offering'), '');
  f_of_id uuid;
...
BEGIN
  ...
  END IF;
  IF p_sort NOT IN ('clicks', 'impressions', 'ctr', 'position', 'key', 'delta_clicks', 'topic', 'offering', 'traffic_class', 'value_score', 'value_band') THEN
  ...
  IF f_tp IS NOT NULL AND f_tp <> 'none' THEN
    ...
    END;
  END IF;
  IF f_of IS NOT NULL AND f_of <> 'none' THEN
    BEGIN
      f_of_id := f_of::uuid;
    EXCEPTION WHEN others THEN
      RAISE EXCEPTION 'gsc_offering_filter_invalid: the offering filter takes an offering id or "none".';
    END;
  END IF;
  ...
  offering_set AS (
    SELECT s.kw_id FROM seo.gsc_offering_keyword_set(p_site_id, f_of_id) s WHERE f_of_id IS NOT NULL
  ),
  loc_hits AS (
  ...
  latest AS (
    ...
    WHERE spd.provider = 'gsc'
      ...
      AND (f_of IS NULL
           OR (f_of = 'none'
               AND NOT EXISTS (SELECT 1 FROM seo.site_keyword_offering sko
                                WHERE sko.site_id = p_site_id AND sko.keyword_id = spd.keyword_id
                                  AND sko.is_primary AND sko.deleted_at IS NULL))
           OR (f_of_id IS NOT NULL AND spd.keyword_id IN (SELECT os.kw_id FROM offering_set os)))
      AND (f_pl IS NULL
           -- Who placed this keyword on one of THIS site's offerings (the
           -- canonical placement; it used to read every tenant's rows).
           OR EXISTS (SELECT 1 FROM seo.site_keyword_offering sko
                       WHERE sko.site_id = p_site_id AND sko.keyword_id = spd.keyword_id
                         AND sko.is_primary AND sko.deleted_at IS NULL
                         AND (f_pl <> 'proposed'
                              OR sko.metadata #>> '{placement,confirmed}' = 'false')
                         AND (f_pl <> 'human' OR sko.assigned_by = 'human')
                         AND (f_pl <> 'agent' OR sko.assigned_by IS DISTINCT FROM 'human')))
      AND (f_lo IS NULL
  ...
  ORDER BY
    ...
    (CASE WHEN p_sort IN ('topic', 'offering', 'traffic_class', 'value_band') AND p_sort_dir = 'desc' THEN f.s_txt END) DESC NULLS LAST,
    (CASE WHEN p_sort IN ('topic', 'offering', 'traffic_class', 'value_band') AND p_sort_dir = 'asc' THEN f.s_txt END) ASC NULLS LAST,
...
CREATE OR REPLACE FUNCTION seo.gsc_breakdown_keyword_ids(p_site_id uuid, p_start date, p_end date, p_filters jsonb DEFAULT '{}'::jsonb, p_search text DEFAULT NULL::text, p_limit integer DEFAULT 5000)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'seo', 'pg_temp'
...
CREATE OR REPLACE FUNCTION seo.gsc_confirm_keyword_topic(p_site_id uuid, p_keyword_ids uuid[])
...
begin
    ...
    -- TRANSITION (brand-offerings cutover): the same confirmation on this
    -- site's canonical placement, which the proposed filter now reads.
    update seo.site_keyword_offering sko
       set metadata = jsonb_set(
                        jsonb_set(coalesce(sko.metadata, '{}'::jsonb),
                                  '{placement,confirmed}', 'true'::jsonb, true),
                        '{placement,confirmed_at}', to_jsonb(now()), true),
           updated_at = now(),
           updated_by = (select auth.uid())
     where sko.site_id = p_site_id
       ...
       and sko.metadata #>> '{placement,confirmed}' = 'false';
