...
INSERT INTO skill.definition (
  ...
  is_active, is_system, is_public, visibility, organization_id
)
SELECT
...
WHERE NOT EXISTS (
  ...
  WHERE skill_id = 'troubleshooting-guides' AND user_id IS NULL
);
...
UPDATE skill.definition SET
  ...
  description      = 'How and when to emit <troubleshooting> render blocks: the exact tag + inner markdown structure (symptom, causes, solutions, steps), syntax rules that prevent render failures, sizing guidance, and editing etiquette.',
  ...
  body             = $BODY$# Troubleshooting Guides
...
```
$BODY$
WHERE skill_id = 'troubleshooting-guides'
  AND user_id IS NULL
