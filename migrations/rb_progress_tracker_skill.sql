...
INSERT INTO skill.definition (
  ...
  platform_targets, version, icon_name,
...
SELECT
  ...
  true, true, true, 'public'::platform.visibility,
  '["web"]'::jsonb,
  '1.0.0',
  'ListChecks',
...
WHERE NOT EXISTS (
  ...
  WHERE skill_id = 'progress-tracker' AND user_id IS NULL
);

-- Refresh body/metadata on re-apply (global row: user/project/task NULL).
UPDATE skill.definition SET
  label = 'Progress Tracker',
  description = 'How and when to emit <progress_tracker> render blocks: the tag, the inner markdown checklist structure, per-item modifiers (priority / hours / optional), syntax rules that prevent render failures, sizing guidance, and editing etiquette.',
  skill_type = 'render_block'::public.skl_skill_type,
  is_system = true, is_public = true, is_active = true,
...
```
$BODY$
WHERE skill_id = 'progress-tracker'
  AND user_id IS NULL
