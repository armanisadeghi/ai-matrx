...
--
-- The helper lives in PUBLIC (a SECURITY DEFINER wrapper that reads iam.*
-- internally), exactly like the assoc_* / mbr_* RPCs. It must NOT live in the
-- iam schema: RLS expressions evaluate as the INVOKING user, and `authenticated`
-- has no USAGE on schema iam (clients reach iam only through public RPCs), so a
-- policy calling `iam.<fn>()` directly errors with "permission denied for schema
-- iam". `public.user_container_ids` is callable (public usage + EXECUTE grant)
-- and reads iam.memberships in the definer's context.
...
create or replace function public.user_container_ids(p_container_type text, p_role_filter text[] default null)
returns setof uuid
...
$fn$;
revoke all on function public.user_container_ids(text, text[]) from public;
grant execute on function public.user_container_ids(text, text[]) to authenticated;

-- ctx_tasks
alter policy tasks_delete on public.ctx_tasks using (
  (user_id = auth.uid())
  OR (project_id IN ( SELECT cid FROM public.user_container_ids('project', ARRAY['owner','admin']) cid ))
...
alter policy tasks_select on public.ctx_tasks using (
  ...
  OR (project_id IN ( SELECT cid FROM public.user_container_ids('project') cid ))
...
alter policy skl_cat_select on public.skl_categories using (
  ...
  OR ((project_id IS NOT NULL) AND (project_id IN ( SELECT cid FROM public.user_container_ids('project') cid )))
...
alter policy skl_defs_insert on public.skl_definitions with check (
  ...
  OR ((project_id IS NOT NULL) AND (project_id IN ( SELECT cid FROM public.user_container_ids('project', ARRAY['owner','admin']) cid ))));
...
-- Remove the earlier iam-schema helper (uncallable from RLS — authenticated has
-- no USAGE on schema iam). Superseded by public.user_container_ids above.
drop function if exists iam.user_container_ids(text, text[]);
