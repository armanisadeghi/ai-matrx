...
-- read_lane_v2_b3_40_web_page — enroll web.page in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b3' from platform.entity_types where schema_name = 'web' and table_name = 'page' and is_active on conflict do nothing;
