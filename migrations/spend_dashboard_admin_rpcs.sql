...
AS $function$
  SELECT * FROM (VALUES
    ...
    ('rag.kg_sweep_run', 'Knowledge-graph sweeps', 'rag', 'kg_sweep_run', 'cost_usd', 'created_at', 'gap',
     'Rows exist, cost is always 0. Sweeps call models, so this is under-reporting, not free work.'),
    ...
    ('docproc.page_extraction_runs', 'Page extraction runs', 'docproc', 'page_extraction_runs', 'total_cost', 'created_at', 'gap',
     'Cost column exists and has never been written.'),
    ('docproc.processed_documents', 'Document cleaning', 'docproc', 'processed_documents', 'clean_content_cost_usd', 'created_at', 'gap',
     'Cost column exists and has never been written.'),
    ('communication.sms_messages', 'SMS (Twilio)', 'communication', 'sms_messages', 'price', 'created_at', 'gap',
     'Twilio returns a price per message and we do not store it.'),
    ...
    ('unmeasured.speech',       'Text-to-speech and transcription', NULL, NULL, NULL, NULL, 'unmeasured',
     'TTS and STT provider calls. No cost row anywhere.'),
    ('unmeasured.search_apis',  'Search and SEO data APIs',       NULL, NULL, NULL, NULL, 'unmeasured',
     'SerpAPI, DataForSEO and Brave are billed per call. No cost row anywhere.'),
