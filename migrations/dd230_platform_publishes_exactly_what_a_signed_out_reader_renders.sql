-- dd230_platform_publishes_exactly_what_a_signed_out_reader_renders
...
--   platform.feature_knob → 3 column(s). lib/knobs/featureKnobs.ts — client feature gating has to resolve before sign-in (its PUBLIC_EXPOSURE_ALLOWED row says so, and it is true). Measured: 194 anon 200s in 24 h, every one select=feature,key,value. Three columns of twenty-three.
--
-- ═══ CLOSED — no signed-out reader exists, so the bound is the EMPTY list ═════════════════════
--   platform.assurance_level — one curl probe. A "static reference enum" with no reader is still a door.
--   platform.flexible_data — one curl probe. features/content-ir/registry/schema-source-flexible-data.ts is the SUPERSEDED schema source; the canonical one is schema-source-kind-tables.ts, which reads content_ir.
--   platform.rulebook — zero anon requests in 24 h.
--   platform.shareable_resource_registry — the ten anon-shaped reads in 24 h all carry the exact select= of scripts/regen-shareable-registry-snapshot.ts, which prefers SUPABASE_SECRET_KEY; a new-style sb_secret_ key carries no JWT and therefore logs with an empty role, which is why it LOOKS anonymous. Its publishable-key fallback is removed in this commit so the script can never be an anonymous reader again.
--   platform.source_authority — one curl probe.
...
begin
  for r in
    select * from (values
      ('platform.feature_knob', array['feature', 'key', 'value'])
    ) as t(rel, keep)
  loop
