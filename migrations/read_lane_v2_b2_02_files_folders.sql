-- read_lane_v2_b2_02_files_folders — enroll files.folders in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b2' from platform.entity_types where schema_name = 'files' and table_name = 'folders' and is_active on conflict do nothing;
