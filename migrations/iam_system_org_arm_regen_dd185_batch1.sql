-- iam_system_org_arm_regen_dd185_batch1 — DD-185 REGENERATION BATCH 1 OF 5 (15 tokens).
--
...
declare
  ...
  v_tokens text[] := array['agent_run','assessment_result','assist','browser_profile','commerce_cloud_sync_connection','commerce_intake_batch','commerce_label_batch','commerce_marketplace_account','commerce_product','esign_envelope_event','game_badge','game_result','heatmap_save','hindsight_enrollment','hr_alert_routing_rule'];
...
begin
  ...
  loop
    ...
    if not (v_pre -> r.token ? 'system_org_arm_respects_class') then
      raise exception 'dd185b1: % does not FAIL system_org_arm_respects_class before this file runs. It is not in the RED set this batch was written against; re-census.', r.token;
    end if;
  end loop;

  -- ═══════ 2. THE BEFORE, pinned to this transaction's instant ═════════════════════════════════
  v_before := iam.access_delta_snapshot('DD-185 b1 BEFORE', v_principals, v_tokens, 400000,
    'DD-185 batch 1: confidential tokens carrying the every-signed-in-user system-org arm, before regeneration', v_as);
  ...
  v_after := iam.access_delta_snapshot('DD-185 b1 AFTER', v_principals, v_tokens, 400000,
    'DD-185 batch 1 confirmation, pinned to the same instant as the baseline', v_as);
  ...
  loop
    if r.verdict = 'WIDER' then
      if not ((r.token || '|' || r.principal_label) = any (v_approved)) then
        ...
        raise notice 'dd185b1: WIDER (approved by name) % for % : % -> %', r.token, r.principal_label, r.count_before, r.count_after;
      end if;
      ...
      v_narrower := array_append(v_narrower,
        format('%s for %s (%s -> %s)', r.token, r.principal_label, r.count_before, r.count_after));
      raise notice 'dd185b1: NARROWED % for % : % -> %', r.token, r.principal_label, r.count_before, r.count_after;
    end if;
  end loop;
  if cardinality(v_hard) > 0 then
    raise exception 'dd185b1: % read(s) this gate could not prove either way: %', cardinality(v_hard), array_to_string(v_hard, ' ; ');
  end if;
  if cardinality(v_unapproved) > 0 then
    raise exception 'dd185b1: % door(s) opened that nobody approved by name: %. Over-opening and over-tightening are the same class of defect and neither ships on a count.',
  ...
  if cardinality(v_stillred) > 0 then
    raise exception 'dd185b1: % token(s) still FAIL system_org_arm_respects_class after regeneration: %. The generator did not remove the arm this batch exists to remove.',
      cardinality(v_stillred), array_to_string(v_stillred, ', ');
  end if;
  if cardinality(v_newfail) > 0 then
    raise exception 'dd185b1: % verify_canonical FAIL(s) that were NOT there before this file ran: %. A regeneration that introduces a finding does not stand.',
      cardinality(v_newfail), array_to_string(v_newfail, ' ; ');
  end if;
  raise notice 'dd185b1: 0 unapproved widenings, % narrowing(s), 0 new verify_canonical FAILs, % token(s) FAIL -> PASS on system_org_arm_respects_class',
