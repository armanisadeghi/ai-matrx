-- get_user_table_data_paginated: stable, total-order pagination.
--
...
-- non-unique: sorting on an arbitrary `data->>'<field>'` JSON value (ties are
-- the normal case for a user dataset column), and the `created_at DESC`
...
BEGIN
    ...
    IF p_search_term IS NOT NULL THEN
        v_query := v_query || ' AND (data::text ILIKE ''%' || p_search_term || '%'')';
    END IF;

    IF v_field_name IS NOT NULL THEN
        v_query := v_query || ' ORDER BY (data->>''' || v_field_name || ''')';
