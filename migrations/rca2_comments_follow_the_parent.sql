-- RC-A2 — A COMMENT FOLLOWS THE RECORD IT IS ON (rich-content STORE-DESIGN §3.8, P2).
-- chair-step: replace comment policy and RPC bodies so every comment access follows the parent record, preserving D347 anonymous publication gates.
-- Register: common-docs/projects/rich-content-unification/REGISTER.md row RC-A2.
--
-- THE DEFECT, measured live 2026-09-23 by scripts/campaign-tests/rca2_comments_follow_the_parent.sql
-- (red on the code this file replaces, 20 findings): platform.comments was generated as an
-- `entity`, so a comment was read by its OWN visibility plus organization membership, and
-- public.cmt_add / cmt_list / cmt_delete asked only iam.has_org_access. A plain member of an
-- organization therefore read, added to and soft-deleted the comments on another member's
-- PERSONAL note, task and CRM party — and a comment quotes the thing it is about, so a private
-- record leaked through its comments. A viewer could add and edit; nothing could resolve.
--
-- THE CLASS FIX, not the instance: a comment is a DETAIL of its parent, and the parent is named
-- by the row itself — (entity_type, entity_id). So:
--   1. platform.comments gains anchor / resolved_at / resolved_by (the columns custom.io_comment
--      has; its 36 rows migrate in later, STORE-DESIGN §3.8 item 4 — not this file).
--   2. iam.apply_rls gains the `detail` lane (the value platform.entity_types.rls_variant has
--      admitted, unused, since the seven table types were declared): read = viewer on the
--      parent, insert = author + commenter, update = author + commenter, delete = author or
--      admin. No platform-staff or organization lane — a detail reaches exactly as far as its
--      parent. iam.verify_canonical learns to certify it.
--   3. `comment` is registered as a detail and regenerated through the generator — no
--      hand-written policy.
--   4. The doors ask the same question: cmt_add commenter, cmt_list viewer (once per call),
--      cmt_edit author + commenter, cmt_delete author or admin, and the new cmt_resolve
--      commenter. A refused edit/delete/resolve now says so instead of updating zero rows.
--
-- Every parent type is covered by construction: the lane asks iam.has_access about whatever
-- (entity_type, entity_id) the row names. Live parent types on 2026-09-23: task (3), party (2);
-- client callers also name crm_deal (the same notes surface) — none needs a line of its own.
--
-- NOT in this file, on purpose: dropping platform.comments.visibility (the aidream ORM's
-- MatrxEntity base still maps it; the lane no longer reads it and verify_canonical WARNs it as
-- a stray second authority), and cmt_add's p_anchor parameter (a new identity needs a DROP of
-- the old one, and it lands with the text_anchor validator and its first writer).
--
-- Emergency post-commit containment: migrations/inverse/rca2_comments_follow_the_parent_contain.sql.
-- The old down migration is retired: it restores the personal-record leak and pre-D347 IAM bodies.
-- based-on: iam._apply_rls_unchecked(text, text, text, text) ffc1abf0ea416b8dd82ecd5bac39ab69b788658ecee0846a25fc775fb70d866e
-- based-on: iam.verify_canonical(text, text, text, text) d53da14b182fb34502cfe6370c55a61dd5f34889f04eccd01079260e1f54eaa5
-- based-on: public.cmt_add(text, uuid, text, uuid, uuid) aacbcef0909018561fed8b3498f0bb55294d4b09aa5c95d31506e1a5b8b27485
-- based-on: public.cmt_list(text, uuid) 1acd8c5c6ba88812a02236810dac6d9ce1e8e152e2c79146845a54b8c66c857c
-- based-on: public.cmt_edit(uuid, text) 009cb380f176cb88ac2c69075f0e3dff16c3968c2146030caeda665c7632c433
-- based-on: public.cmt_delete(uuid) 8aa342da25cd47025b12c0f9882fd39ffe88cc6bb854516605d1151bc40f1210

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. THE COLUMNS. Five rows; each ADD COLUMN is catalog-only.
-- ─────────────────────────────────────────────────────────────────────────────
...
comment on column platform.comments.anchor is
  'Where in the parent this comment points (RC-A2 / STORE-DESIGN §3.8): a kind payload — text_anchor (W3C TextQuote+TextPosition against a content_version) or record_field_anchor (a field of a record, the custom.io_comment field_key). Null = the whole record. It quotes the parent, which is why reading a comment needs viewer on the parent.';
comment on column platform.comments.resolved_at is 'When the thread was resolved (public.cmt_resolve, commenter on the parent). Null = open.';
comment on column platform.comments.resolved_by is 'Who resolved the thread. Null while open.';

alter table platform.comments
  add constraint comments_anchor_is_a_kind check (
    anchor is null
    or (jsonb_typeof(anchor) = 'object'
        and anchor ->> '__kind' in ('text_anchor', 'record_field_anchor')));

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. THE GENERATOR LEARNS THE DETAIL LANE, AND THE VERIFIER LEARNS TO CERTIFY IT.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if p_variant = 'detail' then
    ...
    v_pol := v_pol || format(
      'create policy std_select on %s for select to authenticated using (iam.has_access(entity_type, entity_id, ''viewer''::public.permission_level))',
      v_tbl);
    if not v_no_client_writes then
    v_pol := v_pol || format(
      'create policy std_insert on %s for insert to authenticated with check (created_by = (select auth.uid()) and iam.has_access(entity_type, entity_id, ''commenter''::public.permission_level))',
      v_tbl);
    end if;
    if not v_no_client_writes then
    v_pol := v_pol || format(
      'create policy std_update on %s for update to authenticated using (created_by = (select auth.uid()) and iam.has_access(entity_type, entity_id, ''commenter''::public.permission_level)) with check (created_by = (select auth.uid()) and iam.has_access(entity_type, entity_id, ''commenter''::public.permission_level))',
      v_tbl);
    end if;
    if not v_no_client_writes then
    v_pol := v_pol || format(
      'create policy std_delete on %s for delete to authenticated using (created_by = (select auth.uid()) or iam.has_access(entity_type, entity_id, ''admin''::public.permission_level))',
      v_tbl);
    end if;
    perform iam._rls_plan_is(v_tbl, v_pol, v_kept);  -- POLICY-LOCK: the grants rail asks the caller what the policy set is ABOUT to be
    perform iam.apply_table_grants(p_schema, p_table, p_variant);
...
BEGIN
  ...
  v_actor_req    := v_variant IN ('entity','system','restricted','detail');  -- RC-A2: a detail's created_by is its author
  ...
  IF f_vis AND v_variant IN ('component','ledger','reference','detail') THEN
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. THE DOORS ASK THE SAME QUESTION THE TABLE DOES.
-- ─────────────────────────────────────────────────────────────────────────────
-- public.cmt_add — commenter on the record, decided before anything about the record is said.
...
-- public.cmt_list — viewer on the record, asked ONCE per call (an InitPlan, not per row).
...
-- public.cmt_edit — the author, still holding commenter on the record. Refuses out loud.
...
-- public.cmt_delete — the author, or admin on the record. Refuses out loud.
...
AS $function$
begin
  -- RC-A2: the second arm was iam.has_org_access(organization_id), which let any member of the
  -- organization soft-delete anybody's comment on anything (measured 2026-09-23: a plain member
  -- deleted the owner's comments on three private records). Admin on the RECORD is the rung
  -- that may moderate its discussion.
  update platform.comments c
     set deleted_at = now(), updated_by = (select auth.uid())
   where c.id = p_id and c.deleted_at is null
     and (c.created_by = (select auth.uid())
          or iam.has_access(c.entity_type, c.entity_id, 'admin'::public.permission_level));
  if not found then
    raise exception 'cmt_delete: comment not found, or you may not delete it -- its author may, and so may an admin of the record it is on'
      using errcode = '42501';
  end if;
end $function$;

-- public.cmt_resolve — NEW. Commenter on the record. Resolution is not deletion.
CREATE OR REPLACE FUNCTION public.cmt_resolve(p_id uuid, p_resolved boolean DEFAULT true)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
begin
  -- The resolve half custom.io_comment already had (custom.io_comment_resolve), on the one
  -- comment table (STORE-DESIGN §3.8 item 3). p_resolved=false re-opens the thread.
  update platform.comments c
     set resolved_at = case when p_resolved then now() end,
         resolved_by = case when p_resolved then (select auth.uid()) end,
         updated_by  = (select auth.uid())
   where c.id = p_id and c.deleted_at is null
     and iam.has_access(c.entity_type, c.entity_id, 'commenter'::public.permission_level);
  if not found then
    raise exception 'cmt_resolve: comment not found, or you may not resolve it -- resolving needs the commenter level on the record it is on'
      using errcode = '42501';
  end if;
end $function$;
...
values ('public', 'cmt_resolve', 'p_id uuid, p_resolved boolean',
        array['uuid'::regtype, 'bool'::regtype]::oid[],
        'Resolve or re-open one comment thread on the unified comment table. p_id is admitted only when the caller holds COMMENTER on the record the comment is on (iam.has_access on the row''s own entity_type/entity_id), and a comment that does not exist is refused with the same 42501, so the argument is no existence oracle. p_resolved is a boolean and selects nothing.',
        'rca2_comments_follow_the_parent.sql',
        true, false)
on conflict do nothing;

grant execute on function public.cmt_resolve(uuid, boolean) to authenticated;

-- The declared argument rule for cmt_add named the check it used to make; say the new one.
update platform.client_callable_door d
   set argument_rules = jsonb_set(
         jsonb_set(d.argument_rules, '{arguments,p_entity_id,check}',
                   to_jsonb('p_entity_id -> iam.has_access(p_entity_type, p_entity_id, commenter); refused 42501 alike whether the record is missing or merely not yours (a caller-named p_org_id is checked by iam.has_org_access before the record is compared with it)'::text)),
         '{arguments,p_entity_id,verified}',
         to_jsonb('live 2026-09-23, rca2_comments_follow_the_parent.sql, as a member with no grant'::text))
 where d.schema_name = 'public' and d.function_name = 'cmt_add'
   and d.argument_rules -> 'arguments' ? 'p_entity_id';

-- ─────────────────────────────────────────────────────────────────────────────
-- 4. `comment` IS A DETAIL, AND ITS POLICIES COME FROM THE GENERATOR — LAST, ON PURPOSE.
-- ─────────────────────────────────────────────────────────────────────────────
-- POLICY-LOCK: the policy statements take ACCESS EXCLUSIVE on the auth/storage/realtime
-- relations Supabase's supautils hook names, held to COMMIT. Nothing may follow them.
update platform.entity_types
   set rls_variant = 'detail',
       type = 'detail'
 where token = 'comment';
