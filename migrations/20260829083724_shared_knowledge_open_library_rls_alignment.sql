-- Shared Knowledge Open Library RLS alignment.
--
-- The access kernel correctly treats industry/global Library publications as
-- readable by every authenticated user. Generated entity SELECT policies,
-- however, bounded their exact iam.has_access() check with candidate queries
-- against protected platform tables. Those subqueries run as the caller and
-- return no reachability/entity-grant rows for non-admin users. Use the
-- existing SECURITY DEFINER set resolver as a candidate superset, then keep
-- iam.has_access() as the exact final authority.
--
-- Fresh ingest also emits source_kind=library_doc for the processed row. That
-- legacy spelling now maps to processed_document and receives the canonical
-- registry-driven data_store conveyance edge.
...
-- Regenerate only the canonical entity policies exercised by this matrix.
-- iam.apply_rls remains the sole policy generator; no parallel read policy is
-- introduced.
select iam.apply_rls('rag', 'data_stores', 'data_store', 'entity');
select iam.apply_rls('files', 'files', 'file', 'entity');

-- processed_documents predates the Base-1 generator (owner_id, nullable org,
-- no created_by). Its Library-read policy is correct, but this stale
-- restrictive SELECT policy intersected every permissive read down to platform
-- admins only.
drop policy if exists platform_admin_select_only on docproc.processed_documents;
...
as $$
  select case p_kind
    when 'cld_file' then 'file'
    when 'library_doc' then 'processed_document'
    else p_kind
  end;
$$;

comment on function rag.member_source_entity_token(text) is
  'Maps data-store member source kinds to canonical entity tokens: cld_file to file and library_doc to processed_document.';

insert into platform.association_types (
  source_type, target_type, label, container_side, conveys_max, is_active, notes
)
values (
  'processed_document', 'data_store', null, 'target', 'viewer', true,
  'Shared Knowledge: a data store contains processed library documents. Store publication conveys viewer through reachability. 2026-08-29'
)
...
where dm.deleted_at is null
  and dm.source_kind = 'library_doc'
