-- iam.verify_canonical: add a legacy_is_deleted WARN check (parallel to legacy_is_public).
-- Makes "fully canonical = zero FAIL + zero WARN" objective: a lingering is_deleted column
-- (after deleted_at exists) is now surfaced, so the soft-delete cleanup has a machine-checkable gate.
-- Applied live to txzxabzwovsujtloxrus 2026-06-27. Idempotent (CREATE OR REPLACE).
...
BEGIN
  ...
  IF v_variant='entity' THEN
    ...
    check_name:='legacy_is_deleted'; status:=CASE WHEN v_has_isdeleted THEN 'WARN' ELSE 'PASS' END;
      detail:=CASE WHEN v_has_isdeleted THEN 'is_deleted present; deleted_at is the canonical soft-delete (drop after repoint)' ELSE NULL END; RETURN NEXT;
