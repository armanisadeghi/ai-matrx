-- read_lane_v2_b2_37_chat_code_edit — enroll chat.code_edit in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b2' from platform.entity_types where schema_name = 'chat' and table_name = 'code_edit' and is_active on conflict do nothing;
