-- dd230_iam_publishes_exactly_what_a_signed_out_reader_renders
...
--   iam.industries — its exposure row says "the industry picker must populate on the sign-up form, before an account exists". Measured: https://www.aimatrx.com/sign-up, signed out, issues ZERO requests to db.matrxserver.com, and every caller of useIndustries()/fetchIndustries() is an admin or organization surface. The two anon 200s in 24 h came from a headless browser on production with no session — a signed-in surface racing its own hydration, not the sign-up form.
--   iam.permissions — zero anon requests in 24 h.
