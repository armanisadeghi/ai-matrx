-- dd186_anon_columns_skill — THE SIGNED-OUT COLUMN SURFACE OF THE `skill` SCHEMA
...
-- skill.definition — revoked: is_system, organization_id, created_by, updated_by, metadata, version
revoke select on skill.definition from anon;
grant select (id, skill_id, label, description, skill_type, body, icon_name, model_preference, allowed_tools, trigger_patterns, disable_auto_invocation, platform_targets, semver, config, category_id, parent_skill_id, is_active, sort_order, project_id, task_id, created_at, updated_at, visibility, deleted_at) on skill.definition to anon;

-- skill.render_definition — revoked: organization_id, created_by, updated_by, version, metadata
revoke select on skill.render_definition from anon;
grant select (id, block_id, label, description, icon_name, template, sort_order, is_active, skill_id, category_id, project_id, task_id, created_at, updated_at, visibility, deleted_at, block_type) on skill.render_definition to anon;
...
begin
  ...
  from (
    ...
    from unnest(array['skill.definition','skill.render_definition']) rel
  ...
  if bad is not null then
    raise exception using
      message = 'DD-186 (skill): a signed-out visitor can still read identity/bookkeeping/secret columns: ' || bad,
