-- dd230_tool_publishes_exactly_what_a_signed_out_reader_renders
...
--   tool.surface_defaults → 4 column(s). the chrome-extension tool-drift guards, which read with the PUBLISHABLE key: matrx-extend scripts/check-tool-db-drift.ts (fetchSurfaceDefaults) and matrx-local scripts/check_tool_db_drift.py. Measured: 18 anon 200s in 24 h, every one the same four columns for surface_name in (chrome-extension/assistant, chrome-extension/pilot).
--
-- ═══ CLOSED — no signed-out reader exists, so the bound is the EMPTY list ═════════════════════
--   tool.bundle — one curl probe and three authenticated reads. The extension's drift guard reads tool.binding, tool.definition and tool.surface_defaults — never this.
--   tool.executor — two curl probes from census lanes. Its exposure row says "public tool catalogue"; the public tool catalogue is tool.definition, served by app/api/tools via utils/supabase/server-tools-service.ts, which names its columns and never touches executor.
--   tool.mcp_config — one curl probe.
--   tool.mcp_server — one curl probe and one select=* from manage.aimatrx.com — an ADMIN portal racing its own auth. Thirty-three columns of MCP server configuration is not a public catalogue.
...
begin
  for r in
    select * from (values
      ('tool.surface_defaults', array['surface_name', 'always_include_tools', 'always_include_bundles', 'never_include_tools'])
    ) as t(rel, keep)
  loop
