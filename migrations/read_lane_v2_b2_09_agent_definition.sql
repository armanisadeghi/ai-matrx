...
-- read_lane_v2_b2_09_agent_definition — enroll agent.definition in read-lane v2 and regenerate its policies (one table, one transaction).
...
insert into iam.read_lane_v2_rollout (token, batch) select token, 'b2' from platform.entity_types where schema_name = 'agent' and table_name = 'definition' and is_active on conflict do nothing;
