-- ctx_resolve_full_context_restore_cells_after_schema_move.sql
--
-- RESTORE the features the schema-move rebase silently dropped from
-- resolve_full_context (verified live 2026-07-08: the deployed function body had
-- been rebased onto the new schemas — context.*, chat.conversation,
-- workbench.notes, platform.associations, iam.organization_member — but the
-- rebase started from the OLD pre-cells definition, losing three shipped
-- features):
--
--   1. `cell_values` — the collision-proof {context_item_id: cell} map that
--      scope→agent BINDING resolution requires (aidream
--      scope_binding_resolution._lookup resolves a UUID binding ONLY against
--      this map — with it missing, every bound variable/slot resolved as
--      MISSING). This is the D10 "bound picklist fence" root cause.
--   2. SYSTEM cells (loop 4b) — is_system scope-type cells delivered globally
--      (also feeds the Python ambient layer: current_date etc. override
--      entries whose source == 'system').
--   3. SYSTEM dataset pointers (loop 4c) — feed_type='dataset' descriptors so
--      agents learn rag-searchable resources exist.
--
-- This file is the June 2026 `ctx_resolve_full_context_dataset_pointers.sql`
-- definition rebased onto the post-move schemas. The value CASE reads the
-- FIRST POPULATED value_* column (value_text before value_json …) and never
-- branches on the value_type column — legacy rows carry a stale value_type
-- while a re-saved ```matrx fence lands in value_text.
--
...
BEGIN
    ...
    -- 2) Entity-tagged scopes (associations spine: content=source -> scope=target)
