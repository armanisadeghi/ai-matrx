-- migrations/mnd_member_list_key_pushdown_2026_09_25.sql
-- based-on: mandate._member_list_rows(text, uuid, uuid, text) 29172ef3d11640a4182a084225f22ded4dce9879a4ff734083d51ce0217c95cc
-- based-on: public.mnd_member_list(text, text, text, uuid, uuid, text, jsonb, text, text, integer, integer) 1217ad25f1d6acf54dba1e60fa5de4cc1f68f9ee6a7a364f250082484ba181bb
--
-- The feature Intelligence page (/intelligence/<feature>) asks public.mnd_member_list for an
-- exact set of mandate keys (p_filters.mandateKey select). The row builder built the WHOLE
-- corpus (ladder, holder, health for every mandate the viewer can see) and only then kept
-- the requested keys: ~2 s per call, the whole wait on /intelligence/marketing.
--
-- ADDITIVE:
--   * mandate._member_list_rows(text,uuid,uuid,text,text[]) — the builder with an optional key
--     list applied to the corpus;
--   * mandate._member_list_rows(text,uuid,uuid,text) — unchanged signature, now a thin call
--     with no keys (every existing caller keeps its answer);
--   * public.mnd_member_list — same signature; page and counts pass the exact-key select filter
--     down (the post-filter still runs, so the answer is identical). Facets keep every key.
...
CREATE OR REPLACE FUNCTION mandate._member_list_rows(p_q text, p_res_user uuid, p_res_org uuid, p_level text)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT * FROM mandate._member_list_rows(p_q, p_res_user, p_res_org, p_level, NULL::text[]);
$function$;
...
-- Same doors as the 4-argument builder (schema default privileges already withhold PUBLIC).
GRANT EXECUTE ON FUNCTION mandate._member_list_rows(text, uuid, uuid, text, text[])
  TO authenticated, service_role, dashboard_user, svc_seo;
