...
INSERT INTO skill.definition (
  ...
  platform_targets, version, category_id, is_active, is_system, is_public,
...
WHERE NOT EXISTS (
  ...
  WHERE skill_id = 'structured-info-blocks' AND user_id IS NULL
);

-- Refresh the body/metadata on re-apply (global row: user/project/task NULL).
UPDATE skill.definition SET
  label = 'Structured Information Blocks',
  description = 'How and when to emit ```structured_info render blocks: a collapsible, copy-able card for well-organized reference/summary content written as ordinary markdown — headings, bullets, short tables — with syntax rules that keep it clean and readable.',
  ...
  is_public = true,
  visibility = 'public',
  body = $BODY$# Structured Information Blocks
...
```
$BODY$
WHERE skill_id = 'structured-info-blocks'
  AND user_id IS NULL
