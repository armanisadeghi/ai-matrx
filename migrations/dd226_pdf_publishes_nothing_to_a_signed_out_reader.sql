-- dd226_pdf_publishes_nothing_to_a_signed_out_reader
...
--   pdf.pdf_redaction_audits — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (EMPTY-TABLE (anon cols: 14)); the grant itself is the finding.
--   pdf.pdf_redaction_key_escrow — Latent (the grant is open; no row is on the public side of the gate today). RED not expressible by a visibility flip (NO-VISIBILITY-COLUMN (anon cols: 7)); the grant itself is the finding.
--
