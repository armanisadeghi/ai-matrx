-- dd200_the_certifier_measures_each_variant_against_its_own_contract
-- (DD-200. db-rules §0/§6d/§9. ONE function replaced; no DDL on any live table, no policy change.)
--
-- ═══ THE DEFECT, MEASURED LIVE 2026-09-13 ══════════════════════════════════════════════════════
-- `iam.verify_canonical` measures EVERY registered token against the per-variant base contract
-- and the generated-policy family — including tokens declared `audit_class='machinery'`, which
-- `iam.apply_rls` REFUSES by construction:
--
--   'apply_rls: token % (%.%) is access machinery; generic RLS is forbidden because machinery
--    owns inputs consumed by the access resolver'
--
-- So for a machinery token the gate reports the distance from a regime that, by the 2026-08-24
-- ruling, never runs on it. Measured over every active token on this database today, every FAIL
-- on every machinery token is one of these: `policies_canonical`, `policy_uses_has_access`,
-- `policy_owner_shortcircuit`, `policy_system_public_read`, `pub_read_anon`, `personal_row_wall`,
-- `data_class_derivations` (all of them about policies the generator would have written and did
-- not), `base_id_uuid`, `base_organization_id`, `base_org_not_null`, `base_org_fk`,
-- `base_created_by`, `base_created_by_fk`, `base_updated_by`, `base_updated_by_fk`,
-- `base_created_at`, `base_updated_at`, `base_version`, `base_metadata`, `trg_stamp_actor`,
-- `trg_touch_row` (all of them base columns and triggers §6d-3 derives FROM the generated lane).
-- Not one of them is actionable, and 47 of the 50 active machinery tokens carry a set of them.
--
-- That is not merely noise. B-83 re-registered `user_secret_grant` as machinery for the right
-- reason — four live vault policies read it BY NAME, which is `iam.apply_rls`'s own definition of
-- machinery — and the certifier answered with eleven FAILs, so the one thing certification is for
-- (a real FAIL standing out) could not happen. §6d-3 already ruled on this exact shape: "when the
-- gate and a correctly-classified table disagree, suspect the gate."
--
-- ═══ WHAT THIS FILE CHANGES ════════════════════════════════════════════════════════════════════
-- `iam.verify_canonical` routes on `audit_class`. A machinery token is measured against the
-- MACHINERY contract and the base contract is skipped with a NAMED `SKIP` row:
--
--   machinery_has_reason          the written reason on the row. `entity_types_machinery_reason`
--                                 is a NOT-NULL CHECK and nothing more, so a whitespace string
--                                 declares machinery with nothing written on it.
--   machinery_no_generated_policy no class-regime lane (std_select/std_insert/std_update/
--                                 std_delete/pub_read) exists — the generator refuses this token,
--                                 so one here means a generation that should never have happened.
--                                 `svc_all` and `platform_admin_all` are platform-wide lanes their
--                                 own migrations put on machinery too; they are NAMED in the
--                                 detail rather than counted as generation.
--   machinery_no_client_grant     `anon` holds no privilege, and no permissive client read policy
--                                 carries an unconditional predicate. Deliberately NOT
--                                 "authenticated holds nothing": the ratified machinery contract
--                                 is that these tables keep their BESPOKE policies and several
--                                 must be client-readable through one — `iam.memberships` is read
--                                 by the member it names. The authenticated privileges are named
--                                 in the detail either way.
--   machinery_rls_on              row security is on, so the bespoke policies are reachable.
--
-- 🚨 `machinery` IS AN `audit_class`, NOT AN `rls_variant`. `entity_types_rls_variant_valid`
-- admits only entity/component/system/restricted/ledger/personal — there is no machinery variant
-- to route on. The registered `rls_variant` of a machinery token is still reported, inside the
-- SKIP row, as what the table would be generated as if it ever stopped being machinery.
--
-- ═══ WHAT THIS FILE DOES NOT CHANGE ════════════════════════════════════════════════════════════
-- Nothing about access. No policy, no grant, no table, no registry row. `audit.refresh_static`
-- already skips machinery (`et.audit_class <> 'machinery'`) and records only FAIL/WARN, so
-- `audit.canonical_findings`, `audit.summary` and the admin canonicalisation screen never saw
-- these FAILs and see no change from this file; the new `SKIP` row is filtered out on the same
-- line that already filters the many `SKIP`s this function has always emitted. The lanes that
-- called `iam.verify_canonical` DIRECTLY are the ones that were reading the noise.
--
-- ═══ THE THREE REAL FINDINGS THE FIX IMMEDIATELY SURFACES ════════════════════════════════════════
-- With the inapplicable contract gone, three machinery tokens FAIL and every one of them is real:
--   platform.activity_log (`activity`)   carries a `std_select` policy — a class-regime lane on a
--                                        founding machinery table, present in no migration in
--                                        either repository. Left in place here and reported: it
--                                        is a live read door and removing it is an access change
--                                        with its own before/after gate, not a side effect of a
--                                        certifier fix.
--   platform.edge_payload_kind           `edge_payload_kind_read` is `USING (true)` FOR SELECT to
--                                        `anon, authenticated`.
--   platform.repo (`platform_repo`)      `repo_read` is `USING (true)` FOR SELECT to
--                                        `authenticated`.
-- Same disposition, same reason: reported, not silently widened or narrowed by this file.
--
-- ═══ THE FORCING TEST ══════════════════════════════════════════════════════════════════════════
-- The block at the end builds a scratch machinery token on a scratch table and proves each check
-- RED then GREEN against this live database, then removes every trace of itself. It is
-- re-runnable and it raises — aborting this whole migration — if any limb does not move:
--   blank reason FAILs machinery_has_reason, a written reason PASSes;
--   an `anon` grant FAILs machinery_no_client_grant, revoking it PASSes;
--   a `std_select` policy FAILs machinery_no_generated_policy, dropping it PASSes;
--   RLS disabled FAILs machinery_rls_on, enabling it PASSes;
--   while machinery, NO base-contract check row is emitted and the named SKIP row IS;
--   flipped to audit_class='entity' the base contract comes straight back.
...
BEGIN
  ...
  --   machinery_no_client_grant   — no signed-out reach, and no ungated client door.
  ...
  IF v_audit_class = 'machinery' THEN
    ...
    BEGIN
      ...
      IF v_bad IS NULL THEN
        status := 'PASS';
        detail := CASE WHEN v_auth_privs IS NULL THEN 'no client grant at all'
                       ELSE format('authenticated holds %s; every client lane is gated by a '
                                || 'bespoke policy, which is the ratified machinery contract',
                                   v_auth_privs) END;
      ELSE
        status := 'FAIL'; detail := v_bad || ' (DD-200).';
      END IF;
    END; RETURN NEXT;
...
do $dd200_forcing$
declare
  c_tok  constant text := 'dd200_forcing_probe';
  v_st   text;
  v_n    integer;

  -- every base-contract / generated-policy check name this function emits for an entity token.
  -- NONE of them may appear for a machinery token; ALL of the base ones must come back the moment
  -- the token stops being machinery.
  c_base constant text[] := array[
    'base_id_uuid','base_organization_id','base_org_not_null','base_org_fk','base_created_by',
    'base_created_by_fk','base_updated_by','base_updated_by_fk','base_created_at','base_updated_at',
    'base_version','base_metadata','trg_stamp_actor','trg_touch_row','trg_version_capture',
    'policies_canonical','policy_owner_shortcircuit','policy_uses_has_access'];
begin
  -- The scratch objects below are private to this transaction and nothing reads them, but
  -- CREATE POLICY registers a shared dependency on the `anon`/`authenticated` roles, which any
  -- concurrent GRANT anywhere on this database contends for — measured: the file's 4s budget
  -- (right for a LIVE table) cancelled it twice in a row. A longer bound here waits on a scratch
  -- object only; the 4s bound is restored before this block ends.
  ...
  drop schema if exists dd200_forcing cascade;

  create schema dd200_forcing;
  create table dd200_forcing.probe (
    id              uuid primary key default gen_random_uuid(),
    -- deliberately NO foreign key: an FK to iam.organizations makes this transaction hold a lock
    -- on a hot shared table for the length of the block, and the schema-cache introspection that
    -- our own DDL wakes then deadlocks against the final DROP (measured twice, 40P01). Nothing
    -- the machinery contract checks reads a column, so the FK buys the probe nothing.
    organization_id uuid not null,
    metadata        jsonb not null default '{}'::jsonb,
    created_at      timestamptz not null default now()
  );
  alter table dd200_forcing.probe enable row level security;

  -- ── RED 1: machinery declared with a BLANK reason ────────────────────────────────────────────
  ...
  values
    (c_tok, 'dd200_forcing', 'probe', 'DD-200 forcing probe', 'entity', 'machinery', '   ',
     true, false, false, false, false);

  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_has_reason';
  if v_st is distinct from 'FAIL' then
    raise exception 'DD-200 forcing test RED 1 did not fire: a machinery token with a blank audit_class_reason reported machinery_has_reason=%, expected FAIL', coalesce(v_st,'<no row>');
  end if;

  -- ── GREEN 1: the reason written ──────────────────────────────────────────────────────────────
  update platform.entity_types
     set audit_class_reason = 'DD-200 forcing probe: a scratch machinery token that exists for the length of this migration only.'
   where token = c_tok;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_has_reason';
  if v_st is distinct from 'PASS' then
    raise exception 'DD-200 forcing test GREEN 1 failed: with a written reason machinery_has_reason reported %, expected PASS', coalesce(v_st,'<no row>');
  end if;

  -- ── the base contract is NOT measured, and the skip is NAMED ─────────────────────────────────
  select count(*) into v_n from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = any(c_base);
  if v_n <> 0 then
    raise exception 'DD-200 forcing test failed: % base-contract check rows were still emitted for a machinery token', v_n;
  end if;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'base_contract_not_applicable';
  if v_st is distinct from 'SKIP' then
    raise exception 'DD-200 forcing test failed: the base contract was skipped SILENTLY — base_contract_not_applicable reported %, expected a SKIP row', coalesce(v_st,'<no row>');
  end if;

  -- ── RED 2: a signed-out reader ───────────────────────────────────────────────────────────────
  grant select on dd200_forcing.probe to anon;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_no_client_grant';
  if v_st is distinct from 'FAIL' then
    raise exception 'DD-200 forcing test RED 2 did not fire: with SELECT granted to anon, machinery_no_client_grant reported %, expected FAIL', coalesce(v_st,'<no row>');
  end if;
  revoke select on dd200_forcing.probe from anon;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_no_client_grant';
  if v_st is distinct from 'PASS' then
    raise exception 'DD-200 forcing test GREEN 2 failed: with the anon grant revoked, machinery_no_client_grant reported %, expected PASS', coalesce(v_st,'<no row>');
  end if;

  -- ── RED 2b: an ungated client door ───────────────────────────────────────────────────────────
  grant select on dd200_forcing.probe to authenticated;
  create policy probe_open on dd200_forcing.probe for select to authenticated using (true);
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_no_client_grant';
  if v_st is distinct from 'FAIL' then
    raise exception 'DD-200 forcing test RED 2b did not fire: a permissive USING (true) client read policy reported machinery_no_client_grant=%, expected FAIL', coalesce(v_st,'<no row>');
  end if;
  drop policy probe_open on dd200_forcing.probe;

  -- ── RED 3: the generic class regime on machinery ─────────────────────────────────────────────
  create policy std_select on dd200_forcing.probe for select to authenticated
    using (organization_id is not null);
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_no_generated_policy';
  if v_st is distinct from 'FAIL' then
    raise exception 'DD-200 forcing test RED 3 did not fire: a std_select policy on a machinery token reported machinery_no_generated_policy=%, expected FAIL', coalesce(v_st,'<no row>');
  end if;
  drop policy std_select on dd200_forcing.probe;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_no_generated_policy';
  if v_st is distinct from 'PASS' then
    raise exception 'DD-200 forcing test GREEN 3 failed: with the class lane dropped, machinery_no_generated_policy reported %, expected PASS', coalesce(v_st,'<no row>');
  end if;

  -- ── RED 4: the bespoke policies made unreachable ─────────────────────────────────────────────
  alter table dd200_forcing.probe disable row level security;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_rls_on';
  if v_st is distinct from 'FAIL' then
    raise exception 'DD-200 forcing test RED 4 did not fire: with row security disabled, machinery_rls_on reported %, expected FAIL', coalesce(v_st,'<no row>');
  end if;
  alter table dd200_forcing.probe enable row level security;
  select vc.status into v_st from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = 'machinery_rls_on';
  if v_st is distinct from 'PASS' then
    raise exception 'DD-200 forcing test GREEN 4 failed: with row security enabled, machinery_rls_on reported %, expected PASS', coalesce(v_st,'<no row>');
  end if;

  -- ── the route is CONDITIONAL: stop being machinery and the base contract comes back ──────────
  update platform.entity_types
     set audit_class = 'entity', audit_class_reason = null
   where token = c_tok;
  select count(*) into v_n from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name = any(c_base);
  if v_n = 0 then
    raise exception 'DD-200 forcing test failed: with audit_class=entity the base contract did NOT come back — the machinery route is not conditional';
  end if;
  select count(*) into v_n from iam.verify_canonical('dd200_forcing','probe',c_tok) vc
   where vc.check_name like 'machinery\_%' or vc.check_name = 'base_contract_not_applicable';
  if v_n <> 0 then
    raise exception 'DD-200 forcing test failed: an entity token emitted % machinery check row(s)', v_n;
  end if;

  -- ── every trace of the probe removed ─────────────────────────────────────────────────────────
  delete from platform.entity_types where token = c_tok;
  drop schema dd200_forcing cascade;
  if exists (select 1 from platform.entity_types where token = c_tok)
     or to_regclass('dd200_forcing.probe') is not null then
    raise exception 'DD-200 forcing test failed to clean itself up';
  end if;

  perform set_config('lock_timeout', '4s', true);
  raise notice 'DD-200 forcing test: 4 RED limbs fired, 4 GREEN limbs passed, base contract absent while machinery and back when not, probe removed.';
end
$dd200_forcing$;

-- ═══ THE LIVE STATE THIS FILE LEAVES ═══════════════════════════════════════════════════════════
-- Not an assertion about what is right — an assertion that the three findings named in the header
-- are the ONLY machinery FAILs after this change, so a later lane that sees a fourth knows it is
-- new. It raises if a machinery token FAILs for any reason other than the three recorded here.
do $dd200_state$
declare r record; v_bad text := null;
begin
  for r in
    select et.token, vc.check_name, vc.status
  ...
  loop
    if (r.token, r.check_name) not in
       (('activity','machinery_no_generated_policy'),
        ('edge_payload_kind','machinery_no_client_grant'),
        ('platform_repo','machinery_no_client_grant')) then
      v_bad := coalesce(v_bad || '; ', '') || r.token || '/' || r.check_name;
    end if;
  end loop;
  if v_bad is not null then
    raise exception 'DD-200: a machinery FAIL this file did not record is live — %. Read it before assuming the certifier is wrong again.', v_bad;
  end if;
  raise notice 'DD-200: machinery FAILs after this change are exactly the three recorded in the header.';
end
$dd200_state$;
