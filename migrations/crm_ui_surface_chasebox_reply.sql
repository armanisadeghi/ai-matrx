-- crm_ui_surface_chasebox_reply.sql
-- Chasebox manifest re-sync (outreach-system WP1, round 4).
--
-- Adds the `draft_reply` value — the evidence behind an AI-written REPLY (what
-- it sets out to do, the traced claims it stands on, what it is answering) —
-- and re-states the surface intro so the draft_reviewer role is told the second
-- half of its job. Regenerated from features/surfaces/manifests/
-- crm-chasebox.manifest.ts with scripts/emit-surface-sync-sql.ts; idempotent.
...
('matrx-user/crm-chasebox', 'draft_reviewer', 'Draft reviewer', 'Reads a held draft beside its personalization evidence and says whether the claim is actually supported by the quoted fact and source page, then suggests a better line. Never approves, sends, or edits — it hands wording to the human.', 'single', 'fa6a4506-a658-41c0-9094-8a370e490849', 1, true, 'user-choice', 100)
...
UPDATE ui.ui_surface SET label = 'Chasebox', value_groups = '[{"key":"queue","label":"Queue","sortOrder":100,"description":"Which queue is open and what it currently holds."},{"key":"draft","label":"Draft under review","sortOrder":200,"description":"The exact message a human is being asked to approve, with the evidence behind its AI-written lines."}]'::jsonb, readiness = 'verified', readiness_note = 'Read vocabulary verified against crm_chasebox_items + the draft review dialog; emitter and assist strip both mounted on ChaseboxPage. draft_reviewer carries its default agent (outreach_draft_reviewer, WP5 round 4) and was proven against a real draft + evidence pair. Note: draft_body and visible_items are autoContext:false, so they arrive DEFERRED — verify them with retrieval allowed or an emitter that works looks broken.', url_pattern = '/crm/chasebox', intro = '<surface_intro>
