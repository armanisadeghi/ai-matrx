-- ctx_resolve_full_context_cell_values_by_id.sql
--
-- Make scope→agent binding resolution collision-proof by the database UUID. The existing
-- `variables` map is keyed by context-item `key`, which collapses across scope types: if a
-- Client and a Department both define key "name", the flattened map keeps only one — so a
-- binding resolved by key could get the WRONG scope's value. Binding resolution must instead
-- use the exact `context_item_id` (a UUID that belongs to exactly one scope type).
--
-- This adds a parallel `cell_values` object keyed by context_item_id::text, preserving every
-- active-scope cell uniquely (value/type/description + the source scope_id/scope_type_id for
-- write-back + observability). The `variables` map and everything else are unchanged, so the
-- system-prompt awareness block and brokers keep working byte-for-byte. With this, item keys
-- only need to be unique per scope type (human readability); the binding itself can never
-- collide — single user, org, or global.
--
...
BEGIN
    ...
    -- 5) scope CELLS -> variables (keyed by key, awareness/brokers) AND cell_values (keyed
    --    by context_item_id UUID, the collision-proof source for binding resolution).
    IF v_entity_scopes IS NOT NULL THEN
        FOR rec IN (
            SELECT ci.id AS context_item_id,
                   ci.key,
                   ci.description,
                   ci.value_type::text AS value_type,
                   s.id AS scope_id,
                   s.name AS scope_name,
                   s.scope_type_id AS scope_type_id,
                   CASE
