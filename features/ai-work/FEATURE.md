...
**The star reads where the star is written.** A favorite is PER-USER state, so it cannot live on the shared `chat.conversation` row: two people looking at the same org conversation do not share one star. The canonical store is `platform.user_entity_state` (`entity_type='conversation'`), written through the `ues_set` RPC chokepoint (`favoritesService`) by every star in the app, and read back through `ues_get_bulk`. `chat.conversation.is_favorite` is FROZEN HISTORY — nothing has written it since the cutover, and its column comment says so. Every conversation read now sources the flag canonically: `cvx_list_scoped` (and therefore the counts + facets that delegate to it) LEFT JOINs `user_entity_state` for `auth.uid()`, `chat.conversation_summary` joins it for the row owner, and `readProviderConversation` resolves it with `ues_get_bulk`.

The read has to happen IN the query, not as a client-side overlay: favorites-first ordering, the `favorite` sort, and the `favorite` filter/facet are all computed server-side, and an overlay cannot reach any of them. This is the defect that made the surface lie — un-starring wrote `user_entity_state`, the reload read the dead column, and the star came straight back (fixed `2026-08-17`).
...
- `2026-08-17` — Claude: **the star on `/work/conversations` un-set itself on every refresh — the surface read one store and wrote another.** Un-starring wrote `platform.user_entity_state` through the canonical `ues_set` chokepoint; the reload read `chat.conversation.is_favorite`, a column nothing has written since that cutover, so the star came straight back and the user's change was silently discarded. Fixed at the read, in the DB, for every consumer at once: `cvx_list_scoped` now LEFT JOINs `platform.user_entity_state` for `auth.uid()` and uses that value for the projected flag, the favorites-first ordering, the `favorite` sort and the `favorite` filter — which also fixes `cvx_list_scope_counts` and `cvx_list_facets`, since both delegate to it. A client-side overlay was rejected: the ordering, filter and facet are computed server-side and an overlay cannot reach them. Two more stale readers of the same dead column found and fixed in the same pass — `chat.conversation_summary` (owner-scoped view behind aidream's MCP `conversations` tool) now joins `user_entity_state` for the row owner, and `readProviderConversation` resolves the provenance panel's **Favorite** fact with `ues_get_bulk` instead of selecting the column (the transcript's own star already read canonically, and now seeds from that server value instead of flashing in a round-trip later). Backfilled every star the column still held into `user_entity_state` (`ON CONFLICT DO NOTHING`, so a deliberate un-star is never resurrected), and stamped both the dead column and the canonical table with comments saying which is which. Verified on the live DB by impersonating the affected user: `cvx_list_scoped` now returns exactly the canonical favorite set, and the conversation Arman had un-starred reads `false`.
