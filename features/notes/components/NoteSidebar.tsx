...
export function NoteSidebar({ instanceId }: NoteSidebarProps) {
  ...
  const handleNewNote = useCallback(
    async (folder: string, capturedOrganizationId?: string) => {
      return draftControl.run(async () => {
        ...
        // A folder that already holds notes names its organization; a fresh
        // folder takes the ACTIVE one — waiting for boot to name it rather
        // than refusing the click while it is still resolving.
        const organizationId = capturedOrganizationId ?? target?.organization_id
          ? requireOrganizationContext(capturedOrganizationId ?? target?.organization_id)
          : await resolveOrganization();
        const reference = target ? noteFolderReference(target) : null;
        if (target && target.folder_name && !reference) throw new Error("This folder cannot be used until its identity is available. Choose a folder and try again.");
        const existing = allNotes.find((n) => n._isAutogenerated && !n.content?.trim() && n.organization_id === organizationId && noteFolderIdentityKey(n) === folder);
      ...
      const items: ContextMenuExtraItem[] = [
        {
          ...
          onSelect: () => { void handleNewNote(folder).catch(() => undefined); },
        },
      ];
    ...
    async (folderName: string) => {
      ...
      if (createFolderIntent?.kind === "move-note") {
        ...
        return draftControl.run(async () => {
          ...
          if (!userId) throw new Error("Sign in before starting a new note.");
          await dispatch(initializeAutogeneratedNote({
            id: crypto.randomUUID(), userId, organizationId, instanceId,
            destination: { kind: "create", name: folderName, organizationId },
          })).unwrap();
        });
      }
    },
      ...
      >
        ...
        {listStatus === "error" && (
          <div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center" role="alert">
            <p className="text-xs text-destructive">{listError ?? "Your notes could not be loaded."}</p>
            <button
              type="button"
              onClick={() => {
                dispatch(fetchNotesList());
              ...
              className="inline-flex items-center gap-1 rounded-md border border-border px-2.5 py-1 text-xs font-medium text-foreground transition-colors hover:bg-muted cursor-pointer"
            >
