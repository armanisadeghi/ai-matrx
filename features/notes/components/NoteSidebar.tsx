...
export function NoteSidebar({ instanceId }: NoteSidebarProps) {
  ...
  const handleFolderDrop = useCallback(
    (e: React.DragEvent, targetFolder: string) => {
      ...
      if (note && target?.folder_id && noteFolderIdentityKey(note) !== targetFolder) {
        dispatch(moveNoteToFolder({ noteId, folder: target.folder_name ?? "Draft", folderId: target.folder_id, organizationId: requireOrganizationContext(target.organization_id) }));
      }
      setDraggedNoteId(null);
      setDropTargetFolder(null);
    },
  ...
  const handleNewNote = useCallback(
    (folder: string) => {
      ...
      const organizationId = target?.organization_id ?? activeOrgId;
  ...
  const handleCreateFolder = useCallback(
    async (folderName: string) => {
      ...
      await createFolder(folderName, requireOrganizationContext(activeOrgId));
    ...
    [createFolderIntent, dispatch, handleNewNote, activeOrgId],
  );
