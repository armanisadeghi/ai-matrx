...
import {
  ...
  emptyTrashThunk,
  draftInitializationErrorMessage,
...
export function NoteSidebar({ instanceId }: NoteSidebarProps) {
  ...
  const handleNewNote = useCallback(
    async (folder: string, capturedOrganizationId?: string) => {
      ...
      const organizationId = requireOrganizationContext(capturedOrganizationId ?? target?.organization_id ?? activeOrgId);
      const reference = target ? noteFolderReference(target) : null;
      if (target && target.folder_name && !reference) {
        throw new Error("This folder cannot be used until its identity is available. Choose a folder and try again.");
      }
      const existing = allNotes.find(
        (n) => n._isAutogenerated && !n.content?.trim() && n.organization_id === organizationId && noteFolderIdentityKey(n) === folder,
      );
      if (existing) {
