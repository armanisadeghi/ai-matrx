...
/** Canonical local editor vocabulary; legacy metadata aliases never enter state. */
export type NoteEditorMode = import("../components/NoteEditorCore").EditorMode;
...
/**
 * The newest row observation received while a local edit is in progress.
 * This deliberately does not replace the acknowledged edit base: a remote
 * event is evidence to review, never permission to rebase local input.
 */
export interface NoteRemoteObservation {
  version: number | null;
...
/**
 * A serializable, actor-bound review survives window/editor remounts. The
 * editable proposal and hunk decisions live here; the write permit remains in
 * the existing queue and is never persisted into Redux.
 */
export interface RetainedNoteConflictReview {
  reviewKey: string;
  noteId: string;
  actorId: string;
  organizationId: string;
  decisionId: string;
  reviewId: string;
  localSnapshot: NoteConflictPhysicalSnapshot;
  reviewedRemote: Note;
  openingLiveContent: string;
  refreshSourceReviewKey: string | null;
  proposal: string;
  session: ReviewSession;
  readOnly: boolean;
  dismissed: boolean;
  command: { status: "idle" | "pending" | "refused"; requestId: string | null; error: string | null };
}
...
export interface NoteRecord extends Note {
  /**
   * Editor presentation is local state. Legacy persisted
   * `metadata.lastEditorMode` is read once while constructing a record so old
   * preferences still open as expected, but is never used as a write target.
   */
  _editorMode: NoteEditorMode | null;
  /** Provenance keeps a viewport fallback from becoming a stored choice. */
  _editorModeSource: "uninitialized" | "persisted" | "local";
  ...
  /**
   * The complete row acknowledged by an actual full read or physical write.
   * This is deliberately separate from `_fieldHistory`: field history is an
   * undo aid and can no longer describe a complete row after mid-save edits.
   * Realtime evidence and dirty display values never replace this base.
   */
  _acknowledgedPhysicalSnapshot: Note | null;
   ...
   */
  _lastWrittenValues?: Partial<
    Record<NoteUndoableField, Note[NoteUndoableField]>
  >;

  /** Highest remote evidence retained separately from the acknowledged base. */
  _remoteObservation: NoteRemoteObservation | null;
  /** Present only for an actual compare-and-swap rejection. */
  _conflictDecision: NoteConflictDecision | null;
...
export interface NotesSliceState {
  ...
  /** Per-note request state keeps slow or failed deep links honest. */
  ...
  currentConflictReviewKeys: Record<string, string>;
  retainedConflictReviews: Record<string, RetainedNoteConflictReview>;
...
/**
 * Make an immutable-in-practice serializable row snapshot for prepared edits.
 * The JSON columns and tag array must not share references with a mutable
 * Redux record.
 */
export function cloneAcknowledgedNote(note: Note): Note {
  return {
    ...note,
    tags: note.tags === null ? null : structuredClone(note.tags),
    metadata:
      note.metadata === null || note.metadata === undefined
        ? note.metadata
        : structuredClone(note.metadata),
  };
}
...
function getPersistedEditorMode(metadata: unknown): NoteEditorMode | null {
...
export function editorModeFromPhysicalMetadata(metadata: unknown): NoteEditorMode | null {
  return getPersistedEditorMode(metadata);
}

export function createBlankNoteRecord(note: Note): NoteRecord {
  return {
    ...note,
    _editorMode: getPersistedEditorMode(note.metadata),
    _editorModeSource: "persisted",
    _fetchStatus: null,
    _dirty: false,
    _dirtyFields: new Set(),
    _fieldHistory: {},
    _acknowledgedPhysicalSnapshot: cloneAcknowledgedNote(note),
    _undoPast: [],
    _undoFuture: [],
    _loading: false,
    _saving: false,
...
export function createBlankNoteRecordFromPartial(
  ...
  return {
    ...
    _dirtyFields: new Set(),
    _fieldHistory: {},
    _acknowledgedPhysicalSnapshot: acknowledgedSnapshotFromFullRead(partial, status),
    _undoPast: [],
    _undoFuture: [],
    _loading: false,
    _saving: false,
...
export function createAutogeneratedNoteRecord(
  ...
  return {
    ...
    _dirtyFields: new Set(),
    _fieldHistory: {},
    _acknowledgedPhysicalSnapshot: null,
    _undoPast: [],
    _undoFuture: [],
    _loading: false,
    _saving: false,
