...
- `2026-06-19` — Version history panel on `/notes` now uses `MatrxDynamicPanelHost` (resizable, repositionable) on desktop and a bottom Drawer on mobile instead of a fixed-width inline sidebar that clipped timeline badges/actions. `NoteVersionHistory` API is now `open` / `onOpenChange`; `DiffHistory` accepts `showHeader={false}` when embedded in panel chrome.
