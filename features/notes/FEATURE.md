...
- `2026-06-10` — Added `NotePickerPopover` (lazy `NotesAPI.listItems` on open, folder-grouped search, Drawer on mobile) and `fetchNoteListItems` / `NotesAPI.listItems` for lightweight picker queries without note content. Consumed by transcription cleanup context blocks.
