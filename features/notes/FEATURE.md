...
- `2026-06-11` — Notes tab strip + folder dropdown fixes: `NoteTabBar` now pins the new-note `+` button as a sticky first "little tab" (`sticky left-0`) and pins the **active** tab beside it (`sticky left-8`), so the current tab's label is never scrolled out of view on a full row and the `+` stays reachable while the rest of the strip scrolls. `NoteMetadataBar`'s folder dropdown is now portaled to `document.body` with fixed positioning (anchored to the trigger rect, closes on outside-click/scroll/resize/Escape) — it was being clipped by the metadata row's `overflow-hidden` + upward (`bottom-full`) open, which read as "contents don't render."
