...
- `2026-07-03` — **`createFolder` is now truly atomic — it had no unique constraint behind its get-or-create and silently minted duplicate folders under concurrent Quick-Saves.** Migration `note_folders_created_by_name_unique.sql` (applied + ledger-recorded + live-verified): deduped the one existing `(created_by, name)` dup (40→39; nesting unused, notes reference folders by `folder_name` string not id, so nothing orphaned) and added a FULL unique index `note_folders_created_by_name_unique (created_by, name)`. `createFolder` now `upsert(..., { onConflict: "created_by,name", ignoreDuplicates: true })` → `ON CONFLICT DO NOTHING` (never a 23505/409); on conflict it resolves the existing row and **revives** it if soft-deleted (delete-then-recreate-by-name resurrects the one row, matching the name-keyed model). Part of the app-wide non-atomic get-or-create sweep (see `features/transcript-studio/FEATURE.md` 2026-07-03).
