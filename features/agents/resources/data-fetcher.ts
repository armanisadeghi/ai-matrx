 ...
 *
 * Canonical home: features/agents/resources/data-fetcher.ts
 * (Relocated from features/prompts/utils/resource-data-fetcher.ts)
 */
...
function isTableField(value: unknown): value is TableField {
  return (
    isRecord(value) &&
...
function readTablePayload(value: unknown): {
  fields: TableField[];
  rows: TableRow[];
} {
  if (!isRecord(value)) return { fields: [], rows: [] };
  return {
    fields: Array.isArray(value.fields)
      ? value.fields.filter(isTableField)
      : [],
    rows: Array.isArray(value.rows) ? value.rows.filter(isTableRow) : [],
  };
}
  ...
  try {
    ...
    switch (referenceType) {
      ...
      case "table_schema": {
        ...
        const { data: rowsDataRaw, error } = await supabase.rpc(
          "list_table_rows",
          {
            ref: {
              type: "full_table",
        ...
        const rowsData = readTablePayload(rowsDataRaw);
        return {
      ...
      case "table_row": {
        ...
        const row = isTableRow(rowRaw) ? rowRaw : undefined;
        return {
          row,
      ...
      case "table_column": {
        ...
        if (columnResult.error || rowsResult.error) {
          console.error(
            "Error fetching column data:",
            columnResult.error || rowsResult.error,
          );
          return null;
        }

        const rowsPayload = readTablePayload(rowsResult.data);
        return {
      ...
      case "table_cell": {
        ...
        const { data: cellRaw, error } = await supabase.rpc("get_table_cell", {
          ref: tableRef,
        });
        ...
        return {
          value: isRecord(cellRaw) ? cellRaw.value : undefined,
...
async function enrichTableResource(resource: Resource): Promise<Resource> {
  ...
  switch (fetchedData.referenceType) {
    ...
    case "table_row":
      ...
      enrichedData.rows = fetchedData.row ? [fetchedData.row] : [];
      break;
    ...
    case "table_cell":
      ...
      enrichedData.cell_value = fetchedData.value;
      break;
  }
