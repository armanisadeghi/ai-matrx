 ...
 */
export const RESOURCE_FORMAT_CONFIG = {
  note: {
    includeInContent: true,
    requiresDataFetch: false,
    instructions:
      "This is a user note that can be referenced, quoted, or analyzed. If you need to update, delete, or create notes, you should use the appropriate note management tools.",
  ...
  project: {
    ...
    requiresDataFetch: false,
    instructions:
      "This is a project from the user's project list. You can reference it or suggest updates. To modify projects, use the project management tools.",
  ...
  table: {
    ...
    instructions:
      "This is a data table reference that you can analyze, query, or reference. You can perform calculations, filter data, or answer questions about it. To modify the table, use the table management tools.",
    extractMetadata: (data: TableResourceData) => ({
      name: data.table_name,
      ...(data.description && { description: data.description }),
      reference_type: data.type,
      ...((data.type === "table_row" || data.type === "table_cell") && {
        row_id: data.row_id,
      }),
      ...((data.type === "table_column" || data.type === "table_cell") && {
        column: data.column_display_name || data.column_name,
      }),
      ...(data.row_count !== undefined && {
        row_count: String(data.row_count),
      }),
    }),
  ...
  file: {
    ...
    requiresDataFetch: false,
    instructions:
      "This is a file attachment. You can reference its contents or metadata. If you need to modify or create files, use the file management tools.",
  ...
  webpage: {
    ...
    requiresDataFetch: false,
    instructions:
      "This is web content scraped from a URL. You can reference, summarize, or analyze this content. Note that this is a snapshot and may not reflect the current state of the webpage.",
  ...
  audio: {
    ...
    requiresDataFetch: false,
    instructions:
      "This is an audio file. If a transcript is available, you can reference it. Use audio management tools to modify or create audio files.",
...
} satisfies ResourceFormatConfigMap;
...
function formatTableSchema(
  data: Extract<TableResourceData, { type: "table_schema" }>,
): string {
  if (!data.fields?.length) return `Schema for table "${data.table_name}"`;
  return [
    `## Schema: ${data.table_name}`,
    "",
    ...data.fields.map(
      (field) =>
        `- **${field.display_name || field.field_name}** (${field.field_type})${field.is_required ? " — required" : ""}`,
    ),
  ].join("\n");
}

function formatFullTableAsMarkdown(
  data: Extract<TableResourceData, { type: "full_table" }>,
): string {
...
function formatTableRow(
  data: Extract<TableResourceData, { type: "table_row" }>,
): string {
...
function formatTableColumn(
  data: Extract<TableResourceData, { type: "table_column" }>,
): string {
  const columnName =
    data.column_display_name || data.column_name || "Unknown Column";
  if (!data.rows || data.rows.length === 0) {
...
function formatTableCell(
  data: Extract<TableResourceData, { type: "table_cell" }>,
): string {
  const columnName =
    data.column_display_name || data.column_name || "Unknown Column";
 ...
 */
function formatConfiguredResource<TResource extends Resource>(
  resource: TResource,
  config: ResourceFormatConfig<TResource["data"]>,
): string {
  if (!config.includeInContent) {
    return "";
  }
...
export function formatResourceToXml(resource: Resource): string {
  switch (resource.type) {
    case "note":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.note);
    case "task":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.task);
    case "project":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.project);
    case "table":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.table);
    case "file":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.file);
    case "webpage":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.webpage);
    case "youtube":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.youtube);
    case "image_url":
      return formatConfiguredResource(
        resource,
        RESOURCE_FORMAT_CONFIG.image_url,
      );
    case "file_url":
      return formatConfiguredResource(
        resource,
        RESOURCE_FORMAT_CONFIG.file_url,
      );
    case "audio":
      return formatConfiguredResource(resource, RESOURCE_FORMAT_CONFIG.audio);
    default:
      return "";
  }
}
 ...
 */
export function extractSettingsAttachments(
  resources: Resource[],
): ProcessedResources["settingsAttachments"] {
 ...
 */
export async function processResourcesForMessage(
  resources: Resource[],
): Promise<ProcessedResources> {
  return {
 ...
 */
export function appendResourcesToMessage(
...
function parseResourceXml(
  ...
  try {
    const openingTagMatch = resourceXml.match(
      /<resource\s+type="([^"]+)"\s+id="([^"]+)">/,
    );
    if (!openingTagMatch) {
 ...
 */
export function parseResourcesFromMessage(
  messageContent: string,
): ParsedResource[] {
  const resources: ParsedResource[] = [];
  const resourceRegex =
    /<resource\s+type="[^"]+"\s+id="[^"]+">(.*?)<\/resource>/gs;
  let match;
...
export function messageContainsResources(messageContent: string): boolean {
  return (
    messageContent.includes("<attached_resources>") ||
    messageContent.includes("<resource type=")
  );
}
...
export function extractMessageWithoutResources(messageContent: string): string {
  let cleaned = messageContent.replace(
    /<attached_resources>.*?<\/attached_resources>/gs,
    "",
  );
...
export function splitMessageIntoSegments(
  ...
  for (const resource of resources) {
    if (resource.startIndex > currentIndex) {
      const textContent = messageContent.substring(
        currentIndex,
        resource.startIndex,
      );
      if (textContent.trim()) {
 ...
 */
export function messageHasResource(
  messageContent: string,
  resourceType: string,
  resourceId: string,
): boolean {
