...
export function useCreatorOwnershipSync(agentId: string | null | undefined) {
  ...
  // When ownership is unknown — isOwner not yet fetched (null) — ask the DB
  // for the authoritative access level so a CERTAIN determination becomes
  // possible. One RPC per agent; once isOwner is known (true/false) the
  // condition is false and it never re-fires. The agx_get_access_level path
  // isn't loaded on every surface (e.g. the chat route), so this is what makes
  // the flag actually resolve.
  useEffect(() => {
    if (agentId && isOwnerFlag === null) {
      void dispatch(fetchAgentAccessLevel(agentId));
    }
  }, [dispatch, agentId, isOwnerFlag]);

  const isOwner = !!agentId && isOwnerFlag === true;
