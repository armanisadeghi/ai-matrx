...
Recipe: React run site → `useSlotRunner`; React non-run site (a `defaultAgentId` prop, an on-click launch) → `useAgentSlot` + gate the affordance on resolution; execution-system launch → **pass `slotKey` to `launchAgentExecution`** (never resolve-then-pass-agentId — that path drops the binding's `config_overrides`); other thunk/handler → `await resolveAgentSlot` and forward BOTH `agentId` and `configOverrides`. Drop `<SlotAgentPicker>` wherever the user should be able to choose. Then move the slot from aidream's `scripts/seed_slot_placeholders.py` into a real `declare_slot(...)` in `aidream/services/agent_slots/client_slots.py` and release — `sync_declared_slots` pops the placeholder marker, so the DB stops claiming the hardcoded path still runs.

**Migrated:** research Outputs Studio (3), content-plan setup (7), kind architect, agent-app coding agent, flashcards spoken-front TTS, War Room (3). **Remaining:** cx-chat + new-chat defaults (2), `projects.create_assistant` (debug-only).
...
**`launchAgentExecution` is slot-aware (gap closed 2026-08-09):** pass `slotKey` instead of `agentId` and the launch thunk resolves the slot itself and applies BOTH halves of the binding — the agent, and its `config_overrides` merged over the caller's `config.llmOverrides` (binding wins per key, same precedence as `useSlotRunner`) into the instance-model-overrides slice, so every turn's request carries them as `config_overrides`. `slotKey` is mutually exclusive with `agentId`/`shortcutId` (loud throw). Consumers: content-plan setup (7 slots), flashcards spoken-front TTS. Pinned by `execution-system/thunks/__tests__/launch-slot-config-overrides.test.ts` (real thunks + reducers, slot service mocked; asserts the assembled request body).

**Known gap (narrowed):** consumers that mint durable conversations via `createManualInstance` directly (War Room room/thread personas, the master-tools `message-thread` handler) and the agent-run-window openers (assists `launch_agent`, kind builder) still apply only the slot's AGENT — a settings-only binding does not reach those chats. Instance overrides are Redux-only and a reloaded durable conversation rebuilds without them, so the right fix there is a design decision (persist per-conversation overrides vs re-resolve per turn), not a call-site patch.

## Change Log

- 2026-08-09 — `config_overrides` wired into the `launchAgentExecution` path: the thunk gained first-class `slotKey` support (resolves the slot, seeds the binding's overrides into instance-model-overrides so the request carries them); content-plan setup + flashcards TTS migrated off resolve-then-pass-agentId; slot service `configOverrides` retyped `Partial<LLMParams>` (non-null). Settings-only bindings now take effect on that path; the known gap narrowed to `createManualInstance`-direct durable-conversation consumers and agent-run-window openers.
