...
- 2026-08-09 — DOOR LAW pass on `/agents/slots`: every agent the page names (resolved agent, system default, org-override agents, the editor's "Selected agent") is now an `EntityRef` door (open / new tab / peek), the member-view org narrative links each org (registry gained `organization.hrefFor` → `/organizations/{id}`), and the slot-card header became a div-with-button-semantics so doors can legally nest. Doors pinned by `components/__tests__/slot-overrides-doors.test.tsx`.
