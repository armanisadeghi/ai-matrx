...
Status of the base system: **structure + builder UI shipped** (see [`ORCHESTRAS.md`](./ORCHESTRAS.md)). An Orchestra is an orchestrator agent + `platform.associations` edges; you can build/arrange/annotate one, but the orchestrator does **not yet run its members**. This doc is the prioritized path to make it real — written so another developer can pick up any phase cold.
...
- **FE — live member highlight. DONE** (mount model chosen: **(a) embedded run panel inside the builder** — the real canvas lights up). Desktop Run opens `agent-sets/run/OrchestraRunPanel` (embedded `AgentRunnerPage`, surfaceKey `agent-set-builder:${id}`) beside the canvas; `sub_agent` init/completion events (child `conversation_id` in init metadata → `initial_agent_id` lookup) drive per-member running/done/failed rings on the canvas + dots on the Grid. Mobile keeps the full-runner route. Details: `ORCHESTRAS.md` § Runtime delegation.
...
- **Orchestra modes.** `sequential` (fixed pipeline, output→input wiring), `parallel` (fan-out/gather), `supervisor` (P1 router). Store on the marker.
...
- **Sharing an Orchestra.** Share the orchestrator + ensure members are reachable by the grantee (reuse `features/sharing/`). Define what "sharing an Orchestra" grants.
...
- **Templates / duplicate.** "Save as Orchestra template" + clone-an-Orchestra (duplicate the orchestrator + re-create edges).
...
## Shipped — Generate an orchestrator (template → descriptions → injection → Orchestra)

Users without an orchestrator get one generated: copy the "Agent Orchestrator" template, run the Orchestra Role Describer on the selected agents, inject the `<agent>` blocks into `<available_agents>`, wire the Orchestra. Details + injection invariants in [`ORCHESTRAS.md`](./ORCHESTRAS.md) ("Generating an orchestrator"). A **Sync prompt** builder action keeps the prompt in step with membership.
