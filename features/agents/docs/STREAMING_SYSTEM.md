...
- `2026-07-12` — claude: **Agent Handoff + Conversation Value Store integrated (backend patterns 1+2; contract: aidream `docs/cx_chat/FE_HANDOFF_AGENT_PATTERNS.md`).** (1) **Handoff rebind:** a `record_reserved cx_message` with `metadata.handoff: true` re-keys the live assistant bubble (`promoteMessageId`) from the loop-start placeholder to the durable synthetic row the answer persists under; the tracked turn is updated in place (never pushed as a second reservation) so the end-of-stream commit writes to the durable id/position. (2) **Handoff rewind:** `completion {operation:"sub_agent", status:"failed"}` (never suppressed) truncates the live bubble to the pre-handoff snapshot captured at the last `tool_started` boundary — new `rewindContentForFailedHandoff` reducer drops the specialist's render blocks, reasoning chunks, and CONTENT-bearing timeline entries (a stale `text_end.rawText` would otherwise resurrect the truncated text at commit); the caller's retry streams after into fresh blocks (`breakTextBlock` first). No boundary observed -> loud console.error, no destructive rewind. (3) **Value-store events:** `data` events discriminated by `kind` (`value_store.stored` / `value_store.groomed`; typed in `stream-events.ts` — the aidream generator gap in `scripts/generate_types.py` was fixed to emit them) become stream-time-only render blocks `value_store_stored` (compact "result ready" card; the descriptor fence renders via the envelope chip renderer, never as prose/code) and `context_groomed` (subtle one-line receipt). `selectUnifiedSlots` emits them at their exact slot via the stamped `blockId`; `assembleMessageParts` deliberately ignores them (never persisted — the descriptor lives on the tool_call row). Inline `output_directive:context_groom` matrx fences render via a registered envelope renderer; the `conversation_value` reference resolver live-fetches `key — description` from `chat.conversation_value`.
