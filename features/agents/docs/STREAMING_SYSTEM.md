...
- **Terminal-settlement guard:** once the stream declares the request terminal — `completion(user_request)`, a fatal `error` event, or `end` — `process-stream.ts` arms a **30s** grace window (`POST_TERMINAL_GRACE_MS`); if the server never closes the socket (heartbeats would otherwise keep the watchdog happy for the full 24h lifetime), the client aborts locally and exits through the normal commit path, so awaited thunks ALWAYS settle on a terminal run. Firing is a **server defect** — screamed via `captureError` source `agent-stream-terminal-guard` (D130: headless image pipeline hung >8 min on a completed run).
...
- `providerRetry` / `providerRetryHistory` — recoverable provider-capacity retry state. Anthropic 529 overloads render as a live card with countdown + server-backed Cancel / Retry now controls; they do not populate `error` unless the server later emits a fatal `error` event. A scheduled retry with `discard_partial_output:true` rewinds to the current `cx_request` reservation boundary before the next attempt: failed-attempt text, reasoning, render blocks, citations, and content timeline entries are removed together, while prior iterations remain intact. Missing boundary is loud and non-destructive.
...
Builder manual runs have two identities by design: a stable local Redux conversation key and a fresh server wire conversation for each `/ai/manual` request. Stream dispatch stays on the local key (`forceLocalConversationId`), but any operation that writes a foreign key for a reserved message must use the message's persisted relationship. Artifact materialization enforces this by letting `cx_canvas_upsert` resolve `chat.message.conversation_id` and then using the returned canvas row's conversation for adapter/discovery writes.
...
- **Conversation identity depends on the route.** Durable saved-agent launches use a client-generated ID that the server honors; Builder `/ai/manual` mints a fresh wire ID per request while the UI keeps a separate local key. For persisted child rows, their server-recorded relationship is always authoritative.
...
Cross-repo system-of-record for the planned unification of this pipeline: /Users/armanisadeghi/code/common-docs/projects/unified-content-pipeline/FEATURE.md — read it before touching the parsing/rendering spine in ANY repo.
