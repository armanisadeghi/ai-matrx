...
- `2026-05-23` — claude: Pre-token shimmer wording changed "Processing…" → "Planning…" (`EnhancedChatMarkdown` `isWaitingForContent` branch) per product direction — matches the existing `AgentPlanningIndicator` ("waiting for the server to accept/route the request").
