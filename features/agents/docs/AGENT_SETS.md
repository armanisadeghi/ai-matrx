...
3. **Sync the prompt.** The builder detects a **template orchestrator** (`useOrchestratorPromptStatus`: its prompt has the `<available_agents>` markers) and shows a **Sync agent listings** action — with an amber pulse when the listed agent ids **differ from the current members**. `syncOrchestratorPrompt` runs two headless passes (both ephemeral, `displayMode:"background"`, `autoRun:true`, vars in `runtime.variables`), marker-pre-checked before the slow LLM runs:
   - **(a) Backfill member identities first** (`backfillMemberIdentities`). Members with a **blank name OR description** are dumped (`NAMER_DUMP_COLUMNS` — includes `messages`) to the **Agent Namer** (builtin `a3e9d1c4…`), which returns a JSON array of `{id,name,description}` (`parseNamerOutput`). Only the **blank** field(s) are written back per member (`updateAgentIdentity`) — an author's existing name/description is NEVER overwritten. A member the caller can't edit (RLS) is skipped, not fatal. Runs first so the listing in (b) reflects the fresh names. The count surfaces in the success toast.
   - **(b) Regenerate the listing.** The Agent Description Generator (builtin `62d56534…`) runs on a `{id,name,description,output_schema,variable_definitions}` dump of the **current members** → `<agent>` blocks → injected into `<available_agents>` (`extractAgentBlocks` + `injectAvailableAgents`). The generator's local Redux conversation key is deliberately **not** sent as a wire `conversation_id`; ephemeral turn 1 uses `is_new:false, store:false`, and the stream stays bound to the local key.

   **Reasoning never leaks into the prompt:** the headless `responseText` these passes read is derived via the canonical `deriveAnswerText` (in `launch-agent-execution.thunk.ts`'s `pollForCompletion`), which EXCLUDES `thinking`/`reasoning` blocks. A raw render-block join (the prior bug) wrote the generator's chain-of-thought straight into `<available_agents>`.
...
**Member identity backfill (BUILT):** "Sync agent listings" now also names any member missing a name/description via the Agent Namer — see step 3(a). **Future — auto-add agents (documented, NOT built):** when a user creates a member agent FROM a template, auto-generate that agent's `<agent>` entry and append it to every orchestrator whose set it joins. See [`AGENT_SETS_ROADMAP.md`](./AGENT_SETS_ROADMAP.md).
