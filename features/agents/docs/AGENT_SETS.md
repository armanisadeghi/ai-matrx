...
3. **Sync the prompt — one button, one AI pass** (`syncOrchestratorPrompt`). The builder detects a **template orchestrator** (`useOrchestratorPromptStatus`: its prompt has the `<available_agents>` markers) and shows a **Sync agent listings** action (amber-pulsing when the listed agent ids **differ from the current members**). Marker-pre-checked before the slow LLM run, then:
   ...
   **No `<available_agents>` section → "Enable sync", never a silently-missing button.** The Sync action is gated on `useOrchestratorPromptStatus().isTemplate` (the prompt HAS the markers). When an orchestrator is loaded but its prompt has no section (`ready && !isTemplate`), the builder shows a **yellow "Enable sync"** action instead of hiding everything: one click runs `enableOrchestratorSync` → `orchestratorService.ensureAvailableAgentsSection` (idempotent — appends a labelled empty `<available_agents></available_agents>` to the system message), then opens the **Agent Advanced Editor restricted to the System Instructions tab** (`useOpenAgentContentWindow({ initialTab:"system", tabs:["system"] })`) for review. `fetchFullAgent` refresh flips the action back to the normal "Sync agent listings". The warning pill uses the header template's new `warning` variant (`EntityHeaderAction.warning` → `bg-warning`).
