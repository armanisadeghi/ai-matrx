...
- `2026-05-24` — **Model Settings: every setting always renders; incompatible ones are flagged + one-click repairable; structurally locked.** Root cause of a long-recurring bug: all three settings panels rendered each group as `group.filter(({key}) => !!getControl(key)).map(...)`, so a setting only appeared if the *selected model* declared a control for it — switching models silently changed which settings were visible, and most vanished for sparse-control models. The renderer already supported unsupported rows (FallbackValueInput + amber "unknown" dot) but the upstream filter never let them through. Fixes:
  - **Single chokepoint.** New `lib/redux/slices/agent-settings/settings-catalogue.ts` owns the grouped catalogue + `buildSettingsRows(controls, settings)`, which returns **every** catalogue entry (plus a catch-all for any valued/model-declared key) tagged `{ control, supported, hasValue }`. It cannot drop a row — `controls` only decorates. The three panels (`AgentSettingsCore.tsx`, `features/prompts/.../configuration/ModelSettings.tsx`, `features/agent-settings/.../LLMParamsGrid.tsx`) now map over it; their duplicated `textModelSettings`/`booleanSettings`/`imageVideoSettings`/`audioSettings` arrays were deleted. Unsupported rows render with a caution indicator and read-only/fallback value display (never silently changed).
  - **Repair restored.** New `unsupported_by_model` validation category + rule (`validation/rules.ts`): a valued, recognized LLM param the model declares no control for is flagged `warning` (excludes UI capability flags; skipped when the model declares zero controls — a data gap, not an incompatibility). This feeds the existing top-of-panel `IssueTable` (caution at the top) and is fixable via `apply-fix.ts` (clear) — restoring per-row **Fix** and **Fix all**. `analyze.ts` maps it to `unsupported-key` so model-switch reconciliation detects it too (previously carried such values over silently).
  - **Structural lock (won't regress).** Jest guardrails assert `buildSettingsRows` returns one row per catalogue entry for empty/sparse controls (`lib/redux/slices/agent-settings/__tests__/settings-catalogue.test.ts`) and that the new rule fires + clears (`validation/__tests__/unsupported-by-model.test.ts`). An ESLint `no-restricted-syntax` guard (`eslint.config.mjs`, scoped to the three panels + validation dir) errors on any `.filter`/`.some` that references `getControl`/`controls[key]`/`normalizedControls[key]` — the exact recurring pattern. Verified: 10/10 tests pass, repo-wide tsc clean, guard catches the pattern and stays silent elsewhere.
