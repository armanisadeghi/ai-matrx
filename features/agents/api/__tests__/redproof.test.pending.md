<!-- Pending red-proof artifact preserved by the integration sweep. -->
...
import {
  submitToolResult,
  flushToolResults,
} from "./__redproof__/prefix-module";

// The pre-fix module has no outbox at all — that IS the defect. Stubs so the
// file compiles and the ASSERTIONS are what fail.
const flushUndeliveredToolResults = (_id: string) => () => undefined;
const __getOutboxForTests = () => new Map<string, Map<string, unknown>>();
const __resetOutboxForTests = () => undefined;
