...
| Date | Who | Change |
|---|---|---|
| 2026-07-14 | codex | Cold `/chat/[conversationId]` deep-links now render bottom-first: transcript skeleton until bundle hydration, latest user + assistant groups first, prior four groups auto-revealed above with scroll anchoring, then older loaded groups revealed two at a time before paged DB fetches. Live URL-promotion streams bypass the cold path. |
