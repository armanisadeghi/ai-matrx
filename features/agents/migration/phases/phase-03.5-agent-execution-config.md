...
| 2026-08-14 | claude | **Retired the standing "known follow-up" from the 2026-04-21 row.** That row warned the DB RPCs `agx_get_shortcuts_initial`, `agx_build_shortcut_menu`, and `agx_get_user_shortcuts` "reference old column names and need updating before management-page paths work". They no longer do: later migrations (`agent_legacy_owner_cols_cut.sql`, `assoc_soft_delete_tombstones.sql`, `repoint_project_member_functions_to_iam.sql`) rewrote all three against the v2 column set, and the live-generated `types/database.types.ts` shows the full execution-config surface (`display_mode`, `allow_chat`, `auto_run`, `show_variable_panel`, `variables_panel_style`, `show_definition_messages`, `hide_reasoning`, `hide_tool_results`, `show_pre_execution_gate`, `bypass_gate_seconds`, `default_user_input`, `default_variables`, `context_overrides`, `llm_overrides`) on their return types. Nothing to fix. Separately, the management-page paths never depended on them anyway — `useAgentShortcuts` reads via `fetchShortcutsForScope` / `fetchCategoriesForScope`, not these three RPCs. Do not re-open this as a blocker. |
