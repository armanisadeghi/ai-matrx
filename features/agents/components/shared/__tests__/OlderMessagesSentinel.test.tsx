...
function page(start: number, end: number): MessageRecord[] {
  return Array.from({ length: end - start }, (_, index) => {
    ...
    return {
      ...
      metadata: {},
      createdAt: "2026-09-08T20:21:44Z",
      deletedAt: null,
...
describe("OlderMessagesSentinel automatic fetching", () => {
  ...
  function land(start: number, end: number) {
    act(() => {
      store.dispatch(setOlderLoading({ conversationId, loading: true }));
    });
    act(() => {
      store.dispatch(
        prependMessages({
          conversationId,
          messages: page(start, end),
  ...
  it("does not turn a failed page into an automatic retry loop", () => {
    ...
    act(() => {
      store.dispatch(setOlderLoading({ conversationId, loading: false }));
    });
    frame();
    frame();
    expect(loadOlderMessages).toHaveBeenCalledTimes(1);
