...
2. **Fresh-start guard** (ref-guarded mount effect, runs before the launcher): `dispatch(clearFocus(surfaceKey))` — drops any stale per-agent focus so a previously-used agent can't revive its old conversation.
