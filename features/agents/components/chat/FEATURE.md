...
- `ChatIncognitoHeaderButton.tsx` — toggles incognito UI on fresh chat routes (`/chat/new`, `/chat/a/[agentId]`). Portals a `GhostTapButton` into `#shell-header-right` (immediately left of the avatar). When active, stamps `data-chat-incognito` on `.shell-root` (via `ChatIncognitoProvider`) to black out the sidebar and avatar. API wiring (`isEphemeral` / `store:false`) is deferred.
- `ChatIncognitoProvider.tsx` — chat-layout context for incognito toggle + shell attribute sync.
...
- `2026-06-10` — claude: **incognito UI toggle (sidebar + avatar blackout).** Ghost button now toggles incognito mode on fresh chat routes; active state fills the tap button dark. `ChatIncognitoProvider` stamps `data-chat-incognito` on `.shell-root` to mute `.shell-sidebar`, `.shell-user-menu-wrapper`, and `.shell-mobile-sheet`. Server `store:false` / `isEphemeral` wiring still deferred.
