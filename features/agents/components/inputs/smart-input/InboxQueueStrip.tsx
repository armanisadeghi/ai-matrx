 ...
 *
 * Shared by every composer. Queued drafts stay in the waiting strip; steering
 * messages lift into a separated, right-aligned bubble as soon as delivery is
 * requested, including while promotion is still retracting the queued row. Cards disappear when their message officially
...
export function InboxQueueStrip({ conversationId }: InboxQueueStripProps) {
  ...
  const [promotingIds, setPromotingIds] = useState<ReadonlySet<string>>(
    new Set(),
  );
  ...
  const isDeparting = (item: ConversationInboxItem) =>
    !isCollabNoteItem(item) &&
    (item.mode === "steer" || promotingIds.has(item.injectionId));
  const departing = visible.filter(isDeparting);
  const waiting = visible.filter((item) => !isDeparting(item));
  const ordered = [...departing, ...waiting];

  return (
    <div className="flex flex-col gap-1 px-2 pb-1 shrink-0">
      {ordered.map((item) => {
        const failed = item.status === "failed";
        const promoting = promotingIds.has(item.injectionId);
        const departing = isDeparting(item);
        const busy = item.status === "sending" || promoting;
        const collabNote = isCollabNoteItem(item);
        const editable = item.status === "pending" && !collabNote && !busy;
        const collabAgent = collabNote ? parseCollabNoteAgent(item.text) : null;
        const Icon = collabNote ? Handshake : departing ? ArrowUp : Clock;
        ...
        return (
          <React.Fragment key={item.injectionId}>
            {waiting[0]?.injectionId === item.injectionId && (
            ...
            <div
              data-inbox-mode={departing ? "steer" : "queue"}
              aria-busy={busy}
              className={`flex min-w-0 items-start gap-2 rounded-lg border px-3 py-2 text-xs lg:items-center ${
                departing
                  ? "ml-8 mb-6 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-3 motion-safe:duration-200"
                  : ""
              } ${
                failed
              ...
              <div className="min-w-0 flex-1 self-center">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span
                      tabIndex={0}
                      className={`${departing ? "line-clamp-3 whitespace-pre-wrap break-words" : "block truncate"} rounded-sm text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring`}
                    >
                ...
                {item.mode === "queue" &&
                  ...
                  !busy && (
                    <IconButton
                      ...
                      onClick={async () => {
                        setPromotingIds((ids) =>
                          new Set(ids).add(item.injectionId),
                        );
                        try {
                          await dispatch(
                            promoteQueuedToSteer({
