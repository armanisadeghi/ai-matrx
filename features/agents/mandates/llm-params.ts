/**
 * Pure narrowing of a mandate binding's `config_overrides` Json into the
...
export function toLlmParams(obj: JsonObject): Partial<FeLlmParams> {
  ...
  if (dropped.length > 0) {
    console.warn(
      `[mandates] mandate resolution dropped unsupported config_overrides keys: ${dropped.join(", ")} — extend toLlmParams or apply them server-side`,
    );
  }
  return out;
}
