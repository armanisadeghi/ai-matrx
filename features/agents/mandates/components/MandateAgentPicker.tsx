...
/**
 * MandateAgentPicker — the reusable consumer-facing "which agent runs this step"
 * control. Drop it next to any affordance whose agent resolves through an
 * mandate: it shows the currently-resolved agent (system default vs your
...
export function MandateAgentPicker({
   ...
   * live declaration (canonical `compareContracts`) instead of the mandate's
  ...
  contractSource?: MandateContractSource | null;
}) {
  ...
  const [data, setData] = useState<MandatePickerData | null>(null);
  ...
  const handlePick = async (candidateId: string) => {
    ...
    try {
      ...
      // one (what the surface SHOWS is what we CHECK), else the mandate's
      // stored contract.
        ...
        {loadError ? (
          ...
          <div className="space-y-2.5">
            ...
            <AgentListInlinePicker
              consumerId={`mandate-agent-picker-${mandateKey}`}
