...
3. **ONE core, two hosts**: `workspace/MandateWorkspace` renders identically on
   `/agents/mandates/[mandateKey]` (key OR uuid) and inside `MandateWindow`'s Yours pane.
   Named deliberate divergences only: the window's scope list + Admin pane.
...
| `service.ts`                                   | `resolveMandate(mandateKey)` — client-side resolution for mandates whose consumer runs in this repo. Walks **system → org → user** (org layer added 2026-08-26; user config merges over org per key — the server's rule). Floating-only; loud on unknown/disabled/version-pinned mandates **and on a binding whose Holder is not an agent** (THE HOLDER GATE below) — the resolver's version channel is the one open D1 item (the RUN endpoint already carries `is_version`). 5-min cache + `invalidateMandateCache` (+ `onMandateCacheInvalidated`). |
...
Routes: `app/(core)/agents/mandates/page.tsx` (list) · `app/(core)/agents/mandates/[mandateKey]/page.tsx` (workspace; segment accepts key or uuid) · `app/(core)/organizations/[orgId]/settings/mandates/{,[mandateKey]}` (org principal).
...
- 🚨 **WINDOW FIRST, ROUTE SECONDARY** (Arman's D4 ruling, 2026-08-26, reconciling handled-in-place with the dedicated route). A mandate named on a WORKING surface opens **`mandateWindow`** ([`useOpenMandateWindow`](../../overlays/openers/mandateWindow.tsx)) in place — its Yours pane IS `MandateWorkspace`. The dedicated route `/agents/mandates/[mandateKey]` serves the browse list's name anchor, new-tab, deep links, and shared URLs. A `<Link>` to a mandate from a working surface is still a regression.
