...
function stopIfCancelled(signal?: AbortSignal) { if (cancelled(signal)) throw new Error("The 1Password import was cancelled."); }
...
async function validateDirectoryRange(entry: Entry, signal?: AbortSignal): Promise<void> {
  const runtime = entry as Entry & { getData?: (writer: WritableStream<Uint8Array>, options: { checkOverlappingEntryOnly: true; signal?: AbortSignal }) => Promise<unknown> };
  if (typeof runtime.getData !== "function") throw new Error("The 1Password archive is unsafe.");
  await runtime.getData(new WritableStream<Uint8Array>(), { checkOverlappingEntryOnly: true, signal });
}
function isFileEntry(entry: Entry): entry is Entry & { directory: false; getData: (writer: WritableStream<Uint8Array> | ReturnType<typeof boundedText>, options: { checkCrc32?: boolean; checkOverlappingEntryOnly?: boolean; signal?: AbortSignal }) => Promise<unknown> } {
  return !entry.directory && "getData" in entry && typeof entry.getData === "function";
}
function warnings(reader: ZipReader<Blob>, entry: Entry) { return Boolean(reader.warnings?.length || entry.warnings?.length); }
...
export async function readOnePuxArchive(file: Blob, limits: Pick<CsvImportLimits, "maxFileBytes" | "maxRecords">, signal?: AbortSignal): Promise<OnePuxArchive> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.filename.startsWith("files/") && entry.filename.slice(6) && !entry.filename.slice(6).includes("/")) { binaryMemberCount += 1; continue; }
      throw new Error("The 1Password archive is invalid.");
    }
    const used = { value: declared }; let attributesText: string | undefined; let dataText: string | undefined;
    for (const entry of entries) {
      stopIfCancelled(signal);
      if (entry.directory) await validateDirectoryRange(entry, signal);
      else if (isFileEntry(entry)) await entry.getData(new WritableStream<Uint8Array>(), { checkOverlappingEntryOnly: true, signal });
      else throw new Error("The 1Password archive is unsafe.");
      stopIfCancelled(signal);
