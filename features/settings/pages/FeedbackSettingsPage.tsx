...
function getActiveStageIndex(status: FeedbackStatus): number {
  switch (status) {
    ...
    case "awaiting_review":
      return 4;
    case "user_review":
      return 4; // Same QA stage — waiting on user
    case "resolved":
      return 5;
    case "closed":
      return 6; // past last stage = all complete
    default:
      return -1;
  }
}
...
function CompactProgressIndicator({ status }: { status: FeedbackStatus }) {
  ...
  const currentStage = isAllComplete
    ? STAGES[totalStages - 1]
    : STAGES[Math.max(0, Math.min(activeIndex, totalStages - 1))];
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  (isFuture || (isAllComplete && i === STAGES.length - 1)) &&
                    !isCompleted &&
                    !isActive &&
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-medium text-cyan-700 dark:text-cyan-400">
                  {isUserReview
                    ? "The team needs your help verifying a fix"
...
export default function UserFeedbackPage() {
  ...
  const otherActiveItems = items.filter(
    (i) => !["closed", "wont_fix", "user_review"].includes(i.status),
  );
  const activeItems = [...userReviewItems, ...otherActiveItems];
  const closedItems = items.filter((i) =>
    ["closed", "wont_fix"].includes(i.status),
  );
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 border border-dashed border-border rounded-lg">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Use the feedback button in the bottom-right corner to report bugs or
            request features.
          </p>
        </div>
      ) : (
