...
- Navigation: `SettingsFlatNavigation`, `SettingsTree`, `SettingsDrawerNav`, `SettingsBreadcrumb`
...
**RPCs** (direct from the browser — `SECURITY DEFINER`, granted to `authenticated`, identity from `auth.uid()`)
...
- **Category tabs (e.g. `id: "general"`, `id: "ai"`) have `component: Placeholder`.** Desktop navigation renders them as noninteractive headings; mobile keeps them as drill-in screens. The placeholder is never rendered for a category, but remains required to satisfy the type.
...
- `2026-09-20` — **The settings registry is now the vocabulary a declared `/user-settings/...` href is judged against, whole (V-29 NEW-7).** `lib/route-manifest/vocabulary.ts` used to read the FIRST URL segment only, so `/user-settings/integrations/not-a-real-tab` passed the "does this href land on what it names" guard although `urlToTabId` resolves it to a tab id `findTab` cannot find (which renders no tab at all). The vocabulary now holds every registry tab id, sub-tabs included (`integrations.googleWorkspace`), and its drift test IMPORTS `settingsRegistry` rather than grepping this file's source — the old regex (`^\s+id: "…"`) measured one spelling in one file and missed a single-quoted or computed id. Nothing in `features/settings/` changed: adding, renaming or retiring a tab now fails `lib/route-manifest/__tests__/a-declared-href-lands-on-what-it-names.test.ts` until `vocabulary.ts` knows, and every href built from a registry tab id is asserted to answer. The runtime `config.*` taxonomy sections (`universal/configTree.ts`) are declared as the one family that check cannot enumerate. Evidence: `npx jest lib/route-manifest features/connectors` → 58 suites / 435 passed, proven RED with a planted section.

- **2026-09-20 — the complete-list-read detector follows ONE hop across a return, and the guard now actually runs somewhere (V-28 NEW-7 + NEW-8).** Two holes the hostile verifier found in the F-124 round. (1) The index-shape detector could only see a read and its verdict inside ONE function, so the original knob bug's most natural spelling — `const rows = await loadRows(); const byKey = new Map(rows.map(…)); byKey.get(k) === undefined`, the verifier's `missC` — was **unflagged** while the two in-function variants beside it were caught, and a service/hook split is how this code is normally written. `scripts/check-unbounded-reads.ts` gained shape 3, `returned`: a function whose body performs a bare complete-list read and hands those rows back (`return rows` / `return data ?? []` / `return (data ?? []) as Row[]`) is a READ SOURCE, and every call of it in the SAME file is asked the shape-1 and shape-2 questions of its own result variable; the finding is reported at the READ line, the line that has to change. Deliberately bounded: one hop, one file, and a return DERIVED from the rows (`.filter(…)`, `.length`, `.map(…)`) is not a read source, because what the caller holds is no longer the list. The scan also stops at the end of the enclosing function — borrowing the NEXT helper's `return data ?? []` was a real false positive on `lib/scoped-config/service.ts` and is now quiet. Red-then-green on the verifier's own shape, not ours: the fixture file flagged `[]` under the HEAD detector and `{"line": 5, "shape": "returned", "consumer": "returned from loadRows() → new Map( → byKey .get(…) → undefined check"}` under the extended one; the five new self-test cases (the missC shape, a helper feeding `.find(`, and three that must stay quiet — render-only caller, `.limit(50)` helper, helper returning a count) run in `pnpm check:unbounded-reads --self-test`, fifteen cases, all PASS. Repo-wide the sweep moves 134 → 136 findings: `features/audio/services/audioChunkJournal.ts:241` and `features/mandates/list-door.ts:141`, both a helper's rows judged by a caller's `.length` comparison. (2) The guard was invoked by **NOTHING** — no CI job, no release gate — the same orphaning the five HR guards lived in. It is now a SIGNAL in `.github/workflows/ci.yml`'s `marker-law` job (`continue-on-error`, self-test step + sweep step; it sits in that static job rather than beside the credential-gated signals because those jobs are SKIPPED without the secret and this one is offline), and an advisory entry in BOTH `scripts/run-release-gates.sh` lanes with `--self-test` beside it. The sweep exits 0 by design — a finding is a question, "is this list allowed to be short?" — so the self-test is the half that can go red, and the presence of both steps is locked by `scripts/check-doc-claims.ts` (`run: pnpm check:unbounded-reads\n` and the `--self-test` line, both matched WITH the trailing newline so neither can be satisfied by the other). Proven live with the F-115 sabotage plant: replacing the sweep step with `run: echo SABOTAGE` turned `pnpm check:doc-claims --strict` RED, replacing the self-test step too kept it RED, and restoring both returned `✓ CLAUDE.md's 15 machine-checkable claims all hold`. CLAUDE.md's CI enumeration moved from four signals to five in the same commit.

- **2026-09-20 — the knob CATALOGUE is read as a complete list, not a first page (V-27 NEW-8).** `lib/knobs/featureKnobs.ts` loaded every `platform.feature_knob` row with a bare `.select("feature, key, value")`. PostgREST caps a response at 1000 rows and does not error — it answers 206 with a successful-looking array — and the table already holds ~870 live rows. Every read in that module is an existence check whose miss RAISES by design, so the 1001st knob would have made `knobInt`/`knobBool`/`knobStringList` report `Missing feature knob "<feature>.<key>"` for a row sitting in the table, and the knob system's whole promise (no silent frozen default) would have failed silently for whichever key sorted last. The catalogue now reads through `readAllRows` from `@ai-matrx/data/db` with `{ count: "exact" }` and a stable total order on the primary key pair `(feature, key)`; a read that cannot prove it collected every row throws rather than returning a short list. `__setFeatureKnobPageSizeForTests` is the seam that lets a fixture stand up three rows instead of 1001. `lib/knobs/featureKnobs.paging.test.ts` holds both halves against ONE fake client that truncates at two rows: the old bare read comes back 2 of 3 with the third knob simply absent (before the fix it raised `Missing feature knob "alpha.two"`), the new read comes back 3 of 3 and the fixture records more than one `.range()` request. Nothing else in `lib/knobs/**` or `features/settings/**` is named by `pnpm check:unbounded-reads` — re-verified 2026-09-20 against the EXTENDED detector, which now also names the shape this bug actually had: a bare complete-list read poured into a `Map`/`Record` index (`new Map(`, `Object.fromEntries(`, a `reduce` into an object, or a `for … of rows` loop writing `idx.set(…)` / `idx[key] = …`) whose index then decides existence (`.get(…) === undefined`, `.has(`, `key in idx`, `idx[key] ?? …`), including when the index is returned out of its function and interrogated elsewhere in the file. `pnpm check:unbounded-reads --self-test` replays the pre-`95b3f864` knob reader red-then-green; `--file <path>` scans one file so any pre-fix version can be replayed (`git show 95b3f864^:lib/knobs/featureKnobs.ts` is named at its read line; today's file is clean). The one knob reader the extension DID surface is `features/marketing/seo/topical-map/knobs.ts:273`, which reads `platform.feature_knob` into a `Map` and raises on `byKey.get(key) === undefined` — benign today because the read is `.eq("feature", …).in("key", [two keys])`, not the whole table.

- 2026-09-17 — **The notification-preference rows read the personal organization DELIBERATELY, by name.** `loadNotificationSettings` and `setNotificationPreference` still resolve `scopeOrganizationId ?? personalOrgId` / `organizationId ?? personalOrgId`, and that is correct rather than a fallback: on this surface "no employer named" MEANS "my default everywhere", and the platform stores that default as the row on the person's own organization (hr_l3_116 §1 — the ladder's second rung). Both lines now say so and carry the `// org-fallback-deliberate:` marker the new `scripts/check-org-fallback-shapes.ts` requires, so the guard that forbids substituting the selected organization everywhere else can still see this one and read its reason.

- 2026-09-17 — Plan & usage is scoped to the EXPLICIT active organization instead of the legacy `selectEffectiveOrganizationId` (`organization_id ?? personal_organization_id`). A plan belongs to an account, so with none selected `PlanUsagePanel` and `SpendBudgetCard` show their existing "Pick an organization" states with the picker door — never another account's plan under the wrong name.
- **2026-09-17 — The Media Source Catalog's organization knobs get a tab, drawn by the SERVER's declaration.** New tab `organizations.mediaCatalog` (`features/source-library/settings/SourceLibrarySettingsTab.tsx`, registered under `organizations`, `persistence: "server"`). It holds no list of settings: it renders whatever `GET /media/settings` declares (API-CONTRACT.md §9) — key, label, type, min/max and enum options all come from the response, so a knob the server adds tomorrow appears with no frontend change, and `MEDIA_SETTING_KEYS` stays a typed reference rather than a filter. Each row says in plain English where its value came from ("Platform default", "Inherited from your organization", "Set on this Library"), a declared type this screen cannot draw is shown READ-ONLY with the reason instead of a live-looking dead control, and writes go through `putMediaSettings(dispatch, "org", …)` with the screen re-rendering from the response — a refusal prints the server's own sentence plus its remedy and leaves the control on the value the server still holds. The `reclassification_needed` count is stated plainly (N videos would be classified differently, nothing has been reclassified) and, because this tab is organization-scoped while `POST …/classify` needs one Library, it says to run Reclassify on the Library rather than rendering a reclassify button that could not honestly fire. Loading is a skeleton at the controls' own dimensions; the server half of §9 is not deployed yet, so today's real state is the honest 404 sentence with a remedy and a Check-again control, never an empty tab or an endless spinner.

- **2026-09-14 — the write goes through the KEY's own door, with the key's own audit (DD-221).** DD-203 mounted THE exceptions picker on `/hr/settings/*` and, because `platform.knob_override_set` is the picker's one write path, wrote HR exceptions through it. Measured live 2026-09-14 with the real person `20149d3f-…` (an HR admin in Write Target Sandbox who is `role='member'`, not an org owner or admin), on `hr.employees.adjusted_service_date_rule` at the pay-group rung: `knob_override_set` **refused** them — *"Organization configuration is owner/admin only."* — while `hr_knob_set` **accepted** and filed the `hr.access_audit` row (counted: 1); and an org OWNER writing the same exception through `knob_override_set` succeeded while `hr.access_audit` did not move (1842 rows before, 1842 after). One setting, two doors, disagreeing about who may write AND about whether the write is recorded — decided by which RPC a component imported. **AUTHORITY BELONGS TO THE KEY.** `platform.knob_write_door` (new, provisioned by `platform.create_entity_table`, variant `system`) declares per feature namespace the set door, the removal door, and which authority decides; the `''` row is the default, so nothing falls through to a guess, and `hr.` declares `public.hr_knob_set` / `public.hr_knob_clear` under `hr._l1_settings_gate`. `platform.knob_write_door_for(key, organization)` answers BOTH halves in one read — which door, and whether THIS caller may write through it, computed server-side with the same predicates the real gates use, so the screen never infers authority from `org_role`. `platform.knob_override_set` now REFUSES a key whose namespace declares another door, naming it (`reason: "wrong_door"`); it does NOT forward, because a forward makes the gate you passed and the audit row you left depend on the name you typed. `public.hr_knob_clear` — which already existed for every rung `hr_knob_set` accepts — now names the cleared row in its audit row exactly as the set does; a removal is a write. Client side: `fetchKnobWriteDoor` + `writeKnobOverrideThroughDoor` in `lib/scoped-config/service.ts` (an unknown door is an ERROR naming itself, never a quiet fall back), `KnobOverrideRow` takes a `writeDoor`, and `KnobRungOverrides` reads the declaration on mount — the add control appears exactly when THAT door's authority holds, is absent while the answer is still loading, and the door's own sentence is shown wherever it refuses. `pickableRungsFor` stays PURE and stays about which rungs a reader can ANSWER: `check:settings-ladder-ui` imports it with no Next.js environment and no JWT, so a live authority consult cannot live there. Verified live over PostgREST as **admin@admin.com** on Write Target Sandbox: the declaration answered `public.hr_knob_set` / `public.hr_knob_clear` / `may_write: true` for the HR key and `platform.knob_override_set` for a `records.` key; `knob_override_set` on the HR key came back `wrong_door` with the sentence; the exception was set and removed through HR's own doors, each filing an `hr.access_audit` row whose `target_ids` names the pay group; `knob_resolve` answered `always_carry` at that rung and the platform default after removal; a `records.` table override was set and removed through the default door, unchanged. 0 override rows left (`hr.*` at non-organization rungs: 0).

- **2026-09-14 — HR exceptions are set where HR settings live, with the ONE picker (DD-203).** `platform.feature_knob` offers 194 `hr.*` keys at `employer_profile`, `pay_group` and `location` — 582 key-rung pairs — and no screen in the product offered a single one of them (V-57): the organization configuration page excludes every `hr.*` key by its own first line ("HR settings live on their own page"), and `/hr/settings`' own key row carried a dashed box saying "a scope override is stored on the scope row itself, so it is set where that row is edited", linking to pay-group and location pages that do not set one. **That box never rendered either**: it was gated on `presentation.scopes`, an optional field NOTHING in this repo has ever supplied (no call site passes `presentation` to `useHrKnobs` at all), so an HR key said nothing whatsoever about its three sub-org rungs. The dead field and its type are removed here. **The home is where HR settings already are**: `HrSettingsChrome` mounts `<UniversalSettingsProvider target="organization">` once around every `/hr/settings/*` panel, and `features/hr/settings/components/HrKnobExceptions.tsx` mounts THE picker — `<KnobRungOverrides>` itself, the same searchable `knob_scope_rows` list, the same `knob_override_set` write, the same removal confirm that names the row, the same read-only behaviour for a person the door would refuse (`knob_override_set` is owner/admin, so `canManageOrganization` is that exact test). Nothing here is a second picker; the file is a bridge, because HR's own editor reads the HR-shaped `hr_knob_index` row and the ladder wants `platform.knob_index`'s `ScopedKnob`. **And the rungs are only offered where they are answered.** Measured live from `pg_proc`: EVERY `hr.*` knob is resolved through `hr._knob(feature, key)` or `hr._hr_knob(feature, key, org, default)`, neither of which takes a person, an employment or a location — so neither can name a row-keyed rung, and an exception at one would be saved, listed back, and change nothing (DD-211's defect at 192-key scale). `migrations/dd203_hr_readers_name_the_rungs_hr_settings_offers.sql` teaches the two readers that DO hold a subject — `hr.rehire_service_dates` and `hr.sync_membership_to_employment` — to name all three rungs INLINE (dd211b's lesson: a local variable or a tidy helper blinds both `check:knob-resolve-callers` and the rung census, and the file asserts the text still names them). The dispatchers are deliberately untouched: naming a rung there would mean inventing one. For the rest, `features/settings/universal/rungRules.ts` declares `hr.` a STRICT namespace — inside it, a key the census does not positively name is a key no reader can answer, so the picker is ABSENT rather than selling an exception nothing honours. `rungRules.ts` exists as a pure module (re-exported by `scopeRows.ts`, so no call site changed) because `check:settings-ladder-ui` now grades reachability by asking `pickableRungsFor` what the screen would draw instead of re-implementing it — re-implementing it is how that guard printed green over 582 unreachable pairs. The guard also learned the HR destination, reading the prefix it serves out of `public.hr_knob_index`'s own `feature like 'hr.%'` rather than a constant, and its `--self-test` requires the one reachable HR pair to be reachable WITH `/hr/settings` mounted and unreachable without it. Baseline: **582 → 576** known-debt pairs, with the reason rewritten from "no screen offers it" (now false) to "no reader can answer it" (now the truth, with the owner). Grammar fixed while HR made it visible: "Add override for **an** employer profile", and the empty-state sentence lists every rung in the singular instead of un-pluralising only the last word ("employer profiles, pay groups, location"). Verified live as **admin@admin.com** over PostgREST against `Write Target Sandbox`: `knob_scope_rows` listed 1 employer profile, 13 pay groups and 3 locations BY NAME; an override saved at each rung through `knob_override_set` (`origin` = `employer_profile_override` / `pay_group_override` / `location_override`), `knob_resolve` with that scope returned each one while the organization rung stayed on the platform default; all three removed through the same door with a NULL value (`key_removed: true`), 0 rows left. **The browser proof is UNKNOWN for this lane** — the machine's dev-server lease was held by another session's checkout, so no screenshot was taken; the component tree is covered by jest with RED-first proofs.

- **2026-09-14 — An agent's write carries the AGENT, so the agent rung on the born-confirmed knob is real (DD-211).** V-64 found the rung DD-183's picker sells on `records.confirmation.agent_write_born_confirmed` doing nothing: `overridable_by` is `{organization, agent}`, `knob_override_set` accepted an agent override happily (`{"ok":true,"origin":"agent_override","effective_value":true}`) and this panel listed it back — and the knob's ONLY reader anywhere, `platform._stamp_actor_tier`, named a `table` rung and nothing else, because until now the database had no way to know WHICH AGENT was writing. Measured live before the fix, rolled back: agent rung on + table rung on + an AI-tier INSERT → `unconfirmed`; `knob_resolve` with a table rung → `false`, with an agent rung → `true`; functions in the whole catalog able to read an agent declaration → **0**. The fix is a third declared-actor value carried exactly like the other two (DD-176 R-A/R-B): **`platform.declared_actor_agent()`** reads the `app.actor_agent` GUC on every channel and the `x-matrx-actor-agent` header on the CLIENT channel only (role `authenticated`), warns and returns NULL on anything malformed, and `_stamp_actor_tier` names the agent rung beside the table one when an agent is declared (`migrations/dd211_the_agent_rung_on_a_write_is_real.sql`, then `dd211b_…` which INLINES the rung array at both call sites after running `check:knob-resolve-callers` showed a local variable had made the carrier unreadable to its own guard — 20 provable call sites had become 18). **A person's write carries no agent:** declaring `human` plus an agent is refused with a sentence (`23514`), in the database and again at the Python call site. The server half rides the ONE declaration that already carries tier and system — `provenance.declared_actor(tier, system, agent)` / `actor_session(..., agent=...)` emit `app.actor_agent`, always (empty when there is none, so a nested block can never inherit an outer agent), and the record store passes the running agent resolved once per turn by `provenance.executing_agent_id(ctx)` (a pinned run names a VERSION; the rung is the DEFINITION). **Screen honesty:** `pickableRungsFor` now offers a row-keyed rung only when some database reader NAMES it — the census `KNOB_RUNG_CONSUMERS` in `knobDatabaseConsumers.generated.ts`, measured live from `pg_proc` — so a rung nothing can answer is ABSENT rather than a box that saves a value nothing honours. `pnpm check:knob-database-consumers` gained that arm: it FAILS on any offered-but-unanswerable rung not recorded in `scripts/settings-guards/knob-rung-debt.json`, and fails just as loudly when a recorded one has been FIXED and left behind, so the list only shrinks (7 entries today: six HR sub-org rungs whose readers pass `null`, and `records.children.fan_out_ceiling`'s own `table` rung). Proven RED against a planted knob + reader in a rolled-back transaction (`--self-test`), and RED-then-GREEN in jest against the carrier's OWN historical bytes. Verified live over the real wire as admin@admin.com through the picker's door and PostgREST with `x-matrx-actor-agent`: no overrides → `unconfirmed`; the granted agent → **`confirmed`**; a sibling agent → `unconfirmed`; no agent declared → `unconfirmed`; the organization rung alone → `confirmed` (unchanged); a person claiming an agent → `400 23514` with the sentence. Both overrides cleared and all five probe records deleted — 0 override rows, 0 probe rows left.

- **2026-09-13 — The carrier asks `knob_resolve` with an ARRAY of scopes; `knob_resolve` refuses anything else (DD-198).** DD-183's picker made a latent class reachable: `platform.knob_resolve` dereferences `p_scopes` in exactly one place (`jsonb_array_elements`, inside the candidate-override EXISTS), and that line is reached only once an override row exists at a rung that is neither `organization` nor `user` — so `platform._stamp_actor_tier` (the provenance carrier on 577 tables) passing `jsonb_build_object('table', <id>)`, and `content_ir.edit_kind_instance_value` passing `'{}'::jsonb`, ran green until the day a person could create a table-rung override in one click. Reproduced live first, in rolled-back transactions on `content_ir.kind_instance` in `admin's Workspace`: an AI-tier INSERT with a table override present → `22023 cannot extract elements from an object`; the same with an agent-rung override on `agent_write_born_confirmed` (**it reached the raise because a candidate override row existed at a row-keyed rung — NOT because the agent rung governed the outcome**; before DD-211 nothing named the agent rung, so an agent-rung override on its own never made a row `confirmed`. V-64's correction, 2026-09-13); the same through the edit door with a table override on `confirm_on_human_edit`; and no error at all with no override, which is why it hid. `migrations/dd198_knob_resolve_refuses_a_non_array_scopes.sql` fixes all three call sites to `jsonb_build_array(jsonb_build_object('kind','table','id', …))` — the shape `platform._knob_override_write` already used — and puts the refusal at the TOP of `knob_resolve` (`22023`, a sentence naming what arrived, which knob, and the expression to pass instead), because a wrong argument must be wrong on the first call, not on the first call that happens to reach the line that dereferences it. The edit door derives its rung from the registry (`platform._confirmation_admission`) rather than a constant, and passes NULL when the table is not admitted. The census behind "all three": 15 catalog functions / 18 call sites (8 omit p_scopes, 7 pass null, 1 already correct, 3 were objects) plus every TS/Python call site in matrx-frontend, aidream, matrx-local and matrx-extend (all arrays or null). Standing guard `pnpm check:knob-resolve-callers` re-measures BOTH halves with one classifier (`scripts/knob-resolve-callers/core.ts`), proven RED against a planted catalog function in a rolled-back transaction (`--self-test`) and RED per shape in jest (8 cases); `migrations/` is deliberately not scanned — an applied file is a ledgered record of what already ran, and the SQL that runs is in the catalog. Proven live over the real wire as admin@admin.com through the picker's own door `knob_override_set`: table override on → an AI-tier INSERT lands `confirmation = confirmed`; override removed → the next one lands `unconfirmed`; both probe records deleted, 0 override rows and 0 probe records left. The migration's own assertions (all self-cleaning) additionally prove the org and user rungs unchanged, a sibling table's id resolving to unconfirmed, the edit door honouring a table-rung `confirm_on_human_edit=false`, the 577 carrier attachments preserved (DD-176), all three functions still SECURITY INVOKER, and `knob_resolve`'s anon/authenticated/service_role EXECUTE grants intact (DD-168).

- **2026-09-13 — A rung keyed by a ROW is now a place a person can stand: the per-rung override picker (DD-183).** `platform.feature_knob.overridable_by` opens eight `records.confirmation.*` keys at `table` and/or `agent`, the database honours them there on every write (`platform._stamp_actor_tier` resolves `confirmation.table_allows_born_confirmed` with the row's own table id), and the settings screen could reach exactly two rungs — the organization and the person. `features/settings/universal/KnobRungOverrides.tsx` is the exceptions disclosure under every key whose ladder allows a row-keyed rung: the standing overrides listed by LABEL (never a uuid), an "Add override for a <table|agent|sub-org>…" searchable picker fed by the ONE scope-rows door (`platform.knob_scope_rows` via `fetchScopeRows`), the value control, and removal through the same write door with a NULL value. Every editor is `KnobOverrideRow` mounted at the picked scope (settings-ladder rule 2, via its new `scopeLabel` prop, which also qualifies the row's DOM identity so twenty exceptions are twenty distinct controls). `UniversalSettingsContext` gained a per-knob, on-demand list of rung overrides — it held exactly one scope per section — read through `fetchKnobRungOverrides`, a LIST-only door over `platform.knob_override` under the caller's own RLS (`knob_index` answers "what is the value HERE" for rungs the caller addressed; "which tables have their own value" is a different question, and addressing 800 tables to discover three is not an answer). Honest states throughout: no rows says why, a refusal shows the door's own sentence, a picked-but-unvalued row says it is not saved, and a caller who may not write here gets no add control at all. **A DATABASE consumer is a consumer:** `disposition.ts` was a SOURCE census and could not see inside `pg_proc`, so a knob a trigger resolves could be labelled "Not connected yet"; the in-database census is now generated from `check:settings-orphans`'s own IN-DB tier (`pnpm generate:knob-database-consumers` → `knobDatabaseConsumers.generated.ts`, 121 keys) and overrides the hand audit in the one direction it can. Measured while doing it: of the eight `records.confirmation.*` keys, three are read by database functions and five have no reader anywhere — including `list_hides_unconfirmed`, the only one the map had flagged, so its evidence sentence now names the census that looked in both places. Verified live on `admin's Workspace`: the picker listed all 803 active registered tables by name, an override on "Tool Test Sample" saved through `knob_override_set` and survived a reload, and `knob_resolve` returned `true` for that table while another table and the organization rung stayed `false`; the row was then removed and resolution returned to `false`. 🚨 Found doing it, filed as a critical bug against aidream (feedback `6e7139ad-98b9-4a79-ab09-37274f3734ca`): `platform._stamp_actor_tier` passes `p_scopes` to `knob_resolve` as a jsonb OBJECT where an ARRAY is required, and `knob_resolve` only dereferences it once a non-org override row exists — so the FIRST table override on either born-confirmed key raises on every AI/code insert in that organization. Reproduced live with the exact call shape, then cleaned up.
...
- **2026-09-12 — The three first-screen settings are TRUE: each has a real runtime consumer and reads through the ladder.** `disposition.ts` had honestly hidden the controls for `agents.model_prefs.chat_default_model`, `agents.model_prefs.agent_authoring_default_model` and `media.listening.voice` because nothing read them ("Not connected yet"). Now: (1) **voice** — `features/audio/service/listeningConfig.ts` resolves `media.listening.voice` through `lib/scoped-config/sessionKnob.ts` (`platform.knob_resolve`, organization → user → device — the device rung now rides as a scope in `effectiveKnobs.ts`), every TTS path (`useCartesiaSpeaker`, `useCartesiaStreamingSpeaker`, the playback-queue Cartesia adapter via `getListeningSettings`) speaks the knob's voice, and `useListeningSettings` writes the user rung through `setKnobOverride` (which now invalidates the ladder cache so the same tab hears the change). Speed and language still live in the `ui.ui_surface_config` `listening` namespace — they have no knob row; a person's old namespace voice is carried to their user rung once, announced, and cleared from the row, so the rival store no longer holds a voice. (2) **chat default model** — `features/ai-models/preferredChatModel.ts`; the launch orchestrator seeds the basic-chat door's (`chat.default_new_chat`) instance model override from the knob when the caller named no model, so the chat picker shows it and the turn's request carries `config_overrides.model`; `userPreferences.prompts.defaultModel` is retired (its only picker, in the unmounted `StandalonePromptsPreferences`, is now a door to the universal setting; the persisted key stays null until a migration drops it). (3) **agent-authoring model** — `features/ai-models/preferredAuthoringModel.ts` rides as the Agent Generator run's explicit model, and aidream's Agent Factory (`agent_factory/builder.py::_run_meta_agent`) reads the same knob via `scoped_knob_raw` for the ambient principal. The three left `AUDITED`; the map is hand-kept and should be derived from `pnpm check:settings-orphans` (a follow-up). Also: `UniversalSettingsProvider` registers the platform `settings_changed` directive handler (invalidate register + ladder cache, refetch) so another tab's instant change re-renders the pane without a reload; `ModelListDropdown` now honours `aria-label` (the hyphenated prop had been silently dropped). Verified live: voice write → `knob_override_set` and the Cartesia socket frame naming the chosen voice id; model write → the next `/v2/ai/mandates/chat.default_new_chat` body carrying `config_overrides.model` = the chosen id (evidence `z-*` in the lane scratchpad).
...
- **2026-09-12 — The universal configuration surface is MOUNTED, and a sub-organization rung is a place a person can actually stand.** `features/settings/universal/` had landed on 2026-09-11 with nothing importing it: the `/user-settings` rail read `getTabTreeNodes` directly, so the taxonomy-driven "Configuration" sections and the first screen were unreachable code, and `check:settings-ladder-ui` graded a surface no human could open. `SettingsRouteShell` now mounts `UniversalSettingsProvider` above BOTH the rail and the active tab (they are separate trees and must not disagree), the rail and `SettingsTabContentImpl` both resolve through `useSettingsTree`, and the route's index renders `FIRST_SCREEN_TAB` instead of an empty "choose a category" panel. Three defects found and fixed on the live surface behind that: (1) the first screen called `UniversalSettingsRows` three times, so its "AI" group heading printed twice with "Voice" between them — one call, one list, grouping happens once; (2) every sub-organization rung picker was permanently empty because `scopeRows.ts` read `hr`/`web` tables directly and PostgREST answers "Invalid schema: hr" — the rows now come from ONE door, `platform.knob_scope_rows` (aidream migration 0639), driven by `knob_scope_kind`'s own `scope_schema`/`scope_table`; (3) `KnobOverrideRow` knew only `user_override`/`org_override`, so a pay group's or a location's own value read as "Inherited from your organization" and its Inherit button followed the organization's row — it now takes the resolver's `KnobLadder` and the scope chain decides origin, fallback and whether there is anything here to clear. `formatKnobValue` in `lib/scoped-config/ladder.ts` is THE ONE value+unit formatter (a unit is a word: "15 minutes", never "15minutes"; `%` and `°` stay glued). Verified live at the location rung on `admin's Workspace`: `hr.time_and_attendance.approaching_ot_buffer_minutes` set to 25 at "G2V Long Beach Yard" ("Set here"), then cleared back to "Inherited from the platform" — `platform.knob_override_audit` shows `set` then `clear` at `scope_kind=location`. `pnpm check:settings-ladder-ui` 196 keys / 584 findings → 0 (`platform.spend_popover` filed by aidream migration 0638).
  **Still missing from this surface (lost with an interrupted session, not built here):** the rich field renderer the ladder was designed for — `KnobLadder.control` names `slider`, `model`, `voice`, `secret`, `segmented`, and nothing consumes it, so a model key renders as a free-text box and a voice key shows a raw UUID, with no pick-and-hear preview. `resolveKnobLadder`'s `originLabel`, `cannotWriteBecause` and the locked-key request door are likewise computed and unrendered.
...
- `2026-08-31` — **Integrations completed the mobile surface-check repair.** The GitHub account card now stacks its identity and actions instead of squeezing the account name into a one-character column; MCP, GitHub, Google, filter, detail, credential, refresh, mobile drawer, and breadcrumb controls keep 44pt phone targets while retaining dense desktop sizes. Credential/project inputs stay above the iOS zoom floor, touch copy controls remain visible, and GitHub/MCP disconnects name the access loss before removing the connection. Route-shell navigation now runs through a pending transition with per-destination feedback, dynamically split settings content announces contextual loading instead of a blank screen or bare spinner, and the integrations tab contributes a sanitized catalog/filter/connection snapshot to the canonical settings surface without exposing credentials or internal endpoints. The route shell now mounts the universal v3 context menu once around the active settings pane, restoring desktop right-click and mobile long-press with the live settings scope. Added focused layout/runtime-registry regressions and a distinct `/user-settings` favicon identity.
...
- `2026-08-24` — **General → Notifications is now the canonical Notification System preferences tab.** `tabs/NotificationsTab.tsx` + `notification-preferences.ts` render every registered platform event (`communication.notification_event_type`) with per-channel switches; absence of a `communication.notification_preference` row means the event's declared default applies (owner ruling: CMS form submissions default to email). Direct-to-Supabase reads/writes under entity RLS, org = personal org via `resolvePersonalOrgId()`, upsert on `(user_id,event_key,channel)`. The registry entry `general.notifications` no longer aliases MessagingTab. Server half + event registry: aidream `aidream/services/notifications/`; cross-repo truth `../common-docs/projects/notification-system/HANDOFF.md`. Verified live: toggle off/on wrote and re-read the row (version bump observed), default state renders with no row.
...
- `2026-08-06` — Added the Google Workspace child tab under Integrations. It thinly renders the same focused OAuth reviewer workspace used by `/google-workspace-review`; no parallel connection flow was introduced.
...
- `2026-07-13` — **Shape-drift backfill system (no more console-flooding "self-heals on next save").**
  ...
  check reports drift for all users at `/administration/database/data-integrity`. New invariant above codifies
...
- `2026-07-10` — **Tutor preferences module.** Added the `tutor` module to `userPreferences`
  ...
  `useSetting("userPreferences.tutor.*")`; the tutor vocabulary + a one-time localStorage migration
  live in `features/education/tutor/settings.ts`. No new tab (edited from the inline tutor panel).
