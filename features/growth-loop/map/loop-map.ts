...
export const GAPS: LoopGap[] = [
  ...
  },
  {
    ...
    breaks: ["ai"],
    detail:
      "HUMAN AND CODE PIPES CLOSED 2026-08-13; AI remains. THE ROOT CAUSE, for the record: 3,209 lines landed on 2026-08-11 (aidream 7505afdf6, the stale-machine rescue) and were dead for two days because app.py mounts 138 routers and growth_loop was the one never added. Routes supplied (13 at /api/growth-loop/*, authed), and a real UI drives them: a site owner starts the loop, sees its stage and blocker, and continues/skips/pauses it at /marketing/brands/[brandId]/sites/[siteId]/growth-loop. CODE closed the same day — stage services now record themselves with no human click. aidream/services/growth_loop/stages.py (track_loop_stage + live_loop_for_site / live_loop_for_research_topic) is called from run_pipeline (research), run_generate_plan when apply=true (plan) and draft_node_brief (brief); chat.agent_run became a stage store through ONE row in growth.stage_ref_kind (migration 0345), never a new column. Verified live end to end: loop 1866b32e-f6ba-4fc7-8a6e-bf3eb1735659 (PBW Law Website) walked research -> plan -> brief across 3 loop_stage_run rows and 7 loop_event rows, seq 1-7, each attempt holding only a pointer (research.rs_topic, then chat.agent_run twice) and pipe=ai against a requested human_then_ai. REMAINING: the nine stages after brief (realize -> write-back) still call nothing, and no agent DRIVES a stage — the pipe primitive is not the blocker (G-PIPE-SELECTOR closed 2026-08-13), the missing piece is a supervisor handing a stage to a pipe.step run (G-SUPERVISOR).",
    lane: "L6",
