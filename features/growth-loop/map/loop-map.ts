...
export const GAPS: LoopGap[] = [
  ...
  },
  {
    ...
    breaks: [],
    detail:
      "CLOSED 2026-08-13 — the durable loop run now advances end to end without requiring a DAG to remain attached between stages. Existing services self-record their own stage attempts; the supervisor judges completion only from each attempt's registered (ref_kind, ref_id) store, then opens the next stage and hands it to pipe.step under the loop's stored pipe policy. wf_run_id remains nullable and is detached again after each workflow finishes. Live autonomous proof: loop 89515714-29ae-4ade-b6bc-0a20d1b8d92c advanced serve → crawl from workflow.run 4d86bcea-9c96-47c3-9473-41a5762daad9 with no human stage action; loop events 4-10 show dispatch, the reasoned advance, stage completion, next-stage entry, and next dispatch.",
    lane: "L6",
  ...
  },
  {
    ...
    breaks: [],
    detail:
      "CLOSED 2026-08-13 — the purpose-built growth_loop.supervisor slot uses the existing agent_sets supervisor primitive; no second supervisor system was created. A live 30-second scheduler tick locks each eligible loop, reads its open attempt and authoritative stage-store pointer, records every wait/dispatch/advance/fail judgment as supervisor_decision with reason and evidence, and dispatches the next stage through a one-node pipe.step workflow. Policy remains sovereign: realize/serve/crawl/measure use CODE, while the default stays human-first with AI fallback at exactly 3600 seconds. Live proof loop 89515714-29ae-4ade-b6bc-0a20d1b8d92c event 5 advanced serve from the pointed completed workflow.run, event 7 completed it, and events 8-10 opened and dispatched crawl without human action. HEALTHY as of 2026-08-14 22:12: the 19-hour failure loop is fixed and verified live. It had run 1,919 consecutive failed ticks (89% of 2,157 runs, 02:51-21:59), all on ONE loop (26560438-08ff-40dc-ae58-b37de3d8d7d7), with 'InterfaceError: cannot perform operation: another operation is in progress'. Root cause: the quality judge was spawned with asyncio.create_task from inside complete_stage, which runs inside the supervisor's open transaction — the child inherited matrx-orm's _active_connection ContextVar and ran its first query on the supervisor's own pinned asyncpg connection, killing the tick at COMMIT and rolling the completion back, forever. It now spawns via matrx_utils.detached_task (own Context, own pool connection) and waits for the completion to be visibly committed before scoring. Second fix, because no loop may fail in silence: per-loop supervisor health in loop_run.metadata.supervisor_health defers a failing loop 60s x 2^(n-1) up to 30 min and, at 5 consecutive failures, quarantines it — loop parked blocked with the reason, a supervisor_quarantined event carrying a resume hint, log.critical, and the loop named in every later tick; control_loop(id, 'resume') is the way back. Reaching quarantine now costs 5 ticks, not 1,919. Verified live in aidream v0.2.76: the last InterfaceError was 21:59:07, the first post-deploy tick advanced loop 26560438 measure -> analyze (events 192-193), and the following ticks ran clean (advanced=1 then waiting=2, deferred=0 quarantined=0 errors=0) — 0 failures where the rate had been 89%.",
    lane: "L6",
    evidence: "aidream/aidream/services/growth_loop/supervisor.py",
  },
  {
