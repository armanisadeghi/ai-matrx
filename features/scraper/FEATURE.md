...
- 🚨 **These endpoints require a SIGNED-IN user, not a guest.** aidream mounts all of `/scraper` under `Depends(require_authenticated)`, so a fingerprint-only caller gets `401 token_required` — either header spelling. The public `/seo` analyzers call quick-scrape and therefore dead-end for anonymous visitors (**FOUND_DEFECTS D137**, pre-dates the proxy deletion). Guest-capable SEO work belongs on aidream's `/seo/public` router (`require_guest_or_above`). Do not "fix" this from the frontend.
