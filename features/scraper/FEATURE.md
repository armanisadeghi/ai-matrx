...
- Agent analysis tabs (full scrape): `parts/agent-analysis/*` — Fact Checker (`scraper.fact_check`) and Keyword Analysis (`scraper.keyword_analysis`) are MANDATES (`constants/analysis-agents.ts`), resolved per tab with `useMandate` and run through the CANONICAL execution path — `useLiveAgentRun` → `launchAgentExecution({ mandateKey })` with `expect: "text"` (binding `configOverrides` ride the run); the page text goes as the `scraper.page_analysis` provision's `page_content` variable. 🚨 **Neither tab renders a byte of the stream itself** — output goes through `<LiveRunDisplay>` → `MarkdownStream` → the kind registry, so a `__kind` answer lands on its kind component and reasoning never leaks as text. Section/table parsing runs over the SETTLED answer only. An unresolved mandate renders `AnalysisMandateGate` (picker + `/mandates?feature=scraper` door) — never a hardcoded agent.
- Display: scraped pages use `parts/ScrapedResultDetailTabs.tsx`, `parts/core/*`,
  and `parts/tabs/*`. Keyword-search rows are adapted once by
  `utils/search-kind-adapter.ts` and rendered through
  `KindInstanceRender kind="web_search_results"`, the same canonical kind path
  used by `/search`. The compact hit list remains the selection control; the
  selected URL's Open and Scrape actions remain in the window toolbar. Do not
  restore scraper-owned search result cards or a separate selected-hit detail
  renderer.
- **No Next.js proxy.** `useScraperApi()` is the ONE client path to these endpoints, browser → the standalone scraper directly. It selects the `scraper` service through `useBackendApi("scraper")` / `selectResolvedServiceBaseUrl` and mounts the canonical `ENDPOINTS.scraper.*` paths below the service's `/api` prefix. The old `app/api/scraper/content/route.ts` relay was deleted 2026-08-09. Never reintroduce that route or send this panel through the general aidream service.
- 🚨 **These endpoints require a SIGNED-IN user, not a guest.** The standalone `/api/scraper` routes require authenticated context, so a fingerprint-only caller is refused. **Never call a scraper endpoint from an anonymous surface.** Guest-capable SEO work belongs on aidream's `/seo/public` router (`require_guest_or_above`), not on a frontend workaround.
...
**Endpoints (standalone scraper service, canonical paths declared in `lib/api/endpoints.ts` and mounted below `/api`)**
...
- Streaming guide: `/Users/armanisadeghi/code/common-docs/systems/knowledge/research/STREAMING_GUIDE.md`.
...
Every pipeline starts with an authenticated `POST` (Supabase JWT via `useBackendApi` / `useApiAuth`) to a Python-backend endpoint. Scraper selects the standalone `scraper` service and mounts `ENDPOINTS.scraper.*` below `/api`; PDF stays on the aidream service through `ENDPOINTS.pdf.*`; research goes through `research-endpoints.ts`; transcripts uploads audio to `cld_files` via the universal file handler first, then calls the transcription endpoint with the cld_files UUID.
...
- **Scraper `success: false` rows in a 200 stream.** `useScraperApi` checks `isRawScrapeRowFailed` on each result row — a failed URL in a batch surfaces via `errorDiagnostics` and the shared Error Inspector's `scraper` source, not via the HTTP status. HTTP/network and typed stream errors retain their existing single capture paths. Do not assume 200 means "all URLs succeeded".
- **One scraper transport owner.** Every route, panel, resource picker, podcast resolver, education ingest flow, and demo imports `useScraperApi`; none selects a scraper origin or transports `ENDPOINTS.scraper.*` itself. `pnpm check:scraper-routing` blocks a second service owner, direct endpoint transport, or recreated `app/api/scraper/*` proxy.
...
- `2026-09-17` — 🚨 **`/scraper/quick` stopped being an engineer's console, and a result now says WHICH ENGINE produced it.** Three defects found live on `https://aimatrx.com/scraper/quick` signed in as `admin@admin.com`. (1) **The Full Scrape button looked dead.** It did fire its POST — the deadness was that nothing on the screen moved: the banner read `{error || fullScrapeApi.error}`, so the *quick* lane's stale message stayed on screen after the *full* lane ran and failed too, and the body rendered nothing at all (`viewMode === "full"` with no `fullResult` and not loading matched no branch). The page now picks the failure belonging to the lane whose view is on screen (`activeFailure`) and always renders a body for the full lane — a failure notice, or an honest "returned no page content" empty state. (2) **The raw Diagnostics JSON, stack and endpoint names were the primary body** of a failure on a signed-in user's page. The `(core)/scraper/*` pages were exempted on 2026-09-12 as "the support audience"; that exemption is REVOKED for `/scraper/quick`, which a non-technical person uses. It renders the shared `parts/ScrapeFailureNotice.tsx` (`size="page"`), and the disclosure — renamed "Details" → **"Technical details"** everywhere — is now gated by `detailsAllowed`, which the page passes from `selectIsAdmin`. Nothing is swallowed: `captureScraperError` still files every failure. (3) **Scrape provenance.** The backend now stamps every row with `engine` (`http` | `browser` | `cache`), `escalated`, and `escalation_reason`; `ScraperResult` carries them as `engine` / `escalated` / `escalationReason`, **null when the backend did not say**, and the new `parts/ScrapeProvenance.tsx` shows a calm badge plus one plain sentence for an escalation (`cloudflare_block` → "The normal fetch was blocked (Cloudflare), so we rendered the page in our server browser."). Older responses carry none of these fields and render NOTHING rather than a fabricated default. `classifyScrapeFailure` learned the two new `failure_reason` values — `empty_content` → `empty`, `wrong_entity` → the new **`wrong_page`** kind — and the `site_refused` / `blocked` titles lost their nested parentheses. Guards, both proven failing-then-passing: `features/scraper/parts/__tests__/ScrapeProvenance.test.tsx` (fails the moment the component defaults a missing engine to `http`) and the `wrong_entity` case in `features/scraper/failure/__tests__/scrapeFailure.test.ts`.
- `2026-09-12` — 🚨 **A scrape failure now has two faces: plain words for the person, diagnostics for support (W44).** A non-technical Expert adding `https://georgekao.substack.com/p/...` to a Rulebook was shown `"…: bad_status — failed at useScraperApi.scrapeUrl → validate_result_success (see errorDiagnostics)"`, a JavaScript stack, and a "Diagnostics JSON" block as the PRIMARY body of the picker. The loudness was right and the audience was wrong. `useScraperApi` now returns **`failure: ScrapeFailure | null`** derived on render in ONE place (`features/scraper/failure/scrapeFailure.ts`), so every operation and every failure kind — `site_refused`, `blocked`, `timeout`, `empty`, `bad_address`, `unknown` — carries a plain `title`, a plain `remedy`, the target's HTTP status when known, and the engineer string + diagnostics kept alongside for support. `error` stays the ENGINEER string (it names hook/operation/stage): a surface a person looks at renders `failure` through the shared **`parts/ScrapeFailureNotice.tsx`**, never `error`. The notice puts the words and a pressable remedy first and the stage/stack/Diagnostics JSON behind a collapsed "Details". Nothing is swallowed — `captureScraperError` still files every failure. Consumer census: only `WebpageResourcePicker` rendered `error` at a non-technical person (now converted, with an in-place "Paste the text instead" lane); `features/education/onboard/useIngest.ts` and `features/podcasts/generator/useSourceResolvers.ts` already wrote their own plain messages; the `(core)/scraper/*` and `(dev)/demos/scraper/*` pages are the support audience and keep the diagnostics view. Guards: `features/scraper/failure/__tests__/scrapeFailure.test.ts` (every kind gets plain words, no developer token ever reaches `title`/`remedy`) and `features/resource-manager/resource-picker/__tests__/webpage-picker-failure-plain-words.test.tsx` (renders the picker's live `bad_status` state — proven failing against the old picker, passing after).
...
- `2026-08-27` — Web Scraper keyword results now use the exact canonical
  ...
  collection, including `__kind: "web_result"` on every nested item; deleted
  the duplicate scraper result-card and selected-hit-detail renderers. The
  compact selection list and its Open/Scrape behavior are preserved, with
  Scrape promoted to the existing window toolbar. Adapter test and
  `pnpm type-check` pass. Live provider-result proof remains blocked because
  both deployed search runtimes currently lack
  `BRAVE_SEARCH_API_KEY_PRO_AI`; the shared Error Inspector captures the
  resulting typed `search_error`.
- `2026-08-27` — Audited every executable scraper consumer: all live callers converge on `useScraperApi`; the separate Marketing crawler already targets its own standalone `/api/scraper/crawler/*` contract and the anonymous SEO metadata path correctly stays on aidream `/seo/public`. Deleted stale copy-paste examples that sent scraper paths to `BACKEND_URLS.production` and advertised an unsupported guest call. Added the blocking `check:scraper-routing` release gate so a direct transport, second scraper service owner, or Next.js scraper proxy cannot return.
- `2026-08-27` — `useScraperApi` now targets the configured standalone scraper service through `useBackendApi("scraper")`, adds the service's `/api` mount prefix once, and captures successful-HTTP payload/normalization failures in the shared Error Inspector without duplicating HTTP/network or typed stream captures. Live verification: `https://matrix.ai/` returned its full page content through Web Scraper, while a controlled refused target produced one red `Web scraper failure` row containing `failure_reason`, endpoint, URL, stage, stack, HTTP status, full stream, and raw evidence.

- `2026-08-22` — 🚨 **Both AI analysis tabs converted to the ONE streaming pipeline (streaming-law violation fixed).** `KeywordAnalysis.tsx` and `FactChecker.tsx` hand-rendered the raw agent stream (`useRunAgent` + `onChunk` accumulation → `MarkdownRenderer`/`MarkdownStream` fed a hand-built string). Two live consequences on `/scraper`: the keyword agent's structured `{"__kind":"keyword_variant_set"}` answer rendered as a raw JSON code block even though the kind is registered and active, and the model's chain-of-thought leaked into the panel as literal `<reasoning>…</reasoning>` text. Both tabs now run through the canonical execution system (`useLiveAgentRun` → `launchAgentExecution({ mandateKey })`, `expect: "text"`, `initiation: "auto"`) and render through `<LiveRunDisplay>` → `MarkdownStream` → the kind registry; `hideReasoning` is the primitive's default, so the thinking is consumed by the pipeline instead of printed. Fact Check's section tabs (Summary / Observations / Claims / Concerns / Recommendations / Claims Table) and Keyword's Content Comparison table now parse the **settled** answer text only — never the live stream — and while a run is in flight every tab shows the live output instead of a spinner. **Deleted `hooks/useScraperAgentAnalysis.ts`** (the hand-rendering twin; no other importers). Mandate wiring, `AnalysisMandateGate`, and the `page_content` variable are untouched. The auto-run effect keys on `[mandateReady, value]` and reaches the launcher through a ref — `useLiveAgentRun`'s `run` is a fresh closure each render and would re-fire forever as a dep. **GOTCHA found while verifying live:** the canonical path REFUSES to launch with no active organization (`instances/launch` rejects with "Select an organization before sending this message"), where the old `useRunAgent` path did not — a fresh dev-login session must pick an org before either tab can run. Live-verified on `https://en.wikipedia.org/wiki/Composting`: keyword result renders as the `keyword_variant_set` kind component (primary keyword, 6 selectable keywords, Copy for AI, Add to Page, strategy note), fact check streams live then sections up, zero `<reasoning>` tags and zero `__kind` text in the DOM, and the unbound-mandate gate still renders (probed with a bogus mandate key). **Known gap, NOT fixed here:** Keyword's _Content Comparison_ tab now states plainly that the analysis returned no comparison table — the bound agent answers with structured keywords, not a markdown table, so the tab has nothing to parse. Whether that tab should exist is Arman's call.

- `2026-08-22` — **Analysis tabs resolve Mandates (ROLLOUT F8).** `SCRAPER_ANALYSIS_AGENTS` (two agent ids that no longer existed in the DB + two legacy variable UUIDs) → `SCRAPER_ANALYSIS_MANDATES` (`scraper.fact_check` / `scraper.keyword_analysis`) + `SCRAPER_ANALYSIS_CONTENT_VARIABLE = "content"`. Both mandates are seedless today (no live row) — the tabs gate on `useMandate` and render `AnalysisMandateGate` until an admin binds a Holder.
