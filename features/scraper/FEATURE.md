...
- `2026-08-27` — `useScraperApi` now targets the configured standalone scraper service through `useBackendApi("scraper")`, adds the service's `/api` mount prefix once, and captures successful-HTTP payload/normalization failures in the shared Error Inspector without duplicating HTTP/network or typed stream captures.
