...
- Agent analysis tabs (full scrape): `useScraperAgentAnalysis({ mandateKey, variables })` + `parts/agent-analysis/*` — Fact Checker (`scraper.fact_check`) and Keyword Analysis (`scraper.keyword_analysis`) are MANDATES (`constants/analysis-agents.ts`), resolved per tab with `useMandate` and run via `resolveMandate` → `useRunAgent` (binding `configOverrides` ride the run); the page text goes as the `scraper.page_analysis` provision's `content` variable. An unresolved mandate renders `AnalysisMandateGate` (picker + `/agents/mandates?feature=scraper` door) — never a hardcoded agent.
