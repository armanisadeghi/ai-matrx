...
- `2026-08-23` — Codex: added `SmartCodeEditor.surfaceOwnership`. The default `self` preserves the editor's own surface and write handlers. Embedded hosts may pass `host` only when they publish the live draft through their own surface scope; this prevents the nested editor from replacing the host's identity and agent roster in global page chrome. Kind Registry's component Code tab is the first host-mode adopter.
