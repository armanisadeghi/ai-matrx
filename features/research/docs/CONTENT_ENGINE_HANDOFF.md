...
| **Agent Copy & Update fix** | `agx_duplicate_version(p_version_id, p_as_system)` RPC — forks the PINNED `agx_version` the server runs, not the (drift/corruption-prone) master; FE reads pinned version_ids from the agent-mandate registry (`useResearchAgentRoles`; the hardcoded `SYSTEM_AGENT_VERSION_UUIDS` map was retired 2026-08-08); real-error surfacing | ✅ copy matches version md5 exactly |
...
- Version-dup: `migrations/agx_duplicate_version.sql` · FE pins: DB-truth via `features/research/components/agents/useResearchAgentRoles.ts` (mandate registry) · aidream source of truth: `research/agents.py` (`declare_pinned_agent`)
