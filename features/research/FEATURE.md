...
- `/research/topics/[topicId]` — live-run overview (the orchestra). Sub-routes: `sources`, `sources/[sourceId]`, `content`, `keywords`, `analysis`, `synthesis`, `document`, `documents`, `tags`, `tags/[tagId]`, `media`, `costs`, `settings`, `agents`, `tasks`.
...
- **Analysis view** orders completed-with-content first by `rs_source.rank` (best Google result first), and separates empty/failed into a bordered section. Counts distinguish with-content vs empty vs failed — never "N passed" for N non-failed rows.
