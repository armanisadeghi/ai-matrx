...
- **2026-08-28 (HR SMS contact graph)** — `work_phone` and `personal_phone` now feed the
  ...
  unverified numbers. The self-service field note now directs employees to Messaging settings
  for confirmation instead of saying the HR number is never used.
