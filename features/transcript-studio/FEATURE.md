...
**Hooks**
- `hooks/useStudioAssistant.ts` — creates one assistant conversation per session (kept in the slice), ensures the working document, and rebuilds the named context objects (`recording_NN_raw` / `session_cleaned` / `cleaned_transcripts` / `working_document`) before each turn. Mounted at the `ScribeScreen` level (not just the assistant tab) so a cleanup run that finishes on the capture screen still refreshes assistant context.
