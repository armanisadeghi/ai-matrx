...
**Hooks**
- `hooks/useStudioAssistant.ts` — ensures the working document and resolves the session's **durable** assistant conversation via `ensureAssistantConversationThunk`, then rebuilds the named context objects (`recording_NN_raw` / `session_cleaned` / `cleaned_transcripts` / `working_document`) before each turn. Mounted at the `ScribeScreen` level (not just the assistant tab) so a cleanup run that finishes on the capture screen still refreshes assistant context.
...
- `service/assistantContextBuilder.ts` — builds the `setContextEntries` payload, including the mutable+persisted `working_document` rich context object (`source.kind = "studio_document"`) and the read-only `cleaned_transcripts` entry sourced from the `scribe_cleanup` studio_documents row (explicitly labeled as a DUPLICATE of the raw recordings so the model doesn't double-count).
- `components/scribe/CleanupSheet.tsx` — drawer reached from the session ⋮ menu ("Clean up transcripts"). Reuses `AI_POST_PROCESS_AGENTS` and `useAiPostProcess` from `components/official-candidate/transcription-cleanup/` (custom user agents from prefs are merged in just like the standalone window panel). Defaults to `AI_POST_PROCESS_AGENTS[0]`. Joins every active recording's raw segments into `# Recording NN`-fenced input, runs the agent, strips `<thinking>`, then upserts to `studio_documents` (`kind="scribe_cleanup"`) via `upsertScribeCleanupDocumentThunk` → realtime echoes it back → `refreshContext()` so the assistant sees it on its next turn.
