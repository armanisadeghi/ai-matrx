...
Complete and live-verified on web (settings CRUD, cleanup card, selector window, DB RPCs, tool drift, seeded agents/skills). Compact indicator mounted on transcript studio; the same component drops onto scribe/podcast surfaces with a one-line addition. **Prod re-verify 2026-07-13:** cache-bust run; the Dictionary Assistant calls the `dictionary` tool and writes `dict_entries` end-to-end on prod (the agent had NO tool assigned — fixed via agent_author + cache invalidate). Server auto-injection was broken in prod (`dict_resolve_for` called with jsonb instead of `uuid[]` — `ArrayArg` fix in aidream `dictionary_inject.py`) and the agent's skills never loaded (`SkillBody.version` typed `str`, DB is integer — fixed in matrx-ai); both fixes verified against the live DB locally, **prod effect lands with the next aidream deploy** (then re-verify: injected block visible to a model on `matrx-user/transcripts-cleanup`). Podcast carry-through: payload plumbing + substitution floor verified; audible confirmation needs a human listen.
...
- 2026-09-17 — `activeContextBridge` builds its selection from the EXPLICIT active organization instead of the legacy `selectEffectiveOrganizationId` (`organization_id ?? personal_organization_id`). With none selected the selection simply carries no organization and `includePersonal` still resolves the user's own entries — a narrower dictionary, never another workspace's.

- `2026-08-22` — **Dictionary Assistant runs through its Mandate.** `DICTIONARY_AGENT_IDS.assistant` (raw UUID, ROLLOUT F15) deleted; `useOpenDictionaryAssistant` launches `dictionary.workspace_guide` via `launchMandate` and exposes `unavailable`/`mandateError` so the "Ask assistant" button disables with the reason while unresolved.
- `2026-08-15` — **agent-copy wired across the dictionary surfaces.** The feature had ZERO Copy / Copy-for-AI coverage; every term the user could read was uncopyable. Added `features/dictionary/format.ts` — the ONE set of human summaries + agent projections (`dictEntrySummary`/`dictEntryData`/`dictEntryKeyFields`, `dictEntriesSummary`, `dictOwnerSummary`/`dictOwnerData`, `resolvedDictionarySummary`), deliberately separate from `utils/format.ts`, which renders the deterministic TTS/LLM consumption shapes and is prompt-cache-prefix sensitive — that file feeds MODELS, the new one feeds the CLIPBOARD, and merging them would put UI concerns inside a cache-stable renderer. Wired: `DictionaryManager` (per-row hover `xs` pair + list `sm` pair + `ExportMenu`), `DictionaryContextCard` (resolved-dictionary record pair + per-term pairs + export), `DictionarySelectorWindow` (tier catalogue pair + export + per-tier pairs). Sized to data per the doctrine: the entry list is MEDIUM, so its default click is the **search-narrowed view the user is actually looking at** (`agentVariant` "This view", first) with `keyFieldsAiVariant` (reused from `features/marketing/lib/copy-payloads.ts`, not forked) and a named "All terms" variant as the full dump — a list payload that silently reports only the visible rows would lie about the dictionary's size, so every summary carries `Terms shown` AND `Terms total`. The Advanced section's existing Export CSV / Export JSON buttons are KEPT (never delete a working download affordance); the new `ExportMenu` is additive and distinguishes this-view from all-terms, which the old buttons could not. Two real defects fixed while wiring: `ExportMenu` keys on `item.id`, so the view/all JSON+CSV pairs needed distinct ids (duplicate React keys otherwise), and the tier rows are `<label>` elements — a `<label>` activates its control on ANY descendant click and `stopPropagation` alone does not cancel that default, so copying a tier would have silently toggled it in or out of the merge; the copy pair is wrapped in a `preventDefault` span. `pnpm type-check` clean.
...
- `2026-06-17` — **Multi-provider voice dictionary.** Per-task `customEntries` (DictSelection/DictConsumption + `buildConsumption` fold + selector "Add for this task" UI + payload `custom_entries`). aidream: universal `apply_tts_dictionary` substitution floor wired into all 5 backend TTS providers; two `tts_quality` modes per provider (`QUALITY_TIERS` + deprecation guards) plumbed through LLMParams→UnifiedConfig→provider and the podcast request; per-provider research at `aidream/docs/dictionary/providers/`. Fixed the ElevenLabs no-op: `text_to_dialogue` ignores native locators → substitution applies pronunciation instead. Google TTS = `gemini-3.1-flash-tts-preview` for both modes.
- `2026-06-17` — **Dictionary follows the ONE global active context.** Ambient audio — all Cartesia read-aloud playback **and** recording STT bias — now resolves its dictionary from the single global `appContext` (active org + scope-types + scopes, + personal + global), via `features/dictionary/activeContextBridge.ts` (`buildDictSelectionFromContext` → `resolveActiveContextTtsAliases` / `resolveActiveContextSttPrompt`). Flip context anywhere → the next utterance uses that context's dictionary. This is the default for the Cartesia hooks (`useCartesiaSpeaker`/`useCartesiaStreamingSpeaker`), the unified playback-queue `cartesiaAdapter`, and `useAudioTranscription`; an explicit `dictionarySurfaceKey` only scopes a surface to its own selection (Scribe keeps `SCRIBE_DICTIONARY_SURFACE`). Superseded the earlier fixed per-surface `READ_ALOUD` default (removed).
- `2026-06-17` — **Read-aloud playback never cuts off.** The "Speak" button (`StreamingSpeakerButton`) now enqueues onto the persistent module-singleton `playbackQueue` (via `useTtsSpeak`) instead of owning a WebPlayer in a React hook — so audio survives tab/route changes and new utterances queue behind the current one. Deleted the component-bound `StreamingSpeakerLive` (its unmount cleanup was the cut-off). See [[project_custom_dictionary]].
