...
- `2026-06-17` — **Read-aloud playback now applies the dictionary.** Wired `dictionarySurfaceKey` into every Cartesia read-aloud consumer (chat + cx-chat message read-aloud, global `showAudioModal`/flashcard audio via `SpeakerGroupCore`, transcript Scribe voice-out + "Read aloud"). New constants `READ_ALOUD_DICTIONARY_SURFACE` / `SCRIBE_DICTIONARY_SURFACE` in `constants.ts`. Closes the last unwired TTS surface — on-screen text playback now speaks personal pronunciations by default.
