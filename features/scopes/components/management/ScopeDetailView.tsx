...
import { AlertTriangle, Building, ExternalLink, Network } from "lucide-react";
...
export function ScopeDetailView({ scopeId }: ScopeDetailViewProps) {
  ...
  if (status === "loading" && organizations.length === 0) {
    return (
      <>
        <RouteHeader
          left={<ChevronLeftTapButton href="/scopes" ariaLabel="All scopes" />}
        />
        <div className="h-full overflow-y-auto bg-textured pt-[var(--shell-header-h)]">
          <div className="max-w-5xl mx-auto p-6 md:p-8 space-y-3">
          ...
          <div className="max-w-5xl mx-auto p-6 md:p-8">
            <Card className="p-6 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                This scope may have been deleted, or you may not have
            ...
            <Card className="overflow-hidden">
              {valuesStatus === "loading" ? (
                ...
                <div className="px-4 py-6 text-center text-xs text-muted-foreground">
                  ...
                  <span className="font-medium">{type.label_plural}</span>{" "}
                  yet. Use the legacy editor to add some.
                </div>
              ) : (
