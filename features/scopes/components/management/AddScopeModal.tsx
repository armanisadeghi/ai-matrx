...
// features/scopes/components/management/AddScopeModal.tsx
//
// Canonical "add a scope type" panel (Lane F W8 rebuild of the legacy
// features/scope-system AddScopeModal). Reads the org's existing types from
// the canonical tree (makeSelectScopeTypesForOrg) and writes only through
// the sanctioned RPC-backed thunks (createScopeType + createContextItem),
// which fold the authoritative rows straight into the tree — no refetch.
...
export function AddScopeModal({
  ...
  const existingTypes = useAppSelector((s) => selectScopeTypesForOrg(s, orgId));
