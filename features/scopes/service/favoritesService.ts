...
//
// Typed-RPC note: the `ues_*` functions are deployed + verified live but are
...
// names, so calls go through `rpc()` below — the same single, isolated,
// minimally-typed view of `supabase.rpc` that `associationsService` uses. When
// the types regenerate, delete `rpc()` and call `supabase.rpc("ues_…", …)`
// directly; signatures here stay constant.
...
export const favoritesService = {
  ...
  async setState(args: {
    ...
    try {
      requireUserId();
      const { error } = await rpc<null>("ues_set", {
  ...
  async list(
    ...
    try {
      requireUserId();
      const { data, error } = await rpc<UesListRow[]>("ues_list", {
        p_kind: kind ?? null,
      });
      if (error) return err(...mapPgErrorPair(error));
  ...
  async getBulk(
    ...
    try {
      ...
      const { data, error } = await rpc<UesBulkRow[]>("ues_get_bulk", {
  ...
  async touch(
    ...
    try {
      requireUserId();
      const { error } = await rpc<null>("ues_touch", {
        p_entity_type: entityType,
