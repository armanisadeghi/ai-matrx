...
// assignment table.
//
// Typed-RPC note: `cat_list` / `cat_create` are deployed + verified live
// (migration `user_state_and_category_rpcs.sql`) but are not yet in the
// generated `database.types.ts` (regenerated by `pnpm db-types` in a follow-up
// pass). Until then the `Database`-typed client doesn't know the names, so calls
// go through `rpc()` below — a single, isolated, minimally-typed view of
// `supabase.rpc`. When the types regenerate, delete `rpc()` and call
// `supabase.rpc("cat_…", …)` directly; signatures here stay constant.
...
//
// `cat_*` are not in the generated `Database` types yet, so the typed client
// rejects the names. Cast once, here, to a narrow rpc surface that returns the
// same `{ data, error }` envelope every Supabase rpc returns.
...
export const categoriesService = {
  ...
  async list(
    ...
    try {
      requireUserId();
      const { data, error } = await rpc<CatListRow[]>("cat_list", {
  ...
  async create(args: {
    ...
    try {
      requireUserId();
      const { data, error } = await rpc<string>("cat_create", {
