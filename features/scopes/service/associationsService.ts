...
//
// Typed-RPC note: the `assoc_*` functions are deployed + verified live but are
// not yet in the generated `database.types.ts` (regenerated by `pnpm db-types`
// in a follow-up pass). Until then the `Database`-typed client doesn't know the
// names, so calls go through `rpc()` below — a single, isolated, minimally-typed
// view of `supabase.rpc`. When the types regenerate, delete `rpc()` and call
// `supabase.rpc("assoc_…", …)` directly; signatures here stay constant.
...
// ─── untyped-rpc bridge (see header) ────────────────────────────────
//
// `assoc_*` are not in the generated `Database` types yet, so the typed
// client rejects the names. Cast once, here, to a narrow rpc surface that
// returns the same `{ data, error }` envelope every Supabase rpc returns.
type RpcEnvelope<T> = { data: T | null; error: unknown };
const rpc = supabase.rpc.bind(supabase) as unknown as <T = unknown>(
  fn: string,
  args?: Record<string, unknown>,
) => PromiseLike<RpcEnvelope<T>>;
...
export const associationsService = {
  ...
  async listForEntity(
    ...
    try {
      requireUserId();
      const { data, error } = await rpc<AssocForEntityRow[]>(
        "assoc_for_entity",
        { p_type: type, p_id: id },
      );
  ...
  async add(args: {
    ...
    try {
      requireUserId();
      const { data, error } = await rpc<string>("assoc_add", {
  ...
  async remove(args: {
    ...
    try {
      requireUserId();
      const { error } = await rpc<null>("assoc_remove", {
        p_source_type: args.sourceType,
        p_source_id: args.sourceId,
        p_target_type: args.targetType,
        p_target_id: args.targetId,
  ...
  async setTargets(args: {
    ...
    try {
      ...
      const { error } = await rpc<null>("assoc_set_targets", {
        p_source_type: args.sourceType,
        p_source_id: args.sourceId,
        p_target_type: args.targetType,
        p_target_ids: target,
