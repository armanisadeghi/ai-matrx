...
**First real consumer**: Matters "QME Report" (`allowed_reference_types=['file']`, `max_items=10`; see `migrations/ctx_matter_qme_report_reference.sql`). Existing Matter reference items (Client → Client scope, Practice Area → Practice Area scope) backfilled in the same slice (`migrations/ctx_context_reference_cells.sql`).

**Legacy pre-fence values** — a handful of `value_type='reference'` cells written before this system carry a bare uuid or the legacy `value_reference_id`/`value_reference_type` columns directly. Those columns stay read-only (never write them from new code); migrate a legacy cell to a fence via `context.write_context_value` the same way `ctx_matter_qme_report_reference.sql` did, not by hand-patching the row.
