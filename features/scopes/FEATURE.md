...
- `2026-07-21` — **`/scopes` hub v2 — scopes are the subject.** Replaced the interim "dimension band + tile grid" layout (owner: horizontal tiles buried the scopes) with masonry columns (`columns-1/2/3`) of compact per-scope-type groups: a tiny header (colored type icon, label, count, quiet org name, icon-only Manage/New into the org's type route) over a vertical list of prominent scope rows (15px name, type-colored connector line + dot — the org-overview tree's visual language) linking to `/scopes/{id}`. Live name filter, active-context scopes bolded with filled dot + "active" tag, empty orgs demoted to a dashed footer row. Read-only per the Surface A invariant — the hub navigates, never writes `appContextSlice`.
