...
- `2026-06-23` — claude: **`ScopeIcon` — the one primitive for DB scope-type icons (build-bloat fix).** New `features/scopes/components/ScopeIcon.tsx` is the single sanctioned way to render a scope / scope-type / template icon (`type.icon`, `t.icon`, `scope.icon`) whose name comes from the DB. It (1) paints an animated, payload-free placeholder instantly (a soft pulsing glyph tinted with the scope `color`, so a row reads as "a category" before any icon chunk loads), (2) lazy-loads the heavy `DynamicIcon` via `next/dynamic({ ssr: false })` and only after commit (`requestAnimationFrame`), so the icon payload isn't even requested during a long list's first render, and (3) cross-fades the resolved icon in. Callers just pass `name` (+ optional `color`); the dynamic + transition is fully internal.
- `2026-06-23` — claude: **Full `resolveIcon` leak sweep → `ScopeIcon`.** Killed the entire whole-library `import * as Icons from "lucide-react"` leak family used to render DB scope-type icons. (a) `features/scope-system/utils/resolveIcon.ts` (namespace import) **deleted** — zero importers remain; (b) `ScopeGlyph` (~12 callsites) now delegates to `ScopeIcon`; (c) the scope/context icon-input editor sheets (`EditScopeTypeSheet`, `AddScopeModal`) switched to the `IconInputWithValidation.dynamic` front door; (d) every direct/local `resolveIcon` callsite migrated to `ScopeIcon` across the `scopes`, `scope-system`, `organizations`, `agent-context` (14 files incl. all `hierarchy-selection/*` + `scope-admin/*`), and `tasks` features (~26 usages, 20 files); (e) a dead `import * as icons` in `TaskQuickCreateCore.tsx` removed. Scope-type icon rendering no longer drags lucide into the build anywhere.
