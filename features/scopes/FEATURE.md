...
> This is **Layer A** of the three-layer runtime model (active/passive context vs. reference tree vs. durable object assignment). Full table: `docs/knowledge/scope-model.md` → "Runtime context — three layers". A user *action* (tagging/binding) reads the explicit UI selection (Layer C), never Layer A just because it's loaded.
...
**Direct Entry vs. Reference — the first decision, never a flat type list.** Every UI that lets someone choose a context item's value shape asks ONE question first, via `EntryModeToggle` (`features/scopes/components/reference/EntryModeToggle.tsx`): **Direct Entry** or **Reference**. Only after that does a second control appear — the primitive type (Direct) or `ReferenceConfigFields` (`features/scopes/components/reference/ReferenceConfigFields.tsx`: allowed types, max items, allowed scope types), shown immediately and in full, never behind a further toggle. `ContextItemAddForm` and `ContextItemSettingsForm` both route through these two shared components — there is no third place that reimplements "pick a type." Before this fix, `ContextItemAddForm`'s "Type" field was a single flat `<Select>` listing `String | Number | Boolean | Date | Object | Array | Document | Reference` side by side (`VALUE_TYPE_CONFIG`'s raw keys) — the exact anti-pattern this whole reference system was built to eliminate, since it treated "points at another entity" as if it were just another primitive with no config surfaced at all. The System Context admin page (`app/(admin)/administration/system-context/page.tsx`) had the same flat list in its own local `VALUE_TYPE_OPTIONS` with a "Reference" entry that was additionally dead — `ContextValueInput` requires a `referenceConfig` + `scopeId` for `value_type="reference"` that dialog never supplied, so picking it silently rendered "Reference picker unavailable." Removed until that page gets its own `ReferenceConfigFields` wiring.
