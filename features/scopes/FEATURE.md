...
- `2026-06-15` — claude: **Scope CRUD panels → `MatrxDynamicPanel`.** New shared host `components/matrx/resizable/MatrxDynamicPanelHost.tsx` wraps `MatrxDynamicPanel` with open/close, title, Escape dismiss, and a header close button. Replaced right-side `Sheet` chrome on scope-type / scope / context-item editors: `EditScopeTypeSheet`, `EditContextItemSheet`, `AddScopeModal`, `ScopeTypeFormSheet`, `ScopeFormSheet`, and `ScopeTemplateStarter`'s template picker. Panels are non-blocking, drag-resizable, and repositionable (left/right/top/bottom) on desktop; mobile stays full-screen via `MatrxDynamicPanel`'s existing path. `EditScopeValueSheet` unchanged (value editor, not structural CRUD).
