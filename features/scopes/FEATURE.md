...
| Part | File | What |
|---|---|---|
| **Registry resolver** | `registry/entityRegistry.ts` — `getEntityInfo(token)` | Merges the **generated** `ENTITY_TYPE_METADATA` (schema/table/label/category — the source of truth) with a thin FE **overlay** (`Icon`, `titleColumn`, `ownerColumn`, `hrefFor`) for the two things the DB can't carry (a React icon, a human title column). The single source the cards read display + query metadata from. **This is the canonical replacement for the hand-curated `features/organizations/resource-catalogue.ts`** (which re-lists schema/table/icon per kind and drifts). |
| **Candidate reader** | `service/associationCandidates.ts` + `hooks/useAssociationCandidates.ts` | `listAssociationCandidates({ token, ownerId })` resolves the backing table from the registry and reads `{ id, title }` rows the user may attach (direct supabase-js read; RLS-gated, optional owner-column narrowing). |
| **Container hook** | `hooks/useContainerLinks.ts` | Reads a container's **incoming** edges via `useAssociations({ type: containerType, id })` (shared cache → one fetch per container) and exposes `countFor(token)` / `attachedIdsFor(token)` / `attach` / `detach`. Writes go `source=resource → target=container` (canonical direction, same as scope-tagging) through the association thunks, which reload both endpoints so counts refresh automatically. |
| **Components** | `components/associations/{PrimaryEntityContext,AssociationCard,AssociationPickerSheet}.tsx` | A page mounts `<PrimaryEntityProvider value={{ type, id, orgId, label }}>` once; each `<AssociationCard token="task" />` inside resolves its container from context, shows the registry icon + label + live count + a plus button, and opens `AssociationPickerSheet` — an adaptive right-Sheet (desktop) / bottom-Drawer (mobile) candidate picker that marks attached rows and toggles the edge on click. |

**Direction is canonical and singular:** the resource is always the edge SOURCE, the container the TARGET (`task → organization`, `file → scope`). This is why `organization` is in `ASSOCIATION_TARGET_TYPES` — a container must be a valid target. Listing "things attached to a container" = its incoming edges. First mounted on the org home (`OrgWorkspace`, Tasks + Files); the buggy permissions/`share_resource_with_org` resource grid is a separate, parallel system slated to migrate onto these cards.
