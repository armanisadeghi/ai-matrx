...
- `<ActiveContextLensChip />` / `<LensChip />` — Lens Chip trigger (reimagine T2 face) → Popover/Sheet → `<ActiveContextTree />` (the promoted dense ContextTree). Every explicitly selected organization is listed by its canonical abbreviation (`ME · PBW · 2 scopes`) instead of an organization count; other selected kinds keep compact count summaries. Clear lives in the tree footer. Default chat-header control.
- `<ContextLensBar />` — the composer context control: ONE pill wrapping a clickable "Context" segment (opens the `contextPreviewPanel` overlay — server-truth "what the agent receives" via `POST /ai/context/preview`) + the Lens Chip picker segment. Pure composition over `ActiveContextLensChip`; hosts pass `onOpenPreview`.
- `<ActiveContextTree />` — THE compact picker: the promoted dense `ContextTree` (`active-context/context-tree/`) wired to `appContextSlice`. Real tree with search, keyboard, lazy projects/tasks/fields. **Cascade-up:** selecting a scope (or field) always selects its type + org — you cannot have a child without its ancestors. Production cardinality: one org, multi-scope, one project, one task. Use in popovers/menus (`ContextDocsMenu`, compact `+` fold-in); prefer `ActiveContextPanel` where there's real vertical room. Demo host: `/demos/scopes/context-lab/dense`.
...
- `useActiveContextSelectionEngine()` — the shared Surface-A bridge for Miller Columns and Drill Deck. It maps both faces to `appContextSlice` through `setFullContext` without disturbing `conversation_id`; context-item checks remain session-local.
