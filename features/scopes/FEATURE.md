...
| **Component** | Two reusable shells: `components/associations/EntityAssociator.tsx` — the *source-centric* panel (fed `sourceType` + `sourceId`, renders an entity's relationships both directions as chips + per-target-type "Add"); and the *container-centric* **association cards** (`AssociationCard` + `PrimaryEntityProvider` + `AssociationPickerSheet`) — see §"Association cards" below. |
...
### Association cards — token-driven container UI

The canonical, **fully generic** way to surface "what is attached to this container, and add more" on any page that owns a container entity (an org, a scope, a scope type, a project). Built from four parts, **no per-entity code in any component** — hand it a token and it works:
