...
- `2026-07-19` — **`/scopes` hub rebuilt scope-first.** `ScopesHub` no longer renders org cards; it renders one "dimension band" per scope type aggregated across every org (active org's dimensions first), each band a colored header (type icon/color, count, quiet org metadata) over a grid of scope tiles linking to `/scopes/{id}`, with a live text filter, an active-context dot on globally-selected scopes, per-band Manage/New links into the org's type route, and empty orgs demoted to a dashed footer row. Read-only per the Surface A invariant — the hub navigates, never writes `appContextSlice`.
- `2026-07-19` — **Admin-defined chooser categories + universal candidate reader (pending SQL).** `platform.reference_categories` (+`reference_categories_list`/`admin_upsert_reference_category`) and `entity_types.reference_category` let admins bucket chooser types arbitrarily (schema pretty name is the fallback); the entity-types admin form auto-registers new slugs. Candidate listing + title resolution now try the universal `reference_search_candidates` SECURITY DEFINER RPC first (works for unexposed schemas; owner+org+visibility access model) with a loud direct-read fallback — the RPC's SQL lives in `migrations/reference_categories_and_candidate_search.sql` and still needs to be applied.
