...
**Direct vs. Reference — the first decision, never a flat type list.** Every UI that chooses a context item's value shape asks ONE question first via `EntryModeToggle` (`ToggleGroup`, `h-7 text-xs` — same footprint as `Button size="sm"` type chips): **Direct** or **Reference**. Then either the primitive-type picker (Direct) or `ReferenceConfigFields` (allowed types / max items / allowed scope types via canonical `Field` + `Button size="sm"` chips) — shown immediately, never behind a second toggle.

**The allowed-types chooser is DB-driven and two-tier.** Which entity types appear is governed by `platform.entity_types.reference_pickable` + `title_column` + `content_role` — super-admin-managed at `/administration/relationships/entity-types`, mirrored into the generated registry by `pnpm gen:entity-types`, resolved by `listableTokens()` (`features/scopes/registry/entityRegistry.ts`). The FE overlay no longer gates the list; it only contributes icons, hrefs, and legacy fallbacks. `ReferenceConfigFields` renders the choices in two tiers — tier 1 the `content_role` bucket (`CONTENT_ROLES`: Utilities / Sources / Outputs / Sources & Outputs / Workspaces; synthetic `url`→source, `scope`→container), tier 2 the types in the active bucket — with a cross-bucket "Selected" chip row. It also mounts an inert live preview (`showPreview`, default true): the REAL `ReferenceValuePicker` against throwaway local state so the author sees exactly what fillers get; `ContextItemAddForm` disables it on scope pages where the real "Value for this one" input already shows the picker. A `reference_pickable` token with no `title_column` and no FE candidate source is excluded and `console.error`ed — set its title column in the admin. `ContextItemAddForm` and `ContextItemSettingsForm` both consume these two primitives; do not reimplement "pick a type." `ContextItemAddForm` puts the toggle and primitive `<Select size="sm">` on one row and labels everything with `Field`.
...
- `2026-07-19` — **Reference allowed-types went DB-driven + two-tier + preview.** `platform.entity_types` grew `reference_pickable` / `title_column` / `content_role` (migration `entity_types_reference_pickable.sql`, backfilled from the old overlay); `listableTokens()`/`getEntityInfo()` read them registry-first; the entity-types admin edits them. `ReferenceConfigFields` now selects types via content-role buckets (two tiers) and shows an unsaved live `ReferenceValuePicker` preview.
