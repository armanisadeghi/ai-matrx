...
| **Components**        | `components/associations/{PrimaryEntityContext,AssociationCard,AssociationCardGrid,AssociationPickerSheet}.tsx` | A page mounts `<PrimaryEntityProvider value={{ type, id, orgId, label }}>` once; `<AssociationCardGrid />` renders one card per listable token (or pass `tokens={[…]}` to scope it), and each `<AssociationCard token="task" />` resolves its container from context, shows the registry icon + label + live count + a plus button, and opens `AssociationPickerSheet` — an adaptive right-Sheet (desktop) / bottom-Drawer (mobile) candidate picker that marks attached rows and toggles the edge on click.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
**Direction is canonical and singular:** the resource is always the edge SOURCE, the container the TARGET (`task → organization`, `file → scope`). This is why `organization` is in `ASSOCIATION_TARGET_TYPES` — a container must be a valid target. Listing "things attached to a container" = its incoming edges. **The org home (`OrgWorkspace`) "Resources" section IS this card grid** (`<AssociationCardGrid />` for all listable tokens) — it replaced the old permissions/`container_resource_counts` count grid (`useOrgResourceInventory` + `OrgResourceRoleSection`) that hand-listed kinds and drifted. Scope / scope-type / project container pages are the next surfaces to mount it.
