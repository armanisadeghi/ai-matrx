...
- `<DrillDeckCore />` / `<DrillDeck />` — canonical one-column context navigator for narrow rails, drawers, and menus. Hosts can constrain selectable node kinds, omit Projects/Tasks, own creation/commit callbacks, inject a `Universe` into the core, and use the root back action to return to a parent menu. The Smart Input Resource Picker uses the item-only face for Context Values.
