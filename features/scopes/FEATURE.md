...
- `2026-06-23` — claude: **`ScopeIcon` — the one primitive for DB scope-type icons (build-bloat fix).** New `features/scopes/components/ScopeIcon.tsx` is the single sanctioned way to render a scope / scope-type / template icon (`type.icon`, `t.icon`, `scope.icon`) whose name comes from the DB. It (1) paints an animated, payload-free placeholder instantly (a soft pulsing glyph tinted with the scope `color`, so a row reads as "a category" before any icon chunk loads), (2) lazy-loads the heavy `DynamicIcon` via `next/dynamic({ ssr: false })` and only after commit (`requestAnimationFrame`), so the icon payload isn't even requested during a long list's first render, and (3) cross-fades the resolved icon in. Migrated every scope-icon callsite to it — `ScopesHub`, `ActiveScopeChips`, `TemplatesGalleryPanel`, `EntityScopeTagger` (×2), `ScopeDetailView`, `ActiveContextLayersPanel`. This removed the last static `@/components/official/icons/IconResolver` imports from the `scopes` feature (the build-time leak class). Callers just pass `name` + `color`; the dynamic + transition is fully internal.
