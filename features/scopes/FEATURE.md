...
Scope is the most important _part_ of context, not its synonym. Why important? Because it is the only piece of context users **author by hand** — the org admin decides which dimensions matter, then fills the cells once, and that work feeds every downstream invocation forever.

### Why the distinction matters — the Liquid Facelift example

> User asks: "Can you help me come up with some good keywords for _Liquid Facelift_?"
...
What the user is working on _right now_. Lives in `appContextSlice`. Used by anything not tied to a specific entity (the sidebar, the chat composer at the top level, "create a new note" without a parent, "run this agent" from the home screen).

```
appContextSlice = {
  organization_id,
  scope_selections: Record<scope_id, scope_id>,   // MULTI-SELECT since 2026-06-12: any number of scopes across any types
  active_scope_type_ids: string[],                 // 2026-06-30: whole dimension active, no instance chosen (rare)
  project_id,
  task_id,
  conversation_id,
}
...
> This is **Layer A** of the three-layer runtime model (active/passive context vs. reference tree vs. durable object assignment). Full table: `docs/knowledge/scope-model.md` → "Runtime context — three layers". A user _action_ (tagging/binding) reads the explicit UI selection (Layer C), never Layer A just because it's loaded.
...
- Notice copy: "This [note/task/agent] is scoped to _Rejuvina Medspa_, but your active context has _Dr. Nazarian_. Locally-triggered actions will use _Rejuvina_. Globally-triggered actions still use _Dr. Nazarian_."
...
1. **Contributing scopes** = the entity's scope tags in `platform.associations` (project-tag fallback) ∪ the request's explicit `scope_ids` (membership-validated against the user; foreign ids dropped). `resolve_full_context*` reads `platform.associations` only — zero database functions reference the graveyarded `ctx_scope_assignments` table (verified live 2026-07-15).
...
5. **Ask on mismatch (pre-send gate):** a chat carries its scopes durably, like a coding agent's attached repos. `ensureConversationScopesOrAsk` (`redux/thunks/conversationScopeGate.ts`, called by `smartExecute` before `markInputSubmitted`) compares the active selection **A** against the chat's tags **C**: C = ∅ → proceed (union sync stamps A); A = ∅ → send with `scope_ids = C` (an empty sidebar never strips a chat's context); A = C → proceed; A ≠ C → the 3-way `ScopeMismatchDialogHost` (`components/dialogs/scope-mismatch/`) ALWAYS asks — **Use current selection** (A) / **Combine both** (A ∪ C) / **Keep chat's context** (C); dismiss cancels the send with the composer intact. The choice becomes both the send's `scope_ids` (via `scopeIdsOverride` on `executeInstance`) and the chat's tags (`setEntityScopes`, replace semantics — the one sanctioned replace, since the user explicitly chose; the post-send union sync is skipped so it can't re-add dropped scopes). Never silently retag, never silently drop — but never nag either: the gate remembers the last answered (A, C) pair per conversation (runtime-only) and re-asks only when either set changes; a cancel is not remembered. Pure decision logic + tests: `utils/scopeMismatch.ts`. Skipped for ephemeral chats and manual (Agent Builder) mode. The dialog writes ENTITY tags only — the sidebar's active selection (`appContextSlice`) is untouched by every choice, per the global-vs-local invariant.
...
| `ctx_scope_assignments`      | **GRAVEYARDED — migration complete (verified live 2026-07-15).** Was the M2M scope↔entity join table; replaced by the unified edge `source=(entityType, entityId) → target=('scope', scopeId)` in `platform.associations`, reached through the `assoc_*` RPCs via `associationsService`. **Zero database functions reference the table** (`pg_proc.prosrc` sweep) and zero frontend/aidream code touches it — `get_entity_scopes`/`set_entity_scopes`/`list_entities_by_scopes` and `resolve_full_context*` are all off it. The table still physically exists in the graveyard schema; nothing reads or writes it. |
...
## Context reference cells — the ONE way a cell points at something

A `value_type='reference'` context item (e.g. Matter → "QME Report") never stores a bare uuid in `value_reference_id`/`value_reference_type` (legacy columns, read-only — see below) and never invents a per-feature id format. The cell's `value_text` is ALWAYS the canonical [`features/matrx-envelope/`](../matrx-envelope/FEATURE.md) ` ```matrx ` fence with `kind:"reference"` — the same encoding picklists already use — with the parsed envelope mirrored into `value_json`. Lists (multiple files, etc.) are `items: [...]` inside ONE fence, never sibling "field 2"/"field 3" items.

**Item-definition columns** (on `context.context_items`, gate what a cell may hold):

| Column                                 | Role                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `allowed_reference_types text[]`       | The Matrx reference-fence types (`features/matrx-envelope/envelope.ts` `REFERENCE_TYPES` — `file`, `url`, `scope`, `note`, `task`, `agent`, …) this item may point at. `NOT NULL`/non-empty required when `value_type='reference'` (`context_items_reference_types_required` check). Never `platform.associations` tokens — a reference cell is a typed SLOT value, associations are membership edges (§"The unified association edge"). |
| `max_items integer NOT NULL DEFAULT 1` | Cardinality. `1` = single value; `>1` = list, always explicit (e.g. Matter QME Report allows up to 10 PDFs — a matter can accrue several reports over its life). `CHECK (max_items >= 1)`.                                                                                                                                                                                                                                               |
| `allowed_scope_type_ids uuid[]`        | When `allowed_reference_types` includes `"scope"`, further restricts which scope types are pickable (e.g. Matter→Client points only at Client-type scopes). `NULL`/empty = any scope type in the org.                                                                                                                                                                                                                                    |

**Write path** — `context.write_context_value(p_item_id, p_scope_id, p_value_text, ...)` is the ONE place validation + persistence + reverse-indexing happens, regardless of caller. Both public RPCs (`set_context_value`, `set_scope_context_value`) are thin wrappers around it. It parses the fence, checks `type ∈ allowed_reference_types` and `items.length ≤ max_items` (`context.validate_reference_value`), inserts the new version, and upserts `context.context_value_refs` rows for the new version (`context.index_reference_value`) — old versions keep their rows for history. `resolve_full_context` needs no reference-specific branch: its `value_text IS NOT NULL` case is checked first, so the fence string flows straight through exactly like a picklist fence.

**Template-backed table values** — a reference item may bind `reference_source` to `{container_type:"dataset_template", template_id, dimension:"whole", provision:"per_scope"}`. The shared `ReferenceConfigFields` editor exposes this as “Per-scope table template”. Selecting one fixes the cell contract to one `dataset` reference. Database triggers provision a strict UDT dataset for every `(context_item, scope)` pair (both existing scopes when the item is added and future scopes on insert), copy the ordered template fields, record the durable mapping in `context.scope_dataset_instances`, and save the ordinary dataset fence through `context.write_context_value`. Template-instance schemas and their template/version binding are immutable; users edit rows, not columns. Existing fence rendering, reverse indexing, and agent context resolution work unchanged.

**Reverse index** — `context.context_value_refs (value_id, context_item_id, scope_id, ref_type, ref_key)`, indexed on `(ref_type, ref_key)` and `(value_id)`. Read it through `scopesService.listReferencingValues(refType, refKey)` (→ `public.list_context_value_refs`, membership-checked) — never query the table directly. Answers "which matters point at this file?" as a plain indexed lookup, day one.

**Canonical UI primitives** (`features/scopes/components/reference/` and `features/scopes/utils/referenceCell.ts`) — ONE way each, composed into every editor/display surface (`ScopeFieldInput`, `EditScopeValueSheet`, `ContextItemSettingsForm`, `ScopeDetailView`, `ActiveContextLayersPanel`, `ContextLayerBodies`, the KG-suggestion current-value diff):

| Primitive                                                                                                                                                                  | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ContextValueInput`                                                                                                                                                        | THE value-entry switch for any `value_type`. Routes `reference` → `ReferenceValuePicker`; `boolean`/`date` get dedicated controls; everything else → `ProTextarea` by default, or a plain `Textarea` (no mic/AI-actions/stats chrome) when `compact` is passed — for quick, secondary value entry inside dense forms (e.g. `ContextItemAddForm`'s inline "value for this one" row) where `ProTextarea`'s richness doesn't fit and its `bg-transparent`/no-shadow style visibly clashed with the surrounding `Input`s.       |
| `ReferenceValuePicker`                                                                                                                                                     | Constrained by `allowed_reference_types` + `max_items` (+ `allowed_scope_type_ids`); one "Add" sub-picker per type (`file` → `useFilePicker`, `url` → plain URL+label form, `scope` → org scope tree filtered by allowed types, everything else → `useUniversalEntitySearch`). Constrained record pickers browse available candidates on open, then filter as the user types; they never present an empty “type to search” state when selectable rows already exist. Emits a full fence string or `null` — never a bare id. |
| `ContextValueDisplay`                                                                                                                                                      | Read-only render — parses the fence and hands it to `MatrxEnvelopeBlock` for the SAME live reference-chip renderer used everywhere fences appear. Use in every read-only surface; for surfaces that can't render JSX (dense grid cells, tooltips, KG-suggestion diffs) use `referenceCellSummary()` (`features/scopes/utils/referenceCell.ts`) for a plain-text summary — e.g. "QME Report.pdf" / "3 Files" — instead of dumping the raw fence JSON.                                                                        |
| `parseReferenceCellValue` / `buildReferenceCellValue` / `validateReferenceCellValue` / `referenceConfigFromItem` / `referenceTypeLabel` / `CONTEXT_REFERENCE_TYPE_OPTIONS` | Client-side parse/validate mirror of the DB path, for instant feedback before round-tripping. The RPC is authoritative (also checks `allowed_scope_type_ids`, which needs a DB lookup).                                                                                                                                                                                                                                                                                                                                     |

**`url` as a reference type** — the one type with no Matrx-owned id, for a plain external link (`items: [{ url, label? }]`). Never resolved server-side (nothing to look up); the chip opens the URL directly in a new tab. Built via `buildUrlReferenceFence`/`buildMultiUrlReferenceFence` (`features/matrx-envelope/urlReference.ts`).

**Agent binding** — `buildVariableFromItem`/`buildContextSlotFromItem` (`features/agents/utils/context-item-slot-mapping.ts`) map a `reference` item to slot type `db_ref` (the closest existing `ContextObjectType` — a resolved-server-side pointer, not raw text/json). No slot-side UI needed: the slot just points at the scope cell, which is already edited through `ReferenceValuePicker`.

**First real consumer**: Matters "QME Report" (`allowed_reference_types=['file']`, `max_items=10`; see `migrations/ctx_matter_qme_report_reference.sql`). Existing Matter reference items (Client → Client scope, Practice Area → Practice Area scope, AME/QME → Expert scope) had their item-definition config backfilled in the same slice (`migrations/ctx_context_reference_cells.sql`).

**Legacy pre-fence values — fully migrated.** Every pre-existing `value_type='reference'` cell across the platform (the QME Report bare-uuid value, plus every Client/Practice Area/AME-QME cell still on the old `value_reference_id`/`value_reference_type` columns) has been converted to a canonical fence in `value_text` (`migrations/ctx_matter_qme_report_reference.sql` + `migrations/ctx_reference_legacy_scope_backfill.sql`). `value_reference_id`/`value_reference_type` are legacy, read-only columns now with zero current rows using them — confirmed via a platform-wide sweep. If a legacy cell ever resurfaces (e.g. restored from an old version), migrate it to a fence via `context.write_context_value` the same way those two migrations did, never by hand-patching the row.

**Every reference chip live-resolves — editable or not.** `useResolvedReferenceLabel` (`features/matrx-envelope/referenceResolvers.ts`) is the ONE place a reference item's display value is fetched live from Supabase; both the read-only `ReferenceChip` (`registry.tsx`) and the editable `PickerChip` (`ReferenceValuePicker.tsx`) call it. A baked-in `label` hint (set at author time by a picker) is only ever a first-paint head start — it goes stale when the underlying entity is renamed, and is simply absent on a backfilled or server-authored fence. Before this, `PickerChip` used the static `referenceFallbackLabel` alone, so an item with no baked-in label rendered as a bare, generic type name (e.g. a Client reference showing "Scope") that looked like a real value and silently blocked adding a new one (max_items reached) until the user manually removed it. Never reintroduce a static-only chip label anywhere reference items render — always go through this hook.

**One more display/summary consolidation pass** collapsed the remaining duplicated read/write surfaces onto the primitives above: `ContextValueDisplay` now branches on every `value_type` (not just `reference`) and is the single renderer behind the shared `ContextValueRow` (`features/scopes/components/reference/ContextValueRow.tsx`) used by `ScopeDetailView`, `ActiveContextLayersPanel`, and `ContextLayerBodies` (all three used to carry their own near-identical row component). `summarizeContextCell` (`features/scopes/utils/referenceCell.ts`) is the one plain-text summarizer for dense grid cells — `ScopesList`, `OrgHomeScopeSection`, and `KgSuggestionRowItem` all call it instead of re-deriving a summary from `value_reference_*`/`value_text`/etc. per-callsite. `ContextValueInput`'s `placeholderForType` is exported and reused by `ScopeFieldInput` rather than re-implemented. `BoundVariableChips`' runtime bound-variable editor now renders `ContextValueInput` (config sourced via `referenceConfigFromItem`) instead of the older `VariableInputComponent`, so editing a bound reference variable at run time goes through the same picker as editing the underlying context item.

**Add/Edit/Manage Context Item — the one Window Panel** (`features/window-panels/windows/context-scopes/ContextItemsWindow.tsx`, overlay id `contextItemsWindow`, singleton). Opened via `useOpenContextItemsWindow()` (`features/overlays/openers/contextItemsWindow.tsx`) with `{ scopeTypeId, initialItemId?, openNewOnMount? }` — the same component serves "edit one item" (pass `initialItemId`) and "manage all items for a scope type" (omit it). UI modeled on the Agent Connections window's Skills tab: a sidebar lists every context item for the scope type (search, value-type badges, reorder, "+" add), clicking a row opens it as a closeable tab in a tab bar so several items stay open at once; the "+" opens a blank draft tab that swaps in place for the real item's tab on save. Body reuses the existing canonical forms verbatim — `ContextItemSettingsForm` (edit) and `ContextItemAddForm` (create) — no third editor was forked. Entry points today: "Manage in panel" button + the pencil/edit affordance on `ScopesList` (`features/scope-system/components/ScopesList.tsx`, edit now opens the panel to that item instead of the old `EditContextItemSheet`, which this panel replaces) and `ContextItemsHub` (`features/scope-system/components/ContextItemsHub.tsx`).

**Quick entry point — "Set Context Value" from any chat message.** Any assistant/user message in `/chat` can push its content straight into a scope's context item cell via the message Actions menu (`⋯ → Actions → Set Context Value`, right after **Create Task**) — the same pattern as `Save as → Note`, applied to context cells instead of notes. Opens `setContextValueWindow` (a `WindowPanel`, overlay id `setContextValueWindow`) rendering `SetContextValueCore` (`features/scopes/actions/quick-assign/`): the same content-refine toolbar as the notes quick-save flow (strip-thinking, trim start/end, split/preview/plain view), a target cascade (`ScopeContextTargetPicker`, `features/scopes/components/quick-assign/`: Organization → Scope Type → **specific Scope instance** → Context item — one level deeper than `ContextItemPicker`, which stops at the item definition), and, once a text-compatible item is picked, the identical Append/Overwrite radio + destructive-overwrite confirm + diff-preview UI as `QuickNoteSaveCore`. **Only text-shaped `value_type`s are selectable** (`string`/`object`/`array`/`document`) — a `reference`/`boolean`/`number`/`date` item renders in the picker but disabled with a "text only, for now" hint, since "append this message to the bottom" has no defined meaning for those shapes yet. Writes go through the existing `setScopeContextValue` thunk → `set_scope_context_value` → `context.write_context_value` chokepoint — no new backend surface. `useSetContextValue` (`features/scopes/actions/quick-assign/useSetContextValue.ts`) mirrors `useQuickNoteSave` 1:1: loads the target scope's current values via `getScopeContext`/`selectValuesByScope` to detect `has_value` (append baseline / overwrite warning), builds `existing + "\n\n" + working` on append, and dispatches the write.

**Direct vs. Reference — the first decision, never a flat type list.** Every UI that chooses a context item's value shape asks ONE question first via `EntryModeToggle` (`ToggleGroup`, `h-7 text-xs` — same footprint as `Button size="sm"` type chips): **Direct** or **Reference**. Then either the primitive-type picker (Direct) or `ReferenceConfigFields` (allowed types / max items / allowed scope types via canonical `Field` + `Button size="sm"` chips) — shown immediately, never behind a second toggle. `ContextItemAddForm` and `ContextItemSettingsForm` both consume these two primitives; do not reimplement "pick a type." `ContextItemAddForm` puts the toggle and primitive `<Select size="sm">` on one row and labels everything with `Field`.

**The allowed-types chooser is DB-driven, OPEN, and two-tier by schema.** Which entity types are offered is governed by `platform.entity_types.reference_pickable` + `title_column` (super-admin-managed at `/administration/database/relationships/entity-types`, mirrored by `pnpm gen:entity-types`, resolved by `listableTokens()` in `features/scopes/registry/entityRegistry.ts` — the FE overlay only contributes icons/hrefs/legacy fallbacks). The set is deliberately open: every active type with a detectable title column is pickable (~119 as of 2026-07-19) — we do NOT predetermine what a user may associate; a type is only absent when a generic picker has no way to list its candidates (no `title_column`, no FE candidate source — set the column in the admin to enable it, and `listableTokens()` `console.error`s a pickable token missing both). `ReferenceConfigFields` renders two tiers: tier 1 the DB **schema** with its pretty name from `platform.schemas` (generated `SCHEMA_DISPLAY`; `admin_upsert_schema` writes it; synthetic `url`/`scope` live in a first "Basics" bucket), tier 2 the types in that schema; a search box filters across all buckets; selections span buckets in a removable "Selected" chip row. It also mounts an inert live preview (`showPreview`, default true): the REAL `ReferenceValuePicker` on throwaway state, so the author sees exactly what fillers get; `ContextItemAddForm` disables it on scope pages where the real "Value for this one" input already shows the picker. Type labels resolve via `referenceTypeLabel` (override map → DB `label` → humanized token).
...
The canonical, **fully generic** way to surface "what is attached to this container, and add more" on any page that owns a container entity (an org, a scope, a project, a war-room thread/room). Two faces of one system: **`AssociationCardGrid`** (count cards) and **`AssociationList`** (grouped rows — icon + resolved title + open + pin + detach, sectioned by **ContentRole**: `utility|source|destination|hybrid|container`, the knowledge-model axis now living on the registry overlay as `contentRole` with `CONTENT_ROLES` meta — rescued from the deprecated `resource-catalogue.ts`). `AssociationList` takes an optional **write adapter** (`ContainerResourcesAdapter`) for containers with their own edge semantics (war-room threads: single-active demotion, position, `pinned` — `useThreadResourcesAdapter`); the default wraps `useContainerLinks`. **Universal attach:** `UniversalAssociationPicker` — ONE search box over every listable token (`searchCandidatesAcrossTokens` client fan-out, per-token limit + concurrency cap; empty query = recents via `ues_list`) + a token chip rail narrowing to `TokenCandidateList` (the extracted picker body). A token whose backing table can't be read client-side registers a **`listCandidates` override** on its overlay entry (e.g. `data_store` → `features/rag/service/dataStoreCandidates.ts` — `rag.*` isn't PostgREST-exposed). No per-entity code in any component — hand it a token and it works:

| Part                  | File                                                                                                            | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Registry resolver** | `registry/entityRegistry.ts` — `getEntityInfo(token)`                                                           | Merges the **generated** `ENTITY_TYPE_METADATA` (schema/table/label/category — the source of truth) with a thin FE **overlay** (`Icon`, `titleColumn`, `hrefFor`) for the two things the DB can't carry (a React icon, a human title column). **Owner/org are CONVENTIONS, not per-token config:** `DEFAULT_OWNER_COLUMN = "created_by"` and `DEFAULT_ORG_COLUMN = "organization_id"` (verified live across every cardable table post-2026-reorg); a token overrides only if its table diverges. (The old per-token `ownerColumn: "user_id"` was the `files.user_id` 42703 bug — `files.files`/notes/tasks/etc. have no `user_id`.) Overlay covers all 17 cardable canonical tokens. The single source cards read display + query metadata from. **Canonical replacement for the hand-curated, drifting `features/organizations/resource-catalogue.ts`** — that file is DEPRECATED for display/association and survives only for the `iam.permissions` sharing surface.  |
| **Candidate reader**  | `service/associationCandidates.ts` + `hooks/useAssociationCandidates.ts`                                        | `listAssociationCandidates({ token, ownerId })` resolves the backing table from the registry and reads `{ id, title }` rows the user may attach (direct supabase-js read; RLS-gated, `created_by` owner narrowing). **Loud recovery:** a missing owner column (42703) screams to console and retries RLS-only instead of throwing a 400.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Components**        | `components/associations/{PrimaryEntityContext,AssociationCard,AssociationCardGrid,AssociationPickerSheet}.tsx` | A page mounts `<PrimaryEntityProvider value={{ type, id, orgId, label }}>` once; `<AssociationCardGrid />` renders one `<section>` per content role (role dot + title + tagline from `CONTENT_ROLES`) containing one card per listable token in that role (or pass `tokens={[…]}` to scope it), and each `<AssociationCard token="task" />` resolves its container from context, shows the registry icon in a role-tinted chip under a role accent bar + label + live count + a plus button, and opens `AssociationPickerSheet` — an adaptive right-Sheet (desktop) / bottom-Drawer (mobile) candidate picker that marks attached rows and toggles the edge on click.                                                                                                                                                                                                                                                                                                    |
...
Only Surface A components may dispatch the action creators below. Enforced by exporting the action creators _only_ through `features/scopes/redux/active-context-actions.ts`, which ESLint restricts to be imported only from `features/scopes/components/active-context/**`.
...
- `useContainerLinks({ containerType, containerId })` — container-centric view of associations: per-token counts + attach/detach. Powers the association cards. See §"Association cards".
- `useAssociationCandidates({ token })` — read attachable `{ id, title }` records of a token from the registry-resolved table. The picker's data source.
...
- `features/scopes/service/associationCandidates.ts` — registry-driven reader of attachable records for a token (direct supabase-js table read). The association picker's data source.
- `features/scopes/registry/entityRegistry.ts` — `getEntityInfo(token)`: generated `ENTITY_TYPE_METADATA` + FE display/query overlay. Canonical token→presentation resolver; replaces `features/organizations/resource-catalogue.ts`.
...
- `<ActiveContextLensChip />` / `<LensChip />` — Lens Chip trigger (reimagine T2 face) → Popover/Sheet → `<ActiveContextTree />` (the promoted dense ContextTree). Every explicitly selected organization is listed by its canonical abbreviation (`ME · PBW · 2 scopes`) instead of an organization count; other selected kinds keep compact count summaries. Conversation hosts pass `conversationId`, which makes checked fields real request attachments. Clear lives in the tree footer. Default chat-header control.
- `<ContextLensBar />` — the composer context control: ONE pill wrapping a clickable "Context" segment (opens the `contextPreviewPanel` overlay — server-truth "what the agent receives" via `POST /ai/context/preview`) + the Lens Chip picker segment. Pure composition over `ActiveContextLensChip`; conversation identity is required and flows into the attachment path.
- `<ActiveContextTree />` — THE compact picker: the promoted dense `ContextTree` (`active-context/context-tree/`) wired to `appContextSlice`. Real tree with search, keyboard, lazy projects/tasks/fields. **Cascade-up:** selecting a scope (or field) always selects its type + org — you cannot have a child without its ancestors. In conversation hosts, each checked field is also persisted in that conversation's existing `instanceContext` as an exact lazy `ctx_item` pointer (`scope_id` + `context_item_id`), so closing the popover cannot discard it and `selectContextPayload`/the context preview see the same attachment. Production cardinality: one org, multi-scope, one project, one task. Use in popovers/menus (`ContextDocsMenu`, compact `+` fold-in); prefer `ActiveContextPanel` where there's real vertical room. Demo host: `/demos/scopes/context-lab/dense`.
- `<MillerColumnsCore />` / `<MillerColumns />` — canonical four-column context browser (org → type → scope → item) with OR-merged descendants. `variant="full"` includes the project/task rails; `variant="condensed"` caps every column at five rows and moves Project/Task into footer popovers. `<MillerColumnsPopover />` is the thin desktop-popover/mobile-sheet host. `<ActiveMillerColumns />` is the Surface-A adapter; the registered `contextSwitcherWindow` renders its full face. The core is controlled by a `SelectionEngine`, so assignment/filter hosts can reuse it without importing global-context writes.
- `<DrillDeckCore />` / `<DrillDeck />` — canonical one-column context navigator for narrow rails, drawers, and menus. Hosts can constrain selectable node kinds, omit Projects/Tasks, own creation/commit callbacks, inject a `Universe` into the core, and use the root back action to return to a parent menu. `<DrillDeckPopover />` is the thin desktop-popover/mobile-sheet host; `<ActiveDrillDeck />` and the registered `drillDeckContextWindow` provide the Surface-A face. The Smart Input Resource Picker uses the item-only assignment face for Context Values.
- `useActiveContextSelectionEngine()` — the shared Surface-A bridge for Miller Columns and Drill Deck. It maps both faces to `appContextSlice` through `setFullContext` without disturbing `conversation_id`; context-item checks in those non-agent generic faces remain host-controlled. The agent conversation Lens Chip uses the `instanceContext` attachment path described above.
...
After Phase 3 ships, decide whether Surface A and Surface B should share a single configurable picker. The codepath difference (one writes `appContextSlice`, the other writes `platform.associations` via `associationsService` or a controlled-mode callback) is enforced at the component boundary, so a shared inner UI primitive is plausible — but premature today.

**Phase 4 — Manager + values + templates routes** _(completed)_. The canonical `/scopes` route tree now lives under `app/(a)/scopes/`:
...
**Phase 5 — Consumer migration + teardown** _(in progress)_. Phase 5 is the multi-week teardown after every chokepoint mutation method has shipped on the Python side (see `features/scopes/docs/RPC_CONTRACTS.md`). The Phase-5 turn migrated every read-path consumer that doesn't need a new mutation chokepoint:
...
- `2026-07-27` — **Mobile Lens Chip and Working Context sheet polish.** The shared `LensChip` summary is now a single-line, truncating flex item so narrow chat headers never stack abbreviation/count text vertically. `ActiveContextLensChip` now lets the header control shrink and gives the mobile `ActiveContextTree` balanced full-width sheet insets instead of carrying the desktop-only 320px width into the drawer. The canonical dense `ContextTree` keeps its 22px desktop rows but uses roomier mobile search, rows, selection targets, typography, and footer controls; `ContextSheet` now fulfills its documented safe-area-padding contract for every mobile context surface.
- `2026-07-27` — **Context assignment accepts every registered entity token.** The overlay subject, `useEntityScopes`, entity-scope thunks/selectors, and the single-entity read/write chokepoints now use generated `EntityTypeToken` rather than the older curated `EntityType` subset. This closes the false type barrier for already-registered, scopeable resources such as `web_site`; persistence remains the same canonical `platform.associations` entity→scope edge.
- `2026-07-26` — claude: **Wave 3 doc consolidation (cluster #4).** The superseded scope/context doc sprawl was archived: `features/scope-system/docs/*` → `docs/archive/2026/scope-system/` (SALVAGE_FROM_AGENT_CONTEXT — a still-unported UX-pattern backlog, see it before rebuilding template-apply/dedup-preview flows — plus SCOPE_PAGES_MATRIX, SCOPE_ROUTE_INVENTORY, SCOPE_UI_OVERHAUL) and `docs/ctx/*` → `docs/archive/2026/ctx/` (the ctx-association overhaul set; it shipped as `platform.associations`, not `ctx_associations`). Canonical model: `common-docs/systems/scope-context-system/FEATURE.md`; FE truth: this file. `features/agent-context/FEATURE.md` stays live (brokers + slot-fill mechanics, already subordinated to this doc).
- `2026-07-25` — **Constrained reference pickers now show available records immediately.** `useUniversalEntitySearch` gained an explicit `emptyQueryMode` while preserving recents as the default for universal attach surfaces; `ReferenceValuePicker` uses candidate mode for its single-token Brand/Site/etc. pickers, so opening “Add Brand” or “Add Site” lists accessible rows before typing and reports a truthful “No … available” state only when the candidate query is actually empty.
- `2026-07-23` — **Lens Chip names selected organizations by abbreviation.** The shared chat-header/Smart Input context trigger now renders each selected org as its canonical `OrgNode.abbreviation` (`ME · PBW`) before the existing type/scope/project/task counts, with the old count retained only while org metadata is unavailable.
...
- `2026-07-22` — **`getScopeTree` closed a real org-scoping gap.** `scopesService.ts`'s `scope_types`/`scopes`/`projects` reads in `getScopeTree` were unscoped (relying on RLS alone); added `.in("organization_id", orgIds)` to all three, restricting them to the caller's own memberships (already resolved above via `iam.memberships`) — matching THE VIEW LAW. Also added a `// VIEW LAW:` comment to the read-only `templates` catalog read and to `associationCandidates.ts`'s dynamic-table fallback (documenting its existing conditional owner filter).
- `2026-07-20` — **File picking unified on the canonical picker; bare-list sheet annihilated; file-enumeration leak closed.** `AssociationPickerSheet` (the token-generic right-side sheet whose body was a bare `<ul>` of titles) is DELETED. Its replacement `AssociationPicker` (same props) routes token `file` to the canonical `FilesResourcePicker` (search/filters/recents/thumbnails — the Smart Agent Input "Stored Files" picker) and gives non-file tokens a registry-icon candidate list (`AssociationCandidateBody`, reused by `UniversalAssociationPicker`'s browse mode). The reference `file` sub-picker (`ReferenceValuePicker`) opens the canonical picker in the NON-BLOCKING `FilePickerWindow` (`features/resource-manager/resource-picker/FilePickerWindow.tsx` — WindowPanel host, same pattern as ConversationPickerWindow) the moment the `file` type is chosen (no intermediate "Browse files" click). `AssociationPicker` routes token `file` to the same window. DB side: `reference_search_candidates` now gates file ENUMERATION with `files.is_listable_for` (owner + explicit grants only; by-id title resolution keeps `has_access_for`) — see `features/files/FEATURE.md` 2026-07-20 for the full LISTING doctrine. Never add a plain file list to any picker again; mount `FilesResourcePicker` (inline) or `FilePickerWindow` (overlay).

- `2026-07-21` — **`/scopes` hub v3 — one full-width TABLE per scope type** (owner-directed; replaces the masonry-list v2 and tile-band v1 in the same session). Rows = scopes (click → `/scopes/{id}` via `startTransition`); columns = the type's active context items by `sort_order`, capped at `MAX_ITEM_COLUMNS = 6` with a "+N more columns on the type page" header note; cells = current values summarized by the canonical `summarizeContextCell`. Type header links to the org's scope-type page (+ icon-only open/new). Data: two new batched chokepoint reads `scopesService.listContextItemsForTypes` / `listContextValuesForScopes` (2 round-trips total for the whole page) via the new `hooks/useScopeTypeTables` (view-scoped cache; loud console.error on fetch failure; Strict-Mode-safe fetched-key reset in cleanup). Page wrapper went full-width (`w-full p-4 md:p-6`, was `max-w-6xl`). Live name filter, active-context rows highlighted (inset color bar, no leading dot), bold uppercase column headers, empty orgs demoted to a dashed footer row. Read-only per the Surface A invariant. **Legacy `/scopes/[scopeId]` DELETED** (page + `ScopeDetailView` + the orphaned `/graph` subroute): rows route to the CANONICAL org-scoped page `/organizations/{org}/scopes/{type}/{scope}` (`ScopeDetailEditor`, which embeds its own KG card expanding to `/knowledge-graph?scope=…`) — `/scopes` is the only scope surface outside the org routes.
- `2026-07-19` — **Admin-defined chooser categories + universal candidate reader (pending SQL).** `platform.reference_categories` (+`reference_categories_list`/`admin_upsert_reference_category`) and `entity_types.reference_category` let admins bucket chooser types arbitrarily (schema pretty name is the fallback); the entity-types admin form auto-registers new slugs. Candidate listing + title resolution now try the universal `reference_search_candidates` SECURITY DEFINER RPC first (works for unexposed schemas; owner+org+visibility access model) with a loud direct-read fallback — the RPC is live (applied + ledgered; files listing hardened by `reference_candidates_file_listing_gate.sql`). `is_component` rows are never pickable by default (`entity_types_component_not_pickable.sql`) — they are parts of another entity; re-enable a specific one deliberately in the admin.
- `2026-07-19` — **Reference allowed-types opened up: DB-driven, schema-bucketed, previewed.** `platform.entity_types` grew `reference_pickable`/`title_column`/`content_role`; a second migration auto-detected title columns and opened ~119 types across every schema (nothing user-facing is pre-restricted), and added `platform.schemas` (pretty schema names, `entity_schemas_list`/`admin_upsert_schema`). `ReferenceConfigFields` now picks types via schema buckets + cross-bucket search + selected-chip row, and shows an unsaved live `ReferenceValuePicker` preview. Registry admin at `/administration/database/relationships/entity-types` edits pickability/title column.

- `2026-07-26` — **Checked context fields are real conversation attachments.** `ActiveContextTree` no longer keeps item checks only in popover-local React state on agent conversation surfaces. `ActiveContextLensChip`/`ContextLensBar` carry `conversationId`; checking a field writes an exact lazy `ctx_item` pointer into the existing `instanceContext` request lane, reopening reconstructs the check from that pointer, and deselect/Clear remove it. The request selector regression test pins that the rich pointer reaches `request.context` unchanged.
- `2026-07-19` — **`AssociationCardGrid` is now role-grouped and color-coded; org home no longer mounts it.** The grid renders one `<section>` per `CONTENT_ROLES` entry (role dot + title + tagline) over the tokens in that role, and `AssociationCard` wears the role's `accentBar` (top hairline) + `accentBg`/`accentText` icon chip — the same categorical language as the org resource tiles, at the SAME card size (`px-3 py-2.5`, `h-9 w-9` chip). Roles come from the registry (`getEntityInfo(token).contentRole` + `getContentRoleMeta`), so a new token is categorized by its overlay entry alone — no per-surface copy. Consumers: `ScopesList` (scope type) and `ScopeDetailEditor` (scope), both **moved to the bottom of the page** under an "Resources" heading, after the core content; `OrgWorkspace` **dropped it** (the org home already has the `OrgResourceRoleSection` tile grid — one resource surface per page).
- `2026-07-18` — **Canonical association management now spans the full scope hierarchy.** The existing registry-driven `PrimaryEntityProvider` + `AssociationCardGrid` surface is mounted at Organization (existing), Scope Type, and Scope. `/organizations/:org/scopes/:type` and `/organizations/:org/scopes/:type/:scope` now read live incoming `platform.associations` edges through `useContainerLinks` and expose the same adaptive attach/detach picker as the organization workspace; no new association component, state path, table, or RPC was introduced.
- `2026-07-17` — **Drill Deck promoted from Context Lab and shipped in Smart Input.** The canonical core/wrapper now supports host-owned data, creation, commit, selectable-kind constraints, optional Projects/Tasks, and parent-menu back navigation. The Resource Picker's Context Values row renders the item-only Drill Deck and attaches each chosen cell as a live `context_value` Matrx reference fence.
- `2026-07-17` — **Miller Columns promoted from Context Lab to the canonical context-selection set.** Added reusable full and five-row condensed cores, a popover/mobile-sheet adapter, a real `appContextSlice` Surface-A adapter, and upgraded `contextSwitcherWindow` from the legacy 360px hierarchy tree to the full WindowPanel face. Projects/Tasks collapse to one footer button each in condensed mode; the official component catalogue now demos both portable and registered-window usage.
- `2026-07-16` — **ContextTree: selected projects/tasks promoted above the Projects/Tasks menus.** Active picks also render as a top-level strip (amber project / sky task) — second reference only; originals stay in their lists so clicking never makes a row vanish.
- `2026-07-16` — **ContextTree promoted strip is additive.** Selected projects/tasks render as a top-level strip above Projects/Tasks **and** stay in their lists (second reference) so picking never removes a row mid-browse.
- `2026-07-17` — **`ContextLensBar` + server-truth preview.** The composer's Eye/Context button and Lens Chip merged into ONE pill (`ContextLensBar`); its "Context" segment opens the `contextPreviewPanel` overlay showing the aidream `POST /ai/context/preview` response — the EXACT injected block + tiered variables from the same code path a run uses (`resolve_agent_context_block` / `build_agent_context`). Replaced + deleted the blocking `AgentSeesSheet`.
- `2026-07-16` — **Composer rail uses Lens Chip.** `ConversationContextRail` mounts `ActiveContextLensChip` beside Eye/Context (replaces the Scopes → layers drawer pill). Same Popover → `ActiveContextTree` host as the chat header.
- `2026-07-16` — **ADD IS ADDITIVE (ContextTree).** Selecting a project was wiping all scopes via `enforceSingleOrg` (dropped any scope not in the pinned org's ancestry map — including when the project org differed or ancestry was incomplete). `toggleNodeCascaded` no longer filters other dimensions on select; cascade-up only ADDS ancestors; unchecking org no longer clears scopes. Also: working-context writes omit `conversation_id`; Clear → `clearWorkingContext` (soft). Lens Chip → `ActiveContextTree` is the chat-header control.

- `2026-07-15` — claude: **Chat↔scope "ask on mismatch" pre-send gate shipped.** When the sidebar's active scopes differ from a chat's durable tags, `smartExecute` now asks (3-way dialog: use current / combine / keep; dismiss cancels the send) instead of silently union-stamping; an empty sidebar sends the chat's own tags as `scope_ids` instead of nothing. New: `redux/thunks/conversationScopeGate.ts`, pure logic + jest in `utils/scopeMismatch.ts`, `components/dialogs/scope-mismatch/` (imperative host, mounted in `app/Providers.tsx`); `executeInstance` gained `scopeIdsOverride` (assembles `scope_ids` from it and skips the post-send union sync). See §"Live delivery path" point 5.
- `2026-07-15` — **Slug UI aligned with the live database invariant.** Scope types derive their client preview/default from `label_plural`, while scope slugs remain stable on a name-only rename unless explicitly edited. Context-item `slug` is server-maintained from its underscore-style `key`; the UI no longer authors or sends it and displays `key` as the immutable identifier. The live database backfill, constraints, and `ensure_slug` trigger remain the final authority for every writer.

- `2026-07-15` — **Slug invariant hardened before the database NOT NULL rollout.** Every normal scope-type, scope, and context-item creation path now passes through a shared Redux thunk that derives a kebab slug from its user-authored label/name when a caller omits one and rejects an un-sluggable name. The editable slug forms now regenerate rather than clear a slug; the system-context admin API (the privileged direct-insert exception) stamps slugs on scope types, scopes, and context items too. The database remains the upcoming final authority for NOT NULL and shape/uniqueness constraints.

- `2026-07-15` — codex: **Template-backed per-scope tables and compact agent scope-system automation shipped.** Added organization-scoped UDT dataset templates, immutable template-bound schemas, and `context.scope_dataset_instances`; provisioning covers both existing and newly-created scopes and writes canonical dataset fences through the one context-value writer. Context-item add/settings forms now select a per-scope table template. Aidream adds one `scope_system` tool (`inspect` / transactional ordered `apply`) that manages scope types, item definitions, scopes, values, and table templates by stable keys without tool-name sprawl.

- `2026-07-15` — codex: **Scope value-entry keyboard flow audited end-to-end.** Dense scope fields now expose one primary sequential Tab stop; F6 enters an item-specific auxiliary-action toolbar (definition/page/advanced editor plus voice/device/more), Left/Right/Home/End rove it, and F6/Escape returns to the value. Labels/descriptions are programmatically associated across value, edit, advanced, and create forms; panel forms autofocus their first editor and return focus; the duplicate mid-form create actions were removed. Reference pickers now autofocus their active sub-picker, support Enter submission for URLs, and use Arrow/Home/End roving result lists instead of placing every candidate in the Tab sequence.

- `2026-07-15` — **Keyboard flow hardened across organization scope-type and context-item management.** Full-page and quick-panel forms now follow DOM order with explicitly associated labels, submit semantics, and predictable first/return focus; read-only current-value references no longer interrupt the editable field sequence. Scope/context-item list rows expose their primary destinations to keyboards, hover-revealed actions become visible on focus, and keyboard reorder actions have item-specific names. The canonical scope color picker replaces the pointer-only legacy palette in the full scope-type editor.
- `2026-07-15` — **Scope/context SECURITY DEFINER boundary hardened live.** Scope/type/tree/value/history/template reads and mutations now prove `iam.has_org_access` (or equivalent resource ownership) for caller-selected org/scope/type IDs; anon execution was removed from non-guest scope surfaces. Legacy server-only entity-scope/context resolvers were removed from browser ACLs. Personal-org helpers now accept only `auth.uid()` outside service-role flows. The integrity recurrence guard also detects scope-resource parameters and org DML lacking a visible authorization predicate.
- `2026-07-14` — cursor: **`ContextItemAddForm` densified onto canonical controls — tiny labels + oversized toggle are gone.** Root cause: hand-rolled `text-[10px]` labels next to a padded custom segmented control that dwarfed the `ReferenceConfigFields` type chips, plus default `h-9`/`h-10` Inputs/Selects mixed in. Fix: `EntryModeToggle` rebuilt on `ToggleGroup` at `h-7 text-xs` (same footprint as `Button size="sm"`); `ReferenceConfigFields` chips → `Button size="sm"` + labels → canonical `Field`; `ContextItemAddForm` rebuilt with `Field` everywhere, `SelectTrigger size="sm"`, Inputs at `h-7`, Direct toggle + primitive-type Select on one row (no stacked giant/tiny pair), nested bordered reference card removed. `labelClassName` escape hatch on `ReferenceConfigFields` deleted — house `Field` is the only label path.
- `2026-07-16` — claude: **Dense `ContextTree` promoted to Surface A** (`active-context/context-tree/`). Replaces the old compact `ActiveContextTree` implementation. Cascade-up on select (scope ⇒ type + org); single-org enforce; lazy projects/tasks/fields. Wired via `applyDenseSelectionToRedux` / `setFullContext`. Demo `/demos/scopes/context-lab/dense` re-exports the promoted module. Smart Input: single-row / landing fold docs+tree+Enter/auto-clear into `+` (`foldToolbarExtras`); stacked keeps dedicated Layers button (no duplication).
- `2026-07-13` — claude: **Nine new context-item / agent-variable value types shipped end-to-end (datetime, time, email, url, phone, percent, color, markdown, currency).** Adding a type touches BOTH type ladders — `ContextValueType` (storage) + `VariableComponentType` (input widget, shared with the Agent Builder) — plus storage, resolver, and agent writeback. DB (`migrations/ctx_extended_value_types.sql`, applied + ledgered + `pnpm db-types`): 9 new `public.context_value_type` enum members + two new columns `context.context_item_values.value_timestamp timestamptz` / `value_time time` (every other type rides an existing column — percent→`value_number`, currency→`value_json {amount,currency}`, the rest→`value_text`), threaded through the full writer/reader chain (`context.write_context_value`, `set_scope_context_value`, `set_context_value`, `get_scope_context`, and BOTH cell CASE ladders in `resolve_full_context`). FE: `VALUE_TYPE_CONFIG`, `VARIABLE_COMPONENT_TYPES` + `COMPONENT_TYPE_META`, `componentToValueType`, `buildScopeValuePayload` (column routing), `ScopeContextRow`/`setScopeContextValue` plumbing; new widgets `ScalarVariableInput` (native inputs for datetime/time/email/url/phone/percent/color + markdown editor) + `CurrencyVariableInput` (amount + currency-code) wired into `VariableInputComponent` (so they render in the Agent Builder AND context cells); `ContextValueInput` gains native-input/percent/color branches; `ContextValueDisplay` gains a `valueType` prop + rich renders (mailto/link/tel/color-swatch/rendered-markdown/percent/formatted datetime+currency), threaded through `ContextValueRow` at all three display callers (`ScopeDetailView`, `ActiveContextLayersPanel`, `ContextLayerBodies`). `EditScopeValueSheet`'s duplicate per-type save switch was collapsed onto the shared `buildScopeValuePayload` (it had been routing datetime/time/percent/currency to `value_text`). aidream: `context_writeback.py` `_CTX_ITEM_VALUE_COLUMN` + ORM models regenerated. Verified live end-to-end: authored a datetime item → saved `2026-09-15T14:30` → `value_timestamp` (`value_type=datetime`), field live-refreshed with no reload; currency widget (amount + USD) rendered live; zero console errors. **Same change also flattened the context-item editor UI** (killed the card-in-card value-shape box + nested picklist/options boxes → flat hairline-divider sections in `ContextItemSettingsForm` + `CustomComponentConfigurator` + `PicklistBindingEditor`; fixed the crowded "Value type" label).
- `2026-07-13` — claude: **Editing a context item now refreshes the scope page instantly + the editor shows current values across scopes.** Two fixes on the scope-detail editor (`features/scope-system/`). (1) **Stale-after-edit bug:** `scopeValuesSlice.byScope[scopeId]` rows are a DENORMALIZED join of the item DEFINITION (`value_type`, `custom_component`, `display_name`, `description`, …) + the per-scope value, but `updateContextItem`/`deleteContextItem` only patched `contextItemsSlice` — so after the edit drawer closed, `ScopeFieldInput` kept rendering the old type/component/label until a manual reload. Added mutation-driven `extraReducers` in `scopeValuesSlice` reacting to `updateContextItem.fulfilled` (re-flow the definition fields into every cached row across all scopes; value_* columns untouched) and `deleteContextItem.fulfilled` (drop the row everywhere) — the "successful mutations patch the tree in place" invariant (#5), now honored for the item→value denormalization too. (2) **Current-values preview:** new `ContextItemCurrentValues` (`features/scope-system/components/forms/`) rendered inside the shared `ContextItemSettingsForm` right after the value-shape box — a compact read-only "Current values across {label} (N)" list (scope name → `ContextValueDisplay` cell, or "Empty"), so a type/component change is an informed one (existing values don't auto-convert). Reuses the cached `byScope` rows and only fetches scopes not already loaded. Verified live end-to-end (Goal scope: edited display name → left field label updated with no refresh; preview showed both goals' Target-date values; reverted; zero console errors).
- `2026-07-15` — **Restored scope soft-delete after migration-order regression and enforced structural ACLs.** The later `scope_rpcs_org_membership_guard.sql` had overwritten the 2026-07-11 `delete_scope` / `delete_scope_type` bodies with hard `DELETE` again. `20260715054500_restore_scope_soft_delete_admin_acl.sql` restores recoverable `deleted_at` updates, retains assignments/context values, revokes anonymous execution, and requires an exact active organization owner/admin (or service role). Rolled-back live verification proved ordinary members receive `42501`, owner deletion retains both scope and type rows with `deleted_at`, and no cascade fires. All confirmation copy now says archive/retained-for-recovery instead of permanent deletion.
- `2026-07-11` — claude: **Scope tables now actually versioned + soft-delete — the dataloss path is closed (Tier 1 canonicalization).** `context.scopes` / `scope_types` / `context_items` were registered in `platform.entity_types` with `is_versioned=true` + `has_soft_delete=true`, but the machinery was never installed: **no `_history` trigger** (so `history.row_versions` had 0 rows for any scope token — the flag was cosmetic) and **no `deleted_at` column** (so soft delete was impossible), while `delete_scope` / `delete_scope_type` did a **hard `DELETE`** whose `ON DELETE CASCADE` wiped every `context_item_value` (the cell data), `context_value_refs`, child scopes, dict entries and rag suggestions — unrecoverable. Migration `ctx_scope_tables_versioning_soft_delete.sql` (applied + ledgered + `pnpm db-types` + aidream `db/generate.py`): (1) added `deleted_at`/`version`/`updated_by`/`metadata`; (2) attached the canonical `platform._version_capture` `_history` trigger to all three → every insert/update/delete/soft-delete now lands in `history.row_versions`; (3) rewrote `delete_scope` + `delete_scope_type` as **soft delete** (set `deleted_at`; `context_items` keep their existing `is_active` soft-delete) so no cascade fires and everything is recoverable; (4) added `deleted_at IS NULL` filters to every scope/scope-type reader (`list_scopes`, `get_scope_tree`, `search_scopes`, `list_scope_types`, `get_org_structure`, `get_user_scopes`, `get_entity_scopes`, `get_scope_context`, `get_user_full_context`, `resolve_full_context`) so soft-deleted rows vanish from the UI, pickers, and agent context. `context_item_values` was left untouched — it is already an append-only versioned table (`version`/`is_current`/`change_summary`). Verified live end-to-end via a rolled-back transaction: insert→soft-delete produced `history_rows=2 ops=[INSERT,SOFT_DELETE]` and `list_scopes` excluded the deleted row. **Tier 2 (still open):** replace bespoke RLS with `iam.apply_rls`, add `visibility` + base FK constraints + sharing registry, backfill `context_items.organization_id`, and reach `canonical_certify_ok` — a separate tested pass.
- `2026-07-11` — claude: **Inline delete on the scope-type list page (`ScopesList`) for both individual scopes and context items.** Delete already existed backend-side (`deleteScope` → hard `delete_scope` RPC; `deleteContextItem` → soft `is_active=false`) and via thunk, but was only reachable from the `ScopesRouteHeader` action row _inside_ a single scope's detail page — invisible from the list where users look. Added a `canManage`-gated trash button per scope row (in the sticky Name cell, hover-revealed, `stopPropagation` so it doesn't trigger the row's navigate) and per context-item row (beside the existing Edit pencil), both driven by the imperative `confirm()` (`components/dialogs/confirm/ConfirmDialogHost`) with accurate destructive copy — "permanently … cannot be undone" for scopes (hard delete, recursive over child scopes + associations), "values are retained but hidden" for context items (soft delete). Busy state per-row via `deletingScopeId`/`deletingItemId`; slice reducers already drop the row on `fulfilled`. Not live-verified end-to-end (test browser wouldn't accept the login form and the admin account lacks the `titanium` org; refused to destructively delete real scope data to test) — typecheck clean, reuses the proven confirm+thunk pattern.
- `2026-07-11` — claude: **`ContextValueInput`'s default text branch no longer clashes visually inside dense forms.** New `compact` prop swaps `ProTextarea` (its `bg-transparent`, no-shadow style, plus mic/AI-actions/stats chrome — none of it appropriate for a quick, optional scalar value) for a plain `Textarea` styled flush with a same-row `Input`/`Select` (matching border/bg/shadow, 16px inline font-size per the iOS-zoom rule, monospace only for object/array). Wired into `ContextItemAddForm`'s "Value for this one" field, the one visibly broken in review (oversized, transparent-background, unstyled-looking box next to filled/bordered `Input`s). Also fixed a label-size mismatch: `ReferenceConfigFields` hardcoded `text-xs` labels while `ContextItemAddForm`'s own labels are `text-[10px]` — added a `labelClassName` prop (default `text-xs`, unchanged for `ContextItemSettingsForm`) so `ContextItemAddForm` can match its own convention. Verified live in both the inline add-row (matter detail page) and the "New item" tab of the Add/Edit/Manage Context Item window panel.
- `2026-07-11` — claude: **"Set Context Value" quick action shipped — message content → any scope's context cell, in one click.** New menu item in the chat message Actions section (`messageActionRegistry.ts`, right after "Create Task"), opening a new `setContextValueWindow` `WindowPanel` (`features/window-panels/windows/scopes/SetContextValueWindow.tsx`, registered in `overlay-ids.ts` + `catalogue.ts` + `OverlayController.tsx`) built entirely by mirroring the existing `Save as → Note` family (`useQuickNoteSave`/`QuickNoteSaveCore`/`QuickNoteSaveWindow`) onto context cells instead of notes: new `useSetContextValue` hook, `SetContextValueCore` UI (same content-refine toolbar + Append/Overwrite radio + overwrite-confirm/diff-preview), and a new `ScopeContextTargetPicker` (`features/scopes/components/quick-assign/`) — the missing "Org → Scope Type → **specific Scope instance** → Context item" cascade (`ContextItemPicker` stops at the item definition, `ReferenceValuePicker`'s scope sub-picker only picks a scope as a REFERENCE VALUE, neither picks "the scope I'm about to write this cell on"). Only text-shaped `value_type`s (`string`/`object`/`array`/`document`) are selectable in the item dropdown — reference/boolean/number/date items render disabled with a "text only, for now" hint, since append/overwrite has no defined meaning for those shapes. No new backend surface: writes go through the existing `setScopeContextValue` → `set_scope_context_value` → `context.write_context_value` chokepoint. Verified live end-to-end (menu → window → Doe v. CSV Pharmacy matter → "QME Report Clean Content" → Append → DB row version-bumped with `change_summary="Appended from a chat message"`, then reverted).
- `2026-07-11` — claude: **Direct Entry vs. Reference is now the first decision everywhere a context-item value shape is chosen — the flat type-list anti-pattern is gone.** New shared primitives `EntryModeToggle` + `ReferenceConfigFields` (`features/scopes/components/reference/`): a two-option "Direct Entry | Reference" control, then either a primitive-type picker or the full reference config (allowed types / max items / allowed scope types) shown immediately, never behind a second toggle. `ContextItemSettingsForm` (edit) had this shape already (a `Switch` + hand-rolled reference block) but not framed as the first decision or shared — refactored onto the two new primitives, moved to sit right after the name/slug fields. `ContextItemAddForm` (create) had regressed to a flat `<Select>` listing `String | Number | Boolean | Date | Object | Array | Document | Reference` side by side — the exact pattern this whole system replaced — fixed onto the same two primitives (its primitive-type `<Select>` explicitly excludes `"reference"` from the option list now; full custom-component authoring at create time is still a backend gap, see `create_context_item`'s missing `p_custom_component` param). Also fixed: System Context admin page's (`app/(admin)/administration/scopes-context/system-context/page.tsx`) own local `VALUE_TYPE_OPTIONS` had the same flat "Reference" entry, and it was additionally dead code — `ContextValueInput` requires `referenceConfig`+`scopeId` for `value_type="reference"` that this dialog never supplied, so picking it silently produced "Reference picker unavailable." Removed the option there until that page gets `ReferenceConfigFields` wired in. Also deleted the unrelated dead `REFERENCE_TYPES` constant in `features/agent-context/constants.ts` (legacy prompt-system entity list — `recipe`/`workflow`/`ai_agent`/`prompt`/`context_item` — zero importers, easily confused with the canonical Matrx envelope `REFERENCE_TYPES`).
- `2026-07-15` — claude: **Stale `ctx_scope_assignments` DB-reader claim corrected.** This doc previously claimed `get_entity_scopes`/`set_entity_scopes`/`list_entities_by_scopes` and `resolve_full_context*` still read `ctx_scope_assignments`; live-verified against `common-docs/systems/access-architecture/FEATURE.md` (§7 G11), **zero database functions reference the table** (`pg_proc.prosrc` sweep) — migration to `platform.associations` is fully complete, table is graveyarded with no live readers of any kind. Fixed the M2M-table row (§"DB tables"), the "contributing scopes" resolution-flow claim, and the Surface A/B codepath note.
- `2026-07-11` — claude: **`ContextStatusButton`'s deprecated `size`/`buttonClassName` props removed — they were silently broken.** `buttonClassName` didn't add a class to the icon, it _replaced_ the status-color class (`buttonClassName ?? glassClassName`, not `cn(...)`), and `size` was a no-op destructured-and-discarded shim — TapTargetButton's pill geometry is fixed and was never configurable. Two callers had leaned on these to force the tap-pill into spots it doesn't fit (a full-width metadata-list row in `PdfStudioPagesMeta`'s `PdfFileContextRow`, a 24px tab-strip icon in `NoteContextSection`'s `NoteContextStatusIcon`) and rendered visibly broken (oversized floating pill, stretched/discolored icon). Both rebuilt as small local triggers directly on `ContextAssignmentPopover` (no `ContextStatusButton`) matching their host's actual layout — `PdfFileContextRow` is now a `<dl>` row (label left, colored value+icon right, same envelope as Pages/Characters/Status/Updated below it); `NoteContextStatusIcon` is now a 24px icon button matching the tab strip's other action buttons (`actionBtnClass`). **Pattern:** `ContextStatusButton` only fits header/toolbar tap clusters — anywhere else, compose `ContextAssignmentPopover` + a bespoke trigger instead of fighting its fixed geometry via className overrides.
- `2026-07-10` — **`ContextStatusButton` is a tap button.** Renders `ShieldCheckTapButton` / `ShieldAlertTapButton` (Lucide paths) with `variant="glass"` (default — status via icon color) or `variant="solid"` (filled amber unset / green set). Fits flush in shell header tap clusters; `showScopeLabel` maps to the tap `label` prop.
- `2026-07-11` — claude: **Context reference cells shipped (fence-as-value).** New §"Context reference cells" documents the full slice. Schema: `allowed_reference_types text[]` / `max_items int DEFAULT 1` / `allowed_scope_type_ids uuid[]` on `context.context_items`; new reverse-index table `context.context_value_refs`; new internal writer `context.write_context_value` (parses + validates the fence, inserts the version, indexes refs) backing thin `set_context_value`/`set_scope_context_value` wrappers; new public RPC `list_context_value_refs` → `scopesService.listReferencingValues(refType, refKey)` (`migrations/ctx_context_reference_cells.sql`, applied + ledgered + `pnpm db-types`). Envelope: `url` added to `REFERENCE_TYPES` (`buildUrlReferenceFence`/`buildMultiUrlReferenceFence`, chip + resolver, opens directly in a new tab — never server-resolved). Canonical primitives (`features/scopes/components/reference/`, `features/scopes/utils/referenceCell.ts`): `ContextValueInput` (the value-entry switch for every `value_type`), `ReferenceValuePicker` (per-type "Add" sub-pickers: file/url/scope/generic-entity-search), `ContextValueDisplay` (fence → `MatrxEnvelopeBlock` chips) + `referenceCellSummary()` for dense non-JSX contexts. Wired into every editor (`ScopeFieldInput`, `EditScopeValueSheet`, `ContextItemSettingsForm` — reference-config UI when `value_type='reference'`) and every read-only display surface (`ScopeDetailView`, `ActiveContextLayersPanel`, `ContextLayerBodies` drawer body, `OrgHomeScopeSection`/`ScopesList` grid cells, `KgSuggestionRowItem` current-value diff). `resolve_full_context` needed no change — its `value_text IS NOT NULL` branch already carries the fence straight through. First real consumer: Matters "QME Report" (`file`, `max_items=10`, legacy bare-uuid value backfilled into a fence; `migrations/ctx_matter_qme_report_reference.sql`) + Matter Client/Practice Area backfilled to `scope` + their scope-type filter.
...
- `2026-07-03` — claude: **The association family grew its row face + universal attach (built for the War Room, canonical for all).** (1) **`AssociationList`** (`components/associations/AssociationList.tsx`) — the row-mode sibling of `AssociationCardGrid`: role-grouped rows (new `contentRole` overlay field + `CONTENT_ROLES` meta on `entityRegistry.ts`, rescued from the deprecated catalogue), resolved titles, open (`hrefFor`), pin toggle (edge `metadata.pinned`), detach, per-token "+" pickers, and a **pluggable `ContainerResourcesAdapter`** (default `useContainerLinksAdapter`; war-room supplies its own for single-active/position semantics) + a per-token `renderRow` override (war-room file rows keep media thumbnails). (2) **Universal attach:** `UniversalAssociationPicker` + `hooks/useUniversalEntitySearch` + `searchCandidatesAcrossTokens` (client fan-out over every listable token, RLS-correct, recents via `ues_list` on empty query); `AssociationPickerSheet`'s body extracted as reusable `TokenCandidateList`. (3) **Registry growth:** overlay entries `udt_document`/`working_document`/`studio_session`/`data_store` + `hrefFor` routes (files/f, chat, agents, data, workbooks); NEW overlay field **`listCandidates`** — a candidate-source override for tokens whose schema isn't client-readable (`data_store` → `features/rag/service/dataStoreCandidates.ts`); `data_store` token registered in `platform.entity_types` (`migrations/register_data_store_entity_type.sql`). (4) **New canonical services:** `service/associationEdges.ts` (`isContentSourceEdge` — THE content-vs-structure classifier; whitelists banned), `service/entityTitles.ts` + `hooks/useEntityTitles.ts` (batched title resolution — UUIDs never render), `registry/entityContentAdapters.ts` (per-token agent access hints + client readers + `readEntityRowGeneric`). (5) **Deleted** the consumer-less `EntityAssociator.tsx` + `AssociationAddControl.tsx`. First consumer: the War Room resources overhaul (`features/war-room/FEATURE.md` 2026-07-03).
- `2026-06-29` — claude: **Canonical association core hardened + org grid cut over; subagent campaign armed.** (1) **Registry conventions:** `entityRegistry.ts` now treats owner/org as constants (`DEFAULT_OWNER_COLUMN="created_by"`, `DEFAULT_ORG_COLUMN="organization_id"`, verified live across all 17 cardable tables), dropping the per-token `ownerColumn:"user_id"` that caused the `column files.user_id does not exist` (42703) error; overlay expanded to all 17 cardable canonical tokens (agent/agent_shortcut/app/skill/workflow/content_template/file/folder/transcript/dataset/workbook/note/conversation/flashcard_set/quiz_session/project/task). `associationCandidates.ts` gained loud RLS-only recovery on a 42703. (2) **Org surface = cards:** `OrgWorkspace` "Resources" now renders the new `AssociationCardGrid` (one card per listable token) instead of the permissions/`container_resource_counts` grid; `useOrgResourceInventory`/`OrgResourceRoleSection` retired from the org home. New reusable `components/associations/AssociationCardGrid.tsx`; `useContainerLinks` gained `totalCount`. (3) **`resource-catalogue.ts` deprecated** for display/association (banner added) — survives only for the `iam.permissions` sharing surface. (4) **`OrgShareReviewCard` PGRST205 fixed:** title hydration routes through the schema-qualified `getShareableResource()` (`agent` → `agent.definition`, not `public.agent`); kept as access-control moderation. (5) **Subagent playbook** `.claude/skills/canonical-associations/SKILL.md` (+ `WORK-QUEUE.md` backlog) — the repeatable recipe (replace-an-M2M / put-a-card / canonicalize-a-table-ref) and the load-bearing `platform.associations` vs `iam.permissions`/`iam.memberships` boundary.
- `2026-06-29` — cursor: **Association vocabulary made type-safe + the edge handler hardened.** (1) **Generated entity-token vocabulary:** `types/generated/entity-types.generated.ts` (216 tokens) is now produced by `scripts/generate-entity-types.ts` from `platform.entity_types` via a new public SECURITY-DEFINER RPC `entity_types_list()` (`migrations/entity_types_list_rpc.sql`, applied + ledgered). Exposes `EntityTypeToken` (full FK-valid union), `isEntityTypeToken`, `ENTITY_TYPE_TOKENS`, `ENTITY_TYPE_METADATA`, curated subsets. Wired into `pnpm gen:entity-types` / `pnpm check:entity-types` (drift) / `pnpm sync-types`. `features/scopes/types.ts` re-exports them; `AssociationTargetType` is now a curated list proven valid with `satisfies readonly EntityTypeToken[]`. (2) **Handler guards:** `service/associationGuards.ts` (`checkToken`/`checkUuid`/`checkUuidArray`) run in EVERY `associationsService` method before the RPC — a bad token or non-UUID id becomes a loud `invalid_argument` result, not an opaque PG `22P02`/`23503`. Kills the "agent passed a cute string id" + "guessed token" bug classes. (3) **Typed composite wrappers:** `service/associationHelpers.ts` (`linkEdges`/`linkOneToMany`/`linkManyToOne`/`replaceTargets`/`linkCreated`/`unlinkEdges`). (4) **Graveyarded the two buggy task RPCs** `create_task_with_association` + `associate_with_task` (`migrations/graveyard_buggy_task_assoc_rpcs.sql`, applied + ledgered) — both hand-rolled a 4-column ON CONFLICT that matched no unique index (the real key is the 5-tuple incl. `role`) → 42P10. `taskAssociationsSlice` thunks now insert the task then wire edges through the chokepoint; `get_task_associations` kept. **Open hole flagged:** phantom tokens `agent_app`/`cx_message`/`cx_conversation`/`user_file`/`chat_block` (used in code, not registered) — see FOUND_DEFECTS; cleanup-hunt pending.
- `2026-06-26` — cursor: _\*ctx_* tables moved to `context` schema_* — `scopesService` (and all other FE chokepoints) now reach `context.scope_types` / `context.scopes` / `context.context_items` / etc. via `contextDb(supabase)` (mirror of `workspaceDb`). `pnpm db-types` includes `--schema context`; generated row aliases in `features/scopes/types.ts` point at `Database["context"]["Tables"]`.
...
- `2026-07-01` — claude: **Hydration-safe `initialSelection` in `ContextAssignmentField`.** Selection state no longer seeds from `initialSelection` in `useState` (persisted Redux can differ between SSR and first client paint — caused `ContextSelectionSummary` Org row mismatch on `/rag/search`). Restores from `initialSelection` in a post-mount effect; active-mode `onApplyActive` waits until that sync completes so Redux isn't re-dispatched redundantly.
...
- `2026-06-30` — claude: **Layer B durably warm-cached (`scopesTreePolicy`).** The canonical scope tree (`state.scopesTree`: organizations + order + `treeFetchedAt`) is now a warm-cache citizen of the sync engine — persisted to IDB→localStorage keyed by identity, rehydrated BEFORE paint, so a hard reload restores the tree and `ensureScopeTree()` returns cached without a network round-trip. No `staleAfter`/`remote.fetch` (the tree changes rarely; refresh stays explicit via `scopeTreeInvalidationMiddleware`; cold boot still uses the `ensureScopeTree` fetch). Broadcasts `treeFetchFulfilled` + `scopesReset` for cross-tab coherence. Only the tree is persisted — the lazy per-entity caches (tasks/orphans/entityScopes/associations/categories) are deliberately session-scoped. Registered in `lib/sync/registry.ts`; REHYDRATE handled in `scopesSlice` extraReducers (adopts the cache only if no live fetch already won the race).
- `2026-06-30` — claude: **Three-layer runtime context model documented + Layer A rounded out.** Codified the A/B/C split (active/passive context `appContextSlice` · reference tree `agent-context/redux` · durable object assignment `platform.associations`) as the canonical framing in `docs/knowledge/scope-model.md` and the `context-assignment` skill — the rule being a user _action_ reads the explicit UI selection (Layer C), never the loaded active context (Layer A). Added `appContextSlice.active_scope_type_ids` (+ `setActiveScopeTypes` Surface-A action, selector `selectActiveScopeTypeIds`, ESLint write-gate) for the rare "whole scope type active, no instance chosen" case (e.g. activating "Clients"/"Departments"/"Kids" generally). Fixed stale "exactly one scope per type" docs (multi-select since 2026-06-12). Companion: condemned the bespoke `agent_surface` agent↔surface binding (`features/surfaces/FEATURE.md`) — it must move to canonical associations; it conflated Layer A passive context with Layer C user selection and modeled project/task/scope as single FKs.
...
- `2026-06-29` — claude: **Canonical association-card system** — the generic, token-driven "what's attached to this container, and add more" primitive, built entirely on `platform.associations` (no new tables/RPCs). New parts under `features/scopes/`: `registry/entityRegistry.ts` (`getEntityInfo` — generated `ENTITY_TYPE_METADATA` + FE overlay for icon/`titleColumn`/`ownerColumn`/`hrefFor`; the canonical replacement for the hand-curated `features/organizations/resource-catalogue.ts`), `service/associationCandidates.ts` + `hooks/useAssociationCandidates.ts` (registry-resolved reader of attachable `{id,title}` records), `hooks/useContainerLinks.ts` (container-centric incoming-edge counts + attach/detach), and `components/associations/{PrimaryEntityContext,AssociationCard,AssociationPickerSheet}.tsx` (page provides the primary entity once; each card resolves icon+label+live count from a token, plus button → adaptive Sheet/Drawer picker). **Direction is canonical + singular** — resource = edge SOURCE, container = TARGET (`task→organization`), consistent with scope-tagging; so `organization` was added to `ASSOCIATION_TARGET_TYPES` (+ the `TYPE_LABEL_PLURAL`/`TARGET_LABEL` maps). First mounted on the org home (`OrgWorkspace` — Tasks + Files); the legacy permissions/`share_resource_with_org` resource grid is a parallel system to migrate onto these cards next. New §"Association cards". All new code type-checks clean.
...
- `2026-06-06` — Scope UI: permissions model + safety fix + not-found states (user-reported bugs). **Safety:** the individual scope page (`ScopeDetailEditor`) no longer shows "Edit <Type> Settings" — that button edited the org-wide _dimension_ (all scopes of the type), so exposing it on one instance let a non-admin reconfigure the whole org's handling of that type. It's removed; type editing lives only on the scope-type page, gated. The scope page now has an org›type›scope breadcrumb for "up" nav. **Permissions** (UI layer): `canManageSettings(role)` (owner/admin) gates all _structural_ actions — edit scope type, add/edit/delete/reorder context items, add-field-from-scope-page, delete scope type, delete scope; _instance_ actions (create scope, edit scope name/desc/slug/settings, set values) stay open to members. Each scope page now fetches the caller's org role and passes `canManage` down. (Server-side RLS/RPC enforcement is the necessary next layer — UI gating alone is not a security boundary.) **Not-found:** mistyped scope/type/item slugs previously spun forever; new `ScopeNotFound` card shows once the relevant data has loaded but nothing matches (new `selectScopeTypesLoadedForOrg` / `selectScopesLoadedForType` + existing `selectItemsLoadedForType` distinguish loading from missing).
