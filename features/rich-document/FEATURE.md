...
**Path** _(live since RC-B6: the chat ⋯ menu is `RegistryActionMenu` over this registry, built by `chat/chatMessageActions.ts`; `AssistantActionBar` keeps only the chat inline chrome — thumbs state, speaker, pencil, dialogs)_
...
9a. **ONE math dialect, owned by the core.** `components/markdown-core/math-normalizer.ts` converts `\(…\)` → inline, `\[…\]` → display, unambiguous `$…$` → inline (pandoc rule + a TeX signal, so `$5 and $10` stays currency — the predicate is `isSingleDollarMath` from `@ai-matrx/content-ir/source`, shared with the source tokenizer so editor islands and rendered math can never disagree) and never touches code spans, fences or existing `$$…$$`. Every math-capable `MarkdownCore` preset (`math`, `gfm-math`, `rich`, `chat`, `message`) runs it and uses the shared `REMARK_MATH_OPTIONS` / `REHYPE_KATEX_OPTIONS`; the file previewer and `features/math` import the same exports. Never convert math delimiters in a renderer's own preprocess. Guard: `components/markdown-core/__tests__/math-parity.test.tsx` (every entry point, same math) + `math-normalizer.test.ts`.
...
| 4     | Chat parity migration — replace `AssistantActionBar`                                                                                                                                                                                                                                                                                                                                                                                                                                                           | ✅ Done (RC-B6) — every chat action lives here; chat renders `RegistryActionMenu` from this registry. Guards: `actions/__tests__/chatActionParity.test.ts`, `__tests__/chatStudioSurfaceParity.test.tsx` |
...
| 8     | Cleanup — delete `messageActionRegistry.ts` shim, delete `AssistantActionBar.tsx`, audit remaining `BasicMarkdownContent` imports                                                                                                                                                                                                                                                                                                                                                                              | ✅ Done (RC-B6) — `messageActionRegistry.ts` (both copies), `MessageOptionsMenu.tsx` (both), the cx-chat duplicate `AssistantActionBar`, and the dead `message-actions` slice deleted. The `/chat` `AssistantActionBar` stays as the inline host (verdict state, speaker, dialogs) |
...
- `2026-09-26` — claude (ALC-15 S2/S4): **the registry is ONE provider of the Alchemy action registry.** `actions/registry.ts` is deleted; `actions/provider.ts` holds the list (`registerAction` REFUSES a duplicate id outside fast refresh) and every eligibility answer comes from the package's `computeEligibility` over a `ClickTarget` (`richDocumentClickTarget`). The bar, mini-bar and ⋯/phone sheet are the package layouts (`@ai-matrx/alchemy/react/bar|overflow|sheet`); `MENU_STRUCTURE`'s submenu names become the actions' named sections. Chair rulings recorded: (1) only the universal verb strip greys an unavailable verb, with its sentence as the tooltip; (2) bar-only (primary) actions now appear in every layout — ⋯, right-click, sheet, palette — lossless in the right direction; (4) a row that was disabled-with-reason is now ABSENT. "Copy for Google Docs" + "Copy for Word" (byte-identical) are ONE "Copy formatted" (`copy-formatted`). Announcements and Preferences left the registry (app navigation; the user menu has them). A raw source carries `title` (`deriveContentTitle`), which is how `components/content-actions` (ContentActionBar + contentActionRegistry, 24 importers) folded into `RichDocumentActions`; host buttons became `actions.extra` via `actions/hostActions.ts`. Guards: `actions/__tests__/provider.test.ts` (red on 5 plants).
- `2026-09-25` — claude: **RC-B6 — one action registry, and the actions the best AI apps have.** Every chat ⋯ action (≈50) moved in with its behavior (source-aware: write-back actions declare their sources; host-dialog actions are absent without a host, never dead). New for every source: copy as Markdown / plain / rich text / HTML source, table → CSV / TSV / data table (`markdownTable.ts`), download as HTML / PDF (`@ai-matrx/print`), save as flashcard (reader writes the question; deck "Saved cards"), ask a follow-up / quote into chat (a `quoted_passages` CONTEXT entry — never composer text, THE USER-INPUT LAW). Host dialogs (convert, save table, flashcard) live in `hosts/DocumentDialogsHost.tsx` behind ONE dynamic edge. ProTextarea's "…" menu renders this registry (`variants/RegistryActionList.tsx`) and its Clean up / Help / Custom agent are registry actions (`handlers/ai.ts`) reached via `callbacks.onRequestTextAgentAction`. `/markdown-studio` shows every action for a loaded chat message (same builder as /chat). Not built: translate and explain-this need mandates that do not exist (agents never author agents); .docx download needs a docx generator. Tests red→green: parity 28 failed → 96 pass; studio surface red on the old exclusion list; expanded set 12 failed → 16 pass.
- `2026-09-25` — claude: **RC-B6 verifier round.** Right-click is the same registry (`RegistryContextMenu`; v3 draws copy/export/save, the rest ride as "Document"), and unavailable rows are absent in every layout (`pruneUnavailable` in context-menu-v3). Read-only is a property of the source (`ContentSource.readOnly`): the registry drops every `writesSource` action. Both /chat bars are gone — thumbs (live verdict), read-aloud, delete/fork, edit history are registry actions with dialogs in `hosts/`. Clean up / Help / Custom agent on a rendered document open `hosts/DocumentAgentReview.tsx` (diff, splice save through the source adapter, protected islands refused). PDF and Word use the vector `@ai-matrx/print/document` export; text leaving its home goes through `unwrapKindEnvelopes`. Quotes are a per-turn context key (`PER_TURN_CONTEXT_KEYS`), consumed by the send path.
- `2026-09-26` — claude: **RC-B6 round 2 — one menu tree.** Every menu (⋯, right-click, ProTextarea "…", the phone sheet) renders `buildMenuTree(registryMenuActions(…))`: context-menu v3 draws the registry tree itself (no Convert/Export/Document regrouping), folds the agent libraries into `AI_SUBMENU_LABEL` via `withAiSlot`, and the mobile sheet renders the desktop model. A ⋯ whose content already has a right-click menu opens THAT menu (`useRegistryMenuSource`). `MENU_ALIASES` keeps one Regenerate. Outbound writers take `contentForDestination(ctx)`. Guards: `__tests__/oneMenuTree.test.ts`, `actions/__tests__/destinationContent.test.ts`.
- `2026-09-24` — claude: **The single-`$` rule moves into `@ai-matrx/content-ir/source` (0.12.0).** `math-normalizer.ts` imports `isSingleDollarMath` instead of owning a copy, because the new offset-preserving source tokenizer (RC-B3) must lock exactly the `$…$` spans the renderer treats as math. Behaviour unchanged: `math-normalizer.test.ts` + `math-parity.test.tsx` 65/65. Real-data gate for the tokenizer: `npx tsx scripts/check-source-roundtrip-corpus.ts` (read-only, every stored row).
- `2026-09-23` — claude: **One math dialect across every entry point.** Math rendered differently depending on the door: `chat`/`rich` disabled single-`$`, `math`/`message`/the file previewer enabled it (so "$5 and $10" became a formula there), `\(…\)`/`\[…\]` conversion lived only in the Basic/Configurable preprocess (twice, and `\[…\]` actually rendered INLINE because `$$x$$` alone on a line is not a remark-math fence), `MarkdownRenderer` (legacy flashcards, AI modal) had no math at all, and `features/math` called react-katex directly with its own `$…$` regex. New `math-normalizer.ts` in the core is the one rule; the preprocess copies are deleted; `MarkdownRenderer` moves to the new `gfm-math` preset; `message` gains KaTeX; `features/math` renders through `DisplayMath` / `InlineMathText` over the core. Live DB census that set the single-`$` rule: ~415 unfenced chat messages + ~33 notes carry `$x^2$`-shaped TeX, ~3.5k messages + ~290 notes carry `$<digit>` currency. Parity test failed 23/36 on the old code, passes 36/36.
