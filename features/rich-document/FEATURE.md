...
9a. **ONE math dialect, owned by the core.** `components/markdown-core/math-normalizer.ts` converts `\(…\)` → inline, `\[…\]` → display, unambiguous `$…$` → inline (pandoc rule + a TeX signal, so `$5 and $10` stays currency) and never touches code spans, fences or existing `$$…$$`. Every math-capable `MarkdownCore` preset (`math`, `gfm-math`, `rich`, `chat`, `message`) runs it and uses the shared `REMARK_MATH_OPTIONS` / `REHYPE_KATEX_OPTIONS`; the file previewer and `features/math` import the same exports. Never convert math delimiters in a renderer's own preprocess. Guard: `components/markdown-core/__tests__/math-parity.test.tsx` (every entry point, same math) + `math-normalizer.test.ts`.
