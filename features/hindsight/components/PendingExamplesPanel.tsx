    ...
    <div className={cn("space-y-2", className)}>
      {unsettled > 0 && (
        <p className="flex items-start gap-1.5 rounded-md border border-amber-500/40 bg-amber-500/10 px-2.5 py-1.5 text-xs text-amber-700 dark:text-amber-400">
          ...
          <span>
            {unsettled} of these run{unsettled === 1 ? " is" : "s are"} from the
            last {data.settle_minutes} minutes and still settling —{" "}
            <strong>&ldquo;Review now&rdquo; will not read {unsettled === 1 ? "it" : "them"}</strong>.
