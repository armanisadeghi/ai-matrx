...
type Recipe = ReviewRecipe & { id:string; normalized_origin:string; match_pattern:string|null; field_map: unknown; submit: unknown; success_signals: unknown; failure_signals: unknown; challenge_signals: unknown; status:string; recipe_version:number; version:number; deleted_at:string|null };
...
export function RecipeReview({ id }: { id: string }) {
 ...
 const valid=!!row && !activationRefusal(row);
 const activate=async()=>{if(!row||!valid){setError(row ? activationRefusal(row) : "Recipe is unavailable.");return;} setBusy(true); setError(undefined); const base=supabase.schema("browser").from("login_recipe").select("id").eq("normalized_origin",row.normalized_origin).eq("status","active").is("deleted_at",null).neq("id",row.id); const {data:active}=await (row.match_pattern===null ? base.is("match_pattern",null) : base.eq("match_pattern",row.match_pattern)).maybeSingle(); if(active){setError("An active recipe already exists for this origin and path.");setBusy(false);return;} const {data,error}=await supabase.schema("browser").from("login_recipe").update({status:"active",version:row.version+1}).eq("id",row.id).eq("status","proposed").is("deleted_at",null).eq("recipe_version",row.recipe_version).eq("version",row.version).select("id").maybeSingle(); if(error||!data)setError("Activation did not apply; review the current recipe state."); await load(); setBusy(false);};
