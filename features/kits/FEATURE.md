...
- **Install record** = one record in the organization's own **"Kit installs"** table (record store, slug `kit_installs`, declared on first install through `declareTable`). Fields: `kit_key`, `kit_version`, `status` (`installing|installed|failed|removed`), `steps` (JSON text: every id created, per step), `error`.
- **Why this mechanism:** the records API supports it with no new schema; the record is written FIRST, and each step's ids are written before the next step starts, so a re-run RESUMES from recorded ids and nothing is ever matched by name (a person's own same-named table is never adopted). Created tables get a per-install slug (`kit_<kit>_<table>_<install8>`) and a description naming the install id; agent copies are tagged `kit:<key>` + `kit-install:<id>`; workflows likewise.
