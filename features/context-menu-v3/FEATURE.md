...
- **Surface-declared values pass through verbatim** from `getApplicationScope()` (live, preferred) or `contextData` (static), merged with the captured selection. The static assembly (contextData + captured selection/text_before/text_after) is the **underlay**; the live builder's result overlays it **per key** — a key it emits is authoritative (an explicit `""` is respected, the Vault relies on that for `selection`), a key it never mentions falls back to the underlay. Precedence for `content`: surface → editable field value → DOM-text fallback (the fallback net is **skipped entirely** when a live builder is present — the Vault hardening). Pinned by `__tests__/value-resolution.test.ts`.
...
4. ~~**Note tabs**~~ — DONE `2026-08-26`: `NoteTabItem.tsx` wraps each tab in `NonEditableContextMenu` (`matrx-user/notes`, full manifest scope via `useNotesSurfaceScope`, `contentSource {type:"note"}` + `entity`); the old `menuItems` array became the "Tab" `extraSections` group (same handlers, also still feeding the "…" dropdown) and the fixed-position div / `ctxMenu` state / outside-click handler are deleted.
5. ~~**Image / video block renderers**~~ — DONE `2026-08-26`: both `UnifiedImageBlockRenderer.tsx` and `UnifiedVideoBlockRenderer.tsx` wrap their inline frame in `NonEditableContextMenu` (`sourceFeature` prop, default `"files"`); every bespoke row became the "Image" / "Video" `extraSections` group (identical handlers; `extraActions` stays the leading group), `contextData.content` carries the durable ref (`/files/f/{fileId}` viewer link or the external URL — never a signed URL) and matrx blocks pass `entity {type:"file"}` so Attach To / Share light up. Mobile keeps the media drawer via `suppressed={isMobile}` (same semantics as the old disabled trigger). `MediaExtraAction.icon` is now a `LucideIcon` component so domain actions flow losslessly into v3 (`AssetCard` updated). Zero raw-primitive importers left outside `features/context-menu-v3/**` + the primitive's own test.
6. ~~**Candidate-profile variants**~~ — DONE `2026-08-26`: `CandidateProfileWithCollapseView.tsx`, `ModernCandidateProfileView.tsx`, and `ModernOneColumnProfile.tsx` each wrap their pane in ONE `NonEditableContextMenu` (`content-extractor`, `contextData.content` = extracted JSON) with the same `data-candidate-profile-item` + `resolveContextOnOpen` per-section delegation as the canonical `CandidateProfileView`.
...
- `2026-09-26` — **ALC-15 S3: the menu is drawn by the Alchemy engine.** `MenuContent`, `MobileMenuContent` and `model/layouts.ts` are deleted; the shell opens ONE lazy boundary, `components/AlchemyMenuContent.tsx`, which runs `useContextMenuActions` + `buildMenuModel`, registers the model as this instance's provider (`alchemy-provider.ts`) and renders the package's `ContextMenuPanel` (right-click, floating icon, ⋯), `ActionSheet` (phone long-press) or `ActionPalette` (⌘/Ctrl+Shift+K). The click target is composite, so the rich-document rows come from their own provider — ⋯ and right-click are one engine. The Radix ContextMenu root and the vaul Drawer are gone from the shell (selection capture, WidgetHandle, long-press, nested-innermost and native-text-field rules unchanged; the secondary-release guard moved into the package panel). Chair rulings: (1) only the strip greys, and says why (the v3 strip used to DROP a greyed verb); (2) bar-only actions appear in every layout; (4) disabled-with-reason rows are absent; (6) the phone sheet is the package sheet over the same model (it no longer builds its own list). Headings are only names the classic menu used ("History", a surface section's label, the surface's display label). Guard: `__tests__/alchemy-provider.test.ts` (red on 4 plants); lossless is proven in the package (`menu.test.ts`, `react/layouts.test.tsx`).

- `2026-09-21` — **Note tab "…" is the universal menu, not a second dropdown.** `NoteTabItem.tsx` dropped its bespoke `DropdownMenu` (Tab items only) and now opens the tab's `NonEditableContextMenu` via `openContextMenuForElement` — the canonical overflow trigger FileTreeNode / user-lists already use — so the button offers everything right-click did (Content · Copy as · Export → Print / Save PDF · Convert · Tab …). Same session: `MatrxSplit`'s preview action surface defaults to `actionsBehavior="always"` (a hover-only ⋯ hid the whole toolkit; Arman could not find Print in Notes), and the agents chat `print` label is now "Print / Save PDF" like every other registry. Guard: `features/notes/components/note-tab-one-menu-contract.test.ts`.
...
- `2026-08-26` — **Listening actions land in the Listen panel; stream-to-stream added.** "Summarize for listening" no longer opens the general `AgentRunWindow` — both it and the new "Summarize & listen" open the purpose-built floating Listen panel (`features/window-panels/windows/listen/ListenSummaryWindow.tsx`, overlay `listenSummaryWindow`): the summary streams through the canonical pipeline (`LiveRunDisplay variant="bare"`) above a glass audio transport (play/pause/stop/replay, live equalizer). The live variant speaks the summary AS it streams via the app-root speaker (`voicePlaybackBus` request with the new `includeActive: true`, which stops the owner's baseline from marking the just-launched, already-live request as handled when the lazy audio chunk mounts late). Same two actions were added to the assistant action bar's ⋯ menu (`messageActionRegistry.listeningItems`, agent resolved by `MessageOptionsMenu` via `useSurfaceAgentRoles("matrx-user/assistant-message")`).

- `2026-08-26` — **Data→UI pipeline certification: grouping extracted + tested, scope merge fixed, empty categories visible, lossless law pinned.** (1) The fetch→grouping seam is a pure exported `buildCategoryGroups` (hook memo delegates); 10 unit tests pin placement fidelity, nesting, empty-category retention, no-agent entries, sort stability, kbd-shortcut scope dedupe (`hooks/__tests__/build-category-groups.test.ts`). (2) `resolveApplicationScope` no longer discards `contextData` + the captured selection when a surface passes `getApplicationScope` — static underlay, live overlays per key (explicit `""` respected — Vault unchanged); the DOM-text fallback net now genuinely skips live-builder surfaces, closing the vault's empty-content leak window (`__tests__/value-resolution.test.ts`). (3) An empty category renders greyed instead of vanishing (both renderers); mobile gained kbd hints + the red legacy flag. (4) `handleContentBlockInsert` copies the template (toast / manual-copy dialog) instead of silently no-oping on read-only surfaces; templates pass verbatim. (5) Command filter folds case AND diacritics; `model/__tests__/layout-parity.test.ts` asserts the classic/tiered/command leaf sets are identical — THE LOSSLESS LAW is now a failing test, not a comment.

- `2026-08-26` — **Image/video block renderers + candidate-profile variants onto v3 (backlog rows 5 + 6).** The two canonical media renderers replaced their hand-built raw-primitive right-click menus with `NonEditableContextMenu`: bespoke rows are now the "Image" / "Video" `extraSections` groups (lossless — same handlers, same order, `extraActions` still leads), `contextData.content` is the durable media ref, matrx blocks pass `entity {type:"file"}`, and mobile stays on the media drawer via `suppressed={isMobile}`. `MediaExtraAction.icon` changed from `ReactNode` to `LucideIcon` (sole producer `AssetCard` updated) so domain actions serialize into `ContextMenuExtraItem` without adapters. The three non-canonical candidate-profile views gained the canonical pane-level v3 wiring (`content-extractor` + per-section `resolveContextOnOpen`). Raw `ui/context-menu` primitive importers outside `features/context-menu-v3/**` and the primitive's own folder: ZERO.

- `2026-08-26` — **Full-content `onTextReplace` contract enforced fleet-wide.** `onTextReplace` receives the WHOLE new value (Cut/Paste/Find-Replace and the WidgetHandle's `widget_text_replace/patch/prepend/append` all pass full content); five surfaces that spliced it into the live selection (MessageItem, SystemMessage, CleanupPad, both Monaco code menus — duplicating content on Cut/Paste and agent edits) now replace in full (Monaco via full-range `executeEdits`, undo preserved). `CodeEditorDemoPanel` gained real `getTextarea`/`onTextReplace`; the scraper workspace dropped `getTextarea={() => null}` stubs that falsely advertised insert methods. New guard: `utils/__tests__/widget-handle.test.ts` (capability advertisement = serviceable methods only). Known remaining offender: `features/notes/components/NoteEditor.tsx` (all three modes still selection-splice; owned by the notes agent).

- `2026-08-26` — **Note tabs onto v3 (backlog row 4).** `features/notes/components/NoteTabItem.tsx` — the last live bespoke right-click menu in notes — now wraps every tab in `NonEditableContextMenu` with the editor's surface identity (`matrx-user/notes`), full manifest scope for the right-clicked tab's note (`useNotesSurfaceScope` with a null textarea ref), `contentSource {type:"note"}` + `entity` (Attach To / Share), and the whole former `menuItems` array as ONE "Tab" section (anchor after-compare, identical handlers; the same array still feeds the tab's "…" dropdown so the two can't drift). Bespoke positioned-div menu, its state, and its outside-click listener deleted. `NoteTabs.tsx`/`NotesLayout.tsx` copy untouched — zero mounters (row 7 delete-candidates).

- `2026-08-26` — **Nested row menus and touch overflow controls are real again.** Desktop shells no longer stop the event before an as-child Radix trigger can open; the inner trigger prevents default and every outer shell yields to that handled event. `openContextMenuForElement` gives explicit 44px overflow buttons one canonical entry path without duplicating menu actions; code-tree files, code-library source entries, and user-list nodes use it through the same v3 shell.

- `2026-08-25` — **Initial nested-desktop ownership attempt (superseded 2026-08-26).** The shell added desktop propagation stopping to make the innermost surface win. Live code-tree verification then proved that stopping at the child fiber could abort its as-child Radix open entirely; the following day's handled-event protocol is the corrected mechanism.
