...
| HR task inbox step (`hr.workflow_step`, `hr_wf_inbox` projection) | `hrTaskStepMenuSection` + `hrTaskStepEntityRef` | `features/hr/tasks/task-step-actions.tsx` | LeaveQueueSurface (§4.4 leave projection; decision items stay page-local) — future adopters: HrTaskTable, HrTaskInbox (same `HrInboxRow` shape, no menu today) |
| HR person / employee                   | `hrPersonMenuSection` + `hrPersonEntityRef` + adapters (`hrDirectoryRowTarget`, `leaveBalanceRowTarget`) | `features/hr/people/hr-person-menu.tsx`                             | LeaveEnrollmentSurface (roster + candidates) · LeaveBalancesSurface (`onAdjustBalance` grown in) — future adopters: HrDirectoryCards, directoryColumns, EmploymentPicker, LeaveDecisionDialogs |
