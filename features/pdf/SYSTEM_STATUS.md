...
| **4. (Found during testing) Some PDFs show "Failed to fetch"** | ✅ FIXED | **Class bug**, not one file: public/CDN-bucket PDFs (`s3://cdn.matrxserver.com/…`) made `/download` 302-redirect to the CDN; pdfjs's auth header makes the request *preflighted*, and browsers won't follow a cross-origin redirect of a preflighted request → "Failed to fetch." Private files stream through the API (same origin) so they always worked — which is why only *some* files broke (this is the admin-account "Failed to fetch" you hit). Fixed in `usePdfRemoteSource`: CDN files resolve the direct CORS-friendly CDN url and hand it to pdfjs with no auth header (simple Range GET — no preflight, no redirect); fail-safe fallback to streaming. **Verified in-browser: the previously-broken `t.pdf` now renders; private files unchanged.** |
