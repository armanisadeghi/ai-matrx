...
**Record → draft → finalize (processor):** `RecordingInterface` records → `saveAudioToStorage` uploads to `cld_files` → transcription via `features/audio` hooks → `saveDraftTranscript` (`is_draft=true`) → user reviews in `RecordingPreview` → `finalizeDraft`. Super admins also get `Run safe QA canary`: `qa/deterministicRecordingCanary.ts` generates an audible synthetic WAV, requests no microphone, calls the same recording upload + draft path, and skips all provider calls while stamping `metadata.qa_fixture`.
...
- **Upload retry wrappers never call `console.error`.** `fileHandler` owns the structured transport incident; `audioStorageService` may warn while retrying, but must not create a duplicate `system_error` for the same failure.
