...
[Shared Knowledge Resources](../rag/FEATURE.md#shared-knowledge-resources) (`features/rag/` library stores) are shared by an **ownership-asymmetry** model: a system org owns the content, READ is granted via `rag.data_store_grants` (audience global / industry / org), and WRITE stays gated by data-store ownership. The publish UI (`DataStorePublishPanel`) deliberately _mirrors_ `ShareModal`'s structure but uses its own grant RPCs over HTTP.
