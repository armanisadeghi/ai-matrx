...
- 2026-09-25 — **"Who can see this" — the lane control (SHARE-LANE-CONTROL, VERIFIER-23 item 3).** `components/WhoCanSeeThis.tsx` is the ONE control, at the top of every host's People tab (ShareModal, ShareModalWindow, AgentSharePanel, SiteAccessWorkspace), fed by `useSharing().whoCanSee` / `setWhoCanSee`. Record store: reads `public.store_door_lane` (now naming `organization_id`, `organization_name`, `member_default_level`, `world_open` in every state — `migrations/campaign/sharelane_the_lane_door_names_the_organization.sql`) and writes `custom.share_lane_set` (`setStoreLane`). A kind whose reach IS its `visibility` enum (a site) maps personal/internal onto the same value the Public tab writes; a `card_visibility` kind (agent, workflow) gets NO control, because a card's visibility is not who may open the thing. "Anyone with the link" is drawn only when the world lane would accept it. "Only people I share it with" while members reach it confirms in one sentence first; a non-owner reads the state as text. `OrgAvailabilityNote` no longer lists the organization's own lane row as "also available". Tests: `__tests__/who-can-see-this.test.tsx` (13), `__tests__/who-can-see-this-hosts.test.tsx` (4), `scripts/campaign-tests/sharelane_green.sql`, walk `scripts/campaign-tests/sharelane_walk.mjs`.
- 2026-09-25 — **A share names a person, never an organization (SHARE-PEOPLE-ONLY).** The Organizations tab is removed from ShareModal, ShareModalWindow, AgentSharePanel and the marketing site access screen (its `access-organizations` view is retired; old links land on User access). "Add everyone in <organization>" grants each current member by name (a record-store table goes through `table_share_outside_grant`). DB: `migrations/campaign/sharepeople_a_share_names_a_person.sql` (guard trigger + `grant_org_availability` + conversion: 10 organization shares became per-person grants for their current members and were archived; 4 surface-binding rows and 38 HR-directory rows stamped availability) and `sharepeople_an_archived_grant_is_not_listed.sql`. Tests: `scripts/campaign-tests/sharepeople_green.sql`, `features/sharing/__tests__/a-share-names-a-person.test.tsx`.
