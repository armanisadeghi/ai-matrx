...
// ---------- Icon registry ----------
// Keyed by the string stored in udt_structured_list_items.icon_name so we can render whatever is in the DB.
...
export function PicklistManagerV1({
  ...
  const createItem = async (
    ...
    focusFieldRef?: React.MutableRefObject<string | null>,
  ...
  const handleAddRow = async (
    group: string | null,
    focusField: "label" | "description" | "help_text" = "label",
  ) => {
          ...
          <>
            ...
            <div className="flex flex-col gap-0.5 border-b px-5 py-3">
              ...
              <Input
                ...
                onBlur={(e) =>
                  updateListField(activeList.id, "description", e.target.value)
                }
                    ...
                    {groupItems.map((item, rowIdx) => (
                      <ItemRow
                        ...
                        onFieldChange={(field, value) =>
                          debouncedUpdateItem(item.id, field, value)
                        }
                        ...
                        onEnter={(field) =>
                          handleAddRow(item.group_name, field)
                        }
                        onTabOffLast={(field) =>
                          handleAddRow(item.group_name, field)
                        }
            ...
            <div className="flex items-center justify-between border-t bg-muted/30 px-5 py-1.5 text-[11px] text-muted-foreground">
              <span>
                <Kbd>Tab</Kbd> next field <Kbd>↵</Kbd> new row <Kbd>⌫</Kbd>{" "}
                delete empty
              </span>
      ...
      <PopoverTrigger asChild>
        ...
        >
          {icon ? (
            ...
            <SquarePlus className="h-3.5 w-3.5 opacity-50" />
          )}
        </Button>
      </PopoverTrigger>
