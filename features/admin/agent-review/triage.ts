...
/**
 * THE AREA OF THE PRODUCT A REVIEW ROW IS ABOUT — derived from its `url`.
 *
 * A 390-row queue with no product dimension is unusable: the owner cannot ask
 * "show me the Content Plan ones" without reading every title. `source` is the
 * REPO, `lane` is the kind of work, `required_tools` is the equipment — none
 * of them says WHAT FEATURE the row belongs to.
 *
 * The obvious fix — a `feature` column agents fill in — is the one that does
 * not work: metadata is agent-authored and unenforced, and only 19 of 390 live
 * rows carry any `feature` key at all (measured 2026-08-19). The `url`, by
 * contrast, is on 100% of rows because a row without a place to go is useless
 * by construction. So the area is DERIVED, never stored: it cannot drift, it
 * needs no backfill, and a new feature classifies itself the day its first row
 * lands.
 *
 * Container-first segments (`/marketing/content-plan` → "Marketing › Content
 * Plan") because "marketing" alone would collapse five distinct features into
 * one bucket.
 */
const AREA_CONTAINERS = new Set([
  "marketing",
  "administration",
  "education",
  "agents",
...
/** The area label for a review row's url, or null when it has no usable path. */
export function deriveReviewArea(url: string | null | undefined): string | null {
  const raw = (url ?? "").trim();
  if (!raw) return null;
  let path = raw;
  // Accept absolute urls, protocol-relative, and bare paths alike — rows are
  // written by many agents and all three shapes are live in the table.
  ...
  if (AREA_CONTAINERS.has(first.toLowerCase()) && second) {
    // A uuid/id second segment is not a feature name — keep the container.
    const looksLikeId = /^[0-9a-f]{8}-[0-9a-f]{4}-/i.test(second) || /^\d+$/.test(second);
    if (!looksLikeId) {
      return `${titleizeSegment(first)} › ${titleizeSegment(second)}`;
    }
  }
  return titleizeSegment(first);
}
