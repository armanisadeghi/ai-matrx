...
- `app/(admin)/administration/users/email/page.tsx` — protected bulk-email composer; selected-account mode uses the complete admin directory and canonical advanced recipient picker.
...
- Shared UI: `MatrxDataTable`, `UserSearchField`, shared confirmation dialog, shadcn dialog/select/badge/button primitives.

**Primitives reused or extended**

- Extended the existing Users & Access tab shell and account roster.
- Reused `MatrxDataTable` for both reciprocal lists and the canonical `UserSearchField` for existing-account lookup.
...
- `public.admin_set_guest_block(p_guest_id, p_blocked, p_reason, p_blocked_until)` (migration `migrations/guest_block_admin_control.sql`) is the ONE writer of `users.guest_executions.is_blocked` / `blocked_until` / `blocked_reason`. SECURITY DEFINER, `public.is_super_admin()` gate (42501), declared client door, row-locked; a block's end time must be in the future (22023); unblocking clears end time and reason. Audit lives on the row: `metadata.admin_block_history` (actor, time, action, reason, end time, replaced state; last 50).
- Enforcement is not here: aidream's guest resolver (`resolve_guest_uuid` → 403 `guest_blocked` at the auth boundary) and `public.check_guest_execution_limit` refuse the fingerprint while the block holds. `features/admin/users/lib/guestAccess.ts` mirrors that one rule for display; `lib/guestBlock.ts` is the client call.
- UI: the Acquisition table's **Guest access** column (Allowed / Blocked until … / Block ended) with Block (dialog states the 403 consequence and end time before firing) and Unblock (confirm), plus a Blocked guests tile. Accounts are unaffected: a signed-in request never reaches guest resolution.
...
- `2026-09-12` — Feedback console phone reflow (Arman's report: the `?feedback=<id>` deep link was unusable on a phone). The feedback and announcements tables opt into THE PHONE-STACK TABLE (`app/globals.css`; `<Table wrapperClassName="phone-stack">` + `data-label` / `data-phone` cell hints) so each row is a card below 768px; the pipeline stage bar scrolls sideways instead of crushing six buttons; the search row wraps; the detail dialog's header wraps its title, badges and meta line, drops the copy-button cluster to its own row on phones, uses sheet-safe padding, a non-shrinking scrollable tab strip and a one-column metadata grid. Root causes fixed at the class level: the global mobile rule that made every design-system `<table>` `display:block` (shrink-to-fit + double scroller) now excludes them, and `pnpm check:phone-layout` guards fixed-width tables, `vh`/`h-screen` units and un-reflowed dialog grids repo-wide.
