...
/**
 * "Try it now" — run a mandate with inputs typed right now, no stored test case.
 *
 * This is the cold-start half of the owner bench. Most live mandates have no
 * exemplar at all, so the batch bench has literally nothing to run for them:
 * the only way in is to run the mandate once by hand and keep the result. So this
 * panel does exactly two things:
 *
 *   1. Scaffold a form from what the mandate ALREADY declares — the contract's
 *      required variables plus the variable definitions of the agent that
 *      actually runs (the PINNED VERSION's when the mandate pins one, not the
 *      latest definition's), so the admin never guesses a variable name or its
 *      input type, with the user message field for chat-shaped mandates.
 *   2. On any successful run, "Save as test case" writes the inputs AND the
 *      run — as the reference — into one new exemplar. The mandate is benchable
 *      from that click on.
 *
...
interface BenchField {
  ...
  /** Declared in the mandate's contract — blank is a refusal, not a default. */
...
export function TryItNowPanel({
  ...
  /** The agent this mandate resolves to — its variable definitions supply the
   * real input types (picklists, media, sliders). */
  defaultAgentId: string | null;
  /** Code truth: does any call site pass a user message to this mandate?
  ...
  /** Set only for a version-pinned mandate: the PINNED version's declarations. */
  ...
  useEffect(() => {
    ...
    dispatch(fetchAgentExecutionMinimal(defaultAgentId))
      ...
      .catch((error: unknown) =>
        toast.error(
          `Couldn't load the agent's variables (${describeError(error)}) — the mandate's declared inputs are still shown.`,
        ),
      );
  }, [defaultAgentId, dispatch]);

  useEffect(() => {
    // No reset branch: the bench (and this panel with it) is keyed by mandate id
    // in MandateDetailPanel, so a different mandate means a fresh mount.
  ...
  // mandate whose declaration could not be read would otherwise be benched
  ...
  function buildVariables(): JsonObject | null {
    ...
    for (const field of fields) {
      ...
      if (field.requiredByContract && isBlank(raw)) {
        toast.error(
          `"${field.name}" is a required input for this mandate — fill it in before running.`,
        );
        return null;
      }
  ...
  async function run() {
    ...
    // A mandate that declares nothing legitimately runs as-is; one that declares
    ...
    if (
      ...
      toast.error(
        "Give the mandate something to run on — fill an input or a user message.",
      );
      return;
    }
    setRunning(true);
    ...
    <div className="space-y-2 rounded-md border border-border bg-muted/10 p-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="text-[11px] text-muted-foreground">
          Run this mandate on inputs you type here — no saved test case needed.
        </div>
      </div>
      ...
      {fields.length === 0 && !showsUserInput ? (
        <div className="rounded border border-dashed border-border p-2 text-[11px] text-muted-foreground">
          This mandate declares no inputs, so there is nothing to fill in — run it
          as-is.
        </div>
      ) : (
        <div className="space-y-2">
          {fields.length > 0 && agentDefinitions.length > 0 && (
            <div className="text-[10px] text-muted-foreground">
              Fields come from this mandate&apos;s contract and the{" "}
      ...
      {showsUserInput && (
        <Textarea
          ...
          placeholder={
            passesUserInput
              ? "User message — what the person using this mandate would say"
              : "User message (optional — this mandate's call site could not be read)"
          }
