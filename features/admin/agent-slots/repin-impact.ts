/**
 * Repin pre-flight — "if I swap this slot's agent, what stops flowing?"
 *
 * WHY THIS EXISTS (read before changing anything here):
 * `podcast.deep_research` broke because this console SUGGESTED a repin (correctly
 ...
 *   1. `slot.contract.required_variables` — a DB copy, measured stale on 51 of
 *      143 slots (2026-08-15). Never trusted alone.
...
export interface RepinVariableImpact {
  name: string;
 ...
 * `candidateVariables` = the agent being repinned to.
 * `contractRequired` = the slot's stored contract (weak; unions into "must flow").
 * `codeSuppliedVariables` = what the call site actually passes, when known.
 */
export function computeRepinImpact({
  currentVariables,
  candidateVariables,
  contractRequired = [],
  ...
  const variables: RepinVariableImpact[] = [];
 ...
 */
export function buildRepinFixBrief({
  slotKey,
  candidateName,
  impact,
  codeTruth,
  ...
  return [
    ...
    `THE MISMATCH:`,
    lost.length
      ? `- These values are supplied to the slot but the new agent declares NO variable for them, so they will not reach the prompt: ${lost.join(", ")}.`
      : `- (none lost)`,
    unsupplied.length
    ...
    `- Update the slot contract and the agent definition together so code and DB agree.`,
    `- Re-run the slot's validation and confirm the variables actually arrive in a live run.`,
  ]
    .filter(Boolean)
    .join("\n");
}
