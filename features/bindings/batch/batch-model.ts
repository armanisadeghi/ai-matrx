...
export function applyRefusal(
  ...
  if (unfed > 0) {
    ...
    return `${unfed} required ${unfed === 1 ? "input" : "inputs"} across ${places} ${places === 1 ? "place" : "places"} ${unfed === 1 ? "has" : "have"} nothing feeding ${unfed === 1 ? "it" : "them"} and the holder has no default of its own — this batch cannot submit the job in that state. Feed ${unfed === 1 ? "it" : "them"}, or remove ${places === 1 ? "that place" : "those places"} from the batch.`;
  }
  return null;
}
