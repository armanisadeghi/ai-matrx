...
| `HolderAssignment.tsx` | **THE ONE HOLDER CHOOSER.** Icon-led Agent/Workflow buttons; **Assigned Agent/Workflow** groups the record picker and version dropdown on one row. Version has an accessible label without a separate visible label; no-holder and latest-only workflow states remain disabled and explicit. Latest stores `default_holder_version_id = NULL`. Every host reuses this component. **`onCreateAgent` puts "+ Agent" INSIDE the assignment row** — a way of answering the same question, not a fourth control, so the `data-holder-control` count stays three; absent on a host with no such door. |
...
| `HolderDraftPanel.tsx` | **THE "+ AGENT" DOOR** (Arman, 2026-09-20). The workspace's Create Agent tab: `AgentGenerator` — the very component `/agents/new/generate` mounts — in its **mandate mode**, launched through `mandates.holder_draft` with this job's goal, offered values and output contract as the brief and the person's own words as `user_input`. The created agent lands in the holder controls as Latest (unsaved), the advanced editor opens on it, and the person presses the one Save that already exists. No Holder on the drafting job ⇒ the server's sentence verbatim plus a door to that job — never a dead Generate. |
| `holder-draft-brief.ts` | Pure: `holderDraftOwnerOf` (WHOSE agent the door creates — system rung ⇒ a system agent, an org rung ⇒ that org's, a person's rung ⇒ their own; `null`, and a refusal, when an org rung names no org) and `buildHolderDraftBrief` (the offered values of `mandates.holder_draft_brief`, keyed by their declared names, optional values OMITTED rather than sent empty). Jest-pinned in `__tests__/holder-draft-brief.test.ts`. |
