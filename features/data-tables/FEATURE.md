...
- `2026-09-22` — **Every row change is a platform EVENT — the automation producer.** Airtable's "when a record is created / updated" trigger, built where the platform already has its ONE event spine: `migrations/udt_row_change_events.sql` (applied + ledgered 2026-09-22 04:00 UTC) adds `workbench.udt_row_activity()`, an AFTER trigger on `workbench.udt_dataset_rows` that writes one `platform.activity_log` row per row change through the spine's own writer `platform.log_activity` — `entity_type user_table_row`, `entity_id` = the row, `action` ∈ `row.created` / `row.updated` / `row.archived` / `row.restored` / `row.deleted`, `metadata { table_id, table_name, changed_fields, version }`, `actor_id` = the person or NULL. An UPDATE that changes neither `data` nor `deleted_at` writes nothing; `SET LOCAL matrx.udt_row_events = 'off'` silences a transaction that will announce itself once (large imports — no caller sets it yet). Proven in a rolled-back transaction on the test table: update → `row.updated` with `changed_fields: ["capital"]`, version-only update → no event, archive/restore/insert/delete → the four other actions; nothing persisted after rollback. What it buys today: the live outbound-webhook transport (`files.webhook_dispatch`, matched by `resource_types` / `event_types`) can already deliver table changes to any HTTPS endpoint — Airtable's webhook automation with zero new UI. Next, NOT here: the scheduler's declared-but-empty `"event"` trigger type gets its `TriggerConfig` (entity_type + action + table_id filter) and a scanner over the spine (`features/scheduling` / aidream matrx-scheduler); then "run agent X when a row changes" is a scheduling task, and the row-action class 4 ("aware of other rows") is a workflow step, both outside data-tables by design.
