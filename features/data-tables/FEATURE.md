...
- `2026-09-23` — **The two AI jobs on this surface run by MANDATE (ARE-020 / ARE-021).** The formula box's "Help with this…" runs `data.formula_writing` (Provision `data.formula_box`: purpose, target column name + type, columns as `{Name}` and in full with types/formats, the formula language, the current formula, the parse error, a sample row) and the agent row action runs `data.row_action` (Provision `data.table_row_action`: table, columns, row id/label/JSON/summary, action name + prompt, acting person + can-edit). Both declared in aidream `client_mandates.py`, seeded on General Chat until Arman reviews the mandates and the real agents are created. The row no longer rides `user_input` (`agentActionMessage` deleted → `agentActionOffer`); the action prompt is the only user text. Both disclosed in the top Agents menu via `data-tables.manifest.ts` `agentRoles` — no page content changed. The ProTextarea primitive now runs help by `helpMandateKey` (default `chat.default_new_chat`) through the mandate door, carrying `helpContextItems` as offered values + context (it used to drop them). Guards: `components/official/proTextareaAgentActions.test.ts`, `__tests__/row-actions.test.ts`.

- `2026-09-23` — **Date cells edit through a calendar that can never be clipped (`components/DateCellEditor.tsx`).** Arman: the native `datetime-local` input's calendar button was pushed past the cell edge — in a narrow column the calendar was simply unreachable. Now (Airtable/Notion model) editing a `date`/`datetime` cell — by column format or storage type — opens a calendar layer at once, portalled and collision-positioned, over a plain typeable field in the cell: type "10/3/2026 4:15 pm" / "Sep 30" and the calendar follows; Enter saves and moves down, Tab across, Escape discards, clicking away saves. A date column saves on the day pick; a datetime column keeps the layer open for the time (keeps the chosen time; first pick starts 9 AM) with Now / Clear / Done. Unreadable text is never guessed or dropped — the layer says so and keeps the text. Stored shapes unchanged (`yyyy-MM-dd`, local `yyyy-MM-ddTHH:mm`); an untouched edit hands back the original value so nothing is rewritten; a zoned stored value now shows in local time instead of its raw UTC slice. Portal events are stopped so the grid never steals the click or keystroke. Test: `__tests__/date-cell-editor.test.ts` (west-of-UTC; proven failing with the date-only read disabled). Live-verified on the local preview: 80px column, pick, time keys, typed entry, unreadable message, Escape, Clear, date-only pick.

- `2026-09-25` — **Off-list value in a choice column → "Add as option?" in one click.** Arman, coloring rows by Status after typing ACTIVE into a cell: the value was saved (amber, off-list — right, raw data stays readable) but the Colors dialog could not name it because the column's OPTIONS never learned it. "The system should have asked me if I want to add it as an option — a little quick reminder that doesn't block or annoy." `features/data-tables/choice-option-nudge.tsx`: `decideChoiceNudge(field, saved)` (pure: none / list_bound / offer with the missing values; multi-choice offers only the missing ones; person never) and `offerToAddChoiceOption` — ONE toast, ONE action, appends the value(s) to the column's inline options through `setFieldFormat` and reloads; never twice for the same value in a session; a column bound to a shared pick list is told to add it there. Called from every path that saves a choice cell: the grid cell (`onSaved`), Add row, Edit row. Tests: `__tests__/choice-option-nudge.test.ts`.
- `2026-09-22` — **Every row change is a platform EVENT — the automation producer.** Airtable's "when a record is created / updated" trigger, built where the platform already has its ONE event spine: `migrations/udt_row_change_events.sql` (applied + ledgered 2026-09-22 04:00 UTC) adds `workbench.udt_row_activity()`, an AFTER trigger on `workbench.udt_dataset_rows` that writes one `platform.activity_log` row per row change through the spine's own writer `platform.log_activity` — `entity_type user_table_row`, `entity_id` = the row, `action` ∈ `row.created` / `row.updated` / `row.archived` / `row.restored` / `row.deleted`, `metadata { table_id, table_name, changed_fields, version }`, `actor_id` = the person or NULL. An UPDATE that changes neither `data` nor `deleted_at` writes nothing; `SET LOCAL matrx.udt_row_events = 'off'` silences a transaction that will announce itself once (large imports — no caller sets it yet). Proven in a rolled-back transaction on the test table: update → `row.updated` with `changed_fields: ["capital"]`, version-only update → no event, archive/restore/insert/delete → the four other actions; nothing persisted after rollback. What it buys today: the live outbound-webhook transport (`files.webhook_dispatch`, matched by `resource_types` / `event_types`) can already deliver table changes to any HTTPS endpoint — Airtable's webhook automation with zero new UI. The consumer landed the same day (`features/scheduling/FEATURE.md` 2026-09-22): the scheduler's `event` trigger matches spine rows in the database and enqueues a run; the Actions-header bolt menu carries "When a row changes, run an agent…", which opens the schedule form with the Table change trigger set to this table. "Run agent X when a row changes" is therefore a schedule, and the row-action class 4 ("aware of other rows") is that agent's job — both outside data-tables by design.
- `2026-09-21` — **Attachment column** shipped (`lib/field-formats/FEATURE.md`): file ids from the files feature, chips in the grid, the one file picker to add, remove on the chip. **Live-verified 2026-09-22 on the third independent pass.** Pass 1 found the multi-pick file window had no way to finish a selection (a platform defect for every `openFilePicker({multi:true})` caller — fixed in `FilePickerWindow` + `CloudFilesPickerHostImpl`, `7b9a349239`); pass 2 found the grid tore the cell editor down on the first click inside the picker window (`useGridSelection` now treats a window panel's body shell as a floating layer the grid opened) and that Other kinds excluded every array-based format (`5cbdce9c6f`); pass 3: all five steps pass. The verifier keeps one residual doubt — a coordinate click on "Attach" once discarded the editor where an element click did not — for a real-mouse spot check. **Link-to-another-table: DECIDED NOT TO BUILD HERE.** The relation primitive (stored id, one resolver `custom._words_for`, per-field display spec, `ReferenceBuilder`, batch door `relation_words_many`) shipped 2026-09-21 in the RECORD STORE (`/data-v2`, `@ai-matrx/records` 0.46 / `records-ui` 0.65) by lane RELATION-DISPLAY, whose own finding is that this older store has no id-storing relation and giving it one is a store cutover, not a column — and the doctrine register carries that cutover as DD-031 (migrate `workbench.udt_*` into `custom`). A second relation implementation here would be exactly the fork the primitives law forbids. Lookups/rollups follow the relation, so they wait too. Automations (row-action class 4, "run when a row changes") likewise belong to the scheduling/workflow primitives, not this feature.
