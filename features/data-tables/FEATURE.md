...
- `2026-06-18` — claude: **`/documents` hub toolbar → TapTarget system.** Replaced raw `<Button>`, custom search pill, and `HubToolbarToggle` with `DocumentsHubToolbar` (`TapTargetButtonGroup` for card/table/sort view, controlled search input, `PlusTapButton` / `LoadingTapButton`). Added `LayoutGridTapButton` + `ListTapButton` pre-composed icons.
