...
- **2026-06-24** — Surfaces tab now renders the **common `AgentHeader`** (agent selector, mode tabs Build/Run/Surfaces/…, save status, options) like every other agent sub-route — previously it only showed surface-specific panel toggles, so you couldn't navigate to other modes from it. `SurfacesAdminShell` injects `AgentHeader` into the page-header slot (still via `PageSpecificHeader`, so it works under both `(core)` and `(admin)`) and takes a new `basePath` prop (`/agents` vs the admin base) for the mode-tab links. The four column toggles moved to a slim in-shell `SurfacesPanelToggleBar`; the old `SurfacesAdminHeader` is deleted.
