...
- **2026-08-10 — Keyword Research workbench agent-writable (live-verified).** `matrx-user/keyword-research` declares 2 `applyPolicy:"ask"` targets. `research_seed_keyword` (`mode:"draft"`, `{ keyword: string }`) stages the primary keyword into the Research input; its handler lives in `KeywordResearchLauncher.tsx`, which OWNS that input, registered via `useSurfaceWriteHandlers` — the child-registration seam, because the provider is mounted by the parent `KeywordResearchWorkbench`. It lands through `setPrimaryInput` + `onKeywordChange`, the exact pair the input's own `onChange` calls, so a staged seed and a typed one are indistinguishable to every host. `keyword_selection` (`mode:"ui"`, `{ keyword_ids: string[], mode?: "replace" | "add" }`) moves the row checkboxes through the workbench's own `setSelectedIds`, the same setter `toggleSelected` uses, so the toolbar's bulk-action bar reacts exactly as if the user had clicked. **THE reference for validating a selection against what is on screen:** ids are checked against the live `sorted` rows (the same projection `visible_keywords` emits) and ONE unknown id rejects the entire write — an all-or-nothing rule, so a selection can never half-land on rows the user cannot see and never silently drops the ids it did not recognize. Replace-vs-add is spelled out in the contract prose rather than left to the model to guess. It stays `ask` even though `ui` mode would permit `auto`, because the one bulk action wired to this selection is Archive: silently re-pointing it could tee up a destructive action on rows the user never chose. **The judgment bar rejected more than it accepted.** `runResearch` is not a target — the run is an outbound, billable third-party pipeline, so the Research button stays the user's press (the `image-generate` / `podcast-studio` / scraper precedent); the archive path is not a target because destructive stays human; and `library_search` was considered and REJECTED rather than padded in — it is a plain substring match on `normalized_phrase`, exactly the "pure-mechanical toggle nobody would ask an agent to flip" the bar excludes. Both handlers THROW when `run.status === "running"` rather than mutating state an in-flight paid request owns (the input is locked to it, and a finishing run re-scopes the explorer to its cluster, which would silently discard a selection staged mid-run). **Per-mount split:** `KeywordResearchLauncher` is SHARED with the floating `KeywordResearchWindow`, which mounts a different surface — so the launcher takes an opt-in `writeSurfaceName` prop (null by default) and only the workbench passes it; a launcher can never register a handler for a surface it is not inside. Live-verified with a real Badass Agent run: one message asking for both changes raised a separate ask dialog per target, each carrying the manifest description verbatim; Apply staged the seed into the input (Research button armed but UNPRESSED, `run_status` still idle) and selected the two rows named BY PHRASE, surfacing the "Archive 2 selected" bar; "Keep as is" returned `{ok:false, declined:true}` with no error and a graceful agent acknowledgement; and asked to run the pipeline and archive rows itself, the agent had no tool for either — nothing ran, nothing was archived, and it fell back to the one write it is actually offered. Error Inspector: zero `surface-writeback` captures (only the pre-existing `request_context_changed` server-stream warnings). `check:surface-drift` + `type-check` clean. The manifest's `readinessNote` also lost a stale claim that the surface was unregistered — it has been in `registry.ts` and `route-to-surface.ts` for some time; the `ui_surface` / `ui_surface_write_target` DB mirror is what is still pending.
