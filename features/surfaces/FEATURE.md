...
- **Agent-origin writes (the stream side)** — a run launched on a surface with
  ...
  `library_search` and `cluster_scope` as `ui` writes through the explorer's
  own setters, plus `research_input_keyword` as a `draft` staging the research
  launcher's input. THE reference for splitting JUDGMENT from a PAID BUTTON:
  the agent can name the working set (`cluster_scope` — one object carrying
  the chip's name and its phrases, with an explicit
  `mode: "replace" | "append"` vocabulary imported from the feature's
  `types.ts` so manifest prose and handler validation can never drift) and
  stage the keyword it would research, but STARTING the run is undeclared —
  it spends a DataForSEO request plus agent calls. Every FETCHED value
  (`research_artifact`, `research_result`, `visible_keywords`, `volume_stage`,
  `keywords_total`) is undeclared too: an agent that could overwrite provider
  evidence would be fabricating the market data downstream planning treats as
  real. Row selection is undeclared because its only consumer is the bulk
  Archive button — staging a selection there is staging a soft-delete.
  Handlers on `KeywordResearchWorkbench`'s provider, except
  `research_input_keyword`, which `KeywordResearchLauncher` registers itself
  via `useSurfaceWriteHandlers` because it owns that input — and only when the
  host passes `writeTargetSurfaceName`, so the floating
  `KeywordResearchWindow` mount of the SAME launcher registers nothing and an
  agent on the page underneath can never type into a window that page does not
  own. Setting the cluster is now the single `setCluster` in
  `useKeywordResearch` that a completed run also calls, so an agent-set and a
  run-set cluster normalize identically).
...
- **2026-08-10 — Keyword Research workbench agent-writable; THE reference for separating JUDGMENT from FETCHED EVIDENCE.** `matrx-user/keyword-research` declares 3 `applyPolicy:"ask"` targets: `library_search` and `cluster_scope` (`mode:"ui"`) plus `research_input_keyword` (`mode:"draft"`). `cluster_scope` is the interesting one — an object `{mode, primary_keyword?, phrases[]}` that scopes the explorer to an agent-composed working set, which is exactly what the `keyword_strategist` / `cluster_analyst` roles were already producing as prose. It offers BOTH set semantics (`replace` names a new cluster and requires `primary_keyword`; `append` adds to the cluster on screen and inherits its name, throwing when there is nothing to append to), and the `replace | append` vocabulary now lives in `features/marketing/seo/keyword-research/types.ts` (`KEYWORD_CLUSTER_WRITE_MODES` + `isKeywordClusterWriteMode` + `normalizeClusterPhrase`) so the manifest's prose and the handler's validation are the same constant. **The cluster now has ONE owner:** `useKeywordResearch` gained `setCluster(primaryKeyword, phrases)` — a completed research run calls it, the agent write calls it, and both normalize phrases identically; the explorer chip reads the new `clusterPrimaryKeyword` instead of `run.primaryKeyword`, so an agent-set cluster is named and clearable exactly like a run-set one (before, a phrase set with no run behind it would have filtered the table with no chip and no way to clear it). `research_input_keyword` stages the launcher's box and has a new read twin of the same name, emitted from a REF so a keystroke in the launcher never re-renders the 100-row table under it. **What is deliberately NOT writable, and why this surface is the reference:** starting a run (`run_status` / `run_stage` / `run_id`) spends a DataForSEO request plus agent calls, so the agent stages the keyword and the human presses Research; and every FETCHED value (`research_artifact`, `research_result`, `visible_keywords`, `keywords_total`, `volume_stage`, `run_error`) is undeclared because an agent able to overwrite provider market data would be fabricating the evidence the rest of the platform plans against. Row selection is undeclared too — its only consumer is the bulk Archive button, so staging it is staging a soft-delete. **Per-mount posture:** the workbench is the ONE mount of this surface and registers `library_search` + `cluster_scope` on its provider; `KeywordResearchLauncher` registers `research_input_keyword` itself via `useSurfaceWriteHandlers` because it owns that input, but only when the host passes the new `writeTargetSurfaceName` prop — so the floating `KeywordResearchWindow`, which hosts the SAME launcher over whatever page the user is on and mounts no runtime for this surface, registers nothing. Live-verified with a real Badass Agent run: one message → three ask dialogs each carrying the manifest description verbatim, Apply landed all three (filter `shredding`, chip `Cluster: "sd card destruction"`, launcher input `hard drive shredding service`, Research unpressed); a second run confirmed `append` extends a cluster and keeps its name (chip · 1 → · 2); "Keep as is" returned `{"declined": true, …}` and the agent reported it as declined with nothing changed; an undeclared target came back as `No mounted surface declares write target "keyword_intent_class"`; and three forced-invalid calls returned the handlers' own throws verbatim (`cluster_scope: mode must be one of replace | append.`, `research_input_keyword expects a string keyword phrase, got array.`, `cluster_scope: primary_keyword is required — there is no cluster on screen to append to…`) with nothing staged. Error Inspector: the ONLY `surface-writeback` capture was the deliberate invalid-value test; successful writes captured nothing. `check:surface-drift` + `type-check` clean. DB mirror (`ui.ui_surface_write_target`) sync still pending.
