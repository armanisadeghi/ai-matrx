...
- **Agent-origin writes (the stream side)** — a run launched on a surface with
  ...
  and `matrx-user/image-generate` (4 ask-policy draft targets staging the next
  run's request — `generation_prompt`, `generation_style`, `generation_size`,
  `generation_count`; handlers on `GenerateShellClient`'s own provider. The
  size/count enums interpolate into the target descriptions from the canonical
  `GENERATE_IMAGE_SIZE_LABELS` / `GENERATE_IMAGE_COUNTS` in
  `features/image-studio/types.ts`, so the advertised vocabulary, the select
  options and the handler's validation are one list. Generate itself is
  deliberately NOT a target — starting a metered run stays the user's press).
...
- **2026-08-10 — Image Generate surface agent-writable (live-verified).** `matrx-user/image-generate` declares 4 ask-policy **draft** targets covering the whole Generate request: `generation_prompt` (the jackpot — an authored prompt is exactly what an agent drafts better than a human types), `generation_style`, `generation_size`, `generation_count`. Handlers live on `GenerateShellClient`'s own provider (`getWriteHandlers`) and call the SAME `useState` setters the user's typing does, so an agent write is indistinguishable from a keystroke. **Generate itself is deliberately not a target** — the run is metered, so starting it stays the user's press; nothing an agent does here spends a generation or persists a file. The size vocabulary was promoted out of a re-typed local `type Size` into `features/image-studio/types.ts`, where `GenerateImageSize` DERIVES from the generated `GenerateImageRequest` contract and a `Record<GenerateImageSize, string>` label map makes it exhaustive; the manifest interpolates the enum into its target descriptions, the select options render from it, `GenerateImageBody.size` is typed by it, and the handlers validate with `isGenerateImageSize`/`isGenerateImageCount` — one list, four consumers, no drift. Live-verified with a real Badass Agent run on `/images/generate`: one message drove all four targets, each ask dialog carried this manifest's description verbatim (enum text included), Apply staged into the visible form (Size → "Wide (16:9)", Style → "cinematic matte painting", Count → 3), "Keep as is" on the prompt declined with no error and the agent reported it gracefully and offered the text to paste. Negative paths quoted back verbatim by the agent: an undeclared `generation_model` → `No mounted surface declares write target "generation_model"`, and a literal `ultrawide` → the handler's own `generation_size expects one of: square | portrait | landscape | wide | tall.` Zero `surface-writeback` captures on the happy-path run. The surface's other route, `/images/ai-generate`, mounts the static `AIGenerateHero` placeholder — no provider, no editable field — so it gets no targets by decision, not by oversight. `check:surface-drift` + `type-check` clean; DB mirror (`ui.ui_surface_write_target`) pending a manifest sync.
