...
- **Agent-origin writes (the stream side)** — a run launched on a surface with
  ...
  and `matrx-user/scraper` (ONE ask-policy draft target, `scrape_request` —
  the whole next-scrape form as a single object `{scrape_mode?, target_url?,
  search_keyword?, max_pages?}`, handler on `ScraperFloatingWorkspace`'s own
  provider through `setMode` / `setUrl` / `setKeyword` /
  `keywordForm.setKeywords` / `setMaxPages`. THE reference for a surface
  whose write half is one COMPOSED request rather than independent fields:
  the mode decides which input is even rendered, so the handler validates
  against the POST-patch mode and rejects a field that mode does not use
  rather than silently dropping it, and `search_keyword` routes to whichever
  of the two keyword states the resulting mode actually reads. Also the
  reference for the OUTBOUND-ACTION line — running the scrape fetches a
  third-party site, so it is not a target at any policy and the button stays
  the user's press; scraped content, results and result selection are
  observed evidence and stay undeclared. `target_url` IS writable, but its
  contract forbids URLs the agent recalls rather than reads off `search_hits`
  / `scraped_links`, and the handler rejects anything that is not an absolute
  http(s) URL).
...
- **2026-08-10 — Web scraper surface agent-writable; the COMPOSED-REQUEST and OUTBOUND-ACTION references.** `matrx-user/scraper` declares ONE ask-policy draft target, `scrape_request` — the entire next-scrape form as a single object `{scrape_mode?: quick|full|search, target_url?, search_keyword?, max_pages?}` — with the handler on `ScraperFloatingWorkspace`'s existing provider (it already mounted one for reads; only `getWriteHandlers` was missing). Two things here are worth copying. **(1) One target, not four.** The skill's "separate targets when they're independent decisions" cuts the other way on a request form: the mode decides which input is even rendered and `max_pages` is meaningless outside deep mode, so the handler validates against the POST-patch mode — an agent that switches mode and fills its field in one call is judged by the new mode — and REJECTS a field that mode does not use instead of silently dropping it or silently switching the user's mode. `search_keyword` routes to whichever of the two keyword states the resulting mode reads (`keywordForm.setKeywords` for web search, local `setKeyword` for deep), the same split `contextData` already reports. Validate-then-apply throughout, so a rejected call leaves the form exactly as the user left it. **(2) The outbound-action line.** Running the scrape is an HTTP request against a third-party site, so it is not a target at ANY policy — the button stays the user's press, and the agent said so unprompted when asked to press it. Scraped content/results/selection are observed evidence and stay undeclared: a writable scraped body would let an agent launder invention into something the user reads as fact. `target_url` was the close call and is IN with its risk written into the contract rather than waved off — the surface PRODUCES the URLs worth writing (`search_hits`, `scraped_links`), so the description forbids URLs the agent recalls rather than reads, the handler rejects anything that is not an absolute http(s) URL, and `ask` plus the un-pressed button leave two human gates. Handler refuses loudly while `is_scraping` (the inputs are `disabled` then — an agent gets the same answer the keyboard does). The `quick|full|search` vocabulary is now exported from the manifest (`SCRAPE_MODES`) and the enum in the description is BUILT from it; `buildScraperContextData` maps onto it and derives its reverse map by inverting, so prose, emitter and validator cannot drift. Live-verified with real Badass Agent runs: the ask dialog carried the manifest's own prose, Apply flipped Web→Deep and staged keyword + 7 pages in one call, "Keep as is" left the form untouched with no error, "press Scrape for me" was refused with an explanation and no dialog, and a forced invalid call returned `client_tool_error: scrape_request.max_pages must be a whole number between 1 and 20.` with no partial write. Error Inspector: zero `surface-writeback` captures (only the pre-existing `request_context_changed` stream warning). `check:surface-drift` + `type-check` clean. DB mirror pending the admin manifest sync.
