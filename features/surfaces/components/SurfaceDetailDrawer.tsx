...
export function SurfaceDetailDrawer({ surface, onClose, onChanged }: Props) {
  ...
  return (
    <MatrxDynamicPanelHost
      ...
      description={`Created ${new Date(surface.created_at ?? "").toLocaleDateString()} · Updated ${new Date(surface.updated_at ?? "").toLocaleDateString()}`}
      position="right"
      defaultSize={42}
      ...
      <div className="flex-1 overflow-auto p-5 space-y-6">
        ...
        <section className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Label className="text-xs">
              {surface.is_active
                ? "Active — visible to tools and agents"
          ...
          <div className="space-y-1.5">
            ...
            {renaming ? (
              ...
              <p className="text-[11px] text-muted-foreground">
                Renaming cascades to {surface.toolCount + surface.agentCount}{" "}
                dependent row
                {surface.toolCount + surface.agentCount === 1
                  ? ""
                  : "s"} via{" "}
        ...
        <section className="space-y-2">
          ...
          {!loading && usage && usage.agents.length === 0 && (
            <EmptyHint>
              No agents are scoped to this surface. (Agents with no surface
              scope are visible everywhere.)
            </EmptyHint>
          )}
          ...
          <section className="space-y-2">
            ...
            <ul className="rounded-md border border-border bg-card divide-y divide-border">
              {usage.uiComponents.map((u) => (
                <li
                  key={u.id}
                  className={`px-3 py-2 ${u.is_active ? "" : "opacity-60"}`}
                >
