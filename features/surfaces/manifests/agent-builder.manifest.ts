...
const writeTargets: SurfaceWriteTarget[] = [
  {
    ...
    label: "System instruction",
    description:
      "Writes the agent's system prompt. A plain string REPLACES the whole prompt — read `system_instruction` first and include everything you want kept. To change PART of a long prompt, send an anchored edit instead ({command: \"str_replace\", old_str, new_str}) and leave the rest untouched; {command: \"append\", new_str} adds to the end. Non-text blocks attached to the system message (files, images) are preserved either way. Staged into the editor as unsaved changes; the user reviews and saves.",
    valueType: "string",
