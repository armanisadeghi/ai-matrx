...
Sitemap documents and page-membership evidence are system-written: users only activate/deactivate or delete a document, and a deleted document reappears on the next sync that re-discovers it. The work here is interpreting coverage — a listed URL missing from crawls, a 4xx sitemap, a stale fetch — and recommending the next action, never fabricating registry state.
