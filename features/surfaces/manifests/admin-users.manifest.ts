...
export const adminUsersManifest: SurfaceManifest = {
  ...
  readiness: "partial",
  readinessNote:
    "Emitter wired and verified live (2026-08-13): AccountsTableClient mounts SurfaceRuntimeProvider and every declared value is emitted synchronously from live render state via buildAdminUsersScope. Short of `verified` on two counts: the `ui_surface` row is seeded but the manifest DB sync has not been run from the admin Surfaces page, and only the roster/table anchors carry `data-surface-value` (the focused-account fields render inside the focus banner and the table body, which has no per-field anchor seam). Write targets are ruled OUT by design — see the manifest header.",
