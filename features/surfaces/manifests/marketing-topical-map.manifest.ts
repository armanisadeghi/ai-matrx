...
Every change goes through the topical_map tool, never by writing rows.
  ...
  // from the pages workspace's run controls (Lane F); `topic_curation` from the
  // topic panel's controls (Lane D) (CONTRACTS §6).
  agentRoles: [
    {
      name: "map_curation",
    ...
    // on the first change here after the package publishes (CONTRACTS.md §6).
    {
      ...
      kind: "single",
      defaultAgentId: null,
      mandateKey: dbAuthoredMandateKey(MANDATE_KEYS.seo__page_mapper),
      allowCustom: false,
      autoRun: "never",
      sortOrder: 200,
    },
    {
      ...
      kind: "single",
      defaultAgentId: null,
      mandateKey: dbAuthoredMandateKey(MANDATE_KEYS.seo__page_intent_proposer),
      allowCustom: false,
      autoRun: "never",
      sortOrder: 210,
    },
