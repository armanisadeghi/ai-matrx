...
export const smartCodeEditorManifest: SurfaceManifest = {
  ...
  readiness: "verified",
  readinessNote:
    "Scope + write targets emitted live by the `SurfaceRuntimeProvider` in `SmartCodeEditor` (the shared core behind both the floating window and `SmartCodeEditorModal`). Content and selection are read from the Monaco model at trigger time. The older standalone `MultiFileSmartCodeEditorWindow` is a separate component and does not mount this surface. Re-verified 2026-08-12 on a MAPPED route after the `SurfaceAgentsPanelImpl` overlay fix: with the window open over /dashboard the popover names Smart Code Editor and runs against ITS scope (reverting that one line puts `matrx-user/dashboard` back), which it could not do before — this window's main real caller opens it from a chat code block, i.e. always over the mapped `/chat/[id]`. Exercised live: `content` and `insert_at_cursor` both landed in the Monaco model, `replace_selection` correctly refused with nothing selected, and writes to `file_path` / `language` were refused as read-only context. READS are fine — the agent quoted the exact buffer and language — so this surface is NOT affected by the overlay read gap. Note `/smart-code-editor` is a route→surface mapping with no page behind it; the window and the modal are the real mounts.",
