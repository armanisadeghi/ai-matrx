...
- **Chapter timing is playable before it is public.** `podcastService.saveEpisodeChapters` loads browser audio metadata for every save, uses its exact value to bound markers and disclosures, and records its nearest positive whole-second value in the `pc_episodes.duration_seconds` `int4` column. It accepts only strictly increasing whole-second starts before media end, proportionally repairs a generated list whose stale hint exceeds the real audio, or refuses an impossible list. Both Generate and `episode_chapters` surface writes show the exact actual-duration adjustment and tell the owner to review markers before publishing. A surface-written list remains visible during a regeneration until a newly persisted replacement succeeds; the live run can display raw agent output until then.
...
- 2026-08-20 — **Podcasts is fully on the Mandate system, and now has a door to
  ...
  **`/mandates?feature=podcast`** ("Podcast agents") — before this the
  ...
  `common-docs/systems/mandates/STATE.md`.
