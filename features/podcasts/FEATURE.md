...
- **Chapter timing is playable before it is public.** `podcastService.saveEpisodeChapters` loads browser audio metadata for every save, uses its exact value to bound markers, and records its nearest positive whole-second value in the `pc_episodes.duration_seconds` `int4` column. It accepts only strictly increasing whole-second starts before media end, proportionally repairs a generated list whose stale hint exceeds the real audio, or refuses an impossible list. Both Generate and `episode_chapters` surface writes show the actual-duration adjustment and tell the owner to review markers before publishing; the live run can display raw agent output until the persisted list replaces it.
...
- 2026-09-17 — **Chapter saves cannot publish unreachable seek targets silently.** The shared `podcastService.saveEpisodeChapters` boundary now reads the browser's exact audio metadata duration, records its nearest positive whole-second value in the `duration_seconds` `int4` column, and normalizes stale generated offsets into strictly increasing playable timestamps while preserving `00:00` and pacing. It refuses unknown, nonpositive, malformed, nonincreasing, or impossible lists. The generator and `episode_chapters` write target both disclose an adjustment with the measured duration and a review-before-publish remedy; their rendered final list is the saved list, while the live streaming window may still show the raw agent output.
...
- 2026-09-16 — **Podcast Studio refuses deterministic thin-source failures before a run starts or repeats.** The shared source-readiness boundary mirrors the server’s Gate 1: trimmed `full_content` and `partial_content` need 1,000 characters unless they contain a non-empty `<podcast_dialogue>` block; topic and file sources remain eligible for external preparation. The create form explains an invalid nonempty paste beside the source, while recovery suppresses Resume and Re-run for a saved request that would fail the same gate again. Terminal errors also outrank missing-metadata and stale background-work signals; durable failure detail reaches the humanized banner and survives the later liveness reconcile. A failed rail says `Stopped`, counts completed and failed stages separately, omits success-like percentage progress, and suppresses speculative queued/running media while preserving real delivered or failed artifacts. The page therefore never claims it is still composing, generating, or preparing cover art and never hides its actionable cause behind a generic interruption.
