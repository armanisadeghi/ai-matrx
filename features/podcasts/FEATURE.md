...
- 2026-09-16 — **Podcast Studio refuses deterministic thin-source failures before a run starts or repeats.** The shared source-readiness boundary mirrors the server’s Gate 1: trimmed `full_content` and `partial_content` need 1,000 characters unless they contain a non-empty `<podcast_dialogue>` block; topic and file sources remain eligible for external preparation. The create form explains an invalid nonempty paste beside the source, while recovery suppresses Resume and Re-run for a saved request that would fail the same gate again. Terminal errors also outrank missing-metadata and stale background-work signals; durable failure detail reaches the humanized banner and survives the later liveness reconcile, so a failed run never claims it is still composing or generating and never hides its actionable cause behind a generic interruption.
