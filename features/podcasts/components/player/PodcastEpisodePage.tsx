...
export function PodcastEpisodePage({ episode, articles = [] }: PodcastEpisodePageProps) {
    ...
    // Chapter markers, generated by the podcast.chapter_marker Mandate and
    // persisted on `pc_episodes.metadata.chapters` (already parsed onto the
    // episode by `mapPcEpisodeRow`). The list is rendered by the ONE
    // `media_chapters` component and seeks THIS page's player through its
    // imperative handle — no second renderer, no second audio element.
                            ...
                            {!showNotes && (
                                <div className="flex items-center gap-2.5 rounded-xl border border-dashed border-border bg-muted/20 px-3 py-2.5">
                                    ...
                                    <span className="flex flex-1 items-center gap-2 text-sm text-muted-foreground">
                                        {/* Chapters are live above once generated —
                                            never advertise them as coming soon then. */}
                                        {hasChapters ? 'Show notes' : 'Chapters & show notes'}
                                        <ComingSoonBadge />
                                    </span>
                                </div>
                            )}
