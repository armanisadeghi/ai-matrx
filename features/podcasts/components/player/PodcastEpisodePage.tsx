...
export function PodcastEpisodePage({
  ...
  const thumbnailImage =
    episode.thumbnail_url ?? episode.show?.thumbnail_url ?? coverImage;
  ...
  const effectiveMode = (() => {
    if (
      episode.display_mode === "with_video" &&
      episode.video_url &&
      !videoFailed
    )
      return "with_video";
    if (
      (episode.display_mode === "with_metadata" ||
        episode.display_mode === "with_video") &&
      coverExists
    )
        ...
        <div className="relative z-10 h-full flex flex-col justify-end px-4 sm:px-6 pb-6 w-full">
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <div className="min-w-0 flex-1 max-w-3xl [text-shadow:0_1px_3px_rgb(0_0_0/0.85)]">
              {episode.show?.title && (
                <p className="text-white/70 text-xs font-medium uppercase tracking-widest mb-1 truncate">
                  {episode.show.title}
                </p>
              )}
          ...
          <div className="px-4 sm:px-6 pt-3 pb-4 max-w-3xl mx-auto w-full">
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="min-w-0 flex-1">
                ...
                <h1 className="text-foreground font-bold text-xl sm:text-3xl leading-tight break-words">
                  {episode.title}
                </h1>
            ...
            {showNotes && (
              <EpisodeShowNotes article={showNotes} className="mt-5" />
            )}
      ...
      <div className="flex flex-col items-center gap-4 w-full max-w-2xl mx-auto">
        ...
        {episode.title && (
          <div className="flex items-center gap-2 w-full justify-center">
            <h1 className="min-w-0 text-foreground font-bold text-xl sm:text-2xl text-center leading-snug line-clamp-2 flex-1 break-words">
              {episode.title}
            </h1>
            <ShareButtonLight />
          </div>
