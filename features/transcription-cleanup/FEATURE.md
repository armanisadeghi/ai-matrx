...
- Persist clean/custom output exactly once per completed conversation
  (`persisted*CidRef`).
- **No spinner while AI works — THE FLOATING LAW.** Every in-flight pass shows
  the stage it is actually in (`AI_PHASE_LABEL` / `<LivePhaseBadge>`), and a
  pass whose own pane is off screen (an unselected Custom tab, a hidden Clean
  pane) floats its run in `liveRunWindow` — one window per slot, closed when
  its tab comes back on screen. The only surviving `Loader2`s are the
  session-create button and `PaneLoadingVeil`, which are database work.
...
- 2026-08-15 — **A reload no longer kills a cleanup pass (class D closed).** The pad recorded its `studio_runs` row at COMPLETION, so a page reload mid-run left nothing on the server to find: the run, and the output it was about to produce, were gone. The row is now opened as the pass LAUNCHES (`useCleanupSession.beginRun`, concurrently with the agent so it costs the run no latency), stamped with its conversation the moment one exists (`bindRunConversation`), carries `metadata.target` = `"clean"` or the slot's `docKind`, and is finalized — never re-inserted — by the existing completion effects. On load, `loaded.resumableRuns` feeds the shared primitive [`features/transcript-studio/redux/reattachStudioRun.ts`](../transcript-studio/redux/reattachStudioRun.ts): it floats the run's `LiveRunWindow` (same stable `instanceId` the live float uses), rejoins the run on aidream's runtime spine (`reconnectServerOperation`, cold-load) or takes over when the resumed stream reopens in this tab, and hands the finished text back to the container the run recorded — persisted exactly as a live completion would be. A pass that ended badly closes its row (`failRun`) so nothing sits at "running" forever. Only ANSWER parts are recovered: persisted assistant content is a part array and its `type: "thinking"` part would otherwise have been saved into the user's cleaned transcript (caught in browser verification). Live-verified end to end on `/transcripts/cleanup`: launch → reload mid-run → the window narrates "Rejoining the run / Finishing on the server / Restoring it into your page" → the cleaned text lands in the Clean pane, the cleaned segment is written, and the run row settles `complete`.

- 2026-08-15 — **The pad's local `syncFloatingRun` is gone; visibility-gated floating is now the canonical primitive.** Two hooks expressed "float this run in a window" because parallel sessions each landed one: `useFloatingRunWindow` (launch-time) and this file's local copy (visibility-gated). The gate folded INTO the primitive as `useFloatingRunWindow({ track: { running, conversationId, label, visible } })` — `visible` suppresses the float while the run's own pane is on screen — and the pad now calls it once per pass (clean + the three fixed slots, `owns: false` because each run belongs to its `useAiPostProcess`). Same behaviour, one implementation. Live-verified on `/transcripts/cleanup`: a clean run with its pane visible never floats; a slot run floats the moment its tab is deselected and closes again mid-run when the tab comes back. The launch-time path (`/education/mind-maps/new`) is unchanged and still floats on launch.
- 2026-08-15 — **agent-copy wired onto the cleanup recents rail.** `CleanupSessionList` had no Copy / Copy-for-AI. It now carries a list pair + `ExportMenu` (JSON + CSV) beside the sessions toolbar, and a hover-revealed per-session pair next to the existing delete action. Payloads reuse `features/transcript-studio/format.ts` rather than forking a second session shape — this rail renders `StudioSession[]` from the same store, so a session copied here and the same session copied in the Studio sidebar are byte-identical. The rail's **scope toggle travels in the envelope `attributes`**: "Mine" lists only this surface's `source='cleanup'` sessions while "All" widens to every session RLS allows, so a payload that omitted it would be unreadable — the agent could not tell a short list from a narrow one. `pnpm type-check` clean. Not yet wired: `CleanupPad` (the cleaned-document record) and `CleanupContextPanel`.
- 2026-08-11 — **Every step of the pad narrates its stage; the spinners are gone.** Record → auto-clean → refine-with-N-agents fires up to four agent runs at once and each announced itself with a spinning circle. Each is now a live badge naming the real stage (Starting / Queued / Connecting / Thinking / Writing / Using tools) on the Clean Up button, both pane headers, the Custom slot tabs, and the Run button — while the output streams into the pane, or into that pass's floating `liveRunWindow` when its pane is off screen. Live-verified with a real clean run on a real session. See the Invariants entry above for what stays a loader and why.
- 2026-08-11 — **Every concurrent pass is watchable: a refine slot that runs off-screen now floats (THE FLOATING LAW, multi-step sweep).** The pad's whole model — record → auto-clean → refine with ANY number of custom agents — fires up to `MAX_CUSTOM_SLOTS + 1` agent runs at the same time, but the custom slots are tab pills so only ONE is ever on screen, and an embedded host can hide the Clean pane outright (`sections.clean`). Every hidden pass streamed into a pane nobody could see, represented by a 12px spinner on a tab: exactly the "spinner while AI works" the law bans, and the reason the pad was flagged in the first sweep. It streams into its panes correctly — the defect was purely that most panes are not visible. A pass whose pane is off-screen now opens the floating `LiveRunWindow` bound to its `conversationId`, one window per slot with a stable `instanceId` keyed on the pad's voicePad instance, so a re-run rebinds the same window instead of stacking; switching to a pass's own tab closes its window, because the pane is the better home when it is actually on screen. ~~**Still open (class D):** a reload mid-run loses the output.~~ **Closed 2026-08-15** — see the durable-run entry at the top of this log. See [`docs/handoffs/live-run-streaming-sweep.md`](../../docs/handoffs/live-run-streaming-sweep.md) § Multi-step pipelines.
- 2026-08-10 — **The pad is agent-writable, and finally mounts a `SurfaceRuntimeProvider`.** The page variant's return is now wrapped in `<SurfaceRuntimeProvider surfaceName={CLEANUP_SURFACE_NAME} getScope={buildScope} isEditable getWriteHandlers={…}>`. Before this the pad emitted its scope ONLY through the panes' `getApplicationScope` (context-menu launches); nothing registered into the surface runtime registry, so a run started from the shell header's "Agents for this page" saw no live surface at all. Reusing the existing `buildScope` means both launch paths emit one identical payload. Three write targets, all `applyPolicy:"ask"` (see the `writeTargets` block in [the manifest](../surfaces/manifests/transcripts-cleanup.manifest.ts) for the full rationale): `cleaned_transcript_text` and `custom_output_text` are `mode:"draft"`, take `{text, mode?: 'replace' | 'append'}`, and land through `handleResponseChange` / `handleCustomChange` — the pane textareas' own `onChange`, so an agent's revision is indistinguishable from the user typing; `session_title` is `mode:"entity"` through `updateSessionThunk` (the thunk the sidebar's Rename dispatches). The mode vocabulary is canonical in [`constants.ts`](./constants.ts) (`CLEANUP_TEXT_WRITE_MODES`) so the manifest prose and the handler validation cannot drift. **Only `variant="page"` registers** — embedded pads (War Room thread Audio tabs) deliberately register nothing, since a room can mount several over different sessions and deepest-wins would send an agent write to whichever tile mounted last. `raw_transcript_text` stays undeclared on purpose: it is the source of truth, and the Clean/Custom containers exist so it never has to be overwritten. Recording/transcription flags stay undeclared too — those are device actions, not values. Live-verified with a real Badass Agent run (3 targets in one message all applied while the raw transcript stayed byte-identical, decline clean, undeclared target refused by the seam, invalid value surfaced the handler's own throw); zero `surface-writeback` error captures on the happy path.
