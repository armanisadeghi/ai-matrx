...
Pick-up doc for the next agent. The artifact system is the most important feature in the app. Vision reference: [`ARTIFACT_VISION_AND_DESIGN.md`](./ARTIFACT_VISION_AND_DESIGN.md). Shipped reality: [`../FEATURE.md`](../FEATURE.md).
...
- **R1 pipeline (the heart):** materialized blocks store as normalized text `<artifact type id version>body</artifact>`; UUID-id → render-by-id (R3, `artifact-types/artifactId.ts`); idempotent; `artifact_ref` block type deleted; 19 live messages migrated. Model reads artifacts natively (Wave 1). Versioning IS in place (`canvas_items.version` + `parent_canvas_id` + `cx_canvas_save_user_version`).
