...
## Open hand-offs for the artifact/materialization work (HTML inline preview)

The HTML inline auto-preview shipped on the frontend (`features/html-pages/components/HtmlInlinePreview.tsx`). These items are intentionally left for whoever finalizes artifacts, because they belong to the materialization / publishing layer, not the preview UI:

1. **Canonical `<artifact type="html" id>` rewrite + reference load (the big one).** Today every inline render that lacks a stable artifact id re-publishes/re-saves, so each open spawns a NEW reference (visible as canvas version churn — `2/2`, `3/3`, …). The fix is the R1 rewrite (recognize → assign UUID → rewrite the message) so reload **fetches the artifact by id** instead of regenerating. Dedupe must move to the `(source_message_id, artifact_index)` natural key (like `canvas_items`) — the html-pages page dedupe (by `source_message_id` + identical `html_content`) is a stopgap and collapses multiple html blocks in one message to a single page.
2. **Register the `cx_artifact` / `canvas_items.external_system` link from the inline path.** The `HtmlPreviewBridge` editor path already does this (`registerArtifactThunk`, `external_system: "html_pages"`); the inline preview should converge on the same materialization rather than fire-and-forget publishing.
3. **Content-aware inline height (so the bottom fade only appears when truly truncated).** The inline preview loads a **cross-origin** published URL, so it can't measure real content height — it caps height (`min(70vh,720px)`, expand → `min(85vh,1400px)`) and fades the bottom edge with Expand / Canvas actions as a deliberate, honest truncation. To fit pages exactly (fade only when there's actually more below), have the **published page** post its height to the parent via `postMessage` — i.e. inject a tiny height-reporter `<script>` into the published HTML at publish time. That's a publishing-layer change (it modifies stored page content), so it lands with this work, not in the preview component. The canvas already renders full-height and is the full-fidelity escape hatch.

---

## Change log
- `2026-06-19` (5) — claude: **HTML inline preview sizing + canvas video fix.** Canvas/iframe renderers (`IframeArtifact.tsx`, `PublicCanvasRenderer.tsx`) now include `allow-same-origin` + media `allow` on the cross-origin URL branch — required for YouTube/Vimeo players (was a black frame); still withheld on the `srcDoc` branch (XSS). Inline full-document preview no longer hard-cuts: generous capped height + deliberate bottom fade hosting Expand / Open-in-canvas. Added **"Open hand-offs for the artifact/materialization work"** section above — the artifact-id rewrite/reference-load, the `cx_artifact` link from the inline path, and the publish-time height-reporter for content-aware sizing all belong to that work.
