...
- `2026-07-27` — claude: **`artifact-renderers.tsx` registry converted 30× `React.lazy` → `next/dynamic({ssr:false, loading})`** (React.lazy is banned; it also compiled every heavy renderer graph — mermaid, charts, maps, react-runner — into the server pass of any route that statically reaches the registry, e.g. `/artifacts/[id]` via `CmsArtifactDetail`). Per-type chunking and the `MatrxMiniLoader` loading state are unchanged; `ArtifactRender` keeps its `<Suspense>` for renderers that still suspend internally. Browser-verified on `/artifacts/[id]` (diagram + table).
