...
- 🚨 **ONE PREDICATE DECIDES WHETHER AN ANSWER HAS WORDS: `hasWords` in `answerWords.ts`** (a value has words when it holds at least one non-whitespace character). Every save path asks it and no other question — the write box, the ask table's inline box, the review triage's overturn box, and `saveAnswer` itself as the backstop. Before 2026-09-14 each asked `length === 0` separately, so three spaces overturned a real decision on production (`v2r-02`, `answer_text` hex `202020`, delivered) and the server refiled a question whose whole premise read *"You overturned it and said, verbatim:    "*. The database agrees since `qd_005` (`answer_text ~ '[^[:space:]]'`). **The predicate never touches the value that is stored** — deciding with a trim and storing untrimmed is deliberate: the check is whether he said anything, never a tidy-up of what he said. Forcing test: `__tests__/answer-words.test.ts` (its header carries the red recipe).
- **This surface does NOT wire `ProTextarea`'s `onSubmit`.** Its submit gate is `value.trim().length > 0` and it refuses silently; the save stays this feature's, so the refusal a person meets is always *"Write something first, or use one of the buttons."* — the same predicate, with a sentence.
...
- **The rail's legend reserves the band the shell's floating chrome ACTUALLY occupies** — `--shell-fixed-corner-clearance`, measured and published by the Error Inspector badge itself (`features/admin/error-inspector/useFixedCornerClearance.ts`). A guessed `lg:pb-14` only moved the collision one line up: at 1440×900 the chip still covered "Answers save straight to the record." (finding 6, 2026-09-12 → V2 finding 7, 2026-09-14). Nothing floating there publishes nothing, and the legend reserves nothing. (The schedule alarm deliberately publishes semantic state instead — its card is draggable and can be viewport-tall, so measuring it WOULD feed layout back into itself. Different chrome, different rule.)
- **`V` starts the microphone in BOTH states.** `openWrite({dictate:true})` used to set a pending flag and call `setWriting(true)`, a no-op when the box is already open, so the effect that consumes the flag never re-ran and "Answer by voice" did nothing at all in the state a person is most likely to press it in (V2 finding 4). With the box open it starts the field's recorder now; with it closed the flag survives until the box has committed, because `startDictation()` needs the field to exist.
- **A refusal that arrives LATE still reaches the alert.** `startDictation()` returns `{started:true}` as soon as its five pre-checks pass; a microphone the browser then DENIES rejects afterwards, inside the recorder, and reaches the field's `onTranscriptionError`. Every box on this surface now passes that handler into the same refusal slot it uses for everything else — the sticky bar's `role="alert"` for the write box, the words-error line for the inline and overturn boxes — so the person is told where they are looking and not only by a toast that disappears (V2 finding 3).
- **"k batches done" is DERIVED from the rows** (`answered / batch size`), never component state. It was state nothing incremented, so someone working 117 decisions across sittings always read "0 batches done" beside honest numbers (V2 finding 5). Derived, it survives a reload and stores nothing.
- **The rail says when answers are waiting for an agent.** An answer whose server stamp reads `awaiting_work` is sitting in the `question-desk` Work Loop campaign; its `status_note` says "recording…", which reads as work in flight when it is work WAITING (ten items sat seven hours, V2 finding 6). The count is read off the question rows' `metadata.follow_through` (`followThrough.ts`), not off `runtime.work_item` — that campaign is system-owned and a client read under RLS returns nothing, and a count that is always zero is worse than no count.
...
- `2026-09-14` — lane G (Claude Opus 5), the V2 hostile re-verify's client findings: ONE `hasWords` predicate behind every save path (three spaces can no longer be an answer, and `qd_005` makes the database agree); the denied-microphone refusal reaches the alert slot and not only a toast; `V` works with the box already open; "k batches done" derived from the rows; the rail says how many answers are waiting for an agent to record them; the key legend clears the measured band the shell's error chip occupies. Not closed this round: the pre-hydration keystroke that navigated the app away (V2 finding 8) — an exhaustive search of this repo found NO handler that maps a bare letter to a route and nothing that could navigate to `/settings/organizations` from a keypress, so there is nothing here to fix and the repro needs re-capturing with a breakpoint on `history.pushState`.
