...
- `features/marketing/findings-assists-producer.ts` — the SEO analysis register's open findings become chips (growth-loop gap `G-FINDING-ASSIST`). ONE indexed read per site per session over `web.finding`, allowlisted to `aiRemedyItemKeys()` so a check whose only remedy is a manual instruction can never produce a button with nothing behind it; at most two page chips (severity first, appliable-metadata checks ranked up) plus one cross-page rollup that navigates to the register filtered to that check. `FindingsAssistStrip` is site-filtered by dedupe key and mounted on all three surfaces where findings are read — the register, the priority queue, and the audit rollup. **The site filter must match `seo.finding` as a prefix, NOT `seo.finding.`** — the dotted form silently dropped every `seo.finding_rollup.…` chip from the page strip while it sat in the ledger.
...
- 2026-08-13 — **The loop closes on the PAGE (`G-FINDING-FIX`).** New action kind `apply_page_meta`: accepting a findings chip no longer merely opens an agent — when the fix is derivable, the chip carries the exact replacement text and one verb button lands it as the page's desired metadata plus a CMS DRAFT (`applyFindingFix` → `updatePageIntent` + `executeCmsPush`). No new write path, no publish, no route move. Deliberately NOT a `server_action`: the whole operation is client-side against Supabase and the CMS seam, so routing it through Python would be the DB-gateway anti-pattern.
...
- 2026-08-13 — SEO findings became assists (`G-FINDING-ASSIST` closed). The producer and its strip had been committed and never mounted — dead code, zero finding-sourced ledger rows against 3,736 findings. Mounted on the register, priority queue, and audit rollup, and the strip's site filter fixed so rollup chips stop being emitted-but-invisible. **The lesson for every producer:** a producer with no mounter produces nothing, and it compiles — verify a ledger row exists, never a green build.
