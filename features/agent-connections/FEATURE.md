...
- **Selected binding facts are narrow.** `fetchCodingSessionBindings` may project provider/session/fidelity/runtime/workspace/capability metadata for one canonical conversation, but never raw `chat.coding_session_entry` rows. Account presentation reads only explicit opaque fingerprint fields; arbitrary metadata, email, OAuth credentials, and tokens never render.
