...
// the URL on mount (so a refresh/shared link wins), then a view→URL write via
// `commitUrlParams` (a history write — no Next navigation, no Suspense
// boundary, no scroll jump — that still notifies every other url-state control
// on the page). `thread` only hydrates once the tile is actually visible — a
