...
- Agent tab (`TileAgentTab` → lazy `TileAgentPanel`): the **real Scribe Agent+ panel**, reused unchanged — `AssistantAgentBar` + `WorkingDocumentHeader` + `ExperimentalAgentScreen` (`features/transcript-studio/components/scribe/*`) over hooks `useStudioAssistant` / `useAutoVoiceResponse` / `useWorkingDocumentDraft`, all keyed by a `studio_sessions.id`. `TileAgentTab` resolves the tile's session exactly like Audio (`selectActiveAudioSessionId` → `ensureTileAudioSession`); the panel is code-split (`next/dynamic` ssr:false) and re-fetches the working document on each completed turn to cover the single-active-session realtime gap. Expand via `useOpenTranscriptStudioWindow` (same session as Audio). Nothing reimplemented.
