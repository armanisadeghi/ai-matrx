...
- **Generic task↔entity M2M** → canonical `platform.associations`. The `get_task_associations` / `get_tasks_for_entity` / `associate_with_task` / `dissociate_from_task` / `create_task_with_association` / `create_tasks_bulk` RPCs now read/write `platform.associations` (source/target `task`) — the FE surface (`taskAssociationsSlice`, `AssociateTaskButton`) is unchanged. The legacy `ctx_task_associations` table is retired to the `graveyard` schema.
...
- `2026-06-14` — claude: **Fixed spurious "duplicate" error on project creation (double-write).** The DB trigger `trg_ctx_projects_add_creator_membership` (AFTER INSERT on `ctx_projects`, SECURITY DEFINER, `ON CONFLICT (project_id, user_id) DO NOTHING`) already writes the owner `ctx_project_members` row, but `features/projects/service.ts → createProject` *also* manually inserted that row — the second write violated the `(project_id, user_id)` unique constraint and made `createProject` return `{success:false, error: "duplicate…"}` even though the project + membership were fully created. Removed the manual member insert; the trigger is now the single source of truth (matching the legacy `features/tasks/services/projectService.ts → createProject`, which never inserted it). This surfaced both as "create project fails" and "create project from the tasks sidebar fails" (`createProjectThunk` and `hierarchyService.createProject` both route through the canonical service). **Tasks themselves were never affected** — `ctx_tasks` has only a PK on a server-generated `id`, so task inserts cannot collide.
