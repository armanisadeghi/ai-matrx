...
describe("snapshot receipt cursor", () => {
  ...
  it("round-trips a typed null cursor and rejects malformed cursor input", () => {
    ...
    expect(() => decodeSnapshotReceiptCursor("not-base64")).toThrow(
      "Refresh required",
    );
  });
  ...
  it("uses source identity without offset page or unsupported query controls", () => {
    ...
    const second = snapshotSourceIdentity({
      ...state,
      page: 1,
      anyOf: "another",
      layeredFilters: [],
  ...
  it("keeps whole-word searches explicit instead of aliasing them to contains", async () => {
    ...
    expect(
      snapshotSourceIdentity({ ...state, searchMatchMode: "contains" }),
    ).not.toEqual(snapshotSourceIdentity(state));
  });
  ...
  it("checks the un-cursored exact total before applying a second-page keyset", async () => {
    ...
    await expect(
      listSnapshotReceiptPage({
        siteId: "site-1",
        pageId: "page-1",
        state: { ...state, searchMatchMode: "contains" },
        watermark: cursor.watermark,
        cursor,
        expectedTotal: 3,
      }),
    ).resolves.toMatchObject({ total: 3, rows: [{ id: "snapshot-3" }] });
    expect(countQuery.or).not.toHaveBeenCalledWith(
  ...
  it("refuses a later exact-count drift before merging its rows", async () => {
    ...
    for (const method of methods)
      countQuery[method].mockReturnValue(countQuery);
    countQuery.abortSignal = jest
      .fn()
      .mockResolvedValue({ data: null, error: null, count: 4 });
    jest.mocked(authenticatedWebDb).mockResolvedValue({
  ...
  it("refuses a terminal count increase after a full first page before publishing it as complete", async () => {
    const methods = [
      "select",
      "eq",
      "lte",
    ...
    const firstCount = makeQuery();
    const rows = makeQuery();
    const terminalCount = makeQuery();
    firstCount.abortSignal = jest
      .fn()
      .mockResolvedValue({ data: null, error: null, count: 25 });
    rows.abortSignal = jest.fn().mockResolvedValue({
      data: Array.from({ length: 25 }, (_, index) => ({
        id: `snapshot-${index + 1}`,
        captured_at: `2026-09-12T00:${String(index).padStart(2, "0")}:00Z`,
      })),
      error: null,
    });
    terminalCount.abortSignal = jest
      .fn()
      .mockResolvedValue({ data: null, error: null, count: 26 });
    jest.mocked(authenticatedWebDb).mockResolvedValue({
      from: jest
        .fn()
        .mockReturnValueOnce(firstCount)
        .mockReturnValueOnce(rows)
        .mockReturnValueOnce(terminalCount),
    } as never);
    ...
    await expect(
      listSnapshotReceiptPage({
        siteId: "site-1",
        pageId: "page-1",
        state: { ...state, page: 1, pageSize: 25, searchMatchMode: "contains" },
        watermark: "2026-09-13T00:00:00Z",
        verifyTerminal: true,
      }),
    ).rejects.toThrow("matching count changed at the terminal page");
    expect(rows.abortSignal).toHaveBeenCalledTimes(1);
    expect(terminalCount.abortSignal).toHaveBeenCalledTimes(1);
  });
});
