...
export interface CollectionStatusRow {
  ...
  failure: ReturnType<typeof describeBackendFailure> | null;
...
export function CollectionStatusPanel({
  ...
  const status = useQuery({
    ...
    queryFn: async ({ signal }) => {
      const [response, latestRuns, trackedTargets] = await Promise.all([
        dispatch(
          callApi({
...
/** Every provider we know how to describe, for callers that need the list. */
export const KNOWN_COLLECTION_PROVIDERS = COLLECTION_PROVIDERS;
