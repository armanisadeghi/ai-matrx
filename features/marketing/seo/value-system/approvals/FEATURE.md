...
- `seo.gsc_topic_placement_diff` does not finish for a large site (All Green Recycling, `d0aff5b6…`: the queue shows "Could not load the inherited offering moved proposals", and the same call as the database owner ran past the SQL tool's timeout on 2026-09-14). It walks `history.row_versions` once per inherited keyword. The queue reports the failure with Retry; the read needs a set-based rewrite by the placement-ladder owner (KI-050).
