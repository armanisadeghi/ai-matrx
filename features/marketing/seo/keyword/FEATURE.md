...
| Reads + identity        | `data.ts`, `hooks.ts`                                                                                  | Direct-Supabase `seo` reads. Page-level GSC evidence uses the set-based `seo.gsc_perf_breakdown` RPC with `site_id + page_id`; dossier performance uses `seo.site_keyword_performance_for_keyword`, which applies `site_id + keyword_id` before freshness ranking. Browser reads never scan `seo.search_performance_daily`. `ensureKeywordId` is the canonical explicit-entry path: it upserts an arbitrary phrase through `seo.fn_upsert_keyword` and restores archived matches. `useKeywordVolumeRefresh` enriches the saved phrase through aidream; it is not required before selection. Query keys: `seoKeywordKeys`. |
...
- **DOSSIER PERFORMANCE IS A BOUNDED RPC.**
  `seo.site_keyword_performance_for_keyword` proves site access and puts both
  UUID filters inside the freshness window. Never restore a direct
  `v_site_keyword_performance` read here; PostgREST applies its filters after
  the generic view has ranked the site-wide observation history.
...
- 2026-09-11 — Codex: moved Keyword Intelligence's site-performance read from
  ...
  open exhausted the authenticated statement timeout.
