 ...
 *
 * ONE component behind every surface that analyzes a meta title/description:
 *   - the public Meta Width Calculator page (`/seo/metadata`)
 *   - the Search Appearance window panel (openable from anywhere)
 *   - any future embed (marketing, CMS, agent tool result panes)
 *
 * Fully prop-driven and host-agnostic: pass initial values and it renders the
 * editable inputs, the deterministic analysis (shared `metrics.ts` — exact
 * parity with the Python scraper), the Google chrome + desktop + mobile SERP
 * previews, and recommendations. Layout responds to the CONTAINER (not the
 * viewport) via `@container/serp`, so it two-columns on a wide page or a wide
 * window and stacks when narrow.
 */
export interface MetadataAnalyzerValues {
  url: string;
  title: string;
  description: string;
}

export interface MetadataAnalyzerProps {
  initialUrl?: string;
  initialTitle?: string;
  initialDescription?: string;
  /** Show the scrape-this-URL fetch button next to the URL input. Default true. */
  enableFetch?: boolean;
  /** Fires on every edit with the current values (for hosts that persist). */
  onValuesChange?: (values: MetadataAnalyzerValues) => void;
  className?: string;
}
...
export function MetadataAnalyzer({
  ...
  }, [url, title, description, onValuesChange]);

  // Deterministic + cheap (shared char-width table) — computed inline every
  // render; no canvas, no debounce, identical numbers to the Python scraper.
  const titleEval = evaluateMetaTitle(title);
  const descEval = evaluateMetaDescription(description);
  ...
  return (
    <div className={cn("@container/serp", className)}>
      <div className="grid grid-cols-1 gap-6 @[64rem]/serp:grid-cols-12">
        <aside className="space-y-4 @[64rem]/serp:col-span-4">
        ...
        <section className="space-y-4 @[64rem]/serp:col-span-8">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <span
                  className={cn(
                    ...
                    titleAllOk ? "bg-success" : "bg-destructive",
                  )}
                />
                Title
              </span>
              <span className="flex items-center gap-1.5">
                <span
                  className={cn(
                    "inline-flex h-2 w-2 rounded-full",
                    descAllOk ? "bg-success" : "bg-destructive",
                  )}
                />
                Description
              </span>
          ...
          {hasData ? (
            <Card className="overflow-hidden rounded-2xl shadow-sm">
              ...
              <CardContent className="px-5 py-4">
                <MetaRecommendations
                  titleEval={title ? titleEval : null}
                  descriptionEval={description ? descEval : null}
                />
              </CardContent>
            </Card>
          ) : null}
