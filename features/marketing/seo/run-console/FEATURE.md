...
> _"Instead of running it nightly, you create an admin dashboard for me where I
...
> schedule overrides the organization, which overrides the system."_
...
- `data.ts` — `listConsoleSites`, `listEngineSchedules`,
  `resolveSchedulesForSites` (THE CASCADE), `saveEngineSchedule`,
...
under that site's own `organization_id`. The durable pointer retains the same
per-launch organization for rejoin; the system tier never substitutes the
Matrx System row-owning organization for a brand's request tenancy.
...
- 🚨 **Every manual run names the selected site's organization.** A system
  admin may see every brand, but `X-Organization-Id` is tenancy, not an admin
  privilege or schedule-row owner. `useDurableRun.launch(..., {scopeOverrides})`
  persists that exact organization in the receipt so refresh/rejoin cannot
  drift back to the system organization or the operator's active picker.
...
- `2026-09-11` — **Manual runs carry the selected brand's organization through
  ...
  the site row already supplied its real tenant. `useDurableRun` now accepts a
  per-launch scope override and persists it in the durable receipt; `RunConsole`
  passes each queued site's `organization_id`. The shared regression guard
  proves both the initial request and refresh/rejoin keep the site tenant.
...
- `2026-08-25` — 🚨 **A SECOND ENGINE: SITUATIONAL REFRESH (KI-016), and the
  ...
  **Proven live on Data Destruction**, fixture removed afterwards: a matcher
  ...
  same button reported _"Applying your own rules may not apply anything without
  you — what it found is waiting in Approvals"_ with **matched 326 · proposed
...
- `2026-08-25` — **Organization and site tiers mounted; v1 is now all three
  tiers, live.** Arman: _"the question is if we already have this set up so
  ...
  override our system one."_ Answer: yes, mostly — the shape (`RunConsole`
...
- `2026-08-25` — **The two hand-rolled `<table>`s became the canonical
  `MatrxDataTable`, and the page names its AI.** Arman: _"you're showing
  tabular data, but you're not using our canonical table... you've handled
  this new shitty table that doesn't do what we needed to do."_ The brand
...
- `2026-08-24` — **Historical request-scope fix, superseded 2026-09-11.** The
  admin console originally used the system organization to make launch/rejoin
  independent of the active picker, and `useDurableRun` began forwarding its
  static scope on rejoin. That fixed pickup at the time but conflated a
  system-owned schedule row with a brand run's tenant; the current contract is
  the selected site's organization, persisted per launch as described above.
