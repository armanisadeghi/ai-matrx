...
export function usePlanDrift(
  ...
  /** The RESOLVED paired CMS site id (useCmsLink) — null = unpaired, which is
   * a normal no-overlay state, never an error. */
  cmsSiteId: string | null,
) {
  const dispatch = useAppDispatch();
  const queryClient = useQueryClient();
  const cmsPages = useCmsPageMap(siteId, cmsSiteId);
