 ...
 * level control (the top-level overview), and a live node count. All state
 * lives in PlanTree — this is chrome only. Pure logic: ../lib/tree-view.ts.
 */
...
export function PlanTreeToolbar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 border-b border-border px-1.5 py-1">
          ...
          <label className="mt-2 flex cursor-pointer items-center gap-1.5 rounded px-1 py-0.5 text-xs text-foreground hover:bg-accent/50">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                onFiltersChange({ ...filters, needsReviewer: checked === true })
              }
              className="h-3.5 w-3.5"
            />
            Needs reviewer only
      ...
      <span className="ml-auto whitespace-nowrap pl-1 text-[11px] tabular-nums text-muted-foreground">
        {queryActive
