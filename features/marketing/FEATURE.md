...
3. Manage a page: `PageWorkspace` reads identity from `web.page` and current content from `latest_snapshot_id`, rendering SERP + social previews, indexability, headings outline, content stats, sitemap memberships, and page screenshots; it saves only `target_keyword` plus desired metadata to the page. All snapshot JSON access goes through the typed narrowers in `lib/head-tags.ts` and `lib/snapshot-content.ts` — components never poke raw snapshot JSON.
