...
- **2026-09-17 — Claude (lane F-4): the pre-flight runs BEFORE the first Enable, and the refusal leaves the editor.** The verification of this feature (`common-docs/projects/google-native/VERIFY-U-P4-U-M1.md`, defect B-1) found every property guard gated on `enabled`, so at first setup — the case the pre-flight was built for — nothing judged the choice: the editor saved it and started the ~16-month backfill against it. Live proof: site `d7c4aeb1-…` (`ga4-oauth-qa-00fb6a62a3.invalid`) is bound to `http://bhrcenter.com/`. ONE judge now answers every write path (`judgeGscBindingWrite` + `shouldStartGscFirstImport` in `google/gsc-property.ts`): the draft is judged whether the switch is on or not, `persistBuiltInProvider` refuses by name, a refused binding never starts a backfill, and the auto-bind after a Google connection no longer saves an unmatched property. The refusal is now visible from the LIST as well as the editor — the GSC connection chip (`lib/site-status.ts`) says "Search Console property does not match this site — …" instead of "Connected", and `components/shared/GscBindingRefusalLine.tsx` prints it with a **Fix** door on the portfolio card beside the freshness line (PLAN §5.3). The per-provider button is disabled while its issue stands with the issue printed beside it. Three silent accepts in `preflightGscProperty` are closed: a port and a trailing-dot host are refused by name, and an uppercase `SC-DOMAIN:` ref is normalized to the lowercase ref Google actually accepts, saying so. Guards: `google/gsc-property.preflight.test.ts` (24 cases, 11 of them failing against the previous module), `components/integrations/integration-issues.test.ts`, `lib/site-status.test.ts`.

- **2026-09-17 — Claude: the property-type PRE-FLIGHT, and freshness on the header and the cards.** `google/gsc-property.ts` gained `preflightGscProperty` / `siteCanonicalUrl` (extending the file that already owns THE DOMAIN-PROPERTY RULE, never a second judge): Search Console treats `https://example.com/`, `https://www.example.com/`, `http://example.com/` and `sc-domain:example.com` as four separate properties, and binding the wrong one answers 200 with zero rows forever while every screen calls the connection healthy. A mismatch is now named with BOTH sides and the property to pick instead — a www/non-www swap, a scheme swap, a URL prefix whose path excludes the site, a foreign domain — while the site's own domain property stays `ok` and is never nudged toward a URL version, and a parent-domain property on one subdomain is an advisory. Enforced at both connect points: the site Integrations workspace refuses Save on a mismatch (configuration issue + an inline "Use sc-domain:…" fix) and this route's setup checklist (`setup/siteSetupChecklist.tsx`) fails its `search_console` step with the same sentence and a door — before this, a wrong-type binding passed that step and left every step below it reading "no data yet" forever. Guard: `features/marketing/google/gsc-property.preflight.test.ts`, 10 cases, proven failing-then-passing. **Freshness:** the dashboard header and every portfolio site card now render the ONE `DataFreshnessLine` ("data through Sep 14 · pulled 40 minutes ago · Google runs about three days behind") in place of the bare "data through" fragment; the card's verdict line says how old the DATA is, the freshness line says when we last asked Google, which is the difference between "we are behind" and "Google is behind". The stale threshold is the knob `google.marketing.freshness_warning_hours` (default 72), seeded in `migrations/google_marketing_knobs.sql` and applied 2026-09-17 06:37:24Z — value 72, read back from `platform.feature_knob` that day (this entry's original "NOT YET APPLIED" wording was corrected on 2026-09-17).
