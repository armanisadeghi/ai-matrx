...
## 🚨 Live AI runs — THE FLOATING LAW

> **A user must never watch a spinner while AI works. The run streams, and it streams in `LiveRunWindow`.**
...
**A run that dies on refresh is the same defect as a spinner.** A surface that IS the display for its own run — purpose-built, more specialized than the generic window, and structurally unable to shift content (the Masterwork **Final Checkup** panel, whose whole body is its finding stream with per-finding verbs) — declares `live.surfaceOwnsDisplay` on `useDurableRun`: the stream is still ADOPTED and still rendered by the ONE canonical pipeline, the generic window simply does not also open beside it. The flag decides WHERE the canonical renderer mounts; it never licenses a bespoke one. If the work is server-side, the surface reattaches on load; the durable record is whatever the run's own feature persisted — never this window (it is `ephemeral: true`).

**A surviving viewer retains its request.** EVERY canonical viewer — `StreamAwareChatMarkdown` (the seam under every `MarkdownStream`) and `LiveRunDisplay` — registers a viewer id in `activeRequests` via `useRetainRequestForViewer`; `removeRequest` and `destroyInstance` defer deletion until the final viewer releases it, and `createRequest` never resets an existing row. A route/query remount may clean up its run owner, but it can never blank a still-open surface. **Before touching any reap, adoption, or live-run render path, read [`/Users/armanisadeghi/code/common-docs/systems/agents/execution-runtime/LIVE-RUN-RETENTION.md`]/Users/armanisadeghi/code/common-docs/systems/agents/execution-runtime/LIVE-RUN-RETENTION.md)** — the disappearing-run class doctrine; guard test `request-viewer-retention.test.ts`.
...
- 2026-09-21 — **THE LIGHTWEIGHT WINDOW is a PRESENTATION of the panel host we already had, not a sixth floating family.** Arman, 2026-09-18: *"a slightly more lightweight version of the window panel … doesn't minimize and stuff like that but carries the basics of dragging"*, as the default host for a picker or a form (`common-docs/policies/every-picker-takes-new-input.md` §4). **The reuse decision** (finding I7 of that campaign's plan attack): `components/matrx/resizable/MatrxDynamicPanelHost.tsx` — 59 files — is EXTENDED with `presentation: "docked" | "floating"`, not replaced and not twinned. `"docked"` is every existing call site, byte for byte; `"floating"` routes to the new `components/matrx/resizable/MatrxFloatingFrame.tsx`. One component, one `open`/`onOpenChange` contract, one Escape and focus contract, one portal — the choice is a prop, never a second component. `WindowPanel` stays what it is: the durable workbench with traffic lights, minimize, the tray, persistence, pop-out and URL sync. **What the lite frame carries:** drag by header (arrow keys too — drag is never mouse-only), edge/corner resize, Escape when TOPMOST and no Radix popper is holding the key, focus in on open and back to the opener on close, a nested `PortalContainerProvider` so a `CreatablePicker` popover mounts inside the window instead of behind it, and z-order shared with `WindowPanel` through the SAME `nextZIndex` counter (`windowManagerSlice.transientZ` — a separate map, so every tray/minimize/arrange selector excludes lite windows BY CONSTRUCTION and a half-typed create form can never be stowed). **What it never gets:** minimize, the tray, persistence, pop-out, URL sync, sidebars, a focus trap, `aria-modal`, `inert`, a body scroll lock. **THE NEVER-LOSE-INPUT CONTRACT** (that campaign's finding C1, where crossing 768 with the agent-variable editor open destroyed the editor and everything typed into it): the floating frame and the drawer are the SAME DOM node and the same React subtree — crossing the breakpoint changes `className` and `style` on one `<div>`, and `children` is rendered in exactly one place. An `isNarrow ? <Drawer> : <Floating>` ternary is the defect, and the file says so. The rect the person chose is also kept separately from the rect that is drawn, so a window squeezed to 359px at a phone width comes back at its own 780px when there is room again (measured, and fixed after the first live pass showed it stuck narrow). **Verified live** as `admin@admin.com` at `/administration/ui/lite-window` (examples in `features/window-panels/lite-window/`, also served at `/demos/lite-window`): the real Add-a-scope-type form created a real scope type from a floating window and toasted it; drag moved it 250,92 → 58,212; 1280 → 375 → 1280 kept every character and restored the rect; Escape closed it and focus returned to the opener (the opener is captured in the FIRST render, because a form whose first field carries `autoFocus` has already taken focus by the time an effect runs); the page behind stayed scrollable with no scroll lock, no `inert`, `aria-modal="false"`; and a lite window opened from inside a heavy `WindowPanel` sat at z 1002 over its 1001, with a click on the workbench raising it to 1004 — one z-order, not two.
...
- 2026-08-27 — **Listen panel gains an in-place voice settings pane.** A tiny `Settings2` toggle in the window header swaps the BODY (transport footer stays live) for a settings pane composed from the canonical settings-system primitives, so every change is the app-wide default for all speech. Includes a "Preview voice" button (stops current audio first, then speaks a sample through the canonical queue — no takeover popup). `LiveRunDisplay` is CSS-hidden, never unmounted, so its viewer retention keeps holding live request rows while settings are open. **Superseded 2026-08-28** — the pane originally wrote `userPreferences.voice.*` via `useSetting`; it now reads/writes the tiered `listening` namespace through `useListeningSettings()` (system → org → user). Model + rationale: [`features/audio/service/listeningConfig.ts`](../audio/service/listeningConfig.ts).
...
- 2026-08-11 — **THE FLOATING LAW documented; `LiveRunWindow` resized to the
  ...
  tuned against. Now `760 × 80vh` (~722px usable) derived from named constants,
...
- `registerWindow` clamps the caller's `initial` rect via `clampRectToViewport` (defense in depth for any caller-computed rect), and the watchdog diagnoses/reveals with safe dims so a degenerate measurement can't false-scream `off-screen`.
...
- **2026-08-24** — **A degenerate viewport measurement no longer writes state.** Follow-up to the 2026-07-26 clamp guard, which stopped bad geometry from being _stored_ but still let every degenerate measurement drive a state write. `safeViewportDims()` now returns `degenerate` alongside `vw`/`vh`, and its scream is latched on a `globalThis` symbol (once per page, not once per module copy — duplicate chunk instances were why the log read as "repeating"). Writers stand down on a degenerate read: `WindowTraySync`'s debounced resize handler bails before dispatching (it also stopped reading raw `window.innerWidth`), as do `WindowPanel`'s off-screen rescue and `useWindowPanel`'s drag-release clamp. `clampWindowRect` / `clampAllWindowRects` / `revealWindow` now compare via `rectsEqual` and skip no-op assigns, and `recomputeTrayPositions` refuses a non-positive payload so a single hidden-tab resize can't poison `trayViewport` for every later minimize. Remaining raw reads are the user-click handlers in `SidebarWindowToggle` (a click can't land on a hidden tab). Verified in the managed preview on a real 0×0 tab: 50 synthetic resizes + repeated window opens produce zero `Maximum update depth exceeded` and exactly one degenerate log per page load.
...
- 2026-09-13 — Alchemy Portal owner: prepared-resource Tools entry waits for successful attachment and current identity; RunControlsWindow forwards initialTab to the canonical mobile sheet.
