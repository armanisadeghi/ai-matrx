...
- **2026-06-23** — **`tableViewerWindow` overlay added** — a full-size floating view for a markdown table. New singleton window `windows/TableViewerWindow.tsx` (composition root: body holds only the reused `StreamingTableRenderer` at a larger font; sized to ~85% of the viewport, clamped to 1400×900). Wired through the standard overlay path: `overlay-ids.ts`, `catalogue.ts`, `windowRegistryMetadata.ts`, opener `openers/tableViewerWindow.tsx`, and a gated controller block. Opened from the inline table's new "Open in window" action (`StreamingTableRenderer`, suppressed when already `expanded`).
