...
/**
 * Local-first window preservation coordinator — the ENGINE.
 *
 ...
 * this path.
 *
 * Shell/core split (build-graph): `WindowPersistenceManager.tsx` is the thin
 * always-mounted shell (context + hook + `next/dynamic` boundary). THIS file
 * carries the full persistence graph (registry metadata, serialization,
 * popout, local session store) and is loaded as its own client chunk — it
 * must never be statically imported by anything except the shell's
 * `dynamic()` call and tests. It renders nothing; it computes the context
 * value and publishes it up via `onValue`.
 */
...
import type { WindowPersistenceContextValue } from "./WindowPersistenceManager";

interface WindowPersistenceCoreProps {
  /** Publishes the live context value up to the shell provider. */
  onValue: (value: WindowPersistenceContextValue) => void;
}

export default function WindowPersistenceCore({
  onValue,
}: WindowPersistenceCoreProps) {
  ...
  // Publish the live context value to the shell whenever any part changes.
  // (React Compiler keeps the callbacks stable, so this fires on mount and
  // on `hydrated` flips — not every render.)
  useEffect(() => {
    onValue({ getSessionId, saveWindow, closeWindow, hydrated });
  }, [onValue, getSessionId, saveWindow, closeWindow, hydrated]);
