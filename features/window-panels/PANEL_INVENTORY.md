...
| **Persistence**                | exact `overlayId` + `instanceId`, `onCollectData`, `urlSync` (`?panels=`), default-deny local workspace cache, `ephemeral`, close tombstones, staged hydrate-on-load, instance GC                                                                                                              |
...
### Table A — Functionality, Coverage & Composition (_what it does vs should_)
...
### Table B — Utility, Surface & Construction (_is it using the platform's power + built canonically_)
...
### Table C — Availability & Placement (_reachable + actually used_)
