...
- 🚨 **GLOBAL MEANS NO TENANT OWNER — ON THE WIRE, ALWAYS.** Storage homes global rows in the SYSTEM organization (the column is NOT NULL), but every client read — `matchesScope`, `resolveRowScope`, `selectGlobalCategories`, `selectGlobalShortcuts`, the `CategoryTree` / `DuplicateCategoryModal` ownership badges, the `ShortcutEditorNext` / `BatchShortcutsEditor` category pickers, `format.ts` — defines global as `organizationId === null`. Anything that hands a row to the client applies `toGlobalOwnershipWire` / `toGlobalOwnershipRecord` (`lib/organizations/globalOwnership.ts`) first, and anything that writes a read record back applies `fromGlobalOwnershipRecord`. Skipping it is not cosmetic: it is what made 55 existing global shortcut categories render as ZERO, which in turn made the Create Shortcut modal's required category picker permanently empty.
