...
**Visibility and ownership are separate.** `agent.shortcut.organization_id` is required tenant ownership. Global visibility is the system org; `created_by` is audit and may be stamped with the acting admin even for a global row. Personal visibility is `created_by` outside the system org; project/task scope is a `platform.associations` edge. The frontend compatibility fields (`userId` / `organizationId` / `projectId` / `taskId`) must never be mistaken for four interchangeable ownership columns. See [`features/scopes/FEATURE.md`](../scopes/FEATURE.md).
...
- `2026-08-31` — **FIX-6: the write→read gap that made global categories invisible.** 55 global shortcut categories existed and PostgREST returned all 55 to the admin, yet the Categories tab, both global shortcut lists and the Create Shortcut picker showed zero — the routes queried the flipped storage (`organization_id = system org`) but echoed that org back on the wire, and the client's shared scope model counts any organization as "not global". Fixed once, at the seam: `lib/organizations/globalOwnership.ts`, applied by both API route families (list, get, create, patch, duplicate) and by the two thunks that write straight to the table and put the returned row in the store. `fromGlobalOwnershipRecord` puts the system org back for a write-back, since the column is NOT NULL. Guard: `lib/organizations/globalOwnership.test.ts` walks a real production row through adapter → converter → `matchesScope` and pins BOTH sides — the raw row is not global (the regression itself) and the wired row is.
