...
- **2026-08-30** — The canonical `/scheduler/*` client now applies the
  ...
  is selected. This repairs the `/schedules` duplicate-check failure exposed by
  the rule-2026-08-30.2 terminal workflow canary and protects every scheduling
  read/write sharing the client from the same identity-without-organization
  admission defect.

- **2026-08-30** — Repaired 178 historical `scheduler.sch_run` rows across
  three schedules whose outer status said Success while their canonical agent
  completion said failed/error/cancelled. The repair preserves each original
