...
import CONTRACT_PAYLOADS from "./__fixtures__/contract-export-payloads.json";
...
describe("cross-repo contract: server-generated payloads (BOARD.md B9, 2026-09-19/20)", () => {
  /**
   * 🚨 THE LIVE DEFECT. Dropping the real `My Clippings.txt` fixture on
   * `/exports` produced, on production: "The server sent this screen a shape
   * it cannot read: "library.metrics.top_containers" should be a list and
   * arrived as nothing at all (the field was missing)." (VERIFICATION.md
   * §21.3). ROOT CAUSE: `library.metrics` is a JSONB blob written once at
   * index time; the round-9 container-facet rename
   * (`counts_by_container` -> `top_containers`, aidream 1366118f20 /
   * matrx-frontend 05aaf8ffd7) changed what NEW rows look like but left
   * every row indexed before that release in the OLD shape forever. This
   * screen's own parser was never wrong — it has expected `top_containers`
   * since 05aaf8ffd7 — the defect was an old row reaching a new reader with
   * nothing at the seam to migrate it.
   *
   * `contract-export-payloads.json` is NOT hand-written: it is generated by
   * aidream's own real pipeline (`run_index_pass` on the real Kindle
   * clippings adapter, `normalize_stored_metrics` for the migration) in
   * `aidream/services/bring_your_export/tests/test_contract_fixture_export.py`
   * (`-k write_fixture` regenerates it). Two repos, one fixture, so this
   * screen and the server it talks to cannot silently drift again.
   */
  const payloads = CONTRACT_PAYLOADS as Record<string, unknown>;

  it("parses a freshly indexed Kindle export with real book-title containers", () => {
    const library = parseExportLibrary(payloads.fresh_kindle_clippings);
    expect(library.summary).not.toBeNull();
    expect(library.summary!.total_items).toBeGreaterThan(0);
    expect(library.summary!.top_containers.length).toBeGreaterThan(0);
    // Every container is keyed by a real book title, never blank/null.
    for (const c of library.summary!.top_containers) {
      expect(c.key.length).toBeGreaterThan(0);
      expect(c.count).toBeGreaterThan(0);
    }
  });

  it("PROVEN FAILING: the raw, unmigrated legacy shape is exactly the live defect", () => {
    // This is the RAW blob a pre-round-9 Library actually has in the
    // database — before `normalize_stored_metrics` runs. It reproduces the
    // production sentence byte-for-byte in shape: `top_containers` absent.
    expect(() => parseExportLibrary(payloads.legacy_shape_raw_unmigrated)).toThrow(
      ExportContractError,
    );
    try {
      parseExportLibrary(payloads.legacy_shape_raw_unmigrated);
    } catch (err) {
      expect((err as ExportContractError).message).toContain("top_containers");
      expect((err as ExportContractError).message).toContain("should be a list");
    }
  });

  it("PROVEN PASSING: the same legacy row, after the seam's migration, parses cleanly", () => {
    // What `_library_row` now actually serves for that SAME old row — the fix.
    const library = parseExportLibrary(payloads.legacy_shape_migrated);
    expect(library.summary).not.toBeNull();
    expect(library.summary!.top_containers.length).toBeGreaterThan(0);
    expect(library.summary!.warnings).toEqual(
      expect.not.arrayContaining([expect.stringContaining("top_containers")]),
    );
  });

  it("still treats a never-indexed Library as un-indexed, not an error", () => {
    const library = parseExportLibrary(payloads.not_yet_indexed);
    expect(library.summary).toBeNull();
  });
});
