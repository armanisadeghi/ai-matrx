...
const AppletPreviewCard: React.FC<AppletPreviewCardProps> = ({
  applet,
  className,
  onClick,
}) => {
  ...
  const fieldCount =
    applet.containers?.reduce(
      (count, container) => count + (container.fields?.length || 0),
      0,
    ) || 0;
    ...
    <div className={cn("w-full", className)}>
      <Card
        className={`border-border overflow-hidden ${
          onClick
            ? "cursor-pointer hover:border-gray-300 dark:hover:border-gray-600 transition-all"
            : ""
        }`}
        onClick={onClick}
      >
        ...
        {applet.imageFileId || applet.imageUrl ? (
          ...
          <div
            className={`w-full h-28 flex items-center justify-center ${getPrimaryColorClass("bg")} ${getPrimaryColorClass("border")} border-b`}
          >
        ...
        <CardHeader className="p-3">
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-2">
              {/* Applet Icon */}
              <div
                className={`p-1.5 rounded-md ${getAccentColorClass("bg")} text-white`}
              >
                {renderAppletIcon()}
              </div>
        ...
        <CardContent className="p-3 pt-0">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div
              className={`p-1.5 rounded-md ${getPrimaryColorClass("bg")} ${getPrimaryColorClass("border")} border`}
            >
              <div className="flex items-center space-x-1.5">
                <LayersIcon
                  className={`h-3 w-3 ${getPrimaryColorClass("text")}`}
                />
        ...
        <CardFooter className="p-3 border-t border-border bg-gray-50 dark:bg-gray-800/50">
          <div className="flex items-center justify-between w-full">
            ...
            {applet.compiledRecipeId && (
              <Badge
                className={`${getAccentColorClass("bg")} text-white text-xs`}
              >
                Recipe Ready
              </Badge>
            )}
