...
>(
  (
    ...
    // Derived state
    const baseApplets = appletIds
      ? useAppSelector((state) => selectAppletsByIds(state, appletIds))
      : allApplets;
    ...
    const refreshRef = useRef<
      (specificAppletIds?: string[]) => Promise<CustomAppletConfig[]>
    >(null!);
    ...
    useImperativeHandle(ref, () => {
      ...
      const refreshFn = async (specificAppletIds?: string[]) => {
        ...
        try {
          ...
          let currentApplets = appletsToRefresh
            ? selectAppletsByIds(state, appletsToRefresh)
            : currentAllApplets;

          // Apply appId filter if provided
          if (appId) {
            currentApplets = currentApplets.filter(
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
            ...
            {showCreateButton && (
              <Button
                className="bg-emerald-500 hover:bg-emerald-600 text-white"
                size="sm"
                onClick={handleLocalCreateApplet}
              >
                ...
                return (
                  ...
                  >
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          {applet.containers &&
                            applet.containers.length > 0 && (
                              ...
                              >
                                {applet.containers.length} Container
                                {applet.containers.length !== 1 ? "s" : ""}
                              </Badge>
                            )}
                        </div>
                      </CardContent>
                    ...
                    `}
                      >
                        <div
                          className={`flex w-full gap-2 ${viewMode === "list" ? "flex-col" : ""}`}
                        >
                          {onSelectApplet && (
                            <Button
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    {applet.name}"? This action cannot be undone
                                    and will permanently remove the applet and
                                    all its data.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
