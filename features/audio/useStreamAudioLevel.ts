...
// (`@ai-matrx/browser-audio/core`) — the graph + math live there once.
//
// CONTRACT
// - Pass a live `MediaStream` (with at least one audio track) to meter it;
//   pass `null` to stop metering and reset the level to 0.
// - Uses the SHARED AudioContext (the package’s shared context) — never a
...
import { createStreamLevelMeter } from "@ai-matrx/browser-audio/core";
