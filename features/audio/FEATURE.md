...
- **App-root provider:** `providers/AudioDeviceProvider.tsx` (thin) + `AudioDeviceProviderImpl.tsx` (`next/dynamic ssr:false`, AudioOutputHost pattern) — mounted above `GlobalRecordingProvider` in `app/Providers.tsx`. Wires listeners once and applies the persisted mic/speaker EARLY (before the first recording) — re-resolving by label when the device list changes. Also installs the LAZY preferred-camera resolver (`setPreferredCameraResolver`) + `installCameraPermissionAcquirer()` on the camera stream manager — no boot-time camera acquisition, the resolver is only consulted at `acquireCameraLease` time.
