...
- **UI:** the `audioDevices` overlay (window on desktop, Drawer on mobile via the registry's `mobilePresentation`) = `AudioDevicesWindow` → `AudioDevicesPanel` (mic + speaker pickers, permission + Grant button, live "Test mic" meter, "Test speaker" tone). Opened from the avatar menu (`SETTINGS_ITEMS`). The reusable `components/audio/MicDeviceMenu.tsx` caret sits next to the mic in `ProInput` / `ProTextarea`.
