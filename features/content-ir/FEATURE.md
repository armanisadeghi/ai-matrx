...
Done: 0 extract+tests · 1 registry/session/parser upgrades · 2 accumulator shadow · 3 splitter parity · 4 flashcards render flip. Next: register the 8 `JSON_BLOCK_PATTERNS` kinds → persist envelopes on message parts + accept Python `engine:"py-block-detector"` envelopes (Phase 5) → XML fold-in via `discriminator.ts` (Phase 6) → delete the mirrored brace/fence state machines, parser becomes end oracle (Phase 7). Artifacts any-surface generalization + agent `__kind` injection tracked in the same plan.
