...
The FE accepts pre-built `CanonicalBlockIR` envelopes from aidream and never re-parses what they describe. This is the wire contract Python must satisfy. TS kernel: `@ai-matrx/content-ir` — `core/ir-types.ts` (shapes), `core/fingerprint.ts` (hash), `core/envelope-cache.ts` (part cache); authored in aidream `apps/shared/content-ir-core`.
