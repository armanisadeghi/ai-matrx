...
- ~~`canvas_items_user_content_full_unique (user_id, content_hash)` — two sources with byte-identical content collide on insert.~~ **RESOLVED 2026-07-05** (`migrations/canvas_items_materialized_null_content_hash.sql`): the hash-unique is now scoped to manual user saves by leaving `content_hash` NULL on every RPC-materialized/model row (NULLs are distinct in the full index, so the supabase-js `onConflict:"user_id,content_hash"` arbiter in `canvasItemsService.save` still works). Materialized artifacts dedup on `(source_system, source_id, artifact_index)`, never on content.
...
## Change Log
- 2026-07-05 — Fixed two live-chat bugs on unfenced JSON. (1) **Streaming recognition of truly newline-less minified JSON** (the structured-output shape — one line, no `\n` after the opening `{`): the accumulator's `processLine` only sees complete lines, so it never opened the region and the whole payload projected as raw text until `finalize`. `StreamBlockAccumulator` now opens the `bare_json` region from the trailing fragment the instant `{ "key":` streams in (`maybeOpenBareJsonFromFragment`) and HOLDS the nascent `{…` fragment instead of flashing it as raw text (`emitCurrentBlock`). New coverage in `accumulator-shadow.test.ts` (the old "minified" test cheated with `{\n`). (2) **Materialization 23505** — see Known risks (resolved).
