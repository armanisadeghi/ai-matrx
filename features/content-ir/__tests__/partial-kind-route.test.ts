...
// ---------------------------------------------------------------------------
// The bound-agent stream, replayed from REAL producer bytes
// ---------------------------------------------------------------------------
//
// `bound_agent_unfenced_kind_first` is what every `ai.agent.produce` step emits:
// a bare JSON document, no fence, no XML tag, `__kind` first. These are the
// events the live Python producer really sent for it (regenerated by aidream
// scripts/generate_partial_kind_fixture.py and drift-gated there), so this
// asserts the shipped behaviour rather than a hand-built approximation.

describe("a kind-bound agent's unfenced answer", () => {
  ...
  it("fills card by card and never loses one", () => {
    const counts = rows
  ...
  it("a LATE __kind announces nothing — pre-recognition is first-key only", () => {
    expect(FIXTURES.bound_agent_unfenced_kind_last ?? []).toEqual([]);
  });
});
