 ...
 * `null` — EXCEPT a list, which coerces to `[]` rather than to null (an empty
 * list and an absent one render identically, and every bridge already does
 * this). A field the registry declares REQUIRED keeps its type — coercion
...
export type MaterializedKind<T> = {
  [K in keyof T]-?: undefined extends T[K]
    ? Exclude<T[K], undefined> extends readonly unknown[]
