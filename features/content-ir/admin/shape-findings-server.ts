...
export async function gatherFindingCodePayload(
  ...
  return {
    ...
    findings,
    duplicateSkillCases:
      code === "duplicate-skill" ? buildDuplicateSkillCases(db, findings) : [],
    warnings,
...
function buildDuplicateSkillCases(
  db: DoctorDb,
  findings: ShapeFinding[],
): DuplicateSkillCase[] {
  ...
  const contestedKinds = new Set(
    findings.map((f) => f.kind).filter((k): k is string => Boolean(k)),
  );

  const cases: DuplicateSkillCase[] = [];
  for (const kindSlug of [...contestedKinds].sort()) {
    const kindRow = db.kinds.find((k) => k.kind === kindSlug);
    if (!kindRow) continue;
    const parents = parentKindSlugs(kindSlug, kindIdBySlug, slugByKindId, db.edges);

    const bySyntax = new Map<SkillSyntax, string[]>();
    for (const t of teachings) {
      if (t.kind !== kindSlug) continue;
