...
The catalog and configuration surface for every LLM available to the product — model identity (id, maker, class, deprecated flag), capabilities, controls schema, and declarative runtime constraints. Pricing lives on `ai.offering` (not the model row). Agents (and legacy prompts/builtins) reference rows from this registry by ID; the Agent Builder uses it as a picker.
...
> 🔴 **READ SURFACE = THE VIEWS.** The 2026-07-10 reshape landed: the dissolved `ai.model_definition` columns (`provider`/`endpoints`/`pricing`/`capabilities_pre_canonical`) are **dropped**, `model_provider` was **renamed `provider_id`**, and **`ai.service` no longer exists** — it split into **`ai.endpoint`** (one row per serving vendor) + **`ai.api`** (one row per wire contract; `translator_key` = the old `wire_format` vocabulary). `api_class`/`model_class` still exist on the table but nothing in the app may read them. The clean read surfaces are the DB views **`ai.model_public`** (users; maker + `cost_rating`/`speed_rating`, points-not-dollars, NO vendor/wire-format columns) and **`ai.model_admin`** (admins; + endpoint/api/offering internals). The app attaches a resolved **`maker`** (`ai.provider.name`) to every model row in the fetch/service/SSR layers; **read `maker`, never `provider`.** Users must NEVER see serving vendors (Groq, Together, Cerebras, …) or wire formats — admin-only surfaces may.

**Database tables** (Supabase, `ai` schema — renamed/canonicalized 2026-07-02 "AI-catalog reshape", was flat `public.ai_*`)
- `ai.model_definition` (was `ai.model`) — master registry row. Live columns include `id`, `name`, `common_name`, `provider_id` (FK → `ai.provider`; was `model_provider`), `model_class`, `context_window`, `max_tokens`, `is_deprecated`, `is_primary`, `is_premium`, curated `cost_rating` + `speed_rating` (1-6 smallints; 6 renders as "5+"), retry plumbing `retry_fallback_id` + `retry_max_attempts`, two self-FK columns `mid_fallback_id` + `guest_fallback_id` (see Tier fallbacks below), and JSONB blobs `capabilities`, `controls`, `constraints`. (`api_class`/`model_class` still exist but are never read by the app.)
...
- `AiModel` — `AiModelRow` with `api_class` omitted, JSONB `controls`/`constraints`/`capabilities` narrowed, plus a resolved **`maker: string \| null`** (not a stored column — attached by the fetch/service layer from `ai.provider.name`). Read `maker` for all display/filter/group.
...
Phase D (2026-07-10) is DONE: the resolution layer (`ai.resolve_model_config` + `ai.model_config`) is live, the settings engine reads it (`fetchModelById`), admin rule editing targets `ai.api.rules` / `ai.offering.override` with a backend catalog reload on every save, ratings render in pickers, and `ai.model_alias` / `retry_fallback_id` / `retry_max_attempts` / ratings have admin UI. Remaining: Phase C drops `model_definition.controls`/`constraints`/`api_class`/`model_class` in the DB (nothing in this app reads them anymore).
