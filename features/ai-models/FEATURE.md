...
- `ai.model_alias` — alias registry: `alias` (unique), `model_id`, `kind` (`alias`|`deprecated`|`latest`), `notes`.
...
Configuration surface is stable on the 2026-07-10 reshape: `ai.endpoint` + `ai.api` (service split), `ai.offering` on `endpoint_id`/`api_id`/`override`, `provider_id` rename, stored `cost_rating`/`speed_rating`, `ai.model_alias`. Deferred to later phases: dropping `api_class`/`model_class` from `model_definition` (and then deleting `lab/modelDisplay.ts` entirely), repointing the settings modal's `model_definition.controls` read to a resolved view, and any UI for `ai.model_alias` / `retry_fallback_id`. No breaking changes planned to `ModelConstraint` shape.
