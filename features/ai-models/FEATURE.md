...
> 🔴 **READ SURFACE = THE VIEWS (2026-07-09).** Four `ai.model_definition` columns are **dissolved / dropping** — nothing in the app may read them: `provider` (free text → resolved **`maker`** via the `model_provider` FK), `endpoints` (→ `ai.offering` → `ai.service.wire_format`), `pricing` (→ `ai.offering.pricing`, edited per-offering), `api_class` (→ `ai.service.wire_format` + `vendor`). A fifth column, `capabilities_pre_canonical` (a one-release canonicalization revert snapshot), is dead and dropping too. The clean read surfaces are the DB views **`ai.model_public`** (users; resolves FKs to names, points-not-dollars, service masked) and **`ai.model_admin`** (admins; + real API/offering/service fields). The app attaches a resolved **`maker`** (`ai.provider.name`) to every model row in the fetch/service/SSR layers; **read `maker`, never `provider`.**

**Database tables** (Supabase, `ai` schema — renamed/canonicalized 2026-07-02 "AI-catalog reshape", was flat `public.ai_*`)
- `ai.model_definition` (was `ai.model`) — master registry row. Live columns include `id`, `name`, `common_name`, `model_provider` (FK → `ai.provider`), `model_class`, `context_window`, `max_tokens`, `is_deprecated`, `is_primary`, `is_premium`, two self-FK columns `mid_fallback_id` + `guest_fallback_id` (see Tier fallbacks below), and JSONB blobs `capabilities`, `controls`, `constraints`. (The `provider`/`endpoints`/`pricing`/`api_class`/`capabilities_pre_canonical` columns are dropping — see the read-surface note above.)
- `ai.model_public` (view) — user read surface: `id, name, common_name, model_class, capabilities, context_window, max_tokens, is_primary, is_premium, mid_fallback_id, guest_fallback_id, release_date, description, maker, service_name, usage_basis, token_billed, points_per_million_input/output`. Excludes deprecated/deleted; INNER-joins `provider`. Backs the picker options fetch (`fetchModelOptions`).
- `ai.model_admin` (view) — admin read surface: everything in `model_public` + `is_deprecated`, `offering_id`, `provider_model_id`, real `pricing`, `service_internal_name`, `service_display_name`, `vendor`, `wire_format`, `service_has_base_url`.
...
- `ai.offering` — `model_id × service_id` = the actual callable unit; **the sole home of pricing** (per-service tiers) + availability/capability overrides over `model_definition`. Managed at `/administration/ai-models/offerings` (`OfferingsContainer`, Manage tab; reuses `ModelPricingEditor` for the pricing tiers and `AdminAuditTable` for the dense grid). **Model-level pricing editing was removed** — the model detail Pricing tab is now read-only (`OfferingPricingReadOnly`), and the model-pricing audit category was deleted; there is exactly ONE way to edit pricing (here).
...
- `ai.model_definition.model_provider` (FK → `ai.provider.id`) is now the **sole** way to set a model's provider — `AiModelForm.tsx`'s free-text Provider input was removed; the model's brand is read as the resolved **`maker`** (`ai.provider.name`), never from the dropping plain-text `provider` column. A backfill migration (`migrations/ai_catalog_provider_backfill.sql`) fixed 33 previously-null and ~10 previously-mis-assigned `model_provider` rows and added the providers that existed only as free text (Black Forest, ByteDance, ElevenLabs, Ideogram, Kuaishou, Luma, MiniMax, Recraft, Runway, Together, WAN).
...
- `AiModel` — `AiModelRow` with the dropping columns (`provider`/`endpoints`/`pricing`/`api_class`/`capabilities_pre_canonical`) omitted, JSONB `controls`/`constraints`/`capabilities` narrowed, plus a resolved **`maker: string \| null`** (not a stored column — attached by the fetch/service layer from `ai.provider.name`). Read `maker` for all display/filter/group.
