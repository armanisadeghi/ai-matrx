...
- `ai.model_admin` (view) — admin read surface: everything in `model_public` + `is_deprecated`, `offering_id`, `provider_model_id`, `provider_id`, real `pricing`, `endpoint_id`, `endpoint_internal_name`, `endpoint_display_name`, `vendor`, `endpoint_has_base_url`, `api_id`, `api_name`, `translator_key`, `transport`, `retry_fallback_id`, `retry_max_attempts`, `cost_rating`, `speed_rating`.
...
- `ai.model_offering` (view) — user-facing offering rows with points-based pricing (`points_per_million_input/output/cached_input`). NO `service_name`/`service_slug` — the serving vendor is never exposed here. Surfaced read-only in the Offerings page's Coverage tab, alongside a computed gap list of models with zero offerings.
