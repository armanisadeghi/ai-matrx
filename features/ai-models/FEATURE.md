...
- **2026-09-12** — Current table-feedback correction evidence: Settings width commits `701836174e` and `ee14274985` keep 122 real keys readable at 1280px without wrapping or clipping (description 418px, key 272px, table 1210px). Provider/Endpoint/API row-tap adoption is `267abc0ab0`. Shared design-system fixes `51dbb6573` and `8d23dc540` provide the canonical host/toolbar action and one view selector with Save inside; malformed REST refresh handling has root cursor `1e52c9c4c`. Alchemy is `52b815e31` plus `5bdece4d4`, and dashboard supplies the canonical host in `423f7db79`. Independent source review accepted after fixing the fallback aria issue, SVG issue, and multiple-children issue. A final package-test assertion fix is in progress; registry installation and final browser QA remain pending.
