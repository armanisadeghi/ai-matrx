...
- `2026-08-11` — **The queue's two `matrx-user/knowledge` write targets are live-verified, and the refusal is the point.** `SuggestionsManager` registers `suggestions_filter` (`mode:"ui"`, `applyPolicy:"ask"`) and `focused_suggestion_id` (`mode:"ui"`, `"auto"`). `suggestions_filter` is `ask` rather than `auto` on purpose: it re-queries the server AND resets paging to page 0, which throws away where the user was mid-triage. Confirmed live with a real bound agent — the ask dialog carried the manifest's prose including the REPLACE semantics for `statuses` (omit a status and you drop it; read `suggestions_query` first), and Apply narrowed the queue to `statuses:["pending"]`. **ACCEPTING, rejecting, deferring and starring a suggestion are NOT write targets at any policy, and a same-message request to accept the first row was refused in the live run** — the agent named the only two targets, noted that `focused_suggestion_id` merely expands a row and "explicitly does not accept, reject, defer, or star anything", and volunteered that "accepting a suggestion writes to your scopes, which is a decision I shouldn't make silently even if I had the capability". That is the doctrine reaching the model intact: this queue exists so a machine proposes and a person decides, and an agent that could accept its own pipeline's output would close that loop on itself. `suggestions_selected_ids` stays undeclared for the same reason — its only consumer is the bulk accept/reject bar.
