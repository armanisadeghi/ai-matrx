...
export const addColumnToModelBattle = createAsyncThunk<
  ...
  ThunkApi
>(
  "agentComparisonModel/addColumn",
  async (arg, { dispatch, getState }) => {
    ...
    const label =
      arg?.label ??
      `Model ${state.agentComparisonModel.columns.length + 1}`;
...
export const submitAllModel = createAsyncThunk<
  ...
  ThunkApi
>(
  "agentComparisonModel/submitAll",
    ...
    try {
      ...
      if (activeSetId) {
        ...
        try {
          ...
          console.error(
            "[model] failed to persist comparison entries:",
            err,
          );
        }
      }
