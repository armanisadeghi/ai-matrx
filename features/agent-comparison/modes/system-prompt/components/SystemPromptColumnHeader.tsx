 ...
 *
 * The varied axis here is the column's per-column system prompt. The
 * editor itself is full-width inside the column body, so the header is
 * intentionally lean — drag, label, collapse, remove. A character count
 * surfaces so the user can scan-compare prompt sizes across columns
 * without scrolling each one.
 */
...
interface Props {
  column: SystemPromptColumn;
  onToggleCollapse: () => void;
}

export function SystemPromptColumnHeader({ column, onToggleCollapse }: Props) {
  ...
  // Tiny character-count summary so the user can scan prompt sizes
  // across all columns without opening each.
  const sysMessage = useAppSelector((s) =>
    selectAgentSystemMessage(s, column.syntheticAgentId),
  );
  const sysText = sysMessage
    ? ((sysMessage.content as Array<{ type?: string; text?: string }>)
        .find((b) => b?.type === "text")?.text ?? "")
    : "";
  const charCount = sysText.length;

  const commitLabel = () => {
    const trimmed = labelDraft.trim();
    if (trimmed && trimmed !== column.label) {
      dispatch(
        renameSystemPromptColumn({ columnId: column.columnId, label: trimmed }),
      );
    } else {
  ...
  const handleRemove = () => {
    ...
    dispatch(removeColumnFromSystemPromptBattle({ columnId: column.columnId }));
  };
  ...
  return (
    ...
    >
      ...
      <span
        ...
        title={`${charCount} characters in this column's system prompt`}
      >
