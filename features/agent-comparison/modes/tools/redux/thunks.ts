...
function applyToolBundle(
  ...
  dispatch(setAgentMcpServers({ id: agentId, mcpServers: bundle.mcpServers }));
}
...
>(
  ...
  async ({ agentId }, { dispatch, getState }) => {
    ...
    const inputConversationId = await replaceBattleInputDraft({
      ...
      agentId,
      agentVersionId: null,
      previousConversationId: state.agentComparisonTools.inputConversationId,
      copyVariables: false,
    });
    dispatch(setToolsInputConversationId(inputConversationId));
...
>(
  ...
  async ({ version, versionId }, { dispatch, getState }) => {
    ...
    const inputConversationId = await replaceBattleInputDraft({
      ...
      agentVersionId: version === "current" ? null : (versionId ?? null),
      previousConversationId: post.agentComparisonTools.inputConversationId,
    });
    dispatch(setToolsInputConversationId(inputConversationId));
...
export const addColumnToToolsBattle = createAsyncThunk<
  ...
  ThunkApi
>("agentComparisonTools/addColumn", async (arg, { dispatch, getState }) => {
  const state = getState();
  ...
  const conversationId = generateConversationId();
  const label =
    arg?.label ?? `Variant ${state.agentComparisonTools.columns.length + 1}`;
...
>("agentComparisonTools/submitAll", async (_arg, { dispatch, getState }) => {
  ...
  try {
    const state = getState();
    const { sourceAgentId } = state.agentComparisonTools.locked;
    const inputConversationId = state.agentComparisonTools.inputConversationId;
...
export const clearToolsBattle = createAsyncThunk<void, void, ThunkApi>(
  ...
  async (_arg, { dispatch, getState }) => {
    const state = getState();
    if (state.agentComparisonTools.inputConversationId) {
      dispatch(destroyInstance(state.agentComparisonTools.inputConversationId));
    }
...
export const saveToolsBattleAs = createAsyncThunk<
  ...
  { name: string },
  ThunkApi
>("agentComparisonTools/saveAs", async ({ name }, { dispatch, getState }) => {
  const state = getState();
...
export const loadToolsBattleSet = createAsyncThunk<
  ...
  { setId: string },
  ThunkApi
>("agentComparisonTools/loadSet", async ({ setId }, { dispatch, getState }) => {
  const before = getState();
  if (before.agentComparisonTools.inputConversationId) {
    dispatch(destroyInstance(before.agentComparisonTools.inputConversationId));
  }
  ...
  for (const entry of entries) {
    ...
    const entryMeta = (entry.metadata ?? {}) as
      Partial<PersistedToolsEntryMeta> | undefined;
