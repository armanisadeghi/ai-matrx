...
/**
 * ToolsToolbar — Mode-4 toolbar.
 *
 * Mirrors System-Prompt's structure since the locked-axis flow is
...
export function ToolsToolbar({
  ...
  const handleSave = async () => {
    ...
    try {
      await dispatch(saveToolsBattle()).unwrap();
  ...
  const handleSaveAsConfirm = async (name: string) => {
    setSaveAsBusy(true);
    try {
      await dispatch(saveToolsBattleAs({ name })).unwrap();
      setSaveAsOpen(false);
  ...
  const handleResetConversations = async () => {
    ...
    try {
      await dispatch(
        resetAllToolsConversations({ preserveInputs: false }),
      ).unwrap();
      toast.success("Variants reset (tools reset to source baseline)");
    } catch (err) {
      toast.error(
  ...
  const handleClearResponsesKeepInputs = async () => {
    ...
    try {
      await dispatch(
        resetAllToolsConversations({ preserveInputs: true }),
      ).unwrap();
      toast.success(
        "Responses cleared · per-column tools + locked input preserved",
      );
    } catch (err) {
      toast.error(
        `Couldn't clear: ${err instanceof Error ? err.message : err}`,
  ...
  return (
    <>
          ...
          <DropdownMenuContent align="end" className="w-80">
            <DropdownMenuItem onClick={() => setResetKeepInputsConfirm(true)}>
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-muted-foreground">
                  Wipe responses; keep per-column tools + locked input.
                </span>
              </div>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => setResetConfirm(true)}>
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-muted-foreground">
                  Drop per-column tool edits + responses. Keep source agent.
                </span>
              </div>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
      ...
      <TextInputDialog
        ...
        title="Save tools comparison"
        description="Give this comparison a name. The source agent + variables + user message are saved as part of the set; per-column tool lists are saved per entry."
        placeholder="My tools comparison"
        confirmLabel="Save"
        busy={saveAsBusy}
      ...
      <ComparisonSetLoaderDialog
        ...
        modeFilter="tools"
        loadFn={async (setId) => {
          await dispatch(loadToolsBattleSet({ setId })).unwrap();
        }}
      />
