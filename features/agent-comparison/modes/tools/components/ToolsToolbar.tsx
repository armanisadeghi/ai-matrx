...
export function ToolsToolbar({
  ...
  const handleSave = async () => {
    try {
      const saved = await dispatch(persistToolsBattle()).unwrap();
      recordToast.success(
        {
  ...
  const handleRenameConfirm = async (name: string) => {
    setRenameBusy(true);
    try {
      await dispatch(renameToolsBattle({ name })).unwrap();
      setRenameOpen(false);
    } catch (err) {
      toast.error(
      ...
      ? [
          {
            ...
            label: "Add variant",
            onPress: () => {
              void dispatch(addColumnToToolsBattle(undefined));
            },
          },
        ]
      : []),
  ...
  return (
    <>
      ...
      <TextInputDialog
        ...
        description="The copy keeps the same source agent, shared request and per-column tool lists, and opens as its own battle."
      ...
      <ComparisonSetLoaderDialog
        ...
        onDeleted={(id) => {
          if (id === activeSetId) dispatch(setActiveToolsSet(null));
        }}
      />
