...
/**
 * LockedInputSection — Tuning mode.
 *
 * What's locked across every column:
 *   - Source agent + version (forking baseline for each column's clone)
 *   - Canonical shared Smart Agent Input
 *
 * What VARIES per column: the agent's model + full settings (tuned in
 * each column's body via the existing Builder AgentSettingsModal,
 * pointed at the column's synthetic agent record).
 */
...
import {
  ...
  selectSourceAgentId,
  selectTuningInputConversationId,
...
export function LockedInputSection() {
  ...
  const inputConversationId = useAppSelector(selectTuningInputConversationId);
      ...
      {!collapsed && (
        <div className="p-3 space-y-3 max-w-4xl">
          ...
          <SharedBattleInput
            ...
            surfaceKey="agent-comparison-tuning-input"
          />
        </div>
      )}
    </div>
