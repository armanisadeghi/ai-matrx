...
export function KgGraphCanvas({
  ...
  // Surface scope (matrx-user/knowledge) — the graph half of the Knowledge
  // surface. Built at TRIGGER time from live state, never on mount. The
  // extraction and suggestion halves live on their own routes and emit
  // disjoint values.
  const getSurfaceScope = useCallback(
    () =>
      createKnowledgeScope({
        graph_organization_id: orgFilter ?? undefined,
        graph_scope_id: effectiveScopeId ?? undefined,
        graph_scope_type_id: initialScopeTypeId ?? undefined,
        graph_node_count: nodeCount,
        graph_edge_count: edgeCount,
        graph_entity_kinds: kinds,
        graph_truncated: payload?.truncated ?? undefined,
        graph_payload: payload
          ? (payload as unknown as Record<string, unknown>)
          : undefined,
        graph_selected_entity: selected
          ? (selected as unknown as Record<string, unknown>)
          : undefined,
        graph_kind_filter: kindFilter,
        graph_search: search || undefined,
        graph_detail_level: detail,
        graph_layout: layoutId,
        graph_view_state: {
          detail,
          layout: layoutId,
          color_by: colorBy,
      ...
      <div className="flex h-full w-full flex-col">
        ...
        <div className="flex items-center gap-2 overflow-x-auto border-b border-border bg-card px-3 py-2">
          ...
          {status === "ready" ? (
            <span className="shrink-0 whitespace-nowrap text-xs text-muted-foreground">
              ...
              {payload?.truncated ? (
                ...
                >
                  <AlertTriangle className="h-3 w-3" /> top {nodeCount} — raise
                  Detail for more
                </span>
              ) : null}
            </span>
          ...
          <div className="ml-auto flex flex-none items-center gap-2">
            ...
            <Select
              value={detail}
              onValueChange={(v) => setDetail(v as KgDetailId)}
            >
                  ...
                  {payload && payload.nodes.length > 0 ? (
                    ...
                    <>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        ...
                        {payload.nodes.length === 1 ? "" : "s"} are hidden by
