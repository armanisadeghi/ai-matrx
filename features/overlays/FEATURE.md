...
**A save callback is a settlement boundary.** `fullScreenEditor` and `htmlPreview` register exactly one promise-returning save owner (`onSave` xor `onAction`). The bridge awaits it, retains the group and entered buffer on rejection, and closes/removes only after acknowledgement. Observers are diagnostic-only after success. While pending, `FullScreenOverlay` blocks Save, primary actions, Cancel, X, Escape, backdrop, tabs, and editor interaction; retry uses the retained command. An opener unmount must never dispose a still-visible editor target — the bridge owns terminal cancel/success disposal.
...
- **2026-09-12** — **Fullscreen editor saves settle before close.** `callbackManager.triggerGroupCommand` now awaits one durable `fullScreenEditor` command and retains it on rejection; `FullScreenMarkdownEditorBridge` and `HtmlPreviewBridge` are the terminal close/disposal owners. The markdown and HTML editors share `FullScreenOverlay`'s pending/error/retry boundary, while rich-document save adapters return actual promises and rethrow failures. Covered by callback settlement and rendered editor/bridge tests.
