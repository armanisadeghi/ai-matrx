...
- **2026-07-13** — **New `userTableWindow` overlay — a saved UDT dataset table (by id) at full size in a floating WindowPanel.** Sibling of `tableViewerWindow` (that hosts a markdown table string; this hosts a live `udt_datasets` table via `UserTableViewer`). Component `features/window-panels/windows/UserTableWindow.tsx`; opener `useOpenUserTableWindow({ tableId, title? })`; singleton, `isWindow:true`. Registered across all sites (overlay-ids, catalogue, controller import + `isOpenById` + `dataById` + gated block, opener). Opened from the converted chat-artifact table's new "Window" toolbar action (`features/canvas/artifact-types/renderers/TableArtifact.tsx`), which also gained a "New tab" → `/data/{id}` link.
