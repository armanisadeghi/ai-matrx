...
├── surfaces/
│   └── SidePanelSurface.tsx    # Flexible side-panel chrome for bare content overlays: NON-blocking + drag-to-resize on desktop (width persisted to localStorage), bottom Drawer on mobile. Wrap a gated block in it; pass title + onClose (+ optional defaultWidth/min/max/storageKey).
└── openers/
