 ...
 * catalogue.ts
 *
 * THE single list of overlay ids, plus metadata for each. Its keys define
 * the `OverlayId` union and the runtime `isOverlayId` guard below — there is
 * no second id list. Render-free plain data (no imports): safe for the
 * every-page Redux middleware that validates close actions. The
 * OverlayController does NOT iterate this catalogue; its render path is
 * fully explicit.
 */
...
/**
 * Compile-time union of every overlay id — the catalogue's own keys. Every
 * `dispatch(openOverlay({ overlayId }))` site is narrowed by it: typo a key
 * and type-check fails. Adding an overlay = adding its entry above.
 */
export type OverlayId = keyof typeof OVERLAY_CATALOGUE;

/**
 * Runtime guard for ids arriving from outside the type system (URL params,
 * persisted sessions, extension postMessage payloads). Own keys only, so
 * prototype names like "toString" never pass.
 */
export function isOverlayId(value: unknown): value is OverlayId {
  return (
    typeof value === "string" &&
    Object.prototype.hasOwnProperty.call(OVERLAY_CATALOGUE, value)
  );
}
