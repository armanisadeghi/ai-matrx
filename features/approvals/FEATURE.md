...
1. **The `approval.` producer-policy row did not land, so a client-side proposal is still refused.** Verified in the database on 2026-09-17: `platform.assists.auto_apply_at` EXISTS, `assists_auto_apply_due_idx` EXISTS, and all four `hitl.google` knobs resolve with their intended defaults — but `select count(*) from platform.assist_producer_policy where source_pattern = 'approval.'` returns **0**. `platform.assist_admission_decision` refuses an unregistered source outright, so until §3 of `migrations/platform_approval_queue.sql` is applied, `proposeApproval` returns null for every client-side producer (loudly, and its contract is "do NOT perform the change"). Reading and deciding are unaffected; server-side producers writing as service role are unaffected. `pnpm db-types` must also be regenerated before `auto_apply_at` is a generated column — until then `readAutoApplyAt` reads it off the row without it.
