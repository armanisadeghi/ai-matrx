...
- **2026-06-21** — `page_extraction_jobs.output_schema` flipped `jsonb` → `json` (migration `output_schema_preserve_key_order_remaining.sql`, alongside the agent/prompt/tool `output_schema` columns). Part of the repo-wide fix for Postgres `jsonb` canonicalizing object keys by (length, then bytewise), which silently reorders authored JSON-Schema `properties`. **In this feature the flip is defensive/consistency only:** the durable shape stored here is the array-based `{ kind:"extraction_columns", columns:[…] }`, whose order is carried by the `columns` ARRAY (already preserved by `jsonb`), plus the explicit `column_order` override. The grid's *raw-row* column inference (`inferColumnsFromRows`) still reads `page_extraction_results.payload`, which remains `jsonb` deliberately (high-volume data table; keep `jsonb` operators/indexing available) — define template `columns` or set `column_order` if a specific raw-row order matters. No app/type change (`json`/`jsonb` both map to `Json`).
- **2026-06-21** — Persisted column reordering in the dataset grid (`/knowledge/extractions/[id]`). New nullable-free `page_extraction_jobs.column_order jsonb DEFAULT '[]'` (migration `page_extraction_jobs_column_order.sql`) stores an ordered array of `ExtractionColumn.key`s per dataset. `ExtractionDatasetClient` derives an `orderedColumns` list — keys in `column_order` lead in that order, any unlisted column (e.g. a newly extracted field) keeps its natural position behind them, empty array = natural order (fully back-compatible). Each header is now a `@dnd-kit/sortable` `SortableHeaderCell` with a grip handle (drag = reorder; the label button still toggles sort, since a pure click never trips the 5px drag activation). Reorders persist via `updateJob({ column_order })` with optimistic local update + revert-on-error. Export (`ExportMenu` / `SendToMenu`) follows the visible order for free. `PageExtractionJob.column_order: string[]` added to [types.ts](types.ts) (optional on insert — DB-defaulted).
- **2026-06-14** — Delete-run discoverability. The per-run delete shipped earlier only lived on the inline `RunProgressBar`, which is hidden in the default **All** view and only appears once a run is hydrated into state — users couldn't find it. Two fixes: **(1)** Added a labeled **Delete run data** control to [TemplateReadOnlyView.tsx](components/TemplateReadOnlyView.tsx) (the Chunked Runs tab), shown in a footer "danger zone" whenever the selected template has `latest_run_id`. It's always visible when a template is selected, so it's discoverable where templates are managed. Wired in [ChunkingConfigForm.tsx](components/ChunkingConfigForm.tsx) via the canonical `clearJobResults` RPC (wipes every run for the template, keeps the template), then dispatches `clearRun` + `invalidateResults` and refetches the job. **(2)** The `RunProgressBar` trash button is now a labeled **Delete run** button (icon + text) instead of a bare icon.
