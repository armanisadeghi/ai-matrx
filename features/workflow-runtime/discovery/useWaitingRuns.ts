...
export function useWaitingRuns(): WaitingRunsState {
  ...
  /**
   * THE HYDRATION RACE (same one `useResultSchema` documents): every backend
   * transport calls `requireSelectedOrgId()`, which throws until
  ...
  useEffect(() => {
    ...
    void (async () => {
      const result = await dispatch(
        callApi({ path: "/runs/waiting", method: "GET" }),
      );
      if (!live) return;
      if (result.error) {
  ...
  /**
   * Coalesced refetch. A single answered interrupt can produce several
