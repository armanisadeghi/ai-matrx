...
/**
 * CloudBrowserWindow — the Cloud Browser WindowPanel.
 *
 * Default face is WRITTEN PROGRESS (D-8). "Show me what's happening" opens the
 * bounded screenshot session. The takeover canvas appears ONLY while a person is
 * driving. The controller banner always names who is in control. Sharing uses
 * the canonical access-service dialog; notifications consume shipped channels;
 * telemetry shows live numbers and labels the unmeasured.
 */
...
export function CloudBrowserWindow({ onClose, initialProfileId }: CloudBrowserWindowProps) {
  ...
  const title = useMemo(
    () => `Cloud Browser${cb.activeProfile ? ` — ${cb.activeProfile.displayName}` : ""}`,
    [cb.activeProfile],
  );
