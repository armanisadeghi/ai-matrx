...
- `features/cms/services/cmsService.ts` — `CmsSiteService` / `CmsPageService` / `CmsVersionService` / `CmsComponentService` / `CmsAssetService` / `CmsCollectionService`, all POST `{action}` dispatch against `/api/cms/*` (`CmsAssetService.deleteAsset` throws `AssetInUseError` carrying the live usage detail on a 409)
...
- **Content exceptions are NOT a CMS route.** They are reviewed in THE ONE platform approval queue (`features/approvals/`, kind `cms_content_exception`) and decided by aidream's door `POST /cms/exceptions/{id}/approve|reject` (`features/approvals/cms-door.ts`), which replays `set_exception_status`. `/api/cms/approvals` and `CmsApprovalsService` are DELETED (register Q-1, 2026-09-19)
...
Components tab, the Collections tab, and PageEditor's main toolbar, SEO-tab header, and publish
...
- `client_content_exceptions` — the F3 exception store (aidream CMS migration `0011`; shape: `matrx_content_guard.models.ContentException` + `Violation`, `packages/matrx-content-guard/matrx_content_guard/models.py`). It STAYS in the CMS database — an approved row is the standing policy eight CMS write paths read. This repo neither reads nor writes it any more: `aidream/services/cms/exception_proposals.py` mirrors each pending row as a proposal on `platform.assists`, and the decision flips it server-side.
...
- `2026-08-13` — **AI-everywhere: every CMS editing surface got its in-place agent
  ...
  three roles where their work happens: `page_editor` in the main toolbar, `seo_editor` in the SEO
  tab, and `publish_reviewer` beside Publish. See § Agent surfaces.
