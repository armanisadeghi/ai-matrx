...
export const CmsSiteService = {
  ...
  async updateSite(
    ...
    const res = await callApi<{ site: ClientSite }>("sites", "update", {
      siteId,
      ...updates,
    });
    return res.site;
  ...
  async setResearchLineage(
    siteId: string,
    researchTopicIds: string[],
    researchTagIds: string[],
  ): Promise<ClientSite> {
    const res = await callApi<{ site: ClientSite }>(
      "sites",
      "set-research-lineage",
      {
        siteId,
        researchTopicIds,
        researchTagIds,
  ...
  async verifyDomain(siteId: string): Promise<DomainVerificationResult> {
    return callApi<DomainVerificationResult>("sites", "verify_domain", {
      siteId,
    });
  },
  ...
  async adminListSites(): Promise<ClientSiteSummary[]> {
    const res = await callApi<{ sites: ClientSiteSummary[] }>(
      "sites",
      "admin_list_sites",
    );
    return res.sites;
  ...
  async adminListActivity(
    ...
    const res = await callApi<{ activity: ClientActivityLog[] }>(
      "sites",
      "admin_list_activity",
      params,
    );
...
export const CmsPageService = {
  ...
  async updatePage(
    ...
    const res = await callApi<{ page: ClientPage }>("pages", "update", {
      pageId,
      ...updates,
    });
    return res.page;
  ...
  async saveDraft(
    ...
    const res = await callApi<{ page: ClientPage }>("pages", "save-draft", {
      pageId,
      ...draft,
    });
    return res.page;
   ...
   */
  async setWebPageLink(
    pageId: string,
    webPageId: string | null,
  ): Promise<ClientPage> {
    const res = await callApi<{ page: ClientPage }>(
      "pages",
      "set-web-page-link",
      { pageId, webPageId },
    );
  ...
  async publishDraft(pageId: string): Promise<ClientPage> {
    const res = await callApi<{ page: ClientPage }>("pages", "publish", {
      pageId,
    });
    return res.page;
  },
  ...
  async rollbackToVersion(
    ...
    const res = await callApi<{ page: ClientPage }>("pages", "rollback", {
      pageId,
      versionNumber,
    });
    return res.page;
  ...
  async adminListPages(
    siteId?: string,
  ): Promise<(ClientPageSummary & { client_id: string })[]> {
    const res = await callApi<{
      pages: (ClientPageSummary & { client_id: string })[];
    }>("pages", "admin_list", { siteId });
    return res.pages;
  },
};
...
export const CmsVersionService = {
  ...
  async listVersions(
    rowId: string,
    entityType: CmsEntityType = "client_page",
  ): Promise<ClientEntityVersion[]> {
    const res = await callApi<{ versions: ClientEntityVersion[] }>(
      "versions",
      "list",
      { rowId, entityType },
    );
  ...
  async getVersion(
    versionId: string,
    entityType: CmsEntityType = "client_page",
  ): Promise<ClientEntityVersionDetail> {
    const res = await callApi<{ version: ClientEntityVersionDetail }>(
      "versions",
      "get",
      { versionId, entityType },
    );
    return res.version;
  },
};
...
export const CmsComponentService = {
  async listComponents(siteId?: string): Promise<ClientComponent[]> {
    const res = await callApi<{ components: ClientComponent[] }>(
      "components",
      "list",
      { siteId },
    );
  ...
  async getComponent(componentId: string): Promise<ClientComponent> {
    const res = await callApi<{ component: ClientComponent }>(
      "components",
      "get",
      { componentId },
    );
...
export const CmsAssetService = {
  async listAssets(
    ...
    const res = await callApi<{ assets: ClientAsset[] }>("assets", "list", {
      siteId,
      ...params,
    });
    return res.assets;
  ...
  async adminListAssets(siteId?: string): Promise<ClientAsset[]> {
    const res = await callApi<{ assets: ClientAsset[] }>(
      "assets",
      "admin_list",
      siteId ? { siteId } : {},
    );
  ...
  async getAsset(assetId: string): Promise<ClientAsset> {
    const res = await callApi<{ asset: ClientAsset }>("assets", "get", {
      assetId,
    });
    return res.asset;
  },
  ...
  async createAsset(params: {
    ...
    const res = await callApi<{ asset: ClientAsset }>(
      "assets",
      "create",
      params,
    );
  ...
  async updateAsset(
    ...
    updates: Partial<
      Pick<
        ClientAsset,
        "file_name" | "alt_text" | "folder" | "tags" | "is_active"
      >
    >,
  ): Promise<ClientAsset> {
    const res = await callApi<{ asset: ClientAsset }>("assets", "update", {
      assetId,
      updates,
    });
    return res.asset;
  ...
  async assetUsage(assetId: string): Promise<AssetUsage> {
    const res = await callApi<{ usage: AssetUsage }>("assets", "usage", {
      assetId,
    });
    return res.usage;
  },
...
export const CmsCollectionService = {
  ...
  async listCollections(siteId: string): Promise<SiteCollectionSummary[]> {
    const res = await callApi<{ collections: SiteCollectionSummary[] }>(
      "collections",
      "list",
      { siteId },
    );
  ...
  async getCollection(collectionId: string): Promise<SiteCollection> {
    const res = await callApi<{ collection: SiteCollection }>(
      "collections",
      "get",
      { collectionId },
    );
  ...
  async updateCollection(
    collectionId: string,
    updates: CollectionUpsertParams,
  ...
  async rotateDataKey(siteId: string): Promise<string> {
    const res = await callApi<{ dataApiKey: string }>(
      "collections",
      "rotate_key",
      { siteId },
    );
  ...
  async getItem(itemId: string): Promise<SiteCollectionItem> {
    const res = await callApi<{ item: SiteCollectionItem }>(
      "collections",
      "items_get",
      { itemId },
    );
  ...
  async setItemFlags(
    ...
    const res = await callApi<{ items: SiteCollectionItem[] }>(
      "collections",
      "items_set_flags",
      {
        itemIds,
