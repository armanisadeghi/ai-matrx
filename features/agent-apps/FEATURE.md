...
- `2026-08-10` — claude: **the admin categories editor is agent-writable (3 draft targets).** `app/(admin)/administration/agents/agent-apps/categories/page.tsx` now registers `getWriteHandlers` on the `SurfaceRuntimeProvider` it already mounted, exposing `category_name`, `category_description` and `category_icon` on `matrx-admin/agent-apps` as `mode:"draft"` / `applyPolicy:"ask"`. Every handler calls the same `handleEditChange(field, value)` the form's own inputs call, so an agent write is indistinguishable from typing: it stages into `editData`, arms "Unsaved Changes"/Discard, and the admin's Save press is still what runs `updateAgentAppCategory`. Handlers throw on a bad shape (non-string, empty name) and refuse loudly when no category is selected; `category_icon` is validated with `isRegisteredOrLucideIconName` so an invented name never lands as a broken glyph. Also fixed a read lie the write work exposed: the surface emitted `selected_category` from the SAVED row while the pane rendered `editData`, so it disagreed with the visible form whenever anything was unsaved (agent or human) — it now emits from the live edit buffer, with `category_has_unsaved_changes` as the divergence flag against `categories_list`. Nothing else on this console is writable: featuring/verifying/publishing apps, rate limits, error resolution, category `sort_order`, create and delete stay human. Live-verified with a real Badass Agent run on a throwaway category (staged → admin saved → confirmed in `platform.categories`; decline, invalid-icon and undeclared-target paths all behaved). **Known bug found, not fixed:** `createAgentAppCategory` in `lib/services/agent-apps-admin-service.ts` never sets `created_by`, so `platform.categories`' `std_insert` RLS policy (`created_by = auth.uid()`) rejects it — the Add Category dialog 403s for every admin.
- `2026-08-10` — claude: **the two ENTITY write targets are now live on every `/agent-apps/[id]` sub-route, not just Settings.** Additive follow-up to the entry below (two agents reached this surface at once; that work is kept as-is). `app_category` and `app_tags` persist through `saveAppField` and need only the open row — not the Settings pickers — so [`route/AgentAppSurfaceRuntime.tsx`](route/AgentAppSurfaceRuntime.tsx) (the `[id]` layout, mounted on every sub-route) now registers them via `getWriteHandlers`, and an agent can retag or recategorize an app from overview / run / code / versions. Both mounts import ONE validator set from the new [`route/agent-app-entity-writes.ts`](route/agent-app-entity-writes.ts), so the contract an agent is told about cannot drift between them; the layout persists via `saveAppField().unwrap()` and the Settings registration (which shadows it while that tab is open) still goes through the `saveField` wrapper the pickers use. The three DRAFT targets stay Settings-only on purpose — they stage into inputs that exist nowhere else, and a draft with no input to land in is a write that goes nowhere. Live-verified: from the overview tab the agent was offered exactly `app_category` + `app_tags`, applied a full tag-set replace that survived a reload, and said plainly that `app_description` was unavailable there instead of substituting a target.
- `2026-08-10` — claude: **the Settings > Identity tab is agent-writable (5 surface write targets).** `matrx-user/agent-apps` now declares `writeTargets` for the app's storefront copy — `app_name`, `app_tagline`, `app_description` as `mode:"draft"` and `app_category`, `app_tags` as `mode:"entity"`, all `applyPolicy:"ask"`. Handlers live in `route/AgentAppSettingsContent.tsx` (`useSurfaceWriteHandlers`, registered under the `[id]` layout's `AgentAppSurfaceRuntime` provider) and go through exactly what the tab's own controls use: the three text targets call the same `setName`/`setTagline`/`setDescription` local setters the inputs are bound to, so the field lights up its dirty marker and Save button and the USER commits; the two picker-backed targets call the same `saveField` wrapper (`saveAppField` thunk) that `onChange` calls, so they persist immediately. That per-field split is deliberate — mode follows each control's real behavior, not the page's. `saveField` gained an optional `{ rethrow }` flag so an agent-originated write surfaces its failure to the writeback seam instead of only raising a toast; user clicks are unchanged. Handlers validate and THROW (wrong type, empty name, non-string or duplicate tags, empty-string category) and refuse when the Redux app row has not hydrated. `slug`, `status`, `is_public`, agent/version binding, `component_code`, shell config and rate limits are deliberately undeclared — they change what the app IS or who can reach it, not how it reads. Live-verified end to end with real Badass Agent runs; details and evidence in `features/surfaces/FEATURE.md`.
...
- `2026-07-17` — **Shared allowlist expanded (ratified: in-page user components are the platform's core delivery mechanism).** [`utils/allowed-imports.ts`](utils/allowed-imports.ts) — the sync scope for `compile-slot.ts`, shared by Agent Apps, the DB tool renderer, AND the new Content-IR DB kind components (`features/content-ir/react/db-component/`) — now registers the full shadcn set already proven by the async dynamic-react registry (badge/tooltip/accordion/collapsible/progress/separator/scroll-area/dialog/sheet/dropdown-menu/table/checkbox/radio-group/popover/avatar/alert/skeleton), `@/lib/utils` `cn`, `recharts`, and the full common React hook set. Every entry explicitly named; safe-proxy fallback unchanged; module lives only in lazy chunks so recharts never touches the main bundle. New `getDefaultImportsForKindComponents()` = full registered scope.
- `2026-07-15` — **Legacy-named execution/version RPCs aligned with canonical agent-app ACLs.** `get_prompt_app_execution_payload` remains anon-capable only for `visibility='public'`; private/internal apps require owner/viewer/service access. `pin_prompt_app_to_version` now requires owner/editor/service access and is no longer anon-executable. Both use fixed search paths. The payload's obsolete `output_format` key is retained as `null` rather than referencing the dropped agent column; `output_schema` remains authoritative.
- `2026-06-08` — claude: **execution tracking was silently dead since it was written** — fixed. The migration-ledger audit found `aga_executions.kind` absent in the live DB, and `aga_executions` held only 1 row despite agent-apps being live. Root cause: [`migrations/aga_executions_visit_run_tracking.sql`](../../migrations/aga_executions_visit_run_tracking.sql) was drafted against the **pre-rename** object names (`update_agent_app_success_rate` / `trg_agent_app_rate_limit` / `enforce_agent_app_rate_limit`), but the schema had renamed the `agent_app*` family to `aga_*` (finished in code 2026-04-25). Applied as-is it errored on the `CREATE TRIGGER ... EXECUTE FUNCTION enforce_agent_app_rate_limit()` step (function doesn't exist) and rolled back its `BEGIN/COMMIT` transaction — so the `kind` column never landed, and every `/api/agent-apps/[id]/track` insert (which writes `kind`) failed silently via the fire-and-forget `keepalive` fetch. Rewrote the migration against the live `aga_` names (`update_aga_success_rate`, `trg_aga_rate_limit`, `enforce_aga_rate_limit`) and dropped the in-file `BEGIN/COMMIT` (the applier wraps each file). Applied + recorded via `apply_migrations.py --source matrx-frontend --only`. Live-verified: `kind` column (`text default 'run'`, CHECK `visit|run`), `idx_aga_executions_app_kind_created`, kind-aware `update_aga_success_rate`, and `trg_aga_rate_limit WHEN (NEW.kind='run')` all present; a real `kind='visit'` insert now succeeds (it errored before). `pnpm db-types` regenerated. Visit/run analytics + success-rate rollups now actually record.
- `2026-05-10` — claude: fixed every authenticated `aga_apps` mutation (create, duplicate, edit-with-tracked-fields). The two version-snapshot triggers (`snapshot_aga_version_on_insert` + `snapshot_aga_version`) were inserting into `aga_versions` from inside the calling user's role, but `aga_versions` only has SELECT policies for `authenticated` (INSERT is `service_role`-only). Result: every user-driven `aga_apps` INSERT/UPDATE 500'd with `new row violates row-level security policy for table "aga_versions"`, swallowed by the route as a generic "HTTP 500" toast — duplicate was the most visible victim. Migration [`migrations/aga_versions_snapshot_security_definer.sql`](../../migrations/aga_versions_snapshot_security_definer.sql) flips both functions to `SECURITY DEFINER` with `search_path = public, pg_temp`; aga_versions stays write-locked from the client (no INSERT policy added) since users only ever write through the parent table. Same migration applied to Matrx Main as `aga_versions_snapshot_security_definer`. While the bug was open the duplicate route also wasn't carrying `app_kind` / `shell_kind` / `shell_config` / `slot_overrides` / `slot_code` / `shared_context_slots` / `metadata` / `preview_image_url`, so any successful copy would have silently degraded a `fully_custom` / `widget` / `form_to_result` shell into the default `chat` shell — those are now copied verbatim, and the new row is forced back into the duplicating user's personal scope (org/project/task nulled). `AgentAppsGrid.handleDuplicate` now reads `payload.error` + `payload.details.message` from the response body so future failures show the real Postgres error instead of `HTTP 500`.
