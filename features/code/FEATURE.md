...
- **No sub-routes** — the workspace is a single SPA-style surface; deep state lives in URL params and Redux. **The complete param list is `?open=` + `?agentId=` + `?conversationId=` — there is no other.**
...
- **`code_folder` / `code_repository` have no deep link** and their registry templates are deliberately empty — the workspace exposes no per-folder or per-repository destination, and an honest absence of a link beats a URL that 404s or no-ops. Give them real routes before giving them templates.
