...
- **PTY terminates at the sandbox orchestrator, never Next.js.** `SandboxProcessAdapter.openPty()` mints an existing sandbox-scoped `pty` token through `/api/sandbox/[id]/access-tokens`, then dials the returned `ws_base` directly. `TerminalTab` stays on a visible buffered fallback when mint/connect fails; a 200 SSE response with zero events is an error, never success.
...
- `2026-08-15` — **Restored the real xterm terminal through the existing browser-direct sandbox credential path.** `SessionsHost` mounts `TerminalTab`; `SandboxProcessAdapter.openPty()` mints a short-lived, single-use `pty` credential after the existing ownership check and dials the orchestrator's returned `ws_base` directly, bypassing Vercel's impossible WebSocket upgrade. The Promise still resolves only on `WebSocket.onopen` and rejects after 4 seconds. PTY data follows the daemon's real wire contract: raw text input/raw byte output, with JSON only for resize and `{ type: "signal", name }` control. PTY failure is loud in xterm and retains buffered execution. Streaming exec now rejects HTTP 200 streams that close with zero events, so `TerminalTab` visibly falls back to `/exec`; the exact empty-body case has a regression test. `SimpleTerminal` remains the no-sandbox shell state and the buffered implementation remains available.
