...
- `service.ts` — one definition read over all the feature's prefixes, then ONE `public.mnd_member_list` read per lane (holder-neutral: agent or workflow) filtered to the exact visible keys (`select` filter) — never one per prefix; each lane is a ~2 s full-corpus read on the server, which is now the whole wait (marketing: ~2.3 s of ~3 s).
...
- `places.ts` + `PlacesMap.tsx` — where each job runs. Sources: DECLARED feature maps (every map is listed in `registry.ts`, 45 features as of 2026-09-25) and REGISTERED screens (`ui.ui_surface_agent_role` ⋈ `ui.ui_surface`, read by key prefix in parallel with the jobs and kept to the jobs the viewer can see). Unnamed jobs read "Not recorded yet" — that is the honest state for jobs that run only in the background, in the Workflow Studio app (aidream `apps/workflow-studio`), the Chrome extension or the desktop app, none of which this repo's guard can read.
