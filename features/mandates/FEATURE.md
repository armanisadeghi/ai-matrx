...
- 🚨 **THE ADMIN SEAT** (Arman, 2026-09-26: "No one acts as themselves in admin"): lanes are System / Organizations / Users / All (`ADMIN_LIST_SCOPES` → `p_scope` system/orgs/users/all; narrow by org or by a person's personal org), never Mine / My Orgs. The database corpus is the WHOLE platform (`mandate._admin_list_rows` no longer filters to `iam.my_orgs()`); rows carry `owner_level` / `owner_label` (`mandate._admin_owner_level` / `_admin_owner_label`, applied through the Supabase MCP 2026-09-26) and the Owner column names the organization or person. A person's pin is never "the admin's own" (`workflowAdvanceEligibility`, `ADMIN_WRITE_CONTEXT` on the admin door); the mandate window on the admin seat reads the whole platform. Guard: `pnpm check:admin-no-personal-seat`.
...
- A door is `MandateDoorLink` (`variant="icon"|"inline"`; lands on the feature's intelligence page `/intelligence/<feature>` — the icon variant IS the Intelligence icon; `HeaderAction` passes `icon: "BrainCircuit"`); admin URLs via `adminMandateHref`; a mandate named on a working surface opens `useOpenMandateWindow`, never a `<Link>`.
