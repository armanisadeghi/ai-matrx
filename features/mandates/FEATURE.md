...
| **THE VOCABULARY** (`@ai-matrx/agents/mandates`) | 🚨 **No file in this repo names a mandate by hand.** The declared key set is PUBLISHED by the package (0.10.0), emitted by THE ONE GENERATOR (`aidream/scripts/mandates_generate.py` stage (h)) from the same in-process `declared_mandates()` the boot sync writes `mandate.definition` from. Import the member — `MANDATE_KEYS.seo__keyword_classifier` — never the literal; THE IDENTIFIER RULE is `.` → `__`. `MandateKey` types a carrier so a made-up key fails type-check; `isMandateKey` / `assertMandateKey` narrow anything that crossed a wire; `mandateKeyOfApp` / `mandateKeyOfShortcut` return a branded `DynamicMandateKey` for the DB-authored `app.*` / `shortcut.*` families, deliberately NOT assignable to `MandateKey`. 🚨 **A parse of a STORED ref does not narrow** — the SERVER, not this build's snapshot, is the authority on what exists (the matrx-extend ruling, `matrx-extend/src/lib/mandates.ts`). Guard: `pnpm check:mandate-keys` (below). |
...
| `admin/impact.ts` + `admin/impact-cells.tsx` | **THE STANDING TABLE'S READ OF THE ONE GRADER** (Agent Change Impact I4; contract `common-docs/projects/agent-change-impact/CONTRACT.md`). `fetchStandingImpact` pages every holder agent through aidream `POST /mandates/impact` (descendants off — the console already passes every holder), `groupImpactByMandate` puts each rung's verdict on its mandate row, and the meta tables say what every grade (identical/green/orange/red) and blocker (unreachable/set_aside/tracks_latest/unsupported_holder/none) means. `batchEligibilityOf` is R17 (a blocked row or a person's own pin is never a batch member), `isSafeGreen` is "Advance all green" (green/identical + no blocker + the server's `auto_advance_eligible`), `settingsSignalOf` renders `capability_checked=false` as UNMEASURED and changed settings keys as a change — never clean. `set_aside_reason` is read only when the blocker IS `set_aside` (R36 b). Nothing here grades anything. `MandateDetailPanel.updateToLatest`'s pre-flight reads the same verdict (R12); `computeRebindImpact` is now only its renderer. |
...
| `workspace/MandateCoverageAlert.tsx` | **THE DETAIL PAGE'S HALF OF THE SAME SCOREBOARD** (2026-09-09). The list screamed when a Mandate had no Holder; the detail page did not, so an admin arriving by link at `/administration/mandates/<key>` landed on the Definition tab whose only trace was the grey line "Declared by: Holder name unavailable" and an amber box titled with an INPUT problem. A banner now rides ABOVE the tabs on every host: red says the shared `RED_WORD` while the server's verbatim sentence owns the consequence (only the resolver can state it) and an **Assign a Holder** control opens the Holder tab; orange NAMES the leader carrying it; green is silent; a FAILED report says so out loud with the verbatim error, because a banner that vanishes on error reads as all-clear. One classification, the server's — it reads `GET /mandates/coverage/states` (unscoped, so it answers for a Mandate any organization homes) and inspects no holder, binding or pin of its own. Rules pinned in `workspace/__tests__/mandate-coverage-alert.test.ts`. |
...
- 2026-09-14 — **THE STANDING TABLE OF STALE MANDATES (Agent Change Impact I4).** `/administration/mandates` now opens on "Behind latest" (URL `table.mandates.behind`, default on) and carries two server-graded columns — **Grade** and **Blocker** — straight from aidream `POST /mandates/impact`, with the findings in the server's own sentences on hover, a one-glance legend, per-grade counts, the stale-and-safe count (the success measure), and the server's withheld sentence whenever the read withheld rungs (R31). Row selection uses the design-system `selection` config: only rows whose default rung is advanceable get a checkbox (R17); blocked rows keep a per-row Open door into the mandate window. "Advance selected", "Advance all green" and per-row "Advance anyway" all state their consequence first through `confirm()`. **The write is NOT wired:** `POST /mandates/impact/advance` / `/revert` (aidream a9527cd53) are absent from the generated `api-types.ts` and not deployed, and generated files are never hand-edited — so confirming says in words that nothing moved and logs the exact payload (`buildAdvancePayload`). Per-row batch results and the Revert door wait on those types. `MandateDetailPanel`'s single advance now pre-flights on the server verdict (R12). The flat `version drift` Health state is still present.
...
- 2026-09-10 — **THE DECLARATION GAP IS CLOSED, and four allowlist entries are gone.** The cross-repo item this repo opened the same day — 17 live `origin='code'` rows that no aidream `declare_mandate` call declared, so no generated union could ever contain them — was fixed at the source (aidream feedback `d8889c32`). Sixteen are declared now and one, `workflow.plan_shape_designer`, was retired as superseded: it never had a Holder so it never ran, and its job shipped in August as `workflow.plan_input_kind_authoring` + `workflow.plan_output_kind_authoring`. `@ai-matrx/agents` **0.10.1** publishes the complete set — **368 → 384 keys**, nothing renamed, nothing removed, nothing retiring, so the upgrade is purely additive.
  ...
  Adopted here the same session: `ambient.spoken_summary` (`features/surfaces/manifests/assistant-message.manifest.ts`), `product_capture.instant_analysis` (`features/product-capture/hooks/useInstantAnalysis.ts`) and `seo.session_rule_writer` / `seo.session_stamp_proposer` (`features/marketing/seo/value-system/workbench/session/trial.ts`) are `MANDATE_KEYS` members, and their four entries are **deleted** from `scripts/mandate-keys-allowlist.json`. `pnpm check:mandate-keys` is green against 384 keys with **3 allowlisted**, down from 7.

  The three that remain are the two that no generated union can ever hold, and they are correct as they are: `mandate.goal_writer` is `origin='user'` (Arman created it in the console on 2026-08-31), listed twice because two files name the same job; and `mandates.kind_converter` has no live row at all by design — the affordance renders disabled naming the missing key, pinned by `authoring/AutomationButton.test.tsx`.

  The gap cannot silently reopen: aidream's `scripts/check_mandate_declarations.py` now compares `declared_mandates()` against the live `origin='code'` rows and goes red on either difference (CI job `mandate-declarations`, credential-gated, `continue-on-error` per D23). It was proven RED at 17 live-only and GREEN at 384 == 384.

- 2026-09-10 — **THE VOCABULARY IS ADOPTED: this repo no longer types a mandate key by hand.** `@ai-matrx/agents` 0.10.0 publishes the declared key set (`MANDATE_KEYS`, `MandateKey`) and retires hand-typed literals and local `mandates.ts` mirrors — lane L6. Census over the tracked tree, by POSITION rather than by spelling: **200 literals in 76 production files** named a key in the vocabulary and are now the member (`MANDATE_KEYS.<family>__<name>`); **102 in 40 test/fixture files are left as-is** per the CHANGELOG, because a fixture invents keys on purpose (`zzz.scratch_job`, `should.not.be.sent`) and binding it to the vocabulary would make it a second authority. Position is the test because the namespaces overlap — `education.spoken_practice` is BOTH a mandate key and an entitlement meter id in `features/entitlements/registry.ts`, and only one of those is this rule's business.

  **Seven references name a key the vocabulary does NOT contain, and every one was resolved against live `mandate.definition` (project `brsgrqvjdzwihsvnfqkf`, 2026-09-10) rather than deleted or assumed.** None is a dead reference: five are LIVE rows the generator cannot see, because it emits from aidream's code declarations and nothing in aidream declares them — `ambient.spoken_summary`, `product_capture.instant_analysis`, `seo.session_rule_writer`, `seo.session_stamp_proposer` are `db_only` rows (aidream's own `services/mandates/service.py` names `ambient.spoken_summary` as one of the 24 the purpose guard measures; live there are 385 `origin='code'` rows against 368 declared keys), and `mandate.goal_writer` is `origin='user'` — Arman created it in the console on 2026-08-31. The seventh, `mandates.kind_converter`, has NO row at all and is the documented honest-refusal posture: the affordance renders disabled naming the missing key, pinned by `authoring/AutomationButton.test.tsx` ("a key with no row at all"). All seven sit in `scripts/mandate-keys-allowlist.json` WITH the reason and the verification date — named, never hidden. **The open cross-repo item is the DECLARATION gap on the aidream side**: 17 live `origin='code'` rows that no `declare_mandate` call declares, so no generated union can ever contain them.

  **Guard: `pnpm check:mandate-keys`** (`scripts/check-mandate-keys.ts`) — a TypeScript-AST scan that flags a key-shaped literal in a mandate-key POSITION: the initializer of an identifier/property/JSX attribute whose name contains "mandate", or the key argument of an entry point (`resolveMandate`, `resolveMandateServer`, `useMandate`, `useMandateSet`, `launchMandate`, `adminMandateHref`, …), walking arrays, records, ternaries and `as const` casts. There is NO baseline — the tree was taken to zero, so a finding is always new. **Proven failing then passing:** against a worktree at the pre-adoption commit it reported 200 violations across 76 files and exited 1; against the adopted tree it reports 0 and exits 0. `scripts/__tests__/check-mandate-keys.test.ts` (11 cases) is the durable RED/GREEN self-test — every position the guard claims, plus the three things it must NOT flag (the `MANDATE_KEYS` member, an identically-spelled entitlement meter id, a mandate-named holder carrying a label or a UUID). Registered in `run-release-gates.sh` (both blocks) and per-PR CI, with its self-test, per the `check-doc-claims` convention.

  One collateral fix at the class: jest's `moduleNameMapper` hand-listed each `@ai-matrx/agents` DIRECTORY subpath, so `@ai-matrx/agents/mandates` resolved to `dist/mandates.js` and every suite importing the vocabulary failed to run. The three are now one alternation (`matrx|catalog|mandates`) with the note that a new directory subpath belongs in it.

- 2026-09-09 — **The workspace's `RunThisJobSection` is DELETED; there is ONE run form.** `816ea88701` ("refactor(mandates): separate configuration concerns into tabs") moved every admin concern into tabs and stopped mounting it — the run affordance is the super-admin **Test** tab (`MandateWorkspace` `adminContent` → `MandateDetailView section="test"` → `MandateTestBench` → `TryItNowPanel`), which already ran the same `runMandateAdHocTest`, the same served input surface, the same `RunFailureCard` and the same folded `ServerNotes`, plus exemplar saving. The orphan sat unmounted for a day with two suites still pointed at it, so it is annihilated rather than left as dead code and its guards follow the affordance to its live host: the honesty pair (a 200's notes shown, a 409 kept in the panel) is now `admin/__tests__/run-panel-honesty.test.tsx` against `TryItNowPanel`, and the ONE thing the orphan had that the live form did not — the sentence stating that leaving an OPTIONAL `binding_prompt` input blank makes the run use the holder's own default — was ported into `TryItNowPanel` first, still guarded by `__tests__/invoke-supplied-values.test.ts`. `components/official/__tests__/server-notes-folded.test.tsx` now asserts there is exactly one mandate run form, because a second one is how the fold ruling drifted before.
- 2026-09-09 — **`ScopeHolderBar` renders every `job` prop it accepts.** `96e45f3aa2` replaced the three-cell bar with property rows and deleted the JOB cell; `coverageLine` came back because a guard named it, while `label`, `offeredCount` and `offerSourceLine` went on being declared, being passed by `OneBindingWorkspace`, and reaching nobody. They are restored as ONE row in this surface's density — the job's name and key, the offer count in the row's source slot, and the sentence naming where that offer comes from as visible second-line text, never a hover. `bindings/__tests__/scope-holder-bar-renders-every-job-prop.test.tsx` guards the BOUNDARY, not the three sentences: every `job` field must be visible text or declared as forwarded to a named control, so a field added tomorrow fails until it has a home.
- 2026-09-09 — **Source & Usage now reads the REPORTED inventory** (`GET /mandates/{key}/references` over `mandate.reference`), not code-truth's import-time `call_sites`. The shape is unchanged — Defined in / Used by, one copyable location per row, technical paths system-admin-only — but a call in matrx-frontend, matrx-extend or matrx-local can now appear at all. A row whose flag is not `ok` (orphaned / unresolved / broken / conversion_pending) is KEPT and carries a red glyph with the server's own sentence and remedy (ruling D21: unreachable is a flag, never a filter); `app.*` / `shortcut.*` rows say "one consumption site by design". The empty state NAMES the repositories with no complete scan and never says "unused". The old code-truth declaration survives as a LABELLED fallback for the declaration half only, used exactly when no scan has reported a declaration. Files: `admin/references.ts` (typed reads + ingress narrowing), `admin/MandateSourceUsage.tsx`, rules pinned in `admin/__tests__/mandate-references.test.ts`. The fleet half is the new admin board `/administration/mandates/references` (`admin/MandateReferenceBoardView.tsx`): per ACTIVE `platform.repo` row the last complete candidate and deployed scan or **unverified**, open findings with location and remedy, and the conversion list with its count. Campaign lane L7 — `common-docs/projects/mandate-declaration-reporting/`.
...
- 2026-08-31 — **THE INPUT SURFACE IS SERVED, and "user text only" became a measurement.**
  ...
  ONE rule for the phrase — served nothing AND nothing failed to load. `admin/TryItNowPanel`
