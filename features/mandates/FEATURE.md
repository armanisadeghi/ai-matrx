...
| `workspace/MandateCoverageAlert.tsx` | **THE DETAIL PAGE'S HALF OF THE SAME SCOREBOARD** (2026-09-09). The list screamed when a Mandate had no Holder; the detail page did not, so an admin arriving by link at `/administration/mandates/<key>` landed on the Definition tab whose only trace of the fact was the grey line "Declared by: Holder name unavailable" and an amber box titled with an INPUT problem. A banner now rides ABOVE the tabs on every host: red says `RED_WORD` plus the consequence ("nothing runs when this Mandate is called") with the server's own sentence verbatim and an **Assign a Holder** control that opens the Holder tab; orange NAMES the leader carrying it; green is silent; a FAILED report says so out loud with the verbatim error, because a banner that vanishes on error reads as all-clear. One classification, the server's — it reads `GET /mandates/coverage/states` (unscoped, so it answers for a Mandate any organization homes) and inspects no holder, binding or pin of its own. Rules pinned in `workspace/__tests__/mandate-coverage-alert.test.ts`. |
