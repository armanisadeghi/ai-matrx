...
function FactsPanel({
  ...
  if (diagnosticsOnly || sourceOnly) {
    ...
    if (sourceOnly) {
      ...
      // row — but code-truth's `call_sites` were import-time discovery over
      // one process, so a call in matrx-frontend, matrx-extend or matrx-local
      // could never appear there at all. The old declaration string is kept as
      // a LABELLED fallback for the declaration half only, used exactly when
      // no scanner has reported a declaration for this key yet.
