    ...
    if (sourceOnly) {
      ...
      return (
        <div className="min-w-0 space-y-4">
          <div role="status" className="flex items-start gap-2 rounded-md border border-warning p-3 text-sm">
            <AlertTriangle className="mt-0.5 size-4 shrink-0 text-warning" aria-hidden="true" />
            <div>
              <strong>In development — incomplete data.</strong> Locations may be missing or inaccurate. No discovered calls does not mean unused.
            </div>
          </div>
          <section className="space-y-2" aria-label="Defined in">
            ...
            <div className="flex items-center gap-3 rounded-md border border-border px-3 py-2 text-sm">
              <span className="min-w-0 flex-1 [overflow-wrap:anywhere]">{declaration ?? (truth?.import_error ? "Discovery failed" : "Location not recorded")}</span>
          ...
          <section className="space-y-2" aria-label="Used by">
            ...
            <div className="divide-y divide-border rounded-md border border-border text-sm">
              {sites?.length ? sites.map((site, index) => {
                ...
                return <div key={`${location}:${index}`} className="flex items-center gap-3 px-3 py-2"><span className="min-w-0 flex-1 [overflow-wrap:anywhere]">{location}</span><CopyButton content={location} label="Copy usage location" size="sm" /></div>;
