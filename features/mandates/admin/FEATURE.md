...
`features/mandates/workspace/MandateWorkspace` (the triad INPUT → GOAL → OUTPUT,
...
Manual exemplars accept both variables JSON and `user_input`. The server persists a capped history (100 entries) in the existing `agent.exemplar.metadata.test_bench_results`; the frontend parses that open JSONB defensively. A rejected history entry stays persisted for forensic recovery, while the diagnostic names the mandate, exemplar, entry index/id, received keys, and every failed field. Contract renames migrate stored JSONB in the database — the parser never aliases retired field names. Owner verdict notes and reference promotion are direct, optimistic-concurrency Supabase updates because they are pure record CRUD. Python owns only agent execution and the result append. API request/response aliases come exclusively from `types/python-generated/api-types.ts`; there is no local response mirror or backend `fetch`. Image outputs still render via `InlineMediaRef` (file_id recovered with `fileIdFromUserFilesUrl`, never a raw expiring URL).

**COLD MANDATES: "Try it now" is the way in (`TryItNowPanel.tsx`, 2026-08-15).** A batch needs stored test cases, and most live mandates have none — so for them the batch bench had nothing to run and the console's whole comparison story was unreachable. The panel runs ONE candidate on inputs typed right now (`POST /mandates/{mandate_key}/test` with inline `variables`/`user_input`, no exemplar; the server persists nothing), and **Save as test case** writes the inputs AND that run — as the reference — into one new `agent.exemplar` insert, with the result embedded in `metadata.test_bench_results` in the server's own `MandateTestResult` shape (carrying its new `exemplar_id` and a `promoted_to_reference_at` stamp), so it reads identically to a promoted batch result. Two rules:
...
- 2026-09-08 — **`/administration/mandates/[mandateKey]` is the `matrx-admin/mandate-workspace` surface.** `AdminMandateWorkspacePage` mounts `<SurfaceRuntimeProvider>` (identity + merged descendant contributions); the goal editor publishes `mandate_goal` / `mandate_goal_grounding` / `mandate_output_kind` / `mandate_goal_draft` and owns the one write target `mandate_goal_draft` (draft, ask). _Refine with AI_ opens the agent run window on `mandate.goal_writer` with this surface adopted, so the writer's `agent_mandate_specification` renders as itself and its charge lands in the editor via _Use as goal_ (kind component) or the writer's own `apply_surface_write` call. Details + rationale: `../FEATURE.md` Change Log, same date. Known: `check:agent-disclosure` still lists this route file because its `params: Promise<{ mandateKey: string }>` matches the guard's `mandateKey:` run-signal — a false positive shared with the `(core)` route, not a missing disclosure (the manifest declares the `goal_writer` role).

- 2026-09-07 — **The coverage board counts the rows the console lists, and can no longer see anything wider.** The third unscoped read of the mandate corpus on this page: `MandateCoverageBoard` took the raw `MandateCoverageResponse` from `GET /mandates/coverage`, which classifies the WHOLE corpus (682 definitions, every organization's), so its tiles counted — and its named strips NAMED — mandates the console's own `system`-home table correctly excluded (a walk of v0.4.1718 caught the "Nothing assigned" tile naming an org-homed mandate). The board now takes a `ScopedMandateCoverage` the console derives from its own `allRows` via `scopedCoverageOf` (`features/mandates/coverage.ts`), which intersects the server's verdicts with the caller's corpus and never re-derives the classification — green/orange/red stays aidream's `services/mandates/coverage.py`. The board imports no server-report type at all, so it cannot render an unscoped number. Degradation is unchanged: `coverageError` prints the verbatim server failure, and the view stays `null` until BOTH the rows and the verdicts land, so an unknown count shows "…"/"—" rather than a confident 0. The class guard `features/mandates/__tests__/scoped-corpus-reads.test.ts` gained `unscopedCoverageRenders` plus a proven-red case quoting the shipped board.

- 2026-09-07 — **The console became the `system` HOME of the one list door, and its two unscoped reads are gone.** `fetchMandateConsoleData` read `mandate.definition` with no ownership predicate and `mandate.binding` with none either, its comment claiming _"RLS already narrows them to what the caller may see"_ — for a platform admin `platform_admin_all` is `USING (is_platform_admin())`, so it narrowed nothing and the console pulled every binding in the database (283 live) to filter client-side. Ownership now comes from `mnd_list_scoped(p_home => 'system')` via `features/mandates/list-door.ts` (409 rows, 7 bindings, measured live on production 2026-09-07), the definition rows the console needs are fetched BY ID and the bindings by mandate id, chunked at 100 so a 409-id filter never becomes a 15KB request line. A key-scoped load (the mandate window, the admin workspace) still reads by key with RLS as the ceiling and takes the default home. When the door REFUSES the caller the system home, the console prints the refusal — the `(admin)` tree admits any admin level while the door gates on `is_platform_admin()` — instead of a toast over an empty table.
...
- 2026-08-29 — **ONE mandate UI, and management is admin-side.** Two mandate experiences
  ...
  menu, the legacy `?mandate=` param) navigates to `/administration/mandates/[key]`.
  ...
  Creation moved here (`/administration/mandates/new`) from the user route, which
...
- 2026-08-28 — **The coverage board, the real GOAL, the full Provision, and four
  offenders closed.**
  - **Coverage board** (`MandateCoverageBoard.tsx` + `mandate-coverage.ts`, moved 2026-08-29 to `features/mandates/coverage.ts` so the user-facing lists share it): three
  ...
  - **The goal is CODE.** `agent.mandate` has no `goal` column and aidream
    ...
    (`features/mandates/catalogue.ts` → `useMandateGoal`) and every
...
- 2026-08-26 — **Test cases generalized to the AGENT level: `agent.mandate_exemplar` → `agent.exemplar`** (token `agent_exemplar`). `agent_id` is now the NOT NULL subject; `mandate_id` is optional call-site context, so this bench reads the same table filtered by `mandate_id` while the builder/runner/admin sample surfaces (`features/agents/samples/`, `features/agents/components/samples/`) read it by `agent_id`. New columns: `status` (`candidate`/`approved`/`archived` — approval via `public.agx_exemplar_approve`, cap = knob `agent_exemplars.max_approved_per_agent`), `agent_version`, `input_contract_hash`/`output_contract_hash` (staleness vs the head is DERIVED, never stamped), `source_conversation_id` (borrow provenance), and `source` gains `borrowed`. `createMandateExemplar`/`saveAdHocResultAsExemplar` now take the full mandate row and stamp `agent_id` (`requireMandateAgentId`). Bench saves land as `candidate` — approval is the separate agent-level curation step, and only approved samples surface as one-click defaults (via the builder-only floating launcher — samples never add page chrome). A TEMPORARY `agent.mandate_exemplar` alias view was created for the deploy window — its expiry condition (FE release + aidream deploy) was met 2026-08-30, and the DROP is the top pending item in `common-docs/systems/agents/agent-samples/HANDOFF.md`; agent-level batch runs live at aidream `POST /agent-testing/agents/{id}/tests` (results under `metadata.agent_test_results`, separate from this bench's `test_bench_results`).

- 2026-09-17 — **Search and doors repaired as one discovery path.** A non-empty
  admin search now searches the complete mandate catalogue instead of silently inheriting
  the default Behind-latest work queue; explicit coverage filters still apply. Search text
  includes key, name, label, feature, agent, goal, and description. The human label and
  normal row click now open the canonical admin mandate workspace; raw inspection remains
  available only as an explicit row action. The user and organization lists were censused
  and use the shared server-ranked search door. The census also found that the organization
  table's Job column had frozen the personal route inside the shared column constant; its
  columns are now built from the host's route, matching its cards, compact rows, and row action.
...
- 2026-08-18 — Gave code truth a 60-second connection window, kept retryable
  ...
  aidream now preloads the immutable source inventory.
...
- 2026-08-08 — Bindings are now EDITABLE in the console: `MandateDetail` embeds the shared `MandateOverridePanel` (`features/mandates/components/`) — user + admin-org bindings incl. settings-only `config_overrides`, written via the aidream bind endpoint (`PUT/DELETE /mandates/{mandate_key}/binding`, 422 contract verdicts shown verbatim); the read-only "All overrides" roll-up stays below it.
