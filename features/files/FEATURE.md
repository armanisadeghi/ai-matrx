...
| Routes                                    | `app/(a)/files/` (`/files`; `/cloud-files/*` 308s here), plus internal-test Drive metadata at `app/(core)/files/google-drive/`. Public shares: `app/(public)/s/[token]/`                                                                                                                                               |
...
19. **`FileAcquisitionActions` is the one source chooser.** Local file/folder inputs, existing-Matrx-Files selection, Google Drive's native Picker, and the shared OneDrive/Dropbox/Box storage-source picker belong there; hosts select a presentation and disable sources with props. Every source receives the same `accept` and single/multiple rule. Canonical provider imports live under `features/files/storage-sources/`, upsert their authoritative Files row, and hand consumers only `fileId` identity — never a browser `File`, provider URL, token, or browse metadata. A menu host keeps its chooser mounted until local selection returns, then closes through `onLocalSelectionComplete`. Drag/drop and paste remain host gestures, but their resulting `File[]` must enter the same upload callback. Never synthesize an input, query a page-wide file input, or fork a partial source menu.

20. **Internal Drive metadata browsing is a capability-gated review surface.** The `/files/google-drive` desktop and mobile Files entry appears only when the caller-specific `drive_browse` catalog capability is `eligible`; the backend remains authoritative. It uses only the exact selected Google connection and the bounded metadata-only `google-sync/drive/browse` and `google-sync/drive/file` calls. Page tokens remain tied to immutable applied search/folder criteria, folder and reset actions clear the visible search, incomplete search is shown, and opening Google claims a blank click-owned tab then requires a fresh same-connection accessible file check before navigating it; a listing link is never opened directly. This does not fetch file content, request consent, or replace the canonical Google Picker acquisition flow.
21. **A selectable file thumbnail is `SelectableFileThumbnail`.** Its thumbnail opens the canonical File Preview WindowPanel, its independent 44px control changes selection, and `FileRightClickMenu` supplies the universal menu. Hosts provide labels/icons and the selection callback; they never rebuild any of those three behaviors.
22. **`get_user_file_tree` is an authenticated Data API door.** Keep its exact signature in
...
23. **File Copy for AI starts with `<file_ref>`.** `fileInfoAgentPayload` carries `file_id` plus a
...
24. **Duplicate decisions are durable intents.** `Use existing` returns the canonical id,
...
- **2026-09-26 — Internal Drive metadata review is capability-gated and honest.** Files now exposes the bounded `/files/google-drive` internal-test route through its desktop and mobile navigation only for callers whose `drive_browse` catalog capability is eligible. The selected connection is explicit; search/folder pagination retains applied criteria while folder/reset actions clear the search, incomplete Google results are disclosed, file types are human-readable, and every external Google open claims a blank tab during the click then refreshes selected-file access on that same connection before navigating it. The surface uses metadata-only browse/file APIs, never fetches content or requests a Drive grant.

- **2026-09-21 — The multi-pick file window says how to finish.** `openFilePicker({ multi: true })` resolves the batch when the window closes, but `FilePickerWindow` (`features/resource-manager/resource-picker/`) ran the picker in single mode with no footer, so a click only toggled a highlight and nothing said what to do next — every multi-pick caller could fail silently (found live on the data-table Attachment column, whose picker "had no Attach button"). The window now takes `multi` / `pickedCount` / `onUnpick` from `CloudFilesPickerHostImpl`, runs the picker in `selectionMode="multiple"` (unchecking removes from the batch) and shows "N files selected · Attach N files", which closes and resolves. Single-pick is unchanged. Still open: a picker opened with no organization selected is refused by the files session (400) and reads as a cancel — the caller sees `null` either way.
- **2026-09-19 — Provider imports now reach image selection, and Google Drive uses retained settlement.** The dropzone acquisition adapter carries the host's single/multiple rule and canonical provider-import callback; Image Manager feeds returned `fileId` values through the same `handleUploaded` selection path as local uploads. Native Google Picker still uses `drive.file`, but its import flow now keeps canonical successes separate from consumer acknowledgement, retries delivery without another provider import, retries only failed selections, and waits for an active import when the window is closed.
