...
- **2026-07-16 — Smart Agent Cloud Files picker search and browse reliability.** `FilesResourcePicker` now debounces and cancels authenticated `GET /files/search` requests instead of filtering only the hydrated Recents/tree cache; its compact filter row includes a PDF Extractor preset that mirrors that tool's `.pdf,image/*` acceptance, plus file-kind and sort choices. Expanding a folder whose children have not yet been cached dispatches `loadFolderContents`, so the picker no longer presents unloaded folders as empty. `MediaThumbnail` also no longer attempts to render `Asset.primary_url` / `original` as an image when no thumbnail variant exists; those source URLs can be non-image bytes, so the canonical file icon fallback now remains error-free.
