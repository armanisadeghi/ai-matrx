...
| File bytes (upload / download) + URL signing                              | REST API only. Browser never touches S3.                                                                    |
| Metadata reads (list, tree, versions, permissions, search, trash, usage)  | supabase-js (RLS auto-filters / auth-checked RPCs)                                                          |
| Metadata writes (rename, move, soft-delete, restore, permissions, shares) | **supabase-js direct** — auth-checked `SECURITY DEFINER` RPCs (NOT REST; REST is for byte-bearing ops only) |
...
- `files.files` (legacy name `cloud_files`) — file metadata. `created_by`, `file_path` (logical), `file_name`, `mime_type`, `size_bytes`, `checksum`, `visibility` (public | private | shared), `current_version`, `parent_folder_id`, `metadata` jsonb, timestamps, `deleted_at` (soft delete). The native storage location (`storage_uri`) is **server-only** — its column grant is REVOKEd from `authenticated`, so **every client read selects `FILES_TABLE_COLUMNS` from [filesDb.ts](filesDb.ts), never `select("*")`** (which errors).
...
Python deploy (see [`docs/PYTHON_UPDATES.md`](../../docs/archive/2026/PYTHON_UPDATES.md);
...
> 🚫 **`storage_uri` is BANNED on the client** (eradicated 2026-07-06) — never select it, never model it (no `storageUri`/`fileUri` in any type, slice, adapter, or prop). The column grant on `files.files` is REVOKEd: `select("*")` ERRORS — every read uses `FILES_TABLE_COLUMNS` from [filesDb.ts](filesDb.ts). Identify files by `id`/`file_id`; render/download via the URL contract `url` / `cdn_url` / `signed_url` (+ `signed_url_expires_at`) / `download_url` / `thumbnail_url` — never hand-build a URL from a storage location. ESLint enforces.

1. **Single types file** — [types.ts](types.ts) is the only type source. Import `@/features/files/types` directly; never duplicate types in subfolders.
2. **No direct object-store SDK calls** — every file flow uses this feature.
...
- **2026-07-20b — Picker host sizing, scroll affordance, and window-drag fixes.** Three defects in the new `FilePickerWindow` host. (1) **Cut off with dead space:** `FilesResourcePicker`'s root was hardcoded `max-h-[min(460px,70dvh)]` (chat-popover sizing), so in a 600px window it capped at 460px and left white space under a clipped row. New `fillHost` prop (default `false` — chat popover unchanged) switches the root to `h-full min-h-0` so the list uses the host's full height; `FilePickerWindow` and the association-sheet browse body pass it. (2) **No visible scrollbar:** `scrollbar-thin` is 4px and invisible at rest under macOS overlay scrollbars, so a clipped row read as "the list ends here". Added the `.scrollbar-visible` utility (10px, always-on, `app/globals.css`) plus a bottom fade driven by a `ResizeObserver` + scroll listener that appears only when content extends below the fold. Use both on any bounded list where the user must know there is more. (3) **Window inert / not draggable:** the panel portals to `<body>`, and Radix surfaces (Popover/Select/Dialog) set `pointer-events: none` on `<body>` while open — an inherited value leaves the window visible but completely dead (no drag, no scroll). `FilePickerWindow` re-asserts `pointer-events-auto` on its own root, and its window id now includes `useId()` so two simultaneously-mounted pickers can't share one window-manager entry (the first unmount's `unregisterWindow` silently kills drag for the survivor). Also: `FilePickerWindow` MUST be `next/dynamic`-imported — `WindowPanel` asserts lazy parsing (`lazy-bundle-guard`); all five consumers were converted.
...
- **2026-07-15** — **Direct browser cutover gated after uploads broke on CORS preflight.** `files.matrxserver.com/files/upload` returned HTTP 405 to authenticated browser `OPTIONS` requests, so XHR surfaced the misleading network error "Upload failed — check your connection" before any bytes reached the service. `lib/python-client.ts` now sends browser file traffic through the proven aidream in-process routes by default; the exact standalone route map remains ready behind `NEXT_PUBLIC_FILES_BROWSER_CUTOVER=true`. This restores canonical chat paste/attachment uploads while keeping server-side and public-share usage of `NEXT_PUBLIC_FILES_URL` intact.
- **2026-07-14** — Promoted `FileRightClickMenu` through the files public surface so RAG/document detail consumers can offer the canonical Preview / Download / Copy link / Details / Versions / Context / Visibility / Delete menu without importing file-component internals.
- **2026-07-14** — **File Preview windows can open PDFs at an exact page.** The typed `useOpenFilePreviewWindow({ fileId, pageNumber })` contract now carries a 1-based page through `FilePreviewWindow` → `PreviewPane` → `FileTabsBody` → the canonical controlled `PdfPreview`. Page navigation remains interactive after the initial jump, repeated calls for the same file/page replay the navigation request instead of preserving a page the user browsed to previously, and URL sync serializes the same target as `?panels=file_preview:<fileId>:p-<page>`.
- **2026-07-14** — **Completed browser cutover to `matrx-files`.** The universal Python client now chooses the service origin at its final URL-construction boundary for the exact `/files`, `/assets`, and `/share` routes owned by the standalone service. File-handler URLs use the same files-specific resolver. Aidream-only routes such as `/files/{id}/ingest`, RAG, annotations, and media processing stay on the active general backend; explicit per-request overrides still win. This closes the gap that left `GET /files/{id}`, `/url`, and `/asset` hitting aidream even after `NEXT_PUBLIC_FILES_URL` was configured.
- **2026-07-08** — **D20 closed: OAuth signup no longer orphans guest data.** Email/password keeps the in-place promotion (`lib/services/guest-promotion.ts`); OAuth cannot (no server-side API attaches a provider identity to an existing user), so it gets the transfer twin: `lib/services/guest-oauth-transfer.ts` — OAuth actions stash the `GuestFingerprintField` fingerprint in a 10-min httpOnly cookie (`matrx_guest_oauth_fp`); `/auth/callback` resolves fingerprint → anon UUID and calls `public.transfer_guest_data_to_user` (SECURITY DEFINER, service-role-only, migration `guest_oauth_data_transfer.sql`) which repoints EVERY FK column referencing `auth.users(id)` (discovered at runtime — new tables auto-covered), audits to `public.guest_conversion_audit`, stamps + frees the `guest_executions` mapping. Fail-open: any failure = normal OAuth login, loud log.
- **2026-07-07** — **`42501 permission denied for table files` fixed.** `authenticated` had column-level SELECT on the client-safe column list but no table-level SELECT — PostgREST rejected every `.schema('files').from('files')` query (e.g. `/files/all/Diagrams` folder contents). Migration `files_authenticated_table_select_grants.sql`: `GRANT SELECT, INSERT, UPDATE ON files.files` (+ `file_versions`) to `authenticated`; `storage_uri` stays server-only via explicit column REVOKE + the existing `FILES_TABLE_COLUMNS` allowlist.
- **2026-07-06** — **`storage_uri` eradicated from the client.** The native S3 location is server-only (info-disclosure + useless in a browser); the backend stopped returning it on every response and the DB REVOKEs the column grant on `files.files`. All FE selects now use `FILES_TABLE_COLUMNS` ([filesDb.ts](filesDb.ts)); `storageUri`/`fileUri` deleted from every client model; files identify by `id`/`file_id` and render via the `url`/`cdn_url`/`signed_url`/`download_url` contract. See the Invariants banner above. Older change-log entries below mentioning `storage_uri` are historical.
...
- **2026-07-06** — **`FileContextMenu.extraMenuItems`.** Host surfaces (PDF Extractor sidebar) can inject additional dropdown items before Delete without forking the menu. PDF studio uses this for extractor-only actions on top of the full files-route menu.
...
- **2026-06-23** — **Wired the Files browser into the agent-context surface system (`matrx-user/files`).** New [`agent-context/buildFilesContextData.ts`](agent-context/buildFilesContextData.ts) — a PURE map of live browser state (active file id/name/path/mime/size, active folder id/path, selected file ids/names + count) into the surface's `createFilesScope(...)` (manifest `features/surfaces/manifests/files.manifest.ts`), plus a navigation-map `context` blob; the text baselines (`selection`/`content`/`text_*`) are metadata-empty here and left to the platform floor. Exports `FILES_CONTEXT_MENU_PROPS` (`sourceFeature: "programmatic"` — closest valid `SourceFeature`; no `"files"` literal exists yet). [`PageShell`](components/surfaces/PageShell.tsx) wraps the read-only [`PreviewPane`](components/surfaces/PreviewPane.tsx) in a code-split `UnifiedAgentContextMenu` (`isEditable={false}`, live `getApplicationScope` capturing any highlighted text) so right-click offers Files-bound agent actions about the active file. Editable inputs upgraded to the canonical Pro components: the [`TopBar`](components/surfaces/desktop/TopBar.tsx) search box and the [`RenameDialog`](components/core/RenameDialog/RenameDialog.tsx) name fields are now `ProInput` (voice + copy + clear). No manifest/`SurfaceValue` change — every emitted value was already declared.
- **2026-06-21** — **Route selection sync + folder navigation hardening.** Fixed stale one-shot folder selection (`useOneShotSelection` only ran once — back/forward and remounts could desync Redux from `/files/all/<path>`). New [`FilesRouteSelectionSync`](components/surfaces/FilesRouteSelectionSync.tsx) keeps pathname ↔ `activeFolderId` aligned, client-resolves folder paths when the server Supabase lookup misses, and never wipes a valid in-flight selection while the tree is still loading. Folder drills now call `loadFolderContents` from every entry point (PageShell, NavSidebar, ContentHeader). `loadFolderContents` marks folders fully loaded. Clarified URL contract: folders live in the pathname; files live in `?file=<id>` only.
- **2026-06-21** — **Folder navigation URL fix.** `PageShell.handleSelectFolder` was pushing `/files/<path>` instead of the canonical `/files/all/<path>`, so clicking a folder (e.g. `Other People/ellie`) landed on a non-existent route and broke the workspace. Fixed the push target; added `app/(core)/files/[...path]/page.tsx` to redirect legacy deep-links; aligned share-link folder redirect + `ExportPanel` "Open folder" link.
- **2026-06-20** — **New consumer: the `matrx_file` markdown render-block — recognize OUR file links anywhere in chat and render them as the real file.** A model frequently hands the user a bare link to a file we generated (most often an expiring signed S3 URL, but also a public CDN or share-link URL) instead of an `![](…)` image embed. We now recognize those links and promote them to a type-aware inline previewer that routes through this handler — so signed URLs auto-re-mint and the file renders as exactly what it is (image / pdf / audio / video / code / data / markdown / text / html), degrading to the plain link on any failure. New pieces: **`lib/media/our-file-sources.ts`** — the single, extendable recognizer (`recognizeOurFileUrl` + `OUR_FILE_URL_MARKERS`, mirroring the host list in `eslint.config.mjs`) that returns the strongest `FileSource` (identity beats opaque URL); **`components/mardown-display/blocks/matrx-file/{MatrxFileBlock,UniversalInlineFile}.tsx`** — the renderer, which re-uses this feature's primitives end-to-end (`useFile` for type discovery + row hydration, `useFileSrc` for the minted URL, `InlineMediaRef` for media, and the code-split `FilePreview` previewers `PdfPreview`/`SvgPreview`/`HtmlPreview`/`DataPreview`/`CodePreview`/`MarkdownPreview`/`TextPreview` for documents). Detected client-side **after** image/video/audio in `content-splitter-v2.ts` (`detectMatrxFileMarkdown`), with parity in the stream accumulator + prefilter (`Candidate.MATRX_FILE`). No new storage paths or direct object-store calls — everything flows through the existing handler. See `components/mardown-display/chat-markdown/MARKDOWN_PARSING.md`.
- **2026-06-14** — **Desktop shell: removed the always-on icon rail, made the folders sidebar collapsible, and killed the bottom gap.** Three fixes in [`PageShell.tsx`](components/surfaces/PageShell.tsx) + [`desktop/IconRail.tsx`](components/surfaces/desktop/IconRail.tsx) + [`desktop/NavSidebar.tsx`](components/surfaces/desktop/NavSidebar.tsx): (1) the root wrapper dropped `h-[calc(100dvh-var(--header-height))]` for `h-full` (the shell `main` is already full-viewport per the transparent-header design; subtracting the header height left an empty strip at the bottom) — same fix in [`loading.tsx`](<../../app/(core)/files/loading.tsx>). (2) The slim 4-icon rail used to render _alongside_ the folders sidebar (a misunderstanding — it's meant to BE the collapsed sidebar). It's now rendered only while the sidebar is collapsed, with a top "expand" button; the spare "More" item was dropped. (3) `NavSidebar` gained a collapse button (next to the flat/tree toggle); the `cloud-files-side` panel is driven via `usePanelRef().collapse()/.expand()`, intent mirrored in `useState` (updated from `onResize` so drag-to-collapse also flips the rail) and persisted in the `cloud-files:sidebar-collapsed` cookie. The preview-maximize path guards its imperative `setLayout(SIDE=0)` so it doesn't spuriously trip the collapsed rail.
- **2026-06-14** — **Transcript recordings relocated to the system namespace (backend) + FE alignment.** Backend now relocates every `origin: "transcripts"` upload to the hidden `system-files/transcripts/Recordings/...` root (and corrected 85 mis-typed `video/webm` → `audio/webm`; backfilled all 321 existing recordings; deleted the empty `Transcripts/` user folders). Because they're under `system-files/`, `isSystemPath` now hides recordings from the tree, folder views, AND Recents — managed solely via the Transcripts UI by `cld_files.id`. FE: `TRANSCRIPT_RECORDINGS` constant repointed to `system-files/transcripts/Recordings`; `audioStorageService.UploadResult` dropped its hand-built (now-wrong) `filePath` — only `fileId` is returned/used. **Kept** the legacy `Transcripts/Recordings` path as a defensive Recents guard (`TRANSCRIPT_RECORDINGS_LEGACY` in `isSystemManagedContentPath`): a prod straggler created _after_ the backfill with correct `origin: "transcripts"` metadata still landed in the user namespace, so the relocation hook isn't yet airtight — the guard keeps such misses out of Recents. See docs/files/transcript-recordings-system-relocation.md.
...
- **2026-05-16** — Phase 2 frontend landing for Python's `UnifiedMediaBlock` rollout (Python commit `96f7ff7b` on `main`, not deployed yet at the time of writing — see [docs/PYTHON_UPDATES.md](../../docs/archive/2026/PYTHON_UPDATES.md)). Once deployed, Python emits the canonical media shape directly via a new `data: media_block` stream event covering image / video / audio / document / youtube. Frontend changes shipped this PR:
...
- **2026-04-23** — Phase 8 kickoff + API sprint. **Bug fix:** setState-in-render in `useOneShotSelection` (PageShell was dispatching during `useMemo` which triggered subscribers to setState mid-render). Switched to `useEffect` + ref guard — see [surfaces/PageShell.tsx](components/surfaces/PageShell.tsx). **Infrastructure:** Mounted `<CloudFilesPickerHost />` in [app/Providers.tsx](../../app/Providers.tsx) — `openFilePicker()` / `openFolderPicker()` / `openSaveAs()` now callable from anywhere. **New API:** `createFolder` / `deleteFolder` / `ensureFolderPath` thunks use direct Postgres metadata writes. [hooks/useUploadAndGet.ts](hooks/useUploadAndGet.ts) provides the recommended "File → fileId" flow and accepts either `parentFolderId` or an auto-created `folderPath`. The temporary compatibility shim was deleted after all consumers moved to the canonical Files handler. **First consumer migrated:** [features/audio/services/audioFallbackUpload.ts](../../features/audio/services/audioFallbackUpload.ts). All typecheck clean.
- **2026-04-23** — Phases 9–11 complete. The Python team finished their side and deleted all legacy backend code, which meant Phase 10's "run side-by-side soak" was no longer possible — we compressed it into Phase 11 and flipped everything at once.
  ...
  **Legacy deletion (Phase 11).** Removed from the repo:
  ...
  - Individual files: the former quick-files providers, storage reducers, compatibility adapters, deprecated podcast asset route, and legacy file-operation utilities.
