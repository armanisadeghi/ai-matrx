...
> **Cross-repo system-of-record — the file BACKEND is now a standalone service:** `/Users/armanisadeghi/code/common-docs/matrx-files-service/FEATURE.md`. The aidream file layer was extracted into the independent `matrx-files` microservice (deployed to EC2 2026-07-13). Browser and server clients route the standalone service's exact `/files`, `/assets`, and `/share` surface through `NEXT_PUBLIC_FILES_URL`; file-adjacent routes that still need parity remain on aidream. Read the system-of-record before any file-backend-facing change.
