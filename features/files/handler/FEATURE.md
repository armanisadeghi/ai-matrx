...
- **2026-08-08 — matrx-files FileRecord contract-drift compensation.** The live files service returns `{file_id, folder_id, version}` where its published contract says `{id, parent_folder_id, current_version}`, and `/files/{id}/url` omits `expires_in`. That cost every cold metadata hydration its `fileId` → no signed-URL mint → every `InlineMediaRef` rendered a broken icon. `apiFileRecordToCloudFile` now reads both shapes and emits a loud CONTRACT VIOLATION console error on the drifted one; `mintSignedUrl` derives expiry from the AWS `Expires=` param when the field is missing; `useFileAs` logs resolution failures instead of swallowing them into silent fallback icons. Service-side conformance fix is a spawned task — delete the shim when it lands.
