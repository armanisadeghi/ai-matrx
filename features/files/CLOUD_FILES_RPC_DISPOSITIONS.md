...
| `hard_delete_file` | backend (matrx-utils, matrx-ai) | SERVER | **SERVER — never FE-direct.** ⚠️ The RPC returns the S3 URIs to purge **only to a service-role caller** (`auth.uid() IS NULL`); a JWT/browser caller gets `{deleted, versions_deleted}` with **no URIs**. So an FE-direct hard delete would drop the rows and leave the browser with *nothing to purge* — an unrecoverable orphan leak (the C10 bug). Hard delete MUST stay on the Python server, which calls the RPC as service-role and routes through `SyncEngine.hard_delete_and_purge_async` (purges main + versions). This is why the FE POSTs `DELETE /files/{id}?hard_delete=true`. Do NOT adopt this RPC in the FE. |
