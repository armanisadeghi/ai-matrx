...
- `2026-09-18` — **Bugbot MEDIUM on `eb641aee` (follow-up to the `would_*` fix above): the marketing block's "no rows" footer no longer contradicts a preview it just showed.** `UnmodelledPreviews` (added by the fix directly below) can now print a `would_*` change even when a marketing payload carries none of the read-branch fields `substantiveRead` checked — `verdict`, health flags, `checks`, `containers`, `data` — so a write-preview-only payload fell through and the card ended with "This read returned no rows for the window above" directly under the change it had just shown: claiming nothing came back in the same breath as a preview. Fixed the class with ONE predicate, `hasSubstantiveContent(claim, otherwiseSubstantive)` in `google-result-shared.tsx`: true when the block's own read-branch union is true, OR when `claim.state !== "none"` (any preview, approval hold, unreadable/contradictory hold, or receipt). `GoogleMarketingResultBlock`'s `substantiveRead` now calls it; censused the workspace block for an analogous "empty" sentence and found none, so it needed no change. Proven red-then-green in `kind-google-result-families.test.tsx` against `eb641aee`'s bytes: the write-preview-only marketing fixture now also asserts the "no rows" sentence is absent; the existing truly-empty fixture still asserts it is present.
...
- `2026-09-18` — **F-95 (two Bugbot findings on F-90's `68a6cf3f`, both fixed the class): the write-claim key family and `approval` are now ONE exported constant, `WRITE_CLAIM_KEYS` in [`google-result-shared.tsx`](../../components/mardown-display/blocks/google-kinds/google-result-shared.tsx), spread into both blocks' `PROMOTED` lists so a `dry_run`/`awaiting_approval`/`appended`/`written`/`created`/`imported`/`sent`/`approval` value `NothingWasWritten` already summarizes never ALSO prints as a raw, unlabeled `MetaStrip`/`LeftoverFields` leftover — MEDIUM, `GoogleMarketingResultBlock`'s `PROMOTED` omitted the whole family; `GoogleWorkspaceResultBlock` already hand-listed the same keys and now reads the shared constant instead, so the list and the reader can never drift again. The open-ended `would_*` preview keys (which cannot live in a static array) are now merged into `omit` at render time from `claim.previewKeys`. Second, LOW: the marketing footer's "This read returned no rows" sentence ignored a `tracking_health` answer — a payload carrying `verdict` and/or `has_ga4`/`has_conversion_tag`/`has_consent` with an empty or absent `checks` still printed "no rows" under a real verdict; the predicate now also requires no verdict and no health flag. Red-then-green in `kind-google-result-families.test.tsx` (census assertion that both `PROMOTED` lists carry every `WRITE_CLAIM_KEYS` entry, a would_append+awaiting_approval+approval fixture asserting each fact renders exactly once via occurrence counting, and the two footer-predicate fixtures) against the pre-fix `9e3c597f` bytes. No screen was seen.
...
- `2026-09-18` — **F-109 follow-up: an empty answer is empty at every depth.** Cursor Bugbot (review 5247021300) found the F-109 guard fixing four spellings rather than the class: `isSubstantiveValue` asked only whether a record had KEYS, so `data: { rows: [] }` — the same empty GA4 read with the wrapper the tool actually sends, and the shape this suite's own marketing fixtures use — counted as content and the V-24 NEW-2 empty-read sentence never fired on it; only the flat `rows: []` / `data: []` spellings were caught. The predicate in [`google-result-shared.tsx`](../../components/mardown-display/blocks/google-kinds/google-result-shared.tsx) is now recursive: a record is substantive only when one of its own values is, an array only when one of its elements is, a number or boolean always is and a blank string never is — to a bounded depth of 8, where the answer deliberately flips to "substantive" because a payload nested that deep is never the empty read this predicate exists to recognise and the safe reading is the one that does not claim nothing came back. `hasSubstantiveContent`, the `value.data` check and the residual pass all inherit it from the ONE place, and the frame sentence prints for a nested empty read exactly as for a flat one ("This read returned no rows for example.com, 2026-09-01 to 2026-09-15."). Guard: five more cases in [`kind-google-result-families.test.tsx`](../content-ir/__tests__/kind-google-result-families.test.tsx) ("V-24 NEW-2 (nested)") — `data:{rows:[]}` and `data:{rows:[],totals:{}}` are empty reads, `data:{rows:[{sessions:1}]}` and `data:{summary:{sessions:1234}}` are not, and the predicate itself is asserted directly to depth 3 including a blank string at the bottom. Three red on `8e612aa2`, all 134 green after.
- `2026-09-18` — **F-109: an explicit `null` on a Google write claim is a stated value, and a read's own request parameters are the frame, not the answer.** Two findings from the hostile verifier V-24, both in the Google result cards, both the honesty class. (1) V-24 NEW-1: F-104's ONE unreadable-claim rule excluded `null` by hand (`value[key] !== undefined && value[key] !== null`), so `{action:"append_document", title:"Q3 Plan", appended:null}` — the value a Python tool produces most easily when it declares a key it could not settle (`None`) — fell through to `none`, and because `appended` is a `WRITE_CLAIM_KEYS` member both cards omit from the meta strip AND the leftovers, the key left the screen entirely: the card rendered "Q3 Plan Append document" and nothing about whether the write happened. That is V-23 NEW-4's outcome at a different value. The presence test now lives once, in `statesClaim` in [`google-result-shared.tsx`](../../components/mardown-display/blocks/google-kinds/google-result-shared.tsx) — an ABSENT key states nothing, an explicit `null` is a stated value this card cannot read — and `readWriteClaim` reads the whole family through it, so `appended: null`, `dry_run: null` and every sibling now get the same "states \"appended\" as null rather than true or false … Run it again." lead the `"yes"` case already got. The truth table also learned that the two halves of the unreadable state do not rank the same: an unreadable COMPLETION marker asks the very question nothing else can answer, so it outranks everything but a contradiction; an unreadable HOLD flag ranks BELOW a settled `awaiting_approval: true` (a definite answer is never downgraded to "cannot be read") and above a preview. (2) V-24 NEW-2: `hasSubstantiveContent` counted `printsResidualFacts`, and the residual pass prints the QUESTION — `site`, `start_date`, `end_date` — as facts, so `{action:"traffic_summary", site:"example.com", start_date:…, end_date:…, rows:[]}`, exactly what a GA4 read with no rows returns, printed the question back and never said no rows came; only a payload carrying nothing but `action` reached the sentence. The request's own parameters are now the FRAME (`REQUEST_FRAME_KEYS` / `statedFrame` / `frameSentence`, same file): withheld from the strip that prints answers, printed instead as the window they name ("asked about site example.com start date 2026-09-01 end date 2026-09-15"), and named in the sentence — "This read returned no rows for example.com, 2026-09-01 to 2026-09-15." `metrics` / `dimensions` are deliberately NOT frame keys, because on a GA4 payload either can carry the numbers themselves. An empty collection is not content either (`isSubstantiveValue`), so `rows: []` and `data: []` no longer read as an answer, and the residual pass moved INSIDE `hasSubstantiveContent` so no caller can hand it the wrong one. Guard: nineteen new cases in [`kind-google-result-families.test.tsx`](../content-ir/__tests__/kind-google-result-families.test.tsx) ("V-24 NEW-1" / "V-24 NEW-2" describe blocks) — fifteen red on the pre-fix bytes, all 129 green after, with the other-direction cases (a definite approval hold still leads beside a null `dry_run`; one real row inside the same frame is not an empty read; a non-parameter number is still content) asserted from both sides.
- `2026-09-18` — **Three Cursor Bugbot findings on `5d6bd755` fixed in `GoogleWorkspaceResultBlock.tsx`.** (1) `sent` was in `WRITE_CLAIM_KEYS` (so already withheld from the leftover strip) but the chip row only ever printed a chip for `sent === false` ("not sent") — a `sent: true` receipt left the screen with no word the send happened. Now gated under the same `claim.showsReceiptChips` test as `created`/`appended`/`written`/`imported`; `sent === false` stays ungated, since a negation is always honest. (2) `GenericWritePreview` opens the Person through `RecordDoor type="party" id={preview.person_id}`, but `person_id` was missing from the `shown` set that keeps the raw `rest` dump from repeating what the door already opened — the same uuid printed twice, once as the door's target and once unlabeled underneath it. `person_id` is now in `shown`; census of every other `RecordDoor` in this file (the calendar-event door, the terminal Person section, the imported-tasks door) confirmed no sibling miss — each already withholds the id it opens. (3) The workspace card's completeness chip predicate and mount call read only `value.truncated`, never `value.completeness`, while `GoogleMarketingResultBlock` already passes both to the ONE shared `statesCompleteness`/`TruncationChip` implementation and lists `completeness` in its `PROMOTED` — so a workspace payload stating `completeness: "bounded_preview"` with no `truncated` key got no chip, and `completeness` itself leaked into the leftover strip. Now the workspace card passes both values through and `completeness` is in `PROMOTED`. Guard: five new cases in `kind-google-result-families.test.tsx` ("Bugbot findings on 5d6bd755" describe block), red on the pre-fix component, green after.
...
- `2026-09-18` — **F-99: a queued Google write is no longer reported as a failed one, and ONE module owns the words.** Cursor Bugbot found `sendRowsToSheetOutcome` ([`components/agent-copy/useExportActions.ts`](../../components/agent-copy/useExportActions.ts)) answering the server's `proposed` reply — the organization reviews Google writes first, so the change was filed in the approval queue and NOTHING was written — with `status: "error"`, code `google_sheet_proposed`, so the copy/export host (`AiCopyMenu`, `CopyButtons`, `ExportMenu`) treated a successful queue filing as a failed send. The three sibling call sites (`messageActionRegistry`, `contentActionRegistry`, `SendToGoogleSheetButton`) each carried their own hand-written copy of the same toast and did NOT mark an error — four copies of one sentence, one of which had already drifted. Fixed as a class: the new [`features/google-workspace/export/proposedWrite.ts`](export/proposedWrite.ts) owns the sentence (`NOTHING_CHANGED_YET`), the door label and the toast, and all four sites now call `isProposedGoogleWrite` / `announceProposedGoogleWrite`. The outcome the Alchemy host gets is `success` + `delivered: "action"` — which the kit defines as "an explicit page action completed", never a persisted destination write — with the message stating nothing is in Google yet and the `target` receipt pointing at the approval row, so the menu shows a real door instead of a failure. 🚨 STILL OPEN: `TransferOutcome` in `@ai-matrx/kit` 0.15.5 has NO member for "accepted for review, nothing delivered yet" (success | degraded | cancelled | error). The first-class state belongs IN the kit, plus a `@ai-matrx/design-system` release teaching `outcomeMessage` to render it; this container has no npm credentials, so that release was NOT made and is handed back rather than faked here. Guard: [`components/agent-copy/useExportActions.test.ts`](../../components/agent-copy/useExportActions.test.ts) — a `proposed` reply is never `status: "error"`, never says "Created", carries the queue href, and is announced through the shared toast; a real refusal and a missing connection are still errors. Red on the pre-fix bytes, green on the fix.
- `2026-09-18` — **F-99: an ABSENT completeness key is not a verdict (the Google result cards).** `TruncationChip` was mounted unconditionally on every `google_marketing_result` card, so with `truncated` absent `readBool` returned `null` and the card printed "completeness unknown — the provider does not say" — an invented verdict, fired on every read with no window to cap (tracking health, a Tag Manager container list, a YouTube channel) and on any payload that simply omitted the key. The declared kind says `truncated: bool | None` ("null when unknowable"), so an explicit `null` IS a verdict and an absent key is not. The distinction now lives once, in `statesCompleteness` beside the chip in [`google-result-shared.tsx`](../../components/mardown-display/blocks/google-kinds/google-result-shared.tsx): the chip renders nothing when nothing was stated, and `GoogleWorkspaceResultBlock` frames its `ChipRow` through the same predicate instead of its own `!== undefined` test, so the two cards cannot drift. `CountedFact`'s `bounds` is deliberately NOT the same class and now says so in code: `bounds` is the WINDOW itself, not a verdict about the read, so "window not stated by the provider" is equally true absent or null (V-22 / NEW-10). Guard: five new cases in `kind-google-result-families.test.tsx`, red on the pre-fix component.
