...
- `app/api/webhooks/twilio/test-handset/route.ts` — inbound SMS for the two loopback TEST HANDSETS (harness only; no conversation, no agent turn, no consent adjudication).
- `app/api/webhooks/twilio/test-handset-voice/route.ts` — inbound calls to a test handset: record and hang up.
- `lib/sms/test-handset-inbox.ts` — the durable store behind both, including the test-handset-only filter.
...
5. Aidream resolves `sms.owner_beta` fresh for the exact user and organization on every admitted turn, runs the resolved Holder against the reserved canonical chat conversation with its complete authored tool set, atomically enqueues the reply, and delivers it through the durable outbound worker. Every tool the Holder carries runs over SMS exactly as it runs in web chat — no approval suspension, no re-authentication (Arman, 2026-09-22: *"Absolutely no verifications is the RULE for this text service"*).
...
- **Transport never edits the Holder's tools.** SMS adds only its database-owned channel context. 🚨 **The text service never verifies.** No tool call over SMS is suspended for approval, and nothing on this path ever asks the person to confirm, re-authenticate, enter a code or follow a link — the verified phone→user binding IS the identity, and a signed-in person opening a door we texted them is already themselves (Arman, 2026-09-22). The 2026-08-18 exact-action approval gate (side-effect classes, the `/open/chat` approval door, the confirm endpoint with its "recent sign-in" check, the email-OTP re-authentication in the card) is being deleted on both sides: it gated `github_repositories action=list` — a read — and its 401 sent a Supabase magic link, not a code, so the person could never approve. The feature's purpose is the opposite of a roadblock: letting a person hand us a password or 2FA for a site the vault does not hold yet.
...
- 🚨 **A person's timezone is LEARNED, and the browser is the only one that already knows it.** 749 of 766 people on the platform carry no timezone on any rung, so `communication.sms_notification_gate` judges their quiet hours in UTC — a text held back in the middle of their afternoon, or landing in the middle of their night. The browser reports `Intl.DateTimeFormat().resolvedOptions().timeZone` at two moments — the once-per-browser-session capture in `components/layout/AuthSessionWatcher.tsx` (fired from the branch that also covers `INITIAL_SESSION`, because a session restored from cookies never fires `SIGNED_IN`) and a successful enrolment in `features/sms/hooks/useSmsEnrollment.ts` — and BOTH go through the one route `app/api/person/timezone/route.ts` to the one door, `communication.record_person_timezone` (aidream migration 1015). **Never add a `timezone` field to the `/api/sms/preferences` PUT instead:** that column is NOT NULL with a default of `America/New_York`, so a write there cannot express "only when nothing is known" and would overwrite a timezone the person actually declared. The door decides for itself whether to write, so no caller pre-checks, compares or retries; the user id comes from the verified session and never from the body; and the capture is best-effort — it never blocks sign-in, never raises a toast, and is skipped in silence when no organization has been chosen (calling anyway would open the org picker on top of a fresh sign-in, to answer a question nobody asked).
...
- `2026-09-22` — THE TEXT SERVICE NEVER VERIFIES (Arman). `SmsActionAuthorizationCard` lost its email-OTP re-authentication branch: a 401 from the confirm endpoint is reported as a server-side defect with the one honest remedy, never answered with a magic link. The approval gate itself (aidream: side-effect classification, approval text, confirm endpoint, confirm/consume RPCs, recent-AMR check) is deleted in the backend lane; once the backend no longer emits `execution_authorization`, the card, the `sms_action_authorization` ask kind and the `/open/chat` approval-door copy go too.
