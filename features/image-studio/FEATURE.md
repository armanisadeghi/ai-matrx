...
1. User drops/pastes images into `StudioDropZone`. Client creates `StudioSourceFile` entries with `objectUrl` previews **and immediately auto-saves each ORIGINAL to the user's library** under `Images/Edited/Sources` (private) via `useImageStudio.persistSourceFile` — a genuine user upload that surfaces in "my files" / Recents. This is deliberately a **user-namespace** folder (NOT the `Images/Generated` tree the variants save to, which `isExcludedFromRecents` hides). Provenance tag `source: "image-studio-source"`; per-file lifecycle on `StudioSourceFile.sourceUploadStatus` (`idle`→`uploading`→`saved`/`error`), surfaced by the `SourceSaveIndicator` on the card. The upload is non-blocking and **loud on failure** — `persistSourceFile` inspects `uploadFiles`' `result.uploaded`/`result.failed` (a resolved thunk does NOT mean the bytes landed) and shows "Not saved" rather than a false "Saved".
...
handler, preserves the target file's `organization_id`, then uploads to the
existing logical path; it never falls back to a folder upload or transfers
ownership to the ambient workspace.**
...
- **2026-08-31** — Image version saves resolve the existing logical path and
  organization from canonical `file_id` before upload; an unresolved target
  fails loudly instead of creating a sibling, and ambient workspace context
  cannot transfer ownership of the existing file.
...
- **2026-07-12** — **Killed the S3-signed-URL leak + state-destroying navigation in the variant tiles; routed download/share/view through canonical file primitives.** The preview API returns large variants as an ephemeral 5-minute `signed_url` (S3), which landed verbatim in `ProcessedVariant.dataUrl` → leaked into the tile `<img src>`, and a download `<a href={s3url} download>` is cross-origin so the browser ignored `download` and **navigated the tab to S3**, wiping the studio's in-memory state (back-button → blank studio). Fixes:
  ...
  - **Canonical tile** (`StudioVariantTile.tsx`) — saved variants render via `<InlineMediaRef ref={{ file_id }}>` (re-mints, CDN-aware) instead of a raw `<img>`; **Copy link** routes through `useFileActions(fileId)` (`setVisibility("public")` + `copyShareUrl()` → permanent CDN URL, verified `cdn.matrxserver.com/...?v=…`, never S3); **Open** goes to the canonical `/files/f/{id}` viewer in a **new tab** (never the raw URL, never same-tab). Removed the old `fileHandler.as({html_src})` copy path.
  - **Nav-away audit** — ExportPanel "Open folder" now opens in a new tab too. No studio action navigates the current tab; the only state loss left is deliberate sidebar navigation.
