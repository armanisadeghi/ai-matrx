...
2. **Declare the manifest.** Spread baseline values where they apply, append surface-specific values.
   ...
   ```ts
   import type { SurfaceManifest, SurfaceValue, SurfaceScopePayload } from "@/features/tool-registry/surfaces/types";
...
- `features/tool-registry/surfaces/types.ts` — type definitions.
- `features/tool-registry/surfaces/services/manifest-sync.service.ts` — diff + sync logic.
- `features/tool-registry/surfaces/utils/value-mapping-resolver.ts` — runtime resolver.
- `features/scope-system/FEATURE.md` — multi-scope binding contract.
