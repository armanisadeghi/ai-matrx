 ...
 * progressively enhances the row (id enables the admin click-through link,
...
interface Props {
  ...
  /** Dialog title node, e.g. <>Add a tool to <span className="font-mono">…</span></>. */
  title: React.ReactNode;
  /** Loads the candidate list each time the dialog opens. */
  loadTools: () => Promise<ToolSearchOption[]>;
  /**
   * Called once per add. Resolve to mark the row "added" and drop it from
   * the list; throw to keep it (the error message is toasted).
   */
  onAdd: (tool: ToolSearchOption) => Promise<void> | void;
  /** Per-row action button label. Defaults to "Add". */
  addLabel?: string;
  /** Shown when loadTools returns an empty list. */
  emptyText?: string;
}

/**
 * Generalized searchable multi-add tool picker — search by name / category /
 * description / id, grouped by category, one-click add per row with busy and
 * "added" states. Lifted from the executor-surfaces AddToolBindingDialog so
 * every "pick tools from the catalog" admin flow shares one dialog.
 */
export function ToolSearchDialog({
  ...
  const [tools, setTools] = useState<ToolSearchOption[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [query, setQuery] = useState("");
  const [showInactive, setShowInactive] = useState(false);
  const [adding, setAdding] = useState<string | null>(null);
  /** Tool names successfully added in this dialog session. */
  const [addedNames, setAddedNames] = useState<Set<string>>(new Set());

  // Loaded once per open. A ref keeps the effect independent of the
  // loadTools closure's identity (callers pass inline functions).
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, ToolSearchOption[]>();
  ...
  const handleAdd = async (tool: ToolSearchOption) => {
    ...
    try {
      await onAdd(tool);
      setTools((cur) => cur.filter((t) => t.name !== tool.name));
      setAddedNames((cur) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && !adding && onOpenChange(o)}>
        ...
        <div className="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden -mx-6 px-6">
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="py-8 text-center text-xs text-muted-foreground">
              {tools.length === 0 ? emptyText : "No tools match your search."}
            </div>
          )}
            ...
            <div className="divide-y divide-border">
              {grouped.map(([category, items]) => (
                <div key={category}>
                  ...
                  <div className="divide-y divide-border">
                    {items.map((tool) => {
                      const isAddingThis = adding === tool.name;
                      const wasAdded = addedNames.has(tool.name);
