...
// it (`EducationHome`) and read at agent-trigger time by the Education Hub's
...
// page load's DB round-trips (plan + mastery + goals + streak). `EducationHome`
// already loads exactly this data for its own block ranking, and simply hands
// the derived result over.
//
...
/**
 * Null until the Education home has loaded AND found real study signal — which
 * is exactly the condition under which the Study-today block renders. Anonymous and brand-new
