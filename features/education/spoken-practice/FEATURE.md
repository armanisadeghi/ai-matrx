...
**DEDICATED, MODE-AWARE mandates** (keys in `mandates.ts`; the DB decides which agent fulfils each — swap at `/mandates`). Spoken Practice does NOT reuse the FastFire flashcard grader or the flashcard batch-review lane — those are tuned for card drills and leaked tool-narration + "flashcard" framing into the user-facing oral-exam review (adversarial-review GAP 1, fixed 2026-07-14):

- `education.spoken_practice_design` (Session Designer). Designs the grounded prompt set (oral exam / interview / debate).
- `education.spoken_practice_design_language` (Language Practice Designer). The dedicated designer for the **`pronunciation`** mode: emits the SAME plan shape, but each `prompt` embeds the exact target-language phrase to say (guillemets + English gloss) and `reference_answer` is the clean expected utterance. Consumed by `coercePracticePlan` unchanged.
- `education.spoken_practice_grade` (Spoken Practice Grader). Grades one spoken answer for the three content modes; the mode rides as FACTUAL data on the first line of `rubric` (persona framing + the never-say-"flashcard" rule live in the DB agent definition); output is the unified `SpokenGrade`/`GradeVerdict` shape (consumed by `coerceSpokenGrade` unchanged).
- `education.spoken_practice_grade_pronunciation` (Pronunciation Grader). The dedicated grader for the **`pronunciation`** mode: scores BOTH content correctness AND pronunciation/fluency, emitting the same `SpokenGrade` shape PLUS a `pronunciation` object `{ accuracy, fluency, intelligibility, prosody, notes }`. Honest granularity: our STT gives a transcript, not phoneme scores, so this is a HOLISTIC word/syllable-level judgement of the recording — never phoneme-perfect (the schema + UI say so). `coerceSpokenGrade` narrows the optional `pronunciation` field; it is null for every other mode.
- `education.spoken_practice_review` (Session Review). Mode-aware examiner/interviewer/debate-judge/language-coach review over the serialized transcript; **tools disabled** so it can never narrate DB discovery; output `{ summary, strengths[], weaknesses[] }` (the shape the summary renderer already consumes). Reused across all four modes.

---

## The four modes

One shared loop; the mode changes the persona, the rubric, the designer/grader mandate, and (for debate) whether prompts counter-attack. All configured in `constants.ts#MODE_CONFIG`.
...
features/education/spoken-practice/
  mandates.ts                       SPOKEN_PRACTICE_MANDATES (two designers + two graders + reviewer) + SPOKEN_PROMPT_ITEM_TYPE
...
- **2026-09-08** — `PracticeRunner` mounts `AnswerGradeBlock` (the `answer_grade` kind's component, shared with the voice test and audio review) for the per-prompt verdict; `PracticeSummary` mounts `BatchReviewBlock` for the examiner's `batch_review`. Both bespoke cards deleted; `PronunciationCard` stays as the runner's own extra.
- **2026-08-29** — The setup form opts into the shared `matrx-touch-targets` subtree contract, so Back, text fields, selectors, voice controls, and Start keep the 44px responsive touch floor while desktop density remains unchanged. A focused source contract locks the opt-in to `app/globals.css`.
- **2026-08-18** — all AI steps resolve through mandates (IC-1); UUID registry deleted. `agents.ts` → `mandates.ts` (`SPOKEN_PRACTICE_MANDATES`); `modeRubric` now prepends only FACTUAL mode data (persona framing and the never-say-"flashcard" rule moved into the DB grader agents behind the mandates).
- **2026-08-13** — **This surface exists now, and its setup form is agent-writable.** `route-to-surface.ts` had mapped `/education/practice-oral` → `matrx-user/education-practice-oral` all along, but no manifest existed, so every agent run here resolved a surface with no values, no targets and no intro. Authored it (`features/surfaces/manifests/education-practice-oral.manifest.ts` — 23 values in four groups) with the emitter on `SpokenPracticeSurface.tsx`, the one component spanning home → setup → runner → summary, and the setup form publishing into the new `setupSnapshot.ts` so `getScope` stays synchronous (the Surface Context window polls it every 400ms; an emitter that fetched would hammer the DB behind an idle-looking panel). **Write half: two `ask` targets, both pre-session.** `practice_mode` picks the mode; `practice_setup` fills the form in ONE composite `{focus?, difficulty?, count?, deck_id?, source_text?}` patch, validated in the new `setupWrites.ts` and applied through the SAME `setFocus`/`setDifficulty`/`setCount`/`setDeckId`/`setPasted` the learner's own typing uses — never a parallel path. Because the source kind gates the fields (deck grounding only in the modes whose `offersDeckGrounding` is true, and the paste box is hidden while a deck is chosen) the validator judges the RESULT of the patch rather than each field in isolation. **`COUNT_OPTIONS` was a local literal in `PracticeSetup.tsx` and is now `PROMPT_COUNT_OPTIONS` in the new `vocabulary.ts`**, alongside the icon-free half of `constants.ts` (`MODE_VOCABULARY`, `DIFFICULTY_OPTIONS`, the prompt bounds) — the manifest reads them to build its enum prose and the handler reads them to validate, so the picker the learner sees, the enum the agent is told about, and the check that runs cannot drift; `constants.ts` re-exports every name, so no existing importer changed. **Starting a session is NOT writable** — `start()` spends an agent run, opens the microphone and meters `education.spoken_practice` — so handlers register only while `phase === "idle"` and an agent on the runner or the summary is offered no write tool at all. Grades, transcripts, the pronunciation rollup and the examiner's review are emitted as read-only evidence, and `session_prompts` is the answer key, so it is `autoContext: false` and the intro forbids leaking it. Verified with real agent runs against the setup form; the mic-driven RUN was deliberately not exercised (headless Chromium has no microphone). **The code and runtime contract is the composite two-target design.** Four handlerless field-level rows remain in the shared mirror as stale DB-only drift; removing those inert rows still requires an explicit owner-approved deletion.
