...
**Known gap (flagged):** an assigned deck/quiz is not auto-shared to enrolled students, so a student may not yet be able to OPEN+study a private teacher resource (title falls back to "Untitled …"; the "Study" link needs read access). Assignment-confers-visibility is a follow-up (a sharing/RLS reach, not this task's assignments+analytics core). The member's own content hub is likewise empty for a teacher-org class today (`assoc_for_entity` is org-gated) — assignments are the member's real content channel.
