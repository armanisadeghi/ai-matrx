...
// approximation of what Tailwind would produce. Tailwind v4's automatic
// content detection scans the project tree for class candidates, and
// `fixture.html` sits inside it, so both the pre-fix and post-fix class sets
// it carries are picked up without a `@source` line.
...
async function main() {
  ...
  console.log(`[rule-row-squeeze] compiled ${result.css.length} bytes -> ${OUT}`);
}

main().catch((err) => {
  // eslint-disable-next-line no-console
  console.error("[rule-row-squeeze] CSS compile failed:", err);
  process.exit(1);
});
