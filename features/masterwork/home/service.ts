...
// platform's own operators), so this panel renders ONLY what a signed-in user
// can truly read: the mandate registry (agent.mandate, public rows) and each
// bound agent's definition row (version + when it last changed and by what
// tier). NEVER fabricate review activity — the gap is documented in
...
export async function fetchImprovementRows(): Promise<ImprovementRow[]> {
  const pins = await fetchMandatePins(MASTERWORK_MANDATE_KEYS);
