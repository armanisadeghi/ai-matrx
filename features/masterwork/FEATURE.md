...
- `2026-09-13` — **A triage rejoin happens for every run, not once per page (Bugbot MEDIUM).** `TriageDraftsDialog` reopens itself onto a sort still in flight on the durable spine, and the latch that stops it re-firing was set on the first auto-open and never cleared — so the dialog rejoined exactly ONCE in the life of the page. A later live sort (started in another tab, or on a fresh pointer after `reset` cleared the last one) stayed hidden with the Start button armed, and the Expert could pay to sort the same 336 drafts twice. The latch is now per RUN rather than per mount: it is cleared the moment `run.running` goes false, so the next live sort reopens in its turn, while the `open` guard still keeps the run already on screen from asking a second time. Test: `triage/__tests__/triage-rejoin-latch.test.tsx` drives the real dialog through running → settled and closed → a second running run and expects two `onOpenChange(true)` calls, plus a guard that a still-running run never asks twice; proven RED against the pre-fix component (received `[true]`, expected `[true, true]`). The same never-cleared `reopenedRef` shape is still live in `components/detail/IngestSourceDialog.tsx`, `BodyOfWorkDialog.tsx` and `ChatImportDialog.tsx` — out of scope for this branch, and the class to sweep next.
