...
- **The `tool_ui` runner (`features/tool-call-visualization/dynamic/`) is a SEPARATE, NARROWER DUPLICATE** with its own `buildToolRendererScope`/fetch/cache path. *That* is what hung (FOUND_DEFECTS D6, now re-scoped). It never had a working renderer. **DELETE it; consolidate tool-viz onto the agent-apps runtime.**
