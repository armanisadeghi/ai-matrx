...
function deriveName(file: ResultFileRef): string {
    if (file.file_name && file.file_name.trim().length > 0) return file.file_name.trim();
...
export const ResultFile: React.FC<ResultFileProps> = ({ file, density = "inline", className }) => {
        ...
        async (e: React.MouseEvent) => {
            ...
            if (fileId) {
                ...
                try {
                    ...
                    console.error("[ResultFile] owned-file download failed, falling back to direct URL", err);
    ...
    const metaParts = [typeLabel, formatFileSize(file.byte_size ?? null)].filter(Boolean) as string[];
    ...
    return (
        ...
        >
            <FileIcon fileName={name} mimeType={file.mime_type ?? null} size={iconSize} />
            <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-foreground" title={name}>
                    {name}
                </p>
                {metaParts.length > 0 ? (
                    <p className="truncate text-xs text-muted-foreground tabular-nums">{metaParts.join(" · ")}</p>
                ) : null}
            </div>
