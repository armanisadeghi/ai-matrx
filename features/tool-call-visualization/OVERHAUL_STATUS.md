...
- [x] **SEVENTEEN DB-loaded renderers** (`migrations/tool_ui_db_renderer_examples{,_2,_3,_4}.sql` + `_subtitles.sql` + the agent-authored `read_page` + `agent_call`, contract_version 2, surface `matrx-default/default`). Wave 4 (page-reading): `get_active_tab`, `find_text_on_page`, `get_page_text`. `read_page` was authored end-to-end by the Tool Renderer Author agent. — agent-authored code in `tool_ui`, NONE in the codebase, compiled at runtime via `compileSlotComponent`. Wave 1: `fs_list`, `shell_execute`, `memory`, `travel_get_weather`. Wave 2: `fs_read`, `data` (shape-tolerant — table for `{rows}` / record card for `{record}`), `travel_get_restaurants`, `travel_get_events`. Wave 3 (browser family): `navigate_active_tab`, `tabs`, `find`, `click_element`. The reference set proving "most renderers load from the DB." Each is **fully self-describing** — `tool_ui.display_name` drives the collapsed label AND `header_subtitle_code`/arg-fallback drives the subtitle (so `travel_get_weather` reads "Weather · Miami", `fs_list` reads "Directory · 5 items") with zero in-code work. **Verified live:** `memory` in a real chat (`2e287645`) + all twelve in the gallery (`/demos/tool-viz/result-fields` → "DB-loaded renderers"). Seeds idempotent (ON CONFLICT) + ledger-recorded. **Surface unified** (`db-renderer/surface.ts`): runtime fetch + admin authoring API both use `matrx-default/default`, so author → render is coherent.
