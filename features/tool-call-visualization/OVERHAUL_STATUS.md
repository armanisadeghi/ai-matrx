...
- [x] **EIGHT DB-loaded renderer examples** (`migrations/tool_ui_db_renderer_examples.sql` + `_2.sql`, contract_version 2, surface `matrx-default/default`) — agent-authored code in `tool_ui`, NONE in the codebase, compiled at runtime via `compileSlotComponent`. Wave 1: `fs_list` (list), `shell_execute` (terminal), `memory` (sparse status), `travel_get_weather` (rich visual). Wave 2: `fs_read` (file/code viewer), `data` (shape-tolerant — table for `{rows}` / record card for `{record}`), `travel_get_restaurants` + `travel_get_events` (list cards completing the travel family). The reference set proving "most renderers load from the DB." Each is **self-describing** — `tool_ui.display_name` drives the collapsed label too (via `useDbToolMeta`), so e.g. `travel_get_weather` reads "Weather" not "Travel Get Weather". **Verified live:** `memory` in a real chat (`2e287645`) + all eight in the gallery (`/demos/tool-viz/result-fields` → "DB-loaded renderers"). Seeds idempotent (ON CONFLICT) + ledger-recorded.
