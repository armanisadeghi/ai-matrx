...
| `knowledge_search` | Knowledge Search | ✅ | ✅ (=inline) | ✅ | stay-open; renamed from `rag_search` 2026-07-18 |
...
| `seo` | SEO | ✅ | ✅ | ✅ | stay-open; card chrome. ONE renderer for all 5 actions, dispatching on RESULT SHAPE (`renderers/seo/resolve.ts`): meta checks → the `features/marketing/seo/serp` SERP primitives, `keyword_data` → the `features/marketing/seo/keyword-research` KeywordMetrics primitives, `collect_rank` → the collection receipt. Unified 2026-07-25 |
