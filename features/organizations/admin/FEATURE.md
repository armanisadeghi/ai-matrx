...
- **2026-08-06** — Recovered from a stranded June branch and landed on main (the DB layer had been live-and-orphaned since 2026-06-27; the moved-table sweep had already repaired the RPC bodies to `iam.organization_member` + `users.profiles`). Fixed the notes round-trip: `org_admin_list_members` (and therefore `org_admin_get_member`) now returns `notes` (`migrations/org_admin_list_members_notes.sql`, applied + ledgered) — previously the controls form always rendered empty notes and a save could silently clear them. Service args conform to generated optional types (`?? undefined`; SQL defaults are null so omission ≡ null). All three routes + write path + audit trail verified live in-browser 2026-08-06.
