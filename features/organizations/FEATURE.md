...
- **Active-org enforcement (soft).** `appContextSlice` also holds `personal_organization_id` (the user's `is_personal` org, set once at hydration; never reset by `setOrganization`). Selectors: `selectEffectiveOrganizationId` (= explicit org ?? personal — used by the API/scope layer so every request carries a valid org) and `selectHasExplicitOrganization` (drives the soft-enforcement UI nudge). The active org is **persisted to `localStorage`** (`matrx:lastOrg`) by a central `StoreProvider` subscription and restored on boot. Bootstrap + the sanctioned UI switcher live in `lib/redux/thunks/activeOrgBootstrap.ts` (`bootstrapActiveOrganization` — dispatched by `features/shell/components/DeferredShellData.tsx`; `chooseActiveOrganization` — used by the header switcher). UI: the switcher lives in the user-menu dropdown (`UserMenuOrgSection.tsx`) so it costs no header space; `HeaderOrgIndicator.tsx` shows in the header ONLY as a red "Select organization" nudge while none is chosen (hidden once selected); the avatar (`UserMenuTrigger`) also rings red when no org is explicitly selected.
