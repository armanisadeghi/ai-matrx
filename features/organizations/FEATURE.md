...
- **A record that names its own organization may have it ADOPTED, never guessed.** `useAdoptRecordOrganization(organizationId, recordLabel)` is the ONE primitive for this: a route that loads a durable record carrying `organization_id` and then fires org-scoped requests calls it, holds its body until the answer is in, and stops the reload dead-end where every action died on "Select an organization before sending this request." (wall W3, 2026-09-10). It only acts when nothing is selected, gives a real restore a bounded head start (`waitForOrganizationAdmission` raced against 1.5s), PROVES the organization is readable by this user before adopting (an unreadable RLS row means "not your workspace" and the surface stays fail-closed), and ANNOUNCES the switch with a toast naming the workspace and the remedy. It is NOT a fallback ladder — nothing is invented; the record answered the question. Sibling routes that still need it are censused in `/Users/armanisadeghi/code/common-docs/projects/expert-book-challenge/REGISTER.md` (W3).
