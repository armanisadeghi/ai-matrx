...
- **Opening a record NEVER changes the person's working organization — it OFFERS the switch.** (Arman, 2026-09-23: "The permission is to the person, not the org. ALWAYS.") A page that opens ONE record renders it whatever organization is selected (or none), and when the record lives elsewhere shows `RecordOrganizationSwitchOffer` (`components/RecordOrganizationSwitchOffer.tsx`): one line naming the record's organization and a "Switch to <org>" button — the person's choice, one click. It renders nothing when the record is already in the selected organization, names none, or the person is not a member of it (a record shared from outside still opens). It replaced `useAdoptRecordOrganization` (deleted 2026-09-23), which silently wrote the record's organization into the global selection. First shipped on `/d/[renderId]`; used by the Rulebook doors, `/libraries/[id]`, the brand YouTube panel, and (2026-09-25) every surface where acting lands work or cost in an organization: `/workflows/[id]` (Run form), `/workflows/[id]/triggers` (a new schedule/address and its runs), `/data-v2/[tableId]` (New record), `/cms/[siteId]` (new pages). **Never on a surface that USES an agent or agent app** (`/chat/a/[agentId]`, `/agents/[id]/run`, `/agent-apps/[id]/run` — removed 2026-09-26, Arman: the agent's organization has nothing to do with where the person's chat or run lands; it lands in their working organization) — always passed the RECORD'S own `organization_id`, never the selection. Reads never need an organization client-side (`callApi` / `python-client` send a GET/HEAD with none selected naming none); writes and actions stay fail-closed.
