...
Organizations are the top-level multi-tenant scope in the app — every organization bundles members, projects, tasks, and resources, and a person belongs to as many as they choose (one is created for them at signup). Invitations are the email-based flow that brings a new user into an existing org or project.
...
- `app/(core)/organizations/page.tsx` — the **launcher**: rich cards for every organization the person belongs to (logo, role accent, members, created), search, create. The parent surface to the workspace; matches the OrgWorkspace aesthetic.
...
- `useOrganizationInvitations(orgId)` — pending invitations for an org; mount only for owner/admin callers
...
- `features/organizations/components/ContributeResourceSheet.tsx` — "make your own available": pick a kind → search your items → `grantOrgAvailability` (`grant_org_availability`, organization configuration — a share always names a person). Reads already-shared ids via `listOrgSharedIdsForTable`.
...
- **Canonical org-id resolution for WRITES — `ensureOrgId(orgId)` (`lib/organizations/personalOrg.ts`).** Every org-scoped insert/update/upsert stamps `organization_id` via `await ensureOrgId(orgId?)` — never a null/optional org on a NOT NULL column, never an ad-hoc selector at the write site. Resolution: (1) the explicit `orgId` the call site knows; (2) the selected organization via `getActiveOrgId()` (`lib/organizations/activeOrg.ts`), after joining the sync engine's boot; (3) otherwise the request is HELD — `ensureOrganizationContext()` (`lib/organization/organization-gate.ts`) shows the person their memberships, they set one, and the write proceeds. Nothing substitutes an organization. Server-side: `ensureOrgIdServer(client, orgId)` refuses with the `organization_required` envelope.
...
- `iam.organizations` — `id, name, abbreviation, slug (unique), description, logo_url, website, created_by, is_personal, settings, created_at, updated_at` (`is_personal`: see the live gap above). `abbreviation` is 2–3 uppercase ASCII letters, intentionally non-unique, derived from the organization's own name. RLS: members can SELECT; owners/admins can UPDATE; only owners DELETE.
...
2. `createOrganization()` in `service.ts`:
   ...
   - The RPC inserts the `iam.organizations` row and canonical
...
- **Abbreviation is compact identity, not a key.** It is always 2–3 uppercase ASCII letters and is deliberately not unique. Every organization starts with deterministic initials from its own name (`iam.derive_organization_abbreviation`) and owners/admins may edit them; the database trigger normalizes every insert/update path.
...
- **An organization is created for every person at signup.** The `on_auth_user_created` trigger on `auth.users` calls `public._provision_new_user_personal_org()` → `public.ensure_personal_organization(uuid)` → `public._d31_impl_ensure_personal_organization(uuid)` (idempotent). It is an ordinary organization under the law; the trigger does not block user creation on failure — failures land in `public.system_personal_org_failures` (super-admin readable).
...
- **Organization creation goes through `org_create` only.** Authenticated direct INSERT on `iam.organizations` is revoked; the RPC atomically creates the row and its first owner. The signup organization comes from the separate trigger flow above.
...
| -------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `members_can_add`    | **Live**           | `grant_org_availability` blocks non-admin members                                                                                                                                                                                                            |
| `requires_approval`  | **Live**           | `grant_org_availability` → grant `status = 'pending'`; surfaced in `OrgShareReviewCard` (Approve/Reject)                                                                                                                                                    |
| `default_permission` | **Live**           | `grant_org_availability` uses it when the caller omits the level (the Contribute flow does); pickers still pass explicit levels                                                                                                                             |
