...
- `features/organizations/components/OrgResourceRoleSection.tsx` — one content-role bucket as a labelled grid of count tiles. **This is the org home's Resources grid** and the source of the categorical accent language (`CONTENT_ROLES` dot + accent bar + tinted icon chip) that `AssociationCardGrid` now mirrors on the scope surfaces. Migration to the association edge is tracked in the canonical-associations `WORK-QUEUE.md`.
...
- `features/organizations/components/OrgWorkspace.tsx` — the workspace body, rendered by the primary `/organizations/[orgId]` page (and the `/org-2` alias). Its **"Resources" section is the role-bucketed tile grid** (`OrgResourceRoleSection` + `useOrgResourceInventory` over the `iam.permissions` sharing inventory), grouped by **content role** with categorical accents. The "resources" hero stat is the org's total incoming-edge count (`useContainerLinks().totalCount`). **The canonical `AssociationCardGrid` is deliberately NOT mounted here** (removed 2026-07-19) — it lives on the scope-type and scope pages, where attaching things to a container is the actual job. One resource surface per page; two competing grids on the org home was the confusion this removed.
...
- `features/marketing/competitors/OrgCompetitorLabelsSettings.tsx` — first
  ...
  manage both, permitted members add, and gated members request an exact label
  through Access Gate + internal DM.
...
- **Compute requests fail closed without an organization.** `appContextSlice` holds `personal_organization_id` only as the identity of the personal workspace; `callApi` never treats it as an implicit request organization. The transport requires `selectOrganizationId` or an entity-local `scopeOverrides.organization_id`, sends the same value in the body and `X-Organization-Id`, and rejects mismatches before networking. `selectEffectiveOrganizationId` (`organization_id ?? personal_organization_id`) is **DELETED** (2026-09-17, after every surface was converted); a converted surface either refuses the write with the remedy or shows an honest "no organization is selected" state. The primitives are strict too: `getActiveOrgId()` reads the selection only, and `ensureOrgId()` throws `OrganizationContextError` instead of reaching for the personal-org RPC. `awaitEffectiveOrganizationId` (`features/organizations/awaitWorkspace.ts`) waits for the EXPLICIT selection only — it never settles on the personal workspace — though its exported name still says "effective" until its two out-of-feature consumers are renamed with it. `orgBootstrapResolved`, `selectHasExplicitOrganization`, and `selectShouldPromptForOrganization` own the picker/reminder state. The **starred org is a DISPLAY preference only** — stored in user preferences (`organization.defaultOrganizationId`, synced to `user_preferences`), accessed via `features/organizations/hooks/useDefaultOrganization.ts`, and read by the org picker and nothing else: it draws that organization first and stars it. It does **not** restore the selection and must never be made to (Arman, 2026-09-19 — the default-org rung is deleted from `resolveActiveOrgContext`). Cross-session restore is this device's own remembered choice, the apex `matrx-active-org` cookie; on a browser that has never had one the person picks. The switch says "Keep it at the top" because that is what it does — until 2026-09-21 it said "Set as my default / Next startup opens X" and sent crew D2 hunting a persistence bug that was a sentence. There is **no `localStorage` last-org mechanism.** The sanctioned switcher lives in `lib/redux/thunks/activeOrgBootstrap.ts`: `chooseActiveOrganization` is the Surface-A switcher write; `bootstrapActiveOrganization()` is now only a thin back-compat imperative wrapper over the shared resolver (new code should not call it). UI: the in-menu switcher (`UserMenuOrgSection.tsx`) is always available, with a "Set as default" switch + Default-star badges; `HeaderChooseOrgButton.tsx` renders a "Choose org" control in the shell header's own flow while none is chosen (popover on desktop, Drawer on mobile; never floats over route chrome), and — in the one state it deliberately stays mounted for, the panel still open after a selection — it names the chosen workspace and drops the warning red instead of going on asking; the avatar (`UserMenuTrigger`) rings red while `selectShouldPromptForOrganization`. Canonical reusable pieces: `features/organizations/components/{OrganizationPickerPanel,DefaultOrgSwitch}.tsx`.
...
| Update other members' roles             |           yes           |  yes (members AND admins, never the owner)  |   no   |
| Remove members                          |           yes           |  yes (members AND admins, never the owner)  |   no   |
...
- **Every org has EXACTLY one owner.** `updateMemberRole` and `removeMember` block the last-owner case explicitly (pre-check + select-count of `role = 'owner'`), and since 2026-09-11 the database also refuses a SECOND owner (`migrations/iam_org_ownership_rulings_dd048.sql`). `leaveOrganization` just calls `removeMember(self)` so the same guards apply — a sole owner cannot leave their own org.
- **Changing an organization is gated on the OWNER **or ADMIN** MEMBERSHIP, not on `created_by`.** `org_update_policy` calls `iam.is_org_manager(id, auth.uid())` since 2026-09-11 (`migrations/iam_org_update_role_and_member_orphan_dd048_fix1.sql`). Before that it read `created_by = auth.uid()`, so after an ownership transfer the new owner could not rename the organization (0 rows, a raw `PGRST116` string on Save) and the previous owner still could. Admins are included deliberately — Arman, 2026-09-10: an admin "can act as an admin on things and make all kinds of changes to just about everything"; only TRANSFER and DELETE are owner-only. `updateOrganization` no longer uses `.single()`, so a zero-row RLS refusal becomes a sentence instead of `PGRST116`.
- **Deleting an organization is gated on the OWNER MEMBERSHIP, not on `created_by`.** Until 2026-09-11 `iam.organizations.org_delete_policy` read `created_by = auth.uid()`, so after an ownership transfer the PREVIOUS owner could still delete the organization and the new owner could not. The policy now calls `iam.is_org_owner(id, auth.uid())`. Because an RLS refusal is a zero-row no-op rather than an error, `deleteOrganization` asks for the deleted row back (`.select("id")`) and reports an honest sentence when nothing was deleted — it used to report success for a delete that never happened.
- **Nobody can be left with no organization — and that means every member, not just the person clicking.** `iam.is_last_organization(user, org)` backs four refusals with human sentences: leaving your last organization, being removed from someone's last organization (including by a super admin), deleting your last organization, and **deleting an organization that is some OTHER member's only one** (the refusal names how many members that is). The delete rules live in a `BEFORE DELETE` trigger, so they are real errors and not silent no-ops.
- **A hard DELETE of an organization can exceed the statement timeout.** `iam.organizations` is the target of 655 foreign keys, 240 of which have no leading index [live, 2026-09-11], so the cascade is expensive even for an empty organization. That is an indexing defect, not an access one — recorded in `FOUND_DEFECTS.md`.
...
- **Opening a record NEVER changes the person's working organization — and shows NO organization notice unless a mistake is possible.** (Arman, 2026-09-23: "The permission is to the person, not the org. ALWAYS." Arman, 2026-09-26: *"The Org notice should only be shown where a user might be unintentionally doing something under the wrong org. That's VERY VERY VERY RARE! If you cannot explain why the user might be making a mistake, then it doesn't belong there."*) A page that opens ONE record renders it whatever organization is selected (or none) and never writes the selection (the retired `useAdoptRecordOrganization`, deleted 2026-09-23, did). Editing the record stays on its own row; a CHILD of the record (a page in a site, a row in a table, a trigger on a workflow, a source/run/reminder under a Rulebook, a YouTube mirror under a brand) is written with the PARENT's explicit `organization_id` — never the selection — so there is nothing to warn about; using an agent, agent app or workflow run lands in the person's working organization by design. The old one-line "This X is in <Org> — Switch" offer (`RecordOrganizationSwitchOffer`) was therefore **deleted 2026-09-26**, final census zero: `/d/[renderId]` (frozen, read-only), `/cms/[siteId]` (page writes use the site's org, `app/api/cms/pages/route.ts`), `/workflows/[id]` (a run lands in the working org), `/workflows/[id]/triggers` (aidream stamps the workflow's org, `organization_of_parent`), `/libraries/[id]` (sync and Actions use `library.organization_id` server-side), the Rulebook doors (aidream `record_organization_id`; the capture-plan reminders now send the Rulebook's org via `scopeOverrides`), the brand YouTube panel (the refresh now carries the brand's org), `/data-v2/[tableId]` (store mounted for the table's org), and the agent surfaces (removed earlier the same day). A new notice needs the one-sentence explanation of how the person could misfile something that carrying the parent's organization cannot fix. Reads never need an organization client-side (`callApi` / `python-client` send a GET/HEAD with none selected naming none); writes and actions stay fail-closed.
- **NOTHING substitutes the selected organization, and a guard says so.** `pnpm check:organization-context` starts with `tsx scripts/check-org-fallback-shapes.ts`, a TypeScript-AST sweep of `actions/ app/ components/ features/ hooks/ lib/ providers/ utils/` (`actions/` and `providers/` joined on 2026-09-17 — nothing scanned the Server Actions in `actions/`, where two creates substituted the personal organization) that fails on: any reference to the deleted `selectEffectiveOrganizationId`; a `??` / `||` whose right operand is `personal_organization_id`, `personalOrgId`, `peekPersonalOrgId()`, `resolvePersonalOrgId()`, `resolveSystemOrgId()`, `resolveOrgIdForUserServer()` or `SYSTEM_ORGANIZATION_ID` feeding an `organization_id` / `organizationId` / `*OrgId` target — the SERVER family joined the list on 2026-09-17, together with `ensureOrgIdServer(client, <nothing / null / undefined / x ?? null / parent?.organization_id>)`, which returns the id it is given and otherwise resolves the personal organization, so it is a CARRY when it is handed a real one and a substitution when it is not; `.eq("organization_id", x ?? <one of those>)`; a STATEMENT-FORM substitution in ALL THREE of its shapes — assignment (`if (!orgId) orgId = await resolvePersonalOrgId()`), DECLARATION (`const organization_id = await resolveSystemOrgId()`) and PAYLOAD PROPERTY (`{ organization_id: await resolveSystemOrgId(client) }`) — none of which carries a `??` at all (the declaration and property arms were added 2026-09-17: rule 4 matched assignments only, so the commonest shape of all walked through it); a FIRST-MEMBERSHIP pick (`selected ?? orgs[0]?.id`, `organizations.at(0)`, `memberships.find(...)?.organization_id`) feeding an organization target; and `supabase.rpc("current_personal_org_id")` called outside `lib/organizations/personalOrg.ts`. Those last three are suspended inside the three primitives that legitimately resolve these organizations (`personalOrg.ts`, `systemOrg.ts`, `resolveActiveOrgContext.ts`) and nowhere else. A row that genuinely belongs to the person's OWN workspace — or to the platform itself — reads that organization by name and carries `// org-fallback-deliberate: <reason>` on the line or anywhere in the comment block directly above it (a fixed two-line window punished the honest case: the longer the justification, the further the marker was pushed from the code, so two correctly reasoned reads were reported as violations) — grep that marker for the census (today: the two notification-preference rows, where the personal-organization row IS the cross-organization default, hr_l3_116 §1). `--self-test` plants all ten shapes and proves the guard still fails on each, and passes compliant code. Law: `../../../common-docs/policies/context-is-carried-never-rebuilt.md`.
- **A write that OMITS the organization is the same substitution — a second guard asks that question.** `pnpm check:organization-context` runs `tsx scripts/check-org-insert-scope.ts` right after the fallback sweep. `public._stamp_org_default` is a BEFORE INSERT trigger on 328 tables that stamps the WRITER'S PERSONAL organization on any row arriving with a NULL `organization_id`, so `.insert({ title })` and `.insert({ title, organization_id: x ?? null })` file the row in a workspace nobody selected — an ABSENCE the fallback guard structurally cannot see. The guard derives the organization-scoped table set from `types/database.types.ts` (every generated `Row` carrying `organization_id`; 808 tables today), resolves each `.insert(` / `.upsert(` site's table from its `.schema(...)`/`.from(...)` chain — and, when the table name is BARE, from the schema its chain ROOT applies, followed to its definition across the import (`interviewDb(supabase).from("session")` is `interview.session`; `savedViewDb().from("saved_view")` is `platform.saved_view`) — and classifies the payload CARRIES / MISSING / NULLABLE / UNRESOLVED, following same-file variables, helper returns, `.map()` bodies and spreads. **A bare table is never re-homed into `public.` when a wrapper was involved**: an unfollowable wrapper is reported UNRESOLVED BY NAME, because `public.session` is a table the database does not have, so the site read as "not in types" and the ratchet swallowed it while all 53 live `interview.session` rows sat in their creator's personal organization (fixed 2026-09-17). **`app/api/cms/**` and `app/api/html-pages/**` are OUT OF SCOPE by declaration** — they write the separate CMS product database (Supabase project `viyklljfdhtidwecakwx`), which has no `organization_id` and no stamping trigger — and every run PRINTS how many writing files that covers rather than parking eight permanent entries in a ratchet meant to reach zero. **NULLABLE fails immediately and is never baselined**; MISSING and UNRESOLVED (anything the resolver cannot settle statically fails as UNRESOLVED — the realtime-publication rule) ride a ratcheting baseline, `scripts/org-insert-scope-baseline.json`, that ONLY SHRINKS: a NEW site fails by name, a STALE entry fails too, and `--fix` rewrites it downward but refuses to absorb growth. `--self-test` plants all eleven shapes plus the derivation and proves each still fails — including the two wrapper shapes and an unfollowable wrapper, asserted on the resolved TABLE and not only on the verdict. Law: `../../../common-docs/policies/context-is-carried-never-rebuilt.md` rule 4.
...
- **Cross-links:**
  ...
  - Cross-repo system-of-record for the full access model (ownership, `iam.permissions`, `iam.memberships` incl. org membership roles, `platform.associations` conveyance, admin level): [`common-docs/systems/platform/access/FEATURE.md`](/Users/armanisadeghi/code/common-docs/systems/platform/access/FEATURE.md)
...
**`auto_ingest` mapping caveat** (small follow-up): the aidream gate must resolve the router's `source_kind` to a canonical entity token before matching `org_module_settings.module_key`. It is fully functional for kinds already expressed as tokens. Aidream-internal source kinds (`cld_file`, `code_file`, `library_doc`, `scraped`, `repository`, `cx_message`, `project`) need an explicit source-kind → entity-token translation; never translate through a bare physical table name. An unresolved source kind currently falls through to default-ON, so this wiring remains visible follow-up work.

---

## Change log

- `2026-09-26` — **`RecordOrganizationSwitchOffer` deleted; the org notice is only for a real mistake (Arman's ruling, quoted in Invariants).** Every remaining call site audited: the record's own edits and its children already carry the record's organization (server-side for Rulebooks, triggers, Libraries, CMS pages, data-v2), so the offer was removed; two writes that still used the selection were fixed to carry the parent's organization — the brand YouTube refresh (`BrandChannelPanel`, brand's org) and the Rulebook capture-plan reminders (`capture-plan/service.ts` `scopeOverrides`, when the person is a member). Tests updated: `RulebookLaneRoute.organization.test.tsx` asserts no notice; the YouTube panel suite asserts the refresh carries the brand's org.
...
- `2026-09-20` — **The header org control stopped contradicting its own open
  panel.** `HeaderChooseOrgButton` stays mounted after a selection on purpose —
  the "Set as default" switch only enables once an org is active — and in that
  one state it went on reading "Choose org" in warning red with the chosen
  organization ticked in the panel underneath it (caught on screen by the
  Acquisition Console walk, 2026-09-20, `console-1440-light.png`). A control
  that contradicts its own panel is the same defect class as a dead-looking
  one. The label now follows the selection, the warning colour goes with the
  warning, and `aria-label`/`title` name the workspace. A chosen org whose
  name has not landed yet reads "Change workspace", never the old sentence.
  Guard proven failing-then-passing:
  `features/shell/components/header/header-right-menu/HeaderChooseOrgButton.test.tsx`.
...
- `2026-09-17` — **Both organization guards were reporting green over live personal-organization misfiles; a hostile verification proved it, and both now fail on that class.** (1) `check-org-insert-scope.ts` re-homed an imported schema wrapper's table into `public.`, so `interviewDb(supabase).from("session")` read as `public.session` — not in the generated types — and 13 baseline entries were "scope cannot be proved" rather than real verdicts. The chain ROOT is now followed to its definition across the import (`utils/supabase/*Db.ts`, `features/files/filesDb.ts`, same-file helpers such as `savedViewDb()`), a bare table is NEVER defaulted into `public.` when a wrapper was involved (an unfollowable one is UNRESOLVED **by the wrapper's name**), and the CMS routes are declared out of scope by name with their reason and counted on every run. The baseline went 56 → 40 with every removal accounted for: 10 CMS entries (separate product database), 2 saved-view entries that turned out to CARRY their organization all along, and `interview.session`, which was a real MISSING and is fixed. (2) `check-org-fallback-shapes.ts` could not see the SERVER resolvers at all — `ensureOrgIdServer(client, undefined/null/x ?? null/parent?.organization_id)` and `resolveOrgIdForUserServer()` — and its statement rule matched assignments only, never a declaration or a payload property; `actions/` was not even scanned. All three gaps are closed, the deliberate marker is now honoured anywhere in the comment block above the line (two correctly-reasoned reads were being reported as violations), and both self-tests were proven failing-then-passing against the pre-fix guard. (3) The 22 live sites the corrected guards exposed are fixed: `interview.session`, `app.definition` (create, duplicate, auto-create draft) and `agent.template` now carry `X-Organization-Id` (or `ensureOrgId`) and refuse with a remedy, every caller sends it and no click is dead; a DM and an SMS message take their PARENT conversation's organization and refuse when the parent cannot be read; feedback and quiz Server Actions take the organization as an argument, because a Server Action carries no header; profile, email, SMS preferences and consent, notification defaults and the pre-membership access request are explicit `org-fallback-deliberate` personal carries with their reason; and 32 platform-global writes (the AI catalog, the tool registry, the surface registry, admin email/tool/category/announcement writes, the platform's own error ledger, contact submissions, the admin SMS thread) are marked deliberate with the reason and their gate named.

- `2026-09-17` — **Two guards now ask the whole question, because the database was answering it.** An adversarial verification proved live that `public._stamp_org_default` is a BEFORE INSERT trigger on 328 tables which sets `organization_id` to the INSERTING USER'S PERSONAL organization whenever a row arrives with NULL — so on the client, omitting the key or sending `x ?? null` IS the personal-workspace substitution, and `check-org-fallback-shapes.ts` could see neither (it reads fallback EXPRESSIONS; this one is an ABSENCE). Seven shapes planted against the old guard walked straight through it. **New: `scripts/check-org-insert-scope.ts`** (second step of `pnpm check:organization-context`) — derives the 808 organization-scoped tables from `types/database.types.ts`, resolves every `.insert(`/`.upsert(` site's table and payload across `app/ components/ features/ hooks/ lib/ utils/ providers/`, and fails on NULLABLE immediately while ratcheting MISSING and UNRESOLVED through `scripts/org-insert-scope-baseline.json` (seeded at 60: 4 MISSING + 56 UNRESOLVED; 7 NULLABLE sites fail today and belong to the conversion lanes). **Extended: `check-org-fallback-shapes.ts`** with the three name-independent shapes it can express — statement-form substitution, the first-membership pick, and a raw `supabase.rpc("current_personal_org_id")` outside its one owner — suspended only inside `personalOrg.ts` / `systemOrg.ts` / `resolveActiveOrgContext.ts`. It found 8 live offenders on its first run (`app/api/_lib/apply-scope-to-insert.ts`, `app/api/admin/system-context/route.ts`, `app/api/cms/access-context/route.ts`, `app/api/messages/conversations/route.ts`, `features/admin/users/components/OrganizationsAdminClient.tsx`, `features/secrets/components/VaultWorkspace.tsx` ×3). Both self-tests plant every shape and prove the guards still fail on each.

- `2026-09-17` — **The personal organization stopped being a fallback, in the primitives.** `getActiveOrgId()` returns ONLY `appContext.organization_id` (the `?? personal_organization_id` rung is gone; `getSelectedOrgId` / `requireActiveOrgId` are now aliases of it and of `requireSelectedOrgId`), and `ensureOrgId()` resolves explicit → selected (after joining the store's bootstrap hydration) → **throws** `OrganizationContextError("Select an organization before sending this request.")`, with the loud personal-org RPC backstop and its `captureError` scream deleted. That backstop existed because boot could end with no selection at all; since 2026-09-12 boot is TOTAL (`resolveActiveOrgContext` rung b explicitly selects the person's own workspace), so an empty selection now means genuinely unresolved and the honest answer is a refusal every surface already renders. `selectEffectiveOrganizationId` is deleted from `appContextSlice` and from `scripts/check-access-guards.ts`'s active-org selector list. `resolvePersonalOrgId` / `peekPersonalOrgId` / `primePersonalOrgId` stay for the one question they answer — "which organization is this user's own workspace?". New guard `scripts/check-org-fallback-shapes.ts` (first step of `pnpm check:organization-context`, in CI) stops the shape growing back; `lib/organizations/__tests__/ensureOrgId.test.ts` pins the refusal and the selected-org path instead of the old TRANSITIONAL fallback cases.

- `2026-09-17` — **`awaitWorkspace` waits for the SELECTED organization.** `peekEffectiveOrganizationId` / `awaitEffectiveOrganizationId` read `selectOrganizationId` instead of `selectEffectiveOrganizationId`: a wait that settled with nothing used to answer "ready" with the user's personal workspace, so the action filed its work in an organization the person never chose. It now returns `unavailable` with the remedy, which every caller already renders. Behaviour change for `useIntakeSession`, `useProductCaptureSession`, `OutreachListCreateDialog` and `NewRulebookFlow`; the guard `__tests__/workspace-wait-not-a-dead-toast.test.ts` still pins both halves. The exported names keep the word "effective" until the two consumers outside this feature can be renamed in the same change.
...
- `2026-09-12` — **Cold boot now ends with an explicit organization whenever
  ...
  → stated default → own personal membership → sole membership. A user with
  several memberships and no default therefore no longer sees a personal-org
  header while org-scoped transports refuse every request. The new
  `useOrganizationRequired` gate distinguishes hydration from the settled
  no-membership state so readers wait during boot and show the canonical picker
  only when selection is genuinely unresolved.
...
- `2026-08-23` — **The org nudge moved into header flow; the floating card is
  ...
  on the keyword workbench (`/marketing/brands/[id]/sites/[id]/keywords`) it
  covered the date-range, compare and Columns controls; at 375px it overprinted
  the whole filter row. Replaced by
  `features/shell/components/header/header-right-menu/HeaderChooseOrgButton.tsx`:
  a "Choose org" control rendered inside `shell-header-right` (left of the
  Agents button), so it occupies header space and cannot overlap route chrome at
  any breakpoint. It opens the same `OrganizationPickerPanel` — a real popover
  anchored to itself on desktop, a Drawer on mobile — and renders `null` once an
  org is active. Gone with the old card: the auto-hide timer, the
  `motion/react` drop-in, the exit-window click-eater workaround, and its
  `DeferredSingletonCore` mount. The red avatar ring (`UserMenuTrigger`) is
  unchanged.

- `2026-08-24` — **The canonical Python transport no longer invents request
  organization context.** `callApi` now requires an explicitly selected org or
  entity-local scope override, refuses the personal-org fallback, sends the
  same normalized value in JSON and `X-Organization-Id`, and blocks
  body/query/header disagreement before any fetch. Wire tests prove a state
  containing only `personal_organization_id` performs zero network calls.

- `2026-08-17` — claude: **A starred default organization is now actually selected at startup.** Two independent defects produced the "no org selected" nudge beside a picker showing that org starred as default. (1) The sync engine snapshotted its identity at store creation and nothing ever advanced it, so a tab whose first render was anonymous — auth lands ~100ms later via `usePublicAuthSync`, and the client store is a module-level singleton that outlives that render — kept `guest:*` for its whole life; `appContextPolicy.remote.fetch` returns null for a guest, so the default-org preference was never read, on cold boot or on any 5-minute stale refresh. `makeStore` now watches `onIdentityChange` and calls the new `resyncForIdentity` (`lib/sync/engine/boot.ts`), which re-runs only the identity-scoped half of boot for the new identity. Every warm-cache policy gains this, not just the org. (2) `appContextPolicy`'s `remote.cacheSatisfies` counted a record holding only `personal_organization_id` as sufficient — precisely what an org-less boot persists — so the cached org-less record suppressed the very fetch that would have applied the default; sufficiency is now the active `organization_id`. Added the belt-and-braces `useDefaultOrganizationAutoSelect`, consumed by `useActiveOrganizationPicker`, which applies the stated default from Redux alone and logs loudly when it has to. Both halves of the swap are guarded: a late IDB record read for the OUTGOING identity is never dispatched over the incoming one's state, and boot's cold-boot fetches are skipped once the resync owns them. Regression tests: `lib/sync/__tests__/engine.boot.identitySwap.test.ts`. `pnpm type-check` clean; `lib/sync` + `features/organizations` + `lib/redux` suites green (174 tests).

- `2026-08-15` — **Organization Manage gained exact-addressed custom-value
  governance.** The Competitor labels section is visible to every member and is
  anchored as `org.competitors.custom-labels`. Owners/admins add/remove shared
  values and choose admin-only vs member additions. A member blocked by that
  policy sees the contextual setting-request flow rather than a disabled dead
  end. `platform.org_module_config` and `orgModuleSettings.ts` are the single
  storage/service path; canonical platform contract lives in
  `common-docs/systems/platform/setting-doors/FEATURE.md`.

- `2026-08-30` — **Shared organization/member copy controls now preserve 44px mobile targets, and member payloads include the HR employee relationship facts rendered in each organization row.** Linked, unlinked, and explicitly non-employee states flow through the neutral shared membership copy contract without coupling projects to HR. Invitation copy/export still deliberately omits bearer accept tokens; the separate human-only Copy Link action remains available.

- `2026-08-15` — **One user, one personal organization.** A guest→OAuth transfer had generically rewritten `iam.organizations.created_by` and `iam.memberships.user_id`, turning the guest workspace into a second personal workspace on an existing account. `guest_oauth_personal_org_merge.sql` re-homes guest organization-scoped rows into the permanent user's existing personal org, leaves the guest org/membership with the banned guest identity, excludes both machinery columns from future generic transfers, and adds the partial unique index `organizations_one_personal_per_creator`. `share_links_organization_fk_guest_repair.sql` closes the one affected non-FK edge (`platform.share_links.organization_id`) so the same generic transfer covers it too. Concurrent `ensure_personal_organization` calls now re-read the creator winner instead of mistaking that constraint for a slug collision.

- `2026-08-15` — claude: **Organization surfaces carry Copy / Copy-for-AI / export (agent-copy rollout), and the shared membership panels gained it for orgs AND projects at once.** New `features/organizations/lib/copy.ts` projects an org row to the same shape as the `organizations_summary` value the `matrx-user/organizations` surface already emits, so the copy payload and the agent surface describe one list. Wired: the `/organizations` launcher (header view pair + `ExportMenu`, plus an `xs` card pair carrying the card's rendered dimension/scope counts) and `OrganizationList` / `OrganizationCard` behind the settings tab (same pair + export; the card navigates on click, so `CopyButtons`' `stopPropagation` is what keeps copying from opening settings). **The members and invitations UI is shared with projects** (`components/membership/MembersPanel` / `InvitationsPanel`), so the payload builders live in the new `components/membership/copy.ts` and both panels gained an optional `copyContainer` prop — a roster is meaningless without saying whose members these are. `MembersPanel` falls back to its existing `containerNoun`, so a caller that passes nothing is unaffected; `MemberManagement` and both `InvitationManager`s now pass the real org/project identity. **Invitation payloads deliberately OMIT the accept `token`** — it is a bearer credential that grants membership to whoever holds it, and a payload bound for an agent's context window, a downloaded CSV or a pasted chat message must not carry one; the panel's own human-facing "Copy Link" action is untouched, and the new per-row pair renders for view-only viewers who previously had no copy path at all. Every list copy/export covers ALL rows; where a search box is active the envelope says so explicitly and still carries the full set, so a filtered view can never silently drop rows from a roster an admin is acting on. Peeks, `CreateOrgModal` and the resource sheets are deliberately skipped — composers are not records. **Consolidation debt noted, not fixed here:** `/organizations` re-implements its own `OrgCard` inline rather than using `features/organizations/components/OrganizationCard`, which is now reachable only from the settings tab, and neither uses the canonical `lib/entity-list/` shell. `pnpm type-check` clean.

- `2026-08-13` — **D158 module keys are token-only.** `org_module_settings.module_key`, its get/set RPCs, the frontend catalogue, and moderation hydration all use `shareable_resource_registry.resource_type`. Physical `schema.table` is resolved only at the query boundary; missing registry/token configuration raises instead of falling back to a bare table.
- `2026-08-10` — **The org profile is agent-writable.** `matrx-user/organizations` now declares 4 `applyPolicy:"ask"` / `mode:"entity"` write targets — `org_name`, `org_description` (`{text, mode?: 'replace' | 'append'}`), `org_abbreviation`, `org_website` — serviced by the new `components/OrgWorkspaceWriteTargets.tsx`, a renders-nothing component mounted inside `OrgWorkspace`'s existing `SurfaceRuntimeProvider` (`useSurfaceWriteHandlers`). Every handler goes through `updateOrganization`, the same service `GeneralSettings`' Save button calls — never a bespoke path, never raw supabase — and re-runs the same best-effort `invalidateAndRefetchFullContext()` + `ensureScopeTree({refresh:true})` refresh, handing the returned server row to `setOrganization` so the workspace and its emitted surface values reflect what actually persisted. Because `updateOrganization` swallows failures into `{success:false, error}` rather than throwing, each handler checks that result AND asserts the field on the returned row before reporting success, then throws — otherwise an agent would be told a rejected write landed. Permission is re-checked in the handler (owner/admin only, matching `canEdit` on the settings form), and `org_abbreviation` refuses on personal workspaces, which are always `ME`. `org_slug`, membership/roles, the logo, and scope configuration are deliberately NOT writable. The `/organizations` launcher registers no handlers — no org is open there. Live-verified with a real agent run against a throwaway org (three targets applied and confirmed by SQL, a decline left the row untouched, a bad value shape and an undeclared target both refused).
- `2026-07-27` — claude: **Peek registry converted 19× `React.lazy` → `next/dynamic({ssr:false})`** (`features/organizations/peek/registry.ts`; React.lazy is banned and compiled every peek's feature graph — agents, files, notes, canvas, … — into the server pass of the org resource routes via `ResourcePeekHost`). Per-kind chunking unchanged; `loading: () => null` matches the previous `Suspense fallback={null}` UX.
...
- `2026-08-17` — **A hollow appContext cache no longer starves org-scoped writes for 5 minutes.** The sync engine persists post-reducer state on every mutation, so any appContext change made before the first `remote.fetch` landed wrote an all-null org record to IDB — and because ANY cache hit suppressed the cold-boot fetch, every later boot read that hollow record as a hit and ran with no org until `staleAfter` (5 min), making every org-scoped write take `ensureOrgId`'s loud personal-org RPC fallback (observed live on `/education/flashcards/new`: two `[ensureOrgId] active org MISSING` screams 8s after page load, one per insert). Two independent layers, each sufficient: (1) **the choke point** — new generic `remote.cacheSatisfies(state)` on any sync policy (`lib/sync/types.ts`, validated in `policies/define.ts`, consumed by `scheduleColdBootFallbacks` in `engine/boot.ts`): a cached record that fails the test still rehydrates but reconciles IMMEDIATELY instead of at `staleAfter`, and a throwing predicate is logged and treated as insufficient. `appContextPolicy` declares sufficiency as "has an org (selected or personal)". (2) **the recovery repairs** — `ensureOrgId`'s fallback now dispatches `setPersonalOrganization` with the id the RPC returned, so the hole heals and the scream fires once per session instead of once per write; it never touches `organization_id`, so a later rehydrate or user switch still wins. Tests: `lib/sync/__tests__/engine.boot.idb.test.ts` (3 cases), `lib/organizations/__tests__/ensureOrgId.test.ts` (5 cases). No DB change.
- `2026-07-15` — claude: **Cross-links pointer added to `common-docs/systems/platform/access/FEATURE.md`** — the cross-repo system-of-record for the full access model (ownership, permissions, memberships, associations, admin level), verified live 2026-07-15.
...
- `2026-06-30` — **Active org is now a first-class sync-engine citizen + `ensureOrgId` is the single write-time resolver.** Hydration moved off the `ActiveOrgBootstrap` island (deleted) onto a `warm-cache` `appContextPolicy` (`appContextSlice.ts` + `lib/sync/registry.ts`): the org identity fields persist to IDB→LS keyed by identity and rehydrate through the shared sync boot, with `remote.fetch` → the new pure resolver `lib/organizations/resolveActiveOrgContext.ts` reconciling on cold-boot/stale and cross-tab broadcast on switch. **2026-08-15 correction:** sync boot now runs in the post-hydration layout phase; the former during-render boot could make persisted markup differ from SSR and caused React #418 globally. `bootstrapActiveOrganization()` is now a thin back-compat wrapper over that resolver; `AppShell` no longer renders the retired island. `ensureOrgId` (`personalOrg.ts`) now resolves **explicit → active org (`getActiveOrgId`) → LOUD personal-org fallback**: the last resort emits `console.error` + `captureError({ source: "org-resolution" })` (new source in `errorCaptureStore.ts`) into the Error Inspector, because reaching it means the sync engine failed to keep the org present before a write. Swept the codebase for org-scoped writes that re-derived the org ad-hoc and switched the real gaps to `ensureOrgId(undefined)`: `files/virtual-sources/adapters/notes.ts` (notes + folders were homeless — NO org at all), `flashcards/data/fcService.ts` (sync Redux-only `getOrgId` → async `ensureOrgId`), `data-tables/document-service.ts`, `page-extraction/services/run-from-draft.ts`, `skills/redux/skillsThunks.ts` (category create), `code/redux/codeEditHistoryFlush.ts`, `agents/.../instance-working-document.thunks.ts` (conversation org → active fallback), `messaging/*` + `useSupabaseMessaging.ts`, `podcasts/studio/runs/service.ts`. **Kept personal-pinned by contract:** `assignHomelessNotesToPersonalOrg`, `projectService.createProject`. No DB migration (the extended `get_ssr_shell_data` org payload is retained as the canonical whole-shell payload; the live active-org path is the sync policy). tsc + eslint clean.
- `2026-06-27` — **Org-admin user-management console added** (`features/organizations/admin/`, doc [`admin/FEATURE.md`](./admin/FEATURE.md)). New routes `/organizations/[orgId]/admin{,/users/[userId]{,/resources}}`: member roster with org-scoped metrics (file usage, org-scoped last activity, spend), per-member controls (budget / storage cap / tier / level — advisory in v1), suspend/reactivate, invite (reuses `InvitationManager`), remove, and **resource reassignment** scoped to the org (registry-driven, drift-tolerant). DB: additive `migrations/iam_org_member_governance.sql` (ledgered) — `iam.org_member_controls` overlay + `iam.org_admin_audit` (RLS default-deny, RPC-only) + 9 `public.org_admin_*` SECURITY-DEFINER RPCs gated by `public.is_org_admin`. Membership stays on `public.organization_members` (overlay keyed by `(org,user)`, migration-safe). "Manage users" entry added to `OrgManage` header. `db-types` patched (CLI blocked by env proxy); tsc clean on all touched files.
...
- `2026-09-11` — **DD-048 fix round 1 — the three findings independent verification raised.** Migration `migrations/iam_org_update_role_and_member_orphan_dd048_fix1.sql` (applied + ledgered). (1) The `created_by` class was only half closed: `org_update_policy` still read `created_by = auth.uid()`, so after the Transfer ownership button this lane shipped, the NEW owner could not rename the organization and the PREVIOUS owner still could — it now calls the new `iam.is_org_manager` (owner OR admin, per Arman's 2026-09-10 admin ruling), and `updateOrganization` reports a sentence instead of a raw `PGRST116`. (2) The last-organization rule only protected the deleter, so an owner could delete an organization that was another member's only one and orphan them — the delete guard now counts every member who would be left with none and names the number. (3) `not authorized to remove membership` / `not authorized to update membership role` became sentences that say who CAN act, and a delete that hits D307's statement timeout no longer surfaces the raw Postgres string. Also: the guard `pnpm check:org-ownership` had been leaking its fixtures into the shared database (34 organizations, 19 users, 50 memberships over four runs, teardown swallowing every failure). Every leaked row was swept (392 organizations before the leak, 426 during, 392 after), and the guard was rebuilt around a **fixed, named fixture cohort** — five users at five fixed `dd048-*@matrx-test.invalid` addresses and five organizations at five fixed `dd048-fx-*` slugs, created once and reset to a documented baseline at the start and end of every run. It creates nothing new on a second run, and `verifyCohortOnly()` asserts that exactly the ten declared fixture organizations exist, failing the run outright (never advisory) if there are more. Two consecutive runs left the footprint unchanged at 5 users / 10 organizations / 16 memberships. The cohort exists because a user or organization on this platform cannot actually be deleted over HTTP — see D307 and D309. Guard grew from 11 to 16 cases; three refusal assertions now check the sentence, not just that something errored.
- `2026-09-11` — **DD-048 / DD-044: organization ownership, deletion, leaving and admin management now match Arman's 2026-09-10 rulings (Doctrine R21).** Migration `migrations/iam_org_ownership_rulings_dd048.sql` (applied + ledgered). Two live defects closed: (1) organization delete was keyed on `created_by`, so after `transfer_organization_ownership` the PREVIOUS owner could still delete and the new owner could not — `org_delete_policy` now calls the new `iam.is_org_owner`; (2) leaving had no last-organization guard at all, so a sole member of their only organization could leave and be left with none — the new `iam.is_last_organization` backs refusals in `mbr_remove`, `admin_manage_organization_membership`, and a `BEFORE DELETE` trigger on `iam.organizations`, each with a human sentence. Also: exactly one owner per organization is now enforced (a deferrable constraint trigger on `iam.memberships` plus loud refusals in `mbr_update_role` and the super-admin door, both naming Transfer ownership); admins may now remove and demote other ADMINS (never the owner) in `mbr_remove` / `mbr_update_role`; `transfer_organization_ownership` demotes before it promotes and proves exactly one owner remains. Frontend: the members list drops "Make Owner" and gains **Transfer ownership…** with a consequence-stating confirmation (`components/membership/MembersPanel.tsx`, `MemberManagement.tsx`, new `transferOwnership` in `service.ts`); `deleteOrganization` no longer reports success for an RLS-refused delete; the admin portal's role pickers no longer offer Owner when someone already holds it. Proven RED-then-GREEN by the new `pnpm check:org-ownership` (11 cases; real throwaway users, real minted JWTs, real RPC and PostgREST calls) — 5 failing before, 11/11 after. **`is_personal` guards were deliberately left untouched** — retiring them is DD-045 P3.
