...
- `2026-07-06` — **Fixed header user-menu org switcher Redux wiring.** New `useActiveOrganizationPicker` hook consolidates reads (`selectOrganizationId` / `selectOrganizationName` / `selectShouldPromptForOrganization`) and writes (`chooseActiveOrganization`) with org list data from the canonical scope tree (`useScopeTree` + `ensureScopeTree`) instead of the standalone `useUserOrganizations` fetch. `UserMenuOrgSection` and `OrganizationPickerPanel` now share this hook; the menu accordion shows the active org name in its header (`MenuGroup.trailing`) and auto-expands when no org is chosen. `useUserOrganizations` now waits for `authReady` before fetching so early-mount callers don't permanently cache an empty list.
