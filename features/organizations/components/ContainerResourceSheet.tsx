...
export function ContainerResourceSheet({
  ...
  return (
    ...
    >
      {/* Keyed remount: every open (and any change of target) starts a fresh
          body — loading spinner, clean search, no stale rows — with all state
          derived from mount, so no effect ever sets state synchronously
          (react-hooks/set-state-in-effect). */}
      <SheetBody
        key={open ? `${entry.table ?? "none"}|${column}|${value}` : "closed"}
        active={open && Boolean(entry.table)}
        entry={entry}
        column={column}
        value={value}
      />
...
function SheetBody({
  ...
  const [loaded, setLoaded] = React.useState<Item[] | null>(null);
  const [query, setQuery] = React.useState("");
  const [peekId, setPeekId] = React.useState<string | null>(null);

  const items = loaded ?? [];
  const loading = active && loaded === null;

  React.useEffect(() => {
    if (!active || !entry.table) return undefined;
    let cancelled = false;
    (async () => {
