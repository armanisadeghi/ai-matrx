...
- **Featured image** (`featured_file_id`): drives the Q&A queue + thumbnails (falls back to first photo). Crop reuses the CANONICAL image-studio stack — `InitialCropWindow` (file_id → URL via `fileHandler.use(...).as({kind:"html_src"})` → File → crop → `uploadItemFile` → featured). Never rebuild a cropper; the scanner's `CropSheet`/`QuadEditor` are perspective tools with no crop consumer — wrong tool.
