...
export function transformAppWithApplets(rawConfig: AppAndAppletConfig): {
  ...
  const applets: CustomAppletConfig[] = rawApplets.map((applet) => {
    return {
      ...
      containers: Array.isArray(applet.containers)
        ? (applet.containers as AppletContainer[])
        : [],
      dataSourceConfig:
        (applet.data_source_config as AppletSourceConfig | null) ?? undefined,
      brokerMap: Array.isArray(applet.broker_map)
        ? (applet.broker_map as BrokerMapping[])
        : undefined,
