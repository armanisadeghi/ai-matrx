...
const slice = createSlice({ name: "sandboxLifecycle", initialState, reducers: {
  ...
  applyView: (state, action: PayloadAction<{ actorId: string; view: SandboxLifecycleView }>) => { if (state.actorId !== action.payload.actorId) return; const index = state.views.findIndex((view) => view.operation_id === action.payload.view.operation_id); if (index >= 0) state.views[index] = action.payload.view; else state.views.push(action.payload.view); },
  dismissView: (state, action: PayloadAction<{ actorId: string; operationId: string }>) => { if (state.actorId !== action.payload.actorId) return; const view = state.views.find((candidate) => candidate.operation_id === action.payload.operationId); if (view) view.dismissed = true; },
} });
