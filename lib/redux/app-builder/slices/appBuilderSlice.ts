...
import {
  ...
  type FetchAppByIdSuccessAction,
  saveAppThunk,
} from "../thunks/appBuilderThunks";
import type { AppBuilder, AppsState } from "../types";
...
export const appBuilderSlice = createSlice({
  ...
  reducers: {
    ...
    setApp: (state, action: PayloadAction<AppBuilder>) => {
      ...
      state.apps[id] = {
        ...
        isLocal: action.payload.isLocal || true,
        slugStatus: action.payload.slugStatus || "unchecked",
      };
    },
    updateApp: (
      state,
      action: PayloadAction<{ id: string; changes: Partial<AppBuilder> }>,
    ) => {
      const { id, changes } = action.payload;
      if (!checkAppExists(state, id)) return;

      const isSlugChanged =
        changes.slug && changes.slug !== state.apps[id].slug;
      state.apps[id] = {
        ...state.apps[id],
        ...changes,
        isDirty: true,
        slugStatus: isSlugChanged ? "unchecked" : state.apps[id].slugStatus,
      };
    },
    ...
    addApplet: (
      state,
      action: PayloadAction<{ appId: string; appletId: string }>,
    ) => {
    ...
    removeApplet: (
      ...
      state.apps[appId].appletIds = state.apps[appId].appletIds.filter(
        (id) => id !== appletId,
      );
      state.apps[appId].isDirty = true;
    },
    ...
    setDescription: (
      ...
      state.apps[id] = {
        ...state.apps[id],
        description: description ?? state.apps[id].description,
        isDirty: true,
      };
    },
    ...
    setMainAppIcon: (
      state,
      action: PayloadAction<{ id: string; mainAppIcon?: string }>,
    ) => {
    ...
    setMainAppSubmitIcon: (
      state,
      action: PayloadAction<{ id: string; mainAppSubmitIcon?: string }>,
    ) => {
    ...
    setPrimaryColor: (
      state,
      action: PayloadAction<{ id: string; primaryColor?: string }>,
    ) => {
    ...
    setAccentColor: (
      state,
      action: PayloadAction<{ id: string; accentColor?: string }>,
    ) => {
    ...
    setLayoutType: (
      state,
      action: PayloadAction<{ id: string; layoutType?: AppLayoutOptions }>,
    ) => {
    ...
    setAuthenticatedRead: (
      state,
      action: PayloadAction<{ id: string; authenticatedRead?: boolean }>,
    ) => {
    ...
    setPublicRead: (
      state,
      action: PayloadAction<{ id: string; publicRead?: boolean }>,
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(addAppletThunk.fulfilled, (state, action) => {
      ...
      if (
        state.apps[appId] &&
        !state.apps[appId].appletIds.includes(appletId)
      ) {
        state.apps[appId].appletIds.push(appletId);
      }
      state.isLoading = false;
    ...
    builder.addCase(fetchAppsThunk.fulfilled, (state, action) => {
      ...
      const newApps = action.payload.reduce(
        (acc, app) => {
          const id = app.id;
          if (id) {
            acc[id] = {
    ...
    builder.addCase(
      "appBuilder/fetchAppByIdSuccess",
      (state, action: FetchAppByIdSuccessAction) => {
        const { id } = action.payload;
        if (!id) {
          console.error(
            "fetchAppByIdSuccess: payload is missing id",
            action.payload,
          );
          return;
        }
