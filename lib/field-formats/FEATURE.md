...
- `2026-09-21` — **`attachment`**: files from the files feature on a row. The cell stores FILE IDS (an array), never a URL or a name, so a rename or a move in Files changes nothing and access is the file's own RLS; the text form is "1 file" / "3 files". `editor: "attachment"` (new `FieldEditorKind`), `rich`: drawn by `features/data-tables/components/AttachmentChips.tsx` (hydrates each id through `ensureCloudFileFields`, renders the files feature's ONE `FileChipList`) and edited by `AttachmentInput.tsx` (chips with remove + "Add files…" through the ONE file picker `openFilePicker`; `onDone` so the grid cell commits once). Wired in `FormatAwareInput` (row forms, row actions), `EditableCell` (grid) and `FormattedFieldValue`. Tests: `__tests__/attachment-format.test.ts`.
- `2026-09-21` — **The picker offers every KIND, not only the formats of the current storage type.** Arman: "Under datetime, I see show as but time is not an option." People think in kinds of data (a time, a currency), not storage types, so `FieldFormatPicker` with `onDataTypeChange` lists the formats that live on another base under "Other kinds · changes what the column stores" (each saying what it stores); picking one calls `onDataTypeChange(base, format)` instead of `onChange`, and the caller retypes the column and sets the format in one move. Wired in Add column, Table settings → Fields & Order (the type change resets the draft format, then the pick lands on top) and the new per-column dialog. Computed formats and the json/array bases are not offered as kinds to retype into.
