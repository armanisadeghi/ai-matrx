...
export const DOWNGRADE_RULES: DowngradeRule[] = [
  ...
  {
    id: "note-create-rejection-is-red",
    tier: "red",
    reason:
      "A rejected new-note thunk is a DEAD CLICK — the person pressed + and no note exists. On 2026-09-18 this sat as a minor orange dot while the tab-bar + did nothing for every multi-organization user whose Draft folder lived in another organization. The broad redux-rejected-handled rule below must never demote it.",
    addedAt: "2026-09-18",
    match: {
      source: "redux-rejected",
      relationPattern:
        "^notes/(initializeAutogeneratedNote|findOrCreateEmptyNote|createNewNote|moveNoteToNewFolder|copyNote)$",
    },
  },
  {
