...
> **Arman's ruling, 2026-08-14:** _"anything we can do on their behalf we'll do on
...
Because the same thing was being hand-rolled, worse, everywhere — see _Migration
map_ below. The version this replaced (the top of `SiteIntakeWizard`) was two
...
1. **THE TRUE-CURRENT-STATUS LAW.** Nothing machine-checkable is ever stamped.
   ...
   broke in April reads broken today, and says _"This was set up before and isn't
   any more."_ rather than silently reopening.
2. **Persist only what cannot be derived.** `state` holds human confirmations
   (with who and when), a record of auto steps we performed, and a _paint-fast_
...
| **Org onboarding**                             | Scattered: `features/scope-system/components/ScopeOnboarding.tsx`, `utils/onboarding.ts`, empty states                                                                                                                    | Members invited / first scope created / industry chosen = **verified**; the organization and default-scope creation we already do at signup = **auto**; "who else should be in here" = **confirmed**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
9. **An auto step claimed it was working while it was only CHECKING.** Both
   ...
   now means _this step's `run` is in flight_ and nothing else; locked by a test.
...
- 2026-08-29 — Outreach readiness now bypasses stale HTTP negatives after an
  operator saves a Vault key, and the `gmail.readonly` step names the real
  boundary: a separate restricted-scope campaign after the current ordinary
  Google review, not a permission that the current review can grant.
...
- 2026-08-15 — Fifth consumer: `outreach.production_bring_up` on
  ...
  step whose fact nobody on the surface can act on (`gmail.readonly`, a separate
  restricted campaign after the active ordinary review) is `optional` with the honest reason, not a required
