...
/**
 * GET /api/admin/mandates/agent-identity?agent_id=<id>
 * GET /api/admin/mandates/agent-identity?agent_version_id=<id>
 * GET /api/admin/mandates/agent-identity?agent_id=<id>&agent_version_id=<id>
...
export async function GET(request: Request) {
  try {
    ...
    // OR instead of agent_id. A version-pinned mandate passes both; dropping the
    ...
    if (versionIdParam) {
      ...
      if (versionError) {
        console.error(
          "[mandates/agent-identity] version lookup failed:",
          versionError,
        );
        return NextResponse.json(
    ...
    const { data: agent, error: agentError } = await adminClient
      ...
      .maybeSingle();
    if (agentError) {
      console.error("[mandates/agent-identity] agent lookup failed:", agentError);
      return NextResponse.json(
    ...
    if (agent.created_by) {
      try {
        ...
        console.error(
          "[mandates/agent-identity] owner lookup failed:",
          ownerError,
        );
      }
    }
    ...
    if (agent.agent_type !== "builtin") {
      ...
      if (derivedError) {
        console.error(
          "[mandates/agent-identity] twin lookup failed:",
          derivedError,
        );
      }
      if (derivedTwin) {
        ...
        if (sourceError) {
          console.error(
            "[mandates/agent-identity] source twin lookup failed:",
            sourceError,
          );
        }
        if (sourceTwin) {
    ...
    console.error("[mandates/agent-identity] unexpected failure:", error);
    return NextResponse.json(
      { error: "Failed to resolve the pinned agent" },
      { status: 500 },
    );
