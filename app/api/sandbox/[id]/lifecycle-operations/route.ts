...
import { proxyLifecycleReceipt, validLifecycleRequest } from "@/lib/sandbox/lifecycle-route-proxy";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (resolved.target.deletedAt) return NextResponse.json({ error: "Sandbox lifecycle target unavailable" }, { status: 404 });
