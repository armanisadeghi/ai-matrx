...
const SUB_PAGES = [
  ...
  },
  {
    ...
    description:
      "Env vars · Registry · Services · Event Log · Windows Features",
    badge: "New",
  },
] as const;
...
type QuickPreset =
  ...
  | {
      ...
      tool?: never;
      input?: never;
      sequence: { tool: string; input: Record<string, unknown> }[];
    };
...
export default function LocalToolsDemo() {
  ...
  return (
    <LocalToolsPageShell
      footer={
        <div className="border-t bg-card px-3 py-1 flex items-center gap-4 text-[10px] text-muted-foreground font-mono overflow-x-auto">
          <span className="flex items-center gap-1">
            <Activity className="w-3 h-3 shrink-0" />
            <span className="text-foreground/60">REST</span>
            <span className="truncate max-w-48">
              {local.baseUrl}/tools/invoke
            </span>
          </span>
              ...
              {QUICK_PRESETS.map((preset) => {
                if (preset.sequence) {
                  ...
                  return (
                    <Button
                      ...
                      title={
                        !allAvailable
                          ? `One or more tools (${tools.join(", ")}) are not available on this engine version`
                          : `Runs: ${tools.join(" → ")}`
                      }
                      onClick={async () => {
                        setActiveResult(null);
                ...
                return (
                  <Button
                    ...
                    title={
                      !available
                        ? `${preset.tool} is not available on this engine version`
                        : undefined
                    }
                ...
                >
                  {ALL_TOOLS.map((t) => (
                    <option
                      ...
                      value={t}
                      disabled={
                        availableTools.length > 0 && !availableTools.includes(t)
                      }
                    >
                      {t}
                      {availableTools.length > 0 && !availableTools.includes(t)
