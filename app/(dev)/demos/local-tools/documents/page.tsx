...
type ActiveTab =
  "notes" | "sync" | "versions" | "conflicts" | "local" | "shares" | "mappings";
...
export default function DocumentsPage() {
  ...
  const createFolder = useCallback(
    async (name: string, parentId?: string) => {
      setBusy(true);
      try {
        await restPost("/documents/folders", {
          name,
        ...
        setMsg({
          type: "error",
          text: err instanceof Error ? err.message : "Failed to create folder",
        });
      } finally {
        setBusy(false);
      }
  ...
  const createNote = async () => {
    ...
    try {
      ...
      setMsg({
        type: "error",
        text: err instanceof Error ? err.message : "Failed to create note",
      });
    } finally {
      setBusy(false);
    }
  ...
  const saveNote = async () => {
    ...
    setBusy(true);
    try {
      await restPut(`/documents/notes/${selectedNote.id}`, {
        title: editTitle,
        content: editContent,
      });
      await fetchNotes();
  ...
  const triggerSync = async () => {
    ...
    try {
      ...
      setMsg({
        type: "success",
        text: `Sync triggered: ${JSON.stringify(res)}`,
      });
  ...
  const pullChanges = async () => {
    ...
    try {
      ...
      setMsg({
        type: "success",
        text: `Pull complete: ${JSON.stringify(res)}`,
      });
    } catch (err) {
      setMsg({
        type: "error",
  ...
  const revertToVersion = async (noteId: string, versionId: string) => {
    setBusy(true);
    try {
      await restPost(`/documents/notes/${noteId}/revert`, {
  ...
  const resolveConflict = async (
    ...
    setBusy(true);
    try {
      await restPost(`/documents/conflicts/${conflictId}/resolve`, {
        resolution,
      });
      ...
      setMsg({
        type: "error",
        text: err instanceof Error ? err.message : "Resolve failed",
      });
    } finally {
      setBusy(false);
    }
  ...
  const fetchLocalFiles = async () => {
    try {
      const [foldersData, filesData] = await Promise.all([
        restGet(
          `/documents/local/folders${localPath ? `?path=${encodeURIComponent(localPath)}` : ""}`,
        ) as Promise<unknown[]>,
        restGet(
          `/documents/local/files${localPath ? `?path=${encodeURIComponent(localPath)}` : ""}`,
        ) as Promise<unknown[]>,
      ]);
                      ...
                      >
                        ...
                        {id &&
                          (deletingShareId === id ? (
                            <DeleteConfirm
                              label="Delete?"
                              onConfirm={() => deleteShare(id)}
                              onCancel={() => setDeletingShareId(null)}
                            />
                          ) : (
                            <Button
                              size="sm"
                      ...
                      >
                        ...
                        {id &&
                          (deletingMappingId === id ? (
                            <DeleteConfirm
                              label="Delete?"
                              onConfirm={() => deleteMapping(id)}
                              onCancel={() => setDeletingMappingId(null)}
                            />
                          ) : (
                            <Button
                              size="sm"
...
function MappingCreateForm({
  ...
  const create = async () => {
    setBusy(true);
    try {
      await restPost("/documents/mappings", {
