...
function parseCommand(
  ...
  if (trimmed === "ls -a")
    return { tool: "ListDirectory", input: { show_hidden: true } };
  ...
  if (trimmed === "help" || trimmed === "clear" || trimmed === "cancel")
    return null;
  ...
  if (trimmed.startsWith("cat "))
    return { tool: "Read", input: { file_path: trimmed.slice(4).trim() } };
  if (trimmed.startsWith("head "))
    return {
      tool: "Read",
      input: { file_path: trimmed.slice(5).trim(), limit: 20 },
    };

  if (trimmed.startsWith("find ")) {
    return {
      tool: "Glob",
      input: { pattern: trimmed.slice(5).trim(), path: "." },
    };
  }
  ...
  if (trimmed.startsWith("write ")) {
    ...
    if (spaceIdx === -1)
      return { tool: "Write", input: { file_path: rest, content: "" } };
    return {
      tool: "Write",
      input: {
        file_path: rest.slice(0, spaceIdx),
...
export default function TerminalPage() {
  ...
  const {
    invokeTool,
    cancelAll: ctxCancelAll,
    wsConnected,
    status,
  } = useMatrxLocalContext();
  ...
  const executeCommand = useCallback(
    async (raw: string) => {
      ...
      if (!connected) {
        addLine(
          "error",
          "Not connected to Matrx Local. Waiting for connection…",
        );
        return;
      }
  ...
  return (
    ...
    >
      <div
        className="flex-1 min-h-0 overflow-y-auto bg-zinc-950 text-zinc-100 font-mono text-sm p-4 cursor-text"
        onClick={focusInput}
      >
        {lines.length === 0 && (
          <div className="text-zinc-500 mb-2">
            Matrx Local Terminal — type &quot;help&quot; for commands,
            &quot;cancel&quot; or Ctrl+C to abort
          </div>
        )}
        ...
        {running && (
          <div className="flex items-center gap-2 text-yellow-500 mt-1">
            ...
            <span className="text-xs">
              Running… (type &quot;cancel&quot; or Ctrl+C to abort)
            </span>
          </div>
        )}
