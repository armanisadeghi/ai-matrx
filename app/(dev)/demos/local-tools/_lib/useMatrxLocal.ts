...
async function buildWsUrl(httpUrl: string): Promise<string> {
  ...
  if (!token) {
    console.warn(
      "[MatrxLocal] WS: no auth token — connection will be rejected",
    );
    return wsBase;
  }
...
export type DiscoveryResult = LocalEngineDiscovery;

/** @deprecated Use discoverAllLocalEngines from ./discoverLocalEngines */
export async function discoverMatrxLocal(): Promise<DiscoveryResult | null> {
  return discoverFirstLocalEngine();
}
...
export function useMatrxLocal(): UseMatrxLocalReturn {
  ...
  // Localhost engines found on live + dev port ranges
  const [discoveredEngines, setDiscoveredEngines] = useState<
  ...
  const addLog = useCallback(
    (direction: "sent" | "received", data: unknown, tool?: string) => {
      setLogs((prev) => [
        ...prev,
        {
  ...
  const applyDiscoveredEngine = useCallback((engine: LocalEngineDiscovery) => {
    setBaseUrl(engine.url);
    baseUrlRef.current = engine.url;
    if (engine.availableTools.length > 0)
      setAvailableTools(engine.availableTools);
    if (engine.version) setVersionInfo({ version: engine.version });
  }, []);
      ...
      try {
        ...
        if (options?.autoPick && isLocalUrl && !initialEnginePickRef.current) {
          ...
          if (currentMatch.version)
            setVersionInfo({ version: currentMatch.version });
        }

        if (engines.length > 0) {
          const signature = engines.map((e) => e.url).join("|");
        ...
        } else if (lastScanSignatureRef.current !== "") {
  ...
  const refreshHealth = useCallback(async () => {
    try {
      const [healthRes, portsRes] = await Promise.allSettled([
        fetch(`${baseUrlRef.current}/health`, {
          signal: AbortSignal.timeout(3000),
        }),
        fetch(`${baseUrlRef.current}/ports`, {
          signal: AbortSignal.timeout(3000),
        }),
      ]);
    ...
    ws.onclose = () => {
      ...
      if (wsAutoReconnect.current) {
        ...
        const delay = Math.min(
          2000 * Math.pow(2, wsRetryCountRef.current),
          30_000,
        );
  ...
  const selectEngine = useCallback(
    (url: string) => {
      const engine = discoveredEngines.find((e) => e.url === url);
      if (engine) {
        applyDiscoveredEngine(engine);
      } else {
        setBaseUrl(url);
    ...
    (
      ...
      return new Promise((resolve, reject) => {
        ...
        setActiveRequests((prev) => [
          ...prev,
          { id: reqId, tool, startedAt: new Date() },
        ]);
  ...
  const invokeTool = useCallback(
    async (
      ...
      const effectiveTimeout =
        timeoutMs ??
        (tool === "Research" ? WS_TIMEOUT_RESEARCH : WS_TIMEOUT_DEFAULT);
      const key = `${tool}-${Date.now()}`;
      setLoading(key);

      try {
        const result =
          useWebSocketRef.current && wsConnectedRef.current
  ...
  const restGet = useCallback(
    async (
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `GET ${path} → ${res.status}: ${body || res.statusText}`,
        );
      }
      return res.json();
    },
  ...
  const restPost = useCallback(
    async (
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `POST ${path} → ${res.status}: ${errBody || res.statusText}`,
        );
      }
      return res.json();
    },
  ...
  const restPut = useCallback(
    async (
      ...
      if (!res.ok) {
        const errBody = await res.text().catch(() => "");
        throw new Error(
          `PUT ${path} → ${res.status}: ${errBody || res.statusText}`,
        );
      }
      return res.json();
    },
  ...
  const restDelete = useCallback(
    async (
      ...
      if (!res.ok) {
        const errBody = await res.text().catch(() => "");
        throw new Error(
          `DELETE ${path} → ${res.status}: ${errBody || res.statusText}`,
        );
      }
      return res.json();
    },
