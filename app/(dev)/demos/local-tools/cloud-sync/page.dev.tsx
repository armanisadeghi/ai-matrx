...
export default function CloudSyncPage() {
  ...
  const configure = async () => {
    ...
    try {
      const res = (await restPost("/cloud/configure", {
        jwt,
        user_id: userId,
  ...
  const fetchSettings = async () => {
    ...
    setSettingsMsg(null);
    try {
      const data = (await restGet("/cloud/settings")) as Record<
        string,
        unknown
      >;
      setSettings(data);
  ...
  const syncAction = async (action: "push" | "pull") => {
    ...
    setSyncResult(null);
    try {
      const res = (await restPost(`/cloud/sync/${action}`)) as Record<
        string,
        unknown
      >;
      setSyncResult({
        type: "success",
        text: `${action} complete: ${JSON.stringify(res)}`,
      });
    } catch (err) {
      setSyncResult({
        type: "error",
        ...
        <div className="max-w-screen-2xl mx-auto space-y-4">
          <div className="border rounded-lg p-4 bg-card space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Send Supabase JWT and User ID to enable cloud sync. Without this,
              all sync operations return{" "}
