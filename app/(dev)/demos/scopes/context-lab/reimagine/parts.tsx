...
// Shared atoms for the reimagine picker set — fixed-geometry check targets
...
export function KindGlyph({ node }: { node: PickNode }) {
  ...
  if (node.kind === "item")
    return (
      <Hash className={cn("h-3.5 w-3.5 shrink-0", node.color?.fg ?? "text-muted-foreground")} />
    );
  if (node.kind === "type") {
    const Icon = resolveIcon(node.iconName);
    return (
      <Icon className={cn("h-3.5 w-3.5 shrink-0", node.color?.fg ?? "text-muted-foreground")} />
    );
  }
...
export function PickerFooter({
  ...
  // Live modes emit on every change — exactly what a Surface A host or a
  // filter bar would consume via onApplyActive/onSelectionChange.
  ...
  useEffect(() => {
    ...
    console.log(
      `[context-reimagine] ${mode.toUpperCase()} live emit →`,
      engine.nodes.map((n) => ({ kind: n.kind, id: n.id, label: n.label })),
    );
  }, [live, mode, engine.nodes]);
