...
// ContextTree — THE tree. Replaces the rejected flat quick-pick.
//
// Browsing is a real tree: indentation + chevrons, org → type → scope →
// field, with Projects and Tasks as the two bottom sections. Hierarchy is
// carried by STRUCTURE, never by path prefixes. Search is the one place the
// tree flattens (per the feedback: "search can flatten results").
//
// Interaction model (matches the approved Drill Deck convention):
//   • click a row        = expand / collapse (go deeper) — never selects
...
//
// Laziness: fields fetch on first scope expand; projects/tasks fetch on
// first section expand. NOTHING engagement-shaped is fetched on mount.
...
export function ContextTree({
  ...
  const rows = useMemo<TreeRow[]>(() => {
    ...
    if (q) {
        ...
        for (const t of o.scope_types) {
          ...
          for (const s of t.scopes) {
            ...
            for (const it of data.itemsByType[t.id] ?? []) {
              if (
                `${it.display_name} ${it.key}`.toLowerCase().includes(q)
              )
                out.push({
                  key: `item:${s.id}:${it.id}`,
                  depth: 0,
      ...
      if (q.length > 2 && !out.some((r) => r.kind === "scope")) {
        let n = 0;
        for (const o of data.organizations) {
          for (const t of o.scope_types) {
            if (n >= 4) break;
      ...
      for (const t of o.scope_types) {
          ...
          if (items) {
            out.push({
              ...
              add: {
                ...
                commit: (v) =>
                  fakeCreate("context item", v, { scope_type_id: t.id }),
              },
            });
          }
        }
        out.push({
          ...
          add: {
            ...
            commit: (v) =>
              fakeCreate("scope", v, { org_id: o.id, scope_type_id: t.id }),
          },
        });
      }
      out.push({
        ...
        add: {
          ...
          commit: (v) => fakeCreate("scope type", v, { org_id: o.id }),
        },
      });
    }
      ...
      if (data.tasksStatus === "ready")
        out.push({
          ...
          add: {
            ...
            commit: (v) => fakeCreate("task", v, {}),
          },
        });
    }
    return out;
  }, [data, expanded, q, query]);
  ...
  const doSelect = (r: TreeRow, commit: boolean) => {
    ...
    const next = toggleNode(selection, r.kind, r.id, mode);
    onChange(next);
              ...
              >
                ...
                >
                  {r.isCreate && !r.key.includes("err") ? (
                    <>
                      Create <span className="font-medium">&quot;{r.label}&quot;</span>
                    </>
                  ) : (
                    r.label
                  )}
                ...
                {selectable && (
                  <button
                    type="button"
                    aria-label={on ? `Deselect ${r.label}` : `Select ${r.label}`}
                    onClick={(e) => {
                      e.stopPropagation();
                      doSelect(r, true);
                    }}
