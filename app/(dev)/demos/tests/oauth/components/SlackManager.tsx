...
const SlackManager: React.FC<SlackManagerProps> = ({ tokenData }) => {
  ...
  const accessToken =
    "tokenData" in tokenData ? tokenData.access_token : tokenData.access_token;
  ...
  useEffect(() => {
    async function loadInitialData() {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load Slack data",
        );
  ...
  return (
    <div className="bg-white shadow rounded-lg overflow-hidden">
      <div className="p-4 bg-purple-600 text-white">
        ...
        {botInfo && <p className="text-sm opacity-80">Connected to Slack</p>}
      </div>
                      ...
                      {getUserAvatar(message.user) ? (
                        ...
                        <span className="text-xs font-medium text-gray-600">
                          {isBotMessage(message)
                            ? "Bot"
                            : getUserName(message.user).charAt(0)}
                        </span>
                      )}
                    </div>
                      ...
                      {message.files && message.files.length > 0 && (
                        <div className="mt-2">
                          {message.files.map((file) => (
                            <div
                              key={file.id}
                              className="mt-1 p-2 border-border rounded bg-gray-50"
                            >
