...
// Engine-reaching components MUST sit behind a client-only dynamic boundary so
// the mermaid engine (+ d3/cytoscape/dagre/elk) never enters the server graph
// of this route. The light helpers below (catalog, diagram-type, types) never
...
export default function MermaidPlaygroundPage() {
  ...
  const [source, setSource] = useState(
    MERMAID_CATALOG.flowchart.starterTemplate,
  );
    ...
    <div className="mx-auto max-w-6xl space-y-8 p-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Exercises the <code>components/mermaid</code> renderer, the forgiving
          sanitizer, and the streaming last-good behavior. Detected type:{" "}
      ...
      <section className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          The Expand icon in the header opens the diagram fullscreen (Esc to
          exit).
        </p>
      </section>
      ...
      <section className="space-y-2">
          ...
          <div className="ml-auto flex items-center gap-2 text-xs">
            <label className="flex items-center gap-1">
              ...
              <select
                value={theme}
                onChange={(e) =>
                  setTheme(e.target.value as MermaidRenderOptions["theme"])
                }
            ...
            <label className="flex items-center gap-1">
              ...
              <select
                value={look}
                onChange={(e) =>
                  setLook(e.target.value as MermaidRenderOptions["look"])
                }
                className="rounded border border-border bg-card px-1 py-0.5"
              >
        ...
        <p className="text-xs text-muted-foreground">
          Try the broken sample — the sanitizer auto-fixes the{" "}
          <code>-&gt;</code> arrows, the reserved <code>end</code> node, the
          unquoted <code>( )</code> label, and the <code>{"//"}</code> comment,
          and logs <code>[MermaidSanitize] RECOVERED</code> to the console.
        </p>
      </section>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-medium">
          Catalog ({featured.length} featured types)
        </h2>
    ...
    <section className="space-y-2">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <pre className="h-48 overflow-auto rounded-md border border-border bg-card p-3 font-mono text-xs text-muted-foreground">
          {streamed ||
            "Press Stream to feed the diagram in character-by-character"}
        </pre>
      ...
      <p className="text-xs text-muted-foreground">
        While streaming, the renderer keeps the last good diagram and never
        flashes an error on a half-arrived fence — it only renders once each
        partial validates.
      </p>
    </section>
  );
