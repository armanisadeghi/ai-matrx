...
/**
 * Layout for the mobile transcript studio. Seeds the session list once for the
 * whole section (persists across client navigations between the list, a
 * session, and the unsorted pool) and frames every child in the phone-width
 * column that stays usable on desktop. Per-session routing lives in the child
 * route segments so refresh / deep-link / back all work.
 */
export default async function MobileTranscriptionLayout({
  children,
}: {
  children: ReactNode;
}) {
