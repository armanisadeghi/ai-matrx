import { ImageResponse } from "next/og";

import { resolveShareToken } from "@/utils/permissions/shareLinks";
import { createClient } from "@/utils/supabase/server";

export const runtime = "nodejs";
export const alt = "AI Visibility Report by AI Matrx";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

export default async function SharedReportOgImage({
  params,
}: {
  params: Promise<{ token: string }>;
}) {
  const { token } = await params;
  const supabase = await createClient();
  const resolved = await resolveShareToken(token, supabase as never);
  const stored = resolved.resource?.["result"];
  const report =
    stored && typeof stored === "object" && !Array.isArray(stored)
      ? (stored as Record<string, unknown>)
      : {};
  const brand =
    typeof report.brand_name === "string" ? report.brand_name : "AI Visibility";
  const query =
    typeof report.query === "string"
      ? report.query
      : "See how answer engines recommend this brand";
  const providers = Array.isArray(report.providers)
    ? report.providers.filter(
        (value): value is Record<string, unknown> =>
          Boolean(value) && typeof value === "object" && !Array.isArray(value),
      )
    : [];
  const completed = providers.filter(
    (provider) => provider.status === "completed",
  );
  const mentions = completed.filter(
    (provider) => provider.target_mentioned === true,
  ).length;
  const ranks = completed
    .map((provider) => provider.target_recommendation_rank)
    .filter((value): value is number => typeof value === "number");
  const bestRank = ranks.length ? Math.min(...ranks) : null;

  return new ImageResponse(
    <div
      style={{
        display: "flex",
        width: "1200px",
        height: "630px",
        background:
          "linear-gradient(135deg, #071126 0%, #111b3c 55%, #31205f 100%)",
        color: "white",
        fontFamily: "system-ui, sans-serif",
        padding: "64px",
        position: "relative",
        overflow: "hidden",
      }}
    >
      <div
        style={{
          position: "absolute",
          width: "460px",
          height: "460px",
          borderRadius: "50%",
          background: "#6d5dfc",
          filter: "blur(100px)",
          opacity: 0.2,
          right: "-100px",
          top: "-120px",
        }}
      />
      <div
        style={{
          display: "flex",
          flexDirection: "column",
          width: "100%",
          justifyContent: "space-between",
          position: "relative",
        }}
      >
        <div
          style={{
            display: "flex",
            justifyContent: "space-between",
            alignItems: "center",
          }}
        >
          <div
            style={{
              display: "flex",
              border: "1px solid #786cf8",
              background: "#6d5dfc33",
              borderRadius: "999px",
              padding: "10px 20px",
              fontSize: "18px",
              fontWeight: 650,
            }}
          >
            AI Visibility Report
          </div>
          <div
            style={{
              display: "flex",
              alignItems: "center",
              gap: "10px",
              color: "#cbd5e1",
              fontSize: "20px",
              fontWeight: 650,
            }}
          >
            <div
              style={{
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                width: "36px",
                height: "36px",
                borderRadius: "10px",
                background: "linear-gradient(135deg,#3b82f6,#8b5cf6)",
                color: "white",
              }}
            >
              M
            </div>
            AI Matrx
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
          <div
            style={{
              fontSize: brand.length > 40 ? "46px" : "60px",
              fontWeight: 800,
              letterSpacing: "-0.04em",
              maxWidth: "940px",
              lineHeight: 1.05,
            }}
          >
            {brand}
          </div>
          <div
            style={{
              display: "flex",
              fontSize: "25px",
              color: "#b8c2d8",
              maxWidth: "970px",
              lineHeight: 1.35,
            }}
          >
            {`“${query.slice(0, 170)}”`}
          </div>
        </div>
        <div style={{ display: "flex", gap: "18px" }}>
          <div
            style={{
              display: "flex",
              flexDirection: "column",
              border: "1px solid #ffffff26",
              background: "#ffffff10",
              borderRadius: "16px",
              padding: "14px 22px",
              minWidth: "180px",
            }}
          >
            <span style={{ fontSize: "30px", fontWeight: 750 }}>
              {completed.length}/4
            </span>
            <span style={{ color: "#9aa7bf", fontSize: "16px" }}>
              engines checked
            </span>
          </div>
          <div
            style={{
              display: "flex",
              flexDirection: "column",
              border: "1px solid #ffffff26",
              background: "#ffffff10",
              borderRadius: "16px",
              padding: "14px 22px",
              minWidth: "180px",
            }}
          >
            <span style={{ fontSize: "30px", fontWeight: 750 }}>
              {mentions}
            </span>
            <span style={{ color: "#9aa7bf", fontSize: "16px" }}>
              brand mentions
            </span>
          </div>
          <div
            style={{
              display: "flex",
              flexDirection: "column",
              border: "1px solid #ffffff26",
              background: "#ffffff10",
              borderRadius: "16px",
              padding: "14px 22px",
              minWidth: "180px",
            }}
          >
            <span style={{ fontSize: "30px", fontWeight: 750 }}>
              {bestRank ? `#${bestRank}` : "—"}
            </span>
            <span style={{ color: "#9aa7bf", fontSize: "16px" }}>
              best position
            </span>
          </div>
        </div>
      </div>
    </div>,
    size,
  );
}
