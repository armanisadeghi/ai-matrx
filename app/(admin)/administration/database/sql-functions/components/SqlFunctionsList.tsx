...
export default function SqlFunctionsList({ functions, loading, selectedFunctionKey, onViewDetails, onEditFunction, onDeleteFunction }: SqlFunctionsListProps) {
  ...
  return <MatrxDataTable urlState={{ id: 'sql-functions' }} data={functions} columns={columns} getRowId={getSqlFunctionKey} isLoading={loading} pageSize={25} emptyState={{ title: 'No SQL functions found' }} rowClassName={(row) => selectedFunctionKey === getSqlFunctionKey(row) ? 'bg-primary/10' : undefined} onRowClick={(row) => onViewDetails(row)} toolbar={{ search: true, searchPlaceholder: 'Search loaded functions…' }} detail={{ enabled: false }} />;
}
