...
export default function SqlFunctionsList({ functions, loading, selectedFunctionKey, onViewDetails, onEditFunction, onDeleteFunction }: SqlFunctionsListProps) {
  ...
  return <MatrxDataTable urlState={{ id: 'sql-functions', defaultSort: { id: 'name', direction: 'asc' } }} data={functions} columns={columns} getRowId={getSqlFunctionKey} isLoading={loading} pageSize={25} hideToolbar emptyState={{ title: 'No SQL functions found' }} rowClassName={(row) => selectedFunctionKey === getSqlFunctionKey(row) ? 'bg-primary/10' : undefined} onRowOpen={(row) => onViewDetails(row)} rowActions={(row) => <><Button variant="ghost" size="icon" aria-label={`Edit ${row.name}`} onClick={() => onEditFunction(row)}><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Delete ${row.name}`} className="text-destructive" onClick={async () => { if (await confirm({ title: 'Delete SQL function?', description: `Delete ${row.schema}.${row.name}. This cannot be undone.`, confirmLabel: 'Delete', variant: 'destructive' })) await onDeleteFunction(row.schema, row.name, row.arguments); }}><Trash2 className="h-4 w-4" /></Button></>} detail={{ enabled: false }} />;
}
