...
export default function EnumsList({ enums, loading, onViewDetails, onEditEnum, onDeleteEnum }: EnumsListProps) {
 const columns = useMemo((): MatrxColumnDef<DatabaseEnum>[] => [
  ...
  { id: 'usage_count', header: 'Usage', accessorKey: 'usage_count', filter: 'number', width: 110, cell: (row) => <button type="button" className="rounded focus-visible:ring-2" onClick={() => onViewDetails(row, 'usage')}><Badge variant={row.usage_count > 0 ? 'default' : 'secondary'}>{row.usage_count} tables</Badge></button> },
 ...
 return <MatrxDataTable urlState={{ id: 'database-enums' }} data={enums} columns={columns} getRowId={(row) => `${row.schema}.${row.name}`} isLoading={loading} pageSize={25} emptyState={{ title: 'No enums found' }} onRowClick={(row) => onViewDetails(row)} toolbar={{ search: true, searchPlaceholder: 'Search loaded enums…' }} detail={{ enabled: false }} />;
}
