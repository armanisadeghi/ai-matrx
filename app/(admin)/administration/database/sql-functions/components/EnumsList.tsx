...
export default function EnumsList({ enums, loading, onViewDetails, onEditEnum, onDeleteEnum }: EnumsListProps) {
 const columns = useMemo((): MatrxColumnDef<DatabaseEnum>[] => [
  ...
  { id: 'usage_count', header: 'Usage', accessorFn: (row) => row.usage_count ?? 0, filter: 'number', width: 110, cell: (row) => { const usage = row.usage_count ?? 0; return <button type="button" className="rounded focus-visible:ring-2" onClick={() => onViewDetails(row, 'usage')}><Badge variant={usage > 0 ? 'default' : 'secondary'}>{usage} tables</Badge></button>; } },
  { id: 'actions', header: 'Actions', accessorFn: (row) => row.name, sortable: false, filter: false, width: 100, cell: (row) => <div className="flex justify-end gap-1"><Button variant="ghost" size="icon" aria-label={`Edit ${row.name}`} onClick={(event) => { event.stopPropagation(); onEditEnum(row); }}><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Delete ${row.name}`} className="text-destructive" onClick={async (event) => { event.stopPropagation(); if (await confirm({ title: 'Delete enum?', description: `Delete ${row.schema}.${row.name}. This cannot be undone.`, confirmLabel: 'Delete', variant: 'destructive' })) await onDeleteEnum(row.schema, row.name); }}><Trash2 className="h-4 w-4" /></Button></div> },
 ], [onDeleteEnum, onEditEnum, onViewDetails]);
 return <MatrxDataTable urlState={{ id: 'database-enums' }} data={enums} columns={columns} getRowId={(row) => `${row.schema}.${row.name}`} isLoading={loading} pageSize={25} emptyState={{ title: 'No enums found' }} onRowOpen={(row) => onViewDetails(row)} toolbar={{ search: true, searchPlaceholder: 'Search loaded enums…' }} detail={{ enabled: false }} />;
}
