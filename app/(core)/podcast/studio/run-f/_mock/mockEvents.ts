// app/(core)/podcast/studio/run-f/_mock/mockEvents.ts
//
// Self-contained replay script for the run-f "production booth" demo. Adapted
// from run-a's mockEvents — same real PodcastDataEvent shapes (the exact shapes
// the backend emits), re-timed to land in ~45s and fed through a local reducer
// (see ./reduce.ts) so the booth animates against genuine run state.
//
// The raw backend stage keys are intentionally ugly (prepare_content_researcher,
// generate_metadata…). This variant humanizes them in the UI via ./acts.ts —
// the mock keeps the real keys so the presentation could be promoted unchanged.

import type { PodcastDataEvent } from "./events";
...
// Stable placeholder media (fixed seeds so reruns look identical).
const img = (seed: string) => `https://picsum.photos/seed/${seed}/800/450`;

export const MOCK_RUN_ID = "demo-run-f";
