...
const resolveBlog = cache(async (slug: string) => {
    ...
    const { data: article } = await supabase
        .from('pc_articles')
        .select('*')
        .eq('episode_id', (episode as PcEpisodeWithShow).id)
        .eq('kind', 'blog')
        .eq('status', 'published')
        .maybeSingle();
    if (!article) return null;

    return { episode: episode as PcEpisodeWithShow, article: article as PcArticle };
});
...
export async function generateMetadata({
    ...
    const ogImage =
        ...
        DEFAULT_OG_IMAGE;
    const canonical = article.canonical_url ?? `${SITE_URL}/podcast/${episode.slug}/blog`;
