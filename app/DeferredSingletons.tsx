...
export default function DeferredSingletons() {
  ...
  // Hydrate entitlement state (like adminLevel), keyed on the user id so it also
  // refreshes on in-session login/logout (SPA nav, no reload) and clears on
  // logout. Cheap resolver RPC; fails soft to the free permissive snapshot.
  // Dynamic import keeps it out of this file's static graph.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      ...
      if (!user?.id) {
        dispatch(slice.clearEntitlements());
        return;
      }
      const snapshot = await fetchEntitlementSnapshot();
      if (!cancelled) dispatch(slice.setEntitlementSnapshot(snapshot));
    })();
    return () => {
      cancelled = true;
    };
