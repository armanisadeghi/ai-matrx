...
(async () => {
  ...
  await page.waitForTimeout(500);
  await shot(page, '06d-project-and-scope.png');
  ...
  await page.waitForTimeout(4000);
  await shot(page, '06e-after-save.png');
  const text = await page.textContent('body');
  log('6-save', /Search Sources/i.test(text) ? 'PASS' : 'INFO', 'saved, dialog closed check');
  fs.writeFileSync(path.join(OUT, 'results-6c.json'), JSON.stringify(results, null, 2));
  await context.close();
})();
