...
**Status:** live (P4, 2026-07-23; governance-tier probe added 2026-08-14). **Owner surface:** `pnpm check:access-matrix` · `pnpm check:access-drift` · `pnpm check:governance-tier` (all loud, non-blocking; `:strict` variants exit 1). Run them after ANY change to the access kernel, an RLS policy, a library trigger, or a deploy that touches Shared Knowledge.
...
- **`check-governance-tier.ts`** — the acceptance probe for THE GOVERNANCE-COLUMN TIER, the *column* axis of `viewer < editor < admin` (`common-docs/systems/access-architecture/FEATURE.md` §2.6; `migrations/iam_governance_column_tier.sql`; FOUND_DEFECTS D119). Creates a real `working_document` owned by `admin@admin.com`, grants `editor` to `test@test.com`, and PATCHes it **through PostgREST with a real minted JWT** — the exact path a sharee uses to walk around a UI-only check. Asserts both halves: an editor is refused `visibility` / `created_by` / an owned `organization_id` (42501 → HTTP 403), **and** can still rename, edit content and metadata, trash and un-trash; the owner and an `admin` grantee govern; `created_by` is refused at every level. Fixtures are deleted in a `finally`. An "allowed" failure is as serious as a "refused" one — a blocked legitimate editor is a defect (db-rules §6).
- **`lib.ts`** — env loading (same `.env*` scan as `check-migrations.ts`), service-key RPC calls, **real user JWT minting** (GoTrue admin `generate_link` → `verify`), PostgREST count probes, and `rlsPatch` — a true-RLS *write* probe that distinguishes a real DB refusal from a silent zero-row no-op (they mean very different things).
