...
**Status:** live (P4, 2026-07-23; governance-tier probe added 2026-08-14; guided-setup owner-write probe added 2026-08-15). **Owner surface:** `pnpm check:access-matrix` · `pnpm check:access-drift` · `pnpm check:governance-tier` · `pnpm check:guided-setup-rls` · `pnpm check:reachability-guards` (loud, non-blocking; `:strict` variants exit 1 — except the reachability **definition-parity** half, which blocks in every mode). Run the relevant probe after ANY change to the access kernel, an RLS policy, a library trigger, or a deploy that touches the guarded feature.

## What these are

- **`check-access-matrix.ts`** — the acceptance matrix for "a data-store publication grants exactly the intended READ surface". Parameterized `--store / --entitled / --control`; defaults: AMA-G5 store `0158e878…`, grant-only reader `77c6af70…` (elliesadeghijd), unrelated control `929274b1…`. Asserts, at every tree level (store row, members, file metadata + DB-level download gate, docs, pages, page images, chunks, extraction jobs/runs/results, `files.*` baby tables): entitled **viewer=true / editor=false / rows visible**. The control expectation comes from `public.library_is_open`: industry/global publication is readable by every signed-in user, while organization-only publication stays denied. Refuses a known super-admin as the entitled leg — super-admins report `can_curate=true` and mask exactly the failures this exists to catch.
...
- **`check-reachability-guards.ts`** — the two standing guards on `platform.reachability`, the trigger-maintained closure cache that access decisions depend on (drift audit 2026-08-15 finding 8, risks 1 and 2; SQL in `migrations/reachability_standing_guards.sql`). **Definition parity** (`platform.reachability_definition_parity()`) asserts every `platform.associations` column that `platform.containment_edges` reads appears in `trg_associations_reachability`'s `UPDATE OF` list — the two matched only by manual synchronization, and one forgotten column silently stops the trigger firing. It is catalog-only, deterministic, and has one right answer, so it **blocks**. **Cache drift** (`platform.reachability_drift()`) FULL OUTER JOINs the cache against a fresh `derive_reachability()` walk — both directions, all depths, depth and level equality — and reports `missing_in_cache | extra_in_cache | depth_mismatch | level_mismatch`. That half is **advisory**: it is a full re-derivation, and the answer to a firing is `platform.rebuild_reachability()` plus a filed defect, not a blocked release. Both arrive in one call via `public.reachability_guard_report()`. Not scheduled — a recurring sweep needs Arman's approval by name and interval (`common-docs/policies/no-unapproved-schedules.md`).
...
- **Canonical entity policies use `iam.accessible_entity_ids` as a candidate superset, then `iam.has_access` as the exact judge.** The candidate function is SECURITY DEFINER because `platform.reachability` and `platform.entity_grants` are not caller-readable; duplicating raw subqueries over those protected tables in a generated RLS expression creates judge-yes/RLS-zero drift.
...
- 2026-08-29 — aligned the control leg with the open-library contract and made canonical entity-policy candidates callable under real-user RLS.
- 2026-08-21 — added the reachability standing guards (definition parity, blocking; cache drift, advisory). First live run: 0 drift across 4,476 rows / 468 containers, parity clean.
