...
- The aidream `python-module-unreached` detector reaches from **server** entry points, from every top-level `aidream/scripts/*.py`, and from every module carrying its own `__main__` block (fixed 2026-08-15 — it previously walked from the server boot files alone, so every operator CLI harness read as unwired). A module reachable **only** from an operator entry point is no longer a finding, and it is not erased either: aidream's `--json` carries it in a `cli_only` array naming its driver. This scanner reads only `findings`, so those artifacts stop appearing here — **the rule lives in aidream; never re-derive it on this side.** The distinction it encodes is real and unmeasured by this report: a server-reachable module loads on every request, a CLI-only one runs when somebody remembers to run it.
