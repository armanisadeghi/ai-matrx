...
try {
  ...
  await sleep(2500);
  await page.click('[aria-label="Copy, transform or export Invoices"]');
  await sleep(600);
  const items = await page.evaluate(() =>
    Array.from(document.querySelectorAll('[role="menuitem"], [role="menuitemcheckbox"]')).map((n) => (n.textContent||"").trim())
  );
  console.log("MENU1:", JSON.stringify(items));
  await page.screenshot({ path: "/tmp/explore4.png" });
} finally {
  await browser.close();
}
