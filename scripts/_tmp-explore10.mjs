...
try {
  ...
  const info = await page.evaluate(() => {
    ...
    const matches = all.filter((el) => el.childElementCount === 0 && /actions/i.test(el.textContent || "") && el.textContent.trim().length < 15);
    return matches.slice(0, 5).map((el) => ({
      tag: el.tagName,
      text: el.textContent,
      className: el.className,
      outer: el.outerHTML.slice(0, 300),
      parentOuter: el.parentElement?.outerHTML?.slice(0, 1500),
      grandParentOuter: el.parentElement?.parentElement?.outerHTML?.slice(0, 2000),
    }));
  });
