...
export const MODEL_CHOICE_RULES: ModelRule[] = [
  ...
  },
  {
    ...
    fix: "Gemini Live and Lyria music run their MANDATE's Holder model, resolved on aidream (services/google_specialized.py: voice.gemini_live, audio.live_music). The client sends a job/mandate key, never a model. (Embedding lab files are the dev-only exception.)",
  },
  {
