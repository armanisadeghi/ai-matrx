...
--
-- Run: binlocal/p.sh -f scripts/campaign-tests/writeperf3b_profile.sql
\set ON_ERROR_STOP on
\set suite 'writeperf3b_profile.sql'
...
create temp table wp3b_fx (k text primary key, v text) on commit drop;
...
begin
  ...
  insert into wp3b_fx values ('org', v_org::text), ('home', v_home::text), ('tbl', v_tbl::text),
                             ('car', v_car::text), ('cars', array_to_string(v_cars, ','));
end;
$$;
...
begin
  select v::uuid into v_home from wp3b_fx where k='home';
  select string_to_array(v, ',')::uuid[] into v_cars from wp3b_fx where k='cars';
...
select pg_temp.build('northgate-auto-glass-wp3b-' || substr(md5(random()::text),1,8));

select v as org from wp3b_fx where k='org' \gset
select v as tbl from wp3b_fx where k='tbl' \gset

-- The connected role IS `custom.record_write_many`'s owner, so the triggers below see exactly the
-- effective user they see inside that SECURITY DEFINER door. This is the door's context.
reset role;

\echo ''
\echo '=== EXPLAIN (ANALYZE, BUFFERS) — ONE 250-row insert into custom.record ==='
...
select :'org'::uuid, :'tbl'::uuid, gen_random_uuid(), d
  from unnest(pg_temp.docs(1, 250)) with ordinality as u(d, ord)
 order by ord;
