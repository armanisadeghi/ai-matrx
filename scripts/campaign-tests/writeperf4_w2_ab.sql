-- WRITE-PERF-4 WAVE 2 — THE A/B GATE. THE SAME 250-ROW INSERT, BEFORE AND AFTER, IN ONE
-- TRANSACTION THAT ROLLS BACK.
--
-- GATE (corrected model, rev 3): `custom.applicable_fields` measured 290 us a call and its
-- replacement ~55; the write path makes about 1,000 of those calls per 250-row statement, so the
-- modelled saving is ~235 ms. The gate is 150 ms — six times the 25 ms the no-change control
-- drifts, and 64% of the model. Below 150 ms wave 2 is reverted, not argued.
--
...
--
-- Run: binlocal/p.sh -f scripts/campaign-tests/writeperf4_w2_ab.sql
\set ON_ERROR_STOP on
\set suite 'writeperf4_w2_ab.sql'
...
explain (analyze, buffers, costs off, timing on)
insert into custom.record (organization_id, table_id, id, data)
select :'org4'::uuid, :'tbl4'::uuid, gen_random_uuid(), d
  from unnest(pg_temp.docs(4, 1, 250)) with ordinality as u(d, ord) order by ord;
