...
begin
  ...
  -- DOES EMPTY IT. Without both halves clause 6 would pass for the wrong reason. So: warm the
  ...
  if platform.memo_b_get('udk:' || v_org::text || ':' || v_tbl::text) is null then
  ...
  if platform.memo_b_get('udk:' || v_org::text || ':' || v_tbl::text) is not null
