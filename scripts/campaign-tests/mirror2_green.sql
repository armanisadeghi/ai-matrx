...
begin
  ...
  if (select system_identifier from pg_control_system()) <> 7642734024280108049 then
    raise exception 'mirror2_green.sql runs on the MAIN database only, and this is %',
  ...
  values (v_org, 'ZZ MIRROR-2 Seats', 'zz-mirror2-' || substr(v_org::text, 1, 8), 'ZMT', c_admin);
  ...
  v_tbl := custom.table_declare(v_org, jsonb_build_object(
    'name', 'ZZ Thing', 'slug', 'zz_mirror2_thing', 'type', 'entity',
