-- DOORS-DECIDE-3 — THE RED TWIN. The same five parts, run against the two bodies AS THEY WERE
-- BEFORE this lane, put back inside the same rolled-back transaction. It proves the green
-- suite's clauses 3 and 5 are load-bearing rather than tautologies about what any door does.
--
-- THE REAL USE CASE THIS DATA IS — identical to the green twin by design. Northgate Mechanical
-- is a 22-person commercial HVAC service contractor in Portland: service calls in a Work Orders
-- Table, closed-out calls archived rather than deleted so a late warranty claim can find them,
-- building owners on a client Portal. Cascade Property Group is a property manager they
-- invoice. Dana (test@test.com) is Northgate's dispatcher and a stranger at Cascade.
--
-- WHAT IT ASSERTS: with the OLD bodies in place, at least one of these is true, and the suite
-- FAILS (exit 1) if neither is —
--   3R  custom.portals still refuses the stranger, but the refusal names custom.list_portals,
--       a function no client can execute: the decision was borrowed from a delegate.
--   5R  custom.read_records_archived tells the stranger Northgate's lane vocabulary (22023)
--       for a lane word she got wrong, about an organization she may not reach at all,
--       because the vocabulary check ran before the wall.
--
-- RUN IT exactly like the green twin. Its transaction ends in ROLLBACK, so the old bodies
-- never outlive the run.

\set ON_ERROR_STOP on
\timing off

\set suite 'doorsdecide3_red.sql'
...
begin
  perform set_config('app.actor_system', 'campaign-test/doorsdecide3_red', true);
  ...
  -- ── THE OLD BODIES, PUT BACK. Both are exactly what the live catalogue held before this
  -- lane: custom.portals delegating its whole decision to custom.list_portals, and
  -- custom.read_records_archived validating the lane word before it asks any wall. Done here,
  -- as the owner, before the seat is taken; the ROLLBACK at the foot of the file undoes it.
  execute $old$
...
$body$
  ...
  -- THE RED CLAUSE. With the old body, the wall is only ever reached inside the delegate, so
  -- the sentence names custom.list_portals — a function no signed-in caller can execute.
  if v_caught !~ 'custom\.list_portals' then
    raise exception '3R DID NOT GO RED: the old body refused with "%", which does not name the delegate', v_caught;
  end if;
  raise notice '3R RED AS EXPECTED — the old door borrowed its decision: "%"', v_caught;
  ...
  -- THE RED CLAUSE. With the old body the vocabulary check runs first, so a stranger who got
  -- the lane word wrong is handed Northgate's lane vocabulary for an organization she may not
  -- reach. That is the leak this lane closed, and here it must still be present.
  if v_state <> '22023' then
    raise exception '5R DID NOT GO RED: the stranger with a nonsense lane was refused with % (%), not the old 22023', v_state, v_caught;
  end if;
  raise notice '5R RED AS EXPECTED — the old door told a stranger: "%"', v_caught;
  ...
  raise notice '5R done — the member still gets 22023, as she does on both bodies.';

  raise notice 'DOORS-DECIDE-3 RED — both red clauses reproduced on the old bodies. The green suite is load-bearing.';
end $t$;
