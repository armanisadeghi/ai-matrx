...
if $STRICT; then
    declare -a GATES=(
        ...
        # THE ACCESS LADDER (common-docs/policies/access-ladder.md): every default is
        # Organization, and a table enters Confidential/Private only with Arman's recorded
        # words. One live query, zero rows or it fails; self-test plants each violation in a
        # rolled-back transaction.
        "Every default is Organization|pnpm check:defaults-are-organization"
        # ONE HONEST-STATE VOCABULARY. `files.sync_mappings.state` is generated by
        # the sync engine into matrx-local/crates/matrx-sync/contracts/honest_states.json
        # and rendered by the desktop app, the tray and the browser's Devices & sync
        # page. The browser's transcription had a guard from the day it was written and
        # NOTHING ran it (folder-sync L5-4) — so a state added or reworded in the engine
        # would first be noticed by a user reading "Unrecognised state: <value>". Needs
        # the sibling matrx-local checkout; without it the guard exits 1 saying it is
        # UNMEASURED, never a quiet green. The self-test plants three real drifts.
        "The honest-state guard can still fail|pnpm check:honest-states-parity:self-test"
        "The browser renders the engine's honest-state sentences verbatim|pnpm check:honest-states-parity"
