...
const CATEGORIES: Category[] = [
  ...
  { key: "angleAny", label: "<any>", re: /<any>/g },
  {
    ...
    re: /Record<\s*string\s*,\s*any\s*>/g,
  },
...
function listFiles(scope?: string): string[] {
  ...
  return out
    ...
    .filter((f) =>
      scope
        ? f === scope || f.startsWith(scope.replace(/\/$/, "") + "/")
        : true,
    );
}
...
function main(): void {
  ...
  if (scope) {
    ...
    console.log(
      `${C.bold}Type-escape hatches under ${scope} (${occurrences.length}):${C.reset}\n`,
    );
    ...
    for (const [cat, list] of byCat) {
      ...
      for (const o of list)
        console.log(`  ${o.file}:${o.line}  ${C.dim}${o.snippet}${C.reset}`);
      console.log("");
    }
    return;
  }
  ...
  if (update) {
    ...
    for (const cat of CATEGORIES)
      console.log(`  ${cat.label.padEnd(22)} ${counts[cat.key]}`);
    return;
  }
  ...
  console.log(
    `${C.bold}Type-escape-hatch ratchet${C.reset}  (total: ${total})\n`,
  );
