...
SELECT jsonb_build_object('contract',jsonb_build_object('version',1,'target','workbench.note_folders','search_path',current_setting('search_path'),'transaction_read_only',current_setting('transaction_read_only')::boolean),'table',jsonb_build_object('catalog_oid',c.oid::text,'owner',o.rolname,'rls_enabled',c.relrowsecurity,'rls_forced',c.relforcerowsecurity),'columns',jsonb_build_object('canonical',columns.j,'md5',md5(columns.j::text)),'acl',jsonb_build_object('canonical',acl.j,'md5',md5(acl.j::text)),'policies',jsonb_build_object('canonical',policies.j,'md5',md5(policies.j::text)),'triggers_and_helpers',jsonb_build_object('canonical',triggers.j,'md5',md5(triggers.j::text)),'indexes',indexes.j,'data',counts.j) snapshot
FROM target t JOIN pg_class c ON c.oid=t.rel JOIN pg_roles o ON o.oid=c.relowner CROSS JOIN columns CROSS JOIN acl CROSS JOIN policies CROSS JOIN triggers CROSS JOIN indexes CROSS JOIN counts;
ROLLBACK;
