 ...
 *
 * Sources (all rows, soft-deleted included, READ ONLY — the session is set read-only before the
 * first query):
...
async function* readSource(cx: pg.Client, source: SourceName): AsyncGenerator<Row> {
  const plans: Record<SourceName, { sql: string; expand: (r: Record<string, unknown>) => Row[] }> = {
    notes: {
...
function messageRows(id: string, messages: unknown): Row[] {
...
async function main(): Promise<number> {
  ...
  try {
    for (const source of SOURCES) {
      ...
      report[source] = stats;
      for await (const row of readSource(cx, source)) {
        stats.rows++;
