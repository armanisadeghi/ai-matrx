...
const RED = "\x1b[31m", BOLD = "\x1b[1m", DIM = "\x1b[2m", RESET = "\x1b[0m", YELLOW = "\x1b[33m";

type Entry = { relation: string; old: string; newSchema: string; new: string; since: string; reason: string };
const manifest = JSON.parse(readFileSync(join(ROOT, "scripts/dead-relations.json"), "utf8")).relations as Entry[];

const SCAN_EXT = new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".sql"]);
const SKIP_DIR = new Set(["node_modules", ".next", ".next-preview", ".git", ".turbo", "dist", "build", "coverage", "migrations", "docs"]);
...
function* walk(dir: string): Generator<string> {
  for (const name of readdirSync(dir)) {
    ...
    if (st.isDirectory()) { if (!SKIP_DIR.has(name) && !name.startsWith(".")) yield* walk(full); }
    else if (SCAN_EXT.has(extname(name))) yield full;
  }
}

type Hit = { file: string; line: number; text: string; entry: Entry; kind: string };
const hits: Hit[] = [];
...
for (const file of walk(ROOT)) {
  ...
  for (const entry of manifest) {
    const r = esc(entry.relation), os = esc(entry.old.split(".")[0]), ns = entry.newSchema;
    ...
    const typed = new RegExp(`Database\\[\\s*['"]${os}['"]\\s*\\]\\[\\s*['"]Tables['"]\\s*\\]\\[\\s*['"]${r}['"]`);
    lines.forEach((text, i) => {
      const hasNewSchema = text.includes(`schema("${ns}")`) || text.includes(`schema('${ns}')`) || text.includes(`"${ns}"`) || text.includes(`'${ns}'`);
      let kind = "";
      if (bare.test(text) && !hasNewSchema) kind = "bare .from/.table (resolves to old schema)";
      else if (typed.test(text)) kind = `Database["${entry.old.split(".")[0]}"] type ref`;
      else if (qualified.test(text) && !text.includes(entry.new)) kind = `qualified ${entry.old}`;
      if (kind) hits.push({ file: rel, line: i + 1, text: text.trim().slice(0, 140), entry, kind });
    });
  }
}

if (hits.length === 0) {
  console.log(`${DIM}✓ check-dead-relations: no references to ${manifest.length} dead relation(s).${RESET}`);
  process.exit(0);
}

const byRel = new Map<string, Hit[]>();
for (const h of hits) (byRel.get(h.entry.relation) ?? byRel.set(h.entry.relation, []).get(h.entry.relation)!).push(h);

console.error("");
console.error(`${RED}${BOLD}╔══════════════════════════════════════════════════════════════════════╗${RESET}`);
console.error(`${RED}${BOLD}║  DEAD RELATION REFERENCES — these point at tables that NO LONGER EXIST ║${RESET}`);
console.error(`${RED}${BOLD}║  Clean-cut doctrine: there is no shim. Repoint them or they ERROR.     ║${RESET}`);
console.error(`${RED}${BOLD}╚══════════════════════════════════════════════════════════════════════╝${RESET}`);
for (const [reln, list] of byRel) {
  ...
  for (const h of list) {
    console.error(`  ${RED}${h.file}:${h.line}${RESET}  ${DIM}[${h.kind}]${RESET}`);
    console.error(`      ${h.text}`);
  }
}
console.error(`\n${RED}${BOLD}${hits.length} dead reference(s) across ${new Set(hits.map(h => h.file)).size} file(s).${RESET}`);
console.error(`${DIM}Fix: bare .from("X") → .schema("<newSchema>").from("X"); public.X → <newSchema>.X; Database["public"]→Database["<newSchema>"].${RESET}\n`);
