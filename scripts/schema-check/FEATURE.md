...
| `direct-from-schema` | error | every direct `.from()/.schema()` vs the live snapshot: `.schema("S").from("X")` where `(S,X)` isn't live, including a name removed entirely (PostgREST PGRST205); bare `.from("X")` on the `supabase` client (or a local alias/cast of it) that resolves to public; `.from("X")` through a canonical schema-binder (`docprocDb(supabase)` or an alias of one) validated against that binder's bound schema. Also resolves `X` through a local `const TABLE = "the_name"` string constant, not just a literal argument. |
...
- **2026-08-17** — Missing relations now ERROR even when the old name exists in
  ...
  `platform.rulebook`; the regression test replays that exact PGRST205 shape.
