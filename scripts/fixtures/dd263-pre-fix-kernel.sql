-- THE PRE-DD-263 ACCESS KERNEL, VERBATIM. Captured from production 2026-09-15 with
-- pg_get_functiondef, md5(prosrc) = 6f06342e0151932f978451d909a7d22e.
--
-- This file is NOT a migration and is never applied. It is the RED half of
-- scripts/check-access-kernel-bounded.ts --self-test: installed inside a transaction that is
-- always rolled back, on a NON-PRODUCTION database only, so the guard can be SHOWN failing.
-- With this body live, a negative access question about a record inside a carrying cycle raises
-- 54001 stack depth limit exceeded instead of answering. Its containment walk calls ITSELF with
-- no visited set and no depth bound; the only cycle protection is the `is distinct from` check,
-- which stops self-containment and nothing one hop longer.
--
-- Do not "fix" anything in here. Its value is that it is exactly what was wrong.
