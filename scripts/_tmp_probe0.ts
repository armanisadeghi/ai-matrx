...
for (const f of [".env.local",".env.production.local",".env.production",".env", resolve(ROOT,"..","aidream",".env")]) {
  ...
  const b = parseEnvFile(p); if (VARS.every(v=>b[v])) { bag = b; console.log("env from", p); break; }
}
...
console.log((await c.query("select current_user, session_user, current_database()")).rows);
console.log((await c.query(`select column_name, data_type from information_schema.columns where table_schema='platform' and table_name='client_callable_door' order by ordinal_position`)).rows);
console.log((await c.query(`select audience, count(*) from platform.client_callable_door group by 1`)).rows);
await c.end();
}
