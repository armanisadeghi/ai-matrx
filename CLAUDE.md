...
- 🚨 **AN AGENT'S DEFINITION LIVES IN THE DATABASE. THE CODEBASE IS THE CONNECTION AND CAN NEVER BE THE DEFINITION.** A system prompt, instruction block, role text, or persona written in this repo is **forbidden** — no matter how small the surface, and *especially* as a "fallback if the agent doesn't load", which is the same violation in disguise. **Arman's ruling, 2026-08-16, in anger:** "we use our actual agents that are dynamically bound and controlled from the database entry and not the code base… that's absolutely unacceptable and cannot ever happen." What code MAY hold: which slot to run, how to build its variables, how to render the result. What it may never hold: the agent's identity or its rules — and never a raw agent UUID either (that is what slots exist for). **Every hardcoded prompt found in the 2026-08-16 purge had already gone wrong**: one was dead while reading as authoritative (a 20k-char constant beside a 43.9k-char DB agent), one had drifted into telling an agent it lacked tools it uses every session, and one made the codebase the real definition of every orchestrator in the product while the DB template it "copied" was pure decoration. Converting one = create the agent in the DB → declare a slot in aidream `services/agent_slots/client_slots.py` → resolve it here (`useAgentSlot` / `resolveAgentSlot` / `useAgentLauncher().launchSlot` / `launchAgentExecution({slotKey})`) → **delete the constant**; an unseeded slot must throw, never degrade. **This is one half of a platform-wide law that binds every repo, every `matrx-*` package and sub-package, and every surface — admin, demos, prototypes, scheduled jobs, scripts.** Cross-repo system-of-record: **`/Users/armanisadeghi/code/common-docs/systems/agent-slots/FEATURE.md`**; the worklist of everything still non-conforming (including this repo's open rows) is `/Users/armanisadeghi/code/common-docs/systems/agent-slots/ROLLOUT.md`; how values actually reach the bound agent — **required reading before any repin** — is `/Users/armanisadeghi/code/common-docs/systems/agent-variable-binding/FEATURE.md`. **Exceptions require Arman's explicit written approval, no agent may grant itself one, and exactly ONE exists platform-wide** (aidream's conversation labeler, ruled 2026-06-07 — nothing in this repo qualifies). Read [`features/agents/slots/FEATURE.md`](./features/agents/slots/FEATURE.md) first. **Guard: `pnpm check:hardcoded-prompts`** (loud, advisory, in `run-release-gates.sh` both modes) — its allowlist is a reason-required ratchet whose count only goes DOWN, and an entry reading `UNRESOLVED:` is a real violation awaiting a ruling, not an exoneration.
