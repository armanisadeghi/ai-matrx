...
- **Index a hot table only with `CREATE INDEX CONCURRENTLY`.** The DB is always live: a plain `CREATE INDEX` queues behind every in-flight writer and blocks every new one behind itself, so on a busy table (`platform.activity_log`, `history.row_versions`, any log/event spine) the migration is not slow — it is *stuck*, and it stalls the migrate step of every release until someone reaches for `--no-migrate`, which silently disables migration apply for unrelated work. The applier bounds every transactional migration at `lock_timeout = 15s` (`LOCK_TIMEOUT` in aidream's `db/apply_migrations.py`) so this now fails in seconds with the fix in the message instead of hanging; it also auto-detects `CONCURRENTLY` and runs those files outside a transaction for you. Same reflex for `ALTER TABLE` on a hot table: keep it in its own small migration, never bundled behind other DDL.
