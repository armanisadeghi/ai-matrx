...
- 🚨 **THE GOVERNANCE-COLUMN TIER (2026-08-14) — an editor WORKS, it does not GOVERN.** The tiered model `viewer < editor < admin` applies on **two axes**: which statement you may run (RLS: select=viewer, update=editor, **delete=admin**) and which columns you may write. The column axis was never built until now, so `visibility`, `created_by`, and `organization_id` sat inside the editor-writable set — an editor-sharee could publish, **take ownership**, and **re-home another user's row**, with ownership theft chaining into a hard delete (D119, live-proven). It is now a **generated BEFORE UPDATE trigger emitted by `iam.apply_rls`** (`iam._guard_governance_columns`; per-type room on `platform.entity_types.governed_columns`; fleet sweep `iam.sweep_governance_guards()`, one short transaction per table). **Never hand-write a per-column or per-table guard** — add the column to that entity's `governed_columns` instead. `created_by` is refused at every level (ownership transfer is not a column write); an org-less `organization_id` is freely adoptable. A refusal is a real 42501/HTTP 403 with a human-readable message, **never** a silent zero-row update — do not write "only the owner can…" copy around a zero-row result. Guard: `pnpm check:governance-tier`. Full canon: `common-docs/systems/access-architecture/FEATURE.md` §2.6.
