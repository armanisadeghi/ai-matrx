...
Arman plus dozens of concurrent agents edit this ONE checkout simultaneously; that is the designed workflow. Write your code, `git add <your files>`, commit immediately and often. Never run tree-wide destructive git here (blanket `stash`, `checkout -- .`, `reset --hard`, `clean`, dirty `pull --rebase`) — pathspec-scope to your own files. Someone else touching your file is not a conflict; only contradictory intent is, and that gets resolved in the merge or one line to Arman. **Never spend output complaining about other agents/sessions editing the tree, and never request your own PR/branch/worktree — delete such commentary on sight.** Full ruling: root `CLAUDE.md` § Shared checkout.
...
- **CRM (2026-08-14):** AI Matrx operates in its own CRM under its own **normal** org `5dc930e9-bd65-44a1-8369-af773f6e1a5b` ("AI Matrx") — NOT the system org `39c38960-…`, which is the platform-global/ownerless tier. Being our own customer means being a normal org; a privileged org for our data rebuilds the privileged internal path. People who contact *us* are `crm.party` rows there. **EVERY USER HAS A PARTY — no exceptions, created at signup, never at request time**; the party is the person, the user account is their login, `claimed_by` is the join. The same human being a separate party in each org is CORRECT (orgs never share contacts) — never build a cross-org party bridge.
...
**Purpose-built but unreachable means unfinished.** `pnpm check:unwired` ([`scripts/unwired/FEATURE.md`](./scripts/unwired/FEATURE.md)) scans this repo plus aidream for unmounted React components, unimported hooks/services/producers, unmounted FastAPI routers, unset host installers, unregistered scheduler handlers, and unreachable Python service modules. It is loud, **never blocking**, and advisory in `run-release-gates.sh` in both modes. The size-ranked scoreboard is `/administration/reporting/unwired`; every row opens its source line and copies a finish-the-wiring brief. Refresh it with `pnpm check:unwired:write` and commit the snapshot. A clean report is not proof—read the FEATURE’s *Known limits*.
...
- **`authInterrupts` is ON** (`experimental.authInterrupts: true`) — so `forbidden()` / `unauthorized()` from `next/navigation` and their `forbidden.tsx` / `unauthorized.tsx` boundaries work. `requireAccess(type, id, level, { forbid: true })` is how a Server Component refuses in place (real HTTP 403) instead of redirecting; boundaries are `app/forbidden.tsx` (bare) and `app/(core)/forbidden.tsx` (inside the AppShell). **A `forbidden.tsx` can never name the record** — Next renders the fallback BEFORE the page throws, so no request-scoped handoff can reach it (proven 2026-08-11; don't rebuild one). A surface that must say WHICH record and offer "Request access" returns `<AccessGate token id/>` from the page instead. Turning the flag off is a build error at every `forbidden()` callsite — rewrite this line in the same change.
...
- 🚨 **AN AGENT'S DEFINITION LIVES IN THE DATABASE. THE CODEBASE IS THE CONNECTION AND CAN NEVER BE THE DEFINITION.** A system prompt, instruction block, role text, or persona written in this repo is **forbidden** — no matter how small the surface, and *especially* as a "fallback if the agent doesn't load", which is the same violation in disguise. **Arman's ruling, 2026-08-16, in anger:** "we use our actual agents that are dynamically bound and controlled from the database entry and not the code base… that's absolutely unacceptable and cannot ever happen." **A named job is a Mandate; what fulfils it is a Holder; the decision joining them is a Binding** — renamed from "agent slot"/"slot"/"rebind" by Arman on 2026-08-16, and the old words are now defects wherever they name this system (`context_slots` on an agent is a DIFFERENT lineage — correctly excluded from this rename, but it is not staying either: it becomes **Context Policy** / `context_policies` under its own campaign, ruled 2026-08-17). What code MAY hold: which mandate to run, how to build its variables, how to render the result. What it may never hold: the agent's identity or its rules — and never a raw agent UUID either (that is what mandates exist for). **Every hardcoded prompt found in the 2026-08-16 purge had already gone wrong**: one was dead while reading as authoritative (a 20k-char constant beside a 43.9k-char DB agent), one had drifted into telling an agent it lacked tools it uses every session, and one made the codebase the real definition of every orchestrator in the product while the DB template it "copied" was pure decoration. Converting one = create the agent in the DB → declare a mandate in aidream `services/mandates/client_mandates.py` → resolve it here (`useMandate` / `resolveMandate` / `useAgentLauncher().launchMandate` / `launchAgentExecution({mandateKey})`) → **delete the constant**; an unseeded mandate must throw, never degrade. **This is one half of a platform-wide law that binds every repo, every `matrx-*` package and sub-package, and every surface — admin, demos, prototypes, scheduled jobs, scripts.** Cross-repo system-of-record: **`/Users/armanisadeghi/code/common-docs/systems/mandates/FEATURE.md`**; the worklist of everything still non-conforming (including this repo's open rows) is `/Users/armanisadeghi/code/common-docs/systems/mandates/ROLLOUT.md`; how values actually reach the bound agent — **required reading before any rebind** — is `/Users/armanisadeghi/code/common-docs/systems/agent-variable-binding/FEATURE.md`. **Exceptions require Arman's explicit written approval, no agent may grant itself one, and exactly ONE exists platform-wide** (aidream's conversation labeler, ruled 2026-06-07 — nothing in this repo qualifies). Read [`features/agents/mandates/FEATURE.md`](./features/agents/mandates/FEATURE.md) first. **Guard: `pnpm check:hardcoded-prompts`** (loud, advisory, in `run-release-gates.sh` both modes) — its allowlist is a reason-required ratchet whose count only goes DOWN, and an entry reading `UNRESOLVED:` is a real violation awaiting a ruling, not an exoneration.
- 🚨 **NOTHING STRUCTURED IS EVER PASSED AS `user_input`.** `user_input` / `defaultUserInput` / `runtime.userInput` carries what the human actually typed or dictated — nothing else, at any size. A document, report, transcript, JSON payload, schema, selection, or scope block reaches the agent as a **named variable** (`variables`) or a **context entry** (`setContextEntries`), never as prose in the composer and never concatenated onto the person's own draft. **Arman's ruling, 2026-08-18, in anger** — this is the platform's differentiator from every system that flattens everything into one message. **Writing into the user's draft via `setUserInputText` to smuggle machine content is the client-side form of the violation**, including "there's no outgoing-message transform hook" — that gap is a bug to file, not a licence. Reference implementations that do it right: `components/mermaid/hooks/useMermaidAgentEdit.ts` (diagram → `setUserVariableValues` + `setContextEntries`) and `features/code/agent-context/**`. Law + scenario matrix: `/Users/armanisadeghi/code/common-docs/systems/agent-variable-binding/FEATURE.md` § THE USER-INPUT LAW.
- 🚨 **NEVER hand-render a stream. Doing it once is how this platform dies.** Streamed model output — markdown, `__kind` JSON, tool calls — renders through the ONE canonical pipeline (`MarkdownStream` → `EnhancedChatMarkdown` → `BlockRenderer` → the kind registry), which already parses, routes, upgrades late schemas, finalizes, and recovers. A surface that buckets its own chunk text, opens its own `useLiveJsonRegion` parse session, splits multi-payload text, or hand-routes an envelope into a kind component is **forbidden** — no matter how small or unimportant the surface. `useLiveJsonRegion` is internal to content-ir, **not** a primitive you may consume. **Arman's ruling, 2026-07-28, in anger:** one bespoke renderer becomes tens of thousands of hard-coded ones, the single canonical system is gone, and the company is dead. There is no size of feature that earns an exception, and "it's just a small side page" is the exact argument that produced the two violations we HAD — both deleted 2026-07-29. **Enforced by ESLint: `matrx/no-bespoke-stream-renderer` (error).** Need streamed content rendered outside `/chat`? It needs a `requestId`, and there are exactly two ways to get one: launch through the execution system (a shortcut / `launchAgentExecution`), or — when the run is orchestrated SERVER-side inside a pipeline endpoint — **adopt its stream with `adoptForeignStream`** (`features/agents/redux/execution-system/thunks/adopt-foreign-stream.ts`, passed to `callApi`'s `consumeStream`), then render `<MarkdownStream requestId=…/>` and read `selectKindEnvelope`. That gap — a pipeline stream having no requestId — is what forced the one real violation; it is closed. Read [`features/content-ir/FEATURE.md`](./features/content-ir/FEATURE.md) first. **And the row a viewer renders from must outlive the viewer — reaping/resetting `activeRequests` rows mid-stream is the recurring "my data just disappeared" bug: read [`features/agents/docs/LIVE_RUN_RETENTION.md`](./features/agents/docs/LIVE_RUN_RETENTION.md) before touching `removeRequest`/`createRequest`/adoption code.**
- 🚨 **A SHAPE HAS EXACTLY ONE COMPONENT. Rendering a registered kind with anything else is BANNED.** If a `__kind` is registered, its kind component renders it **everywhere** — chat, a live run, a form, a side panel, a preview card. **Hand-writing a second renderer for a shape we already have a component for is the single most expensive mistake in this repo:** the duplicate always ships new bugs (a paragraph crammed into a 1-line textarea), it never gets the improvements the canonical one gets, and every copy multiplies the surface that must be fixed twice. **Need only PART of a shape somewhere?** Split the canonical component into per-field child components and have the parent compose them — then render the child. You do NOT get to re-render a slice by hand because "it's just the angle" or "just the list". **Need it editable, sortable, filterable, addable, deletable?** Those belong IN the canonical component — that is what a kind component is for (exemplar: the keyword kinds). **Need an action on it** ("Use this brief", "Apply", "Send")? It goes on the kind component through the kind-component action registry (`features/content-ir/react/actions/kind-action-registry.ts`), not on a bespoke card beside it. **Arman's ruling, 2026-08-11, in anger:** exceptions require his explicit approval, and no agent may grant itself one. Read [`features/content-ir/FEATURE.md`](./features/content-ir/FEATURE.md) § THE CANONICAL COMPONENT LAW before rendering any shape data.
- 🚨 **Don't wrap a component in wrappers. Every layer you add is padding the user can see.** A component already carries its own spacing, border, and background. Putting it inside a `<div>` with more padding, then another with a background, then a card, produces the two-tone box-in-a-box with a dead band around it — and the colors fight because `bg-background` and `bg-card` are different on purpose. **A host frame either IS the chrome or has none.** When a container already draws a border/title (a window, a card, a dialog), the content renders **bare** — no second border, no second background, no second status line, no extra padding. Exemplar: `LiveRunDisplay`'s `variant="bare"` inside `LiveRunWindow`.
- 🚨 **A SPINNER IS NEVER THE ANSWER while AI works — the run streams, floating.** Every AI surface, agent, or multi-step pipeline shows live progress: stream the output when you can show it, narrate real stages when you'd rather not (podcast/research), approximate stages when you don't know them. The canonical surface is the floating `LiveRunWindow` (`useOpenLiveRunWindow()` / `<LiveRunWindowController>` in [`features/overlays/openers/liveRunWindow.tsx`](./features/overlays/openers/liveRunWindow.tsx)) — its body is `LiveRunDisplay`, so registered kinds render as their components token-by-token for free. **A live-run block at the TOP of a page is banned everywhere** — it shifts the content the user is editing. Rendering `LiveRunDisplay` inline is the ONE exception and must be earned: purpose-built UI, more specialized than the window, and a guarantee of zero page shift (content at the bottom, page only grows). **A run that dies on page refresh is the same defect as a spinner.** Sizing, the exception bar, and per-kind overrides: [`features/window-panels/FEATURE.md`](./features/window-panels/FEATURE.md) § THE FLOATING LAW.
...
- **Project:** `brsgrqvjdzwihsvnfqkf` (Matrx Main, `us-east-1`, Postgres 17). The only DB this repo talks to. `NEXT_PUBLIC_SUPABASE_URL` → `db.matrxserver.com`. Always pass `project_id: "brsgrqvjdzwihsvnfqkf"` to Supabase MCP tools — do not guess between Matrx Main / My Matrx / Matrx Flow / Matrx DM / Matrx Games.
...
- 🚨 **A LIST YOU TREAT AS COMPLETE MUST BE READ WITH `readAllRows` ([`lib/supabase/readAllRows.ts`](./lib/supabase/readAllRows.ts)).** PostgREST caps every response at 1000 rows and announces it only in a `Content-Range` header — no error, just a shorter list. Any read feeding an **existence check, a diff, a set subtraction, or a `.length` comparison** therefore starts answering *confidently wrong* the moment its table passes 1000 rows. It already fired: a 1005-row ledger read as 1000 reported 5 freshly-applied migrations UNAPPLIED and hard-blocked `--strict` releases, and `check:shapes` (1143 kinds seen as 1000) invented 138 "missing asset" findings. The helper pages to a **verified total and THROWS on a short read** rather than returning a partial; `tryReadAllRows` screams and returns `null` where a gate should SKIP instead of asserting. Never hand-roll a second paging loop. A bare `.select()` stays correct only where a short list is an acceptable answer (rendering, preview, sampling). Advisory sweep: `pnpm check:unbounded-reads` (loud, never blocking; text-scan, so a clean run is a floor not a proof). FOUND_DEFECTS D190.
- 🚨 **THE COMPONENT OWNERSHIP LAW (Arman, 2026-08-14) — `created_by` is an ENTITY column.** On an entity the creator IS the owner, which is the only reason one column can be both audit stamp and access key. On a **component** the actor and the owner come apart (the owner is the **parent**), so `created_by` must NEVER appear in a component policy — `iam.apply_rls(…,'component')` no longer emits one. **Do not "fix" a component by adding `created_by`**; a component with none is correct, and `history.row_versions` already records who acted. If a sub-row needs its own owner with independent access, it is not a component — it is an entity in a containment relationship, and the variant was chosen wrong. Table **GRANTs** are likewise variant-keyed and issued by the generator (`iam.apply_table_grants`) — grants are not where openness is decided, RLS is; and a table with RLS off or zero policies is a **hole, not a closed door** (never grant past it). Full canon: `common-docs/systems/db-rules/FEATURE.md` §6d-1 / §6d-2.
- 🚨 **THE THREE SHARE LEVELS — view / edit / FULL (Arman, 2026-08-14).** When one person shares one item with another: **view** = see everything, change nothing. **edit** = change the content — *including publishing it* (in real companies the publisher is the approver at the end of the line, not whoever clicked "new"), but **NEVER delete**: edit has never meant delete and never will. **full** = the owner's own privileges, delegated. 🚨 **The enum still spells the third level `admin`, and that is NOT the org-admin role** — one is a personal delegation on ONE item, the other is a role over a whole company. Say "full" in every label, comment, and doc; never write "admin access" without saying which. Enforced by a **generated BEFORE UPDATE trigger emitted by `iam.apply_rls`** (`iam._guard_governance_columns`), not by RLS — RLS is row-level and cannot gate a column. The per-item-type set of too-core-for-edit fields is `platform.entity_types.governed_columns` (default `{created_by, organization_id, deleted_at}`); **never hand-write a per-table or per-column guard — add the column to that set instead.** Asymmetries on purpose: restoring from trash and adopting an org-less row stay edit-level; `created_by` is refused at every level (ownership transfer is a deliberate audited action, never a field write). Refusals are real 42501/HTTP 403 errors with human-readable text — **never** a silent zero-row update, so never write "only the owner can…" copy around an empty result. Guard: `pnpm check:governance-tier`. Canon: `common-docs/systems/access-architecture/SHARE_LEVELS.md`.
...
🚨 **THE UNFINISHED-WORK ALARM — before you ever describe anything as "unreferenced", "orphaned", "dead", or "unused", read `/Users/armanisadeghi/code/common-docs/policies/unfinished-work-alarm.md`.** A purpose-built artifact with no runtime consumers is **work a previous agent left unfinished** — hunt down its vision (common-docs, every repo's feature dirs, the ORM configs; a grep is NOT a search), flag it loudly as unfinished, and recommend FINISHING it. **Recommending its deletion is forbidden** until Arman himself names it dead in writing. (Born from the `growth.*` incident, 2026-08-13.)

🚨 **About to canonicalize a table, clean up `graveyard`/`public`, or act on a conformance FAIL count? STOP and read `/Users/armanisadeghi/code/common-docs/policies/database-changeover-doctrine.md` first** — read it before touching this in ANY repo. There are only two sanctioned goals (fix what is *actually* broken; move the changeover forward a few tables at a time), "broken" means you can name a **surface, an action, and a wrong outcome**, and the conformance counts are **known-inflated** (95 of 115 component FAILs are correct tables the checker misreads). It also names the two opposite fatal moves: teaching the canonical system to accommodate a legacy shape, and declaring the canonical system broken because an untransitioned table fails it. Written after a session that measured a "144-table defect" which broke **zero** live surfaces.

App code has **no DDL path** (Supabase JS / PostgREST only); agents apply DDL via the **Supabase MCP** (`apply_migration` / `execute_sql`, project `brsgrqvjdzwihsvnfqkf`).
...
- **Index a hot table only with `CREATE INDEX CONCURRENTLY`.** The DB is always live: a plain `CREATE INDEX` queues behind every in-flight writer and blocks every new one behind itself. The database-level `ddl_lock_timeout_guard` bounds otherwise-unqualified DDL waits at 8s; the migration applier explicitly uses 15s and auto-runs `CONCURRENTLY` files outside a transaction. A timeout means split the change or use the concurrent form—never make a hot-table DDL batch wait longer.
...
🚨 **A user bounced to auth NEVER loses their destination.** Whatever page they asked for is where they land after signing in — through a wrong password, a password reset by email, an OAuth hop, or every auth page in sequence. One primitive owns it: [`utils/auth/auth-destination.ts`](./utils/auth/auth-destination.ts) — **capture once, and NEVER mint a second** (`withAuthDestination` no-ops when the target already carries one). Build every sign-in link with `loginHref()` / `useLoginHref()`, carry it on every error re-render with `preserveAuthDestination()`, and **always pass the 4th `destinationSource` arg to `encodedRedirect()`** — omitting it is exactly how the destination was being erased on every validation error. Write `redirectTo`; `next` / `returnUrl` are read-only legacy aliases (never write a new one). Read [`utils/auth/FEATURE.md`](./utils/auth/FEATURE.md) before touching any auth page, auth action, or `/login` redirect.
...
**Fleet-release invariant.** Ordinary commits intentionally create `CANCELED` Vercel records; those are ignored builds, not releases and not evidence that production is current. The scheduled release owner must compare `origin/main` with the commit behind the latest `Ready` production deployment in **all three** projects. Because shared runtime code (`features/`, `components/`, `lib/`, auth, and `app/api`) can feed every profile, automatic releases use `./scripts/release.sh --target all`; a target-specific release is allowed only when a human-triggered release positively proves the change is isolated to one surface. A runtime-bearing commit may not remain absent from any affected project's latest `Ready` deployment for more than 30 minutes.
...
### Media durability — a signed URL is a HANDOFF, never an identity

**Cross-repo system-of-record: `/Users/armanisadeghi/code/common-docs/systems/media-durability/FEATURE.md` — read it before touching any media/URL code in ANY repo.** Internally the platform passes the `file_id`, exactly like a row id; whoever needs bytes or a URL mints a fresh one from it. A signed URL is minted at the moment of handoff to a third party, used, and forgotten — never stored, never persisted, never passed between components. A tool result that names one of OUR files resolves it to its `file_id` and renders the canonical component; a storage host must never reach the UI (`features/tool-call-visualization/result-fields/shape.ts`, guarded by `__tests__/shape-our-files.test.ts`).
...
- **A column the public web reads MUST hold a public URL.** Register it with the DB-edge guard (`migrations/mtx_public_media_url_guard.sql`): `insert into mtx_public_url_guard(schema_name,table_name,column_name)…` + `mtx_public_url_guard_trigger`. Non-durable writes get logged + queued to `mtx_media_heal_queue`. **Always pass `schema_name`** — the guard keyed on bare table names until 2026-08-11 and silently protected nothing on three anon-facing columns after the schema reorg renamed their tables (D158).
- 🚨 **The defect is the MISMATCH, not the expiry.** An expiring signed URL is CORRECT in many places — a time-boxed share link is a security *feature*, and a TTL cache or an audit row recording exactly what was issued must keep it. Breaking those is worse than the bug. It is a defect only when the consumer must resolve it **later**, or for **anyone** (anon share pages, published episodes, OG images, CDN/blog surfaces). **Never resolve a mismatch by publishing private content** — the durable ref for an access-controlled asset is the `file_id`, re-minted on read; making it permanently public to clean a scan is a data-exposure incident. Can't tell if an asset is meant to be public? Ask Arman. Repeatable detection + the committed per-column classification (with the reading code path for each verdict): **`pnpm check:media-durability`** → [`scripts/media-durability/FEATURE.md`](./scripts/media-durability/FEATURE.md).
...
System (canonical, cross-repo): `/Users/armanisadeghi/code/common-docs/systems/pattern-patrols/VISION.md` + `FEATURE.md` + `PATROL_REGISTRY.md` (10 live patrols: dead ends, unused primitives, mobile breakage, light/dark, copy-everywhere, emojis, browser dialogs, bare Loading, coming-soon compliance, type-suppression debt). Recurring runs execute in Codex; every run appends `.matrx/patrol-runs/<P#>/<run-id>.json`; certification by a second adversarial agent is mandatory for every exact candidate SHA; patrol workers never move `origin/main` or release. **Two standing duties for EVERY agent in this repo — invoke the `pattern-patrol` skill for the mechanics:**
...
| **Access Gate** — nobody ever sees an access error. A failed read resolves to the TRUE state (denied / deleted / missing / signed-out / transient) with a way forward, and a blocked user can ask the owner in one click. Drop `<AccessGate token id/>` into any hand-rolled "couldn't load it" branch; on the server, `requireAccess(type, id, level, { forbid: true })` renders the same surface through `app/forbidden.tsx`. **Never hand-write "not found" / "no permission" copy, and never `notFound()` on an empty single-record read** — under RLS that branch fires for four different reasons and asserting one is wrong most of the time. Measured by `pnpm check:access-errors` (advisory) | `features/access-gate/FEATURE.md` |
...
| **Education / study platform** (`/education` — flashcards, FastFire, AI tutor, quizzes + practice tests, audio study, mind maps, memory aids, spoken practice, handwriting grading, planner, games, classes, the public learn/exam content engine). 🚨 **The product spans this repo + aidream + the shared DB, so its vision, gap analysis, execution plan and live agent-coordination boards live in ONE place: `/Users/armanisadeghi/code/common-docs/projects/education-platform/` — START THERE, not with the repo docs.** The 2026-07 status docs that declared it complete are archived there and **must not be cited as current state** (`GAP_ANALYSIS.md`, 2026-08-17, replaces them). Working on any education surface? Read that project's `README.md` first | `features/education/FEATURE.md` (+ per-sub-feature `FEATURE.md`) · [`features/flashcards/FEATURE.md`](./features/flashcards/FEATURE.md) — the largest subsystem; **read its *Known limits* before reporting any flashcard surface done** |
