...
> 🔴 **TEMPORARY — DB TRANSITION IN PROGRESS (read before touching DB/data code).** The 2026 schema-reorg + canonical-RLS changeover is mid-flight on a shared `main` with many concurrent agents; compat shim views, and temporary workarounds MUST BE REMOVED!!!!

REQUIRED ORDER OF ACTION: Get EVERYTHING changed to the new canonical way, even if other parts haven't caught up!!! It is ok to get errors becuase things are the new way. That gets us one step closer to getting our users back online.

IMPORTANT: If you revert things back to the old system, YOU ARE NOT A HERO!
 - We have missed our deadline for going back online because of wreckless agents putting shims in place and creating workaround to make us think everything is working!
 - All tables MUST be changed over to the new canonical system.
 - Tables in the public schema are to be moved to their proper pre-defined new schema. (Duplicates MAY NOT be kept!!!!!)
 - You must MOVE US FORWARD, NOT BACKWARDS!!!!!
 - Your goal is to get every table closer to this new standard so we can get our users back online! The system is DOWN until that shift is made in full.
