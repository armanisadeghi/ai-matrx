...
- **`authInterrupts` is ON** (`experimental.authInterrupts: true`) — so `forbidden()` / `unauthorized()` from `next/navigation` and their `forbidden.tsx` / `unauthorized.tsx` boundaries work. `forbidden()` is how a Server Component refuses in place instead of redirecting; it carries no payload, so name the record first (`requireAccess(..., { forbid: true })` does this via `lib/access/forbiddenTarget.ts`) or the boundary can only show the generic refusal. Boundaries: `app/forbidden.tsx` (bare) and `app/(core)/forbidden.tsx` (inside the AppShell). Turning the flag off is a build error at every `forbidden()` callsite — rewrite this line in the same change.
