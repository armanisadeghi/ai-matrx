...
> **⚠️ PARTIALLY STALE — project coupling removed (2026-07-21).** This doc's
> project-based architecture claims are obsolete. Research topics are
> PROJECT-INDEPENDENT: no `rs_topic.project_id` FK/trigger, tenancy =
> `organization_id` + `created_by`, RLS via `iam.apply_rls` (never a project
...
A structured, multi-stage research pipeline designed to take a topic from raw keywords all the way to a polished, human-edited document. The system is organization-scoped (a project link is optional classification, never a dependency), fully traceable, and built for AI-assisted analysis at every stage.
...
organizations (tenancy)
  └── rs_topic          ← org-owned; OPTIONAL platform.associations edge → project
        ├── rs_keyword
        ...
        │     └── rs_source_tag (join)
        ├── rs_analysis
        ├── rs_synthesis   ← scope: keyword | topic (legacy 'project') | tag | custom
        ├── rs_tag
        └── rs_document
```

All child tables reference `rs_topic` via `topic_id`. No table carries `project_id` (the leftover nullable `rs_topic.project_id` column is non-authoritative until its Phase-4 drop) — RLS is generated by `iam.apply_rls` on the `research_topic` entity and NEVER references projects.
...
|-------|---------|---------|---------|
| **rs_topic** | A research topic (org-owned; project optional via association edge). Holds config, agent settings, autonomy level. | none (org tenancy; optional `research_topic → project` association) | "All Green Brand Profile" for the "All Green Rebrand" project |
...
**STALE-CORRECTED (2026-07-21):** policies do NOT chain through `projects`.
`rs_topic` is the canonical entity (token `research_topic`); its RLS is
generated by `iam.apply_rls` (ownership / org membership / `iam.has_access`),
and every child table defers to the owning topic's access. Projects play no
part in research authorization.
