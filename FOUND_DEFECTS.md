...
(`common-docs/policies/canonical-first-triage.md`) tells its reader to run `audit.refresh()` first, but
ratchet 2 (`scripts/canonical-ratchets`), the admin canonicalization view, and any reader that skips that
line see the cache, so a regressed certified table reads as clean.
...
run it per schema. The steward ran `audit.refresh()` at 2026-09-26 ~02:40 UTC (it completed): all three
tables now read `certified = false`, so today's cache is honest — the defect is that it goes stale again
with the next DDL and nothing refreshes it.
...
- **D351** — the release sweep shipped an UNFINISHED migration to production (2026-09-26). `rca5d_c_definer_doors_ask_each_record.sql` was committed into `migrations/` at 03:39Z marked "rehearsal pending"; the 30-minute sweep (`scripts/release.sh` → aidream `db/apply_migrations.py --source matrx-frontend --target production`) applied it at 03:45Z and the sidebar hierarchy loader timed out (75 s) on every page load until `rca5d_e` at 04:36Z. **Root cause:** `migrations/` is the swept directory, so a commit there is a release request, and a file had no way to say "not finished" — one lane escaped only by hand-adding `-- chair-step:`. **Not the fix:** making the clone rehearsal a gate (tried and reverted the same night — Arman's 2026-09-18 ruling stands, JUDGMENT.md §6a: the rehearsal copy drifts and is never a precondition). **Guard:** `-- draft: <owner> <reason>` (JUDGMENT.md §1c) — every new migration starts with it; at `--target production` aidream's sweep holds a draft (`held: draft by <owner>`, dependents wait) and every named apply refuses it with "remove the -- draft: line when it's ready"; the clone takes drafts. Proof RED→GREEN: `pnpm db:apply --draft-self-test` (spawns the real runner; shown red with the check disabled), aidream `db/tests/test_migration_draft_marker.py`, and live production dry-runs on a planted draft (sweep held it, named `--only` exited 1, the same bytes without the line were admitted). **Also fixed:** rca5d_c's "8abf1a83 vs 55022a62 drift" was never drift — 8abf1a83 is aidream's `sha256(sql.rstrip())` of the same bytes; `pnpm db:apply` now accepts both forms (CS-30's class); no ledger row was changed. **Follow-up, same night — a draft was LEDGERED without executing (22:44 PT):** the aidream release runs `db/detect_applied.py --quiet` before its sweep, and that scanner auto-ledgers any untracked file whose created objects all exist; `rca5d_j_shortcuts_follow_their_own_visibility.sql` only REPLACES existing functions, so the draft read as "applied" and got a row (checksum = draft bytes, duration 0) though the sweep had held it. Once the draft line came off, `pnpm db:apply` said "the file on disk is not the bytes that ran" and the finished fix would have silently never run (the lane recovered with `--reapply`). **Class:** every ledger-only writer can record bytes that never executed. **Guard:** (1) no path ledgers a draft — detect_applied reports it SKIPPED, `--mark-applied` / `--accept-drift` / aidream `ledger_rebase.py` / frontend `--ledger-rebase` / `--amend-idempotent` / `--restore-ledger` refuse it; (2) detect_applied auto-ledgers an untracked `CREATE OR REPLACE FUNCTION/PROCEDURE` file only when each body it writes IS a live body (`pg_proc.prosrc`), else PARTIAL ("live body … is not this file's body") and left for the applier; (3) `pnpm db:apply` on a row that holds a draft of the file (found in git) or a duration-0 ledger-only row says so and prints the `--reapply` command. Proof RED→GREEN: planted files (never applied to production) — HEAD `detect_applied --dry-run` "would record 3" (draft + never-run replacement + live-body file) vs fixed "would record 1" (live-body file only, the draft SKIPPED, the replacement PARTIAL); on the clone `--mark-applied` of a draft refused, and a changed file over a duration-0 row got the ledger-only sentence + `--reapply`; `pnpm db:apply --draft-self-test`; aidream `db/tests/test_migration_draft_marker.py` (6). **Census of the six ledger-only rows whose bodies were not live — verdicts 2026-09-26 (compared body by body with comments stripped, based-on hashes checked against production):** (1) matrx-utils `044_insert_name_safety.sql` — APPLIED (via MCP 2026-09-21; live bodies equal the file except comments, trigger present, based-on == live); no action. (2) matrx-utils `045_rename_folder_nested_subtree.sql` — SUPERSEDED by `campaign/errorshonest_s2_the_public_doors_say_not_found.sql` (2026-09-25; live = 045 with its not-found RAISE moved to `platform.refuse_not_found`); annotated `-- superseded-by:` + `-- retired:`, row reconciled with `--accept-drift`. (3) aidream `0972_a_spoken_approval…` — SUPERSEDED by the 2026-09-22 deletion of the exact-action authorization class (Arman's ruling; the door and its siblings are absent live); annotated + retired + `--accept-drift`. (4) matrx-frontend `aei_reach_containers_asked_once.sql` — NEVER RAN and CONFLICTED (its body predates RC-A2c `reference_gate` and RC-A2e `token_is_detail`); rebased as `aei_reach_containers_asked_once_rebased.sql` (live body + only the AEI-REACH change + the D249 kernel re-record, inverse in `migrations/inverse/`), rule 27 on the clone complete, applied to production 06:36Z: flash-card list as test@test.com 4,854 ms → 524 ms (production, best of 3; clone 4,596 → ~490), `aei('fc_card')` 4,600 → 277 ms, fingerprint == expected, `test_access_read_latency.py` on production 12 passed (set form == kernel for fc_card / studio_session / processed_document / note × 3 seats) — the 300 ms flash budget is still RED at 522 ms (the rest is outside this function, in the fc_card policy arms); old file annotated + retired + `--accept-drift`. (5) aidream `0969_every_client_door_declares_every_id_argument.sql` — NEVER RAN (0 argument_rules rows carried its mark) and conflicted with storetxn4b; chair ruled REBASE: `1160_every_client_door_declares_every_id_argument_rebased.sql` = today's live `platform.door_body_must_decide` (storetxn4b's refusal_only arm kept) + the per-id rule (a definer door that decides access and takes ≥2 ids needs `argument_rules.arguments.<id>.foreign`; `unchecked` refused), with the per-id notes filled first as registry metadata (33 doors lacked them; existing notes win, other keys kept) and a verify block. Clone: rule 27 through the runner (up → inverse → up; the aidream rehearse harness's measure pass mis-splits adjacent string literals — the known collapsed-literal class — so its lock measurement could not run), every one of 1,957 door rows re-checked by the trigger in a rolled-back pass: 0 refused by the new rule (1 pre-existing `definer_no_access_decision` refusal, `public.share_resource_with_org`, unrelated), RED: a door stripped of its notes is refused; a real `platform.provision` on the clone passed the deferred trigger. Production: applied with `--confirm-chair-step`; population 0 of 549 unruled; 1,962 door rows re-checked (rolled back): 0 refused by the new rule; RED reproduced. Original annotated `-- superseded-by:` + `-- retired:`, `--accept-drift`. (6) aidream `1024_a_provision_holds_its_locks_for_seconds.sql` — NEVER RAN and conflicted; chair ruled REBASE: `1161_a_provision_holds_its_locks_for_seconds_rebased.sql` — 1024's additions merged three-way onto today's bodies (base = 1014's provision, 1010's batch): preflight refuses a statement ceiling over 60 s (`preflight.statement_timeout`) and a transaction older than 5 s (`preflight.transaction_age`); provision issues one grant per role set and writes every non-deferred edge in one insert (live PROVISION-BATCH-FIX rule "defer only while a token is unregistered" kept); batch writes deferred edges in one insert (live certification kept); selfcheck runs its deep probe only 1–4 AM PT. Its proof block now tests 61 s / 90 s (the LOCK-QUEUE rule refuses a file that sets 0 or 15 min). Consistency with the 60 s cap: aidream's runner now caps provisioning-class files at 60 s at the clone as well as production, and `pnpm db:apply` lowers `--statement-timeout` to 60 s for any file calling `platform.provision` (dry-run shows it). Clone: rule 27 through the runner, a real rolled-back `platform.provision` of the self-check spec built 15 relations + its door in 1.2 s under 60 s. Production: applied; preflight at 60 s in a young transaction → no findings, at 90 s → `preflight.statement_timeout`; `provision_selfcheck(true)` ok with deep `deferred to the maintenance window` (which `scripts/check_provisioner_alive.py` reads as not-proved, not an alarm); grant/edge shapes confirmed in the live body. Original annotated + retired + `--accept-drift`. **Two tool defects found doing it, fixed (2026-09-26):** (a) ONE statement splitter — aidream's rehearse measure pass stripped comments without quote awareness and collapsed whitespace, so adjacent literals on two lines (`'a '` newline `'b'`) became a syntax error (1160/1161 could not be measured); this repo's verbatim splitter mis-read `E'it\'s'`. Now `scripts/lib/sql-split.ts` and aidream `db/sql_split.py` (verbatim statements; dollar quotes, E'' strings, nested block comments, quoted identifiers) share `scripts/lib/sql-split-corpus.json`; both rehearse tools execute its output. RED kept in aidream `db/tests/test_sql_split.py` and `pnpm db:apply --sql-split-self-test`; GREEN: `db/rehearse_migration.py` rule 27 on 1160 now measures and completes (exit 0). (b) The live provisioning suites SKIPPED silently since the rehearsal branch was deleted: `db/provisioning_target.py` now resolves a retired BRANCH-REF to the DEV CLONE (CLONE-REF), proves the server by system_identifier AND quarantine (a clone shares production's identifier), and FAILS instead of skipping when the target is unresolvable or unreachable (proved: a bogus CLONE_DATABASE_URL → 6 errors, 0 skips); `test_provisioning_hash_partitions_and_write_door.py` and `test_provisioning_schema_client_exposure.py` moved off their hand-rolled branch fixtures onto the same resolver. Result on the clone: 13 provisioning suites, 155 passed / 44 failed / 4 skipped (was: all skipped). The 44 failures were invisible behind the skips and are NOT from this lane (the two checked against the pre-1161 bodies fail identically): `permission denied for function platform.memo_clear` inside `platform.memo_ddl_forgets_the_shape()` (the wave3 lane guards, 7+), lock timeouts on the shared clone (5), `shape.invalid` validator drift in test specs (every_block `triggers_block`, `payload_kinds…`), iam.apply_rls/apply_governance_guard refusals in hash-partition and certify suites — OPEN, owner: the provisioning lane; `test_provisioning_lock_budget_live.py` still reads SUPABASE_BRANCH_DATABASE_URL directly (3 skips) — OPEN, same owner. Separately, `vridprune_a_person_is_asked_only_about_organizations_they_have_a_way_into.sql` holds an orphan duration-0 row (the file was renamed to `vridprune_the_record_set_…`, which ran) — harmless to the runner, noted for the database-workload-safety lane (D1g). detect_applied now compares bodies with comments stripped (so an MCP apply that dropped comments still counts as applied).
