...
**What remains under D170 is the JSON half and the kind question**, unchanged: a payload with no registered kind still has nothing to render progressively, and whether the podcast article agents earn a kind is still Arman's call. Note also that on the flashcard surfaces the envelope was only *half* the reason for the blank window — that run emits no chunks at all until it completes, which is a separate, chipped, aidream-side problem. Before concluding that a fix here restored a live window, confirm chunks are actually arriving (`renderBlockOrder.length` > 0 mid-run).
