...
- **D351 "RULE27-GATE"** — the release sweep applied an UNREHEARSED migration to production (2026-09-26). `rca5d_c_definer_doors_ask_each_record.sql` was committed 03:39Z with "rehearsal pending"; the 30-minute sweep (`scripts/release.sh` → `aidream db/apply_migrations.py --source matrx-frontend --target production`) applied it at 03:45Z; its body asked access row by row across 142 organizations and the sidebar hierarchy loader timed out (75 s) on every page load for `admin@admin.com` until `rca5d_e` at 04:36Z. **Root cause:** rule 27 (up → inverse → up on the dev clone) was a promise in a commit message — neither production applier could tell a rehearsed file from an unrehearsed one (the 2026-09-18 "rehearsal copy is not a gate" ruling, JUDGMENT.md §6a, removed the only check), and lanes dodged the sweep by hand-adding `-- chair-step:`. **Class:** any automated path (sweep, lane, runner) executing bytes at production that never passed rule 27. **Guard (both runners, one record format):** `db:rehearse` writes `migrations/rehearsed/<sha256 of raw bytes>.json` for the up AND its inverse only after all three legs (and the frontend's parity gate) pass; at `--target production` `pnpm db:apply` and aidream's runner refuse any file whose current bytes are not already ledgered and have no passing record — sweep HOLDS it (ERROR finding, rest carries on), named run exits 1, `--dry-run` does not soften it; one changed byte voids the record; the one door is `--unrehearsed-emergency "<reason>"` on a named run, written into the ledger row's `chair_step`. Libraries `scripts/lib/migration-rehearsal.ts` / `aidream db/migration_rehearsal.py`. Proof RED→GREEN: `pnpm db:apply --rehearsal-gate-self-test`, `aidream uv run pytest db/tests/test_migration_rehearsal.py`, and live 2026-09-26 on a planted probe (refused by both runners at production dry-run → `pnpm db:rehearse` on clone `jxhgzalwckuarngvsdyq` → admitted by both; never applied to production). **Also fixed, same incident:** the "8abf1a83 vs 55022a62 drift" on rca5d_c was never drift — 8abf1a83 is aidream's `sha256(sql.rstrip())` of the SAME bytes; `pnpm db:apply` compared only the raw form and told every reader "the file on disk is not the bytes that ran". It now accepts both forms (CS-30's class, already fixed in `check:migrations`); no ledger row was changed.
