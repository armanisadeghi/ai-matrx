...
**STILL OPEN on `files.pages`:** unfiltered `select count(*)` went 13.2 s → 8.4–8.7 s for an identity admitted few rows, i.e. **still over the 8 s cap** (it passes at 6.9–7.7 s for the grant reader and one super-admin; filtered reads, which every real surface uses, are ~100 ms). What remains is intrinsic — ~15,600 kernel invocations for 6,567 rows, ~2.6 s of it plpgsql's un-cacheable dynamic SQL. **Two closing moves need Arman's call, not an agent's:** merging `entity_row_access_attrs` with the parent-FK fetch (~0.9 s, but changes when a registry-drift error surfaces), or marking the kernel `PARALLEL SAFE` (blocked on `entity_row_access_attrs`'s `BEGIN … EXCEPTION`, and security-adjacent). The set-wise twin is **not** an option — already tried, measured and reverted in the parent migration.
