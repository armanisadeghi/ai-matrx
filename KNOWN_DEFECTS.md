...
- **Permissions — STILL OPEN (needs device testing).** Add `track.onended`/`onmute` listeners + a `navigator.permissions` change watcher to `micStream.ts` that **screams** (loud recovery doctrine) instead of silently re-acquiring; keep the AudioContext/stream warm across the Record/Agent/Live tab switches (don't let the Live tab's separate context tear the shared grant down); consider one canonical AudioContext rather than one per `useChunkedRecordAndTranscribe` instance to avoid iOS context exhaustion. Held for verification on a real device — this touches the never-lose-audio capture path and the failing scenario only reproduces on mobile.
