...
### D27 — Phantom association tokens: `agent_app`, `cx_message`, `cx_conversation`, `user_file`, `chat_block` — RESOLVED 2026-07-07
**Severity was: medium — created 2026-06-29 during the association-system type-safety hardening ([`features/scopes/FEATURE.md`](features/scopes/FEATURE.md) Change Log 2026-06-29). Closed via option (a): map at the write boundary + repoint the RPC reads.**

- **What it was.** Several entity-type tokens were used in code as association `source_type` / scope-tag `entity_type` but were **NOT registered** in `platform.entity_types`, whose enforced FK means any such write FK-violates (`23503`): `agent_app`, `cx_message`, `cx_conversation`, `user_file`, `chat_block`. On top, `get_task_associations` *read* those phantom tokens for its preview buckets, so the buckets were permanently empty.
- **The fix (all live-verified 2026-07-07).** Every phantom got a canonical disposition: `cx_message→message` (chat.message), `cx_conversation→conversation` (chat.conversation), `user_file→file` (files.files), `agent_app→app` (app.definition — confirmed in live `platform.entity_types`), `chat_block→message` (a "block" is a slice of a chat message; the edge id IS the message id, `metadata.block_index` carries the slice).
  - **One shared primitive:** `normalizeEntityToken()` + `LEGACY_ENTITY_TOKEN_ALIASES` in `features/scopes/service/associationGuards.ts`, applied in EVERY `associationsService` method before `checkToken` — an alias reaching the chokepoint is recovered AND screamed (`console.error`), so the class is structurally dead while stale callsites stay visible.
  - **Callsites emit canonical tokens directly:** `taskService.uploadTaskAttachment` (`file`), `buildTaskSeedFromMessage` (`message` + parent `conversation`, shared by `messageActionRegistry`/`contentActionRegistry`/rich-document save), `AgentAppHierarchyCascade` (`app`); war-room keeps its boundary map (`user_file→file`).
  - **DB read side:** `public.get_task_associations` repointed to canonical filters — `migrations/get_task_associations_canonical_tokens.sql` + `migrations/get_task_associations_canonical_read.sql`, applied + ledgered + live body verified. Return shape unchanged (`cx_messages`/`cx_conversations` keys kept for FE parity); `blocks` bucket kept as `[]` — do not resurrect `chat_block`.
  - **Data:** no phantom rows exist in `platform.associations` (FK made them impossible); `ctx_scope_assignments` is graveyarded — nothing to backfill.
...
### D26 — Working-document rebuild: legacy litter CLEARED (functional part RESOLVED 2026-06-29; STEP 3 RESOLVED 2026-07-02, verified 2026-07-07)
**Severity: low — created 2026-06-29 by the working-document end-to-end rebuild (FEATURE.md `features/agents/FEATURE.md` Change Log 2026-06-29; [[project_working_document_rebuild]]). Functional gap AND hygiene both closed; only the multi-attach design note below remains.**
...
- **STEP 3 RESOLVED (applied 2026-07-02 via `migrations/working_document_canonicalize_step3_drop_legacy.sql`; live-verified 2026-07-07).** `conversation_id` + `user_id` dropped from `workbench.working_documents` (provenance copied into `metadata.origin_conversation_id` before the drop; owner is `created_by`), and `chat.conversation_documents` moved to `graveyard` (never dropped). Verified live 2026-07-07: columns absent, junction in graveyard, zero `pg_proc` functions reference `conversation_documents`, ledger row present with matching checksum, `database.types.ts` + `entity-types.generated.ts` current, aidream HEAD (= origin/main = deployed) selects only `created_by, version, deleted_at`, and the writeback keeps working post-drop (8 doc updates in the trailing 3 days). Residual harmless litter: `cx-working-document.service.ts` row type still declares optional `conversation_id?`/`user_id?` fields that are now always undefined (metadata fallback covers) — trim opportunistically.
...
### D25 — Prompts-system deletion: residuals (dead chain CLEARED 2026-06-29; context-menu surfaces + dead preload CLOSED 2026-07-07; applet gate + ModelSettings stub remain)
...
- **`PromptEditorContextMenu` removed from 4 surfaces — RESTORED via v3 (verified 2026-07-07).** All 4 (`features/content-templates/admin/ContentTemplateManager.tsx`, `features/content-templates/components/SaveTemplateModal.tsx`, `components/admin/MarkdownTester.tsx`, `components/official/content-editor/ContentEditor.tsx` plain mode) wrap their textareas in the canonical `EditableContextMenu` (`features/context-menu-v3/`) with `getTextarea` wired — right-click content-block insert works again (v3 `handleContentBlockInsert`; data from `/api/agent-context-menu` → `agent.context_menu_view`, live `content-block` placement group confirmed). No new component was needed.
...
- **Dead data flow — REMOVED 2026-07-07.** `features/shell/components/DeferredShellData.tsx` no longer preloads `contextMenuCacheSlice` (v1 reader deleted) or `agentContextMenuCacheSlice` (its planned v2 "warm signal" read was never wired; v2/v3 fetch on open via `/api/agent-context-menu`). The orphaned `getSSRAgentShellData` helper (`utils/supabase/ssrShellData.ts`) was deleted with its only caller. Both slices are kept per the original disposition (currently writer/reader-less).
