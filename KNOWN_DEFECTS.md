...
- **`lib/services/system-prompts-service.ts` 404s at runtime (RESURFACED 2026-06-28 by `pnpm check:schema`).** It still does bare `supabase.from("system_prompts")` (lines 31, 303) + `.from("system_prompt_executions")` (328) — both tables are graveyarded (`public.system_prompts` is gone), so every read 404s. Live-imported via `hooks/useSystemPrompts.ts` → `components/dynamic/DynamicButtons.tsx`/`DynamicCards.tsx` (the same stubbed surfaces above). Fix: migrate the service+hook to `agent.definition` (agents replaced prompts, same UUIDs) or delete them — **not** repoint at graveyard (you don't read retired tables). `schema-truth-check` (`direct-from-schema`) screams until then.
