...
**What's open.**
- **Phase 1 — run-lifecycle producers — PARTIAL.** Live + verified on the canonical-ready tables (`migrations/run_lifecycle_activity_events.sql`: 6-arg `platform.log_activity` overload + generic `platform.emit_run_lifecycle()` trigger): **`files.file_rag_jobs`, `public.kg_sweep_run`** emit `run.completed`/`run.failed` with `actor_id` = owner. `public.ai_runs` is **not** a producer (`status` = active/archived/deleted = record state). The rest are blocked on the DB changeover retrofit — **attach the same trigger once each gets the missing column(s):**
  - need `organization_id`: `public.agent_run`, `public.pc_studio_runs`, `public.sch_run`
  - need `organization_id` **+ a `status` column**: `public.scrape_cycle_run`, `scraper.crawl_runs`
  - need `organization_id` **+ an owner column** (`user_id`/`created_by`): `public.studio_runs`, `public.page_extraction_runs`, `public.page_extraction_page_runs`, `public.derive_runs`, `legal.ingest_runs`
  - The trigger reads canonical names (`status`, `organization_id`, `user_id`, `id`); a retrofit using those names needs only a one-line `create trigger … emit_run_lifecycle()`. The event MUST carry `actor_id` = owner or owner-scoped webhooks can't match.
