...
**Triage 2026-06-29 — DEFERRED (accepted).** Recon confirmed routing scribe through `smartExecute` is NOT a drop-in: scribe's durable single-conversation model (studio slice + session-row persistence + roster, all keyed on `selectAssistantConversationId`) is orthogonal to the chat path's focus-registry split, and the split is also gated on `autoClear` (false for scribe). A naive swap orphans the conversation. Given the symptom is benign (draft preserved; the `console.error` IS the loud-recovery layer working as designed), a risky change to the delicate streaming core isn't justified now. Revisit if scribe gets a proper per-surface decouple.
