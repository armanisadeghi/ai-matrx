...
-- Post-1W storage is mandate.definition / mandate.provision / mandate.binding;
-- the retired agent.* tables are graveyarded and must never re-enter this RPC.
--
...
BEGIN
  ...
  user_b AS (
    SELECT DISTINCT ON (b.mandate_id)
           b.mandate_id AS ub_mandate_id, b.holder_id AS ub_agent_id,
           b.holder_version_id AS ub_version_id,
           (b.holder_version_id IS NULL) AS ub_use_latest,
    ...
    WHERE v_org IS NULL
      ...
      AND b.holder_type = 'agent' AND b.holder_id IS NOT NULL
    ORDER BY b.mandate_id, b.updated_at DESC, b.id
  ),
  -- 'mine': any of the caller's orgs may decide. 'org': only the named one.
  org_b AS (
    SELECT DISTINCT ON (b.mandate_id)
           b.mandate_id AS ob_mandate_id, b.holder_id AS ob_agent_id,
           b.holder_version_id AS ob_version_id,
           (b.holder_version_id IS NULL) AS ob_use_latest
    FROM mandate.binding b
  ...
  resolved AS (
    SELECT
      ...
      CASE WHEN ub.ub_mandate_id IS NOT NULL THEN ub.ub_agent_id
           ...
           ELSE CASE WHEN m.default_holder_type = 'agent'
                     THEN m.default_holder_id END END AS r_agent_id_raw,
      CASE WHEN ub.ub_mandate_id IS NOT NULL THEN ub.ub_version_id
           WHEN ob.ob_mandate_id IS NOT NULL THEN ob.ob_version_id
           ELSE m.default_holder_version_id END AS r_version_id,
      CASE WHEN ub.ub_mandate_id IS NOT NULL THEN coalesce(ub.ub_use_latest, false)
           ...
           ELSE (m.default_holder_version_id IS NULL) END AS r_use_latest,
